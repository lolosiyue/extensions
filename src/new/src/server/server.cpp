...
void Server::processNewConnection(ClientSocket *socket)
{
	...
	s.append(QString(":%1").arg(playerCount));
...
void Server::checkUpnpAndListServer()
{
	...
	} else if(Config.value("serverconfig/addtolistserver",false).toBool())
		addToListServer();
}
...
void Server::upnpFinished()
{
	...
	if(Config.value("serverconfig/addtolistserver",false).toBool())
		addToListServer();
}
