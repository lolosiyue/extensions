    ...
    QString card_str, QString arg, QString arg2, QString arg3, QString arg4, QString arg5)
{
    ...
    if (Config.value("recorder/eventsave").toBool()){
    ...
    if (!from_general.isEmpty())
        from = bold(ClientInstance->getPlayerName(from_general), Qt::green);
    ...
    if(card_str.isEmpty()){}
        ...
        append(log);
        return;*/
    }else if (type.startsWith("#UseCard") && !from_general.isEmpty()) {
        ...
        const Card *card = Card::Parse(card_str);
        if (card) log.replace("%card", bold(card->getLogName(), Qt::yellow));
		else{
			QStringList log_name;
			foreach (QString str, card_str.split("+")) {
				card = Sanguosha->getCard(str.toInt());
    ...
    QString final_log = append(QString("<font color='%2'>%1</font>").arg(log).arg(Config.TextEditColor.name()));
