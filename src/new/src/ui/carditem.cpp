...
void CardItem::setCard(const Card *card)
{
    if (!card) return;
	m_cardId = card->getId();
    if (card->isKindOf("Xumou")) return;
	const Card *engineCard = Sanguosha->getEngineCard(m_cardId);
	if (engineCard->objectName().contains("_zhizhe_")) engineCard = card;
...
void CardItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
    ...
    if (_m_isUnknownGeneral)
        painter->drawPixmap(G_COMMON_LAYOUT.m_cardMainArea, G_ROOM_SKIN.getPixmap("generalCardBack", "", true));
    else{
        if (zhizhe)
