...
{
	//card_item->setEnabled(ClientInstance->getStatus()==Client::Playing&&card_item->getCard()->isAvailable(Self));
...
{
    QMenu *menu = _m_renpile_menu;/*
    menu->clear();
    menu->setTitle("��");
	bool has = false;
    foreach (QVariant qv, Self->tag["ren_pile"].toList()) {
        const Card *c = Sanguosha->getCard(qv.toInt());
        QAction *action = menu->addAction(c->getLogName());
		action->setData((int)qv.toInt());
		connect(action, SIGNAL(triggered()), this, SLOT(changeShefuState()));
		has = true;
    }
	if (!has){
...
void Dashboard::updatePending()
{
    ...
    foreach (CardItem *item, m_handCards) {
        if (!item->isSelected() || pendings.isEmpty())
            item->setEnabled(!item->getCard()->hasFlag("using")&&view_as_skill->viewFilter(pended,item->getCard()));
        if (!item->isEnabled())
            animations->effectOut(item);
    }
    ...
    for (int i = 0; i < S_EQUIP_AREA_LENGTH; i++) {
        ...
        if (equip) {
            if(!equip->isMarked())
				equip->setMarkable(!equip->getCard()->hasFlag("using")&&view_as_skill->viewFilter(pended,equip->getCard()));
