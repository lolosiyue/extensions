...
void SmallTuxiCard::onEffect(CardEffectStruct &effect) const
{
...
public:
    ...
    bool triggerable(const ServerPlayer *target) const
    {
        return target != nullptr && target->getGeneralName() == "zhangliao"
    ...
    bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        case DrawNCards: {
            if (player->getPhase() == Player::Draw) {
				...
				if (draw.reason=="draw_phase"&&!room->getTag("BurnWuchao").toBool()) {
                    draw.num--;
        ...
        case Damaged: {
            if (room->getTag("BurnWuchao").toBool()) return false;
            ...
            if (player->getGeneralName() == "yuanshao" && damage.nature == DamageStruct::Fire
                ...
                foreach (QString name, tos) {
                    ...
                    if (to == nullptr || to->containsTrick("supply_shortage"))
                        continue;
