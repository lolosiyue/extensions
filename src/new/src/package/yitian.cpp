...
public:
	...
	bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
	{
		if (triggerEvent == DamageComplete) {
			if (player!=room->getCurrent()&&WeaponSkill::triggerable(player))
...
public:
	...
	bool onPhaseChange(ServerPlayer *caochong, Room *room) const
	{
		if (caochong->getPhase() == Player::Finish && caochong->getHandcardNum() > 13) {
			room->broadcastSkillInvoke(objectName());
			room->notifySkillInvoked(caochong, objectName());
...
public:
	...
	void onDamaged(ServerPlayer *target, const DamageStruct &damage) const
	{
		...
		if(target->hasSkill(this)){
			...
			foreach (ServerPlayer *p, room->getOtherPlayers(target)) {
				if (p->getMark("@tied")>0&&p->hasSkill(this)) {
					bool has = false;
					for (int i = 0; i < damage.damage; i++) {
...
public:
	...
	{
		if (target->getHp() > 0 && target->hasSkill(this) && !target->getWeapon())
			return target->getHp();
		return -1;
	}
};
