...
public:
    ...
    bool trigger(TriggerEvent, Room *room, ServerPlayer *zhangliao, QVariant &data) const
    {
		...
		if (draw.reason!="draw_phase"||!zhangliao->hasFlag("koftuxi")) return false;
        zhangliao->setFlags("-koftuxi");

        foreach (ServerPlayer *p, room->getOtherPlayers(zhangliao)) {
            if (p->hasFlag("KOFTuxiTarget")) {
                p->setFlags("-KOFTuxiTarget");
				int card_id = room->askForCardChosen(zhangliao, p, "h", "koftuxi");
    ...
    bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &) const
    {
        if (player->getPhase() == Player::Finish) {
			foreach (ServerPlayer *hansui, room->getOtherPlayers(player)) {
				if (hansui->isNude()||!hansui->hasSkill(this)) continue;
