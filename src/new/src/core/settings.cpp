...
QString Settings::getAndroidConfigPath()
{
    ...
    QString standardPath = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + "/config.ini";
    return standardPath;
}
...
void Settings::reinitializeConfigFile()
{
    ...
    if (QFile::exists(userConfig)) {
        ...
        QStringList keys = userSettings.allKeys();
        for (const QString &key : keys) {
            QVariant userValue = userSettings.value(key);
            setValue(key, userValue);
        }
...
void Settings::init()
{
    ...
    QStringList roles_ban, kof_ban, doudizhu_ban, happy2v2_ban, hulao_ban, xmode_ban, god_ban, bossmode_ban, basara_ban, hegemony_ban, pairs_ban;
    ...
    banlist = value("Banlist/05_ol").toStringList();
    if (banlist.isEmpty()) {
        foreach (const QString &ban_general, god_ban)
            banlist << ban_general;

        setValue("Banlist/05_ol", banlist);
    }

    banlist = value("Banlist/06_ol").toStringList();
    if (banlist.isEmpty()) {
        foreach (const QString &ban_general, god_ban)
            banlist << ban_general;

        setValue("Banlist/06_ol", banlist);
    }
