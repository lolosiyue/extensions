...
function askYishi(ysdata)
	...
	for _, p in sgs.qlist(log.to) do
		...
		if type(dc) == "userdata" then
			...
			for _, id in sgs.qlist(dc:getSubcards()) do
				...
				x = math.max(x, ysdata.color_num[cs])
				table.insert(ysdata.colors, cs)
	...
	local data =
		sgs.QVariant("yishiresult:" .. ysdata.reason .. ":" .. ysdata.from:objectName() .. ":" .. table.concat(ysdata.tos, "+") .. ":" .. table.concat(ysdata.ids, "+") .. ":" .. ysdata.result)
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardsMoveOneTime then
			...
			if
				...
				and move.reason.m_skillName ~= "keqijulian"
				and move.to
...
keqitongjueex = sgs.CreateProhibitSkill {
	...
	is_prohibited = function(self, from, to, card)
		return from:getMark(to:objectName() .. "usekeqitongjue-Clear") > 0 and not card:isKindOf("SkillCard")
	end,
}
...
keqiyingmen = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		for _, name in sgs.list(RandomList(sgs.Sanguosha:getLimitedGeneralNames())) do
			if table.contains(Generals, name) or table.contains(aps, name) then
				continue
			end
			...
			room:addPlayerMark(player, "&keqiyingmenGenerals")
			room:getThread():delay(500)
			if #Generals > 3 then
				break
			end
		end
		for _, s in sgs.qlist(qiPingSkills(player)) do
			if s:inherits("TriggerSkill") then
				room:getThread():addTriggerSkill(sgs.Sanguosha:getTriggerSkill(s:objectName()))
			else
				for _, rs in sgs.qlist(sgs.Sanguosha:getRelatedSkills(s:objectName())) do
					if rs:inherits("TriggerSkill") then
						room:getThread():addTriggerSkill(sgs.Sanguosha:getTriggerSkill(rs:objectName()))
					end
				end
			end
		end
...
local function qiPingSkills(player)
	...
	for _, g in ipairs(player:property("keqiyingmenGenerals"):toString():split("+")) do
		...
		if g then
			for _, s in sgs.qlist(g:getVisibleSkillList()) do
				if s:isLordSkill() or s:isChangeSkill() or s:isLimitedSkill() or s:isHideSkill() or s:isShiMingSkill() or s:getFrequency(player) == sgs.Skill_Wake then
					continue
				end
				table.insert(sk, s)
			end
...
keqipingjianCard = sgs.CreateSkillCard {
	...
	about_to_use = function(self, room, use)
		...
		for _, s in sgs.list(qiPingSkills(use.from)) do
			...
			if s:inherits("ViewAsSkill") and sgs.Sanguosha:getViewAsSkill(s:objectName()):isEnabledAtPlay(use.from) then
				table.insert(qipjsks, s:objectName())
			end
			if s:inherits("TriggerSkill") then
				local ts = sgs.Sanguosha:getTriggerSkill(s:objectName())
				ts = ts:getViewAsSkill()
				if ts and ts:isEnabledAtPlay(use.from) then
					table.insert(qipjsks, ts:objectName())
				end
			end
		end
...
keqipingjianvs = sgs.CreateViewAsSkill {
	...
	n = 999,
	view_filter = function(self, selected, to_select)
		if sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_PLAY and sgs.Self:getMark("keqipingjianSkill-PlayClear") < 1 then
			return
		end
	...
	enabled_at_play = function(self, player)
		for _, s in sgs.list(qiPingSkills(player)) do
			...
			local vs = sgs.Sanguosha:getViewAsSkill(s:objectName())
			if vs and vs:isEnabledAtPlay(player) then
				return true
			end
		end
		return false
...
keqipingjian = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if player:hasSkill(self, true) then
			if event == sgs.ChoiceMade then
				...
				if cm:startsWith("notifyInvoked:") then
					player:addMark("keqipingjianUse" .. cm:split(":")[2])
				end
			elseif event == sgs.CardFinished then
				room:setPlayerMark(player, "keqipingjianSkill-PlayClear", 0)
				local use = data:toCardUse()
				if use.card:getSkillName(false) ~= "" and player:hasSkill(self) then
					for _, s in sgs.list(qiPingSkills(player)) do
						if table.contains(use.card:getSkillNames(), s:objectName()) and player:property("pingjian_triggerskill"):toString() ~= s:objectName() then
							room:broadcastSkillInvoke(self:objectName())
							...
							if sgs.Sanguosha:getGeneral(choice):hasSkill(s:objectName()) then
								player:drawCards(1, self:objectName())
							end
							break
						end
		...
		for _, owner in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
			for _, s in ipairs(qiPingSkills(owner)) do
				local ts = sgs.Sanguosha:getTriggerSkill(s:objectName())
				if ts and ts:hasEvent(event) then
					local has = false
					owner:setMark("keqipingjianUse" .. ts:objectName(), 0)
					room:setPlayerProperty(owner, "pingjian_triggerskill", ToData(s:objectName()))
					if ts:triggerable(player, room, event) then
						has = ts:trigger(event, room, player, data)
					end
					...
					if owner:getMark("keqipingjianUse" .. ts:objectName()) > 0 then
						...
						if sgs.Sanguosha:getGeneral(choice):hasSkill(ts:objectName()) then
...
keqiyizhengCard = sgs.CreateSkillCard {
	...
	will_throw = false,
	filter = function(self, targets, to_select, player)
		return #targets == 0 and player:canPindian(to_select, true) and to_select:getHandcardNum() > player:getHandcardNum()
	end,
	on_use = function(self, room, player, targets)
		local target = targets[1]
...
keqifusong = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if not target:hasSkill("keqifengzi", true) then
			table.insert(skills, "keqifengzi")
		end
		if not target:hasSkill("keqijizhanw", true) then
			table.insert(skills, "keqijizhanw")
		end
		if #skills == 0 then
			return false
		end
...
kechengduxingCard = sgs.CreateSkillCard {
	...
	about_to_use = function(self, room, use)
		...
		for _, p in sgs.qlist(use.to) do
			...
			for _, h in sgs.qlist(p:getHandcards()) do
				if h:getSkillName() == self:getSkillName() then
					cs:append(h)
				end
			end
...
kechengguanjue = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardResponded then
			...
			if table.contains(ss, response.m_card:getSuitString()) then
				return
			end
		...
		if event == sgs.CardUsed then
			...
			local ss = player:property("kechengguanjueSuits"):toString():split(",")
			if table.contains(ss, use.card:getSuitString()) then
				return
			end
	...
	limit_pattern = function(self, player, card)
		for _, p in sgs.qlist(player:getAliveSiblings()) do
			if p:hasSkill("kechengguanjue") then
				...
				if ss ~= "" and string.find(ss, card:getSuitString()) then
					return ".|" .. ss
				end
			end
		end
...
kechengnianenCard = sgs.CreateSkillCard {
	...
	feasible = function(self, targets, from)
		if sgs.Sanguosha:getCurrentCardUseReason() ~= sgs.CardUseStruct_CARD_USE_REASON_RESPONSE and self:getUserString() ~= "" then
			local us = self:getUserString():split("+")
...
kechengqingjiaoCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, source)
		...
		if to_select:getHandcardNum() > source:getHandcardNum() and source:getMark("&useqingjiaotxzf-Clear") < 1 then
		...
		elseif to_select:getHandcardNum() < source:getHandcardNum() and source:getMark("&useqingjiaochdj-Clear") < 1 then
	...
	about_to_use = function(self, room, use)
		if use.to:first():getHandcardNum() > use.from:getHandcardNum() and use.from:getMark("&useqingjiaotxzf-Clear") < 1 then
			local txzf = sgs.Sanguosha:cloneCard("yj_tuixinzhifu")
			txzf:setSkillName("kechengqingjiao")
			txzf:addSubcard(self)
			txzf:deleteLater()
			use.card = txzf
		elseif use.to:first():getHandcardNum() < use.from:getHandcardNum() and use.from:getMark("&useqingjiaochdj-Clear") < 1 then
			local chdj = sgs.Sanguosha:cloneCard("yj_chenhuodajie")
			chdj:setSkillName("kechengqingjiao")
			chdj:addSubcard(self)
			chdj:deleteLater()
		...
		self:cardOnUse(room, use)
		if use.card:objectName() == "yj_tuixinzhifu" then
			room:setPlayerMark(use.from, "&useqingjiaotxzf-Clear", 1)
		elseif use.card:objectName() == "yj_chenhuodajie" then
	...
	on_use = function(self, room, source, targets)
		...
		if target:getHandcardNum() > source:getHandcardNum() then
			...
			local txzf = sgs.Sanguosha:cloneCard("yj_tuixinzhifu", card:getSuit(), card:getNumber())
			txzf:setSkillName("kechengqingjiao")
		...
		elseif (target:getHandcardNum() < source:getHandcardNum()) and (usethis == 0) then
			local card = sgs.Sanguosha:getCard(self:getSubcards():first())
			local chdj = sgs.Sanguosha:cloneCard("yj_tuixinzhifu", card:getSuit(), card:getNumber())
			chdj:setSkillName("kechengqingjiao")
...
kechengqingxiCard = sgs.CreateSkillCard {
	...
	mute = true,
	filter = function(self, targets, to_select, player)
		return #targets < 1 and player:getHandcardNum() - to_select:getHandcardNum() == self:subcardsLength() and to_select:getMark("beusekechengqingxi-PlayClear") < 1
	end,
	on_use = function(self, room, player, targets)
		local target = targets[1]
...
kechengjinmieCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		...
		slash:deleteLater()
		return (#targets == 0) and (to_select:getHandcardNum() > player:getHandcardNum()) and player:canSlash(to_select, slash, false)
	end,
	on_use = function(self, room, player, targets)
		local target = targets[1]
		local slash = sgs.Sanguosha:cloneCard("fire_slash")
...
kechengzhangdengCard = sgs.CreateSkillCard {
	...
	on_validate = function(self, use)
		...
		local tos = sgs.SPlayerList()
		for _, p in sgs.list(room:getAlivePlayers()) do
			if p:hasSkill(self:getSkillName()) then
				tos:append(p)
			end
		end
		if tos:isEmpty() then
...
kechengjixiangCard = sgs.CreateSkillCard {
	...
	on_validate = function(self, use)
		...
		for _, p in sgs.list(room:getOtherPlayers(use.from)) do
			if p:hasSkill(self:getSkillName()) and p:hasFlag("CurrentPlayer") then
				tos:append(p)
			end
		end
		...
		for _, pn in sgs.list(self:getUserString():split("+")) do
			if tos:getMark(pn .. "kechengjixiang-Clear") < 1 then
				table.insert(us, pn)
			end
		end
		if #us < 1 then
			...
			if p:hasSkill("kechengjixiang") and p:hasFlag("CurrentPlayer") then
				for _, pn in sgs.list(patterns()) do
					...
					if dc then
						...
						dc:deleteLater()
						if dc:getTypeId() == 1 and dc:isAvailable(player) then
							return true
						end
					end
				end
...
kechengchengxianCard = sgs.CreateSkillCard {
	...
	about_to_use = function(self, room, use)
		...
		if card:isKindOf("AOE") then
			orinum = orinum - 1
		elseif card:targetFixed() and card:isSingleTargetCard() then
			orinum = 1
		end
		if orinum < 1 then
			return
		...
		for _, cn in ipairs(patterns()) do
			if use.from:getMark(cn .. "kechengchengxian-Clear") > 0 or table.contains(choices, cn) then
				continue
			end
			local dc = dummyCard(cn, "kechengchengxian")
			dc:addSubcard(self)
			if dc:isNDTrick() and dc:isAvailable(use.from) then
				local trannum = room:getCardTargets(use.from, dc):length()
				if dc:isKindOf("AOE") then
					trannum = trannum - 1
				elseif dc:targetFixed() and dc:isSingleTargetCard() then
					trannum = 1
				end
				if trannum == orinum then
		...
		room:setPlayerProperty(use.from, "kechengchengxianCn", ToData(choice))
...
kechengchengxian = sgs.CreateViewAsSkill {
	...
	view_as = function(self, cards)
		if sgs.Sanguosha:getCurrentCardUsePattern() == "@@kechengchengxian" then
			local transcard = sgs.Sanguosha:cloneCard(sgs.Self:property("kechengchengxianCn"):toString())
	...
	enabled_at_play = function(self, player)
		return player:usedTimes("kechengchengxianCard") - player:getMark("exusetimekechengchengxian-Clear") < 2
	end,
}
...
local function zhuanJfNames(player)
	...
	for _, p in sgs.qlist(player:getAliveSiblings(true)) do
		for _, s in sgs.qlist(p:getVisibleSkillList()) do
			...
			ption = ption .. s:getDescription(p)
		end
	end
	local names = {}
	for id = 0, sgs.Sanguosha:getCardCount() - 1 do
		local c = sgs.Sanguosha:getEngineCard(id)
		if c:getTypeId() > 2 or table.contains(names, c:objectName()) or player:getMark(c:getType() .. "kezhuanzhenfeng-PlayClear") > 0 then
			continue
		end
		if (c:isNDTrick() or c:isKindOf("BasicCard")) and (not c:isKindOf("kezhuan_ying")) and string.find(ption, "【" .. sgs.Sanguosha:translate(c:objectName()) .. "】") then
...
kezhuanzhenfengvs = sgs.CreateViewAsSkill {
	...
	view_as = function(self, cards)
		...
		if pattern == "@@kezhuanzhenfeng" then
			local dc = sgs.Self:getMark("kezhuanzhenfeng_id")
			dc = sgs.Sanguosha:getEngineCard(dc)
	...
	enabled_at_response = function(self, player, pattern)
		if pattern == "@@kezhuanzhenfeng" then
			return true
		elseif sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_RESPONSE or player:getPhase() ~= sgs.Player_Play then
			return
		end
	...
	enabled_at_nullification = function(self, player)
		return player:getPhase() == sgs.Player_Play and player:getMark("trickkezhuanzhenfeng-PlayClear") < 1
	end,
	enabled_at_play = function(self, player)
...
kezhuanxushiCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, player, targets)
		...
		for _, id in sgs.qlist(sgs.Sanguosha:getRandomCards(true)) do
			if sgs.Sanguosha:getEngineCard(id):isKindOf("Ying") and room:getCardOwner(id) == nil then
				dummy:addSubcard(id)
				if dummy:subcardsLength() >= n * 2 then
					break
				end
			end
		end
...
kezhuanfenjianCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		...
		duel:deleteLater()
		return #targets < 1 and to_select:objectName() ~= player:objectName() and not player:isProhibited(to_select, duel)
	end,
	on_use = function(self, room, player, targets)
		local duel = sgs.Sanguosha:cloneCard("duel")
		duel:setSkillName("kezhuanfenjian")
...
kezhuanguijiCard = sgs.CreateSkillCard {
	...
	will_throw = false,
	filter = function(self, targets, to_select, player)
		return #targets < 1 and (to_select:getGender() == sgs.General_Male) and (to_select:getHandcardNum() < player:getHandcardNum())
	end,
	on_use = function(self, room, player, targets)
		local target = targets[1]
		...
		local exchangeMove = sgs.CardsMoveList()
		local move1 =
			sgs.CardsMoveStruct(player:handCards(), target, sgs.Player_PlaceHand, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_SWAP, player:objectName(), target:objectName(), "kezhuanguiji", ""))
		local move2 =
			sgs.CardsMoveStruct(target:handCards(), player, sgs.Player_PlaceHand, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_SWAP, target:objectName(), player:objectName(), "kezhuanguiji", ""))
		exchangeMove:append(move1)
		exchangeMove:append(move2)
		room:moveCardsAtomic(exchangeMove, false)
		player:setFlags("-kezhuanguijiTarget")
...
kezhuanjiaohaoCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, liubei)
		if #targets ~= 0 or (to_select:objectName() == liubei:objectName()) or (not to_select:hasSkill("kezhuanjiaohao")) then
			return false
		end
		local card = sgs.Sanguosha:getCard(self:getSubcards():first())
		...
		if event == sgs.EventPhaseStart then
			...
			if (player:getPhase() == sgs.Player_Start) and player:hasSkill(self:objectName()) then
				...
				for _, id in sgs.qlist(sgs.Sanguosha:getRandomCards(true)) do
					if sgs.Sanguosha:getEngineCard(id):isKindOf("Ying") and room:getCardOwner(id) == nil then
						dummy:addSubcard(id)
						if dummy:subcardsLength() >= num then
							break
						end
					end
				end
	...
	about_to_use = function(self, room, use)
		...
		for _, id in sgs.qlist(sgs.Sanguosha:getRandomCards()) do
			...
			if tcard:isDamageCard() then
				if table.contains(choices, tcard:objectName()) then
					continue
				end
				local transcard = sgs.Sanguosha:cloneCard(tcard:objectName())
				transcard:setSkillName("kezhuancuifeng")
				--transcard:addSubcard(self)
				if not transcard:isAvailable(use.from) then
					continue
				end
				if (not tcard:isKindOf("DelayedTrick")) and (tcard:isSingleTargetCard() or ((use.from:aliveCount() == 2) and (tcard:isKindOf("AOE")))) then
...
kezhuandengnanCard = sgs.CreateSkillCard {
	...
	about_to_use = function(self, room, use)
		...
		for _, id in sgs.qlist(sgs.Sanguosha:getRandomCards()) do
			local tcard = sgs.Sanguosha:getEngineCard(id)
			if not tcard:isDamageCard() and tcard:isNDTrick() then
				if table.contains(choices, tcard:objectName()) then
					continue
				end
				local transcard = sgs.Sanguosha:cloneCard(tcard:objectName())
...
kezhuanmanjuanVsCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, from)
		...
		if pattern == "@@kezhuanmanjuan" then
			...
			for i = 0, sgs.Sanguosha:getCardCount() - 1 do
				local c = sgs.Sanguosha:getEngineCard(i)
				if from:getMark(i .. "manjuanPile-Clear") > 0 and from:getMark(c:getNumber() .. "manjuanNumber-Clear") < 1 and not from:isLocked(c) then
	...
	on_validate = function(self, use)
		...
		local pattern = self:getUserString()
		if pattern == "@@kezhuanmanjuan" then
			use.from:skillInvoked("kezhuanmanjuan") --播放配音
			...
			for i = 0, sgs.Sanguosha:getCardCount() - 1 do
				local c = sgs.Sanguosha:getEngineCard(i)
				if use.from:getMark(i .. "manjuanPile-Clear") > 0 and use.from:getMark(c:getNumber() .. "manjuanNumber-Clear") < 1 and not use.from:isLocked(c) then
					local pn = c:isKindOf("Slash") and "slash" or c:objectName()
					if string.find(pattern, pn) and (use.from:canUse(c, use.to) or c:targetFixed()) then
						ids:append(i)
					end
				end
			end
	...
	on_validate_in_response = function(self, from)
		...
		local pattern = self:getUserString()
		if pattern == "@@kezhuanmanjuan" then
			from:skillInvoked("kezhuanmanjuan") --播放配音
			local c = sgs.Sanguosha:getCard(from:getMark("kezhuanmanjuan_id"))
			room:addPlayerMark(from, c:getNumber() .. "manjuanNumber-Clear")
			return c
		else
			local ids = sgs.IntList()
			local hm = sgs.Sanguosha:getCurrentCardUseReason() ~= sgs.CardUseStruct_CARD_USE_REASON_RESPONSE
			for i = 0, sgs.Sanguosha:getCardCount() - 1 do
...
kezhuanmanjuanCard = sgs.CreateSkillCard {
	...
	about_to_use = function(self, room, use)
		...
		for i = 0, sgs.Sanguosha:getCardCount() - 1 do
			local c = sgs.Sanguosha:getCard(i)
			if use.from:getMark(i .. "manjuanPile-Clear") > 0 and use.from:getMark(c:getNumber() .. "manjuanNumber-Clear") < 1 and c:isAvailable(use.from) then
				ids:append(i)
			end
		end
		room:fillAG(ids, use.from)
...
kezhuanyangmingCard = sgs.CreateSkillCard {
	...
	will_throw = false,
	filter = function(self, targets, to_select, player)
		return #targets == 0 and (to_select:objectName() ~= player:objectName()) and (player:canPindian(to_select, true))
	end,
	on_use = function(self, room, player, targets)
		local target = targets[1]
		while player:isAlive() and target:isAlive() and player:canPindian(target) do
			if player:pindian(target, self:getSkillName()) then
				if player:isAlive() and target:isAlive() and player:canPindian(target) and player:askForSkillInvoke(self:getSkillName(), ToData("kezhuanyangming-jixu:" .. target:objectName())) then
				else
					break
				end
			else
	...
	on_trigger = function(self, event, player, data)
		...
		if (event == sgs.EventLoseSkill) and data:toSkillChange().skillName == "kezhuanninghan" then
			...
			for _, p in sgs.qlist(room:getAllPlayers()) do
				...
				local cs = sgs.CardList()
				for _, h in sgs.qlist(p:getHandcards()) do
					if h:getSkillName() == "kezhuanninghan" then
						cs:append(h)
					end
				end
				room:filterCards(p, cs, true)
...
kezhuanninghanbuff = sgs.CreateFilterSkill {
	...
	view_filter = function(self, to_select)
		local room = sgs.Sanguosha:currentRoom()
		return room:getCardPlace(to_select:getEffectiveId()) == sgs.Player_PlaceHand and to_select:getSuit() == sgs.Card_Club and to_select:isKindOf("Slash")
	end,
	view_as = function(self, original)
		local slash = sgs.Sanguosha:cloneCard("ice_slash", original:getSuit(), original:getNumber())
...
kezhuanhuozhongCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, source)
		...
		return not to_select:containsTrick("supply_shortage") and to_select:objectName() == source:objectName() and to_select:hasJudgeArea() and not source:isProhibited(to_select, shortage)
	end,
	on_use = function(self, room, source, targets)
		local target = targets[1]
...
sgs.LoadTranslationTable {
	...
	["kezhuanyangming"] = "养名",
	[":kezhuanyangming"] = "出牌阶段限一次，你可以拼点：若对方赢，其摸X张牌（X为其本阶段拼点没赢的次数）且你回复1点体力，否则你可以对其重复此流程。",
...
kehechushiCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, player, targets)
		...
		ys.effect = function(ys_data)
			if ys_data.result == "red" then
				local aps = SPlayerList(player, zhugong)
				room:sortByActionOrder(aps)
				while player:isAlive() or zhugong:isAlive() do
					room:drawCards(aps, 1, self:getSkillName())
					if player:getHandcardNum() + zhugong:getHandcardNum() >= 7 then
						break
					end
				end
			elseif ys_data.result == "black" then
...
kehejinfaCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, player, targets)
		...
		for _, p in sgs.list(ys.tos) do
			if p ~= player:objectName() and (ys.to2color[p]:contains(ys.to2color[player:objectName()]) or ys.to2color[player:objectName()]:contains(ys.to2color[p])) then
				return
			end
		end
...
kehefumouex = sgs.CreateProhibitSkill {
	name = "#kehefumouex",
	is_prohibited = function(self, from, to, card)
		return card:getSkillName() == "kehefumou" and card:isKindOf("Chuqibuyi") and to and to:getMark("&kehefumou+red") + to:getMark("&kehefumou+black") < 1 and from:hasSkill("kehefumou")
	end,
}
...
kehetuigu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardsMoveOneTime then
			...
			if
				move.to_place == sgs.Player_PlaceHand
				and player:objectName() == move.reason.m_playerId
			...
			then
				...
				room:setPlayerCardLimitation(to, "use", table.concat(ids, ","), true)
			end
		end
	end,
}
...
kehejingjuCard = sgs.CreateSkillCard {
	...
	on_validate = function(self, use)
		...
		for _, p in sgs.list(room:getOtherPlayers(use.from)) do
			for _, j in sgs.list(p:getJudgingArea()) do
				if use.from:containsTrick(j:objectName()) then
					continue
				end
				tos:append(p)
				break
	...
	on_validate_in_response = function(self, from)
		...
		for _, p in sgs.list(room:getOtherPlayers(from)) do
			for _, j in sgs.list(p:getJudgingArea()) do
				if from:containsTrick(j:objectName()) then
					continue
				end
				tos:append(p)
				break
		...
		if event == sgs.Damage then
			...
			elseif str:startsWith("askyishicard:") then
				...
				if player:hasSkill(self) then
					...
					for _, p in sgs.qlist(room:getAllPlayers()) do
						if table.contains(strs[4]:split("+"), p:objectName()) and p:getMark("&kehebazheng+#" .. player:objectName() .. "-Clear") > 0 then
	...
	on_trigger = function(self, event, player, data)
		...
		while jcs:length() > 0 do
			...
			if string.find(jc:objectName(), "kehexumou") then
				...
				if xmc:isKindOf("Slash") then
					m_name = "slash"
				end
				if table.contains(xmcn, m_name) then
					continue
				end
				local cs = sgs.CardList()
				cs:append(xmc)
...
function xumouCard(player, card)
	...
	local ids = { card }
	if type(card) ~= "number" and card:isVirtualCard() then
	...
	for _, id in sgs.list(ids) do
		...
		ids2:append(id:getEffectiveId())
		n = n + 1
		cs:append(sgs.Sanguosha:cloneCard("__kehexumou" .. n, 6, 0))
	end
	if ids2:isEmpty() then
		return false
	end
	room:moveCardsAtomic(sgs.CardsMoveStruct(ids2, player, sgs.Player_PlaceTable, sgs.CardMoveReason()), false)
	for i, id in sgs.qlist(ids2) do
		local wc = sgs.Sanguosha:getWrappedCard(id)
		wc:takeOver(cs:at(i))
		room:broadcastUpdateCard(room:getPlayers(), id, wc)
	end
	room:moveCardsAtomic(sgs.CardsMoveStruct(ids2, player, sgs.Player_PlaceDelayedTrick, sgs.CardMoveReason()), true)
	return true
end
...
kehefuhaiCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, player, targets)
		...
		if room:askForChoice(player, "kehefuhai", "ssz+nsz") == "ssz" then
			tos = sgs.reverse(tos)
		end
		local c, n
		...
		for i, p in ipairs(tos) do
			...
			if i == 1 then
				if c:getNumber() > a then
					b = 1
				elseif c:getNumber() < a then
					b = -1
				else
					break
				end
				x = x + 1
			elseif b == 1 and c:getNumber() > a or b == -1 and c:getNumber() < a then
				x = x + 1
			else
				break
			end
	...
	on_use = function(self, room, player, targets)
		...
		for _, p in sgs.qlist(room:getOtherPlayers(player)) do
			if target:inMyAttackRange(p) and not p:isNude() then
				local dis = room:askForDiscard(p, "kehechiying", 1, 1, false, true)
				if dis then
					alldis:append(dis:getEffectiveId())
				end
			end
		end
		room:getThread():delay(800)
		...
		if event == sgs.CardFinished then
			...
			if table.contains(use.card:getSkillNames(), "kehedanxin") then
				for _, p in sgs.qlist(use.to) do
					...
					p:setMark(player:objectName() .. "kehedanxin-Clear", 0)
				end
			end
		end
		if event == sgs.CardOnEffect then
...
kehejiuxianCard = sgs.CreateSkillCard {
	...
	on_validate = function(self, cardUse)
		...
		source:skillInvoked("kehejiuxian", 0)
...
sgs.LoadTranslationTable {
	...
	["__kehexumou9"] = "蓄谋",
	["#kehexumou"] = "蓄谋",
	[":__kehexumou0"] = "判定阶段，你可以使用蓄谋牌（每种牌名限一次）",
	[":__kehexumou1"] = "判定阶段，你可以使用蓄谋牌（每种牌名限一次）",
	[":__kehexumou2"] = "判定阶段，你可以使用蓄谋牌（每种牌名限一次）",
	[":__kehexumou3"] = "判定阶段，你可以使用蓄谋牌（每种牌名限一次）",
	[":__kehexumou4"] = "判定阶段，你可以使用蓄谋牌（每种牌名限一次）",
	[":__kehexumou5"] = "判定阶段，你可以使用蓄谋牌（每种牌名限一次）",
	[":__kehexumou6"] = "判定阶段，你可以使用蓄谋牌（每种牌名限一次）",
	[":__kehexumou7"] = "判定阶段，你可以使用蓄谋牌（每种牌名限一次）",
	[":__kehexumou8"] = "判定阶段，你可以使用蓄谋牌（每种牌名限一次）",
	[":__kehexumou9"] = "判定阶段，你可以使用蓄谋牌（每种牌名限一次）",
	...
	["keheyoujinnum"] = "诱进点数",
	[":keheyoujin"] = "<font color='green'><b>出牌阶段开始时，</b></font>你可以拼点，然后你与对方本回合不能使用或打出点数小于本次各自拼点牌的牌，且赢的角色视为对没赢的角色使用一张【杀】。",
...
keshuaiyanshaslashex = sgs.CreateProhibitSkill {
	...
	is_prohibited = function(self, from, to, card)
		...
		return card:getSkillName() == "keshuaiyansha" and card:isKindOf("Slash") and to and to:getMark("&keshuaiyansha") < 1
	end,
}
...
keshuaiyanggeCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		if #targets == 0 then
			return (to_select:objectName() ~= player:objectName()) and to_select:hasSkill("keshuaiyangge")
		end
		return false
	end,
	on_use = function(self, room, player, targets)
...
keshuaisaojianCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, player, targets)
		...
		room:showCard(target, card_id, room:getOtherPlayers(target))
		for i = 1, 5 do
...
keshuaiguanshi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardEffect then
			local effect = data:toCardEffect()
			if table.contains(effect.card:getSkillNames(), "keshuaiguanshi") and effect.card:hasFlag("keshuaiguanshijd") then
				local juedou = sgs.Sanguosha:cloneCard("duel")
...
keshuaiqiluanCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		...
		if not slash or slash:targetFixed() then
			return false
		end
		local plist = sgs.PlayerList()
		for i = 1, #targets do
...
xingqiantunCard = sgs.CreateSkillCard {
	...
	will_throw = false,
	filter = function(self, targets, to_select, player)
		return #targets < 1 and to_select ~= player and to_select:getHandcardNum() > 0
	end,
	on_use = function(self, room, player, targets)
		local tos = {}
		for _, p in sgs.list(targets) do
	...
	on_use = function(self, room, player, targets)
		...
		ys.effect = function(ys_data)
			...
			elseif ys_data.result == "red" then
				local tos = sgs.SPlayerList()
				for _, p in sgs.qlist(room:getOtherPlayers(player)) do
					if table.contains(ys.tos, p:objectName()) then
						continue
					end
					tos:append(p)
				end
		...
		if event == sgs.CardsMoveOneTime then
			...
			if move.to_place == sgs.Player_DiscardPile then
				for _, id in sgs.list(move.card_ids) do
					...
					if player:getMark(s .. "xingciyingSuit-Clear") > 0 then
						continue
					end
		...
		if event == sgs.CardsMoveOneTime then
			...
			if move.to_place == sgs.Player_DiscardPile and move.card_ids:length() > player:getHp() and move.from and move.from:objectName() == player:objectName() then
				if
					...
					or move.reason.m_reason == sgs.CardMoveReason_S_REASON_USE
				then
					local ids = sgs.IntList()
					for _, id in sgs.list(move.card_ids) do
...
xingpiqiCard = sgs.CreateSkillCard {
	...
	will_throw = false,
	filter = function(self, targets, to_select, player)
		if to_select:getMark("&xingpiqiUse-PlayClear") > 0 then
			return
		end
		local dc = dummyCard("snatch")
		dc:setSkillName(self:getSkillName())
		if #targets > sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_ExtraTarget, player, dc, to_select) or player:isProhibited(to_select, dc) then
			return
		end
		return player ~= to_select
	end,
...
xingzhuweiCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, player, targets)
		for _, target in sgs.list(targets) do
			...
			local tos = sgs.SPlayerList()
			for _, p in sgs.list(aps) do
				if player:isProhibited(p, c) then
					continue
				end
				local n = c:getRealCard():toEquipCard():location()
