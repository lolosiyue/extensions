...
berserk_xinxian = sgs.CreateTriggerSkillV2 {
	name = "berserk_xinxian",
	events = { sgs.Damaged },
	can_trigger = function(skill, event, room, player, data)
		if not player:isAlive() or not player:hasSkill(skill:objectName()) then
			return false
		end
...
--场上全部的音符标记。
berserk_benghuai = sgs.CreateTriggerSkillV2 {
		...
		for _, t in sgs.qlist(room:getAlivePlayers()) do
			if t:getMark("@note") > 0 then
				if t:objectName() ~= player:objectName() then
					room:damage(sgs.DamageStruct(skill:objectName(), player, t, math.ceil(t:getMark("@note") / 2), sgs.DamageStruct_Fire))
...
berserk_menghuan = sgs.CreateTriggerSkillV2 {
	...
	on_effect = function(skill, event, room, miku, ctx)
		local a = 0
		local dummy = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)
		room:sendCompulsoryTriggerLog(miku, skill:objectName())
		room:broadcastSkillInvoke(skill:objectName())
		...
		while a < 3 + miku:getLostHp() do
			...
			move.reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_TURNOVER, miku:objectName(), skill:objectName(), nil)
			...
			elseif card:isBlack() then
				local p = room:askForPlayerChosen(miku, room:getAlivePlayers(), skill:objectName(), "@menghuan-target2", true)
				if p then
...
berserk_jichang = sgs.CreateTriggerSkillV2 {
	...
	can_trigger = function(skill, event, room, player, data)
		...
		if player:getMark("@jichang") <= 0 then
			return false
		end
		return skill:objectName()
	end,
