...
function addToSkills(...)
	...
	for _, s in ipairs({ ... }) do
		if sgs.Sanguosha:getSkill(s:objectName()) then
			continue
		end
		skills:append(s)
	end
...
function CardIsAvailable(player, name, sn, suit, num)
	local dc = sgs.Sanguosha:cloneCard(name or "slash", suit or 6, num or 0)
	if dc then
		if sn then
			dc:setSkillName(sn)
		end
...
function AgCardsToName(player, type_ids, NODT, no_names)
	...
	for _, id in sgs.qlist(sgs.Sanguosha:getRandomCards()) do
		...
		if NODT and c:isKindOf("DelayedTrick") or table.contains(tocs, c:objectName()) or table.contains(no_names, c:objectName()) then
			continue
		end
		if c:isAvailable(player) and string.find(type_ids, c:getType()) then
			table.insert(tocs, c:objectName())
			toids:append(c:getId())
		end
	end
	if toids:isEmpty() then
		return
	end
	...
	room:clearAG(player)
	return sgs.Sanguosha:getEngineCard(toids):objectName()
end
function ShimingSkillDoAnimate(self, player, success, gn)
	...
	if msg and msg:hasSkill(self) then
		gn = gn or player:getGeneral2Name()
	end
...
function SkillWakeTrigger(self, player, n, gn)
	...
	room:sendLog(log)
	n = n or -1
	room:sendCompulsoryTriggerLog(player, self, true, true)
	room:addPlayerMark(player, self)
	log = player:getGeneral2()
	if log and log:hasSkill(self) then
		gn = gn or player:getGeneral2Name()
	end
	gn = gn or player:getGeneralName()
...
function NotifySkillInvoked(self, player, tos, num)
	...
	room:broadcastSkillInvoke(self, num or -1, player)
	local msg = sgs.LogMessage()
...
function hasCard(player, name, he)
	...
	if name:match(",") then
		for _, n in ipairs(name:split(",")) do
...
function UseCardRecast(player, card, reason, n)
	...
	local r = string.sub(log.arg, 1, 1)
	if r == "_" or r == "@" then
		log.type = "$RecastCard" .. r
		log.arg = string.sub(reason, 2, -1)
	end
	local room = player:getRoom()
	...
	if player:hasSkill(log.arg, true) then
		room:notifySkillInvoked(player, log.arg)
	end
	r = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_RECAST, player:objectName(), log.arg, "")
	room:moveCardTo(card, player, nil, sgs.Player_DiscardPile, r, true)
	return player:drawCardsList(n or card:subcardsLength(), "recast")
end
...
function BeMan(room, owner, dead)
	...
	for _, p in sgs.qlist(room:getPlayers()) do
		if
			p:objectName() == owner -- and (dead~=true or p:isAlive())
		then
			return p
		end
	end
...
function dummyCard(name, suit, number, sn)
	...
	local dc = sgs.Sanguosha:cloneCard(name or "slash", suit or 6, number or 0)
	if dc then
		if sn then
			dc:setSkillName(sn)
		end
...
function YijiPreview(self, player, cards, bool, reason)
	...
	local move = sgs.CardsMoveStruct(cards, player, nil, sgs.Player_PlaceHand, sgs.Player_PlaceTable, sgs.CardMoveReason(reason, player:objectName(), self, nil))
	local room = player:getRoom()
	if bool then
		local slash = dummyCard()
		for _, id in sgs.qlist(cards) do
			if room:getCardPlace(id) ~= sgs.Player_PlaceTable and room:getCardPlace(id) ~= sgs.Player_DrawPile then
				slash:addSubcard(id)
			end
		end
		room:moveCardTo(slash, nil, sgs.Player_PlaceTable)
		move = sgs.CardsMoveStruct(cards, nil, player, sgs.Player_PlaceTable, sgs.Player_PlaceHand, sgs.CardMoveReason(reason, player:objectName(), self, nil))
	end
...
function PlayerHandcardNum(player, self, num)
	...
	room:sendLog(msg)
	if n > num then
		room:askForDiscard(player, self, n - num, n - num)
	elseif num > n then
		room:drawCards(player, num - n, self)
	end
end
function Skill_msg(self, player, num)
	...
	room:broadcastSkillInvoke(self, num or 0, player)
	room:notifySkillInvoked(player, self)
	local msg = sgs.LogMessage()
	msg.type = "#Skill_msg"
	msg.from = player
...
sgs.LoadTranslationTable {
	...
	["$RecastCard_"] = "%from 执行“%arg”的效果，重铸了 %card",
	["$RecastCard@"] = "%from 发动“%arg”，重铸了 %card",
...
function getPileSuitNum(Pile, suit)
	...
	for _, c in sgs.list(Pile) do
		...
		if suit then
			if s == suit then
				table.insert(suits, c)
			end
		elseif not table.contains(suits, s) then
...
function MovePlayerCard(player, players, Pile, reason, prompt)
	...
	for _, p in sgs.list(players) do
		local can = false
		for _, c in sgs.qlist(p:getCards(Pile)) do
			can = true
			break
		end
		if can then
	...
	for _, p in sgs.qlist(room:getAlivePlayers()) do
		...
		if index > -1 then
			if p:getEquip(index) then
				continue
			end
		end
		tos:append(p)
	end
	room:setTag("QiaobianTarget", ToData(totos))
	local to1 = room:askForPlayerChosen(player, tos, reason, prompt)
	local mx = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_TRANSFER, player:objectName(), reason, "")
	if to1 then
		room:moveCardTo(card, totos, to1, place, mx)
	end
...
function Log_message(msg_type, from, to, cid, arg, arg2, arg3, arg4, arg5)
	...
	msg.card_str = type(cid) == "userdata" and cid:toString() or cid or "."
	msg.arg = arg or ""
	msg.arg2 = arg2 or ""
	msg.arg3 = arg3 or ""
	msg.arg4 = arg4 or ""
...
function BreakCard(player, card)
	...
	room:sendLog(msg)
	for _, id in sgs.qlist(room:getTag("BreakCard"):toIntList()) do
...
function SearchCard(player, card_names)
	...
	local function SearchCardNames(c)
		if type(card_names) == "table" then
			if table.contains(card_names, c:objectName()) or table.contains(card_names, c:getClassName()) then
				return true
			end
		elseif string.find(card_names, c:objectName()) or string.find(card_names, c:getClassName()) then
			return true
		end
	end
	local room = player:getRoom()
...
function ThrowEquipArea(self, player, cancel, invoke, n, x)
	...
	if #choices > 0 and (not invoke or player:askForSkillInvoke(self)) then
		if cancel then
...
function GetCardPlace(source, to_select)
	if source:getHandcards():contains(to_select) then
		return 1
	end
	if source:getEquips():contains(to_select) then
		return 2
	end
	if source:getJudgingArea():contains(to_select) then
		return 3
	end
	return -1
end
...
function MarkRevises(player, mark1, mark2)
	...
	if mark1:endsWith("_lun") then
		...
		for _, m in ipairs(player:getMarkNames()) do
			...
			if m:startsWith(mark[1]) and m:endsWith("_lun") then
				c_m = to_m[1] .. "+" .. mark2
	...
	for _, m in ipairs(player:getMarkNames()) do
		...
		local to_m = m:split("-")
		if m:startsWith(mark[1]) and (#mark < 2 or to_m[#to_m] == mark[#mark]) then
			c_m = to_m[1] .. "+" .. mark2
			if #mark > 1 then
				c_m = c_m .. "-" .. mark[#mark]
			end
...
function ExchangePileCard(self, player, name, n, can_equipped, will, compulsory)
	...
	if compulsory or player:askForSkillInvoke("Exchange", sgs.QVariant("Exchange:" .. self .. ":" .. name), false) then
		...
		local move = sgs.CardsMoveStruct(cids, player, player, sgs.Player_PlaceSpecial, sgs.Player_PlaceHand, sgs.CardMoveReason(reason, player:objectName(), self, nil))
		local moves = sgs.CardsMoveList()
		moves:append(move)
		room:notifyMoveCards(true, moves, false, guojia)
		room:notifyMoveCards(false, moves, false, guojia)
		...
		move = sgs.CardsMoveStruct(cids, player, nil, sgs.Player_PlaceHand, sgs.Player_PlaceTable, sgs.CardMoveReason(reason, player:objectName(), self, nil))
		...
		if ns and ns:subcardsLength() > 0 then
			...
			for i, id in sgs.list(ns:getSubcards()) do
				if cids:contains(id) then
					continue
				end
				ids:addSubcard(id)
			end
			for i, id in sgs.qlist(cids) do
				if ns:getSubcards():contains(id) then
					continue
				end
				c:addSubcard(id)
			end
...
function PreviewCards(self, player, cards, x, n, optional, prompt, throw)
	...
	local move = sgs.CardsMoveStruct(cards, owner, player, place, sgs.Player_PlaceHand, sgs.CardMoveReason(reason, player:objectName(), self, nil))
	local moves = sgs.CardsMoveList()
	moves:append(move)
	room:notifyMoveCards(true, moves, false, guojia)
	room:notifyMoveCards(false, moves, false, guojia)
	...
	if throw then
		ns = room:askForDiscard(player, self, x, n, true, optional, prompt, ns, self)
	else
		ns = room:askForExchange(player, self, x, n, true, prompt, optional, ns)
	end
	player:removeTag("PreviewCards")
	move = sgs.CardsMoveStruct(cards, player, nil, sgs.Player_PlaceHand, sgs.Player_PlaceTable, sgs.CardMoveReason(reason, player:objectName(), self, nil))
	moves = sgs.CardsMoveList()
	moves:append(move)
	room:notifyMoveCards(true, moves, false, guojia)
	room:notifyMoveCards(false, moves, false, guojia)
...
function CardVisible(player, id, special)
	if type(id) ~= "number" then
		id = id:getEffectiveId()
	end
	local place = player:getRoom():getCardPlace(id)
	if place == sgs.Player_PlaceEquip or place == sgs.Player_PlaceJudge or place == sgs.Player_PlaceTable or place == sgs.Player_DiscardPile or place == sgs.Player_PlaceDelayedTrick then
		return true
...
function SetShifa(self, player, x)
	...
	if type(x) ~= "number" then
		x = room:askForChoice(player, "shifa", "shifa1+shifa2+shifa3")
	end
	if x == "shifa1" then
		x = 1
	elseif x == "shifa2" then
		x = 2
	elseif x == "shifa3" then
		x = 3
	end
	for t, m in ipairs(sgs.shifa_skills) do
		if m.name == self and m.playerId == player:objectName() then
			table.remove(sgs.shifa_skills, t)
		end
	end
...
function GainOvBaonieNum(target, n)
	n = tonumber(n)
	if n > 0 then
		local x = math.min(n, 5 - target:getTag("ov_baonieNum"):toInt())
		if x < 1 then
			return
		end
	...
	elseif n < 0 then
		local x = math.min(-n, target:getTag("ov_baonieNum"):toInt())
		if x < 1 then
			return
		end
		local m = target:getTag("ov_baonieNum"):toInt() - x
...
function SetWangxing(self, player, x)
	...
	if type(x) ~= "number" then
		x = tonumber(room:askForChoice(player, "wangxing", "1+2+3+4"))
	end
	for t, m in ipairs(sgs.wangxing_skills) do
		if m.name == self and m.playerId == player:objectName() then
			table.remove(sgs.wangxing_skills, t)
		end
	end
...
function targetsPindian(self, player, targets)
	...
	for _, t in sgs.qlist(log.to) do
		...
		if not c or room:getCardPlace(c:getEffectiveId()) ~= sgs.Player_PlaceTable then
			continue
		end
		local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_PINDIAN, pd.from:objectName(), t:objectName(), pd.reason, "")
...
function InstallEquip(eid, player, self, to)
	...
	if type(eid) ~= "number" then
		eid = eid:getEffectiveId()
	else
		e = sgs.Sanguosha:getCard(eid)
	end
...
function MoveFieldCard(self, player, flags, froms, tos)
	...
	if froms:isEmpty() then
		for i, p in sgs.qlist(room:getAlivePlayers()) do
			if p:getCards(mfc.flags):length() > 0 then
				froms:append(p)
			end
		end
	end
	...
	tos = tos or room:getAlivePlayers()
	for _, c in sgs.qlist(mfc.from:getCards(mfc.flags)) do
		local has = true
		for _, p in sgs.list(tos) do
			...
			if c:getTypeId() == 3 then
				if p:getEquip(c:getRealCard():toEquipCard():location()) then
					continue
				end
			end
			has = false
	...
	for _, p in sgs.list(tos) do
		if player:isProhibited(p, mfc.card) then
			continue
		end
		if mfc.to_place == sgs.Player_PlaceEquip then
			if p:getEquip(mfc.card:getRealCard():toEquipCard():location()) then
				continue
			end
		end
		canTos:append(p)
	...
	room:doAnimate(1, player:objectName(), mfc.to:objectName())
...
function GetZhizheEquipObjectNameByArea(area)
	if area == sgs.EquipCard_WeaponLocation or area == 0 then
		return "_zhizhe_weapon"
	end
	if area == sgs.EquipCard_ArmorLocation or area == 1 then
		return "_zhizhe_armor"
	end
	if area == sgs.EquipCard_DefensiveHorseLocation or area == 2 then
		return "_zhizhe_defensivehorse"
	end
	if area == sgs.EquipCard_OffensiveHorseLocation or area == 3 then
		return "_zhizhe_offensivehorse"
	end
	if area == sgs.EquipCard_TreasureLocation or area == 4 then
		return "_zhizhe_treasure"
	end
	return nil
end

function SafeTurnHandCardToEquip(room, player, card_id, equip_name, skill_name)
	if not room or not player or type(card_id) ~= "number" or type(equip_name) ~= "string" then
		return false
	end
	...
	local source_card = sgs.Sanguosha:getCard(card_id)
	if not source_card then
		return false
	end
	local equip = sgs.Sanguosha:cloneCard(equip_name, source_card:getSuit(), source_card:getNumber())
	if not equip or not equip:isKindOf("EquipCard") then
		return false
	end

	local location = equip:getRealCard():toEquipCard():location()
	if not player:hasEquipArea(location) then
		return false
	end

	local wrapped = sgs.Sanguosha:getWrappedCard(card_id)
	if not wrapped then
		return false
	end
	...
	local moves = sgs.CardsMoveList()
	local old_equip = player:getEquip(location)
	if old_equip then
		moves:append(
			sgs.CardsMoveStruct(
				old_equip:getEffectiveId(),
				nil,
				sgs.Player_DiscardPile,
				sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_CHANGE_EQUIP, player:objectName(), skill_name or "", "change equip")
			)
		)
	end
	moves:append(sgs.CardsMoveStruct(card_id, player, sgs.Player_PlaceEquip, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_USE, player:objectName(), skill_name or "", "")))
	room:moveCardsAtomic(moves, true)
	return true
end

function SafeTurnHandCardToZhizheEquip(room, player, card_id, area, skill_name)
	local equip_name = GetZhizheEquipObjectNameByArea(area)
	if not equip_name then
		return false
	end
	return SafeTurnHandCardToEquip(room, player, card_id, equip_name, skill_name)
end

sgs.GetZhizheEquipObjectNameByArea = GetZhizheEquipObjectNameByArea
sgs.SafeTurnHandCardToEquip = SafeTurnHandCardToEquip
sgs.SafeTurnHandCardToZhizheEquip = SafeTurnHandCardToZhizheEquip
...
function patterns(class)
	...
	if bp ~= banPs then
		...
		for id = 0, sgs.Sanguosha:getCardCount() - 1 do
			local c = sgs.Sanguosha:getEngineCard(id)
			if table.contains(cardNames, c:objectName()) then
				continue
			end
			class2Names[c:getClassName()] = c:objectName()
			if string.sub(c:objectName(), 1, 1) == "_" or table.contains(bp, c:getPackage()) then
				continue
			end
			if c:getTypeId() < 3 then
				table.insert(cardNames, c:objectName())
			else
				table.insert(equip_patterns, c:objectName())
			end
		end
	end
	if class == true then
...
OnSkillTrigger = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.HpChanged then
			...
			if damage.to and damage.to:isAlive() then
				for _, skill in sgs.qlist(damage.to:getSkillList()) do
					if skill:property("ov_baonieNum"):toBool() then
						GainOvBaonieNum(damage.to, damage.damage)
						break
					end
				end
			end
		elseif event == sgs.PreCardUsed then
			...
			if use.from == player and use.card:getTypeId() > 0 and player:getPhase() == sgs.Player_Play then
				for i = 1, 2 do
					if player:getTag("zhengsu" .. i):toBool() then
						local zhengsu = player:getTag("zhengsu-" .. i):toIntList()
						if i < 2 then
							zhengsu:append(use.card:getNumber())
						else
							zhengsu:append(use.card:getSuit())
						end
		...
		elseif event == sgs.EventPhaseChanging then
			...
			if change.to == sgs.Player_NotActive then
				for _, p in sgs.qlist(room:getAllPlayers()) do
					for t, sf in pairs(sgs.shifa_skills) do
						if sf.m < 1 or sf.playerId ~= p:objectName() then
							continue
						end
						sf.m = sf.m - 1
					...
					for t, wx in pairs(sgs.wangxing_skills) do
						if wx.x < 1 or wx.playerId ~= p:objectName() then
							continue
						end
				...
				for i = 1, 3 do
					if player:getTag("zhengsu" .. i):toBool() then
						...
						if can then
							...
							if msg == "drawCards_2" then
								player:drawCards(2, "zhengsu")
							else
								room:recover(player, sgs.RecoverStruct("zhengsu"))
							end
		...
		elseif event == sgs.EventPhaseStart or event == sgs.EventPhaseProceeding then
			local n = player:getTag("FinishPhase"):toInt()
			if n > 0 and n <= player:getPhase() then
				player:removeTag("FinishPhase")
				return n == player:getPhase()
			end
		end
	end,
}
IsProhibited = sgs.CreateProhibitSkill {
	...
	is_prohibited = function(self, from, to, card)
		return card:getTypeId() > 0 and to and to:property("aiNoTo"):toBool()
	end,
}
