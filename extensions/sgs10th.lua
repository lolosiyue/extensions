		...
		if damage.card then
			for _, p in sgs.qlist(room:findPlayersBySkillName("ny_10th_shangyu")) do
				if p:getMark("ny_10th_shangyu_slash") == damage.card:getId() then
					...
					local aps = SPlayerList(p, damage.from)
					room:sortByActionOrder(aps)
					room:drawCards(aps, 1, "ny_10th_shangyu")
				end
			end
		end
	end,
...
ny_10th_changquCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		local room = source:getRoom()
		local move = 1
		if source:getNextAlive(1):objectName() ~= targets[1]:objectName() then
			move = room:getAlivePlayers():length() - 1
		end
...
ny_10th_humeiCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		...
		if choice == "give" and to_select:isNude() then
			return false
		end
		if choice == "give" and to_select:objectName() == player:objectName() then
			return false
		end
		if choice == "recover" and (not to_select:isWounded()) then
			return false
		end
...
ny_10th_moyuCard = sgs.CreateSkillCard {
	name = "ny_10th_moyu",
	filter = function(self, targets, to_select, player)
		return to_select:objectName() ~= player:objectName() and (not to_select:isAllNude()) and to_select:getMark("ny_10th_moyu_chosen-PlayClear") == 0 and #targets == 0
	end,
	on_effect = function(self, effect)
		local room = effect.from:getRoom()
...
ny_pandi_tenthCard = sgs.CreateSkillCard {
	name = "ny_pandi_tenth",
	filter = function(self, targets, to_select, player)
		return #targets == 0 and to_select ~= player and to_select:getMark("ny_pandi_tenth_damage-Clear") == 0
	end,
	on_use = function(self, room, source, targets)
...
ny_tenth_poruiCard = sgs.CreateSkillCard {
	...
	will_throw = true,
	filter = function(self, targets, to_select, player)
		return to_select ~= player and to_select:getMark("&ny_tenth_porui-Clear") > 0 and to_select:getPhase() == sgs.Player_NotActive
	end,
	on_use = function(self, room, source, targets)
		...
		if source:isAlive() and target:isAlive() and source:getMark("ny_10th_gonghu_damage") == 0 then
			...
			local give = room:askForExchange(source, self:objectName(), n, n, false, prompt)
			if give then
...
ny_tenth_porui = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
			if p:getPhase() ~= sgs.Player_Finish and ((p:getMark("ny_tenth_porui_used_lun") < 1) or (p:getMark("ny_tenth_porui_used_lun") < 2 and p:getMark("ny_10th_gonghu_lose") > 0)) then
	...
	on_trigger = function(self, event, player, data)
		...
		if player:hasTurn() and move.from and move.from:objectName() == player:objectName() then
			if move.from_places:contains(sgs.Player_PlaceHand) or move.from_places:contains(sgs.Player_PlaceEquip) then
				room:setPlayerMark(player, "&ny_tenth_porui-Clear", 1, room:findPlayersBySkillName("ny_tenth_porui"))
			end
		end
	end,
	can_trigger = function(self, target)
...
ny_10th_gonghu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed then
			if player:getMark("ny_10th_gonghu_damage") == 0 or player:getMark("ny_10th_gonghu_lose") == 0 then
				return false
			end
			local use = data:toCardUse()
...
ny_10th_qinshi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.TargetConfirmed then
			if player:getMark("&ny_10th_qinshi-Clear") ~= player:getHandcardNum() then
				return false
			end
...
ny_10th_taji = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.DamageCaused then
			if player:getMark("&ny_10th_taji") == 0 then
				return false
			end
			local damage = data:toDamage()
...
ny_10th_huiling_record = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		local move = data:toMoveOneTime()
		if move.to_place == sgs.Player_DiscardPile or move.from_places:contains(sgs.Player_DiscardPile) then
		else
			return false
		end
		local red = 0
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.Damage then
			...
			if damage.card and damage.card:hasFlag("ny_10th_jiuxian_target_" .. damage.to:objectName()) then
				...
				if not targets:isEmpty() then
					local target = room:askForPlayerChosen(player, targets, "ny_10th_jiuxian", "@ny_10th_jiuxian:" .. damage.to:getGeneralName(), true, true)
					if target then
...
ny_10th_chenyong = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart then
			...
			if player:getMark("&ny_10th_chenyong-Clear") <= 0 then
				return false
			end
...
ny_10th_yizhao = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		player:gainMark("&ny_10th_huang", card:getNumber())
		local m = player:getMark("&ny_10th_huang")
		if m < 10 then
			return false
		end
...
ny_10th_sijun = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getMark("&ny_10th_huang") <= room:getDrawPile():length() then
			return false
		end
		if not room:askForSkillInvoke(player, self:objectName(), sgs.QVariant("draw")) then
			return false
		end
		room:broadcastSkillInvoke(self:objectName())
		...
		if math.random(1, 6) == 1 then
			...
			for i = 0, 99 do
				...
				for _, c in ipairs(cards) do
					local num = c:getNumber()
					if num <= n and c:isAvailable(player) then
						dummy:addSubcard(c)
						n = n - num
						if n == 0 then
...
ny_10th_tianjie = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseChanging then
			...
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if p:isAlive() then
					local targets = room:askForPlayersChosen(p, room:getOtherPlayers(p), self:objectName(), 0, 3, "@ny_10th_tianjie", true, true)
					if targets and (not targets:isEmpty()) then
						room:broadcastSkillInvoke(self:objectName())
						...
						for _, target in sgs.qlist(targets) do
							if target:isAlive() then
								...
								room:damage(sgs.DamageStruct(self:objectName(), p, target, jink, sgs.DamageStruct_Thunder))
							end
						end
					end
				end
...
ny_tenth_wanglu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart then
			...
			if ((not player:getEquip(4)) or (player:getEquip(4) and player:getEquip(4):objectName() ~= "_ny_tenth_dagongche")) and player:hasEquipArea(4) then
				local card = nil
...
ny_tenth_xianzhu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getMark("&ny_tenth_xianzhu_update") >= 5 then
			return false
		end
		local damage = data:toDamage()
		if damage.card and damage.card:isKindOf("Slash") then
			if room:askForSkillInvoke(player, self:objectName(), sgs.QVariant("update")) then
				...
				local choice = room:askForChoice(player, self:objectName(), table.concat(choices, "+"), sgs.QVariant(), table.concat(except, "+"), nil)
...
ny_10th_jingdianzhihengCard = sgs.CreateSkillCard {
	...
	about_to_use = function(self, room, use)
		if use.from:getHandcardNum() > 0 then
			local has = true
			for _, h in sgs.qlist(use.from:getHandcards()) do
				if not self:getSubcards():contains(h:getId()) then
					has = false
				end
			end
			if has then
				room:setCardFlag(self, "ny_10th_jingdianzhiheng_all")
			end
		end
	...
	on_use = function(self, room, source, targets)
		...
		if self:hasFlag("ny_10th_jingdianzhiheng_all") then
			n = n + 1
		end
...
ny_10th_jingdianzhiheng_buff = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.Damage then
			local damage = data:toDamage()
			if damage.to ~= player and player:hasTurn() and damage.to:getMark("ny_10th_jingdianzhiheng_trigger_" .. player:objectName() .. "-Clear") == 0 then
...
ny_tenth_jingdianrendeCard = sgs.CreateSkillCard {
	name = "ny_tenth_jingdianrende",
	filter = function(self, targets, to_select, player)
		return #targets < 1 and to_select ~= player and to_select:getHandcardNum() > 0 and to_select:getMark("ny_tenth_jingdianrende_get-PlayClear") == 0
	end,
	on_effect = function(self, effect)
		local room = effect.from:getRoom()
		...
		if effect.from:isAlive() and obtain:subcardsLength() > 1 then
			...
			local choice = room:askForChoice(effect.from, self:objectName(), table.concat(can, "+"), sgs.QVariant(), table.concat(cant, "+"), "ny_tenth_jingdianrende_choice")
...
ny_tenth_huishu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardsMoveOneTime then
			if player:getMark("&ny_tenth_huishu_target-Clear") == 0 then
				return false
			end
			if player:isDead() then
				return false
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardsMoveOneTime then
			...
			if move.from and move.from:objectName() == player:objectName() and (move.from_places:contains(sgs.Player_PlaceHand) or move.from_places:contains(sgs.Player_PlaceEquip)) then
				...
				local changemax = {}
				if max == draw then
					table.insert(changemax, "draw=" .. max)
				end
				if max == discard then
					table.insert(changemax, "discard=" .. discard)
				end
				if max == get then
					table.insert(changemax, "get=" .. get)
				end

				local min = math.min(draw, discard, get)
				local changemin = {}
				if min == draw then
					table.insert(changemin, "draw=" .. draw)
				end
				if min == discard then
					table.insert(changemin, "discard=" .. discard)
				end
				if min == get then
					table.insert(changemin, "get=" .. get)
				end
				...
				if string.find(cmax, "draw") then
					room:addPlayerMark(player, "SkillDescriptionArg1_ny_tenth_huishu", -1)
				end
				if string.find(cmax, "discard") then
					room:addPlayerMark(player, "SkillDescriptionArg2_ny_tenth_huishu", -1)
				end
				if string.find(cmax, "get") then
					room:addPlayerMark(player, "SkillDescriptionArg3_ny_tenth_huishu", -1)
				end

				if string.find(cmin, "draw") then
					room:addPlayerMark(player, "SkillDescriptionArg1_ny_tenth_huishu", 2)
				end
				if string.find(cmin, "discard") then
					room:addPlayerMark(player, "SkillDescriptionArg2_ny_tenth_huishu", 2)
				end
				if string.find(cmin, "get") then
					room:addPlayerMark(player, "SkillDescriptionArg3_ny_tenth_huishu", 2)
				end
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventLoseSkill then
			...
			if data:toString() == "qicai" and player:hasSkill(self) then
				if player:getTreasure() or player:hasSkill("qicai", true) then
					return false
				end
		...
		elseif event == sgs.CardUsed then
			...
			if use.card:isKindOf("Slash") or use.card:isNDTrick() then
				if player:getTreasure() or player:getArmor() or player:getDefensiveHorse() or player:getOffensiveHorse() then
					return false
				end
				room:broadcastSkillInvoke(self:objectName())
		...
		elseif event == sgs.InvokeSkill then
			if data:toString() == "eight_diagram" and player:getArmor() == nil then
				room:sendCompulsoryTriggerLog(player, self)
			end
		end
	end,
...
ny_10th_lvecheng_buff = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if change.to == sgs.Player_NotActive then
			...
			for _, p in sgs.qlist(room:getOtherPlayers(player)) do
				if p:getMark("ny_10th_lvecheng_from" .. player:objectName() .. "-Clear") > 0 and p:getHandcardNum() > 0 then
...
ny_10th_yaoyi = sgs.CreateProhibitSkill {
	...
	is_prohibited = function(self, from, to, card)
		...
		if st1 == 2 or st2 == 2 then
			return false
		end
		if card and (not card:isKindOf("SkillCard")) and (st1 == st2) then
			return true
		end
		return false
	end,
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart then
			if player:getPhase() == sgs.Player_Start then
				...
				if (not all) or (all:isEmpty()) then
					return false
				end
...
ny_10th_jxxiantu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseEnd then
			if player:getMark("ny_10th_jxxiantuda-PlayClear") > 0 then
				return false
			end
			for _, p in sgs.qlist(room:getOtherPlayers(player)) do
...
ny_10th_jxqiangzhi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getMark("&ny_10th_jxqiangzhi+" .. ctype .. "-PlayClear") == 0 then
			return false
		end
...
ny_tenth_shanjia = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardsMoveOneTime then
			local move = data:toMoveOneTime()
			if player:getMark("&ny_tenth_shanjia") >= 3 then
				return false
			end
			if move.from and move.from:objectName() == player:objectName() and (move.from_places:contains(sgs.Player_PlaceHand) or move.from_places:contains(sgs.Player_PlaceEquip)) then
			else
		...
		if event == sgs.EventPhaseStart then
			...
			if room:askForSkillInvoke(player, self:objectName(), sgs.QVariant(prompt)) then
				...
				if canslash then
					room:askForUseCard(player, "@@ny_tenth_shanjia", "@ny_tenth_shanjia", -1, sgs.Card_MethodUse, false, nil, nil, "RemoveFromHistory")
				end
			end
		end
	end,
...
ny_10th_poyuan = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if ((not player:getEquip(4)) or (player:getEquip(4) and player:getEquip(4):objectName() ~= "_ny_tenth_piliche")) and player:hasEquipArea(4) then
			local card = nil
			for _, id in sgs.qlist(sgs.Sanguosha:getRandomCards(true)) do
				if sgs.Sanguosha:getEngineCard(id):objectName() == "_ny_tenth_piliche" then
					card = sgs.Sanguosha:getEngineCard(id)
					break
				end
			end
			if card then
				if not room:askForSkillInvoke(player, self:objectName(), sgs.QVariant("put")) then
					return false
				end
				room:broadcastSkillInvoke(self:objectName())
...
ny_10th_huace_record = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed or event == sgs.CardResponded then
			...
			if (not card) or (not card:isKindOf("TrickCard")) then
				return false
			end
...
ny_10th_jijiao_buff = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseChanging then
			...
			if player:getMark("ny_10th_jijiao_new-Clear") == 0 then
				return false
			end
...
ny_10th_qingbei = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardFinished then
			...
			if (not use.card:isKindOf("SkillCard")) and use.m_isHandcard then
				caninvoke = true
			end
			if not caninvoke then
				return false
			end
			room:sendCompulsoryTriggerLog(player, self:objectName(), true, true)
...
ny_10th_jiudun = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.TargetConfirmed then
			...
			if use.from ~= player and (not use.card:isKindOf("SkillCard")) and use.card:isBlack() then
			else
				return false
			end
...
ny_10th_zhaowen = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed or event == sgs.CardResponded then
			...
			if (not card:isRed()) or (not card:hasFlag("ny_10th_zhaowen")) then
				return false
			end
			room:sendCompulsoryTriggerLog(player, self:objectName(), true, true)
			player:drawCards(1, self:objectName())
...
local function ny_gusuan_chosen(player, selected, num)
	...
	local prompt
	if num == 1 then
		prompt = "ny_10th_gusuan_draw"
	elseif num == 2 then
		prompt = "ny_10th_gusuan_discard"
	elseif num == 3 then
		prompt = "ny_10th_gusuan_change"
	end
		...
		elseif event == sgs.CardsMoveOneTime then
			...
			for _, id in sgs.qlist(move.card_ids) do
				...
				for _, m in sgs.list(player:getMarkNames()) do
					if m:endsWith("+#ny_10th_geyuan") and player:getMark(m) > 0 then
						mr = m
...
ny_tenth_shujianCard = sgs.CreateSkillCard {
	...
	on_effect = function(self, effect)
		...
		if effect.to:isAlive() then
			choice = room:askForChoice(effect.to, self:objectName(), table.concat(choices, "+"), data)
		end
		...
		if effect.to:isAlive() and string.find(choice, "dis") then
			...
			for i = 1, n, 1 do
				...
				if not room:askForUseCard(effect.to, "@@ny_tenth_shujian", prompt) then
					break
				end
				if effect.to:isDead() then
					break
...
ny_10th_youzhan_buff = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.DamageInflicted then
			if player:getMark("ny_10th_youzhan_damageup-Clear") == 0 then
				return false
			end
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.InvokeSkill then
			...
			if #skills <= 0 then
				...
				player:setTag("ny_tenth_longsong_skills", sgs.QVariant(table.concat(skills, "+")))
			end
		end
		if event == sgs.EventPhaseEnd then
			if player:getPhase() ~= sgs.Player_Play then
...
ny_tenth_niji = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.TargetConfirmed then
			local use = data:toCardUse()
			if (use.card:isKindOf("BasicCard") or use.card:isKindOf("TrickCard")) and use.to:contains(player) and player:hasSkill(self) then
				if room:askForSkillInvoke(player, self:objectName(), sgs.QVariant("draw")) then
					room:broadcastSkillInvoke(self:objectName())
					local cards = player:drawCardsList(1, self:objectName())
					for _, id in sgs.qlist(cards) do
...
ny_10th_beiniCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		if from and from:isAlive() and to and to:isAlive() and from:canSlash(to, false) then
...
ny_tenth_xiaoyinCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		...
		for _, other in sgs.qlist(to_select:getAliveSiblings()) do
			if other:hasFlag("ny_tenth_xiaoyin") then
				inneed = false
			end
		end
		if inneed then
		...
		for _, other in sgs.qlist(to_select:getAliveSiblings()) do
			if other:hasFlag("ny_tenth_xiaoyin") and to_select:isAdjacentTo(other) then
				return true
			end
		end
		return false
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart then
			...
			if room:askForSkillInvoke(player, self:objectName(), sgs.QVariant(prompt)) then
				...
				while room:askForUseCard(player, "@@ny_tenth_xiaoyin", "@ny_tenth_xiaoyin") do
				end
...
ny_tenth_xiaoyin_buff = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.DamageCaused then
			local damage = data:toDamage()
			if damage.to:getPile("ny_tenth_xiaoyin"):isEmpty() then
				return false
			end
			...
			if damage.nature == sgs.DamageStruct_Fire then
				...
				if card then
					...
					for _, id in sgs.qlist(card:getSubcards()) do
						if room:getCardPlace(id) == sgs.Player_PlaceHand or room:getCardPlace(id) == sgs.Player_PlaceEquip then
							local log = sgs.LogMessage()
							log.type = "$DiscardCard"
							log.from = player
...
ny_tenth_huahuoCard = sgs.CreateSkillCard {
	...
	on_validate = function(self, cardUse)
		...
		if choice == "change" then
			...
			for _, other in sgs.qlist(room:getOtherPlayers(player)) do
				if (not other:getPile("ny_tenth_xiaoyin"):isEmpty()) and (not player:isProhibited(other, card)) then
...
ny_10th_yuandi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getMark("ny_10th_yuandi-PlayClear") > 0 then
			return false
		end
			...
			if p:isAlive() and p:objectName() ~= player:objectName() then
				...
				if room:askForSkillInvoke(p, self:objectName(), _data) then
					...
					if string.find(choice, "draw") then
						local aps = SPlayerList(player, p)
						room:sortByActionOrder(aps)
...
ny_10th_xinyou_buff = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getMark("ny_10th_xinyou_recover-Clear") == 0 and player:getMark("ny_10th_xinyou_draw-Clear") == 0 then
			return false
		end
...
ny_10th_ronghuo = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.DamageCaused then
			local damage = data:toDamage()
			if damage.card and (damage.card:objectName() == "fire_slash" or damage.card:objectName() == "fire_attack") and not damage.chain then
				room:broadcastSkillInvoke(self:objectName())
				local kingdoms = {}
				for _, p in sgs.qlist(room:getAlivePlayers()) do
...
ny_10th_yingmou = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardFinished then
			...
			local targets = sgs.SPlayerList()
			for _, p in sgs.qlist(use.to) do
				if (p:objectName() ~= player:objectName()) and p:isAlive() then
					targets:append(p)
				end
			end
			if targets:isEmpty() then
...
ny_10th_zuowei = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif player:getHandcardNum() < n then
			if player:getMark("ny_10th_zuowei-Clear") > 0 then
				return false
			end
			if room:askForSkillInvoke(player, self:objectName(), sgs.QVariant("draw")) then
				room:broadcastSkillInvoke(self:objectName())
...
ny_10th_ziguCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		if to:objectName() == source:objectName() and source:isAlive() then
			source:drawCards(1, self:objectName())
		end
	end,
	--[[on_effect = function(self, effect)
		...
		if event == sgs.EventPhaseStart then
			if player:getPhase() == sgs.Player_Start then
				if room:askForSkillInvoke(player, self:objectName(), data, false) then
					...
					local targets = room:askForPlayersChosen(player, room:getAlivePlayers(), self:objectName(), 0, 999, "@ny_10th_miyi:" .. self:objectName() .. "_" .. choice, false, true)

					room:setPlayerFlag(player, "-ny_10th_miyi_" .. choice)
					if (not targets) or (targets:isEmpty()) then
						return false
					end
...
ny_10th_yinjun = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardFinished then
			local use = data:toCardUse()
			if not use.card:hasFlag("ny_10th_yinjunBf") then
				return false
			end
			...
			local target = use.to:at(0)
			if target == player or target:isDead() then
				return false
			end
			local slash = sgs.Sanguosha:cloneCard("slash")
			...
			room:addPlayerMark(player, "&ny_10th_yinjun-Clear")

			local change = player:getGeneralName() == "ny_10th_caoyi"
			if change then
			...
			if change then
				room:changeHero(player, "ny_10th_caoyi", false, false)
			end
		end
...
ny_10th_qiongyingCard = sgs.CreateSkillCard {
	...
	on_effect = function(self, effect)
		...
		if target:getCards("ej"):isEmpty() then
			return false
		end
		local disable_ids = sgs.IntList()
		...
		for _, card in sgs.qlist(target:getEquips()) do
			...
			if cant then
				disable_ids:append(card:getEffectiveId())
			end
		end
		for _, card in sgs.qlist(target:getJudgingArea()) do
			local cant = true
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart then
			...
			for i = 1, n, 1 do
				...
				local card = sgs.Sanguosha:cloneCard(pattern, sgs.Card_SuitToBeDecided, -1)
				card:deleteLater()
				if not card:isAvailable(target) then
					break
				end
...
ny_10th_cuguo = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		local effect = data:toCardEffect()
		if effect.card:isKindOf("SkillCard") then
			return false
		end
		player:addMark("ny_10th_cuguo-Clear")
		...
		if player:getMark("ny_10th_cuguo-Clear") == 1 and player:canDiscard(player, "he") and player:hasSkill(self:objectName()) then
			room:sendCompulsoryTriggerLog(player, self)
			room:askForDiscard(player, self:objectName(), 1, 1, false, true)
...
ny_10th_lianzhi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EnterDying then
			if player:getMark("ny_10th_lianzhi-Clear") > 0 or not player:hasTurn() then
				return false
			end
			...
			if dying.who == player then
				...
				room:sendCompulsoryTriggerLog(player, self:objectName(), true, true)
				player:addMark("ny_10th_lianzhi-Clear", 1)
				room:recover(player, sgs.RecoverStruct(self:objectName(), player, 1))
				local aps = SPlayerList(player, target)
				room:sortByActionOrder(aps)
				room:drawCards(aps, 1, self:objectName())
			end
		end
...
ny_10th_caisi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		local use = data:toCardUse()
		if use.from and use.from:objectName() == player:objectName() and use.card:isKindOf("BasicCard") and player:isAlive() then
...
ny_10th_zhuoli = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardsMoveOneTime then
			...
			local move = data:toMoveOneTime()
			if move.to and move.to:objectName() == player:objectName() and player:isAlive() and move.to_place == sgs.Player_PlaceHand then
...
ny_10th_danyi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardResponded then
			local response = data:toCardResponse()
			if response.m_isUse then
				if response.m_card:isKindOf("SkillCard") then
					return false
				end
				for _, p in sgs.qlist(room:getAlivePlayers()) do
					room:setPlayerMark(p, "&ny_10th_danyi+#" .. player:objectName(), 0)
...
ny_10th_wangxi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if (not target) or target:objectName() == player:objectName() or target:isDead() then
			return false
		end
		for i = 1, damage.damage, 1 do
			...
			if player:isDead() or target:isDead() then
				return false
			end
		end
	end,
...
ny_10th_lingkong_second = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardsMoveOneTime then
			...
			if player:getMark("ny_10th_lingkong_second-Clear") > 0 then
				return false
			end
			local move = data:toMoveOneTime()
			if move.to and move.to:objectName() == player:objectName() and move.to_place == sgs.Player_PlaceHand and player:isAlive() then
				room:sendCompulsoryTriggerLog(player, self:objectName(), true, true)
...
ny_10th_lingxi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardsMoveOneTime then
			...
			local move = data:toMoveOneTime()
			if move.from and move.from:objectName() == player:objectName() and move.from_places:contains(sgs.Player_PlaceSpecial) then
				local cant = true
...
ny_tenth_zhifouCard = sgs.CreateSkillCard {
	...
	will_throw = true,
	filter = function(self, targets, to_select, player)
		if (not player:hasFlag("ny_tenth_zhifou_lose")) and to_select:isNude() then
			return false
		end
		return #targets < 1 and to_select:objectName() ~= player:objectName()
	end,
...
ny_tenth_zhifou = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getPile("ny_10th_yi"):length() < n then
			return false
		end
		if n > 3 then
			return false
		...
		if not room:askForSkillInvoke(player, self:objectName(), data, false) then
			return false
		end
...
ny_10th_xiongmu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.DamageInflicted then
			...
			if room:askForSkillInvoke(player, self:objectName(), sgs.QVariant("reduce")) then
			else
				return false
			end
			room:broadcastSkillInvoke(self:objectName())
		...
		if event == sgs.RoundStart then
			...
			for _, id in sgs.qlist(room:getDrawPile()) do
				...
				if get:subcardsLength() >= dc:subcardsLength() then
					break
				end
			end
			for _, id in sgs.qlist(room:getDiscardPile()) do
...
ny_10th_weiwanCard = sgs.CreateSkillCard {
	...
	on_effect = function(self, effect)
		local room = effect.from:getRoom()
		if effect.from:isDead() or effect.to:isDead() then
			return false
		end
	...
	about_to_use = function(self, room, use)
		...
		if use.to:isEmpty() then
			...
			if choice == "top" then
				...
				room:moveCardsToEndOfDrawpile(player, self:getSubcards(), self:objectName(), false)
			end
		else
			local card = sgs.Sanguosha:getCard(self:getSubcards():at(0))
			local target = use.to:at(0)
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart then
			if player:getPhase() == sgs.Player_Finish then
				...
				_data:setValue(player)
				for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					if player:getMark("ny_tenth_shiji_failed-Clear") > 0 then
						return false
					end
...
ny_tenth_posuoCard = sgs.CreateSkillCard {
	...
	on_validate = function(self, card_use)
		...
		if mark and player:getMark(mark) > 0 then
			if string.find(mark, suit .. "_char") then
				return false
			end
			room:setPlayerMark(player, mark, 0)
...
ny_tenth_zongshiCard = sgs.CreateSkillCard {
	...
	feasible = function(self, targets, player)
		...
		for _, hand in sgs.qlist(player:getHandcards()) do
			if hand:getSuit() == show_card:getSuit() and hand:getEffectiveId() ~= show_id then
	...
	about_to_use = function(self, room, use)
		...
		local card = sgs.Sanguosha:cloneCard(show_card:objectName())
		for _, hand in sgs.qlist(player:getHandcards()) do
			if hand:getSuit() == show_card:getSuit() and hand:getEffectiveId() ~= show_card:getEffectiveId() then
				card:addSubcard(hand)
			end
		end
...
ny_10th_aoshi_giveCard = sgs.CreateSkillCard {
	...
	will_throw = false,
	filter = function(self, targets, to_select, player)
		return #targets < 1 and to_select:objectName() ~= player:objectName() and to_select:hasLordSkill("ny_10th_aoshi")
	end,
	on_effect = function(self, effect)
		local room = effect.from:getRoom()
...
ny_10th_xiace = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.Damage then
			...
			if not room:askForDiscard(player, self:objectName(), 1, 1, true, true, "@ny_10th_xiace-discard", ".", "ny_10th_xiace") then
		...
		if event == sgs.Damaged then
			if player:getMark("ny_10th_xiace_damaged-Clear") > 0 then
				return false
			end
	...
	on_trigger = function(self, event, player, data)
		...
		for _, pl in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
			if pl:getMark("@ny_10th_yuxin_mark") > 0 then
				local n = 1
				if pl:objectName() ~= player:objectName() then
					n = pl:getHp()
				end
...
ny_tenth_chanjuanCard = sgs.CreateSkillCard {
	...
	on_validate = function(self, card_use)
		...
		if card:isKindOf("Slash") then
			mark = string.format("ny_tenth_chanjuan_%s", "Slash")
		end
		room:addPlayerMark(player, mark, 1)
...
ny_tenth_chanjuan = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardFinished then
			...
			if use.to:length() == 1 and use.from:objectName() == player:objectName() and player:isAlive() and use.m_isHandcard and (use.card:isKindOf("BasicCard") or use.card:isNDTrick()) then
			...
			if use.from:objectName() == player:objectName() and player:isAlive() and use.card:hasFlag("ny_tenth_chanjuan") then
		...
		if room:askForSkillInvoke(player, self:objectName(), data) then
			...
			for _, general in sgs.qlist(sgs.Sanguosha:getAllGenerals()) do
				if general:getKingdom() == "shu" then
					...
					if name == "ny_10th_ganfurenmifuren" then
						continue
					end
					local translation = sgs.Sanguosha:translate(name)
					if string.find(translation, "甘夫人") or string.find(translation, "糜夫人") then
						local can = true
...
ny_10th_zhuangpo = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.TargetConfirmed then
			local use = data:toCardUse()
			if use.from == player and use.card:isKindOf("Duel") and use.card:hasFlag("ny_10th_zhuangpo") then
				for _, to in sgs.qlist(use.to) do
					if player:getMark("&ny_10th_shenxuchu_qing") > 0 and (not to:isNude()) and player:isAlive() and to:isAlive() then
						local _data = sgs.QVariant()
						_data:setValue(to)
		...
		if event == sgs.DamageCaused then
			local damage = data:toDamage()
			if damage.card:hasFlag("ny_10th_zhuangpo_" .. player:objectName()) and damage.to:getMark("&ny_10th_shenxuchu_qing") > 0 then
				local log = sgs.LogMessage()
...
ny_10th_zhengqing = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getMark("ny_10th_zhengqing_max_lun") == max and player:getMark("ny_10th_zhengqing_max_lun") >= player:getMark("ny_10th_zhengqing_max_history") then
...
ny_tenth_miyun = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.RoundStart then
			...
			room:obtainCard(player, card, true)
			if room:getCardOwner(card) == player and player:isAlive() then
				room:setCardFlag(card, "ny_tenth_miyun_an")
...
ny_10th_danyin_buff = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getMark("ny_10th_danyin-Clear") ~= 1 then
			return false
		end
		...
		if event == sgs.RoundStart then
			...
			for _, id in sgs.qlist(sgs.Sanguosha:getRandomCards()) do
				...
				if card:isKindOf("Slash") then
					...
					if (card:isKindOf("BasicCard") or card:isNDTrick()) and card:isDamageCard() then
...
local function yanjiaoAutoDivide(card_ids)
	...
	while true do
		if yanjiaoAutoJudgeFinish(plans) then
			break
		end
		for i = 1, num, 1 do
			if plans[i] < 2 then
		...
		while true do
			...
			if choice == 1 then
				for _, id in sgs.qlist(try:getSubcards()) do
					if not last:contains(id) then
						last:append(id)
					end
					if forself:contains(id) then
						forself:removeOne(id)
					end
					if forother:contains(id) then
						forother:removeOne(id)
					end
				end
			elseif choice == 2 then
				for _, id in sgs.qlist(try:getSubcards()) do
					if not forself:contains(id) then
						forself:append(id)
					end
					if last:contains(id) then
						last:removeOne(id)
					end
					if forother:contains(id) then
						forother:removeOne(id)
				...
				for _, id in sgs.qlist(try:getSubcards()) do
					if not forother:contains(id) then
						forother:append(id)
					end
					if last:contains(id) then
						last:removeOne(id)
...
ny_10th_quanmouCard = sgs.CreateSkillCard {
	...
	mute = true,
	filter = function(self, targets, to_select, player)
		return player:inMyAttackRange(to_select) and #targets < 1 and to_select ~= player and to_select:getMark("ny_10th_quanmou_selelcted-PlayClear") == 0
	end,
	on_effect = function(self, effect)
		local room = effect.from:getRoom()
...
ny_10th_quanmou = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		local damage = data:toDamage()
		if damage.to:getMark("&ny_10th_quanmou_first+#" .. player:objectName() .. "-PlayClear") > 0 and event == sgs.DamageCaused then
		...
		if damage.to:getMark("&ny_10th_quanmou_second+#" .. player:objectName() .. "-PlayClear") > 0 and event == sgs.Damage then
			...
			local damages = room:askForPlayersChosen(player, targets, self:objectName(), 0, 3, "@ny_10th_quanmou-damage:" .. damage.to:getGeneralName(), true, true)
...
ny_10th_jianzhuan = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed or event == sgs.CardResponded then
			...
			if (not allchoices) or (#allchoices == 0) then
				return false
			end
			local items = {}
		...
		if event == sgs.EventPhaseEnd then
			...
			if player:getMark("&ny_10th_jianzhuan-PlayClear") < #items then
				return false
			end
			room:sendCompulsoryTriggerLog(player, self:objectName(), true, true)
			local remove = {}
...
ny_10th_fudou = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if use.from == player and use.to:length() == 1 and use.to:at(0):objectName() ~= player:objectName() and use.to:at(0):isAlive() then
			local target = use.to:at(0)
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.RoundStart then
			...
			for _, to in sgs.qlist(tos) do
				if to:isDead() or to:isKongcheng() then
					continue
				end
				if player:isDead() then
					return false
		...
		if event == sgs.CardsMoveOneTime then
			...
			local move = data:toMoveOneTime()
			if move.from and move.from:objectName() == player:objectName() and player:getHandcardNum() < player:getMaxHp() then
...
ny_10th_jujianCard = sgs.CreateSkillCard {
	name = "ny_10th_jujian",
	filter = function(self, targets, to_select, player)
		return #targets < 1 and to_select:getKingdom() == "wei" and to_select:objectName() ~= player:objectName()
	end,
	on_effect = function(self, effect)
		local room = effect.from:getRoom()
	...
	on_effect = function(self, effect)
		...
		if effect.from:isAlive() and effect.to:isAlive() then
			...
			if give then
				...
				if room:getCardOwner(cc:getEffectiveId()) == effect.from and effect.from:isAlive() then
...
ny_tenth_yixianCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, player, targets)
		...
		if choice == "field" then
			...
			local num = {}
			for _, p in sgs.qlist(room:getAlivePlayers()) do
				if p:isDead() or player:isDead() then
					continue
				end
...
ny_tenth_jvewu_selectCard = sgs.CreateSkillCard {
	...
	about_to_use = function(self, room, use)
		...
		for _, name in ipairs(names) do
			...
			if card:isKindOf("Slash") then
				mark = "ny_tenth_jvewu_Slash-Clear"
			end
			if player:getMark(mark) == 0 and card:isAvailable(player) then
...
ny_tenth_jvewu_dividend = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.StartJudge then
			...
			if math.random(1, (player:getMaxHp() + 1)) > (player:getLostHp() + 1) then
				return false
			end
...
ny_tenth_jvewu_change = sgs.CreateFilterSkill {
	...
	view_as = function(self, card)
		local new_card = sgs.Sanguosha:cloneCard(card:objectName(), card:getSuit(), 2)
...
ny_tenth_shuiyanqijun = sgs.CreateTrickCard {
	...
	filter = function(self, targets, to_select, source)
		return not source:isProhibited(to_select, self) and #targets <= sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_ExtraTarget, source, self) + 1
	end,
	--[[feasible = function(self,targets,player)
	...
	about_to_use = function(self, room, use)
		local data = sgs.QVariant()
		data:setValue(use)
		room:setTag("ny_tenth_shuiyanqijun" .. self:toString(), data)
		self:cardOnUse(room, use)
	end,
	on_use = function(self, room, source, targets)
		local use = room:getTag("UseHistory" .. self:toString()):toCardUse()
		local use2 = room:getTag("ny_tenth_shuiyanqijun" .. self:toString()):toCardUse()
	...
	on_effect = function(self, effect)
		...
		room:damage(sgs.DamageStruct(self, effect.from, effect.to, 1, sgs.DamageStruct_Thunder))
...
ny_tenth_shuiyanqijun:setParent(extension)
...
ny_10th_wuwei_clear = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart then
			if player:getPhase() == sgs.Player_Play then
				if not player:hasSkill("ny_10th_wuwei") then
					return false
				end
...
ny_10th_kongwuCard = sgs.CreateSkillCard {
	...
	on_effect = function(self, effect)
		...
		if effect.from:getChangeSkillState(self:objectName()) <= 1 then
			room:setChangeSkillState(effect.from, self:objectName(), 2)
			if effect.to:isDead() or effect.to:isNude() then
				return false
			end
...
ny_10th_jijie = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getMark("ny_10th_jijie_card-Clear") > 0 then
			return false
		end
		local move = data:toMoveOneTime()
		if move.to and move.to:objectName() ~= player:objectName() and move.to_place == sgs.Player_PlaceHand then
			local target = room:findPlayerByObjectName(move.to:objectName())
			if target and target:getPhase() == sgs.Player_NotActive then
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getPhase() == sgs.Player_NotActive then
			...
			for _, p in sgs.qlist(room:findPlayersBySkillName("ny_10th_jijie")) do
				if p:getMark("ny_10th_jijie_recover-Clear") == 0 and p:isWounded() and p:objectName() ~= player:objectName() then
...
ny_10th_huiji = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed then
			local use = data:toCardUse()
			if use.card:isKindOf("AmazingGrace") and use.card:getSkillName() == self:objectName() then
				local reason1 = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_PUT, player:objectName(), self:objectName(), "")
				local move1 = sgs.CardsMoveStruct(player:handCards(), player, nil, sgs.Player_PlaceHand, sgs.Player_PlaceTable, reason1)

				local list = sgs.QVariant()
				list:setValue(player:handCards())
				room:setTag("AmazingGrace" .. use.card:toString(), list)
				room:moveCardsAtomic(move1, true)
			end
		else
			local effect = data:toCardEffect()
			if effect.card:isKindOf("AmazingGrace") and effect.card:getSkillName() == self:objectName() and effect.from:isAlive() then
				local use = room:getUseStruct(effect.card)
				if use.to:last() == effect.to then
					local ids = room:getTag("AmazingGrace" .. effect.card:toString()):toIntList()
					if ids:isEmpty() then
						return
					end
					room:removeTag("AmazingGrace" .. effect.card:toString())
					local reason2 = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GOTCARD, effect.from:objectName(), self:objectName(), "")
					local move2 = sgs.CardsMoveStruct(ids, nil, effect.from, sgs.Player_PlaceTable, sgs.Player_PlaceHand, reason2)
					room:moveCardsAtomic(move2, true)
				end
			end
		end
...
ny_10th_huijiCard = sgs.CreateSkillCard {
	...
	on_effect = function(self, effect)
		...
		if effect.to:isAlive() and effect.to:getHandcardNum() >= room:getAlivePlayers():length() then
			...
			card:setSkillName("_ny_10th_huiji")
			room:useCard(sgs.CardUseStruct(card, effect.to, room:getAlivePlayers()))
			card:deleteLater()
			--[[
            local use = sgs.CardUseStruct(card, effect.to, room:getAllPlayers())
            ...
            local cards = effect.to:handCards()
            ...
            local list = sgs.QVariant()
            list:setValue(cards)
            room:setTag("AmazingGrace"..card:toString(), list)

            room:fillAG(cards)
            for _,p in sgs.qlist(use.to) do
                if p:isAlive() then
                    room:cardEffect(card, effect.to, p, use.to:length()>1)
                end
            end
            room:clearAG()

            cards = room:getTag("AmazingGrace"..card:toString()):toIntList()

            if (not cards:isEmpty()) then
                if effect.to:isAlive() then
                    local reason2 = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GOTCARD, effect.to:objectName(), self:objectName(),"")
                    local move2 = sgs.CardsMoveStruct(cards, nil, effect.to, sgs.Player_PlaceTable, sgs.Player_PlaceHand, reason2)
                    room:moveCardsAtomic(move2,true)
                else
                    local reason2 = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_NATURAL_ENTER, effect.to:objectName(), self:objectName(),"")
                    local move2 = sgs.CardsMoveStruct(cards, nil, nil, sgs.Player_PlaceTable, sgs.Player_DiscardPile, reason2)
                    room:moveCardsAtomic(move2,true)
                end
            end
...
ny_10th_yiyong = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if not (player:canDiscard(player, "he") and damage.to:canDiscard(damage.to, "he")) then
			return false
		end

		local skill_data = sgs.QVariant()
		skill_data:setValue(damage.to)
		if not room:askForSkillInvoke(player, self:objectName(), skill_data) then
			return false
		end
		room:broadcastSkillInvoke(self:objectName())
...
ny_10th_suchou = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed then
			local use = data:toCardUse()
			if player:getMark("&ny_10th_suchou-PlayClear") == 0 then
				return false
			end
			if use.card:isKindOf("SkillCard") then
				return false
...
ny_10th_porong = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed then
			local use = data:toCardUse()
			if use.card:isKindOf("Slash") and not use.card:hasFlag("ComboMoves") then
				local cmc = player:getTag("ComboMovesCard"):toCard()
				if cmc and cmc:isDamageCard() and player:hasSkill(self) then
					for _, to in sgs.qlist(use.to) do
						if player:askForSkillInvoke(self:objectName(), to) then
							room:broadcastSkillInvoke(self:objectName())
							room:setCardFlag(use.card, "ComboMoves")
							--杀对其额外结算一次
							room:setCardFlag(use.card, "ny_10th_porongUse" .. to:objectName())
							--获得该角色及其相邻角色各一张手牌
							local targets = {}
							table.insert(targets, to)
							for _, other in sgs.qlist(room:getOtherPlayers(player)) do
								if other:isAdjacentTo(to) and other ~= to then
									table.insert(targets, other)
								end
							end
							for _, target in ipairs(targets) do
								if target:isAlive() and (not target:isKongcheng()) and player:isAlive() then
		...
		if event == sgs.CardEffect then
			--处理额外结算
			local effect = data:toCardEffect()
			if effect.card:hasFlag("ny_10th_porongUse" .. player:objectName()) then
...
ny_10th_zhenrao = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if use.from == player or use.to:contains(player) then
			local targets = sgs.SPlayerList()
			if use.from:getHandcardNum() > player:getHandcardNum() and use.from:getMark("ny_10th_zhenrao_" .. player:objectName() .. "-Clear") == 0 then
			...
			for _, to in sgs.qlist(use.to) do
				if to:getHandcardNum() > player:getHandcardNum() and to:getMark("ny_10th_zhenrao_" .. player:objectName() .. "-Clear") == 0 then
					targets:append(to)
				end
			end
			if not targets:isEmpty() then
...
ny_10th_sanshi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed then
			...
			local use = data:toCardUse()
			if (not use.card) or (not use.card:getTag("ny_10th_sishi"):toBool()) then
				return false
			end
			room:sendCompulsoryTriggerLog(player, self:objectName(), true, true)
		...
		if event == sgs.CardsMoveOneTime then
			...
			if move.from and move.from:hasSkill(self:objectName()) and (bit32.band(move.reason.m_reason, sgs.CardMoveReason_S_MASK_BASIC_REASON) == sgs.CardMoveReason_S_REASON_USE) then
				return false
			end
			local add = {}
			for _, id in sgs.qlist(move.card_ids) do
...
ny_10th_chenlve = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif player:getPhase() == sgs.Player_Play and player:getMark("ny_10th_chenlve") > 0 then
			local ids = player:getTag("ny_10th_chenlve"):toIntList()
			if room:getCardPlace(ids:last()) == sgs.Player_PlaceTable then
				return
			end
			room:sendCompulsoryTriggerLog(player, self:objectName())
...
if not sgs.Sanguosha:getSkill("ny_10th_jiaoxia_filter") then
	skills:append(ny_10th_jiaoxia_filter)
end
if not sgs.Sanguosha:getSkill("ny_10th_taji") then
	skills:append(ny_10th_taji)
end
if not sgs.Sanguosha:getSkill("ny_10th_qinghuang") then
	skills:append(ny_10th_qinghuang)
end
if not sgs.Sanguosha:getSkill("ny_tenth_dagongche_slash") then
	skills:append(ny_tenth_dagongche_slash)
end
if not sgs.Sanguosha:getSkill("ny_tenth_dagongche_slashtr") then
	skills:append(ny_tenth_dagongche_slashtr)
end
if not sgs.Sanguosha:getSkill("#ny_tenth_dagongche_destory") then
	skills:append(ny_tenth_dagongche_destory)
end
if not sgs.Sanguosha:getSkill("ny_tenth_dagongche_buff") then
	skills:append(ny_tenth_dagongche_buff)
end
if not sgs.Sanguosha:getSkill("ny_10th_paiyi") then
	skills:append(ny_10th_paiyi)
end
if not sgs.Sanguosha:getSkill("ny_10th_shoutan") then
	skills:append(ny_10th_shoutantr)
end
if not sgs.Sanguosha:getSkill("ny_tenth_piliche_target") then
	skills:append(ny_tenth_piliche_target)
end
if not sgs.Sanguosha:getSkill("ny_tenth_piliche_buff") then
	skills:append(ny_tenth_piliche_buff)
end
if not sgs.Sanguosha:getSkill("ny_tenth_piliche_destory") then
	skills:append(ny_tenth_piliche_destory)
end
if not sgs.Sanguosha:getSkill("ny_tenth_piliche_recover") then
	skills:append(ny_tenth_piliche_recover)
end
if not sgs.Sanguosha:getSkill("ny_10th_shouze") then
	skills:append(ny_10th_shouze)
end
if not sgs.Sanguosha:getSkill("ny_10th_aoshi_give") then
	skills:append(ny_10th_aoshi_give)
end
if not sgs.Sanguosha:getSkill("ny_10th_fudou") then
	skills:append(ny_10th_fudou)
end
if not sgs.Sanguosha:getSkill("ny_tenth_wuyou_other") then
	skills:append(ny_tenth_wuyou_other)
end
--用处懂得都懂
if not sgs.Sanguosha:getSkill("global_fuckusetimes") then
	skills:append(global_fuckusetimes)
end
...
sgs.LoadTranslationTable {
	...
	[":ny_tenth_porui2"] = "每轮限一次，其他角色的结束阶段，你可以弃置一张牌并选择一名此回合内失去过牌的另一名其他角色，你视为对该角色依次使用X+1张【杀】（X为其失去的牌数且最多为5）。",
	[":ny_tenth_porui3"] = "每轮限两次，其他角色的结束阶段，你可以弃置一张牌并选择一名此回合内失去过牌的另一名其他角色，你视为对该角色依次使用X+1张【杀】（X为其失去的牌数且最多为5）。",
	...
	[":ny_10th_porong"] = "连招技（伤害牌+【杀】），你可以获得目标角色以及与其的相邻角色的各一张手牌并令此【杀】对其额外结算一次。",
