extension = sgs.Package("zzz")
local debug = true
savedata1 = "save3.json" --存档
readData1 = function()
	local json = require "json"
	local record = io.open(savedata1, "r")
	local t = { Record = {} }
	if record ~= nil then
...
writeData1 = function(t)
	local record = assert(io.open(savedata1, "w"))
	local order = { "Record" }
...
saveRecord1 = function(player, record_type) --record_type: 0. +1 gameplay , 1. +1 win , 2. +1 win & +1 gameplay
...
saveMvp1 = function(player) --record_type: 0. +1 gameplay , 1. +1 win , 2. +1 win & +1 gameplay
	local t = readData1()
...
allrecord2 = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		local function loser(role)
			local tt = t:split("+")
			if not table.contains(tt, role) then
				return true
			end
			return false
		end
...
addToSkills(allrecord2)
winshow2 = sgs.General(extension, "winshow2", "", 0, true, true, false)
winshow2:setGender(sgs.General_Sexless)
winrate2 = sgs.CreateMasochismSkill {
	name = "winrate2",
	on_damaged = function() end,
}
winshow2:addSkill(winrate2)
...
sgs.LoadTranslationTable {
	...
	["winshow2"] = "胜率(3人局)[斗地主]",
	["#winshow2"] = "角色资讯",
	["designer:winshow2"] = "高达杀制作组",
	["cv:winshow2"] = "贴吧：高达杀s吧",
	["illustrator:winshow2"] = "QQ群：565837324",
	["winrate2"] = "胜率",
	[":winrate2"] = g_property,
}
return { extension }
