...
local function getGameModeName(mode_string)
	-- 标准身份局
	if string.find(mode_string, "02p") then
		return "2p_standard"
	elseif string.find(mode_string, "03p") then
		return "3p_standard"
	elseif string.find(mode_string, "04p") then
		return "4p_standard"
	elseif string.find(mode_string, "05p") then
		return "5p_standard"
	elseif string.find(mode_string, "06p") then
		return "6p_standard"
	elseif string.find(mode_string, "08p") then
		return "8p_standard"
	-- 特殊模式
	elseif mode_string == "06_3v3" then
		return "3v3"
	elseif mode_string == "06_XMode" then
		return "1v3"
	elseif mode_string == "04_boss" then
		return "boss"
	elseif mode_string == "08_defense" then
		return "defense"
	elseif mode_string == "02_mini_scene" then
		return "mini_scene"
	else
		-- 其他模式使用原始名称
		return mode_string
...
GeneralStatsRoundRecorder = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Start then
			...
			-- 检查是否被跳过回合
			local is_skipped = player:isSkipped(sgs.Player_Play) or player:hasFlag("Global_ForbidPhase")
