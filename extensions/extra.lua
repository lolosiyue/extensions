...
heg_chenglue = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		...
		if use.to:length() > 1 and not use.card:isKindOf("SkillCard") then
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if use.from:isAlive() and room:askForSkillInvoke(p, self:objectName(), sgs.QVariant(use.from)) then
					room:doAnimate(1, p:objectName(), use.from:objectName())
					room:drawCards(use.from, 1, self:objectName())
				end
				if use.card:hasFlag("heg_chenglue".. p:objectName()) then
					...
					if target and use.card:hasFlag("heg_chenglue".. use.from:objectName()) and target:getMark("@heg_yinyangyu") == 0 and target:getMark("@heg_xianqu") == 0 then
						room:broadcastSkillInvoke(self:objectName())
			...
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				...
				if card then
					...
					elseif choice == "thunder_slash" then
						local targets = sgs.SPlayerList()
						for _, q in sgs.qlist(room:getOtherPlayers(current)) do
							if CanToCard(thunder_slash, current, q) then
								targets:append(q)
							end
						end
						local use = sgs.CardUseStruct()
		...
		if event == sgs.EventPhaseProceeding then
			if player:getPhase() == sgs.Player_Start then
				for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					...
					slash:setSkillName(self:objectName())
					slash:deleteLater()
					if p:objectName() ~= player:objectName() and p:canSlash(player, slash, false) then
						while true do
...
heg_xiongnve = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart then
			if player:getPhase() == sgs.Player_Play then
				local general_list = player:property("heg_shilus_generals"):toString():split("+")
				if #general_list > 0 and room:askForSkillInvoke(player, self:objectName(), sgs.QVariant("start")) then
					local gn 
		...
		if dying.who and dying.who:objectName() == player:objectName() and player:getGeneral2() and player:getMark("@heg_ov_shigong") > 0 then
			...
			if room:askForSkillInvoke(player, self:objectName(), data) then
				...
				if player:getGeneral2():isMale() then
					room:changeHero(player, "heg_sujiang", false, false, true, true, player:getHp())
				elseif player:getGeneral2():isFemale() then
					room:changeHero(player, "heg_sujiangf", false, false, true, true, player:getHp())
				end
...
sgs.LoadTranslationTable{
	...
	["heg_chenglue"] = "成略",
  	[":heg_chenglue"] = "当一名角色使用多目标的牌结算后，你可令其摸一张牌。若你受到过此牌造成的伤害，你可令一名没有国战标记的角色获得一个“阴阳鱼”标记。",
	["heg_shicai"] = "恃才",
	...
	["heg_congcha"] = "聪察",
  	[":heg_congcha"] = "①每名角色限一次，准备阶段，你可选择一名角色，然后直到你的下回合开始限一次，其受到伤害后，其可以令你与其各摸两张牌；否则其失去1点体力。②摸牌阶段，你若没有使用①，你可多摸两张牌。",
	...
	["heg_xiongnve:end"] = "你可以移去两张“戮”，然后直到你的下回合，当你受到其他角色造成的伤害时，此伤害-1。",
	["heg_xiongnve:damage"] = "对其造成伤害时，令此伤害+1",
	["heg_xiongnve:draw"] = "对其造成伤害时，你获得其一张牌",
