...
shootUse = function(self, room, source, targets, isSpreadShoot)
	...
	for _,to in sgs.list(hit_targets)do
		local effect = sgs.CardEffectStruct()
		effect.from = source
		effect.card = self
		effect.multiple = hit_targets:length()>1
		effect.to = to
...
shootEffect = function(self, effect)
	...
	if jink_num > 0 then
		...
		if jink_num == 1 then
			local jink = room:askForCard(target, "jink", "shoot-jink:"..source:objectName()..":"..self:objectName(), data, sgs.Card_MethodResponse, source:isAlive() and source or nil, false, "", false,effect.card)
			...
			for i = jink_num, 1, -1 do
                ...
                asked_jink = room:askForCard(target, "jink", prompt, data, sgs.Card_MethodResponse, source:isAlive() and source or nil, false, "", false,effect.card)
