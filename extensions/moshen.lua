...
function getExceptionDrawpileIds(except_pattern)
	...
	for _, id in sgs.qlist(room:getDrawPile()) do
		if not sgs.Sanguosha:getCard(id):isKindOf(except_pattern) then
			ids:append(id)
		end
	end
...
function invalidateRandomSkill(who, room)
	...
	for _, sk in sgs.qlist(who:getVisibleSkillList()) do
		if who:getMark("Qingcheng" .. sk:objectName()) == 0 then
			table.insert(ava, sk:objectName())
		end
	end
	if #ava > 0 then
...
function activateRandomSkill(who, room)
	...
	for _, sk in sgs.qlist(who:getVisibleSkillList()) do
		if who:getMark("Qingcheng" .. sk:objectName()) > 0 then
			table.insert(inv, sk:objectName())
		end
	end
	if #inv > 0 then
...
function throwRandomEquip(target)
	...
	local room = sgs.Sanguosha:currentRoom()
	for i = 0, 4 do
		if target:hasEquipArea(i) then
			table.insert(area, "EquipItem_" .. i)
		end
	end
...
function hasCorrespondingEquip(who, i)
	if who:hasEquipArea(i) then
		if i == 0 and who:getWeapon() ~= nil then
			return true
		end
		if i == 1 and who:getArmor() ~= nil then
			return true
		end
		if i == 2 and who:getDefensiveHorse() ~= nil then
			return true
		end
		if i == 3 and who:getOffensiveHorse() ~= nil then
			return true
		end
		if i == 4 and who:getTreasure() ~= nil then
			return true
		end
	end
	return false
...
function throwRandomEquipWithExtra(source, target, reason)
	...
	if target:hasEquipArea() then
		...
		room:sendLog(msg)
		if flag then
			room:damage(sgs.DamageStruct(reason, source, target, 2, sgs.DamageStruct_Thunder))
		end
	end
end
...
function lackedEquipNum(target)
	local n = 0
	if target:getWeapon() == nil then
		n = n + 1
	end
	if target:getArmor() == nil then
		n = n + 1
	end
	if target:getOffensiveHorse() == nil then
		n = n + 1
	end
	if target:getDefensiveHorse() == nil then
		n = n + 1
	end
	if target:getTreasure() == nil then
		n = n + 1
	end
	return n
end
...
function isFrozen(yeti) --判断一名角色是否处于“冰冻”状态
	...
	if yeti:getMark("@icebrick") == 0 then
		return false
	end
	local flag = false
...
if not sgs.Sanguosha:getSkill("#akn_BeforeFrozen") then
	sks:append(akn_BeforeFrozen)
end
if not sgs.Sanguosha:getSkill("#akn_AfterFrozen") then
	sks:append(akn_AfterFrozen)
end
if not sgs.Sanguosha:getSkill("frozen_effect") then
	sks:append(frozen_effect)
end
...
hidden_exdraw = sgs.CreateDrawCardsSkill {
	...
	draw_num_func = function(self, player, n)
		...
		if player:getMark("niyuan_over") > 0 then
			x = x + 1
		end
		return n + x
	end,
...
hidden_atkrange = sgs.CreateAttackRangeSkill {
	...
	extra_func = function(self, player, include_weapon)
		...
		if player:getMark("moren_atk") > 0 then
			n = n + 9999
		end
...
local sks = sgs.SkillList()
if not sgs.Sanguosha:getSkill("ms_jingji") then
	sks:append(ms_jingji)
end
if not sgs.Sanguosha:getSkill("ms_bingyu") then
	sks:append(ms_bingyu)
end
if not sgs.Sanguosha:getSkill("ms_tianhui") then
	sks:append(ms_tianhui)
end
if not sgs.Sanguosha:getSkill("ms_moren") then
	sks:append(ms_moren)
end
...
if not sgs.Sanguosha:getSkill("hidden_exdraw") then
	sks:append(hidden_exdraw)
end
if not sgs.Sanguosha:getSkill("hidden_maxcard") then
	sks:append(hidden_maxcard)
end
if not sgs.Sanguosha:getSkill("hidden_extarget") then
	sks:append(hidden_extarget)
end
...
if not sgs.Sanguosha:getSkill("ss_yunchui") then
	skills2:append(ss_yunchui)
end
if not sgs.Sanguosha:getSkill("ss_fengyang") then
	skills2:append(ss_fengyang)
end
if not sgs.Sanguosha:getSkill("ss_ranhun") then
	skills2:append(ss_ranhun)
end
if not sgs.Sanguosha:getSkill("ss_xuanbing") then
	skills2:append(ss_xuanbing)
end
if not sgs.Sanguosha:getSkill("ss_dizai") then
	skills2:append(ss_dizai)
end
if not sgs.Sanguosha:getSkill("ss_yingzhan") then
	skills2:append(ss_yingzhan)
end
if not sgs.Sanguosha:getSkill("ss_lingxu") then
	skills2:append(ss_lingxu)
end
if not sgs.Sanguosha:getSkill("ss_xuese") then
	skills2:append(ss_xuese)
end
if not sgs.Sanguosha:getSkill("ss_shenjian") then
	skills2:append(ss_shenjian)
end
if not sgs.Sanguosha:getSkill("ss_tunshi") then
	skills2:append(ss_tunshi)
end
if not sgs.Sanguosha:getSkill("ss_leilao") then
	skills2:append(ss_leilao)
end
	...
	on_use = function(self, room, source, targets)
		...
		for _, id in sgs.qlist(self:getSubcards()) do
			...
			if c:getSuit() == sgs.Card_Spade then
				if not source:hasSkill("sy_renji") then
					table.insert(skills, "sy_renji")
				end
			elseif c:getSuit() == sgs.Card_Heart then
				if not source:hasSkill("sy_bolue") then
					table.insert(skills, "sy_bolue")
				end
			...
			elseif c:getSuit() == sgs.Card_Diamond then
				if not source:hasSkill("sy_shisha") then
					table.insert(skills, "sy_shisha")
				end
			end
		end
		if #skills > 0 then
			...
			if #skills >= 2 then
				room:loseHp(source, 1, true, source, "ms_guimou")
			end
			if #skills >= 3 then
...
if not sgs.Sanguosha:getSkill("ms_qiyuan") then
	skills:append(ms_qiyuan)
end
if not sgs.Sanguosha:getSkill("ms_yuedong") then
	skills:append(ms_yuedong)
end
	...
	on_use = function(self, room, source, targets)
		...
		elseif suit == sgs.Card_Diamond then
			...
			for _, name in ipairs(sks) do
				if target:hasSkill(name) then
					table.removeOne(sks, name)
				end
			end
			if #sks <= 0 then
				return false
...
sgs.LoadTranslationTable {
	...
	["#yongheng_club"] = '%from 将执行“<font color="yellow"><b>永恒</b></font>”的技能效果：下个回合摸牌阶段摸牌时，摸牌数<font color="yellow"><b>+2</b></fo' .. "nt>",
	...
	on_use = function(self, room, source, targets)
		...
		if not source:hasSkill("sy_canlue") then
			...
			if target:getHandcardNum() >= 2 then
				table.insert(choices, "giveself2")
			end
			...
			if choice == "giveself2" then
				...
				if not source:hasSkill("sy_canlue") then
					...
					sgs.Sanguosha:addTranslationEntry(
						":ms_mimou",
						"" .. string.gsub(sgs.Sanguosha:translate(":ms_mimou"), sgs.Sanguosha:translate(":ms_mimou"), sgs.Sanguosha:translate(":ms_mimouEX"))
					)
				end
			end
		else
			...
			if target:getCards("he"):length() >= 2 then
				table.insert(choices, "throw2cards")
			end
			local choice = room:askForChoice(target, "ms_mimou", table.concat(choices, "+"), _data)
