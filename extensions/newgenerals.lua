...
zhiren = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if player:hasFlag("CurrentPlayer") or player:getMark("&yaner-Self" .. sgs.Player_RoundStart .. "Clear") > 0 then
			...
			if player:getMark("zhi0renRecord-Clear") == 1 and player:getMark("zhirenUsed-Clear") < 1 and player:hasSkill(self) and player:askForSkillInvoke(self, data) then
				player:peiyin(self)
...
yaner = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if
			...
			or move.from:getPhase() ~= sgs.Player_Play
			or not move.is_last_handcard
		...
		for _, p in sgs.qlist(room:getOtherPlayers(from)) do
			if from:isDead() or not from:hasTurn() then
				break
			end
			if p:isDead() or not p:hasSkill(self) or p:getMark("yanerUsed-Clear") > 0 then
				continue
			end
			if not p:askForSkillInvoke(self:objectName() .. "$-1", from) then
				continue
			end
...
thzhuangpo = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.TargetSpecified then
			...
			if use.card:isKindOf("Duel") and table.contains(use.card:getSkillNames(), self:objectName()) and player:hasSkill(self:objectName()) then
		...
		elseif event == sgs.ConfirmDamage then
			local damage = data:toDamage()
			if damage.card and damage.card:isKindOf("Duel") and table.contains(damage.card:getSkillNames(), self:objectName()) and damage.card:hasFlag("thzhuangpoDB") then
				local log = sgs.LogMessage()
...
quedi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.TargetSpecified then
			...
			if not room:hasCurrent() or not player:hasSkill(self) then
				return false
			end
			local use = data:toCardUse()
			...
			if not player:askForSkillInvoke(self, to) then
				return false
			end
			player:peiyin(self)
...
chuifeng = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if event == sgs.DamageInflicted then
			local damage = data:toDamage()
			if damage.card and table.contains(damage.card:getSkillNames(), "chuifeng") and damage.to:getMark("chuifengfrom-Clear") > 0 then
...
chongjianCard = sgs.CreateSkillCard {
	...
	on_validate = function(self, cardUse)
		...
		local room = source:getRoom()
		local user_string = self:getUserString()
		if (string.find(user_string, "slash") or string.find(user_string, "Slash")) and sgs.Sanguosha:getCurrentCardUseReason() ~= sgs.CardUseStruct_CARD_USE_REASON_RESPONSE then
...
qingjue = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if player:getHp() <= to:getHp() or to:hasFlag("Global_Dying") then
			return false
		end
		for _, p in sgs.qlist(room:getOtherPlayers(player)) do
			if p:isDead() or not p:hasSkill(self) or p:objectName() == to:objectName() or p:getMark("qingjue_lun") > 0 then
				return false
			end
			if not p:askForSkillInvoke(self, data) then
				continue
			end
			p:peiyin(self)
...
yilie = sgs.CreatePhaseChangeSkill {
	...
	on_phasechange = function(self, player, room)
		...
		if not player:askForSkillInvoke(self) then
			return false
		end
		player:peiyin(self)
...
mobilefenmingCard = sgs.CreateSkillCard {
	...
	on_effect = function(self, effect)
		...
		if to:isChained() then
			...
			if to:getCards(flags):length() <= 0 then
				return
			end
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.Damaged then
			...
			if
				...
				and player:askForSkillInvoke(self, ToData(damage.from))
			then
				room:broadcastSkillInvoke(self:objectName())
			...
			local damage = data:toDamage()
			if damage.card and damage.card:isKindOf("Slash") and table.contains(damage.card:getSkillNames(), self:objectName()) and damage.to:getEquip(0) == nil then
				player:damageRevises(data, 1)
			end
		end
	end,
...
yongshanxie = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.CardFinished then
			...
			for _, p in sgs.qlist(room:getAlivePlayers()) do
				if p:hasFlag("yongshanxieJink" .. use.card:toString()) then
					p:setFlags("-yongshanxieJink" .. use.card:toString())
...
xiuhao = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		local damage = data:toDamage()
		if damage.to:isDead() or damage.to:objectName() == player:objectName() then
			return false
		end
		local sp = sgs.SPlayerList()
		sp:append(player)
		...
		for _, p in sgs.qlist(sp) do
			if p:isDead() or not p:hasSkill(self) or p:getMark("xiuhaoUsed-Clear") > 0 then
				continue
			end
			--local spp = sp
			...
			if not p:askForSkillInvoke(self, pp) then
				continue
			end
			p:peiyin(self)
...
sujian = sgs.CreatePhaseChangeSkill {
	...
	on_phasechange = function(self, player, room)
		...
		for _, id in sgs.qlist(player:handCards()) do
			if this_turn_ids:contains(id) then
				continue
			end
			cards:append(id)
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.DamageDone then
			...
			if use.card:hasFlag("olfuman_used_slash") or table.contains(use.card:getSkillNames(), "olfuman") then
				...
				for _, p in sgs.qlist(room:getAllPlayers()) do
					if p:isDead() or not p:hasSkill(self) then
						continue
					end
...
olxingbuEffect = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.DrawNCards then
			...
			for i = 1, mark do
				if player:isDead() or player:isKongcheng() then
					break
				end
				sendZhenguLog(player, "olxingbu")
...
secondwenji = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardUsed then
			...
			data:setValue(use)
		elseif player:getPhase() == sgs.Player_Play and player:hasSkill(self) then
			local sp = sgs.SPlayerList()
			for _, p in sgs.qlist(room:getOtherPlayers(player)) do
				if p:isNude() then
					continue
...
secondtunjiang = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif player:getPhase() == sgs.Player_Finish and player:getMark("tunjiang-Clear") < 1 and player:hasSkill(self) and player:askForSkillInvoke(self) then
...
yise = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if not move.from_places:contains(sgs.Player_PlaceEquip) and not move.from_places:contains(sgs.Player_PlaceHand) then
			return false
		end

		local to = room:findPlayerByObjectName(move.to:objectName())
		if not to or to:isDead() then
			return false
		end
...
shunshiMAX = sgs.CreateMaxCardsSkill {
	...
	extra_func = function(self, player)
		...
		if player:hasSkill("ofquanshu") then
...
jinzhaosong = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseEnd then
			...
			for _, p in sgs.qlist(room:getOtherPlayers(player)) do
				if player:getMark("&jzslei") > 0 or player:getMark("&jzsfu") > 0 or player:getMark("&jzssong") > 0 then
					return false
				end
				if player:isKongcheng() or player:isDead() then
					return false
				end
				if p:isDead() or not p:hasSkill(self) then
					continue
				end
				if not p:askForSkillInvoke(self, player) then
					continue
				end
				p:peiyin(self)
				...
				if not mark or player:isDead() then
					continue
				end
		...
		elseif event == sgs.Dying then
			...
			if dying.who:objectName() ~= player:objectName() or player:getMark("&jzslei") <= 0 then
				return false
			end
			if not player:askForSkillInvoke("jinzhaosong_lei", sgs.QVariant("recover")) then
				return false
			end
			player:peiyin(self)
		...
		elseif event == sgs.EventPhaseStart then
			if player:getPhase() ~= sgs.Player_Play or player:getMark("&jzsfu") <= 0 then
				return false
			end
			local targets = sgs.SPlayerList()
			for _, p in sgs.qlist(room:getAlivePlayers()) do
		...
		elseif event == sgs.CardUsed then
			if player:getMark("&jzssong") <= 0 then
				return false
			end
			local use = data:toCardUse()
			...
			for _, p in sgs.qlist(room:getOtherPlayers(player)) do
				if use.to:contains(p) or not player:canSlash(p, use.card) then
					continue
				end
		...
		elseif event == sgs.CardFinished then
			local use = data:toCardUse()
			if not use.card:isKindOf("Slash") or not use.card:hasFlag("jinzhaosong_song") then
				return false
			end
			...
			if num >= 2 or not from or from:isDead() then
				return false
			end
			room:loseHp(from)
			for _, p in sgs.qlist(room:getAllPlayers()) do
		...
		elseif event == sgs.DamageDone then
			local damage = data:toDamage()
			if not damage.card or not damage.card:isKindOf("Slash") or not damage.card:hasFlag("jinzhaosong_song") then
				return false
			end
...
jinlisi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if move.reason.m_reason == sgs.CardMoveReason_S_REASON_USE or move.reason.m_reason == sgs.CardMoveReason_S_REASON_LETUSE then
			...
			if not card or not room:CardInPlace(card, sgs.Player_DiscardPile) then
				return false
			end
...
fusong = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		local skills = {}
		if not target:hasSkill("fengzi", true) then
			table.insert(skills, "fengzi")
		end
		if not target:hasSkill("jizhanw", true) then
			table.insert(skills, "jizhanw")
		end
		if #skills == 0 then
			return false
		end
...
keolhuanfu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardFinished then
			...
			if use.card:isKindOf("Slash") then
				for _, p in sgs.qlist(room:getAllPlayers()) do
					...
					if n > 0 and n == use.card:getMark("keolhuanfuda") then
						p:drawCards(2 * n, self:objectName())
					end
		...
		elseif event == sgs.TargetConfirmed then
			local use = data:toCardUse()
			if use.to:contains(player) and player:hasSkill(self) and use.card:isKindOf("Slash") and not player:isNude() then
		...
		elseif event == sgs.TargetSpecified then
			local use = data:toCardUse()
			if use.from == player and use.card:isKindOf("Slash") and player:hasSkill(self) and not player:isNude() then
				player:setTag("keolhuanfuData", data)
				local askdis = room:askForDiscard(player, self:objectName(), player:getMaxHp(), 1, true, true, "keolhuanfudis", ".", self:objectName())
				if askdis then
					player:peiyin(self)
...
keolqingyiCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, player, targets)
		...
		while #targets > 0 do
			...
			if not (can and player:askForSkillInvoke("keolqingyi", ToData("jixu"), false)) then
				break
			end
		end
		...
		elseif event == sgs.DamageDone then
			...
			if damage.from then
				if not player:hasFlag("CurrentPlayer") then
					room:addPlayerMark(damage.to, damage.from:objectName() .. "keolzeyueda-SelfClear")
				end
				if damage.card and (table.contains(damage.card:getSkillNames(), "keolzeyue") or damage.card:hasFlag("keolzeyue")) then
					player:setTag("keolzeyueDamageDone", ToData(true))
				end
			end
		elseif event == sgs.EventPhaseStart and player:getMark("@keolzeyue") > 0 and player:getPhase() == sgs.Player_Start and player:hasSkill(self) then
			local chooseplayers = sgs.SPlayerList()
			for _, p in sgs.qlist(room:getAllPlayers()) do
				if player:getMark(p:objectName() .. "keolzeyueda-SelfClear") > 0 then
			...
			if person then
				...
				for _, skill in sgs.qlist(gen:getVisibleSkillList()) do
					if (not table.contains(skills_list, skill:objectName())) and (skill:getFrequency() ~= sgs.Skill_Compulsory) then
...
keoltongdu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.EventPhaseEnd and player:getPhase() == sgs.Player_Play and player:hasFlag("keoltongdu") then
...
keolkangrui = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if event == sgs.ConfirmDamage then
			--local damage = data:toDamage()
			if player:getMark("&keolkangrui-Clear") > 0 then
				room:sendCompulsoryTriggerLog(player, self:objectName())
				room:removePlayerMark(player, "&keolkangrui-Clear")
				player:addMark("readydismax-Clear")
				player:damageRevises(data, 1)
			end
		end
		if event == sgs.Damaged then
			--local damage = data:toDamage()
			if player:hasFlag("CurrentPlayer") and player:getMark("keolkangruifirst-Clear") < 1 then
				player:setMark("keolkangruifirst-Clear", 1)
				for _, zy in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					...
					if zy:askForSkillInvoke(self, to_data) then
						...
						if room:askForChoice(zy, self:objectName(), "huifu+damage", to_data) == "huifu" then
							room:recover(player, sgs.RecoverStruct(self:objectName(), zy))
						else
							room:addPlayerMark(player, "&keolkangrui-Clear")
						end
					end
				end
			end
...
keolcuipo = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed then
			...
			if player:getMark("keolcuipo-Clear") == use.card:nameLength() and player:isAlive() and player:hasSkill(self) then
				room:sendCompulsoryTriggerLog(player, self)
				room:setPlayerMark(player, "&keolcuipo-Clear", player:getMark("keolcuipo-Clear"))
		...
		elseif event == sgs.CardResponded then
			...
			if response.m_isUse then
				player:addMark("keolcuipo-Clear")
				if player:getMark("keolcuipo-Clear") == response.m_card:nameLength() and player:isAlive() and player:hasSkill(self) then
					room:setPlayerMark(player, "&keolcuipo-Clear", player:getMark("keolcuipo-Clear"))
					player:drawCards(1, self:objectName())
				end
			end
	...
	on_trigger = function(self, event, player, data)
		...
		if move.from_places:contains(sgs.Player_PlaceHand) and move.from:getMark("&keolqiejian_lun") < 1 and move.from:isAlive() and move.is_last_handcard then
			...
			if player:askForSkillInvoke(self, to_data) then
				room:broadcastSkillInvoke(self:objectName())
				local players = SPlayerList(player, from)
				...
				if not players:contains(from) and player:canDiscard(from, "ej") then
					players:append(from)
				end
				...
				if qh:getMark("&keolnishoujiaohuan-Clear") > 0 then
					...
					if eny then
						...
						local move1 = sgs.CardsMoveStruct(
							...
							eny,
							sgs.Player_PlaceHand,
							sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_SWAP, qh:objectName(), eny:objectName(), self:objectName(), "")
						)
						local move2 = sgs.CardsMoveStruct(
							eny:handCards(),
							qh,
							sgs.Player_PlaceHand,
							sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_SWAP, eny:objectName(), qh:objectName(), self:objectName(), "")
						)
		...
		elseif event == sgs.CardsMoveOneTime then
			...
			then
				...
				if player:canUse(shandian, player) then
					table.insert(result, "shandian")
				end
				if player:getMark("&keolnishoujiaohuan-Clear") < 1 then
					table.insert(result, "jiaohuan")
				end
				if room:askForChoice(player, self:objectName(), table.concat(result, "+")) == "shandian" then
					for i, id in sgs.qlist(move.card_ids) do
						if move.from_places:at(i) == sgs.Player_PlaceEquip and room:getCardPlace(id) == move.to_place then
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed then
			...
			if use.card:isKindOf("Slash") then
				--别人额外目标
				for _, p in sgs.qlist(room:getOtherPlayers(player)) do
					if p:hasSkill(self) and (not use.to:contains(p)) and player:askForSkillInvoke(self, ToData("keolguangao-ask:" .. p:objectName()), false) then
						room:doAnimate(1, player:objectName(), p:objectName())
						use.to:append(p)
						room:sortByActionOrder(use.to)
					end
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseChanging then
			...
			if change.to == sgs.Player_NotActive then
				for _, wq in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					if wq:getMark(self:objectName()) > 0 then
						continue
					end
					local n = 0
					for _, p in sgs.qlist(room:getAlivePlayers()) do
						if p:getMark("keolhuiqiTarget-Clear") > 0 then
							n = n + 1
						end
					end
					if n == 3 and wq:getMark("keolhuiqiTarget-Clear") > 0 or wq:canWake(self:objectName()) then
		...
		elseif event == sgs.TargetConfirmed then
			...
			if use.card:getTypeId() > 0 then
				...
				for _, p in sgs.qlist(room:getAlivePlayers()) do
					if p:getMark(self:objectName()) < 1 and p:hasSkill(self, true) then
						wqs:append(p)
					end
				end
				for _, p in sgs.qlist(use.to) do
					p:addMark("keolhuiqiTarget-Clear")
					if wqs:length() > 0 then
...
keolkenshang = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardFinished then
			local use = data:toCardUse()
			if table.contains(use.card:getSkillNames(), "keolkenshang") or use.card:hasFlag("keolkenshang") then
				if room:getTag("keolkenshangda"):toInt() < use.card:subcardsLength() then
					use.from:drawCards(1, self:objectName())
				end
			...
			elseif n < 0 then
				...
				for _, id in sgs.qlist(dc:getSubcards()) do
					...
					for _, idd in sgs.qlist(room:getDiscardPile()) do
						if id ~= idd and sgs.Sanguosha:getCard(idd):getType() == sgs.Sanguosha:getCard(id):getType() then
							cids:append(idd)
						end
					end
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.BeforeCardsMove then
			...
			if
				...
				and player:getMark("bankeolzenrunuse-Clear") < 1
			...
			then
				...
				for _, p in sgs.qlist(room:getOtherPlayers(player)) do
					if p:getMark("keolzenrun" .. player:objectName()) < 1 and p:getCardCount() >= n then
						canchoose:append(p)
					end
				end
			...
			if eny then
				...
				room:returnToTopDrawPile(ids)
					...
					room:obtainCard(player, dc, false)
					if room:askForChoice(eny, self:objectName(), "mopai+ban", ToData(player)) == "ban" then
						room:addPlayerMark(eny, "keolzenrun" .. player:objectName())
					else
						eny:drawCards(n, self:objectName())
					end
				end
			end
		end
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.EventPhaseStart then
			if player:getPhase() == sgs.Player_Finish and player:hasSkill(self) then
				...
				for _, p in sgs.qlist(room:getAllPlayers()) do
					if p:getMark("beusedzhuyanhp" .. player:objectName()) + p:getMark("beusedzhuyansp" .. player:objectName()) < 2 then
						...
						if p:getMark("beusedzhuyanhp" .. player:objectName()) < 1 then
							if n < 2 then
								room:setPlayerMark(p, "&keolzhuyanhpzero", 1, from)
							else
								room:setPlayerMark(p, "&keolzhuyanhp", n, from)
							end
						end
						n = p:getMark("keolzhuyansp")
						if p:getMark("beusedzhuyansp" .. player:objectName()) < 1 then
							if n < 2 then
								room:setPlayerMark(p, "&keolzhuyanspzero", 1, from)
							else
								room:setPlayerMark(p, "&keolzhuyansp", n, from)
							end
						end
					end
				end
				...
				if theone then
					local choices = {}
					if theone:getMark("beusedzhuyanhp" .. player:objectName()) < 1 then
						table.insert(choices, "hp")
					end
					if theone:getMark("beusedzhuyansp" .. player:objectName()) < 1 then
						table.insert(choices, "sp")
					end
					...
					elseif choice == "sp" then
						...
						if n > 0 then
							room:askForDiscard(theone, self:objectName(), n, n)
						elseif n < 0 then
							theone:drawCards(-n, self:objectName())
						end
					end
				end
...
keolsuji = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardFinished then
			local use = data:toCardUse()
			if table.contains(use.card:getSkillNames(), "keolsuji") or use.card:hasFlag("keolsuji") then
				for _, p in sgs.qlist(room:getAllPlayers()) do
					local to = p:getTag("keolsujiTo"):toPlayer()
					p:removeTag("keolsujiTo")
					if to and use.card:hasFlag("DamageDone_" .. to:objectName()) and to:getCardCount() > 0 then
		...
		if event == sgs.EventPhaseStart and player:isWounded() and player:getPhase() == sgs.Player_Play then
...
keollangdao = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.TargetSpecifying then
			local use = data:toCardUse()
			if use.to:length() == 1 and use.card:isKindOf("Slash") then
				local choices = {}
				for _, ch in sgs.list({ "da", "tar", "res" }) do
					if player:getMark("removekeollangdao" .. ch) < 1 then
						table.insert(choices, ch)
					end
				end
				if #choices < 1 or not player:askForSkillInvoke(self, data) then
					return
				end
				room:broadcastSkillInvoke(self:objectName())
				...
				if choicetwo == "res" or choice == "res" then
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardsMoveOneTime then
			...
			if move.from and move.from:objectName() == player:objectName() and move.card_ids:length() == 1 and move.reason.m_reason == sgs.CardMoveReason_S_REASON_DISMANTLE then --弃置一张手牌
				...
				if who and move.from_places:contains(sgs.Player_PlaceHand) then
					who:addMark("keolgoudedis-Clear")
				end
			end
			if
				move.to
			...
			then
				player:addMark("keolgoudedraw-Clear")
			end --摸一张牌
		elseif event == sgs.CardUsed then
			local use = data:toCardUse()
			if use.card:isKindOf("Slash") and use.card:isVirtualCard() and use.card:subcardsLength() < 1 then
				player:addMark("keolgoudeslash-Clear")
			end
		elseif event == sgs.KingdomChanged then
			player:addMark("keolgoudekingdom-Clear")
		elseif event == sgs.EventPhaseChanging then
			local change = data:toPhaseChange()
			if change.to ~= sgs.Player_NotActive then
				return
				...
				if p:hasSkill(self) then
					...
					for _, ch in sgs.list({ "draw", "dis", "slash", "kingdom" }) do
						...
						for _, pp in sgs.qlist(room:getAlivePlayers()) do
							if pp:getKingdom() ~= p:getKingdom() then
								continue
							end
							if pp:getMark("keolgoude" .. ch .. "-Clear") > 0 then
								has = true
								break
							end
						end
...
keolzhenyingCard = sgs.CreateSkillCard {
	...
	will_throw = false,
	filter = function(self, targets, to_select, player)
		return #targets < 1 and to_select:objectName() ~= player:objectName() and to_select:getHandcardNum() <= player:getHandcardNum()
	end,
	on_use = function(self, room, player, targets)
		local aps, hns = {}, {}
		...
		for _, target in sgs.list(aps) do
			...
			if n > 0 then
				room:askForDiscard(target, "keolzhenying", n, n)
			elseif n < 0 then
				target:drawCards(-n, "keolzhenying")
			end
		end
			...
			if change.to == sgs.Player_NotActive then
				for _, dsw in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					for _, p in sgs.qlist(room:getOtherPlayers(dsw)) do
						if p:getHandcardNum() == dsw:getMark("&keolfudao") and p:getMark("&keolfudao+#" .. dsw:objectName()) > 0 and dsw:askForSkillInvoke(self, ToData(p)) then
							room:broadcastSkillInvoke(self:objectName())
							local aps = SPlayerList(p, dsw)
							room:sortByActionOrder(aps)
							room:drawCards(aps, 1, self:objectName())
						end
					end
...
keolfengyan = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.Damaged then
			...
			if damage.from and damage.from ~= player then
				...
				if room:askForChoice(player, self:objectName(), "memo+hemo", ToData(damage.from)) == "memo" then
					player:drawCards(1, self:objectName())
					if damage.from:isDead() then
						return
					end
					local card = room:askForExchange(player, self:objectName(), 1, 1, true, "kenewmijichoose:" .. damage.from:objectName(), false)
					if card then
						room:giveCard(player, damage.from, card, self:objectName())
					end
				elseif damage.from:isAlive() then
		...
		elseif event == sgs.CardResponded then
			local res = data:toCardResponse()
			if res.m_card:getTypeId() > 0 and res.m_toCard and res.m_who and res.m_who ~= player then
				room:sendCompulsoryTriggerLog(player, self)
				if room:askForChoice(player, self:objectName(), "memo+hemo", ToData(res.m_who)) == "memo" then
					player:drawCards(1, self:objectName())
					if res.m_who:isDead() then
						return
					end
					local card = room:askForExchange(player, self:objectName(), 1, 1, true, "kenewmijichoose:" .. res.m_who:objectName(), false)
					if card then
						room:giveCard(player, res.m_who, card, self:objectName())
					end
				elseif res.m_who:isAlive() then
...
keolbixinCard = sgs.CreateSkillCard {
	...
	feasible = function(self, targets, player)
		...
		return use_card:targetFixed() or use_card:targetsFeasible(qtargets, player)
	end,
	on_validate = function(self, use)
		local room = use.from:getRoom()
		local results = {}
		for _, pn in sgs.list({ "basic", "trick", "equip" }) do
			if use.from:getMark("keolbixin_" .. pn) < 3 then
				table.insert(results, pn)
			end
		end
		if #results < 1 then
			return nil
		...
		for _, h in sgs.qlist(use.from:getHandcards()) do
			if h:getType() == result then
				use_card:addSubcard(h)
			end
		end
		return use_card
	end,
	on_validate_in_response = function(self, from)
		...
		local results = {}
		for _, pn in sgs.list({ "basic", "trick", "equip" }) do
			if from:getMark("keolbixin_" .. pn) < 3 then
				table.insert(results, pn)
			end
		end
		if #results < 1 then
...
keolbixinVS = sgs.CreateViewAsSkill {
	...
	enabled_at_response = function(self, player, pattern)
		...
		if sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_RESPONSE or player:getMark("keolximotimes") < 3 then
			return false
		end
		local can = false
		for _, pn in sgs.list({ "basic", "trick", "equip" }) do
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.RoundEnd then --清除记录
			for _, m in sgs.list(player:getMarkNames()) do
				if m:startsWith("keolbixin_guhuo_remove_") then
		...
		elseif event == sgs.EventPhaseStart then
			if (player:getPhase() == sgs.Player_Start) or (player:getPhase() == sgs.Player_Finish) then
				for _, zz in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					...
					if
						(t < 1 or player == zz) --第一次修改后不能在其他回合发动发动
						and (
							t < 2 and player:getPhase() == sgs.Player_Start --准备阶段不能被删除
							or t < 3 and player:getPhase() == sgs.Player_Finish
						) --结束阶段不能被删除
					then
						local results = {}
						for _, pn in sgs.list({ "basic", "trick", "equip" }) do
							if zz:getMark("keolbixin_" .. pn) < 1 then
								table.insert(results, pn)
							end
						end
						if #results < 1 then
						...
						for _, id in sgs.qlist(room:getAvailableCardList(zz, "basic", "keolbixin")) do
							...
							--是基本牌，能使用，且没有被本轮记录过
							if zz:getMark("keolbixin_guhuo_remove_" .. tcard:objectName()) < 1 then
								table.insert(choices, tcard:objectName())
							end
						end
						if #choices < 1 or not zz:askForSkillInvoke(self, ToData(table.concat(choices, "+"))) then
							continue
						end
						...
						if room:askForUseCard(zz, "@@keolbixin!", "keolbixin0:" .. result .. ":" .. choice) then
							continue
						end
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardUsed then
			...
			if use.card:getTypeId() > 1 then
				room:setCardFlag(use.card, "keolgangshuBf")
			end
		elseif event == sgs.DrawNCards then
			local draw = data:toDraw()
			if draw.reason ~= "draw_phase" or player:getMark("keolgangshuMp") < 1 then
				return
			end
		...
		elseif event == sgs.CardFinished then
			local use = data:toCardUse()
			if not use.card:hasFlag("keolgangshuBf") then
				return false
			end
			...
			if player:getAttackRange() < 5 then
				table.insert(choices, "Gjfw")
			end
			if 2 + player:getMark("keolgangshuMp") < 5 then
				table.insert(choices, "Mp")
			end
			if 1 + sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_Residue, player, dummyCard()) < 5 then
				table.insert(choices, "Sha")
			end
		...
		if event == sgs.Damaged then
			...
			if one then
				...
				while one:isAlive() do
					...
					--额定摸牌数默认2，不太严谨
					if
						one:getHandcardNum() == 2 + player:getMark("&keolgangshu_mp")
	...
	about_to_use = function(self, room, use)
		if self:getUserString() ~= "@@olxiaofan2" then
			...
			for _, id in sgs.list(ids) do
				if sgs.Sanguosha:getCard(id):isAvailable(use.from) then
					table.insert(cans, id)
				end
			end
...
olxiaofanCard = sgs.CreateSkillCard {
	...
	feasible = function(self, targets, player)
		...
		return dc:targetFixed() or dc:targetsFeasible(qtargets, player)
	end,
	on_validate = function(self, use)
		local n = 1
		local room = use.from:getRoom()
...
olxiaofanvs = sgs.CreateViewAsSkill {
	...
	view_filter = function(self, selected, to_select)
		...
		if pattern == "@@olxiaofan1" then
			...
			return table.contains(pattern, to_select:toString()) and sgs.Self:getPileName(to_select:getId()) == "#olxiaofan"
		end
		if pattern == "@@olxiaofan2" then
			...
			for _, pn in sgs.list(pattern:split("+")) do
				if to_select:sameNameWith(pn) then
					return sgs.Self:getPileName(to_select:getId()) == "#olxiaofan" and not sgs.Self:isLocked(to_select)
				end
			end
		end
	end,
...
olruishi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.CardUsed then
			local use = data:toCardUse()
			if use.card:getTypeId() > 0 then
				if use.card:getNumber() == 1 or use.card:getNumber() == 11 or use.card:getNumber() == 12 or use.card:getNumber() == 13 then
					room:sendCompulsoryTriggerLog(player, self)
		...
		elseif event == sgs.EventPhaseChanging then
			local change = data:toPhaseChange()
			if change.to ~= sgs.Player_NotActive or player:getMark("olruishiDamageCard-Clear") < 3 then
				return
			end
...
olruishibf = sgs.CreateCardLimitSkill {
	...
	limit_pattern = function(self, player)
		if player:hasSkill("olruishi") then
...
zhiming = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		room:sendCompulsoryTriggerLog(player, self)
		player:drawCards(1, self:objectName())
		if player:isDead() or player:isNude() then
			return false
		end
...
xingbuEffect = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.DrawNCards then
			...
			local use = data:toCardUse()
			if use.card:isKindOf("SkillCard") or player:getMark("xingbuEffect-PlayClear") > 0 then
				return false
			end
			player:addMark("xingbuEffect-PlayClear")
			for i = 1, mark do
				if player:isDead() or not player:canDiscard(player, "he") then
					break
				end
				sendZhenguLog(player, "xingbu")
...
kemobileyijin = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.DamageInflicted then
			...
			if damage.nature ~= sgs.DamageStruct_Thunder and damage.to:getMark("diskeyijin_tongshen-SelfClear") > 0 and player:getMark("@diskeyijin_tongshen") > 0 then
				room:sendCompulsoryTriggerLog(player, "diskeyijin_tongshen")
				return player:damageRevises(data, -damage.damage)
			end
		elseif
			event == sgs.EventPhaseStart --贾凶
			and player:getPhase() == sgs.Player_Play
			and player:getMark("keyijin_guxiong-SelfClear") > 0
			and player:getMark("@keyijin_guxiong") > 0
		then
			room:sendCompulsoryTriggerLog(player, "keyijin_guxiong")
			room:loseHp(player)
			room:addMaxCards(player, -3)
		elseif
			event == sgs.DrawNCards --V我50
			and player:getMark("keyijin_wushi-SelfClear") > 0
			and player:getMark("@keyijin_wushi") > 0
		then
			local draw = data:toDraw()
			if draw.reason ~= "draw_phase" then
				return
		...
		elseif event == sgs.EventPhaseChanging then
			local change = data:toPhaseChange()
			if
				(change.to == sgs.Player_Play or change.to == sgs.Player_Discard) --金迷
				and player:getMark("keyijin_jinmi-SelfClear") > 0
				and player:getMark("@keyijin_jinmi") > 0
			then
			...
			elseif
				change.to == sgs.Player_Draw --拥蔽
				and player:getMark("keyijin_yongbi-SelfClear") > 0
				and player:getMark("@keyijin_yongbi") > 0
			then
			...
			elseif change.to == sgs.Player_NotActive then
				if
					player:getMark("keyijin_houren-SelfClear") > 0 --厚任
					and player:getMark("@keyijin_houren") > 0
				then
				...
				for _, m in sgs.list(yijinMarks) do
					if player:getMark(m .. "-SelfClear") > 0 then
						player:loseMark("@" .. m)
					end
				end
			end
...
nosxingtu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed then
			...
			if n < 1 or use.card:isKindOf("SkillCard") then
				return false
			end
...
local function gainmobiledaming(player, n)
	...
	for _, m in sgs.list(player:getMarkNames()) do
		if m:startsWith("&mobiledamingNum+") then
			room:setPlayerMark(player, m, 0)
		end
	end
...
mobiledamingCard = sgs.CreateSkillCard {
	...
	mute = true,
	filter = function(self, targets, to_select, from)
		return #targets < 1 and to_select:hasSkill("mobiledaming") and from:getMark(to_select:objectName() .. "mobiledamingUse-PlayClear") < 1
	end,
	on_use = function(self, room, source, targets)
		...
		for _, p in sgs.list(targets) do
			...
			if source:isDead() or p:isDead() then
				break
			end
...
mobilexiaoni = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif player:getPhase() == sgs.Player_Play then
			for _, pn in sgs.list(patterns()) do
				local dc = dummyCard(pn)
				if dc and dc:isDamageCard() then
					continue
				end
...
jiebing = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if to and to:getCardCount() > 0 then
			...
			if player:handCards():contains(c:getEffectiveId()) and c:isKindOf("EquipCard") and player:canUse(c, player) then
...
hannanCard = sgs.CreateSkillCard {
	...
	will_throw = false,
	filter = function(self, targets, to_select, from)
		return #targets < 1 and to_select ~= from and from:canPindian(to_select)
	end,
	on_use = function(self, room, source, targets)
		for _, p in sgs.list(targets) do
...
mobileshushen = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardsMoveOneTime then
			local move = data:toMoveOneTime()
			if
				not room:getTag("FirstRound"):toBool()
				and move.card_ids:length() >= 2
				and move.to_place == sgs.Player_PlaceHand
				and move.to:objectName() == player:objectName()
				and player:getMark("mobileshushen_move-Clear") < 1
			then
				local players = sgs.SPlayerList()
				for _, p in sgs.qlist(room:getOtherPlayers(player)) do
					if p:isWounded() then
...
mobilezhijie = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
			if event == sgs.EventPhaseStart then
				if player:getPhase() == sgs.Player_Play and not player:isKongcheng() and p:getMark("mobilezhijie_lun") < 1 and p:askForSkillInvoke(self:objectName(), player) then
					room:broadcastSkillInvoke(self:objectName())
			...
			elseif event == sgs.EventPhaseEnd then
				if player:getPhase() == sgs.Player_Play then
					if
						player:getMark("mobilezhijie+equip-SelfPlayClear") > player:getMark("mobilezhijie+equip_dis-SelfPlayClear")
						or player:getMark("mobilezhijie+basic-SelfPlayClear") > player:getMark("mobilezhijie+basic_dis-SelfPlayClear")
						or player:getMark("mobilezhijie+trick-SelfPlayClear") > player:getMark("mobilezhijie+trick_dis-SelfPlayClear")
					then
						room:sendCompulsoryTriggerLog(p, self)
						local aps = SPlayerList(p, player)
						room:sortByActionOrder(aps)
...
olwanshaInvalidity = sgs.CreateInvaliditySkill { --成功运行过两次，再后来就会崩了。。
	...
	local al = player:getAliveSiblings(true)
	local wansha = false
...
olhaoshi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.AfterDrawNCards then
			local draw = data:toDraw()
			if draw.reason ~= "draw_phase" or not player:hasFlag("olhaoshi") then
				return
			end
			player:setFlags("-olhaoshi")
...
olhaoshiEffect = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if not use.card:isKindOf("Slash") and not use.card:isNDTrick() then
			return false
		end
		for _, p in sgs.qlist(room:getOtherPlayers(player)) do
			if player:isDead() then
			...
			if p:isDead() or p:getMark("&olhaoshi+#" .. player:objectName()) <= 0 or p:isKongcheng() then
				continue
			end
...
oldimengCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		local exchangeMove = sgs.CardsMoveList()
		local move1 = sgs.CardsMoveStruct(a:handCards(), b, sgs.Player_PlaceHand, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_SWAP, a:objectName(), b:objectName(), "oldimeng", ""))
		local move2 = sgs.CardsMoveStruct(b:handCards(), a, sgs.Player_PlaceHand, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_SWAP, b:objectName(), a:objectName(), "oldimeng", ""))
		exchangeMove:append(move1)
		exchangeMove:append(move2)
		room:moveCardsAtomic(exchangeMove, false)
		room:sendLog(log)
...
keolshuangxiong = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.EventPhaseEnd and player:getPhase() == sgs.Player_Draw and player:hasSkill(self) then
...
keolqinxue = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if (player:getPhase() == sgs.Player_Start or player:getPhase() == sgs.Player_Finish) and (player:getHandcardNum() - player:getHp() >= 2 or player:canWake(self:objectName())) then
			local room = player:getRoom()
			room:sendCompulsoryTriggerLog(player, self)
			room:doSuperLightbox(player, self:objectName())
			room:addPlayerMark(player, self:objectName())
			if room:changeMaxHpForAwakenSkill(player, -1, self:objectName()) then
				if player:isWounded() and room:askForChoice(player, self:objectName(), "keolqinxue1+keolqinxue2") == "keolqinxue1" then
					room:recover(player, sgs.RecoverStruct(self:objectName(), player))
				else
					player:drawCards(2, self:objectName())
				end
...
keolbotu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardsMoveOneTime then
			local move = data:toMoveOneTime()
			if move.to_place == sgs.Player_DiscardPile and player:hasSkill(self:objectName(), true) and player:hasFlag("CurrentPlayer") then
				for _, id in sgs.qlist(move.card_ids) do
		...
		elseif event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_NotActive then
			...
			if num >= 4 and player:hasSkill(self) and player:getMark("keolbotuuse_lun") < math.min(room:getAlivePlayers():length(), 3) and room:askForSkillInvoke(player, self:objectName(), data) then
				room:broadcastSkillInvoke(self:objectName())
...
keolshizhanCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		...
		duel:deleteLater()
		return #targets < 1 and to_select ~= player and duel:targetFilter(sgs.PlayerList(), player, to_select)
	end,
	on_use = function(self, room, player, targets)
		for _, p in sgs.list(targets) do
			local duel = sgs.Sanguosha:cloneCard("duel")
...
nosjinwanyiCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		...
		local card = dummyCard(self:getUserString())
		card:addSubcard(self)
...
nosjinwanyi = sgs.CreateOneCardViewAsSkill {
	...
	view_filter = function(self, to_select)
		...
		local ec = sgs.Sanguosha:getEngineCard(to_select:getId())
		if ec:property("YingBianEffects"):toString() == "" then
			return false
		end
...
nosjinxuanbei = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.GameStart then
			local ids = sgs.IntList()
			for _, id in sgs.qlist(room:getDrawPile()) do
				if sgs.Sanguosha:getEngineCard(id):property("YingBianEffects"):toString() == "" then
					continue
				end
				ids:append(id)
			end
			...
			local use = data:toCardUse()
			if use.card:isVirtualCard() or use.card:getSkillName() ~= "" then
				return false
			end
			if sgs.Sanguosha:getEngineCard(use.card:getEffectiveId()):property("YingBianEffects"):toString() == "" then
				return false
			end
			if not room:CardInPlace(use.card, sgs.Player_DiscardPile) then
				return false
			end
...
zhi2shengxi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseProceeding then
			if player:getPhase() == sgs.Player_Start and player:askForSkillInvoke(self) then
				...
				for i, c in sgs.list(PatternsCard("Tiaojiyanmei", true, true)) do
					if room:getCardOwner(c:getEffectiveId()) then
						continue
					end
					player:obtainCard(c)
					break
				end
			elseif player:getPhase() == sgs.Player_Finish and player:getMark("zhi2shengxiUse-Clear") > 0 and player:getMark("zhi2shengxiDamage-Clear") < 1 and player:askForSkillInvoke(self) then
				room:broadcastSkillInvoke("zhishengxi")
				for _, c in sgs.list(PatternsCard(sgs.ZhinangClassName, true, true)) do
					if room:getCardPlace(c:getEffectiveId()) == sgs.Player_PlaceTable or room:getCardOwner(c:getEffectiveId()) then
						continue
					end
					player:obtainCard(c)
					break
		...
		elseif event == sgs.Damage and player:hasFlag("CurrentPlayer") then
			player:addMark("zhi2shengxiDamage-Clear")
		elseif event == sgs.CardFinished and player:hasFlag("CurrentPlayer") then
			local use = data:toCardUse()
			if use.card:getTypeId() > 0 then
	...
	on_use = function(self, room, source)
		...
		while source:isAlive() do
			...
			if room:getCardPlace(judge.card:getEffectiveId()) == sgs.Player_PlaceJudge then
				self:addSubcard(judge.card:getEffectiveId())
			end

			if judge:isGood() and source:isAlive() and source:getMaxHp() < 10 and source:askForSkillInvoke(self:getSkillName()) then
...
huishiiCard = sgs.CreateSkillCard {
	...
	on_effect = function(self, effect)
		...
		for _, sk in sgs.qlist(target:getVisibleSkillList()) do
			if sk:getFrequency(target) ~= sgs.Skill_Wake or target:getMark(sk:objectName()) > 0 then
				continue
			end
...
local function isSpecialOne(player, name)
	if string.find(player:getGeneralName(), name) then
		return true
	end
	if player:getGeneral2() then
		if string.find(player:getGeneral2Name(), name) then
			return true
		end
	end
	return false
...
tianzuo = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.GameStart then
			...
			if not effect.card:isKindOf("Qizhengxiangsheng") then
				return false
			end
			local log = sgs.LogMessage()
			log.type = "#WuyanGooD"
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.TargetConfirming then
			...
			for _, id in sgs.qlist(sgs.Sanguosha:getRandomCards()) do
				...
				if not c:isKindOf("TrickCard") or table.contains(tricks, c:objectName()) then
					continue
				end
...
Qizhengxiangsheng = sgs.CreateTrickCard {
	...
	available = function(self, player)
		for _, to in sgs.list(player:getAliveSiblings()) do
			if self:targetFilter(sgs.PlayerList(), to, player) then
	...
	on_effect = function(self, effect)
		...
		if choice:startsWith("zhengbing") then
			if not (card and card:isKindOf("Jink")) then
				if from:isDead() or to:isNude() then
					return
				end
...
xin2chuhai = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if event == sgs.PindianVerifying then
			...
			if pindian.reason ~= "xin2chuhai" or pindian.from:objectName() ~= player:objectName() then
				return false
			end
		...
		elseif event == sgs.CardsMoveOneTime then
			local move = data:toMoveOneTime()
			if move.to_place == sgs.Player_PlaceEquip and move.to:objectName() == player:objectName() and player:getEquips():length() >= 3 then
				local log = sgs.LogMessage()
		...
		elseif event == sgs.Pindian then
			local pindian = data:toPindian()
			if pindian.reason ~= "xin2chuhai" or pindian.success then
				return false
			end
...
xinzhangming = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.Damage then
			local damage = data:toDamage()
			if player:getMark("xinzhangming-Clear") > 0 or not player:hasTurn() or damage.to == player then
				return false
			end
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.PreCardUsed then
			...
			if use.card:isKindOf("Slash") and not use.card:isVirtualCard() and player:getPhase() == sgs.Player_Play then
				player:addMark("xinliubingUse-PlayClear")
				if player:getMark("xinliubingUse-PlayClear") < 2 and player:hasSkill(self) then
					room:sendCompulsoryTriggerLog(player, self)
		...
		elseif event == sgs.CardFinished then
			local use = data:toCardUse()
			if use.card:isKindOf("Slash") and use.card:isBlack() and not use.card:hasFlag("DamageDone") and not use.card:isVirtualCard() and player:getPhase() == sgs.Player_Play then
				for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					if p == player or room:getCardOwner(use.card:getEffectiveId()) then
						continue
					end
					room:sendCompulsoryTriggerLog(p, self)
...
dulie = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.GameStart then
			...
			local use = data:toCardUse()
			if not use.card:isKindOf("Slash") or not use.to:contains(player) then
				return false
			end
			if not use.from or use.from:isDead() or use.from:getMark("&stscdlwei") > 0 then
				return false
			end
			room:sendCompulsoryTriggerLog(player, self)
...
powei = sgs.CreateTriggerSkillV2 {
    ...
    on_effect = function(self, event, room, player, ctx)
		...
		if event == sgs.DamageCaused then
			local damage = data:toDamage()
			if not damage.card or not damage.card:isKindOf("Slash") or not damage.by_user or damage.to:isDead() or damage.to:getMark("&stscdlwei") <= 0 then
				return false
			end
...
dangmoSlash = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		local use = data:toCardUse()
		if not use.card:isKindOf("Slash") or player:getMark("dangmo-PlayClear") > 0 then
			return false
		end
...
fuhaisc = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.CardResponded then
			local res = data:toCardResponse()
			if res.m_card:isKindOf("SkillCard") or not res.m_isUse then
				return false
			end
			local sp = sgs.SPlayerList()
			for _, p in sgs.qlist(room:getAllPlayers()) do
		...
		elseif event == sgs.TargetSpecifying then
			if not room:hasCurrent() or player:getMark("fuhaisc_draw-Clear") > 1 then
				return false
			end
			local use = data:toCardUse()
			if use.card:isKindOf("SkillCard") then
		...
		elseif event == sgs.Death then
			local death = data:toDeath()
			if not death.who or death.who:objectName() == player:objectName() then
				return false
			end
...
pinghe = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if player:isKongcheng() or player:getMaxHp() < 2 then
			return false
		end
		room:sendCompulsoryTriggerLog(player, self)
...
tenyearjianshuCard = sgs.CreateSkillCard {
	...
	on_effect = function(self, effect)
		...
		local targets = sgs.SPlayerList()
		for _, p in sgs.qlist(room:getOtherPlayers(from)) do
			if p ~= to and to:canPindian(p) then
				targets:append(p)
			end
		end
		...
		if winner then
			for _, c in sgs.qlist(RandomList(winner:getCards("he"))) do
				if winner:canDiscard(winner, c:getId()) then
					room:throwCard(c, self:objectName(), winner)
				end
			end
		end
...
secondkangge = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseStart then
			...
			if player:getMark("secondkangge_Round-Keep") ~= 1 then
				return false
			end
		...
		elseif event == sgs.Dying then
			if player:getMark("secondkangge_lun") > 0 then
				return false
			end
			local dying = data:toDying()
			if dying.who:getMark("&secondkangge+#" .. player:objectName()) <= 0 then
				return false
			end
			if not player:askForSkillInvoke(self, dying.who) then
				return false
			end
			player:peiyin(self)
		...
		elseif event == sgs.Death then
			local death = data:toDeath()
			if death.who:getMark("&secondkangge+#" .. player:objectName()) <= 0 then
				return false
			end
			room:sendCompulsoryTriggerLog(player, self)
			...
			if not room:hasCurrent() or player:getMark("secondkangge-Clear") > 2 then
				return false
			end
			local move = data:toMoveOneTime()
			if not move.to or move.to:getMark("&secondkangge+#" .. player:objectName()) <= 0 or move.to:hasFlag("CurrentPlayer") then
				return false
			end
...
secondjielie = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		for _, p in sgs.qlist(room:getAllPlayers()) do
			if p:isDead() or p:getMark("&secondkangge+#" .. player:objectName()) <= 0 then
				continue
			end
...
gongxiu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if player:getMark("jinghe_Used-Clear") <= 0 then
			return false
		end
		...
		if #choices == 0 or not player:askForSkillInvoke(self) then
			return false
		end
		player:peiyin(self)
		local choice = room:askForChoice(player, self:objectName(), table.concat(choices, "+"))
	...
	on_use = function(self, room, source, targets)
		...
		for _, p in ipairs(targets) do
			...
			for _, sk in ipairs(tianshu_skills) do
				if p:hasSkill(sk, true) or not sgs.Sanguosha:getSkill(sk) then
					continue
				end
				table.insert(new_tianshu_skills, sk)
			end
			if #new_tianshu_skills <= 0 then
				continue
			end
...
jingheVS = sgs.CreateViewAsSkill {
	...
	view_filter = function(self, selected, to_select)
		...
		for _, c in ipairs(selected) do
			if to_select:sameNameWith(c) then
				return false
			end
		end
		return true
...
jinghe = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		for _, p in sgs.qlist(room:getAllPlayers()) do
			...
			local lose = {}
			for _, sk in ipairs(p:getTag("jinghe_GetSkills_" .. player:objectName()):toString():split(",")) do
...
nhlundao = sgs.CreateMasochismSkill {
	...
	on_damaged = function(self, player, damage)
		...
		if fhand > hand and player:canDiscard(from, "he") then
			if not player:askForSkillInvoke(self, from) then
				return
			end
			player:peiyin(self)
...
nhyanzhengCard = sgs.CreateSkillCard {
	...
	on_effect = function(self, effect)
		local room = effect.from:getRoom()
		room:damage(sgs.DamageStruct("nhyanzheng", effect.from, effect.to))
	end,
}
...
nhyanzheng = sgs.CreatePhaseChangeSkill {
	...
	on_phasechange = function(self, player, room)
		...
		for _, c in sgs.qlist(player:getCards("he")) do
			if c:getEffectiveId() == card:getEffectiveId() or not player:canDiscard(player, c:getEffectiveId()) then
				continue
			end
			slash:addSubcard(c)
		end
...
tenyearyunjiu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardsMoveOneTime then
			local move = data:toMoveOneTime()
			if (move.from_places:contains(sgs.Player_PlaceHand) or move.from_places:contains(sgs.Player_PlaceEquip)) and move.from:isDead() and move.reason.m_skillName == "bury" then
				local ids = sgs.IntList()
				for _, id in sgs.qlist(move.card_ids) do
...
sgs.LoadTranslationTable {
	...
	["$tenyearmoshou1"] = "好战必亡，此亘古之理",
	["$tenyearmoshou2"] = "天下汹汹之势，恪守方得自保",
	["tenyearyunjiu"] = "运柩",
	[":tenyearyunjiu"] = "当其他角色死亡时，你可以将该角色弃置的任意一张牌交给另一名其他角色，然后你加1点体力上限并回复1点体力。",
	["tenyearyunjiu0"] = "你可以发动“运柩”选择一名其他角色将其中一张牌交给其",
	["$tenyearyunjiu1"] = "此吾主之柩，请诸君勿扰",
	["$tenyearyunjiu2"] = "故者为大，尔等欲欺大者乎？",
	["~tenyear_yuanyin"] = "臣不负忠，虽死如是",
}
...
keolshanduan = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.EventPhaseStart then
			...
			if player:getPhase() == sgs.Player_Draw then
				if #shanduanNum[player:objectName()] < 1 then
					return
				end
				room:sendCompulsoryTriggerLog(player, self)
			...
			elseif player:getPhase() == sgs.Player_Play then
				...
				if choices0 > 0 then
					room:addSlashCishu(player, choices0 - 1)
				end
			elseif player:getPhase() == sgs.Player_Discard then
				if #shanduanNum[player:objectName()] < 1 then
					return
...
keolyilieCard = sgs.CreateSkillCard {
	...
	on_validate = function(self, card_use)
		...
		if self:getUserString() == "slash" and sgs.Sanguosha:getCurrentCardUseReason() ~= sgs.CardUseStruct_CARD_USE_REASON_RESPONSE then
		...
		if self:subcardsLength() == 1 then
			card = sgs.Sanguosha:cloneCard(sgs.Sanguosha:getCard(self:getSubcards():first()))
		end
		local user_str
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed then
			...
			if table.contains(use.card:getSkillNames(), self:objectName()) or use.card:hasFlag(self:objectName()) then
				room:addPlayerMark(player, "keolyilie_guhuo_remove_" .. use.card:objectName())
			end
		else
			for _, m in sgs.list(player:getMarkNames()) do
				if m:startsWith("keolyilie_guhuo_remove_") then
					room:setPlayerMark(player, m, 0)
				end
			end
		end
...
ny_ol_hezhong = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardsMoveOneTime then
			...
			if player:getMark("ny_ol_hezhong_up_used-Clear") > 0 and player:getMark("ny_ol_hezhong_down_used-Clear") > 0 then
				return false
			end
			...
			if #choices > 1 then
				prompt = string.format("show:%s::%s:", "ny_ol_hezhong_all", card:getNumber())
			end
			...
			if player:askForSkillInvoke(self, sgs.QVariant(prompt)) then
				...
				if player:isAlive() then
					...
					if string.find(choice, "up") then
						player:addMark("ny_ol_hezhong_up_used-Clear")
						room:setPlayerMark(player, "&ny_ol_hezhong_up_mark-Clear", card:getNumber())
					else
						player:addMark("ny_ol_hezhong_down_used-Clear")
		...
		if event == sgs.CardUsed then
			...
			if use.card:isNDTrick() then
				local n = 0
				if
					player:getMark("ny_ol_hezhong_up_used-Clear") == 1
					and use.card:getNumber() > player:getMark("&ny_ol_hezhong_up_mark-Clear")
					and player:getMark("&ny_ol_hezhong_up_mark-Clear") > 0
				then
					player:addMark("ny_ol_hezhong_up_used-Clear")
					room:setPlayerMark(player, "&ny_ol_hezhong_up_mark-Clear", 0)
					n = n + 1
				end
				if player:getMark("ny_ol_hezhong_down_used-Clear") == 1 and use.card:getNumber() < player:getMark("&ny_ol_hezhong_down_mark-Clear") then
					player:addMark("ny_ol_hezhong_down_used-Clear")
...
sgs.LoadTranslationTable {
	...
	["ny_ol_hezhong:show"] = "你可以发动“和衷”展示手牌并摸一张牌",
	["ny_ol_hezhong_up"] = "大于",
...
huamu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardFinished then
			...
			local can = false
			for _, m in sgs.list(player:getMarkNames()) do
				if m:startsWith("&huamu+") and player:getMark(m) > 0 then
					room:setPlayerMark(player, m, 0)
					can = true
				end
			end
			room:setPlayerMark(player, "&huamu+" .. use.card:getColorString() .. "-Clear", 1)
			if cs < 1 and can and use.m_isHandcard and room:getCardOwner(use.card:getEffectiveId()) == nil then
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardsMoveOneTime then
			...
			if not room:getTag("FirstRound"):toBool() then
				local source
				if table.contains(move.from_pile_names, "ercao_yushu") or table.contains(move.from_pile_names, "ercao_lingshan") then
					source = move.from
				elseif move.to_pile_name == "ercao_yushu" or move.to_pile_name == "ercao_lingshan" then
					source = move.to
				end
				if
					source
					and (source:getPile("ercao_yushu"):isEmpty() or source:getPile("ercao_lingshan"):isEmpty() or source:getPile("ercao_yushu"):length() == source:getPile("ercao_lingshan"):length())
				then
...
local function canLyUse(from, cn)
	...
	if dc:subcardsLength() > 0 and not from:isLocked(dc) and from:getMark(cn .. "liangyuan_lun") < 1 then
		return dc
	end
end
	...
	on_validate_in_response = function(self, from)
		...
		if sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_PLAY then
			for _, pc in ipairs({ "peach", "analeptic" }) do
				...
				if dc and dc:isAvailable(from) then
					table.insert(choice, pc)
				end
			end
		else
	...
	on_validate = function(self, use)
		...
		if sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_PLAY then
			for _, pc in ipairs({ "peach", "analeptic" }) do
				...
				if dc and dc:isAvailable(use.from) then
					table.insert(choice, pc)
				end
			end
		else
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Start and player:getMark("@jisi") > 0 and player:hasSkill(self) then
			...
			for _, sk in sgs.qlist(general:getVisibleSkillList()) do
				if player:getMark(sk:objectName() .. "jisiUse") > 0 then
			...
			sgs.Sanguosha:addTranslationEntry(
				"jisi_invoke",
				"你可以令一名其他角色获得你武将牌上一个发动过的技能，然后你弃置所有手牌并视为对其使用【杀】"
					.. "\n可选技能："
					.. table.concat(choices2, ",")
			)
		...
		elseif event == sgs.ChoiceMade then
			...
			if #promptlist < 2 or promptlist[1] ~= "notifyInvoked" then
				return
			end
...
keolzhuri = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardsMoveOneTime and player:getPhase() ~= sgs.Player_NotActive then
			local move = data:toMoveOneTime()
			if
				(move.from_places:contains(sgs.Player_PlaceHand) and move.from:objectName() == player:objectName())
				or (move.to_place == sgs.Player_PlaceHand and move.to:objectName() == player:objectName())
			then
				player:setMark("keolzhuri_changed", 1)
			end
		elseif event == sgs.EventPhaseEnd then
			if player:getPhase() ~= sgs.Player_NotActive and player:getPhase() ~= sgs.Player_RoundStart and player:getMark("keolzhuri_changed") > 0 and player:isAlive() and player:hasSkill(self) then
				local targets = sgs.SPlayerList()
				for _, p in sgs.qlist(room:getOtherPlayers(player)) do
					if player:canPindian(p) then
						targets:append(p)
				...
				if target then
					...
					if pd.success then
						...
						room:askForUseCard(player, "@@keolzhuri", "@keolzhuri_use")
						...
						if room:askForChoice(player, self:objectName(), "hp+skill") == "hp" then
							...
							player:addMark("keolzhuri_lose-Clear")
							room:detachSkillFromPlayer(player, "keolzhuri")
						end
					end
				end
			end
			player:setMark("keolzhuri_changed", 0)
		elseif event == sgs.EventPhaseChanging then
			local change = data:toPhaseChange()
			if change.to ~= sgs.Player_NotActive then
				return false
...
keolranji = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseStart then
			...
			if player:getMark("keolranji_used-Clear") >= player:getHp() then
				table.insert(skills, "kunfen")
			end
			if player:getMark("keolranji_used-Clear") <= player:getHp() then
				table.insert(skills, "zhaxiang")
			end
			if #skills == 1 then
				string = "keolranji1:"
			end
		...
		elseif event == sgs.PreHpRecover then
			local recover = data:toRecover()
			if player:getMark("&keolranji_ban") < 1 then
				return
			end
			local log = sgs.LogMessage()
...
keolhetao = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.TargetSpecified then
			local use = data:toCardUse()
			if (use.to:length() <= 1) or use.card:isKindOf("SkillCard") then
				return false
			end
			for _, ys in sgs.qlist(room:getOtherPlayers(use.from)) do
				ys:setTag("keolhetao", data)
				if ys:hasSkill(self) and room:askForDiscard(ys, "keolhetao", 1, 1, true, true, "keolhetaoreddis", ".|" .. use.card:getColorString(), "keolhetao") then
					room:broadcastSkillInvoke(self:objectName())
...
keolshenli = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.TargetSpecifying then
			local use = data:toCardUse()
			if use.card:isKindOf("Slash") and player:getPhase() == sgs.Player_Play and player:getMark("bankeolshenli-PlayClear") < 1 and player:askForSkillInvoke(self, data) then
				room:broadcastSkillInvoke(self:objectName())
...
keolsizhaojianskill = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		local use = data:toCardUse()
		if event == sgs.TargetSpecified then
			if use.card:isKindOf("Slash") and use.from == player and player:hasWeapon("_keolsizhaojian") then
				local n = use.card:getNumber()
				if n < 1 then
					return
				end
...
Keolsizhaojianex = sgs.CreateCardLimitSkill {
	...
	limit_pattern = function(self, player)
		if player:getMark("Sizhaojian-Clear") > 0 then
			return "Jink|.|1~" .. player:getMark("Sizhaojian-Clear")
		end
		return ""
	end,
}
...
keoldulie = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardFinished then
			...
			then
				if player:getMark("keoldulie-Clear") < 1 and player:askForSkillInvoke(self:objectName() .. "$-1", data) then
...
keolweilin_red = sgs.CreateFilterSkill {
	name = "keolweilin_red&",
	view_filter = function(self, to_select)
		return to_select:isRed() and to_select:hasFlag("keolweilin") and sgs.Sanguosha:currentRoom():getCardPlace(to_select:getId()) == sgs.Player_PlaceHand
	end,
	view_as = function(self, original)
		local slash = sgs.Sanguosha:cloneCard("slash", original:getSuit(), original:getNumber())
		slash:setSkillName("keolweilin")
		--local card = sgs.Sanguosha:getWrappedCard(original:getId())
		--card:takeOver(slash)
		return slash
	end,
}
keolweilin_black = sgs.CreateFilterSkill {
	...
	view_filter = function(self, to_select)
		return not to_select:isRed() and to_select:hasFlag("keolweilin") and sgs.Sanguosha:currentRoom():getCardPlace(to_select:getId()) == sgs.Player_PlaceHand
	end,
	view_as = function(self, original)
		local slash = sgs.Sanguosha:cloneCard("slash", original:getSuit(), original:getNumber())
		slash:setSkillName("keolweilin")
	...
	on_use = function(self, room, source, targets)
		...
		if targets[1] and targets[1]:objectName() ~= source:objectName() then
			...
			for _, id in sgs.qlist(sgs.Sanguosha:getRandomCards()) do
				...
				if not cd:isKindOf("Slash") or table.contains(choices, cd:objectName()) then
					continue
				end
				...
				for _, p in sgs.qlist(use.to) do
					for _, h in sgs.qlist(p:getHandcards()) do
						if h:getColor() == use.card:getColor() then
							...
							wrap:takeOver(toc)
							room:notifyUpdateCard(p, h:getId(), wrap)
						end
					end
				end
			end
...
keolduoshou = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.CardUsed then
			local use = data:toCardUse()
			local has = false
			if use.card:isRed() and player:getMark("keolduoshou_red-Clear") < 1 then
			...
			if use.card:isKindOf("BasicCard") and player:getMark("keolduoshou_basic-Clear") < 1 then
...
olmouhulie = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.TargetSpecified then
			local use = data:toCardUse()
			if
				(use.card:isKindOf("Duel") or use.card:isKindOf("Slash"))
				and use.to:length() == 1
			...
			if use.card:isKindOf("Slash") and use.to:length() == 1 then
				...
				for _, p in sgs.qlist(use.to) do
					...
					if p:getAttackRange() >= player:getAttackRange() and player:canDiscard(p, "he") then
						...
						local players = sgs.SPlayerList()
						for _, p in sgs.qlist(room:getOtherPlayers(player)) do
							if use.to:contains(p) or player:isProhibited(p, use.card) then
								continue
							end
							players:append(p)
						end
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.TargetSpecifying then
			if
				...
				and not use.card:isKindOf("Fqizhengxiangsheng")
				and player:getMark("mouyingwuUsed-Clear") < 2
				and player:getPhase() == sgs.Player_Play
			then
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseChanging then
			...
			for _, p in sgs.list(room:findPlayersBySkillName("moulongdan")) do
				...
				if n < 3 or n < 4 and p:getMark("moujizhuoBf") > 0 then
					room:addPlayerMark(p, "&moulongdanLast")
				end
			end
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.DamageComplete then
			...
			if damage.from and damage.from:getMark("&XL_tongchou") > 0 then
				...
				if n >= 4 then
					for _, p in sgs.qlist(room:getAlivePlayers()) do
						if p:getMark("&XL_tongchou") < 1 then
							continue
						end
		...
		elseif event == sgs.GameStart then
			if player:isLord() and room:getMode():contains("p") and not table.contains(sgs.Sanguosha:getBanPackages(), "51OLTrick") then
				room:acquireSkill(player, "olmomingcha")
			end
		elseif event == sgs.CardsMoveOneTime then
			local move = data:toMoveOneTime()
			--协力[并进]：共计摸至少八张牌。
			if player:getMark("&XL_bingjin") > 0 and move.from_places:contains(sgs.Player_DrawPile) and move.to_place == sgs.Player_PlaceHand and move.to:objectName() == player:objectName() then
				...
				if n >= 8 then
					for _, p in sgs.qlist(room:getAlivePlayers()) do
						if p:getMark("&XL_bingjin") < 1 then
							continue
						end
			...
			then
				...
				for i, id in sgs.qlist(move.card_ids) do
					...
					if table.contains(suits, su) then
						continue
					end
		...
		elseif event == sgs.BuryVictim then
			for _, ch in sgs.list(Xlchoices) do
				if player:getMark(ch .. "from") > 0 then
					for _, p in sgs.qlist(room:getAlivePlayers()) do
						if player:getMark(ch .. "XLto" .. p:objectName()) > 0 then
							room:setPlayerMark(p, "&" .. ch, 0)
						end
					end
				end
			end
		...
		elseif event == sgs.Dying then
			...
			if dy.who ~= player and dy.who:hasFlag("Global_Dying") and player:isAlive() and not table.contains(sgs.Sanguosha:getBanPackages(), "51OLTrick") then
				local can = ""
				if hasCard(player, "Luojingxiashi", "&h") then
					can = "ol_luojingxiashi"
				else
					for _, skill in sgs.qlist(player:getSkillList(true, false)) do
						local vs = sgs.Sanguosha:getViewAsSkill(skill:objectName())
						if vs and vs:isEnabledAtResponse(player, "ol_luojingxiashi") then
							can = "ol_luojingxiashi"
							break
						end
					end
				...
				if can ~= "" then
					...
					room:askForUseCard(player, can, "ol_luojingxiashi0:" .. dy.who:objectName())
					if player:isDead() or not dy.who:hasFlag("Global_Dying") then
						return
					end
				end
				can = ""
				if hasCard(player, "Shengsiyugong", "&h") then
					can = "ol_shengsiyugong"
				else
					for _, skill in sgs.qlist(player:getSkillList(true, false)) do
						local vs = sgs.Sanguosha:getViewAsSkill(skill:objectName())
						if vs and vs:isEnabledAtResponse(player, "ol_shengsiyugong") then
							can = "ol_shengsiyugong"
							break
						end
					end
				...
				if can ~= "" then
					...
					room:askForUseCard(player, can, "ol_shengsiyugong0:" .. dy.who:objectName())
				end
			end
		elseif player:getMark("&XL_luli") > 0 then
			local card
			if event == sgs.CardUsed then
				card = data:toCardUse().card
			else
			...
			if card and card:getSuit() < 4 then
...
moupaoxiao = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.ConfirmDamage then
			local damage = data:toDamage()
			if damage.card and damage.card:isKindOf("Slash") and player:getMark("moupaoxiaoSlashUsed-PlayClear") > 1 then
				damage.damage = damage.damage + 1
				data:setValue(damage)
			end
		elseif event == sgs.Damage then
			local damage = data:toDamage()
			if damage.card and damage.card:isKindOf("Slash") and player:getMark("moupaoxiaoSlashUsed-PlayClear") > 1 and damage.to:isAlive() then
				room:loseHp(player, 1, true, player, self:objectName())
...
moumingxuanCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		for _, p in pairs(targets) do
			...
			if p:canSlash(source, nil, false) and room:askForUseSlashTo(p, source, "@moumingxuan-slash:" .. source:objectName()) then
				room:addPlayerMark(p, "&moumingxuan+#" .. source:objectName())
				continue
			end
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getPhase() == sgs.Player_Play then
			...
			if player:getHandcardNum() > 0 and n > 0 then
				...
				if room:askForUseCard(player, "@@moumingxuan!", "@moumingxuan-card") then
					return
				end
				room:broadcastSkillInvoke(self:objectName())
				n = sgs.IntList()
				...
				for _, p in sgs.qlist(room:getOtherPlayers(player)) do
					if p:getMark("&moumingxuan+#" .. player:objectName()) < 1 then
						...
						if mingxuaners:length() >= n:length() then
							break
						end
					end
				end
				if n:length() > 0 and mingxuaners:length() > 0 then
					dc:use(room, player, mingxuaners)
				end
			end
		end
	end,
...
mouxianchou = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.Damaged then
			local damage = data:toDamage()
			if damage.from and player:hasSkill(self) then
				local plist = sgs.SPlayerList()
				for _, p in sgs.qlist(room:getOtherPlayers(player)) do
					if p ~= damage.from and p:canSlash(damage.from, nil, false) then
						plist:append(p)
					end
				end
				local machao = room:askForPlayerChosen(player, plist, self:objectName(), "@mouxianchou-fujunAvanger", true, true)
				if machao and room:askForCard(machao, "..", "@mouxianchou-slash", data) then
					local dc = dummyCard()
		...
		elseif event == sgs.CardFinished then
			local use = data:toCardUse()
			if (table.contains(use.card:getSkillNames(), "mouxianchou") or use.card:hasFlag("mouxianchou")) and use.card:hasFlag("DamageDone") then
...
mouqiaoshi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.Damaged then
			local damage = data:toDamage()
			if damage.from and damage.from ~= player and player:getMark("mouqiaoshiUse-Clear") < 1 and player:hasTurn() and damage.from:askForSkillInvoke(self, data, false) then
				local log = sgs.LogMessage()
		...
		if event == sgs.PreCardUsed then
			if use.card:isKindOf("Duel") then
				if table.contains(use.card:getSkillNames(), self:objectName()) then
					room:addPlayerMark(player, "moujiangUse-PlayClear")
				end
				local extra_targets = room:getCardTargets(player, use.card, use.to)
...
mouzhiba = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.Dying then
			local dying = data:toDying()
			if dying.who == player and player:getMark("@mouzhiba") > 0 and player:hasLordSkill(self:objectName()) and player:askForSkillInvoke(self, data) then
			...
			local death = data:toDeath()
			if death.who:getKingdom() == "wu" and death.damage and death.damage.reason == self:objectName() and player:hasFlag("mouzhibaDamage" .. death.who:objectName()) then
...
mouzhibas = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.Dying then
			local dying = data:toDying()
			if dying.who == player and player:getMark("@mouzhibas") > 0 and player:hasLordSkill(self:objectName()) and player:askForSkillInvoke(self, data) then
			...
			local death = data:toDeath()
			if death.who:getKingdom() == "wu" and death.damage and death.damage.reason == self:objectName() and player:hasFlag("mouzhibasDamage" .. death.who:objectName()) then
				room:drawCards(player, 3, self:objectName())
			end
		end
	end,
...
moulieren = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		local use = data:toCardUse()
		if event == sgs.TargetSpecified then
			if use.card:isKindOf("Slash") and use.to:length() == 1 and player ~= use.to:first() and player:canPindian(use.to:first()) and player:askForSkillInvoke(self, ToData(use.to:first())) then
				room:broadcastSkillInvoke(self:objectName())
...
makijuxiang = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.EventPhaseProceeding then
			if player:getPhase() == sgs.Player_Finish and player:hasSkill(self) and player:getMark("makijuxiangUseSA-Clear") < 1 then
				for _, id in sgs.qlist(sgs.Sanguosha:getRandomCards(true)) do
					local nmrq = sgs.Sanguosha:getEngineCard(id)
					if nmrq:objectName() ~= "_savage_assault" or room:getCardOwner(id) then
						continue
					end
...
moutianxiangCard = sgs.CreateSkillCard {
	...
	will_throw = false,
	filter = function(self, targets, to_select, player)
		return #targets < 1 and to_select:objectName() ~= player:objectName() and to_select:getMark("&moutianxiang+heart") < 1 and to_select:getMark("&moutianxiang+diamond") < 1
	end,
	on_effect = function(self, effect)
...
moutianxiang = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.DamageInflicted then
			...
			for _, p in sgs.qlist(room:getAllPlayers()) do
				if p:getMark("&moutianxiang+heart") > 0 or p:getMark("&moutianxiang+diamond") > 0 then
					tianxiangers:append(p)
				end
			end
			...
			if txr then
				...
				if txr:getMark("&moutianxiang+diamond") > 0 then
					...
					if card then
						room:giveCard(txr, player, card, self:objectName())
					end
				end
			end
...
mouhongyan_vsc = sgs.CreateFilterSkill {
	name = "#mouhongyan_vsc",
	view_filter = function(self, to_select)
		return to_select:getSuit() == 0 and sgs.Sanguosha:currentRoom():getCardPlace(to_select:getId()) == sgs.Player_PlaceHand
	end,
	view_as = function(self, card)
		local new_card = sgs.Sanguosha:cloneCard(card:objectName(), sgs.Card_Heart, card:getNumber())
...
mouhongyan = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.CardsMoveOneTime then
			local move = data:toMoveOneTime()
			if move.from_places:contains(sgs.Player_PlaceHand) and move.from:objectName() == player:objectName() and move.to and move.to:objectName() ~= player:objectName() then
				for i, id in sgs.qlist(move.card_ids) do
...
moujieyin = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.EventPhaseStart then
			if player:getPhase() ~= sgs.Player_Play or not player:hasSkill(self:objectName()) or player:getMark("moujieyin_fail") > 0 then
				return false
			end
			for _, p in sgs.qlist(room:getAllPlayers()) do
			...
			for i = 0, move.card_ids:length() - 1 do
				...
				if move.from_places:at(i) == sgs.Player_PlaceEquip then
					...
					for _, lb in sgs.qlist(room:getAlivePlayers()) do
						if player:canDiscard(lb, "ej") then
							FJ:append(lb)
						end
					end
...
mouwusheng = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Play and player:hasSkill(self:objectName()) then
			...
			for _, p in sgs.qlist(room:getOtherPlayers(player)) do
				if (string.sub(mode, -1) == "p" or string.sub(mode, -2) == "pd" or string.sub(mode, -2) == "pz") and p:isLord() then
					continue
				end
				targets:append(p)
			end
...
mouyijue = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.DamageInflicted then
			local damage = data:toDamage()
			if damage.from and damage.from ~= player and damage.from:hasFlag("CurrentPlayer") and damage.from:hasSkill(self:objectName()) then
		...
		elseif event == sgs.TargetSpecifying then
			...
			if use.card:getTypeId() > 0 and player:hasSkill(self) then
				...
				for _, p in sgs.qlist(tos) do
					if p:getMark(player:objectName() .. "mouyijued-Clear") > 0 then
						use.to:removeOne(p)
					end
				end
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.TargetSpecified then
			...
			if use.card:isKindOf("SavageAssault") then
				...
				if player:hasSkill(self) and player:getPhase() == sgs.Player_Play and player:getMark("mouhuoshouUseSA-PlayClear") < 1 then
...
mouzaiqi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.Damage then
			if player:getMark("mouzaiqiXL-Clear") < 1 and player:hasTurn() and player:getMark("&charge_num") < xuLiMax(player) then
...
mouluanji = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardResponded then
			...
			if
				resp.m_card:isKindOf("Jink")
				and resp.m_who
				and resp.m_who:hasSkill(self)
				and player ~= resp.m_who
				and resp.m_toCard:isKindOf("ArcheryAttack")
				and resp.m_who:getMark("mouluanjiDraw-Clear") < 3
			then
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardsMoveOneTime then
			...
			if
				...
				and move.to:objectName() == player:objectName()
				and move.reason.m_skillName ~= "mobilemoutuxi"
				and player:getMark("&mobilemoutuxi-Clear") < 3
			then
					...
					if target then
						...
						if #choices > 0 then
							...
							if choice ~= "2" then
								local dc = dummyCard()
								for i = 1, 2 do
									if dc:subcardsLength() >= target:getEquips():length() then
										break
									end
...
moujianxiong = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.Damaged then
			local damage = data:toDamage()
			if damage.card and damage.card:getEffectiveId() >= 0 and player:askForSkillInvoke(self, data) then
				room:broadcastSkillInvoke(self:objectName())
				if room:getCardOwner(damage.card:getEffectiveId()) == nil then
					room:obtainCard(player, damage.card)
				end
		...
		for _, to in ipairs(targets) do
			...
			if id >= 0 then
				...
				for _, h in sgs.qlist(to:getHandcards()) do
					if sgs.Sanguosha:getCard(id):getSuit() == h:getSuit() and source:canDiscard(to, h:getId()) then
						dc:addSubcard(h)
					end
				end
				if dc:subcardsLength() > 0 then
...
mouqingzheng = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getPhase() == sgs.Player_Play then
			...
			for _, c in sgs.qlist(player:getHandcards()) do
				if suits:contains(c:getSuit()) then
					continue
				end
			...
			if suits:length() >= m and room:askForUseCard(player, "@@mouqingzheng", "@mouqingzheng:" .. m) then
				if n < 2 and player:hasSkill("moujianxiong", true) and player:askForSkillInvoke(self, sgs.QVariant("mouqingzheng"), false) then
...
moufanjianCard = sgs.CreateSkillCard {
	...
	on_effect = function(self, effect)
		...
		if choicc == "3" then --直接开摆，猜猜猜，我猜nm呢
			...
			if choicc == "1" and self:getSuitString() == choice or choicc == "2" and self:getSuitString() ~= choice then --猜对
				room:broadcastSkillInvoke("moufanjian", 2)
				room:setPlayerFlag(effect.from, "moufanjian_cantUse")
...
mourendeCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, source)
		...
		return #targets < 1 and to_select:getMark("mourendeCardGet-PlayClear") < 1 and to_select:objectName() ~= source:objectName()
	end,
	feasible = function(self, targets, source)
		return self:subcardsLength() < 1 and source:getMark("mRenWang-Clear") < 1 or #targets > 0
	end,
	on_validate = function(self, use)
...
mourende = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Play and player:hasSkill(self) then
...
mouzhangwuCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		for _, p in sgs.qlist(tos) do
			if n < 1 or p:isDead() or source:isDead() then
				continue
			end
			local card = room:askForExchange(p, "mouzhangwu", n, n, true, "#mouzhangwu:" .. n)
			if card then
				room:giveCard(p, source, card, "mouzhangwu")
			end
		end
...
moujijiangCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, source)
		if #targets == 1 then
			if to_select:inMyAttackRange(targets[1]) and to_select:getHp() >= source:getHp() and to_select:objectName() ~= source:objectName() then
	...
	on_use = function(self, room, source, targets)
		...
		if use.to:at(1):canSlash(use.to:at(0), dc) then
			choice = "1=" .. use.to:at(0):objectName() .. "+2"
		end
...
moukanpo = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if event == sgs.RoundStart then
			...
			local names = player:getTag("moukanpoNames"):toString():split("+")
			player:removeTag("moukanpoNames")
			...
			for id = 0, sgs.Sanguosha:getCardCount() do
				if rcids:contains(id) then
					local cd = sgs.Sanguosha:getEngineCard(id)
					if cd:getTypeId() > 2 or table.contains(names, cd:objectName()) or cd:isKindOf("Slash") and table.contains(names, "slash") then
						continue
					end
					record:append(id)
			...
			local prediction = 4
			if room:getMode() == "04_2v2" or room:getMode() == "03_1v2" then
				prediction = 2
			end
			prediction = prediction - player:getMark(self:objectName())
			if prediction < 1 or record:isEmpty() then
				return false
			end
			room:sendCompulsoryTriggerLog(player, self)
			local namess = {}
			...
			log.type = "#moukanpo"
			log.from = player
			for id = 1, prediction do
				room:fillAG(record, player)
				local id = room:askForAG(player, record, #namess > 0, self:objectName())
				room:clearAG(player)
				if id < 0 then
					break
				end
				local eid = sgs.Sanguosha:getEngineCard(id)
				if eid:isKindOf("Slash") then
					table.insert(namess, "slash")
				else
					table.insert(namess, eid:objectName())
				end
				player:addMark(self:objectName())
				log.arg = namess[#namess]
				room:sendLog(log, player)
			end
			player:setTag("moukanpoNames", sgs.QVariant(table.concat(namess, "+")))
			room:setPlayerMark(player, "&moukanpo_lun", #namess)
		elseif event == sgs.CardUsed then
			if use.card:getTypeId() < 1 then
				return
			end
			for _, mzg in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				...
				local names = mzg:getTag("moukanpoNames"):toString():split("+")
				if use.card:isKindOf("Slash") and table.contains(names, "slash") or table.contains(names, use.card:objectName()) then
					mzg:setTag("moukanpo", data)
					if mzg:askForSkillInvoke(self, data) then
						room:broadcastSkillInvoke(self:objectName())
						if use.card:isKindOf("Slash") then
						...
						mzg:setTag("moukanpoNames", sgs.QVariant(table.concat(names, "+")))
						room:setPlayerMark(mzg, "&moukanpo_lun", #names)
						local nullified_list = use.nullified_list
	...
	on_trigger = function(self, event, player, data)
		...
		if player:hasSkill("mouguanxing", true) then
			if player:getPile("mouStarsXing"):isEmpty() then
				...
				judge.reason = self:objectName()
				room:judge(judge)
				if judge.card:getNumber() > player:getPile("mouStarsXing"):length() then
					return false
				end
				damage.damage = damage.damage - 1
				data:setValue(damage)
...
mouliuliCard = sgs.CreateSkillCard {
	...
	will_throw = true,
	filter = function(self, targets, to_select, from)
		return #targets < 1 and from ~= to_select and from:inMyAttackRange(to_select) and not to_select:hasFlag("mouliuliUseFrom")
	end,
	on_effect = function(self, effect)
		local room = effect.from:getRoom()
...
mouliuli = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		local use = data:toCardUse()
		if use.card:isKindOf("Slash") and player:hasSkill(self) and player:canDiscard(player, "he") and room:alivePlayerCount() > 2 then
			...
			if uc then
				...
				if uc:getSuit() ~= 2 or player:getMark("mouliuliDis-Clear") > 0 then
					return
				end
				local sunce = room:askForPlayerChosen(player, aps, self:objectName(), "mouliuli-extraEffect", true)
				if sunce then
					player:addMark("mouliuliDis-Clear")
					room:doAnimate(1, player:objectName(), sunce:objectName())
					for _, p in sgs.qlist(room:getAlivePlayers()) do
...
mouqixiCard = sgs.CreateSkillCard {
	...
	on_effect = function(self, effect)
		...
		for _, h in sgs.qlist(effect.from:getHandcards()) do
			suits[h:getSuitString()] = (suits[h:getSuitString()] or 0) + 1
		end
		local m = 0
		...
		local n = 0
		while #choices > 0 and effect.to:isAlive() and m >= 0 do
			...
			if #msuits < 1 or table.contains(msuits, choice) then
				...
				if #choices > 0 and effect.from:askForSkillInvoke("mouqixi", sgs.QVariant("mouqixi"), false) then
					continue
				end
			end
			if n > 0 and effect.from:canDiscard(effect.to, "hej") then
				local dc = dummyCard()
				n = math.min(n, effect.to:getCardCount(true, true))
				for i = 1, n do
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed then
			...
			if use.card:isKindOf("IronChain") then
				if table.contains(use.card:getSkillNames(), self:objectName()) then
					room:addPlayerMark(player, "moulianhuanICUsed-PlayClear")
				end
		...
		elseif event == sgs.TargetSpecified then
			local use = data:toCardUse()
			if use.card:isKindOf("IronChain") and use.card:hasFlag("moulianhuanBF") then
				for _, p in sgs.qlist(use.to) do
					if p:isKongcheng() or p:isChained() then
						continue
					end
					for i = 0, 9 do
...
moulianhuanbf = sgs.CreateCardLimitSkill {
	...
	limit_pattern = function(self, player, card)
		if player:getMark("moulianhuanICUsed-PlayClear") > 0 and card:getSkillName() == "moulianhuan" and player:hasSkill(self, true) then
			return "IronChain"
		end
		return ""
	end,
...
moulijianCard = sgs.CreateSkillCard {
	...
	will_throw = true,
	filter = function(self, targets, to_select, source)
		if #targets == self:subcardsLength() + 1 then
			return false
		end
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.EventPhaseProceeding then --双保险
			if player:getPhase() == sgs.Player_Finish and player:hasSkill(self) then
				local n = 1
				for _, p in sgs.qlist(room:getAllPlayers()) do
					if p:hasFlag("moubiyue_damagedTargets") then
						n = n + 1
					end
				end
				room:sendCompulsoryTriggerLog(player, self)
...
mobilemouyiji = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.EnterDying then
			...
			if player:getMark("mobilemouyiji_lun") == 1 and player:askForSkillInvoke(self:objectName(), data) then
				room:broadcastSkillInvoke(self:objectName())
				player:drawCards(1, self:objectName())
				local ids = player:handCards()
...
sgs.LoadTranslationTable {
	...
	[":mobilemoutiandu1"] = '转换技，出牌阶段开始时，<font color="#01A5AF"><s>阳：你可弃置两张手牌，然后视为使用一张普通锦囊牌</s></font>；阴：你进行判定并获得此判定牌，若结果与你本局游戏发动“天妒”弃置过的牌花色相同，你受到1点无来源伤害。',
	[":mobilemoutiandu2"] = '转换技，出牌阶段开始时，阳：你可弃置两张手牌，然后视为使用一张普通锦囊牌；<font color="#01A5AF"><s>阴：你进行判定并获得此判定牌，若结果与你本局游戏发动“天妒”弃置过的牌花色相同，你受到1点无来源伤害</s></font>。',
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed then
			...
			if use.card:getTypeId() > 0 then
				...
				if player:hasSkill(self) and player:getMark("mouzhaxiangCardUsed-Clear") <= player:getLostHp() then
					room:sendCompulsoryTriggerLog(player, self)
					local no_respond_list = use.no_respond_list
					table.insert(no_respond_list, "_ALL_TARGETS")
					use.no_respond_list = no_respond_list
...
mouqicaiCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, source)
		return #targets < 1
			...
			and (source:getMark("f_mode_doudizhu") > 0 and to_select:hasEquipArea(1) or source:getMark("f_mode_doudizhu") == 0 and to_select:hasEquipArea())
	end,
	on_use = function(self, room, source, targets)
		if self:getSubcards():length() > 0 then
			...
			for _, id in sgs.qlist(room:getDiscardPile()) do
				...
				if source:getMark("f_mode_doudizhu") > 0 then
					if c:isKindOf("Armor") and source:getMark("mouqicaiArmor" .. c:objectName()) < 1 then
						eqp_or_amr:append(id)
					end
	...
	enabled_at_play = function(self, player)
		...
		for _, id in sgs.qlist(player:property("mouqicaiDiscardPile"):toIntList()) do
			if player:getMark("f_mode_doudizhu") > 0 then
				if sgs.Sanguosha:getEngineCard(id):isKindOf("Armor") and player:getMark("mouqicaiArmor" .. sgs.Sanguosha:getEngineCard(id):objectName()) < 1 then
					return true
				end
			elseif sgs.Sanguosha:getEngineCard(id):isKindOf("EquipCard") then
				return true
			end
		end
		for _, h in sgs.qlist(player:getHandcards()) do
			if player:getMark("f_mode_doudizhu") > 0 then
				if h:isKindOf("Armor") then
					return true
				end
			elseif h:isKindOf("EquipCard") then
				return true
			end
		end
		return false
...
mouqicai = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if room:getMode() == "03_1v2" then
			room:setPlayerMark(player, "f_mode_doudizhu", 1)
		end
		if move.to_place == sgs.Player_PlaceHand and move.to:objectName() ~= player:objectName() and move.to:isAlive() and move.to:getMark("&mouQI") > 0 then
			local to = room:findPlayerByObjectName(move.to:objectName())
			for _, id in sgs.qlist(move.card_ids) do
		...
		elseif move.to_place == sgs.Player_DiscardPile or move.from_places:contains(sgs.Player_DiscardPile) then
...
moumingren = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.GameStart then
			...
			if dc then
				player:addToPile("mouResponsibility", dc, false)
			end
...
mouzhenliangCard = sgs.CreateSkillCard {
	name = "mouzhenliang",
	filter = function(self, targets, to_select, from)
		return #targets < 1 and math.max(1, math.abs(to_select:getHp() - from:getHp())) == self:subcardsLength() and from:inMyAttackRange(to_select) and from:objectName() ~= to_select:objectName()
	end,
	on_effect = function(self, effect)
		local room = effect.from:getRoom()
	...
	on_trigger = function(self, event, player, data)
		...
		if card and not card:isKindOf("SkillCard") then
			for _, mlz in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if mlz:hasFlag("CurrentPlayer") or mlz:getChangeSkillState("mouzhenliang") ~= 2 or mlz:getPile("mouResponsibility"):isEmpty() then
					continue
				end
...
mobilejingxieCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		for _, id in sgs.qlist(sgs.Sanguosha:getRandomCards(true)) do
			...
			if "_" .. c:objectName() == rc:objectName() and c:getNumber() == rc:getNumber() and c:getSuit() == rc:getSuit() then
				room:getThread():delay()
				local moves = sgs.CardsMoveList()
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardAsked then
			...
			if string.find(pattern[1], "jink") then
				...
				if player:isCardLimited(jink, pattern[3] == "use" and sgs.Card_MethodUse or sgs.Card_MethodResponse) or not player:askForSkillInvoke(self, data) then
					return
				end
...
_vineSkill = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.CardEffected then
			local effect = data:toCardEffect()
			if effect.card:objectName() == "slash" or effect.card:isKindOf("ArcheryAttack") or effect.card:isKindOf("SavageAssault") then
				room:sendCompulsoryTriggerLog(player, "_vine")
				room:setEmotion(player, "armor/vine")
				effect.nullified = true
			end
...
mobileqiaosiCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		while source:isAlive() do
			...
			if choice:startsWith("qs_wang") then
				qs.qs_wang = math.min(100, qs.qs_wang + 33)
			elseif choice:startsWith("qs_shang") then
				qs.qs_shang = math.min(100, qs.qs_shang + 75)
			elseif choice:startsWith("qs_gong") then
				qs.qs_gong = 100
			elseif choice:startsWith("qs_nong") then
				qs.qs_nong = 100
			elseif choice:startsWith("qs_shi") then
				qs.qs_shi = math.min(100, qs.qs_shi + 75)
			elseif choice:startsWith("qs_jiang") then
				qs.qs_jiang = math.min(100, qs.qs_jiang + 33)
			end
			local has = 0
			for ch, n in pairs(qs) do
				if n > 99 then
		...
		local function QScard(name, n)
			...
			for _, c in ipairs(ds) do
				if c:isKindOf(name) or c:objectName() == name then
					table.insert(cs, c)
				end
				if #cs >= n then
					break
		...
		if qs.qs_shang > 99 then
			...
			if #can > 0 then
				table.insert(cards, can[math.random(1, #can)])
			end
		end
		...
		if #cards < 1 or source:isDead() then
			return
		end
		local dc = dummyCard()
		for _, c in ipairs(cards) do
		...
		if room:askForUseCard(source, "@@mobileqiaosi!", "mobileqiaosi0:" .. #cards, -1, sgs.Card_MethodNone) or source:isDead() then
			return
		end
		local dc = mobileqiaosi2Card:clone()
		for _, c in ipairs(cards) do
			if source:isJilei(c) or dc:subcardsLength() >= #cards then
				continue
			end
			dc:addSubcard(c)
		end
	...
	on_trigger = function(self, event, player, data, room)
		if player:getPhase() == sgs.Player_Draw then
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				...
				if player:getHp() <= p:getHp() or card_ids:isEmpty() or not p:askForSkillInvoke(self, ToData(player)) then
					continue
				end
				room:broadcastSkillInvoke(self:objectName())
				local dc = dummyCard()
...
rensongshuFlag = sgs.CreateProhibitSkill {
	name = "#rensongshuFlag",
	is_prohibited = function(self, from, to, card)
		return from:hasFlag("rensongshuBf") and from ~= to and card:getTypeId() > 0
	end,
}
		...
		elseif event == sgs.TargetSpecifying then
			...
			for _, qg in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				...
				if ids:contains(use.card:getId()) and player ~= qg then
					...
					if table.contains(names, use.card:objectName()) and qg:askForSkillInvoke(self, data) then
...
rengonghuan = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.DamageInflicted then
			local damage = data:toDamage()
			for _, p in sgs.qlist(room:getOtherPlayers(player)) do
				if p:getMark("rengonghuan-Clear") < 1 and p:hasTurn() and player:getHp() <= p:getHp() and p:hasSkill(self) then
					room:sendCompulsoryTriggerLog(p, self)
					damage.transfer = true
					damage.to = p
			...
			for _, tip in sgs.list(damage.tips) do
				if tip:startsWith("rengonghuanDamage:") then
					local ts = tip:split(":")
					for _, p in sgs.qlist(room:getAlivePlayers()) do
						if table.contains(ts, p:objectName()) then
							p:loseMark("&luanchouyin")
						end
					end
				end
			end
		...
		if event == sgs.CardsMoveOneTime then
			...
			if move.from_places:contains(sgs.Player_PlaceTable) then
				...
				if renPile:length() > 0 then
					if move.reason.m_reason ~= sgs.CardMoveReason_S_REASON_RULEDISCARD or move.reason.m_eventName ~= "removeRenPile" then
						for _, id in sgs.qlist(move.card_ids) do
			...
			if move.reason.m_reason == sgs.CardMoveReason_S_REASON_USE and move.to_place == sgs.Player_DiscardPile then
				local use = move.reason.m_useStruct
				if use.card and use.card:getTypeId() > 0 and not use.card:hasFlag("DamageDone") and room:getCardPlace(use.card:getEffectiveId()) == sgs.Player_DiscardPile and use.from == player then
					room:sendCompulsoryTriggerLog(player, self)
...
renbinglun = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseChanging then
			local change = data:toPhaseChange()
			if change.to ~= sgs.Player_NotActive or player:hasFlag("renbinglun") then
				return
			end
...
mobile_wuling = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart then
			...
			local wl = ""
			for _, m in sgs.list(player:getMarkNames()) do
				if m:endsWith("+#wlMark") and player:getMark(m) > 0 then
					wl = m
					break
				end
			end
		...
		elseif event == sgs.DamageInflicted then
			local damage = data:toDamage()
			if player:getMark("wl_xiong_mark") > 0 and player:hasTurn() and player:getMark("wl_xiong_mark-Clear") < 1 then
		...
		elseif event == sgs.TargetSpecified then
			local use = data:toCardUse()
			if player:getMark("wl_hu_mark") < 1 or use.card:getTypeId() < 1 or use.to:length() ~= 1 then
				return false
			end
...
mobile_wulingbf = sgs.CreateProhibitSkill {
	...
	is_prohibited = function(self, from, to, card)
		if card:isKindOf("DelayedTrick") and to:getMark("wl_lu_mark") > 0 then
			self:setObjectName("wl_lu_mark")
			return true
		end
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseEnd then
			...
			for _, id in sgs.qlist(player:getTag("youyiDiscard"):toIntList()) do
				if room:getCardPlace(id) == sgs.Player_DiscardPile and not ids:contains(id) then
					ids:append(id)
				end
			end
			if player:hasSkill(self) and ids:length() > 0 and player:askForSkillInvoke(self) then
				room:broadcastSkillInvoke(self:objectName())
				local dc = dummyCard()
				dc:addSubcards(ids)
				--addRenPile(dc,player,self)
...
mobile_dingzouCard = sgs.CreateSkillCard {
	...
	will_throw = false,
	filter = function(self, targets, to_select, from)
		return #targets < 1 and from:objectName() ~= to_select:objectName() and self:subcardsLength() == to_select:getEquips():length() + to_select:getJudgingArea():length()
	end,
	on_effect = function(self, effect)
		local room = effect.to:getRoom()
	...
	on_trigger = function(self, event, player, data)
		...
		if c == nil then
			for _, h in sgs.qlist(to:getCards("he")) do
				if to:isCardLimited(h, sgs.Card_MethodRecast) then
					continue
				end
				c = h
				break
...
yandiaodu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if target then
			...
			local to = room:askForPlayerChosen(player, players, "yandiaodu_to", "@yandiaoduto", false, false)
			if to then
				room:doAnimate(1, player:objectName(), to:objectName())
...
yandiancai = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardsMoveOneTime then
			...
			then
				room:addPlayerMark(player, "&yandiancai-PlayClear")
			end
		elseif event == sgs.EventPhaseEnd and player:getPhase() == sgs.Player_Play then
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if p ~= player and p:getMaxHp() > p:getHandcardNum() and p:getMark("&yandiancai-PlayClear") >= p:getHp() and p:askForSkillInvoke(self) then
					room:broadcastSkillInvoke(self:objectName())
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.FinishRetrial then
			...
			for _, zys in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if zys:getMark("yantaoluanUse-Clear") < 1 and player:hasTurn() and zys:askForSkillInvoke(self:objectName() .. "$-1", data) then
					zys:addMark("yantaoluanUse-Clear")
					local choice = "yantaoluan1"
					if zys ~= player then
						choice = "yantaoluan1+yantaoluan2"
					end
					...
					if p:getHandcardNum() >= player:getHandcardNum() then
						if player:askForSkillInvoke(self, ToData(damage.to)) then
							...
							for _, h in sgs.qlist(damage.to:getHandcards()) do
								if h:isRed() and player:canDiscard(damage.to, h:getId()) then
									dc:addSubcard(h)
								end
							end
		...
		elseif event == sgs.EventForDiy then
			...
			if zs:startsWith("zhengsu:") then
				...
				if zss[3] == "zhengsu_successful" then
					...
					if to then
						if room:askForChoice(to, "zhengsu", "drawCards_2+recover_1") ~= "drawCards_2" then
							room:recover(to, sgs.RecoverStruct("zhengsu"))
						else
							to:drawCards(2, "zhengsu")
						end
					end
				end
			end
...
yanyangjieCard = sgs.CreateSkillCard {
	...
	--will_throw = false,
	filter = function(self, targets, to_select, source)
		return #targets < 1 and source:objectName() ~= to_select:objectName() and source:canPindian(to_select)
	end,
	on_use = function(self, room, source, targets)
		for _, p in sgs.list(targets) do
			if source:pindian(p, self:getSkillName()) then
				continue
			end
			local dc = dummyCard("fire_slash")
...
yanjuxiang = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		local dying = data:toDying()
		for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
			if p:getMark("@yanjuxiang") < 1 or p == dying.who or not p:askForSkillInvoke(self, ToData(dying.who)) then
				continue
			end
			room:broadcastSkillInvoke(self:objectName())
...
yanhoufeng = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart then
			...
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if p:inMyAttackRange(player) and p:askForSkillInvoke(self, ToData(player)) then
					room:broadcastSkillInvoke(self:objectName(), 1)
			...
			if zs:startsWith("zhengsu:") then
				...
				for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					if zss[3] == "zhengsu_successful" and p:getMark(player:objectName() .. "ZhengsuChoice-Clear") == tonumber(zss[2]) then
						if room:askForChoice(p, "zhengsu", "drawCards_2+recover_1") ~= "drawCards_2" then
							room:recover(p, sgs.RecoverStruct("zhengsu"))
						else
							p:drawCards(2, "zhengsu")
						end
					end
				end
			end
...
olyuheng = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_RoundStart and player:getCardCount() > 0 then
			room:sendCompulsoryTriggerLog(player, self:objectName())
			if room:askForUseCard(player, "@@olyuheng!", "@olyuheng-card") then
				return
			end
			for _, c in sgs.qlist(player:getCards("he")) do
				if player:isJilei(c) then
					continue
				end
				local sc = olyuhengCard:clone()
				sc:addSubcard(c)
...
oldili = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if room:changeMaxHpForAwakenSkill(player, -1, self:objectName()) then
			local loseskill = {}
			for _, skill in sgs.qlist(player:getVisibleSkillList()) do
				if skill:objectName() == self:objectName() or skill:isAttachedLordSkill() then
					continue
				end
			...
			loseskill = #excg
			if loseskill >= 1 then
				table.insert(excg, "ol_shengzhi")
			end
			if loseskill >= 2 then
				table.insert(excg, "ol_quandao")
			end
			if loseskill >= 3 then
				table.insert(excg, "ol_chigang")
			end
	...
	on_trigger = function(self, event, player, data)
		...
		if str:startsWith("notifyInvoked:") then
			...
			if player:hasSkill(strs[2], true) and not player:hasEquipSkill(strs[2]) then
				...
				if sk and sk:getFrequency(player) ~= sgs.Skill_Compulsory and not sk:isAttachedLordSkill() then
					room:sendCompulsoryTriggerLog(player, self)
					local log = sgs.LogMessage()
		...
		if event == sgs.RoundStart then
			...
			if from and from:isAlive() then
				...
				for _, p in sgs.qlist(room:getAlivePlayers()) do
					...
					if p:hasSkill(self, true) then
						for _, m in ipairs(p:getMarkNames()) do
							if m:startsWith("&lianpojx+:+") and p:getMark(m) > 0 then
								local ms = m:split("+")
								hasr[ms[3]] = (hasr[ms[3]] or 0) + 1
							end
						end
					end
				end
				...
				if from:isWounded() then
					choices = "lianpojx1+lianpojx2"
				end
				if room:askForChoice(from, "lianpojxDeath", choices) == "lianpojx2" then
...
lianpojxbf1 = sgs.CreateMaxCardsSkill {
	...
	extra_func = function(self, target)
		...
		if hasr.has == 1 then
			...
			for _, p in sgs.qlist(target:getSiblings(true)) do
				if p:isDead() then
					local r = p:getRole()
					if r == "lord" then
						r = "loyalist"
...
zhaoluanvs = sgs.CreateViewAsSkill {
	...
	enabled_at_play = function(self, player)
		...
		local has = false
		for _, p in sgs.qlist(tos) do
			if p:getMark("&zhaoluan+#" .. player:objectName()) > 0 then
				has = true
				break
			end
		end
		for _, p in sgs.qlist(tos) do
			if has and p:getMark("zhaoluanDamage" .. player:objectName()) < 1 then
				return true
			end
		end
		return false
...
zhaoluan = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getHp() < 1 and player:hasFlag("Global_Dying") then
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if p:getMark("@zhaoluan") < 1 or player:getHp() > 0 then
					continue
				end
				if p:askForSkillInvoke(self, ToData(player)) then
					...
					for _, sk in sgs.list(player:getVisibleSkillList()) do
						if sk:getFrequency(player) == sgs.Skill_Compulsory or sk:isAttachedLordSkill() then
							continue
						end
					...
					if p == player then
						...
						if dc then
							...
							if player:hasCard(dc) then
								room:giveCard(player, p, dc, self:objectName(), true)
							end
						end
					end
				elseif n == 2 then
...
fengtu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.Death then
			...
			for _, p in sgs.qlist(room:getAlivePlayers()) do
				if p:getTag("fengtuseat"):toInt() < 1 then
					aps:append(p)
				end
			end
...
jiudingmoujiuyuan = sgs.CreateZeroCardViewAsSkill {
	...
	enabled_at_play = function(self, player)
		...
		for _, p in sgs.qlist(player:getAliveSiblings()) do
			if p:getKingdom() == "wu" and p:hasEquip() then
				return true
			end
		end
	end,
			...
			if effect.from and effect.from:hasFlag("CurrentPlayer") and effect.card:getTypeId() > 0 then
				...
				for _, wy in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					...
					for _, pt in sgs.list(patterns()) do
						local dc = dummyCard(pt)
						if wy:getMark(pt .. "keolcihuang_lun") < 1 and dc and (dc:isSingleTargetCard() and dc:isNDTrick() or dc:isKindOf("NatureSlash")) and wy:canUse(dc, effect.from) then
							table.insert(choices, pt)
						end
					end
					if #choices < 1 then
						continue
...
olspzhubeiCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		for _, p in ipairs(targets) do
			local choices = {}
			if source:getMark("slasholspzhubeiUse-PlayClear") < 1 and p:canUse(dummyCard(), source) then
				table.insert(choices, "slash")
			end
			if source:getMark("duelolspzhubeiUse-PlayClear") < 1 and p:canUse(dummyCard("duel"), source) then
			...
			for _, c in sgs.qlist(p:getCards("he")) do
				...
				if not p:isLocked(transcard) then
					table.insert(ids, c:toString())
				end
			...
			if dc then
				transcard:addSubcards(dc:getSubcards())
			end
...
olspzhubei = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardFinished then
			...
			if table.contains(use.card:getSkillNames(), self:objectName()) then
				for _, p in sgs.qlist(use.to) do
					if use.card:hasFlag("DamageDone_" .. p:objectName()) or p:isDead() or use.from:isDead() then
						continue
					end
					if p:hasSkill(self, true) then
						room:recover(p, sgs.RecoverStruct(self:objectName(), p))
...
SkillEvent["SkillEvent2"] = function(self, event, player, data)
	if event == sgs.CardFinished then
		...
		if use.card:getTypeId() > 0 then
			for _, p in sgs.qlist(use.to) do
				if p ~= player and p:hasSkill(self:objectName()) then
					return p
				end
			end
		end
...
SkillEvent["SkillEvent3"] = function(self, event, player)
	return event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Play and player:hasSkill(self:objectName()) and player
end
...
SkillEvent["SkillEvent5"] = function(self, event, player)
	return event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Start and player:hasSkill(self:objectName()) and player
end
SkillEvent["SkillEvent6"] = function(self, event, player)
	return event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Finish and player:hasSkill(self:objectName()) and player
end
...
SkillEvent["SkillEvent9"] = function(self, event, player, data)
	if event == sgs.Dying and player:hasSkill(self:objectName()) then
		return player
	end
end
SkillEvent["SkillEvent10"] = function(self, event, player, data)
	if event == sgs.CardsMoveOneTime then
		local move = data:toMoveOneTime()
		if move.from_places:contains(sgs.Player_PlaceEquip) and move.from:objectName() == player:objectName() and player:hasSkill(self:objectName()) then
			return player
		end
	end
end
...
SkillEvent["SkillEvent13"] = function(self, event, player, data)
	if event == sgs.CardsMoveOneTime then
		local move = data:toMoveOneTime()
		if move.from_places:contains(sgs.Player_PlaceHand) and move.from:objectName() == player:objectName() and player:hasSkill(self:objectName()) then
			return player
		end
	end
end
SkillEvent["SkillEvent14"] = function(self, event, player, data)
	if event == sgs.CardOffset and player:hasSkill(self:objectName()) then
		return player
	end
end
...
SkillEvent["SkillEvent18"] = function(self, event, player, data)
	if event == sgs.CardsMoveOneTime then
		...
		if
			(move.from_places:contains(sgs.Player_PlaceHand) or move.from_places:contains(sgs.Player_PlaceEquip))
			and (move.from ~= move.to or not move.to)
			and move.from:objectName() == player:objectName()
...
SkillEvent["SkillEvent19"] = function(self, event, player)
	return event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Discard and player:hasSkill(self:objectName()) and player
end
...
SkillEvent["SkillEvent21"] = function(self, event, player)
	return event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Draw and player:hasSkill(self:objectName()) and player
end
SkillEvent["SkillEvent22"] = function(self, event, player, data)
	if event == sgs.TargetConfirmed then
		local use = data:toCardUse()
		if use.card:isNDTrick() and use.to:contains(player) and player:hasSkill(self:objectName()) then
			return player
		end
	end
end
...
SkillEvent["SkillEvent25"] = function(self, event, player, data)
	if event == sgs.CardsMoveOneTime then
		local move = data:toMoveOneTime()
		if move.from_places:contains(sgs.Player_PlaceHand) and move.is_last_handcard and player:hasSkill(self:objectName()) then
			return player
		end
	end
end
SkillEvent["SkillEvent26"] = function(self, event, player)
	if event == sgs.HpChanged and player:hasSkill(self:objectName()) then
		return player
	end
end
...
SkillEvent["SkillEvent30"] = function(self, event, player, data)
	if event == sgs.Death and player:hasSkill(self:objectName()) then
		return player
	end
end
...
SkillEffect["SkillEffect5"] = function(self, player, data)
	local room = player:getRoom()
	local damage = data:toDamage()
	if damage.card and damage.card and room:getCardOwner(damage.card:getEffectiveId()) == nil and player:askForSkillInvoke("SkillEffect", ToData("s5"), false) then
		room:addPlayerMark(player, "useTime" .. self:objectName())
		player:skillInvoked(self, -1)
		room:obtainCard(player, damage.card)
	end
...
SkillEffect["SkillEffect16"] = function(self, player, data)
	...
	room:acquireSkill(player, "#SkillEffect11", true, false, false)
	local judge = data:toJudge()
	if judge and judge.card and room:getCardPlace(judge.card:getEffectiveId()) == sgs.Player_PlaceJudge and player:askForSkillInvoke(self, player:getTag("JudgeData"):toJudge()) then
		room:addPlayerMark(player, "useTime" .. self:objectName())
...
SkillEffect["SkillEffect18"] = function(self, player)
	...
	local tps = sgs.SPlayerList()
	for _, p in sgs.qlist(room:getOtherPlayers(player)) do
		if p:isWounded() and player:canPindian(p) then
			tps:append(p)
		end
	end
...
SkillEffect["SkillEffect19"] = function(self, player)
	...
	if tos:length() > 0 then
		room:addPlayerMark(player, "useTime" .. self:objectName())
		room:drawCards(tos, 1, self:objectName())
	end
end
...
SkillEffect["SkillEffect29"] = function(self, player, data)
	local room = player:getRoom()
	player:setTag("SkillEffect", data)
	...
	on_trigger = function(self, event, player, data, room)
		...
		for _, f in ipairs(ses) do
			if
				...
				and str ~= "SkillEvent7"
				and str ~= "SkillEvent9"
				and str ~= "SkillEvent17"
				and str ~= "SkillEvent20"
				and str ~= "SkillEvent24"
				and str ~= "SkillEvent28"
				and str ~= "SkillEvent29"
				and str ~= "SkillEvent30"
			then
				continue
			end
			if f == "SkillEffect13" and str ~= "SkillEvent8" and str ~= "SkillEvent14" and str ~= "SkillEvent22" then
				continue
			end
			if f == "SkillEffect15" and str ~= "SkillEvent12" then
				continue
			end
			if f == "SkillEffect16" and str ~= "SkillEvent12" and str ~= "SkillEvent15" then
				continue
			end
			if f == "SkillEffect29" and str ~= "SkillEvent28" and str ~= "SkillEvent29" then
				continue
			end
			if f == "SkillEffect30" and str ~= "SkillEvent28" and str ~= "SkillEvent29" then
				continue
			end
			table.insert(choices, f)
			if #choices > 2 then
		...
		if event == sgs.EventAcquireSkill then
			if data:toSkillChange().skillName:startsWith("qingshu_tianshu") then
				...
				for _, s in sgs.qlist(player:getVisibleSkillList()) do
					if s:objectName():startsWith("qingshu_tianshu") then
						table.insert(ts, s:objectName())
					end
				end
				if #ts > n then
					n = #ts - n
	...
	on_use = function(self, room, source, targets)
		for _, p in sgs.list(targets) do
			...
			for _, s in sgs.qlist(source:getVisibleSkillList()) do
				if s:objectName():startsWith("qingshu_tianshu") and source:getMark("useTime" .. s:objectName()) < 1 then
					table.insert(choices, s:objectName())
				end
			end
			if #choices < 1 then
				continue
			...
			local trs = source:getTag(choice .. "Translation"):toString()
			room:changeTranslation(p, choice, trs)
		end
	end,
}
...
zhengjingCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source)
		...
		if has_ids:length() < 1 or source:isDead() then
			return
		end
...
xdrenhai = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if damage.to:isAlive() then
			...
			if choices == "" then
				choices = "1xdrenhai+2xdrenhai+3xdrenhai+4xdrenhai"
			end
			for d = 1, damage.damage do
				local _choices = choices
				if d > 1 then
					_choices = _choices .. "+cancel"
				end
...
xdtiantong = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseStart then
			...
			player:obtainCard(dc)
			if room:askForChoice(player, self:objectName(), "xdtiantong1+xdtiantong2") == "xdtiantong2" then
				player:setTag("xdrenhai_choice", sgs.QVariant())
			else
				player:turnOver()
			end
		end
	...
	on_validate_in_response = function(self, from)
		if sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_RESPONSE_USE then
			...
			for _, pc in ipairs(self:getUserString():split("+")) do
				...
				for _, p in sgs.list(room:getAlivePlayers()) do
					if p:hasSkill("xdyifu") then
						...
						if n == 1 and pc == "lightning" or n == 2 and pc == from:property("Suijiyingbian"):toString() or n == 3 and pc == "iron_chain" then
							has = true
							break
						end
					end
			...
			for _, p in sgs.list(room:getAlivePlayers()) do
				if p:hasSkill("xdyifu") then
					local n = p:getChangeSkillState("xdyifu")
					if n == 1 and choice == "lightning" or n == 2 and choice == from:property("Suijiyingbian"):toString() or n == 3 and choice == "iron_chain" then
						tos:append(p)
					end
				end
			end
	...
	on_validate = function(self, use)
		if sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_RESPONSE_USE then
			...
			for _, pc in ipairs(self:getUserString():split("+")) do
				...
				for _, p in sgs.list(room:getAlivePlayers()) do
					if p:hasSkill("xdyifu") then
						...
						if n == 1 and pc == "lightning" or n == 2 and pc == use.from:property("Suijiyingbian"):toString() or n == 3 and pc == "iron_chain" then
							has = true
							break
						end
					end
			...
			for _, p in sgs.list(room:getAlivePlayers()) do
				if p:hasSkill("xdyifu") then
					local n = p:getChangeSkillState("xdyifu")
					if n == 1 and choice == "lightning" or n == 2 and choice == use.from:property("Suijiyingbian"):toString() or n == 3 and choice == "iron_chain" then
						tos:append(p)
					end
				end
			end
			...
			for _, p in sgs.list(use.to) do
				...
				if n == 1 then
					choice = "lightning"
				elseif n == 2 then
					choice = use.from:property("Suijiyingbian"):toString()
				elseif n == 3 then
					choice = "iron_chain"
				end
			end
			...
			for _, p in sgs.list(player:getAliveSiblings(true)) do
				if p:hasSkill("xdyifu") then
					...
					if n == 1 then
						choice = "lightning"
					elseif n == 2 then
						choice = player:property("Suijiyingbian"):toString()
					elseif n == 3 then
						choice = "iron_chain"
					end
...
xdyifu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if event == sgs.TargetConfirmed then
			local use = data:toCardUse()
			if use.card:getTypeId() > 0 and table.contains(use.card:getSkillNames(), self:objectName()) and use.to:contains(player) and player:hasSkill(self) then
				room:sendCompulsoryTriggerLog(player, self)
				player:drawCards(1, self:objectName())
			end
		end
					...
					if p:hasSkill(self) then
						...
						if _judge.who:hasFlag("xdtianjieBf") then
							if _judge:isBad() then
								...
								if id > 0 then
									damage.card = sgs.Sanguosha:getCard(id - 1)
								end
								room:damage(damage)
							end
						else
			...
			if change.to == sgs.Player_NotActive then
				for _, p in sgs.list(room:getAllPlayers()) do
					...
					if ids:length() > 0 then
						...
						if dc:isAvailable(p) and room:askForUseCard(p, "@@xdfangchan", "xdfangchan0:" .. c:objectName()) then
							continue
						end
						p:obtainCard(c)
					end
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.BeforeCardsMove then
			...
			if move.from_places:contains(sgs.Player_PlaceEquip) and move.reason.m_skillName ~= "BreakCard" and move.from:objectName() == player:objectName() then
				for i, id in sgs.list(move.card_ids) do
					if move.from_places:at(i) == sgs.Player_PlaceEquip and sgs.Sanguosha:getCard(id):isKindOf("Wuqibingfa") then
						room:sendCompulsoryTriggerLog(player, self:objectName())
						local ids = sgs.IntList()
						ids:append(id)
						move:removeCardIds(ids)
						...
						local n = 0
						for _, s in sgs.qlist(player:getVisibleSkillList()) do
							if not s:inherits("SPConvertSkill") and not s:isAttachedLordSkill() then
								n = n + 1
							end
						end
						if n < 1 then
	...
	on_use = function(self, room, source, targets)
		...
		for _, p in sgs.list(targets) do
			...
			for _, s in sgs.qlist(p:getVisibleSkillList()) do
				if not s:isAttachedLordSkill() and s:getFrequency() == sgs.Skill_Limited then
					table.insert(SkillList, s:objectName())
				end
			end
			if #SkillList > 0 and p:isAlive() then
...
olmoguofuCard = sgs.CreateSkillCard {
	...
	about_to_use = function(self, room, use)
		...
		for _, pn in sgs.list(use.from:property("olmoguofuPns"):toString():split("+")) do
			if use.from:getMark(pn .. "olmoguofuBan-Clear") > 0 then
				continue
			end
			local dc = dummyCard(pn)
			if dc then
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.PreCardUsed then
			...
			for _, p in sgs.qlist(room:getAlivePlayers()) do
				...
				if damage.card then
					if table.contains(pns, damage.card:objectName()) then
						continue
					end
					table.insert(pns, damage.card:objectName())
				else
					if table.contains(pns, damage.reason) then
						continue
					end
					if sgs.Sanguosha:getSkill(damage.reason) then
						table.insert(pns, damage.reason)
					end
				end
...
olmoguofubf = sgs.CreateCardLimitSkill {
	...
	limit_pattern = function(self, player, card)
		if card:hasTip("olmoguofu") and player:hasSkill("olmoguofu", true) then
		...
		if event == sgs.EventPhaseStart then
			...
			if #pns > 2 and player:getMark("&olmorumo") < 1 and player:askForSkillInvoke(self, data) then
				...
				for _, st in sgs.list(pns) do
					if table.contains(getOLmoDamageSks(), st) then
						table.insert(skills, st)
					end
				end
				if #skills > 0 then
...
olmorumo = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.DamageDone then
			...
			if player:getMark("&olmorumo") > 0 and player:getMark("olmorumoDamage_lun") < 1 then
	...
	if #olmoDamageSks < 1 then
		for _, gn in sgs.list(sgs.Sanguosha:getLimitedGeneralNames()) do
			for _, sk in sgs.list(sgs.Sanguosha:getGeneral(gn):getVisibleSkillList()) do
				if table.contains(olmoDamageSks, sk:objectName()) then
					continue
				end
				local skp = sk:getDescription()
				if string.find(skp, "对") and string.find(skp, "造成") and string.find(skp, "点") and string.find(skp, "伤害") then
					skp = skp:split("对")[2]
...
olmozhouxi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.DamageDone then
			...
			for _, p in sgs.list(room:getAllPlayers()) do
				if player:getMark(p:objectName() .. "olmozhouxiDamage_lun") > 0 and p:hasSkill(self, true) and player:canSlash(p, false) then
...
local ol_trick = sgs.Package("51OLTrick", sgs.Package_CardPack)
...
ol_shengsiyugong = sgs.CreateTrickCard {
	...
	filter = function(self, targets, to_select, source)
		return to_select ~= source
			and to_select:hasFlag("Global_Dying")
...
ol_hongyundangtou = sgs.CreateTrickCard {
	...
	filter = function(self, targets, to_select, source)
		return to_select ~= source
			...
			and not source:isProhibited(source, self)
			and not source:isProhibited(to_select, self)
			and #targets <= sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_ExtraTarget, source, self, to_select)
	end,
	about_to_use = function(self, room, use)
	...
	on_effect = function(self, effect)
		...
		if sc then
			...
			for _, id in sgs.list(RandomList(hearts)) do
				dc:addSubcard(id)
				if dc:subcardsLength() >= sc:subcardsLength() then
					break
				end
			end
...
ol_younantongdang = sgs.CreateTrickCard {
	...
	on_use = function(self, room, source, targets)
		...
		for _, to in sgs.list(targets) do
			...
			if to:isChained() or effect.nullified then
...
ol_liangleichadao = sgs.CreateTrickCard {
	...
	filter = function(self, targets, to_select, source)
		return to_select ~= source and not source:isProhibited(to_select, self) and #targets <= sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_ExtraTarget, source, self, to_select)
	end,
	on_effect = function(self, effect)
		local room = effect.to:getRoom()
		local ids = {}
...
ol_qianjiu = sgs.CreateTrickCard {
	...
	on_effect = function(self, effect)
		local room = effect.to:getRoom()
		if room:askForUseCard(effect.to, "$Analeptic#.|.|9", "ol_qianjiu0") then
			return
		end
...
ol_wutianwujie = sgs.CreateTrickCard {
	...
	can_recast = false,
	available = function(self, player)
		return self:cardIsAvailable(player) and not player:isProhibited(player, self)
	end,
	filter = function(self, targets, to_select, source)
		return not source:isProhibited(to_select, self) and #targets <= sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_ExtraTarget, source, self, to_select)
	end,
	about_to_use = function(self, room, use)
		if use.to:isEmpty() then
			use.to:append(use.from)
	...
	on_effect = function(self, effect)
		...
		local choice = {}
		for _, s in sgs.list(RandomList(getOLmoDamageSks())) do
			if effect.to:hasSkill(s, true) then
				continue
			end
			table.insert(choice, s)
			if #choice > 2 then
...
for i = 0, 3 do
	ol_shengsiyugong:clone(i, 4):setParent(ol_trick)
	ol_hongyundangtou:clone(i, 5):setParent(ol_trick)
	ol_younantongdang:clone(i, 6):setParent(ol_trick)
	ol_luojingxiashi:clone(i, 7):setParent(ol_trick)
	ol_leigongzhuwo:clone(i, 8):setParent(ol_trick)
	ol_liangleichadao:clone(i, 10):setParent(ol_trick)
	ol_xiongdiqixin:clone(i, 11):setParent(ol_trick)
	ol_qianjiu:clone(i, 12):setParent(ol_trick)
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.BeforeCardsMove then
			...
			then
				...
				if draw.reason ~= move.reason.m_skillName then
					return
				end
					...
					if pokerDrawPile:isEmpty() then
						if pokerDiscardPile:isEmpty() then
							for i = 0, sgs.Sanguosha:getCardCount() - 1 do
								if ids:contains(i) or room:getCardOwner(i) then
									continue
								end
								if sgs.Sanguosha:getEngineCard(i):objectName():endsWith("poker") then
									pokerDiscardPile:append(i)
								end
							end
							if pokerDiscardPile:isEmpty() then
								return
				...
				if draw.top then
					...
					room:returnToEndDrawPile(move.card_ids)
				end
				move.card_ids = ids
				data:setValue(move)
...
local function getdianmoSks()
	if #dianmoSks < 1 then
		for _, gn in sgs.list(sgs.Sanguosha:getLimitedGeneralNames()) do
			for _, sk in sgs.list(sgs.Sanguosha:getGeneral(gn):getVisibleSkillList()) do
				if table.contains(dianmoSks, sk:objectName()) then
					continue
				end
				if sgs.Sanguosha:getViewAsSkill(sk:objectName()) == nil then
					continue
				end
				local skp = sk:getDescription()
				if string.find(skp, "你可以将") and (string.find(skp, "牌当") or string.find(skp, "】当")) then
					skp = skp:split("你可以将")[2]
					if (string.find(skp, "牌当") or string.find(skp, "】当")) and (string.find(skp, "使用") or string.find(skp, "打出")) then
...
dianmo = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseStart then
			...
			if player:getMark("dianmoDamaged-Clear") > 1 then
				return
			end
		end
		local sks = {}
...
zaibi = sgs.CreateViewAsSkill {
	...
	view_filter = function(self, selected, to_select)
		...
		for _, c in sgs.list(selected) do
			if math.abs(c:getNumber() - to_select:getNumber()) == 1 then
				return true
			end
		end
		return #selected < 1
...
_chunqiubiCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		for _, ct in sgs.list(choices) do
			if table.contains(choices2, ct) then
				continue
			end
			table.insert(choices2, ct)
		end
...
chunqiubiskill = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.BeforeCardsMove then
			...
			if move.from_places:contains(sgs.Player_PlaceEquip) and move.reason.m_skillName ~= "BreakCard" and move.from:objectName() == player:objectName() then
				for i, id in sgs.qlist(move.card_ids) do
					if move.from_places:at(i) == sgs.Player_PlaceEquip and sgs.Sanguosha:getEngineCard(id):isKindOf("Chunqiubi") then
						local ids = sgs.IntList()
						ids:append(id)
						move:removeCardIds(ids)
						data:setValue(move)
...
ofsifenCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, source)
		if source:getMark("ofsifenRed-PlayClear") > 0 then
			...
			dc:addSubcards(self:getSubcards())
			return source:getMark(to_select:objectName() .. "ofsifenDuel-PlayClear") > 0 and CanToCard(dc, source, to_select, targets)
		end
...
offunan = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if (pattern ~= "slash") or string.find(prompt, "jijiang-slash") or not player:hasTurn() or player:getMark("offunanUse-Clear") > 0 then
			return false
		end
		local lieges = room:getLieges("shu", player)
		if lieges:isEmpty() or not player:askForSkillInvoke(self, data) then
			return false
		end
...
ofjunwei = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.CardFinished then
			local use = data:toCardUse()
			if use.card:isKindOf("Nullification") and table.contains(use.card:getSkillNames(), "ofjunwei") then
				if table.contains(use.nullified_list, "_ALL_TARGETS") then
					return
				end
				use = room:getTag("UseHistory" .. use.card:toString()):toCardUse()
				if use.whocard and table.contains(use.no_offset_list, "_HAS_EFFECT") then
					use = room:getTag("UseHistory" .. use.whocard:toString()):toCardUse()
					if use.to:isEmpty() or use.whocard:isKindOf("DelayedTrick") then
						return
					end
		...
		elseif event == sgs.PostCardEffected then
			...
			if effect.card:hasFlag("ofjunweiBf") then
				...
				if use2.to:contains(effect.to) then
					return
				end
				local use = room:getTag("UseHistory" .. effect.card:toString()):toCardUse()
				local tps = sgs.SPlayerList()
				...
				for _, p in sgs.qlist(use.to) do
					if p == effect.to and tps:last() ~= p then
						break
					end
					if use2.to:contains(p) and p:isAlive() and p:getMark("ofjunweiBf") < 1 then
						p:addMark("ofjunweiBf")
				...
				if tps:last() == effect.to then
					for _, p in sgs.qlist(use2.to) do
						p:removeMark("ofjunweiBf")
					end
				end
			end
		end
...
ofshezuo = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.Pindian then
			...
			if player:getPhase() == sgs.Player_Start and player:hasSkill(self) and player:askForSkillInvoke(self) then
	...
	on_use = function(self, room, player, targets)
		...
		for i, p in sgs.list(room:getAlivePlayers()) do
			...
			for ii, q in sgs.list(room:getAlivePlayers()) do
				if table.contains(targets, q) or i <= ii then
					continue
				end
				if x + q:getHandcardNum() < n then
					...
					if use.card then
						...
						while use.to:length() > 0 do
							...
							moves:append(
								sgs.CardsMoveStruct(ids:last(), tp, sgs.Player_PlaceHand, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GIVE, player:objectName(), tp:objectName(), "ofjixu", ""))
							)
							...
							if use.to:isEmpty() then
								moves:append(
									sgs.CardsMoveStruct(ids, tp, sgs.Player_PlaceHand, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GIVE, player:objectName(), tp:objectName(), "ofjixu", ""))
								)
							end
						end
					end
...
ofyouchongCard = sgs.CreateSkillCard {
	...
	on_validate_in_response = function(self, user)
		...
		local tps = sgs.SPlayerList()
		for _, p in sgs.list(room:getAlivePlayers()) do
			if p:getHandcardNum() > user:getHandcardNum() then
				tps:append(p)
			end
		end
...
nyarz_falu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		local suits = player:getTag("nyarz_zhenyi_suits"):toString()
		room:setPlayerMark(player, "&nyarz_falu+" .. suits, 0)
		player:removeTag("nyarz_zhenyi_suits")
		suits = suits:split("+")
		for i, id in sgs.qlist(move.card_ids) do
			if move.from_places:at(i) == sgs.Player_PlaceHand then
				table.insert(suits, sgs.Sanguosha:getCard(id):getSuitString() .. "_char")
			end
		end
		while #suits > 3 do
			table.remove(suits, 1)
		end
		if #suits >= 3 then
			local st1 = suits[1]
			local st2 = suits[2]
			local st3 = suits[3]
			if st1 == st2 and st2 == st3 and st1 == st3 then
				local st = st1:sub(1, #st1 - 5)
				local _suits = { st }
				local move = sgs.CardsMoveStruct()
				for _, id in sgs.qlist(room:getDrawPile()) do
					local card = sgs.Sanguosha:getCard(id)
					if not table.contains(_suits, card:getSuitString()) then
						table.insert(_suits, card:getSuitString())
						move.card_ids:append(id)
					end
				end
				move.to = player
				move.to_place = sgs.Player_PlaceHand
				move.reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_DRAW, player:objectName(), self:objectName(), "")
				room:sendCompulsoryTriggerLog(player, self)
				room:moveCardsAtomic(move, true)
				if player:hasSkill("nyarz_zhenyi") and player:getMark("nyarz_zhenyi_" .. st) == 0 then
					local names = {}
					_suits = {}
					room:setPlayerMark(player, "nyarz_zhenyi_" .. st, 1)
					for _, name in ipairs(player:getMarkNames()) do
						if name:startsWith("nyarz_zhenyi_") then
							name = string.gsub(name, "nyarz_zhenyi_", "")
							table.insert(names, name)
							table.insert(_suits, name .. "_char")
						elseif name:startsWith("&nyarz_zhenyi+") then
							room:setPlayerMark(player, name, 0)
						end
					end
					room:sendCompulsoryTriggerLog(player, "nyarz_zhenyi")
					player:setSkillDescriptionSwap(self:objectName(), "%arg1", table.concat(names, "+|"))
					room:changeTranslation(player, self:objectName(), 2)
					room:setPlayerMark(player, "&nyarz_zhenyi+" .. table.concat(_suits, "+"), 1)
				end
				return false
			elseif st1 ~= st2 and st2 ~= st3 and st1 ~= st3 then
				if player:hasSkill("nyarz_zhenyi") and player:getMark("nyarz_dianhua") < 4 then
					room:addPlayerMark(player, "nyarz_dianhua", 1)
				end
				local target = room:askForPlayerChosen(player, room:getAlivePlayers(), self:objectName(), "@nyarz_falu-choose", false, true)
				if target then
					player:peiyin(self)
					if room:askForChoice(player, self:objectName(), "recover+losehp", ToData(target)) == "recover" then
...
nyarz_dianhua = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		local n = 1 + player:getMark("nyarz_dianhua")
...
ny_10th_zhangqiying:addSkill(nyarz_falu)
ny_10th_zhangqiying:addSkill(nyarz_zhenyi)
ny_10th_zhangqiying:addSkill(nyarz_zhenyi_buff)
ny_10th_zhangqiying:addSkill(nyarz_dianhua)
extension:insertRelatedSkills("nyarz_zhenyi", "#nyarz_zhenyi_buff")
sgs.LoadTranslationTable {
	["ny_10th_zhangqiying"] = "张琪瑛[十周年]",
	["nyarz_falu"] = "法箓",
	[":nyarz_falu"] = "锁定技，当你失去一张手牌时，记录此牌花色（只记录最近三次）。当记录的三个花色相同时，你移除记录花色并从牌堆中获得其他三种花色的牌各一张；当记录的三个花色各不相同时，你可以移除记录花色并令一名角色失去或回复1点体力。",
	[":nyarz_falu2"] = '锁定技，当你失去一张手牌时，记录此牌花色（只记录最近三次）。当记录的三个花色相同时，你移除记录花色并从牌堆中获得其他三种花色的牌各一张；当记录的三个花色各不相同时，你可以移除记录花色并令一名角色失去或回复1点体力。<font color="red"><b>已强化：%arg1</b></font>',
	["@nyarz_falu-choose"] = "使一名角色回复或失去1点体力",
	["nyarz_falu:recover"] = "回复体力",
	["nyarz_falu:losehp"] = "失去体力",
	["nyarz_zhenyi"] = "真仪",
	[":nyarz_zhenyi"] = "锁定技，当“法箓”移除三个同花色后，你本局游戏使用此花色的牌无距离限制且不能被响应。当“法箓”移除三个各不相同的花色后，你本局游戏“点化”可观看的牌数+1（最多+4）。",
	["nyarz_dianhua"] = "点化",
	[":nyarz_dianhua"] = "准备阶段或结束阶段，你可以观看牌堆顶1张牌，然后获得其中一张牌，其余的牌以任意顺序置于牌堆顶。",
	["nyarz_dianhua:view"] = "你可以发动“点化”观看牌堆顶的%src张牌并以任意顺序置于牌堆顶，然后摸一张牌。",
}
...
cangqiao = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardUsed then
			local use = data:toCardUse()
			if
				use.card:isKindOf("EquipCard")
				and player:getTag("cangqiaoIds"):toIntList():contains(use.card:getId())
				and player:getHandcardNum() < player:getMaxHp()
				and player:askForSkillInvoke(self)
			then
				player:drawCards(player:getMaxHp() - player:getHandcardNum(), self:objectName())
			end
		else
			local dc = dummyCard()
			local cns = { "yj_numa", "yj_nvzhuang", "yj_zheji" }
			for i = 0, sgs.Sanguosha:getCardCount() - 1 do
				local c = sgs.Sanguosha:getEngineCard(i)
				if c:getPackage() == "yongjian" and room:getCardOwner(i) == nil and room:getCardPlace(i) ~= sgs.Player_DrawPile then
					if table.contains(cns, c:objectName()) then
						table.removeOne(cns, c:objectName())
						dc:addSubcard(i)
						if #cns < 1 then
							break
						end
			...
			if dc:subcardsLength() > 0 and player:askForSkillInvoke(self) then
				player:setTag("cangqiaoIds", ToData(dc:getSubcards()))
				player:obtainCard(dc)
			end
		end
	end,
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardFinished then
			...
			if use.card:hasFlag("godshenjiUse") and use.to:length() == 1 then
				for _, p in sgs.qlist(use.to) do
					if p:isAlive() and p:getMark("godshenjiUse-Clear") < 1 and p:hasTurn() and p:hasSkill(self) then
						local cns = p:getTag("godshenjiCNS"):toString():split("+")
						if #cns < 1 then
							cns = sgs.Sanguosha:getCardNames("DelayedTrick")
							p:setTag("godshenjiCNS", ToData(table.concat(cns, "+")))
						end
						local cns2 = {}
						for _, n in sgs.list(cns) do
							local dc = dummyCard(n, self:objectName())
						...
						for _, q in sgs.qlist(room:getAlivePlayers()) do
							...
							room:acquireSkill(q, "#godshenjibf", true, false, false)
						end
						local tp = room:askForPlayerChosen(p, tps, self:objectName(), "godshenji0", true)
						if tp then
							local cn = room:askForChoice(p, self:objectName(), table.concat(cns2, "+"))
							table.removeOne(cns, cn)
							room:setPlayerProperty(p, "godshenjiCN", ToData(cn))
							room:notifyMoveToPile(p, tp:getEquipsId(), self:objectName())
							if room:askForUseCard(p, "@@godshenji", "godshenji1:" .. cn) then
								p:setTag("godshenjiCNS", ToData(table.concat(cns, "+")))
							end
						end
					end
			...
			local use = data:toCardUse()
			if use.card:getTypeId() > 0 and use.card:isBlack() then
				room:setCardFlag(use.card, "godshenjiUse")
			end
			if table.contains(use.card:getSkillNames(), self:objectName()) then
...
godshenjibf = sgs.CreateViewAsEquipSkill {
	name = "#godshenjibf",
	view_as_equip = function(self, target)
		local ens = {}
		for _, j in sgs.qlist(target:getJudgingArea()) do
			if j:getSkillName() == "godshenji" then
				local ec = sgs.Sanguosha:getEngineCard(j:getEffectiveId())
				if ec:isKindOf("EquipCard") then
					table.insert(ens, ec:objectName())
					for _, p in sgs.qlist(target:getAliveSiblings(true)) do
						local uer = p:property("huaxiuUpEquip"):toString()
						if uer == "" then
							continue
						end
						uer = uer:split("=")
						if uer[2] == ec:objectName() then
							table.removeOne(ens, ec:objectName())
							table.insert(ens, uer[3])
						end
					end
				end
			end
		...
		return table.concat(ens, ",")
	end,
}
shenhuangyueying:addSkill(godshenjibf)
huaxiuCard = sgs.CreateSkillCard {
	name = "huaxiuCard",
	target_fixed = true,
	will_throw = false,
	on_use = function(self, room, source, targets)
		local cn = "hxup=yj_zheji=_ofhun_crossbow+hxup=yj_nvzhuang=_ofhun_eight_diagram+hxup=yj_numa=_of_lingling"
		cn = room:askForChoice(source, "huaxiu", cn)
		room:setPlayerProperty(source, "huaxiuUpEquip", ToData(cn))
		for _, q in sgs.qlist(room:getAlivePlayers()) do
			room:acquireSkill(q, "#huaxiubf", true, false, false)
		end
		cn = cn:split("=")
...
huaxiu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseChanging then
			...
			local death = data:toDeath()
			if death.whp ~= player then
				return
			end
		end
		room:setPlayerProperty(player, "huaxiuUpEquip", sgs.QVariant())
		for _, m in ipairs(player:getMarkNames()) do
			if m:endsWith("+#hxup") then
				room:setPlayerMark(player, m, 0)
			end
		end
	end,
...
huaxiubf = sgs.CreateViewAsEquipSkill {
	name = "#huaxiubf",
	view_as_equip = function(self, target)
		local ens = {}
		for _, p in sgs.qlist(target:getAliveSiblings(true)) do
...
shenhuangyueying:addSkill(huaxiubf)
huaxiubf2 = sgs.CreateCardLimitSkill {
	name = "#huaxiubf2",
	limit_list = function(self, player, card)
		return "effect"
	end,
	limit_pattern = function(self, player, card)
		for _, p in sgs.qlist(player:getAliveSiblings(true)) do
			local ue = p:property("huaxiuUpEquip"):toString()
			if ue == "" then
				continue
			end
			local uer = ue:split("=")
			if uer[2] == card:objectName() then
				return card:toString()
			end
		end
		return ""
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.TargetSpecified then
			...
			if use.card:isKindOf("Slash") and player:hasWeapon("_ofhun_crossbow") then
				if player:usedTimes("Slash") > 1 then
					room:setEmotion(player, "weapon/crossbow")
				end
				local choices = {}
				for _, p in sgs.qlist(room:getPlayers()) do
					if p:isDead() then
						table.insert(choices, "_ofhun0=" .. p:objectName())
					end
				end
				if #choices < 1 or not player:askForSkillInvoke("_ofhun_crossbow", data) then
					return
				end
				local can = {}
				for i = 1, #choices do
					local choice = room:askForChoice(player, "_ofhun_crossbow", table.concat(choices, "+"), data)
					if i == 1 then
						table.insert(choices, "cancel")
					end
					if choice == "cancel" then
					...
					table.removeOne(choices, choice)
					table.insert(can, choice:split("=")[2])
				end
				for _, p in sgs.qlist(room:getAllPlayers(true)) do
					if table.contains(can, p:objectName()) then
						for _, t in sgs.qlist(use.to) do
							local id = room:doGongxin(p, t, t:handCards(), "_ofhun_crossbow")
							if id < 0 then
								continue
							end
							UseCardRecast(t, id, "_ofhun_crossbow", 1)
						end
					end
				end
			end
...
_ofhun_crossbow = sgs.CreateWeapon {
	name = "_ofhun_crossbow",
	class_name = "Crossbow",
	range = 1,
	suit = 6,
	...
	on_install = function(self, player)
		local room = player:getRoom()
		room:acquireSkill(player, _crossbowSkillbf, true, true, false)
		return false
	end,
	on_uninstall = function(self, player)
		local room = player:getRoom()
		room:detachSkillFromPlayer(player, "#_ofhun_crossbowbf", true)
		return false
	end,
}
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardAsked then
			...
			if string.find(pattern[1], "jink") and player:hasArmorEffect(self:objectName()) then
				local jink = dummyCard("jink")
				jink:setSkillName("__ofhun_eight_diagram")
				if player:isCardLimited(jink, pattern[3] == "use" and sgs.Card_MethodUse or sgs.Card_MethodResponse) then
					return
				end
				if player:askForSkillInvoke(self, data) then
					...
					if #choices > 0 and player:askForSkillInvoke("_ofhun_eight_diagram0", ToData("tp"), false) then
						local choice = room:askForChoice(player, self:objectName(), table.concat(choices, "+"))
						for _, p in sgs.qlist(room:getPlayers()) do
							if choice:endsWith(p:objectName()) then
								choices = room:getNCards(3)
								room:askForGuanxing(p, choices)
							end
						end
					end
					room:setEmotion(player, "armor/eight_diagram")
...
_ofhun_eight_diagram = sgs.CreateArmor {
	name = "_ofhun_eight_diagram",
	class_name = "EightDiagram",
	suit = 6,
	number = 0,
	on_install = function(self, player)
		local room = player:getRoom()
		room:acquireSkill(player, _ofhun_eight_diagramSkill, true, true, false)
		return false
	end,
	on_uninstall = function(self, player)
		local room = player:getRoom()
		room:detachSkillFromPlayer(player, "_ofhun_eight_diagram", true, true)
		return false
	end,
}
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseStart then
			if player:getPhase() == sgs.Player_Start then
				...
				if tp then
					room:damage(sgs.DamageStruct("_of_lingling", player, tp, 1, sgs.DamageStruct_Thunder))
				end
			end
		else
			...
			if oh:objectName() == "_of_lingling" or player:property("huaxiuUpEquip"):toString():contains(oh:objectName()) then
				local choices = {}
				for _, p in sgs.qlist(room:getPlayers()) do
					if p:isDead() then
						local choice = room:askForChoice(p, self:objectName(), "_of_lingling1+_of_lingling2")
						table.insert(choices, choice)
					end
				end
				for _, t in ipairs(choices) do
					local owner = room:getCardOwner(oh:getId())
					if owner == nil then
						break
					end
					if t == "_of_lingling1" then
						owner = owner:getNextAlive(room:alivePlayerCount() - 1)
					else
						owner = owner:getNextAlive()
					end
					InstallEquip(oh, owner, "_of_lingling")
				end
			end
		end
		return false
...
_of_lingling = sgs.CreateOffensiveHorse {
	name = "_of_lingling",
	class_name = "OfLingling",
	correct = -2,
	suit = 6,
	number = 0,
	on_install = function(self, player)
		local room = player:getRoom()
		room:acquireSkill(player, _of_linglingSkill, true, true, false)
	end,
	on_uninstall = function(self, player)
		local room = player:getRoom()
		room:detachSkillFromPlayer(player, "_of_lingling", true, true)
	end,
}
extension:addSkills(_of_linglingSkill)
_of_lingling:setParent(extension)

of_zhonghui = sgs.General(extension, "of_zhonghui", "wei", 4)
ofquanweiCard = sgs.CreateSkillCard {
	name = "ofquanweiCard",
	will_throw = false,
	filter = function(self, targets, to_select, from)
	...
	on_use = function(self, room, player, targets)
		...
		local ys = { reason = "ofquanwei", from = player }
		ys.tos = { player }
		for _, p in sgs.list(targets) do
			table.insert(ys.tos, p)
		end
		ys.effect = function(ys_data)
			if ys_data.result == self:getColorString() then
				local n = 0
				for i = sgs.Player_Judge, sgs.Player_Finish do
					if player:isSkipped(i) then
						break
					end
					if room:askForChoice(player, "ofquanwei", "ofquanwei_" .. i .. "+cancel") == "cancel" then
						break
					end
					player:skip(i)
					n = n + 1
				...
				local tps = room:askForPlayersChosen(player, "ofquanwei", 0, 2, "ofquanwei1:" .. n)
				for _, p in sgs.qlist(tps) do
					room:doAnimate(1, player:objectName(), p:objectName())
				end
				for _, p in sgs.qlist(tps) do
					room:recover(p, sgs.RecoverStruct("ofquanwei", player, n))
				end
			elseif player:askForSkillInvoke("ofquanwei", ToData("str"), false) then
				room:loseMaxHp(player, 1, "ofquanwei")
				if player:isDead() then
					return
				end
				for _, p in sgs.qlist(room:getAllPlayers()) do
					if table.contains(ys_data.tos, p:objectName()) then
						if ysdata.to2color[p:objectName()]:contains(ysdata.to2color[player:objectName()]) or ysdata.to2color[player:objectName()]:contains(ysdata.to2color[p:objectName()]) then
							continue
						end
						local dc = dummyCard()
						dc:addSubcards(p:getHandcards())
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventForDiy then
			...
			if str:startsWith("yishiresult:") then
				str = str:split(":")
				for _, p in sgs.qlist(room:getAllPlayers()) do
					if table.contains(str[4]:split("+"), p:objectName()) and p:hasSkill(self:objectName()) then
						local n = p:getCards("ej"):length()
						if n > 0 and p:askForSkillInvoke(self) then
							p:drawCards(n, self:objectName())
							local dc = room:askForExchange(p, self:objectName(), 99, 1, false, "ofquanshu0", true)
							if dc then
								xumouCard(p, dc)
							end
						end
		...
		elseif player:hasSkill(self:objectName()) then
			local n = player:getCards("ej"):length()
			if n > 0 and player:askForSkillInvoke(self) then
				player:drawCards(n, self:objectName())
				local dc = room:askForExchange(player, self:objectName(), 99, 1, false, "ofquanshu0", true)
				if dc then
					xumouCard(player, dc)
				end
			end
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseStart then
			if player:getPhase() == sgs.Player_Finish and player:getMark("@ofquanqing") > 0 and player:getHandcardNum() > 0 then
				...
				if dc then
					room:removePlayerMark(player, "@ofquanqing")
					player:skillInvoked(self)
					room:doSuperLightbox(player, self:objectName())
					local sc = ofquanweiCard:clone()
					sc:addSubcard(dc)
					local use = sgs.CardUseStruct(sc, player)
					for _, p in sgs.qlist(room:getAlivePlayers()) do
					...
					room:useCard(use)
					sc:deleteLater()
					local k = room:askForKingdom(player, self:objectName(), "", false)
					if k ~= player:getKingdom() then
						room:changeKingdom(player, k)
						room:acquireSkill(player, "ofqiangyi")
					end
				end
			end
		end
...
ofqiangyiCard = sgs.CreateSkillCard {
	name = "ofqiangyiCard",
	will_throw = false,
	filter = function(self, targets, to_select, user)
		local card = sgs.Sanguosha:cloneCard("_of_yushijiesui")
		card:addSubcards(self:getSubcards())
		card:setSkillName("ofqiangyi")
		card:deleteLater()
		if card:targetFixed() or to_select:getKingdom() ~= user:getKingdom() then
			return false
		end
		local plist = sgs.PlayerList()
		for i = 1, #targets do
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardsMoveOneTime then
			...
			if move.to_place == sgs.Player_PlaceDelayedTrick and move.to:objectName() == player:objectName() then
				for _, id in sgs.qlist(move.card_ids) do
					if sgs.Sanguosha:getCard(id):isKindOf("Xumou") then
						room:sendCompulsoryTriggerLog(player, self)
						for _, id in sgs.qlist(sgs.Sanguosha:getRandomCards(true)) do
							if sgs.Sanguosha:getCard(id):isKindOf("Ying") and room:getCardOwner(id) == nil then
...
_of_yushijiesui = sgs.CreateTrickCard {
	name = "_of_yushijiesui",
	class_name = "Yushijiesui",
	subclass = sgs.LuaTrickCard_TypeSingleTargetTrick,
	target_fixed = false,
	can_recast = false,
	is_cancelable = true,
	...
	filter = function(self, targets, to_select, user)
		return #targets <= sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_ExtraTarget, user, self, to_select) and not user:isProhibited(to_select, self)
	end,
	feasible = function(self, targets, user)
		return #targets > 0 or not user:isProhibited(user, self)
	end,
	about_to_use = function(self, room, use)
		if use.to:isEmpty() then
			use.to:append(use.from)
...
sgs.LoadTranslationTable {
	...
	["of_zhonghui"] = "钟会[线下]",
	["#of_zhonghui"] = "荡涤山河",
	["illustrator:of_zhonghui"] = "鬼画府",
	["ofquanwei"] = "权威",
	[":ofquanwei"] = "准备阶段，你可以展示一张手牌，然后与一名其他角色议事，若结果与之颜色：相同，你依次跳过下任意个阶段，令至多两名角色回复等量体力；不同，你可以扣减1点体力上限，获得与你意见不同角色的所有手牌。",
	["ofquanshu"] = "权术",
	[":ofquanshu"] = "当你议事结束或受到伤害后，你可以摸X张牌并蓄谋（X为你场上的牌数）；你的手牌上限+X。",
	["ofquanqing"] = "权倾",
	[":ofquanqing"] = "限定技，结束阶段，你可以发动“权威”且目标改为攻击范围内所有角色，然后你可以变更势力并获得“戕异”。",
	["ofqiangyi"] = "戕异",
	[":ofqiangyi"] = "主公技，当你蓄谋后，你获得一张【影】；你可以将【影】当【玉石皆碎】对势力与你相同的角色使用。",
	["_of_yushijiesui"] = "玉石皆碎",
	[":_of_yushijiesui"] = "锦囊牌·单目标锦囊<br/><b>时机</b>：出牌阶段，对一名角色使用<br/><b>效果</b>：你对目标造成1点伤害，然后你失去1点体力。",
	["ofquanwei0"] = "你可以发动“权威”选择手牌展示并选择角色议事",
	["ofquanwei1"] = "权威：你可以选择至多2名角色回复%src点体力",
	["ofquanwei_2"] = "跳过判定阶段",
	["ofquanwei_3"] = "跳过摸牌阶段",
	["ofquanwei_4"] = "跳过出牌阶段",
	["ofquanwei_5"] = "跳过弃牌阶段",
	...
	["illustrator:shenhuangyueying"] = "鬼画府",
	["cangqiao"] = "藏巧",
	[":cangqiao"] = "每轮开始时，你可以获得游戏外或弃牌堆的【折戟】、【女装】、【驽马】至多各一张；当你使用这些牌时，你可以摸牌至体力上限。",
	["godshenji"] = "神机",
	[":godshenji"] = "每回合限一次，你为唯一目标的黑色牌结算后你可以将场上一张装备牌当未以此法使用过的延时锦囊使用（均使用后清除记录），这些牌于判定区里同时视为拥有转化所用装备牌效果。",
	["huaxiu"] = "化朽",
	[":huaxiu"] = "出牌阶段限一次，你可以将一种“藏巧”装备牌效果修改对应副类别的牌直到你下回合开始；【魂·诸葛连弩】、【魂·八卦阵】、【軨軨】。",
	["godshenji0"] = "你可以发动“神机”选择角色装备当做延时锦囊",
	["godshenji1"] = "神机：请选择装备当做【%src】",
	["huaxiu:hxup"] = "%src->%arg",
	["#godshenji"] = "对方装备",

	["_ofhun_crossbow"] = "魂·诸葛连弩",
	[":_ofhun_crossbow"] = "装备牌/武器<br/><b>攻击范围</b>：1<br/><b>武器技能</b>：锁定技，你使用【杀】无次数限制；当你使用【杀】指定目标后，你可以令任意名死亡角色依次观看目标手牌且可以重铸其中一张。",
	["_ofhun_eight_diagram"] = "魂·八卦阵",
	[":_ofhun_eight_diagram"] = "装备牌/防具<br/><b>防具技能</b>：当你需要使用或打出一张【闪】时，你可以进行判定（你可以先令一名死亡角色卜算3）：若结果为红色，视为你使用或打出了一张【闪】。",
	["_of_lingling"] = "軨軨",
	[":_of_lingling"] = "装备牌/坐骑<br/><b>坐骑技能</b>：锁定技，你计算与其他角色距离-2；准备阶段，你对一名角色造成1点雷电伤害；每轮结束时，所有死亡角色同时秘密选择上家或下家，然后按死亡顺序依次移动此牌。",
	["_ofhun0"] = "选择%src",
	["_ofhun_eight_diagram0:tp"] = "你可以先令一名死亡角色卜算3",
	["_of_lingling0"] = "軨軨：请选择对一名角色造成伤害",
	["_of_lingling1"] = "【軨軨】移至上家",
	["_of_lingling2"] = "【軨軨】移至下家",
	["ofhun_crossbow"] = "魂·诸葛连弩",
	...
	["renyizhu"] = "遗珠",
	[":renyizhu"] = "结束阶段，你摸2张牌，然后选择2张牌作为“遗珠”，随机洗入牌堆顶前X张牌中（X为场上存活角色数×2），并记录“遗珠”牌的牌名。其他角色使用“遗珠”牌指定唯一目标时，你可以取消之，然后你将此牌从“遗珠”记录中移除。",
	...
	["huishi"] = "慧识",
	[":huishi"] = "出牌阶段限一次，若你的体力上限小于10，你可以进行一次判定，若判定结果与此阶段内以此法进行判定的判定结果花色均不同，且你的体力上限小于10，你可以加1点体力上限并重复此流程。"
...
juemouCard = sgs.CreateSkillCard{
	name = "juemouCard",
	filter = function(self,targets,to_select,player)
		if #targets==1 then return targets[1]~=to_select and targets[1]:canDiscard(to_select,"he") end
		return #targets<1
	end,
	...
	about_to_use = function(self,room,use)
		room:setTag("juemouUse",ToData(use))
		self:cardOnUse(room,use)
	end,
	on_use = function(self,room,player,targets)
		room:setChangeSkillState(player,"juemou", 1)
		if player:getGeneralName():endsWith("jiangwei") then
			player:setAvatarIcon(player:getGeneralName())
		end
		local use = room:getTag("juemouUse"):toCardUse()
		local id = room:askForCardChosen(use.to:first(),use.to:last(),"he","juemou",false,sgs.Card_MethodDiscard)
		if id>=0 then
			room:throwCard(id,"juemou",use.to:last(),use.to:first())
			room:damage(sgs.DamageStruct("juemou",use.to:last(),use.to:first()))
		end
	end
}
        ...
        elseif event == sgs.GameStart then
			...
			if event == sgs.EventPhaseChanging then
				...
				if change.to == sgs.Player_NotActive or change.from == sgs.Player_NotActive then
					if player:getMark("juemou_modity")<1 then return end
				else return end
			else
				local use = data:toCardUse()
				if use.card:getTypeId()~=2 then return end
			end
			if player:getChangeSkillState(self:objectName()) == 1 then
				if player:askForSkillInvoke(self:objectName()+"$-1",data) then
					room:setChangeSkillState(player,self:objectName(), 2)
					if player:getGeneralName():endsWith("jiangwei") then
						player:setAvatarIcon("thmou_jiangwei2")
					end
					room:damage(sgs.DamageStruct(self:objectName(), player, player))
					if player:isAlive() then
						room:drawCards(player, math.min(5,player:getLostHp()), self:objectName())
					end
				end
			else
		...
		for _,p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
			if p:getMark("@fuzhan")>0 and player:getHp() < player:getMaxHp() then
				if room:askForSkillInvoke(p, self:objectName(), player) then
					room:removePlayerMark(p, "@fuzhan")
					room:broadcastSkillInvoke(self:objectName())
					room:doSuperLightbox(p, self:objectName())
					room:recover(player, sgs.RecoverStruct(self:objectName(), p, player:getMaxHp() - player:getHp()))
					room:addPlayerMark(p, "juemou_modity")
				end
			end
		end
	end,
...
-- 谋姜维
local tenyear_mou = sgs.Sanguosha:getPackage("tenyear_mou")
thmou_jiangwei = sgs.General(tenyear_mou, "thmou_jiangwei", "shu", 4, true, false, false, 3)
thmou_jiangwei:addSkill(juemou)
thmou_jiangwei:addSkill(fuzhan)
sgs.Sanguosha:setPackage(tenyear_mou)

sgs.LoadTranslationTable{
	["thmou_jiangwei"] = "谋姜维",
	["#thmou_jiangwei"] = "锲谋昭汉",
	["illustrator:thmou_jiangwei"] = "鬼画府",

	["juemou"] = "绝谋",
	[":juemou"] = "转换技，游戏开始时自选①②状态。你使用锦囊牌时，你可以①对自己造成1点伤害并摸X张牌（X为你已损失体力值且至多为5）②令一名角色弃置另一名角色1张牌并受到其造成的1点伤害。若你因此技能进入濒死，你将体力值回复至1点。",	
	[":juemou1"] = "转换技，游戏开始时自选①②状态。你使用锦囊牌时，你可以①对自己造成1点伤害并摸X张牌（X为你已损失体力值且至多为5）<font color='#01A5AF'><s>②令一名角色弃置另一名角色1张牌并受到其造成的1点伤害</s></font>。若你因此技能进入濒死，你将体力值回复至1点。",	
	[":juemou2"] = "转换技，游戏开始时自选①②状态。你使用锦囊牌时，你可以<font color='#01A5AF'><s>①对自己造成1点伤害并摸X张牌（X为你已损失体力值且至多为5）</s></font>②令一名角色弃置另一名角色1张牌并受到其造成的1点伤害。若你因此技能进入濒死，你将体力值回复至1点。",	
	["@juemou_discard"] = "绝谋：你选择一名角色弃置另一名角色1张牌",
	["fuzhan"] = "复盏",
	[":fuzhan"] = "限定技，有角色脱离濒死时，你可令其回复满体力值，然后修改“绝谋”，增加回合开始和结束时可发动。",

	["$juemou1"] = "我若束手待缚，四百年来英雄血，岂不枉流！",
	["$juemou2"] = "武侯遗志，绝不可成泡影！",
	["$fuzhan1"] = "丞相！五丈原的那盏灯，没有灭！",
	["$fuzhan2"] = "汉贼不两立，王业不偏安！",
	["~thmou_jiangwei"] = "天地君师在上，维尽力了。",
}

--qiaoguolao = sgs.General(ol_ccxh,"qiaoguolao","wu",3)

--[[shendiaochan = sgs.General(extension,"shendiaochan","god",3,false)
shendiaochan:addSkill(meihun)
shendiaochan:addSkill(huoxin)]]

ofjie_yuji = sgs.General(extension, "ofjie_yuji", "qun", 3)
ofguhuoCard = sgs.CreateSkillCard{
	name = "ofguhuoCard",
	will_throw = false,
	filter = function(self,targets,to_select,user)
		local card = sgs.Sanguosha:cloneCard(self:getUserString():split("+")[1])
		card:setSkillName("ofguhuo")
		card:deleteLater()
	...
	on_validate = function(self,use)
		local room = use.from:getRoom()
		local tc = room:askForChoice(use.from,"ofguhuo",self:getUserString())
		local dc = dummyCard(tc)
		room:addPlayerMark(use.from,dc:getType().."ofguhuoBan-Clear")
		if use.from:getPhase()==sgs.Player_Play then
			for _,cn in sgs.list(patterns())do
				if dc:getType()==dummyCard(cn):getType() then
					room:addPlayerMark(use.from,"ofguhuo_guhuo_remove_"..cn.."-Clear")
				end
			end
		end
		local tps = sgs.SPlayerList()
		...
		for _,p in sgs.qlist(tps)do
			local choice = "question+noquestion"
			if p:hasSkill("ofhuinu") then
				room:sendCompulsoryTriggerLog(p,"ofhuinu")
				choice = "question"
			end
			choice = room:askForChoice(p,"ofguhuo",choice,ToData(use.from))
		...
		room:moveCardTo(self,nil,sgs.Player_PlaceTable,true)
		local c = sgs.Sanguosha:getCard(self:getEffectiveId())
		if tc=="normal_slash" and c:objectName()=="slash"
		or tc=="slash" and c:isKindOf("Slash")
		or c:sameNameWith(tc,true) then
			for _,p in sgs.qlist(has)do
				room:loseHp(p,1,true,use.from,"ofguhuo")
				if p:isAlive() then
					room:acquireSkill(p,"ofhuinu")
				end
			end
			return c
		end
		room:throwCard(self:getSubcards(),"ofguhuo",nil)
		for _,p in sgs.qlist(tps)do
			if has:contains(p) or p:isKongcheng() then continue end
			dc = room:askForExchange(p,"ofguhuo",1,1,false,"ofguhuo0:"..use.from:objectName())
			if dc then room:giveCard(p,use.from,dc,"ofguhuo") end
			if use.from:isDead() then break end
		end
		return nil
	end,
	on_validate_in_response = function(self,user)
		local room = user:getRoom()
		local tc = room:askForChoice(user,"ofguhuo",self:getUserString())
		local dc = dummyCard(tc)
		room:addPlayerMark(user,dc:getType().."ofguhuoBan-Clear")
		if user:getPhase()==sgs.Player_Play then
			for _,cn in sgs.list(patterns())do
				if dc:getType()==dummyCard(cn) then
					room:addPlayerMark(user,"ofguhuo_guhuo_remove_"..cn.."-Clear")
				end
			end
		end
		local tps = sgs.SPlayerList()
		...
		for _,p in sgs.qlist(tps)do
			...
			choice = room:askForChoice(p,"ofguhuo",choice,ToData(user))
			if choice=="question" then
				room:setEmotion(p, "question");
				has:append(p)
			else
		...
		or c:sameNameWith(tc,true) then
			for _,p in sgs.qlist(has)do
				room:loseHp(p,1,true,user,"ofguhuo")
				if p:isAlive() then
					room:acquireSkill(p,"ofhuinu")
				end
			end
		...
		for _,p in sgs.qlist(tps)do
			if has:contains(p) or p:isKongcheng() then continue end
			dc = room:askForExchange(p,"ofguhuo",1,1,false,"ofguhuo0:"..user:objectName())
			if dc then room:giveCard(p,user,dc,"ofguhuo") end
			if user:isDead() then break end
		end
		return nil
	end,
}
...
oftianxiang = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self,event,player,data,room)
		if event==sgs.CardsMoveOneTime then
			...
			and move.from:objectName()==player:objectName() and player:hasSkill(self) then
				local dc = dummyCard()
				for _,id in sgs.qlist(move.card_ids)do
					local c = sgs.Sanguosha:getCard(id)
					if c:hasFlag("visible") and c:hasFlag("oftianxiangBf")
					and room:getCardPlace(id)==move.to_place
					then dc:addSubcard(id) end
				end
				if dc:subcardsLength()<1 then return end
				local tp = room:askForPlayerChosen(player,room:getOtherPlayers(player),self:objectName(),"oftianxiang1",true,true)
				if tp then
					room:setPlayerMark(tp,"&oftianxiang-Clear",1)
					room:giveCard(player,tp,dc,self:objectName())
				end
			end
		elseif event==sgs.BeforeCardsMove then
			local move = data:toMoveOneTime()
			...
			and move.from:objectName()==player:objectName() then
				for _,id in sgs.qlist(move.card_ids)do
					...
					if c:getSuit()==sgs.Card_Heart then
						room:setCardFlag(c,"oftianxiangBf")
					end
				end
			end
		elseif event==sgs.EventPhaseChanging then
			...
			for _,p in sgs.qlist(room:getAllPlayers())do
				if p:getMark("&oftianxiang-Clear")>0 then
					room:loseHp(p,1,true,nil,self:objectName())
				end
			end
		elseif player:canDiscard("he") and player:hasSkill(self)
		and room:askForCard(player,".|heart","oftianxiang0",data,objectName()) then
			local damage = data:toDamage()
			return player:damageRevises(data,-damage.damage)
		end
	end,
...
ofhongyuanbf = sgs.CreateFilterSkill{
	name = "#ofhongyuanbf",
	view_filter = function(self,card)
		return card:getSuit()==2
	end,
	view_as = function(self,card)
		local ex = sgs.Sanguosha:cloneCard(card:objectName(),sgs.Card_Heart,card:getNumber())
    	ex:setSkillName("ofhongyuan")
...
ofhongyuan = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self,event,player,data,room)
		...
		for _,p in sgs.qlist(room:getOtherPlayers(player))do
			if p:isMale() then
				for _,s in sgs.qlist(p:getVisibleSkillList())do
					local str = s:getDescription(p)
					for _,t in ipairs(ofhongyuan_suits)do
...
require("lua.config")
table.insert(config.kingdoms,"west")
...
ofducai = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self,event,player,data,room)
		if event==sgs.EventPhaseChanging then
			...
			if change.from==sgs.Player_NotActive then
				if player:hasSkill(self) then
					room:sendCompulsoryTriggerLog(player,self)
					for _,p in sgs.qlist(room:getOtherPlayers(player))do
						p:addMark(player:objectName().."ofducaiBf-Clear")
						room:setPlayerCardLimitation(p,"use",".",false)
					end
				end
			elseif change.to==sgs.Player_NotActive then
				for _,p in sgs.qlist(room:getOtherPlayers(player))do
					if p:getMark(player:objectName().."ofducaiBf-Clear")<1 then continue end
		...
		elseif event==sgs.EventAcquireSkill then
			if player:hasSkill(self) then
				for _,p in sgs.qlist(room:getOtherPlayers(player))do
					if p:getMark(player:objectName().."ofducaiBf-Clear")>0 then continue end
					p:addMark(player:objectName().."ofducaiBf-Clear")
					room:setPlayerCardLimitation(p,"use",".",false)
				end
			end
		elseif event==sgs.EventLoseSkill then
			if data:toString()==self:objectName() then
				for _,p in sgs.qlist(room:getOtherPlayers(player))do
					if p:getMark(player:objectName().."ofducaiBf-Clear")<1 then continue end
					p:removeMark(player:objectName().."ofducaiBf-Clear")
					room:removePlayerCardLimitation(p,"use",".")
				end
			end
		end
			...
			if use.card:getTypeId()>0 then
				for _,p in sgs.qlist(use.to)do
					...
					if player:getChangeSkillState(self:objectName())==1 then
						...
						if p~=player and p:hasEquip() then
							room:sendCompulsoryTriggerLog(player,self)
							room:setChangeSkillState(player,self:objectName(),1)
							local dc = dummyCard()
							dc:addSubcards(p:getEquips())
							player:obtainCard(dc)
	...
	on_trigger = function(self,event,player,data,room)
		if event==sgs.EventPhaseStart then
			if player:getPhase()==sgs.Player_RoundStart and player:getKingdom()=="west" then
				for _,p in sgs.qlist(room:getAllPlayers())do
					if p:hasLordSkill(self) and p:getLostHp()>0 then
						room:sendCompulsoryTriggerLog(p,self)
						room:recover(p,sgs.RecoverStruct(self:objectName(),p))
						p:drawCards(1,self:objectName())
					end
	...
	on_trigger = function(self,event,player,data,room)
		if event==sgs.EventPhaseStart then
			if player:getPhase()==sgs.Player_Play and player:getKingdom()=="west" then
				for _,p in sgs.qlist(room:getOtherPlayers(player))do
					if p:getMark("ofwanwangUse_lun")<1 and p:hasSkill(self)
					and p:askForSkillInvoke(self,player) then
						p:addMark("ofwanwangUse_lun")
						p:setPhase(sgs.Player_Play)
						room:broadcastProperty(p, "phase")
						room:getThread():trigger(sgs.EventPhaseProceeding,room,p,data)
...
ofnagong = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self,event,player,data,room)
		if event==sgs.EventPhaseStart then
			if player:getPhase()==sgs.Player_Start then
				for _,p in sgs.qlist(room:getOtherPlayers(player))do
					if player:getCardCount()>0 and p:hasSkill(self) then
						local dc = room:askForCard(player,"..","ofnagong0:"..p:objectName(),ToData(p),sgs.Card_MethodNone)
						if dc then
							player:skillInvoked(self,-1,p)
							room:giveCard(player,p,dc,self:objectName())
							player:setTag("ofnagongKingdom",ToData(player:getKingdom()))
							room:changeKingdom(player,"qun")
							player:addMark("ofnagongBf")
						end
					end
				end
			elseif player:getPhase()==sgs.Player_RoundStart and player:getMark("ofnagongBf")>0 then
				player:setMark("ofnagongBf",0)
...
ofdengtian = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self,event,player,data,room)
		if event==sgs.RoundStart then
			...
			room:addPlayerMark(player,"&ofdengtian+"..choice)
		elseif event==sgs.DrawNCards then
			local draw = data:toDraw()
			if draw.reason~="draw_phase" then return end
			draw.num = draw.num+player:getMark("&ofdengtian+1")
			data:setValue(draw)
		elseif event==sgs.ConfirmDamage then
			if player:getMark("ofdengtian3-Clear")>0 then return end
			player:addMark("ofdengtian3-Clear")
...
ofjiaodou = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self,event,player,data,room)
		if event==sgs.TargetSpecified then
			local use = data:toCardUse()
			if use.card:isKindOf("Slash") then
				room:setCardFlag(use.card,"ofjiaodouBf")
			end
		else
			local use = data:toCardUse()
			if use.card:hasFlag("ofjiaodouBf") then
				for _,p in sgs.qlist(use.to)do
					if use.card:hasFlag("DamageDone_"..p:objectName()) then continue end
					if p:isAlive() and p:hasSkill(self) then
						local dc = dummyCard(nil,"_ofjiaodou")
						if p:canSlash(player,dc,false) and p:askForSkillInvoke(self,player) then
		...
		if event==sgs.DrawNCards then
			...
			for _,p in sgs.qlist(room:getAlivePlayers())do
				for _,c in sgs.qlist(p:getCards("ej"))do
					if c:isKindOf("Horse") then n = n+1 end
				end
			end
...
ofzongmaCard = sgs.CreateSkillCard{
	name = "ofzongmaCard",
	will_throw = false,
	filter = function(self,targets,to_select,from)
		return #targets<1 and (to_select:hasEquipArea(2) and to_select:getEquip(2)==nil or to_select:hasEquipArea(3) and to_select:getEquip(3)==nil)
	end,
	on_use = function(self,room,player,targets)
		for _,p in sgs.list(targets)do
			local choices = {}
			if p:hasEquipArea(2) and p:getEquip(2)==nil then table.insert(choices,"EquipArea2") end
			if p:hasEquipArea(3) and p:getEquip(3)==nil then table.insert(choices,"EquipArea3") end
			if #choices<1 then continue end
			local choice = room:askForChoice(player,"ofzongma",table.concat(choices,"+"),ToData(p))
			room:moveCardTo(self,player,sgs.Player_PlaceTable,true)
			choice = choice:endsWith("2") and "_zhizhe_defensivehorse" or "_zhizhe_offensivehorse"
			local info = {choice,self:getSuitString(),self:getNumber(),"ofzongma"}
			room:setTag("ZhizheFilter_"..self:getEffectiveId(),ToData(table.concat(info,"+")))
			info = room:getTag("ofzongmaId"):toIntList()
			info:append(self:getEffectiveId())
			room:setTag("ofzongmaId",ToData(info))
			for _,q in sgs.qlist(room:getAlivePlayers())do
				room:acquireSkill(q,"#zhizhe")
			end
			local wc = sgs.Sanguosha:getWrappedCard(self:getEffectiveId())
			info = sgs.Sanguosha:cloneCard(choice,wc:getSuit(),wc:getNumber())
			info:setSkillName("ofzongma")
			wc:takeOver(info)
			room:broadcastUpdateCard(room:getPlayers(),wc:getId(),wc)
			room:moveCardTo(wc,p,sgs.Player_PlaceEquip,true)
		end
	end,
}
...
ofzongma = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self,event,player,data,room)
		if event==sgs.EventPhaseStart then
			if player:getPhase()==sgs.Player_Play then
				if player:getCardCount()>0 and player:hasSkill(self) then
					room:askForUseCard(player,"@@ofzongma","ofzongma0:")
				end
			end
		elseif event==sgs.Predamage then
			for _,c in sgs.qlist(player:getEquips())do
				if c:objectName()=="_zhizhe_offensivehorse" and c:getSkillName()==self:objectName() then
					player:damageRevises(data,1)
					break
				end
			end
		elseif event==sgs.DamageForseen then
			for _,c in sgs.qlist(player:getEquips())do
				if c:objectName()=="_zhizhe_defensivehorse" and c:getSkillName()==self:objectName() then
					player:damageRevises(data,1)
					break
				end
			end
		...
		elseif event==sgs.CardsMoveOneTime then
			...
			and move.to_place~=sgs.Player_PlaceEquip and move.from:objectName()==player:objectName() then
				local ids = room:getTag("ofzongmaId"):toIntList()
				for i,id in sgs.qlist(move.card_ids)do
					if ids:contains(id) and move.from_places:at(i)==sgs.Player_PlaceEquip then
						room:removeTag("ZhizheFilter_"..id)
						ids:removeOne(id)
						room:setTag("ofzongmaId",ToData(ids))
					end
				end
			end
		end
...
ofxi_makang:addSkill("mashu")

sgs.LoadTranslationTable {
	["shendiaochan"] = "神貂蝉",
    ["#shendiaochan"] = "欲界非天",
    --["illustrator:shendiaochan"] = "鬼画府",
	["information:shendiaochan"] = "ᅟᅠ<i>“下一个……就选你吧~”</i>",
	["meihun"] = "魅魂",
	[":meihun"] = "结束阶段或当你成为【杀】的目标后，你可以令一名男性角色交给你一张你声明的花色的牌，若其没有，则观看其手牌并弃置其中一张。",
	["huoxin"] = "惑心",
	[":huoxin"] = "出牌阶段限一次，你可以展示两张花色相同的手牌并交给两名其他角色各一张，然后这两名角色进行拼点，没赢的角色获得1枚“魅惑”标记；“魅惑”标记数大于等于2的角色回合开始时，其移去所有标记，然后此回合改为由你操控。",
	["meihun0"] = "你可以发动“魅魂”选择一名男性角色",
	["meihun1"] = "魅魂：请交给%src张%arg牌，否则其观看你手牌并弃置一张",
	["meihuo"] = "魅惑",

	["ofxi_kaisa"] = "凯撒",
	["#ofxi_kaisa"] = "无冕之皇",
    ["illustrator:ofxi_kaisa"] = "绘绘子酱",
	["ofducai"] = "独裁",
	[":ofducai"] = "持恒技，你的回合内，你使用牌无距离与次数限制，其他角色不能使用牌且所有技能失效。",
	["ofzhitong"] = "治统",
	[":ofzhitong"] = "转换技，当你使用牌时，若目标为①自己，摸两张牌并回复1点体力②其他角色，你获得其装备区所有牌并对其造成1点伤害。",
	[":ofzhitong1"] = "转换技，当你使用牌时，若目标为①自己，摸两张牌并回复1点体力<font color=\"#01A5AF\"><s>②其他角色，你获得其装备区所有牌并对其造成1点伤害</s></font>。",
	[":ofzhitong2"] = "转换技，当你使用牌时，若目标为<font color=\"#01A5AF\"><s>①自己，摸两张牌并回复1点体力</s></font>②其他角色，你获得其装备区所有牌并对其造成1点伤害。",
	["ofjiquan"] = "集权",
	[":ofjiquan"] = "主公技，锁定技，西势力角色回合开始时，你回复1点体力并摸一张牌。",

	["ofxi_aer"] = "阿尔达希尔一世",
	["&ofxi_aer"] = "阿尔",
	["#ofxi_aer"] = "萨珊的创始人",
    ["illustrator:ofxi_aer"] = "绘绘子酱",
	["ofwanwang"] = "万王",
	[":ofwanwang"] = "每轮限一次，其他西势力的角色出牌阶段开始时，你可以代替其执行本阶段。",
	["ofsashan"] = "萨珊",
	[":ofsashan"] = "锁定技，游戏开始时，你将全场势力改为西。",
	["ofnagong"] = "纳贡",
	[":ofnagong"] = "其他角色的准备阶段，其可以交给你一张牌，然后直到其下回合开始前，其势力更改为群。",
	["ofnagong0"] = "纳贡：你可以交给%src一张牌将势力改为群",

	["ofxi_make"] = "马克里努斯",
	["&ofxi_make"] = "马克",
	["#ofxi_make"] = "传奇的角斗士",
    ["illustrator:ofxi_make"] = "绘绘子酱",
	["ofdengtian"] = "登天",
	[":ofdengtian"] = "锁定技，每轮开始时，你选择以下一项的数值+1：1.摸牌阶段摸牌数；2.手牌上限；3.每回合首次造成的伤害。",
	["ofmingshu"] = "命数",
	[":ofmingshu"] = "锁定技，当“登天”的某一项数值达到3及以上时，你失去所有体力。",
	["ofjiaodou"] = "角斗",
	[":ofjiaodou"] = "当其他角色对你使用的【杀】结算后，若未对你造成伤害，你可以视为对其使用一张无距离限制的【杀】。",
	["ofdengtian:1"] = "摸牌阶段摸牌数+1",
	["ofdengtian:2"] = "手牌上限+1",
	["ofdengtian:3"] = "每回合首次造成的伤害+1",

	["ofxi_makang"] = "马抗",
	["#ofxi_makang"] = "马米科尼扬",
    ["illustrator:ofxi_makang"] = "绘绘子酱",
	["ofxiru"] = "西入",
	[":ofxiru"] = "锁定技，摸牌阶段，你的摸牌数+X（X为场上坐骑数的一半，向上取整）。",
	["ofzongma"] = "纵马",
	[":ofzongma"] = "出牌阶段开始时，你可以将一张牌当做防御/进攻坐骑置入一名角色装备区，受到/造成的伤害+1，且受到/造成伤害后将此牌置入弃牌堆。",
	["ofzongma0"] = "你可以发动“纵马”选择牌和置入目标",

	["ofjie_yuji"] = "界于吉[线下]",
	["#ofjie_yuji"] = "江东的福神",
    ["illustrator:ofjie_yuji"] = "月月岛",
	["ofguhuo"] = "蛊惑",
	[":ofguhuo"] = "每回合每种类别限一次，你可以扣置一张手牌当做任意基本牌或普通锦囊牌使用，手牌数大于你的角色依次选择是否质疑，然后翻开此牌：若为真，质疑角色失去1点体力并获得“恚怒”；若为假，不质疑角色依次交给你一张手牌。",
	["ofhuinu"] = "恚怒",
	[":ofhuinu"] = "锁定技，你必须质疑“蛊惑”。",
	["ofguhuo0"] = "蛊惑：请将一张手牌交给%src",

	["ofjie_xiaoqiao"] = "界小乔[线下]",
	["#ofjie_xiaoqiao"] = "秋水并蒂",
    ["illustrator:ofjie_xiaoqiao"] = "月月岛",
	["oftianxiang"] = "天香",
	[":oftianxiang"] = "当你受到伤害时，你可以弃置一张♥牌防止之；当你正面失去♥牌时，你可以将之交给一名其他角色，其于此回合结束时失去1点体力（不叠加）。",
	["ofhongyuan"] = "红颜",
	[":ofhongyuan"] = "锁定技，你的♠牌视为♥牌；游戏开始时，你选择获得其他男性角色的一个含颜色或花色的技能。",
	["oftianxiang0"] = "你可以发动“天香”弃置♠牌防止此伤害",
