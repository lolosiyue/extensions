...
kesxjimeng = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if (event == sgs.EventPhaseStart) and (player:getCardCount() > 0) and (player:getPhase() == sgs.Player_Start) then
...
kesxquedi = sgs.CreateOneCardViewAsSkill {
	...
	enabled_at_response = function(self, player, pattern)
		return pattern:contains("duel") and sgs.Sanguosha:getCurrentCardUseReason() ~= sgs.CardUseStruct_CARD_USE_REASON_RESPONSE
	end,
}
...
kesxqiaoying = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.DamageInflicted then
			...
			if room:getCurrent():hasSkill("kesxqiaoying") and (damage.to:getHandcardNum() > damage.to:getMark("&kesxqiaoying-Clear")) then
...
kesxyibing = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.Dying then
			...
			if source ~= player and source:getCardCount() > 0 and player:askForSkillInvoke(self, source) then
				room:broadcastSkillInvoke(self:objectName())
...
kesxyouqi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		if (event == sgs.EventPhaseStart) and player:hasLordSkill(self:objectName()) and (player:getPhase() == sgs.Player_Start) then
	...
	on_trigger = function(self, event, player, data)
		...
		if (event == sgs.CardAsked) and (player:hasFlag("kesxhuijiAsked")) then
			...
			for i, fri in sgs.qlist(lieges) do
				if i == 0 then
					if not player:askForSkillInvoke(self, data, false) then
						break
					end
				end
...
kesxxiongxiaVS = sgs.CreateViewAsSkill {
	...
	view_as = function(self, cards)
		if #cards > 1 then
			...
			duel:setSkillName("kesxxiongxia")
			duel:deleteLater()
			if sgs.Self:isLocked(duel) then
				return
			end
	...
	enabled_at_play = function(self, player)
		return player:getMark("&bankesxxiongxia-Clear") < 1 and player:getCardCount() > 1
	end,
}
...
sxjujian = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardFinished then
			local use = data:toCardUse()
			if use.card:hasFlag("sxjujianbf") and player:getMark("sxjujian-Clear") < 1 and player:hasTurn() and room:getCardPlace(use.card:getEffectiveId()) == sgs.Player_DiscardPile then
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardOffset then
			...
			if effect.card:getTypeId() > 0 and effect.card:getEffectiveId() > -1 and room:getCardOwner(effect.card:getEffectiveId()) == nil then
				...
				if use.from ~= player and use.from:isAlive() and use.from:getMark("sxfunan-Clear") < 1 and player:hasTurn() and use.from:hasSkill(self) and use.from:askForSkillInvoke(self, data) then
...
sxjuyi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.DamageCaused then
			...
			if player ~= damage.to and damage.to:hasLordSkill(self) and player:getMark(damage.to:objectName() .. "sxjuyi-Clear") == 1 and player:askForSkillInvoke(self, damage.to) then
...
sxjujing = sgs.CreateTriggerSkill {
	...
	events = { sgs.Damaged },
	can_trigger = function(self, target)
		return target and target:isAlive() and target:hasLordSkill(self:objectName())
	end,
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		if event == sgs.Damaged then
			...
			if
				player ~= damage.from
				and damage.from
				and damage.from:getKingdom() == "qum"
				and room:askForDiscard(player, self:objectName(), 2, 2, true, true, "sxjujing0:", ".", self:objectName())
			then
				player:peiyin(self)
...
sxshuchen = sgs.CreateViewAsSkill {
	...
	enabled_at_response = function(self, player, pattern)
		return string.find(pattern, "peach") and not player:hasFlag("CurrentPlayer") and player:getHandcardNum() > player:getMaxCards()
	end,
	enabled_at_play = function(self, player)
		return not player:hasFlag("CurrentPlayer") and player:getHandcardNum() > player:getMaxCards() and dummyCard("peach"):isAvailable(player)
	end,
}
...
sxchengji = sgs.CreateViewAsSkill {
	...
	enabled_at_response = function(self, player, pattern)
		return string.find(pattern, "slash") and player:getCardCount() > 1
	end,
	enabled_at_play = function(self, player)
		return player:getCardCount() > 1 and dummyCard():isAvailable(player)
	end,
}
...
sxzhengnan = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.Death then
			local death = data:toDeath()
			if death.damage and death.damage.card and death.damage.from == player and table.contains(death.damage.card:getSkillNames(), self:objectName()) then
				player:drawCards(2, self:objectName())
			end
		end
	end,
...
sxzhanjue = sgs.CreateViewAsSkill {
	...
	enabled_at_play = function(self, player)
		return player:usedTimes("#sxzhanjueCard") < 1 and player:getHandcardNum() > 0
	end,
}
	...
	view_as = function(self, cards)
		if #cards > 1 then
			...
			for _, c in sgs.list(cards) do
				if sgs.Self:getPileName(c:getEffectiveId()) == "#sxhuituo" then
					x = x + 1
				else
					n = n + 1
				end
...
sxmingjian = sgs.CreateViewAsSkill {
	...
	enabled_at_play = function(self, player)
		return player:usedTimes("#sxmingjianCard") < 1 and player:getCardCount() > 0
	end,
}
...
sxchoulve = sgs.CreateViewAsSkill {
	...
	enabled_at_play = function(self, player)
		return player:usedTimes("#sxchoulveCard") < 1 and player:getHandcardNum() > 0
	end,
}
...
sxfenxunvs = sgs.CreateViewAsSkill {
	...
	enabled_at_play = function(self, player)
		return player:usedTimes("#sxfenxunCard") < 1 and player:getCardCount() > 0
	end,
}
...
sxchuyi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		local damage = data:toDamage()
		for _, p in sgs.list(room:getOtherPlayers(player)) do
			if p:inMyAttackRange(damage.to) and p:getMark("sxchuyi_lun") < 1 and p:hasSkill(self) and p:askForSkillInvoke(self, damage.to) then
...
sxmieji = sgs.CreateViewAsSkill {
	...
	enabled_at_play = function(self, player)
		return player:usedTimes("#sxmiejiCard") < 1 and player:getHandcardNum() > 0
	end,
}
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.Damaged then
			...
			if damage.card and (damage.card:isKindOf("Slash") or damage.card:isKindOf("Duel")) then
				for _, p in sgs.list(room:getAllPlayers()) do
					if p:hasSkill(self) and (p:canDiscard(player, "he") or p:canDiscard(damage.from, "he")) and p:askForSkillInvoke(self, data) then
						p:peiyin("moucheng")
...
sxzhongyan = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.Death then
			local death = data:toDeath()
			if death.who:getKingdom() == "qun" and player:hasLordSkill(self) and player:isWounded() and player:askForSkillInvoke(self) then
				room:recover(player, sgs.RecoverStruct(self:objectName(), player))
			end
		end
	end,
...
sxjiaochong = sgs.CreateTriggerSkill {
	...
	events = { sgs.EventPhaseStart },
	can_trigger = function(self, target)
		return target and target:isAlive() and target:isMale() and target:getPhase() == sgs.Player_Finish
	end,
	on_trigger = function(self, event, player, data, room)
		for _, p in sgs.list(room:getAllPlayers()) do
			if p:hasSkill(self) then
...
sxwanlan = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.Dying then
			...
			if dying.who ~= player and player:getMark("@sxwanlan") > 0 and player:getHandcardNum() > 0 and player:askForSkillInvoke(self, dying.who) then
		...
		if event == sgs.CardsMoveOneTime then
			...
			then
				...
				if tp then
					...
					if id > -1 then
						room:throwCard(id, self:objectName(), tp, player)
					end
				end
			end
		end
...
sxaocai = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseChanging then
			...
			if change.to == sgs.Player_NotActive then
				for _, p in sgs.list(room:getAllPlayers()) do
					if p:isKongcheng() and p:hasSkill(self:objectName()) and p:askForSkillInvoke(self) then
						p:peiyin("aocai")
...
sxbingzheng = sgs.CreateTriggerSkill {
	...
	events = { sgs.EventPhaseStart },
	can_trigger = function(self, target)
		return target and target:isAlive() and target:hasSkill(self) and target:getPhase() == sgs.Player_Finish
	end,
	on_trigger = function(self, event, player, data, room)
		local tps = sgs.SPlayerList()
		for _, p in sgs.list(room:getAlivePlayers()) do
...
sxqingcheng = sgs.CreateViewAsSkill {
	...
	enabled_at_play = function(self, player)
		return player:getCardCount() > 1 and player:usedTimes("#sxqingchengCard") < 1
	end,
}
				...
				if p:hasSkill(self) and p:getCardCount() > 0 then
					...
					if dc then
						...
						for _, h in sgs.list(player:getHandcards()) do
							if h:isKindOf("Jink") and player:canDiscard(player, h:getId()) then
								dc:addSubcard(h)
							end
						end
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.DamageInflicted then
			...
			if damage.nature ~= sgs.DamageStruct_Normal then
				for _, p in sgs.qlist(room:getAllPlayers()) do
					if p:hasSkill(self) and p:getMark("sxchenghaoUse-Clear") < 1 and p:hasTurn() and p:askForSkillInvoke(self, data) then
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseChanging then
			...
			if
				...
				and room:askForDiscard(player, self:objectName(), 2, 2, true, false, "sxshejian0:" .. use.from:objectName(), ".", self:objectName())
			then
...
sxkengqiang = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.ConfirmDamage then
			...
			local use = data:toCardUse()
			if use.card:isDamageCard() and player:getMark("sxkengqiangUse-Clear") < 1 and player:hasTurn() and player:hasSkill(self) and player:askForSkillInvoke(self, data) then
...
sxshangjue = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.Dying then
			local dying = data:toDying()
			if dying.who == player and player:getMark("@sxshangjue") > 0 and player:askForSkillInvoke(self, data) then
...
sxshibei = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.Damaged then
			if player:getMark("@sxshibei") > 0 and player:askForSkillInvoke(self, data) then
		...
		if event == sgs.CardUsed then
			...
			if use.card:getTypeId() > 0 then
				...
				if player:hasSkill(self) then
					...
					if #suits == player:getMark("sxtongliNum-PlayClear") and player:askForSkillInvoke(self:objectName(), data) then
...
sxshezang = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.Dying then
			local dying = data:toDying()
			if dying.who == player and player:getMark("@sxshezang") > 0 and player:askForSkillInvoke(self, data) then
...
sxpingjianvs = sgs.CreateViewAsSkill {
		...
		if #selected < 1 then
			...
			if pattern ~= "" then
				for _, cn in sgs.list(pattern:split("+")) do
					if to_select:objectName():contains(cn) then
	...
	enabled_at_response = function(self, player, pattern)
		if string.sub(pattern, 1, 1) == "." or string.sub(pattern, 1, 1) == "@" or player:isKongcheng() or sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_RESPONSE then
			return
		end
...
sxfushivs = sgs.CreateViewAsSkill{
	name = "sxfushi",
	n = 998,
	response_or_use = true,
	view_filter = function(self,selected,to_select)
...
sxfushi = sgs.CreateTriggerSkill{
	name = "sxfushi",
	...
	on_trigger = function(self,event,player,data,room)
		if event==sgs.CardFinished then
			...
			if use.card:hasFlag("sxfushiBf") then
				for _,p in sgs.qlist(room:getAllPlayers())do
					if use.card:hasFlag("sxfushiBf"..p:objectName()) and p:hasSkill(self) then
...
sxyuqi = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self,event,player,data,room)
		if event==sgs.Damaged then
			for _,p in sgs.qlist(room:getAllPlayers())do
				if p:distanceTo(player)<=p:getHp() and p:hasSkill(self)
...
sxshanshen = sgs.CreateTriggerSkill{
	name = "sxshanshen",
	events = {sgs.Death},
	on_trigger = function(self,event,player,data,room)
		if event==sgs.Death then
			local death = data:toDeath()
			if death.damage and death.damage.from==player then return end
			if player:isWounded() and player:askForSkillInvoke(self) then
...
sxkoujingvs = sgs.CreateViewAsSkill{
	name = "sxkoujing",
	n = 998,
	response_or_use = true,
	view_filter = function(self,selected,to_select)
...
sxkoujing = sgs.CreateTriggerSkill{
	name = "sxkoujing",
	...
	on_trigger = function(self,event,player,data,room)
		if event==sgs.Damaged then
			local damage = data:toDamage()
			if damage.card and table.contains(damage.card:getSkillNames(),self:objectName()) and player~=damage.from then
				if player:askForSkillInvoke(self,ToData("0:"..damage.from:objectName()),false) then
					room:swapCards(player,damage.from,"h",self:objectName())
				end
			end
		else
			local use = data:toCardUse()
...
sxzhijie = sgs.CreateTriggerSkill{
	name = "sxzhijie",
	events = {sgs.EventPhaseStart,sgs.CardUsed},
	can_trigger = function(self,target)
		return target and target:isAlive()
	...
	on_trigger = function(self,event,player,data,room)
		if event==sgs.EventPhaseStart then
			for _,p in sgs.qlist(room:getAllPlayers())do
				if p:getMark("sxzhijieUse_lun")<1 and p:hasSkill(self) and player:getHandcardNum()>0
				and p:askForSkillInvoke(self:objectName(),player) then
					p:peiyin("zhijie")
					p:addMark("sxzhijieUse_lun")
					...
					room:setPlayerMark(player,"&sxzhijie+"..id.."+#"..p:objectName().."-PlayClear",1)
					player:addMark("sxzhijieUse-PlayClear")
				end
			end
		else
			local use = data:toCardUse()
			if use.card:getTypeId()>0 and player:getMark("sxzhijieUse-PlayClear")>0
			and player:getMark("sxzhijieNum-PlayClear")<3 then
...
sxshushen = sgs.CreateTriggerSkill{
	name = "sxshushen",
	events = {sgs.EventPhaseChanging,sgs.CardsMoveOneTime},
	can_trigger = function(self,target)
		return target~=nil
	...
	on_trigger = function(self,event,player,data,room)
		if event==sgs.EventPhaseChanging then
			...
			if (change.to == sgs.Player_NotActive) then
				for _,p in sgs.qlist(room:getAllPlayers())do
					if p:getMark("sxshushenNum-Clear")>2 and p:hasSkill(self:objectName()) then
						local tps = sgs.SPlayerList()
						for _,q in sgs.qlist(room:getOtherPlayers(p))do
							if q:isWounded() then tps:append(q) end
						end
						local tp = room:askForPlayerChosen(p,tps,self:objectName(),"sxshushen0",true,true)
						if tp then
							p:peiyin("shushen")
							room:recover(tp,sgs.RecoverStruct(self:objectName(),p))
						end
					end
				end
			end
			...
			and move.to:objectName()==player:objectName() and player:hasTurn() then
...
sxcuorui = sgs.CreateTriggerSkill{
	name = "sxcuorui",
	events = {sgs.Death,sgs.GameStart},
	on_trigger = function(self,event,player,data,room)
		if event==sgs.Death then
			local death = data:toDeath()
			if death.damage and death.damage.from==player then else return end
		end
		local x = math.min(7,room:getPlayers():length())
		if player:getHandcardNum()<x and player:askForSkillInvoke(self) then
...
sxanda = sgs.CreateTriggerSkill{
	name = "sxanda",
	events = {sgs.Dying},
	on_trigger = function(self,event,player,data,room)
		if event==sgs.Dying then
			local dying = data:toDying()
			if player:getMark("sxandaUse_lun")<1 and dying.damage and dying.damage.from
			and player:askForSkillInvoke(self,dying.who) then
				player:addMark("sxandaUse_lun")
				player:peiyin("anda")
				if dying.damage.from:getCardCount()>0 then
					local dc = room:askForCard(dying.damage.from,"..","sxanda0:"..dying.who:objectName(),data,sgs.Card_MethodNone)
					if dc then
						room:giveCard(dying.damage.from,dying.who,dc,self:objectName())
						return
					end
				end
...
sxgaobanvs = sgs.CreateViewAsSkill{
	name = "sxgaoban",
	n = 1,
	expand_pile = "#sxgaoban",
	response_pattern = "@@sxgaoban",
	view_filter = function(self,selected,to_select)
		return sgs.Self:getPileName(to_select:getId())=="#sxgaoban"
	end,
	view_as = function(self,cards)
		return #cards>0 and cards[1]
	end,
}
sxgaoban = sgs.CreateTriggerSkill{
	name = "sxgaoban",
	...
	on_trigger = function(self,event,player,data,room)
		if event==sgs.EventPhaseChanging then
			local change = data:toPhaseChange()
			if (change.to == sgs.Player_NotActive) then
				local ap = room:getTag("sxgaobanDone"):toString()
				for _,p in sgs.qlist(room:getOtherPlayers(player))do
					if ap:contains("+") then break end
					if p:hasSkill(self:objectName()) then
						for _,q in sgs.qlist(room:getAlivePlayers())do
							if q:objectName()==ap then
								...
								room:sendCompulsoryTriggerLog(p,self:objectName())
								for _,id in sgs.qlist(room:getDiscardPile())do
									if p:getMark(id.."sxshushenId-Clear")>0 then
										local c = sgs.Sanguosha:getCard(id)
								...
								if ids:length()>0 then
									...
									if room:askForUseCard(q,"@@sxgaoban","sxgaoban0") then continue end
								end
								room:setPlayerMark(p,"&sxanran+#num",0)
							end
						end
					end
				end
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.Damage then
			...
			if
				...
				and player:askForSkillInvoke(self, damage.to)
			then
				local id = room:askForCardChosen(player, damage.to, "he", self:objectName())
				if id >= 0 then
...
zcbizunVS = sgs.CreateViewAsSkill {
	...
	enabled_at_response = function(self, player, pattern)
		return player:getMark("zcbizunBan-Clear") < 1 and player:hasTurn() and (string.find(pattern, "slash") or string.find(pattern, "jink"))
	end,
	enabled_at_play = function(self, player)
		return player:getMark("zcbizunBan-Clear") < 1 and player:hasTurn() and dummyCard():isAvailable(player)
	end,
}
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseStart then
			...
			elseif player:getPhase() == sgs.Player_Discard then
				for _, p in sgs.qlist(room:getOtherPlayers(player)) do
					if player:getMark("&zcdechong+#" .. p:objectName() .. "-Clear") > 0 and player:getHandcardNum() >= player:getHp() and p:askForSkillInvoke(self, player, false) then
		...
		if event == sgs.EventPhaseStart then
			if player:getPhase() == sgs.Player_Finish then
				...
				if tp then
					...
					if tp:isAlive() and tp:canSlash(player, dc, false) and tp:askForSkillInvoke(self, player, false) then
				...
				for i, p in sgs.list(tps) do
					...
					if p:hasCard(c) then
						if c:isAvailable(p) and room:askForUseCard(p, c:toString(), "zclijun1:" .. c:objectName()) then
							...
							room:throwCard(c, self:objectName(), p)
						end
					end
				end
			end
...
zczhuying = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.DamageInflicted then
			...
			if damage.nature == sgs.DamageStruct_Normal then
				for i, p in sgs.qlist(room:getAllPlayers()) do
					if not damage.to:isChained() and p:hasSkill(self) and p:askForSkillInvoke(self, damage.to) then
