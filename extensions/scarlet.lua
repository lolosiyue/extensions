...
s4_ganglu_buff = sgs.CreateTargetModSkill{
	...
	distance_limit_func = function(self, from, card)
		if from:hasSkill("s4_ganglu") and card and card:getSkillName() == "s4_ganglu" then
			return 1000
		else
			return 0
		end
...
s4_longxin = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardResponded then
	        	...
	        	if resp.m_card:getSkillName() == "s4_longxin" and resp.m_who and (not resp.m_who:isKongcheng()) then
	            ...
	            local use = data:toCardUse()
	            if use.card:getSkillName() == "s4_longxin" then
	                for _, p in sgs.qlist(use.to) do
...
s4_jianghulimit = sgs.CreateProhibitSkill{
    ...
    is_prohibited = function(self, from, to, card)
        if card:getSkillName() == "s4_jianghu" and not to:hasFlag("s4_jianghu") then
            return true
        end
        return false
    end,
...
s4_wuhu_wusheng = sgs.CreateTriggerSkill{
    ...
    on_trigger = function(self, event, player, data)
        ...
        local general = player:getTag("s4_baijiang_general"):toString()
        if card:getSkillName() == "s4_wuhu_wusheng" and player:getMark("&s4_wuhu_wusheng-Clear") == 0 and ((gerenal and gerenal == "s4_wuhu_guanyu") or can_invoke) then
...
s4_wuhu_longdan = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
        ...
        if event == sgs.CardUsed or event == sgs.CardResponded then
            ...
            local use = data:toCardUse()
            if use.card:getSkillName() == "s4_wuhu_longdan" then
                local log = sgs.LogMessage()
...
s4_lieren_buff = sgs.CreateTargetModSkill{
    ...
    pattern = "Slash",
    distance_limit_func = function(self, from, card, to)
        if from:hasSkill("s4_lieren") and card and card:getSkillName() == "s4_lieren" then return 1000 end
        return 0
    end,
}
...
s4_lieren = sgs.CreateTriggerSkill{
    ...
    on_trigger = function(self, event, player, data)
        ...
        if event == sgs.CardUsed then
            local use = data:toCardUse()
            if use.card and use.card:getSkillName() == "s4_lieren" then
        ...
        elseif event == sgs.CardOffset then
            local effect = data:toCardEffect()
            if effect.card and effect.card:isKindOf("Slash") and effect.card:getSkillName() == "s4_lieren" then
...
s4_wusheng = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
        local use = data:toCardUse()
        if event == sgs.TargetSpecified then
            if use.card and use.card:getSkillName() == "s4_wusheng" and player:hasSkill(self:objectName()) then
        ...
        elseif event == sgs.CardUsed then
			local use = data:toCardUse()
			if use.card and use.card:isKindOf("Jink") and use.whocard and use.whocard:getSkillName() == "s4_wusheng" and use.whocard:hasFlag(self:objectName()) then
...
s4_longdan_buff = sgs.CreateTargetModSkill{
	...
	pattern = "Slash" ,
    distance_limit_func = function(self, from, card, to)
        if from:hasSkill("s4_longdan") and card and card:getSkillName() == "s4_longdan" then return 1000 end
        return 0
    end,
}
...
s4_s_yexi = sgs.CreateTriggerSkill{
    ...
    on_trigger = function(self, event, player, data)
        ...
        if use.card and use.card:isKindOf("Slash") and use.card:isBlack() and use.card:hasFlag("DamageDone") then
            for _, p in sgs.qlist(use.to) do
                if player:canDiscard(p, "h") and room:askForSkillInvoke(player, self:objectName(), data) then
                    room:broadcastSkillInvoke(self:objectName())
    ...
    on_trigger = function(self, event, player, data)
        ...
        if event == sgs.CardUsed then
            ...
            if use.card and use.card:isKindOf("Slash") and use.from:hasSkill(self:objectName()) and player:getPhase() == sgs.Player_Play then
                if room:askForCard(player, string.format("%s|.|.|hand", use.card:getSuitString()), "@s4_s_jianta:"..use.card:getSuitString(), data, sgs.Card_MethodDiscard, player, false, self:objectName(), false, nil) then
                    room:broadcastSkillInvoke(self:objectName())
...
s4_s_zhongzhuang = sgs.CreateTriggerSkill{
    ...
    on_trigger = function(self, event, player, data)
                ...
                if reason ~= sgs.CardMoveReason_S_REASON_CHANGE_EQUIP then
                    for i,id in sgs.list(move.card_ids)do
                        ...
                        and sgs.Sanguosha:getCard(id):isKindOf("Weapon") or sgs.Sanguosha:getCard(id):isKindOf("Armor") then
                            ...
                            player:setTag("s4_s_zhongzhuang", sgs.QVariant(id))
                        end
                    end
                end
            end
        elseif event == sgs.CardsMoveOneTime then
            ...
            local reason = move.reason.m_reason
            if reason ~= sgs.CardMoveReason_S_REASON_CHANGE_EQUIP then
                local id = player:getTag("s4_s_zhongzhuang"):toInt()
                if id and id ~= 0 then
                    ...
                    player:removeTag("s4_s_zhongzhuang")
                end
            end
        end
    end,
...
s4_s_weigong = sgs.CreateTriggerSkill{
    ...
    on_trigger = function(self, event, player, data)
        if event == sgs.CardEffected then
            ...
            if card and card:isKindOf("Slash") then
                local target = data:toCardEffect().to
                if target and player:hasSkill("s4_s_weigong") then
                ...
                if player:hasSkill("s4_s_weigong") then
                    ...
                    if target and slash and slash:isKindOf("Slash") then
                        ...
                        if target:getMark("s4_s_weigongDuel-Clear") > 0 then
                            room:removePlayerCardLimitation(target, "use", "Slash|.|1~"..target:getMark("s4_s_weigongDuel-Clear"))
                        end
                        room:setPlayerCardLimitation(target, "use", "Slash|.|1~"..slash:getNumber(), false)
            ...
            if card and card:hasFlag("s4_s_weigong") then
                ...
                for _, p in sgs.qlist(room:getAlivePlayers()) do
                    if p:getMark("s4_s_weigongDuel-Clear") > 0 then
                        room:removePlayerCardLimitation(p, "use", "Slash|.|1~"..p:getMark("s4_s_weigongDuel-Clear"))
...
s4_s_zhuizhan = sgs.CreateTriggerSkill{
    ...
    on_trigger = function(self, event, player, data)
        ...
        if damage.to and damage.to:isAlive() and damage.card and (damage.card:isKindOf("Slash") or damage.card:isKindOf("Duel") or damage.card:isKindOf("FireAttack")) then
            for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
                if p:inMyAttackRange(damage.to) then
        ...
        if damage.from and damage.from:isAlive() then
            if room:askForSkillInvoke(player, self:objectName(), data) then
                ...
                if damage.from:hasJudgeArea() and judge:isGood() then
                    local c = sgs.Sanguosha:getWrappedCard(judge.card:getId())
...
s4_s_tuji = sgs.CreateTriggerSkill{
    ...
    can_trigger = function(self, target)
        return target and target:getPhase() == sgs.Player_Start and target:getMark(self:objectName()) == 0
    end,
}
...
s4_s_anqi_buff = sgs.CreateTargetModSkill{
    ...
    pattern = "Slash",
    distance_limit_func = function(self, from, card)
        if card and card:getSkillName() == "s4_s_anqi" then
            return 1000
        end
        return 0
    end,
