...
fengwangCard = sgs.CreateSkillCard {
	...
	target_fixed = false,
	filter = function(self, targets, to_select, player)
		return #targets < 1 and to_select:objectName() ~= player:objectName() and to_select:hasEquip() and
...
newfengwangCard = sgs.CreateSkillCard {
	...
	target_fixed = false,
	filter = function(self, targets, to_select, player)
		return #targets < 1 and to_select:objectName() ~= player:objectName() and
...
wangzheCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		local rangefix = 0
		if player:getWeapon() and player:getWeapon():getId() == self:getSubcards():first() then
			local card = player:getWeapon():getRealCard():toWeapon()
			rangefix = card:getRange() - player:getAttackRange(false)
		end
		local slash = sgs.Sanguosha:cloneCard("Slash", sgs.Card_NoSuit, 0)
		slash:setSkillName(self:objectName())
		return player:canSlash(to_select, slash, true, rangefix, Table2Playerlist(targets)) and
		#targets < (1 + sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_ExtraTarget, player, slash))
	end,
	on_use = function(self, room, source, targets)
...
LuamuguanCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		return #targets == 0 and to_select:objectName() ~= player:objectName() and to_select:getMark("muguan") == 0
	end,
	on_effect = function(self, effect)
...
haniel = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if event == sgs.EventAcquireSkill and player:getMark(self:objectName()) > 0 then
			room:addPlayerMark(player, "Qingcheng" .. data:toSkillChange().skillName)
		end
...
guanchaCard = sgs.CreateSkillCard {
	...
	will_throw = false,
	filter = function(self, targets, to_select, player)
		return #targets < 1 and self:getSubcards():length() == (math.floor((player:getHandcardNum() + 1) / 2)) and
		to_select:objectName() ~= player:objectName()
	end,
	feasible = function(self, targets, player)
		return #targets == 1 and self:getSubcards():length() == math.floor((player:getHandcardNum() + 1) / 2)
	end,
	on_use = function(self, room, source, targets)
...
moxing = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventAcquireSkill then
			local str = data:toSkillChange().skillName
...
moxing = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if (event == sgs.EventPhaseEnd and player:getPhase() == sgs.Player_Finish and player:hasSkill(self:objectName())) or (event == sgs.EventLoseSkill and data:toSkillChange().skillName == self:objectName()) then
			for _, target in sgs.qlist(room:getAlivePlayers()) do
...
geassCard = sgs.CreateSkillCard {
	...
	target_fixed = false,
	filter = function(self, targets, to_select, player)
		return #targets < 1 and to_select:getSeat() ~= player:getSeat() and not to_select:isKongcheng() and
		to_select:hasEquip()
	end,
	feasible = function(self, targets, player)
		return #targets == 1
...
jiewqCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source)
		...
		room:loseHp(source, choice, true, source, self:objectName())
...
qsasCard = sgs.CreateSkillCard {
	...
	target_fixed = false,
	filter = function(self, targets, to_select, player)
		return #targets < 1 and to_select:getSeat() ~= player:getSeat() and to_select:getMark(self:objectName()) == 0
	end,
	feasible = function(self, targets, player)
		return #targets == 1
	end,
...
nidaorenCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		room:loseHp(source, num, true, source, self:objectName())
...
feils2Card = sgs.CreateSkillCard {
	...
	target_fixed = false,
	filter = function(self, targets, to_select, player)
		return #targets < 1 and player:canSlash(to_select, false)
	end,
	feasible = function(self, targets, player)
		return #targets == 1
	end,
...
kuixinCard = sgs.CreateSkillCard {
	...
	target_fixed = false,
	filter = function(self, targets, to_select, player)
		return #targets < 1 and to_select:getSeat() ~= player:getSeat() and not to_select:isKongcheng()
	end,
	feasible = function(self, targets, player)
		return true
	end,
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart then
			...
			elseif player:getPhase() == sgs.Player_Finish and yin > 0 then
				room:sendCompulsoryTriggerLog(player, self:objectName())
				room:loseHp(player, yin, true, player, self:objectName())
				if player:isAlive() then
					local dummy = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)
...
guangzijupaoCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		if (#targets == 0) and to_select:objectName() ~= player:objectName() then
			return player:inMyAttackRange(to_select) and not to_select:isNude()
		end
	end,
	on_use = function(self, room, source, targets)
		local target = targets[1]
...
jueduiyazhiCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		if to_select:objectName() ~= player:objectName() then
			return #targets < player:getMark("@lanyu")
		end
	end,
	on_use = function(self, room, source, targets)
		source:loseMark("@lanyu", #targets)
...
xieyancard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		...
		local card = sgs.Sanguosha:getCard(self:getSubcards():first())
		if card and card:targetFixed() and card:isAvailable(player) then
			return false
		else
			return card and card:targetFilter(plist, to_select, player) and
			not player:isProhibited(to_select, card, plist) and card:isAvailable(player)
		end
	end,
	feasible = function(self, targets, player)
		local plist = sgs.PlayerList()
		for i = 1, #targets do plist:append(targets[i]) end
		local card = sgs.Sanguosha:getCard(self:getSubcards():first())
		return card and card:targetsFeasible(plist, player) and card:isAvailable(player)
	end,
				...
				if player:isAlive() and player:getMark("@liufangzhe") == 1 then
					for _, wtz in sgs.qlist(room:getAlivePlayers()) do
						if wtz and wtz:isAlive() and wtz:getMark("@weituozhe") == 1 then
							...
							room:loseHp(player, num, true, wtz, self:objectName())
						end
					end
				end
...
duandaiCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		...
		local flags = { "e", "h", "j" }
		if player:hasFlag("duandai_h") or not player:canDiscard(to_select, "h") then
			table.removeOne(flags, "h")
		end
		if player:hasFlag("duandai_e") or not player:canDiscard(to_select, "e") then
			table.removeOne(flags, "e")
		end
		if player:hasFlag("duandai_j") or not player:canDiscard(to_select, "j") then
...
luajieaoCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		if #targets == 0 then
			if not to_select:isKongcheng() and player:canPindian(to_select) then
				return to_select:objectName() ~= player:objectName()
			end
		end
		return false
...
zhongerhxCard   = sgs.CreateSkillCard {
	...
	--handling_method = sgs.Card_MethodNone,
	filter = function(self, targets, to_select, player)
		local card = player:getMark("zhongerhuan") - 1
...
qgjiesiCard     = sgs.CreateSkillCard {
	name = "qgjiesi",
	filter = function(self, targets, to_select, player)
		return #targets < player:getMark(self:objectName())
	end,
	feasible = function(self, targets, player)
		return #targets > 0 and #targets <= player:getMark(self:objectName())
	end,
	on_use = function(self, room, source, targets)
		local sum = 0
		for _, p in pairs(targets) do
...
zjyizhenCard    = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		...
		(to_select:isWounded()) and to_select:objectName() ~= player:objectName()
	end,
	on_effect = function(self, effect)
		local dest = effect.to
		local room = dest:getRoom()
...
mingmendiaCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		return #targets < 2 and to_select:objectName() ~= player:objectName() and (not to_select:isAllNude())
	end,
	on_use = function(self, room, source, targets)
		...
		if targets[2] then
			...
			if card:getSuit() ~= carda:getSuit() then
				room:loseHp(player, 1, true, source, "mingmendia")
			end
		end
	end
}
...
yanguangCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select, player)
		...
		slash:deleteLater()
		return (#targets < 1) and slash:targetFilter(targets_list, to_select, player) and
		player:canSlash(to_select, false)
	end,
	on_effect = function(self, effect)
		local room = effect.from:getRoom()
		room:loseHp(effect.from, 1, true, effect.from, "luanyanguang")
		local slash = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)
		slash:setSkillName("luanyanguang")
...
zchizeCard = sgs.CreateSkillCard {
	...
	will_throw = false,
	filter = function(self, targets, to_select, player)
		return #targets == 0 and (not to_select:isKongcheng() and not player:isKongcheng()) and
		to_select:objectName() ~= player:objectName()
	end,
	on_use = function(self, room, source, targets)
		local target = targets[1]
	...
	on_trigger = function(self, event, player, data)
		...
		if room:askForSkillInvoke(player, self:objectName(), data) then
			...
			if s then
				...
				elseif mb > xx then
					room:loseHp(s, 1, true, player, self:objectName())
				end
			end
		end
	end,
