...
extension_fakeoffline = sgs.Package("heg_fakeoffline", sgs.Package_GeneralPack)
...
function ChangeGeneral(room, player, skill_onwer_general_name, kingdom)
	...
	if num == 0 then
		...
		local generals_new = {}
		for i = 1, math.min(num, #generals) do
			local index = math.random(1, #generals)
			table.insert(generals_new, generals[index])
			table.remove(generals, index)
		end
		local general = room:askForGeneral(player, table.concat(generals_new, "+"))
...
function CardIsHezong(id)
	...
	if id>=0 then
		...
		and table.contains(ec:property("CharTag"):toStringList(),"transfer_card")) or sgs.Sanguosha:getCard(id):hasFlag("heg_transfer_card"))
	end
end
...
heg_transferCard = sgs.CreateSkillCard{
	...
	on_use = function(self,room,source,targets)
		...
		for _,id in sgs.list(ids) do
			...
			room:obtainCard(target,sgs.Sanguosha:getCard(id),reason,false)
		end
		
		-- 摸等量的牌
		source:drawCards(count, "heg_transfer")
		local log = sgs.LogMessage()
		log.type = "#heg_transfer"
		log.from = source
		log.to:append(target)
		log.arg = tostring(count)
		room:sendLog(log)
	end
}
...
hegMark = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		...
		if mark.name=="@heg_xianqu" and mark.gain < 0 and player:getMark("@heg_xianqu") == 0 then
			room:detachSkillFromPlayer(player,"heg_xianqu",true,true)
		end
		...
		if mark.name=="@heg_yinyangyu" and mark.gain < 0 and player:getMark("@heg_yinyangyu") == 0 then
			room:detachSkillFromPlayer(player,"heg_yinyangyu",true,true)
		end
	end
}
...
heg_qingchengCard = sgs.CreateSkillCard{
	...
	about_to_use = function(self,room,card_use)
		...
		if (skill_qc ~= "") then
			ChoiceLog(player, skill_qc, nil, "heg_qingcheng")
	...
	on_use = function(self, room, source, targets)
		...
		if source:getMark(self:objectName().."engine") > 0 then
			...
			if #SkillList > 0 then
				...
				ChoiceLog(source, choice, nil, self:objectName())
...
bf_yigui = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		...
		if #generals > 0 then
			...
			ChoiceLog(player, general, player, "bf_yigui")
			...
			ChoiceLog(player, general2, player, "bf_yigui")
		...
		if event == sgs.Damaged then
			...
			if damage.to and damage.to:objectName() == player:objectName() and player:hasSkill(self:objectName()) then
				...
				if #generals > 0  and room:askForSkillInvoke(player, self:objectName()) then
					room:broadcastSkillInvoke(self:objectName())
					local general = generals[math.random(1, #generals)]
					ChoiceLog(player, general, player, self:objectName())
					room:addPlayerMark(player, "bf_yigui"..general)
					room:doAnimate(4, player:objectName(), general)
				end
			end
				...
				for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					if p:objectName() ~= dying.who:objectName() and p:getKingdom() ~= dying.who:getKingdom() then
						...
						if #generals > 0  and room:askForSkillInvoke(p, self:objectName()) then
							room:broadcastSkillInvoke(self:objectName())
							local general = generals[math.random(1, #generals)]
							ChoiceLog(p, general, p, self:objectName())
...
bf_nos_xinsheng = sgs.CreateMasochismSkill{
	...
	on_damaged = function(self, player, damage)
		...
		if #generals > 0 and room:askForSkillInvoke(player, self:objectName()) then
			...
			if player:getMark(self:objectName().."engine") > 0 then
				local general = generals[math.random(1, #generals)]
				ChoiceLog(player, general, player, "bf_nos_huashen")
				room:addPlayerMark(player, "bf_nos_huashen"..general)
				local SkillList = {}
				for _,skill in sgs.qlist(sgs.Sanguosha:getGeneral(general):getVisibleSkillList()) do
					if not skill:inherits("SPConvertSkill") and not skill:isAttachedLordSkill() and not skill:isLordSkill() and skill:getFrequency() ~= sgs.Skill_Wake and skill:getFrequency() ~= sgs.Skill_Limited and skill:getFrequency() ~= sgs.Skill_Compulsory then
...
function startCommand(from, skill_name, num)
	...
	num = num or 2
	if from:hasSkill("heg_fofl_2_zhengrong") then
		num = num + 1
	end
	local commands = {}
	local indices = {}
...
heg_zhengbiCard = sgs.CreateSkillCard{
	...
	on_effect = function(self, effect)
		...
		local recv_card = room:askForUseCard(effect.to, "@@zhengbidiscard!", prompt)
		if recv_card then
		...
		if (not recv_card) then
			...
			for _, c in sgs.qlist(effect.to:getCards("he")) do
				if not c:isKindOf("BasicCard") then
					to_obtain:addSubcard(c)
					nonbasic = true
					break
				end
			end
			if to_obtain:subcardsLength() < 2 and not nonbasic then
				for _, c in sgs.qlist(effect.to:getCards("he")) do
					if c:isKindOf("BasicCard") and to_obtain:subcardsLength() < 2 then
						to_obtain:addSubcard(c)
					end
				end
			end
		...
		if to_obtain then
			room:obtainCard(effect.from, to_obtain, true)
		end
	end
}
...
heg_fengyingVS = sgs.CreateZeroCardViewAsSkill{
	name = "heg_fengying",
	view_as = function()
		local skillcard = sgs.Sanguosha:cloneCard("heg_threaten_emperor", sgs.Card_NoSuit, 0)
...
heg_fengying = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		local use = data:toCardUse()
		if use.card and use.card:isKindOf("heg_threaten_emperor") and table.contains(use.card:getSkillNames(), self:objectName()) then
			room:removePlayerMark(player, "@heg_fengying")
			room:broadcastSkillInvoke(self:objectName())
...
heg_yuanshu = sgs.General(extension_hegquan, "heg_yuanshu", "qun", 4)

heg_yongsi_buff = sgs.CreateViewAsEquipSkill{
	name = "#heg_yongsi_buff",
	view_as_equip = function(self, player)
		...
		if can_invoke and player:hasSkill("heg_yongsi") then
			return "heg_jade_seal"
		end
	end
}
heg_yongsi = sgs.CreateTriggerSkill{
	name = "heg_yongsi",
	waked_skills = "heg_jade_seal",
	frequency = sgs.Skill_Compulsory,
	events = {sgs.TargetConfirmed},
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		local use = data:toCardUse()
		if use.card and use.card:isKindOf("heg_known_both") and use.to:contains(player) and not player:isKongcheng() then
			room:sendCompulsoryTriggerLog(player, "heg_yongsi")
			room:showAllCards(player)
		end
		return false
	end
...
heg_weidiCard = sgs.CreateSkillCard{
	name = "heg_weidi",
	filter = function(self, targets, to_select)
		return #targets == 0 and to_select:objectName() ~= sgs.Self:objectName() and to_select:getMark("heg_weidi-Clear") > 0
	end,
	on_effect = function(self, effect)
		local room = effect.from:getRoom()
		local command = startCommand(effect.from, self:objectName())
		local invoked = doCommand(effect.to, self:objectName(), command, effect.from)
		if not invoked then
			room:broadcastSkillInvoke(self:objectName())
			...
			room:obtainCard(effect.from, to_obtain, false)
			local give_card = room:askForExchange(effect.from, self:objectName(), to_obtain:subcardsLength(), to_obtain:subcardsLength(), true, "WeidiGive::"..effect.to:objectName(), false)
			if give_card then
				room:giveCard(effect.from, effect.to, give_card, self:objectName())
			end
		end
	end
}
heg_weidiVS = sgs.CreateZeroCardViewAsSkill{
	name = "heg_weidi",
	view_as = function()
		return heg_weidiCard:clone()
	end,
	enabled_at_play = function(self, player)
		return not player:hasUsed("#heg_weidi")
	end
}
heg_weidi = sgs.CreateTriggerSkill{
	name = "heg_weidi",
	events = {sgs.CardsMoveOneTime},
	view_as_skill = heg_weidiVS,
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		local move = data:toMoveOneTime()
		if move.to and move.to_place == sgs.Player_PlaceHand and move.from_places:contains(sgs.Player_DrawPile) then
			local to = room:findPlayerByObjectName(move.to:objectName())
			room:addPlayerMark(to, "heg_weidi-Clear")
		end
	end
}

heg_yuanshu:addSkill(heg_yongsi)
heg_yuanshu:addSkill(heg_yongsi_buff)
extension_hegquan:insertRelatedSkills("heg_yongsi", "#heg_yongsi_buff")
heg_yuanshu:addSkill(heg_weidi)
...
heg_zhanglu = sgs.General(extension_heglordex,  "heg_zhanglu", "qun+wei", 3, true, true)
			...
			if use.from and use.from:getMark("heg_midao-PlayClear") == 0 and (use.card:isKindOf("Slash") or (use.card:isKindOf("TrickCard") and use.card:isDamageCard())) and use.to:length() > 0 then
				...
				for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					...
					if can_invoke then
						...
						ChoiceLog(p, nature, nil, self:objectName())
...
heg_simayi = sgs.General(extension_hegpurplecloud,  "heg_simayi", "jin", 4)

heg_yingshi = sgs.CreateTriggerSkill{
	name = "heg_yingshi",
	events = { sgs.EventPhaseStart },
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		if player:getPhase() == sgs.Player_Play then
			...
			known_both:setSkillName(self:objectName())
			known_both:deleteLater()
			for _, p in sgs.qlist(room:getAllPlayers()) do
				local others = room:getCardTargets(p, known_both)
				if not others:isEmpty() and known_both:isAvailable(p) then
					targets:append(p)
				end
			end
			if not targets:isEmpty() then
				local from = room:askForPlayerChosen(player, targets, self:objectName(), "heg_yingshi-invoke", true, true)
				if from then
					local others = room:getCardTargets(from, known_both)
					local to = room:askForPlayersChosen(player, others, "heg_yingshi_target", 1, 1, "@heg_yingshi:"..from:objectName(), true, true)
					local use = sgs.CardUseStruct()
					use.from = from
					use.to:append(to:first())
					use.card = known_both
					room:useCard(use)
...
heg_shunfuCard = sgs.CreateSkillCard{
	name = "heg_shunfu",
	target_fixed = false,
	filter = function(self, targets, to_select)
	...
	on_use = function(self, room, source, targets)
		room:removePlayerMark(source, "@heg_shunfu")
		for _, p in ipairs(targets) do
			p:drawCards(2, "heg_shunfu")
		end
		for _, p in ipairs(targets) do
			if not p:isDead() then
				...
				if not victims:isEmpty() then
					room:askForUseSlashTo(p, victims, "heg_shunfu-slash", false, false, false, source, nil, "heg_shunfu")
				end
			end
...
heg_shunfuVS = sgs.CreateZeroCardViewAsSkill{
	name = "heg_shunfu",
	view_as = function(self)
		local skill_card = heg_shunfuCard:clone()
		return skill_card
	end,
	enabled_at_play = function(self, player)
		return player:getMark("@heg_shunfu") > 0
	end
}
heg_shunfu = sgs.CreateTriggerSkill{
	name = "heg_shunfu",
	events = { sgs.PreCardUsed },
	limit_mark = "@heg_shunfu",
	frequency = sgs.Skill_Limited,
	view_as_skill = heg_shunfuVS,
	on_trigger = function(self, event, player, data, room)
		if event == sgs.PreCardUsed then
			local use = data:toCardUse()
			if use.card and use.card:isKindOf("Slash") and use.card:hasFlag("heg_shunfu") then
				local no_respond_list = use.no_respond_list
				table.insert(no_respond_list, "_ALL_TARGETS")
				use.no_respond_list = no_respond_list
				data:setValue(use)
...
heg_simayi:addSkill(heg_yingshi)
heg_simayi:addSkill(heg_shunfu)
...
heg_jin_simazhao = sgs.General(extension_hegpurplecloud,  "heg_jin_simazhao", "jin", 3)
...
heg_beiluan_Clear = sgs.CreateTriggerSkill{
	name = "#heg_beiluan_Clear",
	events = { sgs.EventPhaseChanging },
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		local change = data:toPhaseChange()
		if change.to == sgs.Player_NotActive then
			for _,p in sgs.qlist(room:getAlivePlayers())do
				...
				for _,c in sgs.qlist(p:getHandcards())do
					if c:getSkillName()== "heg_beiluan" then
						hw:append(c)
					end
				end
				room:filterCards(p,hw,false)
			end
		end
		return false
	end,
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.Damaged then
			...
			if damage.from and not damage.from:isKongcheng() and room:askForSkillInvoke(player, self:objectName(), data) then
				room:addPlayerMark(damage.from, "&heg_beiluan+to+#"..player:objectName().."-Clear", 1)
				room:broadcastSkillInvoke(self:objectName())
...
heg_jin_simazhao:addSkill(heg_beiluan_Clear)
extension_hegpurplecloud:insertRelatedSkills("heg_beiluan", "#heg_beiluan_Clear")
...
heg_pojingCard = sgs.CreateSkillCard{
	...
	on_use = function(self, room, source, targets)
		...
		if not target:isAllNude() then
			table.insert(choicelist, "obtain="..source:objectName())
		end
		local others = room:getOtherPlayers(target)
		others:removeOne(source)
		local targets = sgs.SPlayerList()
		for _, other in sgs.qlist(others) do
			if other:getMark("heg_pojing"..source:objectName()) == 0 then
				targets:append(other)
			end
		end
		local choice = room:askForChoice(target, "heg_pojing", table.concat(choicelist, "+"), ToData(source))
		if string.startsWith(choice, "obtain") then
			...
			local damage = sgs.DamageStruct()
			damage.reason = "heg_pojing"
			damage.to = target
			damage.damage = 1
			for _, other in sgs.qlist(targets) do
				if room:askForSkillInvoke(other, "heg_pojing", ToData(target)) then
					room:addPlayerMark(other, "heg_pojing"..source:objectName(), 1)
					local splayer = sgs.SPlayerList()
					splayer:append(source)
					room:addPlayerMark(other, "&heg_pojing+to+#"..source:objectName(), 1, splayer)
					damage.from = other
					room:damage(damage)
				end
			end
	...
	on_trigger = function(self, event, player, data)
		...
		if change.to == sgs.Player_Draw then
			if room:askForSkillInvoke(player, self:objectName()) then
				...
				local x = 0
				local n = targets:length() -- 獲取目標總數

				-- 必須先判斷列表不為空，否則對 0 取餘數會報錯或崩潰
				if n > 0 then
					for i = 1, 4 do
						-- Lua 的 for 循環通常從 1 開始，但 C++ 的列表索引通常從 0 開始
						-- (i - 1) 把 1~4 變成 0~3
						-- % n 確保索引永遠不會超過人數，會自動回到開頭
						local index = (i - 1) % n
						
						local p = targets:at(index)
						p:drawCards(1, self:objectName())
					end
				end
				player:skip(sgs.Player_Draw)
...
heg_shejus = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if damage.to then
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if damage.to:getMark("heg_shejus"..p:objectName()) == 0 and p:isWounded() and p:canDiscard(p, "h") then
					if room:askForSkillInvoke(damage.to, self:objectName(), ToData(p)) then
						...
						local splayer = sgs.SPlayerList()
						splayer:append(p)
						room:addPlayerMark(damage.to, "&heg_shejus+to+#"..p:objectName(), 1, splayer)
					end
				end
			end
		end
...
heg_zhulan = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if damage.from and damage.from:getKingdom() == player:getKingdom() then
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if p:canDiscard(p, "he") and p:objectName() ~= player:objectName() then
					room:setTag("heg_zhulan", data)
...
heg_luanchang = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if change.to == sgs.Player_NotActive and not player:isKongcheng() then
			...
			local current = room:getCurrent()
			if invoke and current and current:isAlive() then
				for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					if not current:isKongcheng() and p:getMark("@heg_luanchang")
...
heg_zhuosheng = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.CardsMoveOneTime then
            local move = data:toMoveOneTime()
            if not room:getTag("FirstRound"):toBool() and move.to and move.to:objectName() == player:objectName() and player:getMark("&heg_zhuosheng-Clear") < 1 then
                local ids = sgs.IntList()
                for _, id in sgs.qlist(move.card_ids) do
                ...
                if room:askForSkillInvoke(player, self:objectName(), data) then
                    room:setPlayerMark(player, "&heg_zhuosheng-Clear", 1)
                end
            end
        elseif event == sgs.DamageCaused then
            local damage = data:toDamage()
		 ...
		 elseif event == sgs.PreCardUsed then
            local use = data:toCardUse()
            if use.card:isKindOf("SkillCard") then return false end
            if player:getMark("&heg_zhuosheng-Clear") > 0 then
                room:setCardFlag(use.card, self:objectName())
			end
        end
        return false
...
heg_ciwei = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		local use = data:toCardUse()
		local current = room:getCurrent()
		if current and current:hasSkill(self:objectName()) and player:objectName() ~= current:objectName() and use.card and not use.card:isKindOf("SkillCard") then
			local invoke = false
			for _, p in sgs.qlist(room:getOtherPlayers(player)) do
			...
			if invoke and current:canDiscard(current, "he") then
				if room:askForCard(current, ".", "@heg_ciwei:"..use.card:getLogName(), data, sgs.Card_MethodDiscard, nil, false, self:objectName(),	false, nil) then
					room:broadcastSkillInvoke(self:objectName())
...
heg_yanxiCard = sgs.CreateSkillCard{
	...
	on_use = function(self, room, source, targets)
		...
		for i,target in ipairs(targets)do
			...
			to_obtain:append(id)
			local choice = room:askForChoice(target, "heg_yanxi", table.concat(choices, "+"), ToData(id))
			ChoiceLog(target, choice, nil, "heg_yanxi")
			choiceslist[target:objectName()] = choice
		end
		for _, id in sgs.qlist(to_obtain) do
			room:showCard(source, id)
		end
		room:fillAG(to_obtain, source)
		local to_get = room:askForAG(source, to_obtain, false, self:objectName())
		room:clearAG(source)
		local from = room:getCardOwner(to_get)
		local declared = choiceslist[from:objectName()]
		local obtained_card = sgs.Sanguosha:getCard(to_get)
		source:obtainCard(obtained_card)
		if obtained_card:getClassName() ~= declared then
			for _, id in sgs.qlist(to_obtain) do
				if id ~= to_get then
					source:obtainCard(sgs.Sanguosha:getCard(id), false)
				end
			end
		end
	end
...
heg_shiren = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		local damage = data:toDamage()
		for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
			if p:getMark("heg_shiren-Clear") == 0 and p:objectName() ~= damage.to:objectName() and p:getCardCount(true) >= 2 then
				if room:askForSkillInvoke(p, self:objectName(), data) then
					room:broadcastSkillInvoke(self:objectName())
					local to_give = room:askForExchange(p, self:objectName(), 2, 2, false, "@heg_shiren-give:"..player:objectName(), false)
...
heg_weiguan = sgs.General(extension_hegpurplecloud,  "heg_weiguan", "jin", 3)

heg_chengxi = sgs.CreateTriggerSkill{
	name = "heg_chengxi",
	frequency = sgs.Skill_NotFrequent,
	events = {sgs.EventPhaseProceeding, sgs.CardsMoveOneTime, sgs.CardFinished},
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseProceeding and player:getPhase() == sgs.Player_Start and player:hasSkill(self:objectName()) then
			local card = sgs.Sanguosha:cloneCard("heg_await_exhausted", sgs.Card_NoSuit, 0)
			card:setSkillName(self:objectName())
			card:deleteLater()
			
			local dest = room:askForPlayerChosen(player, room:getAlivePlayers(), self:objectName(), "@heg_chengxi:"..card:objectName(), true, true)
			if dest then
				local others = room:askForPlayersChosen(player, room:getAlivePlayers(), self:objectName(), 0, room:getAlivePlayers():length(), "@heg_chengxi_target:"..card:objectName(), false, true)
				if others and others:length() > 0 then
					room:setPlayerMark(dest, "heg_chengxi_user-Clear", 1)					
					local use = sgs.CardUseStruct()
					use.from = dest
					use.card = card
					use.to = others
		...
		elseif event == sgs.CardsMoveOneTime then
			local move = data:toMoveOneTime()
			if player:getMark("heg_chengxi_user-Clear") > 0 and move.to_place == sgs.Player_DiscardPile and move.from then
				if move.reason.m_reason == sgs.CardMoveReason_S_REASON_THROW then
					local from = room:findPlayerByObjectName(move.from:objectName())
					if from and from:hasFlag("EXCard_YYDL_effected") then
						for _, id in sgs.qlist(move.card_ids) do
							local c = sgs.Sanguosha:getCard(id)
							if not c:isKindOf("BasicCard") then
								room:setPlayerMark(from, "heg_chengxi_has_nonbasic-Clear", 1)
								break
							end
						end
					end
		...
		elseif event == sgs.CardFinished then
			local use = data:toCardUse()
			if use.card and use.card:isKindOf("heg_await_exhausted") and use.from and use.from:getMark("heg_chengxi_user-Clear") > 0 then
				local invoke = false
				for _, to in sgs.qlist(use.to) do
					if to:isAlive() and to:getMark("heg_chengxi_has_nonbasic-Clear") > 0 then
						invoke = true
						break
					end
				end
				if invoke then
					for _, to in sgs.qlist(use.to) do
						if to:isAlive() then
							room:setPlayerMark(to, "heg_chengxi_has_nonbasic-Clear", 0)
							room:damage(sgs.DamageStruct(self:objectName(), use.from, to, 1))
						end
					end
				end
				-- Clear marks
				room:setPlayerMark(use.from, "heg_chengxi_user-Clear", 0)
			end
		end
		return false
	end,
...
heg_jiantongCard = sgs.CreateSkillCard{
	name = "heg_jiantong",
	will_throw = false,
	handling_method = sgs.Card_MethodNone,
	...
	on_use = function(self, room, source, targets)
		...
		if not source:getEquips():isEmpty() and room:askForSkillInvoke(source, "heg_jiantong", sgs.QVariant("exchange")) then
			local equip_id = room:askForCardChosen(source, source, "e", "heg_jiantong", false, sgs.Card_MethodNone, sgs.IntList() ,true)
			if equip_id >= 0 then
				local hand_ids = sgs.IntList()
				for i = 1, math.min(2, handcards:length()), 1 do
					local id = room:askForCardChosen(source, target, "h", "heg_jiantong", false,sgs.Card_MethodNone, hand_ids)
					if id >= 0 then
						hand_ids:append(id)
					end
				end
				...
				if not hand_ids:isEmpty() then
					...
					move_to_table1.card_ids:append(equip_id)
					move_to_table1.to_place = sgs.Player_PlaceTable
					move_to_table1.reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_SWAP, source:objectName(), "heg_jiantong", "")
					
					-- Move hand cards to table
					local move_to_table2 = sgs.CardsMoveStruct()
					move_to_table2.card_ids = hand_ids
					move_to_table2.to_place = sgs.Player_PlaceTable
					move_to_table2.reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_SWAP, source:objectName(), "heg_jiantong", "")
					...
					-- Move from table to destinations
					local move1 = sgs.CardsMoveStruct()
					move1.card_ids = sgs.IntList()
					move1.card_ids:append(equip_id)
					move1.to = target
					move1.to_place = sgs.Player_PlaceHand
					move1.reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_SWAP, source:objectName(), "heg_jiantong", "")
					
					local move2 = sgs.CardsMoveStruct()
					move2.card_ids = hand_ids
					move2.to = source
					move2.to_place = sgs.Player_PlaceHand
					move2.reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_SWAP, source:objectName(), "heg_jiantong", "")
...
heg_jiantongVS = sgs.CreateViewAsSkill{
	name = "heg_jiantong",
	n = 0,
	view_as = function(self, cards)
		local card = heg_jiantongCard:clone()
		card:setSkillName(self:objectName())
		return card
	end,
	enabled_at_play = function(self, player)
	...
	enabled_at_response = function(self, player, pattern)
		return pattern == "@@heg_jiantong"
	end,
}

heg_jiantong = sgs.CreateTriggerSkill{
	name = "heg_jiantong",
	view_as_skill = heg_jiantongVS,
	events = {sgs.Damaged},
	on_trigger = function(self, event, player, data, room)
		if event == sgs.Damaged then
			room:askForUseCard(player, "@@heg_jiantong", "@heg_jiantong", -1, sgs.Card_MethodNone)
		end
		return false
	end,
}

heg_weiguan:addSkill(heg_chengxi)
heg_weiguan:addSkill(heg_jiantong)
...
heg_jianzhi = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.DamageCaused then
			local damage = data:toDamage()
			if damage.to and damage.damage >= damage.to:getHp() + damage.to:getHujia() then
...
extension_hegpurplecloud:insertRelatedSkills("heg_chujue", "#heg_chujue_buff")
		...
		if card and card:isKindOf("BasicCard") and player:getPhase() == sgs.Player_NotActive then
			...
			if not targets:isEmpty() then
				...
				if target then
					...
					local to_discard = room:askForCardChosen(player, target, "h", self:objectName(), true, sgs.Card_MethodDiscard, disabled)
		...
		if use.card and use.card:isDamageCard() then
			...
			if not targets:isEmpty() then
				...
				if target then
					...
					local first_color = sgs.Sanguosha:getCard(to_show:first()):getColor()
					for _, c in sgs.qlist(to_show) do
						if sgs.Sanguosha:getCard(c):getColor() ~= first_color then
							same_color = false
							break
						end
					end
...
heg_chengliuCard = sgs.CreateSkillCard{
	...
	on_use = function(self, room, source, targets)
		...
		damage.damage = 1
		room:damage(damage)
		if room:askForSkillInvoke(source, "heg_chengliu", ToData(target)) then
			...
			local move1 = sgs.CardsMoveStruct(source_ids, target, sgs.Player_PlaceEquip, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_SWAP, source:objectName(), self:objectName(), ""))
			local move2 = sgs.CardsMoveStruct(target_ids, source, sgs.Player_PlaceEquip, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_SWAP, target:objectName(), self:objectName(), ""))
			local moves = sgs.CardsMoveList()
			moves:append(move1)
			moves:append(move2)
			room:moveCardsAtomic(moves, true)			
...
heg_lvxian = sgs.CreateTriggerSkill{
    ...
    on_trigger = function(self, event, player, data)
        ...
        if event == sgs.CardsMoveOneTime then
            local move = data:toMoveOneTime()
            if move.from and move.from:objectName() == player:objectName() and (move.from_places:contains(sgs.Player_PlaceHand) or move.from_places:contains(sgs.Player_PlaceEquip)) and player:hasSkill(self:objectName()) then
				local count = move.card_ids:length()
				room:addPlayerMark(player, "heg_lvxian_lost-Clear", count)
            end
        elseif event == sgs.EventPhaseChanging then
            local change = data:toPhaseChange()
            if change.to == sgs.Player_NotActive then
				for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					room:setPlayerMark(p, "heg_lvxian_lastlost", p:getMark("heg_lvxian_lost-Clear"))
					room:setPlayerMark(p, "&heg_lvxian", p:getMark("heg_lvxian_lost-Clear"))
				end
				
				for _, p in sgs.qlist(room:getAllPlayers()) do
					if p:getMark("last_turn_player") > 0 then
...
heg_duanqiu_buff = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if card and card:isKindOf("Slash") and resp.m_toCard and resp.m_toCard:isKindOf("Duel") and table.contains(resp.m_toCard:getSkillNames(), "heg_duanqiu") then
			...
			if current then
				room:addPlayerMark(current, "heg_duanqiu_slash_count")
			end
		end
		return false
	end,
...
heg_duanqiu_limit = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if use.card and not use.card:isKindOf("SkillCard") then
			local current = room:getCurrent()
			if current and current:hasSkill("heg_duanqiu") and current:getMark("heg_duanqiu-Clear") > 0 then
  	...
  	on_trigger = function(self, event, player, data)
		...
		if player:getPhase() == sgs.Player_Start then
			...
			for _, p in sgs.qlist(room:getAlivePlayers()) do
				...
				if k ~= player:getKingdom() and not table.contains(kingdom_list, k) and CanToCard(duel, player, p) and not sgs.Sanguosha:isProhibited(player, p, duel) and duel:isAvailable(player) then
					table.insert(kingdom_list, k)
				end
			end
			if #kingdom_list == 0 then return false end
			if room:askForSkillInvoke(player, self:objectName()) then
				local kingdom = room:askForKingdom(player,  self:objectName().."_ChooseKingdom", table.concat(kingdom_list, "+"), true)
				for _, p in sgs.qlist(room:getAllPlayers()) do
					if p:getKingdom() == kingdom and CanToCard(duel, player, p) and not sgs.Sanguosha:isProhibited(player, p, duel) and duel:isAvailable(player) and player:isAlive() then
						local use = sgs.CardUseStruct()
						use.card = duel
						use.from = player
						use.to:append(p)
				...
				room:addPlayerMark(player, "heg_duanqiu-Clear")
			...
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if room:askForSkillInvoke(p, self:objectName(), ToData(player)) then
					...
					if choice == "range" then
						room:addAttackRange(p,  1, true)
					elseif choice == "maxcard" then
						room:addMaxCards(p, 1, true)
					elseif choice == "slash" then
						room:addSlashCishu(p, 1, true)
					end
					ChoiceLog(p, choice, nil, self:objectName())
					room:addPlayerMark(player, "&heg_huaiyuan+to+#" .. p:objectName().."-Clear")
				end
			end
		end
		return false
...
heg_fushou = sgs.CreateTriggerSkill{
  	name = "heg_fushou",
	frequency = sgs.Skill_Compulsory,
  	events = { sgs.GameStart, sgs.EventAcquireSkill, sgs.EventLoseSkill, sgs.KingdomChanged },
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
...
heg_neiji = sgs.CreateTriggerSkill{
  	...
  	on_trigger = function(self, event, player, data)
		...
		if player:getPhase() == sgs.Player_Play then
			local targets = sgs.SPlayerList()
			for _, p in sgs.qlist(room:getOtherPlayers(player)) do
				if not p:isKongcheng() and p:getHandcardNum() >= 2 then
					targets:append(p)
				end
			end
			local target = room:askForPlayerChosen(player, targets, self:objectName(), "heg_neiji-invoke", true, true)
			if not target then return false end
			room:setPlayerFlag(target, "heg_neiji")
			local source_cards = room:askForExchange(player, self:objectName(), 2, 2, false, "@heg_neiji-show", false)
			local target_cards = room:askForExchange(target, self:objectName(), 2, 2, false, "@heg_neiji-show", false)
			room:setPlayerFlag(target, "-heg_neiji")
			local slash_count = 0
			local from_count = 0
			local to_count = 0
			for _, id in sgs.qlist(source_cards:getSubcards()) do
				local card = sgs.Sanguosha:getCard(id)
			...
			for _, id in sgs.qlist(target_cards:getSubcards()) do
				local card = sgs.Sanguosha:getCard(id)
...
heg_xiace = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		local use = data:toCardUse()
		if use.card and use.card:isKindOf("Nullification") and table.contains(use.card:getSkillNames(), self:objectName()) then
			local current = room:getCurrent()
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getPhase() == sgs.Player_Finish then
			if room:askForCard(player, "^BasicCard", "@heg_limeng", data, sgs.Card_MethodDiscard, nil, false, self:objectName(), false, nil) then
				...
				damage1.to = to1
				damage1.damage = 1
				damage1.reason = self:objectName()
				room:damage(damage1)
				local damage2 = sgs.DamageStruct()
				damage2.from = to1
				damage2.to = to2
				damage2.damage = 1
				damage2.reason = self:objectName()
				room:damage(damage2)
			end
		end
		return false
...
heg_sunxiu = sgs.General(extension_goldenseal,  "heg_sunxiu", "jin", 3)
...
heg_xiejianCard = sgs.CreateSkillCard{
	...
	on_use = function(self, room, source, targets)
		...
		local choice = room:askForChoice(source, "start_command_heg_xiejian", table.concat(commands, "+"))
	...
	on_trigger = function(self, event, player, data)
		...
		if effect.card:isKindOf("Collateral") and table.contains(effect.card:getSkillNames(),self:objectName()) then
			...
			if effect.to:isAlive() then
				...
				local useslash
				if slash == "Slash!" then
					useslash = room:askForCard(effect.to,"Slash","collateral-slash:"..target:objectName()..":"..effect.from:objectName(),data,sgs.Card_MethodNone,effect.from,false,"collateral",false,effect.card)
				...
				elseif not effect.to:isKongcheng() then
					...
					if (effect.from:isAlive() and effect.to:getWeapon()) then
...
heg_duyu = sgs.General(extension_goldenseal,  "heg_duyu", "jin", 3)
...
heg_mobile_luxun = sgs.General(extension_hegmobile,  "heg_mobile_luxun", "wu", 3)

heg_mobile_qianxun = sgs.CreateTriggerSkill{
	name = "heg_mobile_qianxun",
	...
	on_trigger = function(self, event, player, data, room)
		...
		local jie_pile = player:getPile("heg_mobile_jie")
		if jie_pile:length() < 3 then
			room:broadcastSkillInvoke(self:objectName())
			SendComLog(self, player)
			use.to:removeOne(player)
			data:setValue(use)
			
			player:addToPile("heg_mobile_jie", use.card:getEffectiveId())
		end
		return false
	end,
}

heg_mobile_duoshiVS = sgs.CreateViewAsSkill{
	name = "heg_mobile_duoshi",
	n = 3,
	expand_pile = "heg_mobile_jie",
	view_filter = function(self, selected, to_select)
		local jie_pile = sgs.Self:getPile("heg_mobile_jie")
		local is_jie = jie_pile:contains(to_select:getEffectiveId())
		
		if is_jie then
			-- Using Jie pile cards - need 3 cards
			if #selected >= 3 then return false end
			for _, c in ipairs(selected) do
				if not jie_pile:contains(c:getEffectiveId()) then
					return false
				end
			end
			return true
		else
			-- Using hand card - need 1 red card
			if #selected > 0 then return false end
	...
	view_as = function(self, cards)
		...
		local jie_pile = sgs.Self:getPile("heg_mobile_jie")
		local is_jie = jie_pile:contains(cards[1]:getEffectiveId())
		
		if is_jie and #cards == 3 then
			-- Using 3 Jie pile cards - can become fire damage cards
			local dc = sgs.Self:getTag("heg_mobile_duoshi"):toCard()
			if dc then
				dc = sgs.Sanguosha:cloneCard(dc:objectName())
				for _, c in ipairs(cards) do
					dc:addSubcard(c)
				end
				dc:setSkillName("heg_mobile_duoshi")
				return dc
			end
		elseif not is_jie and #cards == 1 then
			-- Using 1 red hand card - becomes heg_await_exhausted
			if cards[1]:isRed() then
				local card = sgs.Sanguosha:cloneCard("heg_await_exhausted", cards[1]:getSuit(), cards[1]:getNumber())
				card:addSubcard(cards[1])
				card:setSkillName(self:objectName())
				return card
			end
	...
	enabled_at_play = function(self, player)
		return player:getMark("heg_mobile_duoshi-PlayClear") == 0
	end,
}

heg_mobile_duoshi = sgs.CreateTriggerSkill{
	name = "heg_mobile_duoshi",
	view_as_skill = heg_mobile_duoshiVS,
	juguan_type = "fire_slash,fire_attack,heg_burning_camps",
	events = {sgs.CardUsed},
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardUsed then
			local use = data:toCardUse()
			if use.card and table.contains(use.card:getSkillNames(), self:objectName()) then
				room:setPlayerMark(player, "heg_mobile_duoshi-PlayClear", 1)
			end
		end
		return false
	end,
...
heg_mobile_luxun:addSkill(heg_mobile_qianxun)
heg_mobile_luxun:addSkill(heg_mobile_duoshi)

heg_mobile_huangzhong = sgs.General(extension_hegmobile,  "heg_mobile_huangzhong", "shu", 4)
heg_mobile_liegong = sgs.CreateTriggerSkill {
	name = "heg_mobile_liegong",
	...
	on_trigger = function(self, event, player, data, room)
			...
			for _, p in sgs.qlist(use.to) do
				...
				if (player:getHp() <= handcardnum) or (player:getAttackRange() >= handcardnum) then
					...
					if player:askForSkillInvoke(self:objectName(), _data) then
						...
						room:sendLog(log)
						jink_table[index] = 0
						if (player:getHp() <= handcardnum) and (player:getAttackRange() >= handcardnum) then
							room:setCardFlag(use.card, "heg_mobile_liegong"..p:objectName())
						end
					end
				end
				index = index + 1
		...
		elseif event == sgs.ConfirmDamage then
			local damage = data:toDamage()
			if damage.card and damage.card:hasFlag("heg_mobile_liegong"..damage.to:objectName()) then
				damage.damage = damage.damage + 1
				local log = sgs.LogMessage()
				log.type = "#skill_add_damage"
				log.from = damage.from
...
heg_mobile_huangzhong:addSkill(heg_mobile_liegong)

heg_mobile_ganfuren = sgs.General(extension_hegmobile,  "heg_mobile_ganfuren", "shu", 3, false)

heg_mobile_shushen = sgs.CreateTriggerSkill{
	name = "heg_mobile_shushen" ,
		...
		elseif event == sgs.CardsMoveOneTime then
			...
			if move.from and move.from:objectName() == player:objectName() and move.from_places:contains(sgs.Player_PlaceHand) or move.from_places:contains(sgs.Player_PlaceEquip) then
				...
				for i = 0, move.card_ids:length() - 1, 1 do
					...
					if from_place == sgs.Player_PlaceHand or from_place == sgs.Player_PlaceEquip then
						count = count + 1
					end
				end
				if count >= player:getHp() then
...
heg_mobile_ganfuren:addSkill(heg_mobile_shushen)
heg_mobile_ganfuren:addSkill("shenzhi")

heg_mobile_tianfeng = sgs.General(extension_hegmobile,  "heg_mobile_tianfeng", "qun", 3)

heg_mobile_suishi = sgs.CreateTriggerSkill{
	name = "heg_mobile_suishi" ,
	...
	on_trigger = function(self, event, player, data, room)
		...
		if event == sgs.Dying then
			...
			if (dying.who:objectName() ~= player:objectName()) and target then
				if room:askForSkillInvoke(target, self:objectName(), sgs.QVariant("draw:" .. player:objectName())) then
					player:drawCards(1, self:objectName())
				end
			end
		elseif event == sgs.Death then
			...
			if target then
				if room:askForSkillInvoke(target, self:objectName(), sgs.QVariant("discard:" .. player:objectName())) then
					local choicelist = {"losehp"}
					if player:canDiscard(player, "h") then
						table.insert(choicelist, "discardhandcard")
					end
					local choice = room:askForChoice(player, self:objectName(), table.concat(choicelist, "+"), ToData(player))
					if choice == "discardhandcard" then
						player:throwAllHandCards(self:objectName())
					else
						room:loseHp(player, 1, true, target, self:objectName())
					end
				end
			end
		end
...
heg_mobile_kuangfu = sgs.CreateTriggerSkill{
	name = "heg_mobile_kuangfu",
	events = {sgs.Damage},
	on_trigger = function(self, event, player, data, room)
		if event == sgs.Damage then
			local damage = data:toDamage()
			...
			if not damage.transfer and not damage.chain and not damage.to:isNude() then
				if player:askForSkillInvoke(self:objectName(), ToData(damage.to)) then
					local id = room:askForCardChosen(player, damage.to, "he", self:objectName())
					local choicelist = {"obtain"}
...
heg_mobile_panfeng:addSkill(heg_mobile_kuangfu)

heg_mobile_xiahoudun = sgs.General(extension_hegmobile,  "heg_mobile_xiahoudun", "wei", 5)

heg_mobile_xiahoudun:addSkill("nosganglie")
...
heg_ov_zhugejin = sgs.General(extension_heg,  "heg_ov_zhugejin", "wu", 3)
heg_ov_hongyuanCard = sgs.CreateSkillCard{
	name = "heg_ov_hongyuan",
	target_fixed = true,
	will_throw = false,
	on_use = function(self, room, source, targets)
		local id = self:getSubcards():first()
		local card = sgs.Sanguosha:getCard(id)
		-- card:addCharTag("transfer_card") --TODO
		room:setCardFlag(card, "heg_transfer_card")
		room:setCardTip(id, "heg_ov_hongyuan")
		local wrap = source:getTag("heg_ov_hongyuan"):toIntList()
		wrap:append(id)
		source:setTag("heg_ov_hongyuan",ToData(wrap))
		room:attachSkillToPlayer(source,"heg_transfer")
	end
}

heg_ov_hongyuanVS = sgs.CreateOneCardViewAsSkill{
	name = "heg_ov_hongyuan",
	view_filter = function(self, card)
		return not CardIsHezong(card:getEffectiveId())
	end,
	view_as = function(self, card)
		local skill_card = heg_ov_hongyuanCard:clone()
		skill_card:addSubcard(card)
		return skill_card
	end,
	enabled_at_play = function(self, player)
		return not player:hasUsed("#heg_ov_hongyuan")
	end,
}

heg_ov_hongyuan = sgs.CreateTriggerSkill{
	name = "heg_ov_hongyuan",
	events = {sgs.DrawNCards},
	view_as_skill = heg_ov_hongyuanVS,
	waked_skills = "heg_transfer",
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		local draw = data:toDraw()
		if draw.reason == "heg_transfer" then
			local target = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(), "@heg_ov_hongyuan", true, true)
			if target then
				draw.num = 0
				data:setValue(draw)
				target:drawCards(1, self:objectName())
...
heg_ov_hongyuan_Clear = sgs.CreateTriggerSkill{
	name = "#heg_ov_hongyuan_Clear",
	events = {sgs.EventPhaseChanging},
	...
	on_trigger = function(self,event,player,data)
		...
		local change = data:toPhaseChange()
		if change.to == sgs.Player_NotActive then
			for _,owner in sgs.list(room:findPlayersBySkillName("heg_ov_hongyuan"))do
				local ids = player:getTag("heg_ov_hongyuan"):toIntList()
				if ids:isEmpty() then continue end
				
				for _,id in sgs.list(ids)do
					if sgs.Sanguosha:getCard(id):hasFlag("heg_transfer_card") then room:setCardFlag(id, "-heg_transfer_card") 
						room:setCardTip(id , "-heg_ov_hongyuan")
					end
				end
				player:setTag("heg_ov_hongyuan",sgs.QVariant())
			end
		end
		return false
	end,
...
heg_ov_zhugejin:addSkill("huanshi")
heg_ov_zhugejin:addSkill(heg_ov_hongyuan)
heg_ov_zhugejin:addSkill(heg_ov_hongyuan_Clear)
extension_heg:insertRelatedSkills("heg_ov_hongyuan", "#heg_ov_hongyuan_Clear")
heg_ov_zhugejin:addSkill("mingzhe")
				...
				for _, caocao in sgs.qlist(room:findPlayersBySkillName("heg_lord_jianan")) do
					...
					if #choices > 0 and player:canDiscard(player, "he") and room:askForDiscard(player, self:objectName(), 1, 1, true, true, "@heg_lord_jianan-discard") then
						...
						if #player_skills > 0 then
							...
							ChoiceLog(player, to_disable, nil, self:objectName())
...
-- heg_lord_simayi
heg_lord_simayi = sgs.General(extension_heg, "heg_lord_simayi", "jin", 4)
...
heg_jiaping_AttachCard = sgs.CreateSkillCard{
	...
	on_use = function(self, room, source, targets)
		...
		if lord then
			...
			local skills = {"heg_shunfu", "heg_fengying", "heg_jianglue", "bf_yongjin", "luanwu"}
			local available = {}
...
heg_jiaping = sgs.CreateTriggerSkill{
	...
	frequency = sgs.Skill_NotFrequent,
	events = {sgs.GameStart, sgs.EventAcquireSkill},
	waked_skills = "heg_shunfu,heg_fengying,heg_jianglue,bf_yongjin,luanwu",
	on_trigger = function(self, event, player, data, room)
		if event == sgs.GameStart or event == sgs.EventAcquireSkill and data:toString() == self:objectName() then
			for _, p in sgs.qlist(room:getAllPlayers()) do
...
if not sgs.Sanguosha:getSkill("heg_jiaping_Attach") then skills:append(heg_jiaping_Attach) end
	...
	on_use = function(self, room, source, targets)
		...
		for _, t in ipairs(targets) do
			if t:hasFlag("EXCard_YYDL_effected") and t:canDiscard(t, "he") then
				...
				room:askForDiscard(t, self:objectName(), num, num, false, true, "#heg_await_exhausted-discard", ".")
			end
		end
...
heg_threaten_emperor = sgs.CreateTrickCard{
	...
	class_name = "heg_threaten_emperor",
	...
	available = function(self, player)
		-- 只有大势力角色才能使用
		if not IsBigKingdomPlayerClient(player) and not table.contains(self:getSkillNames(), "heg_fengying") then
			return false
		end
		...
		if player:getPhase() == sgs.Player_Discard then
			...
			if not player:isKongcheng() then
				if room:askForDiscard(player, "heg_threaten_emperor", 1, 1, true, false, "@heg_threaten_emperor") then
					-- 设置标记用于在回合结束时授予额外回合
					room:setTag("heg_threaten_emperor_extra_turn", ToData(player))
				end
			end
		end
		return false
...
heg_burning_camps = sgs.CreateTrickCard{
	...
	filter = function(self, targets, to_select, player)
		...
		if player:isProhibited(to_select, self) then return false end
		
		-- 判断是否是下家：下家的座位号应该是玩家座位号+1（考虑循环）
		local player_seat = player:getSeat()
		local to_seat = to_select:getSeat()
		local alive_count = player:aliveCount()
		
		-- 计算下家座位号（循环处理）
		local next_seat = (player_seat % alive_count) + 1
		
		return to_seat == next_seat
	end,
	available = function(self, player)
		return self:cardIsAvailable(player)
	end,
			...
			if not use.to:isEmpty() then
				...
				for _, p in sgs.qlist(room:getAlivePlayers()) do
					if is_big then
						...
						if not IsBigKingdomPlayer(p) and HasBigKingdomPlayer(room) and not use.from:isProhibited(p, self) then
							use.to:append(p)
						end
					end
				end
...
heg_halberd_Card = sgs.CreateSkillCard{
	...
	filter = function(self, targets, to_select, player)
		...
		for _, p in sgs.qlist(use.to) do
			...
			if p_kingdom == target_kingdom then
				return false
			end
		end
		
        return use.card and use.card:targetFilter(qtargets, to_select, player) and not player:isProhibited(to_select, use.card, qtargets)
	end,
	on_use = function(self, room, source, targets)
		for _, target in ipairs(targets) do
			room:setPlayerFlag(target, "heg_halberd_target")
		end
	end
}
heg_halberdVS = sgs.CreateZeroCardViewAsSkill{
	name = "heg_halberd",
	response_pattern = "@@heg_halberd",
	view_as = function(self)
		return heg_halberd_Card:clone()
	end
}

heg_halberd_Skill = sgs.CreateTriggerSkill{
	name = "heg_halberd_Skill",
	view_as_skill = heg_halberdVS,
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.TargetSpecified then
			...
			if use.card and use.card:isKindOf("Slash") and player:hasWeapon("heg_halberd") then
				player:setTag("heg_halberd_use", data)
				room:askForUseCard(player, "@@heg_halberd", "@heg_halberd")
				player:removeTag("heg_halberd_use")
				for _, p in sgs.qlist(room:getAllPlayers()) do
					if p:hasFlag("heg_halberd_target") then
						use.to:append(p)
						room:setPlayerFlag(p, "-heg_halberd_target")
					end
				end
				room:sortByActionOrder(use.to)
				data:setValue(use)
...
heg_iron_armor_Skill = sgs.CreateTriggerSkill{
	...
	events = {sgs.TargetConfirming, sgs.ChainStateChange},
	frequency = sgs.Skill_Compulsory,
	can_trigger = function(self, target)
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.TargetConfirming then
			...
			if use.card then
				...
				if use.card:isKindOf("FireAttack") or use.card:isKindOf("heg_burning_camps") or
		...
		elseif event == sgs.ChainStateChange then
			...
			if not IsBigKingdomPlayer(player) and HasBigKingdomPlayer(room) then
				return true
			end
		end
		return false
...
sgs.LoadTranslationTable{
	...
	["#heg_await_exhausted-discard"] = "以逸待劳：请弃置两张牌",
	...
	["@heg_threaten_emperor"] = "受到【挟天子以令诸侯】影响，你可以弃置一张手牌，获得一个额外的回合",
	...
	["@heg_zhengbi-receive"] = "征辟：交给%src一张非基本牌或两张基本牌",
	["heg_zhengbi_distance"] = "选择一名角色，直至此回合结束，你对其使用牌无距离与次数限制",
	["heg_zhengbi_give"] = "将一张基本牌交给一名角色，然后其交给你一张非基本牌或两张基本牌",
	["heg_zhengbi-distance"] = "你可以发动“征辟”<br/> <b>操作提示</b>: 选择一名其他角色→点击确定<br/>",
	["@heg_zhengbi-give"] = "征辟：你可以将一张基本牌交给一名角色，然后其交给你一张非基本牌或两张基本牌",
	["heg_zhengbi"] = "征辟",
	[":heg_zhengbi"] = "出牌阶段开始时，你可选择一项：1.选择一名角色，直至此回合结束，你对其使用牌无距离与次数限制；2.将一张基本牌交给一名角色，然后其交给你一张非基本牌或两张基本牌。",
	["@heg_fengying"] = "奉迎：你可以令任意名角色将手牌补至其体力上限",
	["heg_fengying"] = "奉迎",
	...
	["heg_yingshi-invoke"] = "你可以发动“鹰视” <br/> <b>操作提示</b>: 选择一名角色→点击确定<br/>",
	["@heg_yingshi"] = "鹰视：你可以令%src视为对另一名角色使用一张【知己知彼】",
	["heg_yingshi_target"] = "鹰视",
	["heg_yingshi"] = "鹰视",
  	[":heg_yingshi"] = "出牌阶段开始时，你可以令一名角色视为对你指定的另一名角色使用一张【知己知彼】，然后若使用者不为你，你摸一张牌。",
	["heg_shunfu-slash"] = "瞬覆：你可以使用一张【杀】（无距离限制且不可被响应）",
	["heg_shunfu"] = "瞬覆",
	...
	["heg_yimie_attach"] = "夷灭",
	[":heg_yimie_attach"] = "司马师的回合内，与处于濒死状态的角色势力不同的角色可将一张<font color='red'>♥</font>手牌当【桃】对处于濒死状态的角色使用。",
	["heg_ruilve_attach"] = "睿略",
	[":heg_ruilve_attach"] = "出牌阶段限一次，你可展示并交给司马师一张伤害类牌，然后你摸一张牌。",
	["heg_ruilve"] = "睿略",
	...
	[":&heg_beiluan+to"] = "非装备手牌视为【杀】（由%dest标记）",
	["heg_beiluan"] = "备乱",
	...
	["heg_pojing:obtain"] = "令%src获得你区域内的一张牌",
	["heg_pojing:damage"] = "所有其他角色可以对你造成1点伤害",
	["heg_pojing"] = "迫境",
  	[":heg_pojing"] = "出牌阶段限一次，你可以令一名其他角色选择一项：1.令你获得其区域内的一张牌；2.<font color=\"green\"><b>每名角色限一次，</b></font>所有其他角色可以对其造成1点伤害。",
	...
	["@heg_gongzhi"] = "共执：你可以跳过摸牌阶段，令任意角色依次摸一张牌，直到共计摸四张牌",
	["heg_gongzhi"] = "共执",
  	[":heg_gongzhi"] = "你可以跳过摸牌阶段，令任意角色依次摸一张牌，直到共计摸四张牌。",
	["heg_shejus"] = "慑惧",
  	[":heg_shejus"] = "<font color=\"green\"><b>每名角色限一次，</b></font>当其他角色受到伤害后，其可以令你回复1点体力，然后你弃置所有手牌。",
	...
	["@heg_yanxi"] = "宴戏：你可以选择至多三名其他势力的角色各一张手牌",
	["heg_shiren"] = "识人",
  	[":heg_shiren"] = "每回合限一次，当一名其他角色受到伤害后，你可以交给其两张牌并摸两张牌。",
	["@heg_shiren-give"] = "识人：请交给 %src 两张牌",
	...
	["heg_chengxi"] = "乘隙",
  	[":heg_chengxi"] = "准备阶段，你可以令一名角色视为使用一张你指定目标的【以逸待劳】，结算结束后，若因此【以逸待劳】弃置的牌中包含非基本牌，此【以逸待劳】的使用者对目标各造成1点伤害。",
	["@heg_chengxi"] = "乘隙：你可以令一名角色视为使用【%arg】",
	["@heg_chengxi_target"] = "乘隙：请选择【%arg】的目标角色",
	["heg_jiantong:exchange"] = "你可以发动“监统”，用装备区内的一张牌交换其中至多两张牌",
	["heg_jiantong"] = "监统",
  	[":heg_jiantong"] = "当你受到伤害后，你可以观看一名角色的手牌，然后你可以用装备区内的一张牌交换其中至多两张牌。",
	["@heg_jiantong"] = "监统：你可以观看一名角色的手牌，然后用装备区的一张牌交换其中至多两张手牌",
	["~heg_jiantong"] = "选择装备区的一张牌→选择对方至多两张手牌→点击确定",
	...
	["@heg_zhefu"] = "哲妇：你可以观看一名手牌数不小于你的角色的手牌",
	["heg_zhefu"] = "哲妇",
  	[":heg_zhefu"] = "当你于回合外使用或打出基本牌后，你可以观看一名手牌数不小于你的角色的手牌，然后你可以弃置其中一张基本牌。",
	["@heg_yidu"] = "遗毒：你可以展示一名未受到此牌伤害的目标角色至多两张手牌",
	["heg_yidu"] = "遗毒",
	...
	["@heg_xijue-tenyeartuxi"] = "袭爵：你可以弃置一张牌以发动“突袭”",
	["@heg_xijue-xiaoguo"] = "袭爵：你可以弃置一张牌以发动“骁果”",
	["heg_xijue"] = "袭爵",
  	[":heg_xijue"] = "你可以弃置一张牌以发动“突袭”或“骁果”。",
	[":&heg_lvxian"] = "上回合失去的牌数",
	["heg_lvxian"] = "履险",
  	[":heg_lvxian"] = "主将技，当你每回合首次受到伤害后，若执行上回合的角色不为你，你可以摸X张牌（X为你上回合失去的牌数）。",
	["@heg_yingwei"] = "盈威：你可以重铸至多两张牌",
	["heg_yingwei"] = "盈威",
	...
	[":&heg_duanqiu"] = "所有角色本回合内至多共计使用%src张手牌",
	["heg_duanqiu"] = "断虬",
	...
	["heg_huaiyuan:range"] = "攻击范围",
	["heg_huaiyuan:maxcard"] = "手牌上限",
	["heg_huaiyuan:slash"] = "使用【杀】的次数上限",
	-- [":&heg_huaiyuan+to"] = "本回合 %arg +1（由%dest标记）",
	["heg_huaiyuan"] = "怀远",
	...
	["@heg_neiji-show"] = "内忌：展示两张手牌",
	["heg_neiji-invoke"] = "你可以发动“内忌” <br/> <b>操作提示</b>: 选择一名其他角色→点击确定<br/>",
	["heg_neiji"] = "内忌",
	...
	["@heg_limeng"] = "离梦：你可以弃置一张非基本牌",
	["@heg_limeng-target"] = "离梦：选择场上两名角色分别对另一名角色造成1点伤害",
	["heg_limeng"] = "离梦",
	...
	["heg_sanchen"] = "三陈",
  	[":heg_sanchen"] = "<font color=\"green\"><b>出牌阶段每名角色限一次，</b></font>你可令一名角色摸三张牌然后弃置三张牌，若其因此弃置了类别相同的牌，此技能这阶段失效。",
	["heg_pozhu"] = "破竹",
  	[":heg_pozhu"] = "准备阶段，你可以将一张牌当【杀】使用，结算后你展示唯一目标一张手牌，若两张牌花色不同，你可以重复此流程。",
	["@heg_pozhu-slash"] = "破竹：你可以将一张牌当【杀】使用",
	...
	["heg_jiaping_Attach"] = "嘉平",
	[":heg_jiaping_Attach"] = "每轮限一次，你可以移除副将的武将牌并发动以下一个未以此法发动过的技能：“瞬覆”，“奉迎”，“将略”，“勇进”和“乱武”。",
	["heg_jiaping"] = "嘉平",
  	[":heg_jiaping"] = "<b><font color='goldenrod'>君主技</font></b>，你拥有“八荒死士令”。<br>#<b>八荒死士令</b>：每轮限一次，<font color=#CC33CC><b>晋势力</b></font>角色可以移除其副将的武将牌并发动以下一个未以此法发动过的技能：“瞬覆”，“奉迎”，“将略”，“勇进”和“乱武”。",
	["heg_guikuang"] = "诡诳",
	...
	["heg_mobile_luxun"] = "陆逊-国[手杀]",
    ["&heg_mobile_luxun"] = "陆逊",
    ["#heg_mobile_luxun"] = "擎天之柱",
    ["~heg_mobile_luxun"] = "",
    ["designer:heg_mobile_luxun"] = "",
    ["cv:heg_mobile_luxun"] = "",
    ["illustrator:heg_mobile_luxun"] = "",
	["heg_mobile_qianxun"] = "谦逊",
	[":heg_mobile_qianxun"] = "锁定技，当你成为其他角色使用锦囊牌的唯一目标时，若“节”的数量小于3，你取消之并将此牌置于武将牌上，称为“节”。",
	["heg_mobile_jie"] = "节",
	["heg_mobile_duoshi"] = "度势",
	[":heg_mobile_duoshi"] = "出牌阶段限一次，你可以选择一项：1.将一张红色手牌当【以逸待劳】使用；2.移去三张“节”，视为使用一张火属性伤害牌。",
	["@heg_mobile_duoshi"] = "度势：你可以移去三张“节”，视为使用一张火【杀】",
	["~heg_mobile_duoshi"] = "选择一名角色→点击确定",

	["heg_mobile_huangzhong"] = "黄忠-国[手杀]",
    ["&heg_mobile_huangzhong"] = "黄忠",
    ["#heg_mobile_huangzhong"] = "老当益壮",
    ["~heg_mobile_huangzhong"] = "",
    ["designer:heg_mobile_huangzhong"] = "",
    ["cv:heg_mobile_huangzhong"] = "",
    ["illustrator:heg_mobile_huangzhong"] = "",
	["heg_mobile_liegong"] = "烈弓",
	[":heg_mobile_liegong"] = "当你于出牌阶段内使用【杀】指定一名角色为目标后，若其手牌数不小于你的体力值或不大于你的攻击范围，你可以令其不能使用【闪】响应此【杀】。乘势：此【杀】对其造成的伤害+1。",
	
	["heg_mobile_ganfuren"] = "甘夫人-国[手杀]",
    ["&heg_mobile_ganfuren"] = "甘夫人",
    ["#heg_mobile_ganfuren"] = "昭烈皇后",
    ["~heg_mobile_ganfuren"] = "",
    ["designer:heg_mobile_ganfuren"] = "",
    ["cv:heg_mobile_ganfuren"] = "",
    ["illustrator:heg_mobile_ganfuren"] = "",
	["heg_mobile_shushen"] = "淑慎",
	[":heg_mobile_shushen"] = "当你回复1点体力或一次性失去大于X张牌后（X为你的体力值），你可以令一名其他角色摸一张牌。",

	["heg_mobile_tianfeng"] = "田丰-国[手杀]",
    ["&heg_mobile_tianfeng"] = "田丰",
    ["#heg_mobile_tianfeng"] = "河北瑰杰",
    ["~heg_mobile_tianfeng"] = "",
    ["designer:heg_mobile_tianfeng"] = "",
    ["cv:heg_mobile_tianfeng"] = "",
    ["illustrator:heg_mobile_tianfeng"] = "",
	["heg_mobile_suishi:discardhandcard"] = "弃置所有手牌",
	["heg_mobile_suishi:losehp"] = "失去1点体力",
	["heg_mobile_suishi:discard"] = "你可以发动“随势”，令%src选择一项：1.失去1点体力；2.弃置所有手牌",
	["heg_mobile_suishi:draw"] = "你可以发动“随势”，令%src摸一张牌",
	["heg_mobile_suishi"] = "随势",
	[":heg_mobile_suishi"] = "当其他角色进入濒死状态时，伤害来源可以令你摸一张牌；当其他角色死亡时，伤害来源可以令你选择一项：1.失去1点体力；2.弃置所有手牌。",

	["heg_mobile_panfeng"] = "潘凤-国[手杀]",
    ["&heg_mobile_panfeng"] = "潘凤",
    ["#heg_mobile_panfeng"] = "联军上将",
    ["~heg_mobile_panfeng"] = "",
    ["designer:heg_mobile_panfeng"] = "",
    ["cv:heg_mobile_panfeng"] = "",
    ["illustrator:heg_mobile_panfeng"] = "",
	["heg_mobile_kuangfu:obtain"] = "获得其一张牌",
	["heg_mobile_kuangfu:discard"] = "弃置其一张牌",
	["heg_mobile_kuangfu"] = "狂斧",
	[":heg_mobile_kuangfu"] = "当你使用【杀】对目标角色造成伤害后，你可以弃置或获得其一张牌。",

	["heg_mobile_xiahoudun"] = "夏侯惇-国[手杀]",
    ["&heg_mobile_xiahoudun"] = "夏侯惇",
    ["#heg_mobile_xiahoudun"] = "独目的苍狼",
    ["~heg_mobile_xiahoudun"] = "",
    ["designer:heg_mobile_xiahoudun"] = "",
    ["cv:heg_mobile_xiahoudun"] = "",
    ["illustrator:heg_mobile_xiahoudun"] = "",
	...
	["heg_ov_zhugejin"] = "诸葛瑾-国[海外]",
    ["&heg_ov_zhugejin"] = "诸葛瑾",
    ["#heg_ov_zhugejin"] = "联盟的维系者",
    ["~heg_ov_zhugejin"] = "",
    ["designer:heg_ov_zhugejin"] = "",
    ["cv:heg_ov_zhugejin"] = "",
    ["illustrator:heg_ov_zhugejin"] = "",
	["@heg_ov_hongyuan"] = "弘援：你可以改为令另一名角色摸牌",
	["heg_ov_hongyuan"] = "弘援",
	[":heg_ov_hongyuan"] = "当你因合纵摸牌时，你可以改为令另一名角色摸牌。出牌阶段限一次，你可以令一张无合纵标记的手牌于本回合视为有合纵标记。",
...
heg_fofl_yangzhi = sgs.General(extension_fakeoffline,  "heg_fofl_yangzhi", "jin", 3, false)

heg_fofl_maihuo = sgs.CreateTriggerSkill {
	name = "heg_fofl_maihuo",
	events = {sgs.TargetConfirmed, sgs.EventPhaseStart, sgs.DamageCaused},
	on_trigger = function(self, event, player, data, room)
		if event == sgs.TargetConfirmed then
			local use = data:toCardUse()
			if not use.card or not use.card:isKindOf("Slash") then return false end
			if not use.to:contains(player) then return false end
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if p:getMark("heg_fofl_maihuo"..player:objectName().."-Clear") == 0 and p:askForSkillInvoke(self:objectName(), data) then
					room:notifySkillInvoked(p, self:objectName())
					room:addPlayerMark(p, "heg_fofl_maihuo"..player:objectName().."-Clear")
					local nullified_list = use.nullified_list
					table.insert(nullified_list, player:objectName())
					use.nullified_list = nullified_list
					data:setValue(use)
					room:addPlayerMark(p, "&heg_fofl_maihuo")
					break
				end
			end
		elseif event == sgs.EventPhaseStart then
			if player:getPhase() == sgs.Player_Play then
				...
				slash:deleteLater()
				for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					if p:getMark("&heg_fofl_maihuo") > 0 then
						if player:canSlash(p, slash, true) and player:inMyAttackRange(p) and player:askForSkillInvoke(self:objectName(), data) then
							room:notifySkillInvoked(p, self:objectName())
							for i = 1, p:getMark("&heg_fofl_maihuo") do
								local use = sgs.CardUseStruct()
								use.card = slash
								use.from = player
								use.to:append(p)
								room:useCard(use)
								if not p:isAlive() or not player:isAlive() then break end
							end
						end
					end
				end
		...
		elseif event == sgs.DamageCaused then
			local damage = data:toDamage()
			if damage.card and damage.card:isKindOf("Slash") and table.contains(damage.card:getSkillNames(), self:objectName()) and damage.to:hasSkill(self:objectName()) then
				if damage.to and damage.to:getGeneral2() then
					local log = sgs.LogMessage()
					log.type = "#YHXiandao2"
					log.arg = damage.to:getGeneral2Name()
					log.from = damage.to
					room:sendLog(log)
					room:changeHero(damage.to, "", false, false, true, false)
					damage.prevented = true
					data:setValue(damage)
					return true
				end
...
heg_fofl_yangzhi:addSkill("nosjinwanyi")
heg_fofl_yangzhi:addSkill(heg_fofl_maihuo)

heg_fofl_yangyan = sgs.General(extension_fakeoffline,  "heg_fofl_yangyan", "jin", 3, false)

heg_fofl_xuanbeiCard = sgs.CreateSkillCard{
	name = "heg_fofl_xuanbei",
	will_throw = false,
	target_fixed = true,
	on_use = function(self, room, source, targets)
		source:drawCards(2, "heg_fofl_xuanbei")
		room:addPlayerMark(source, "&heg_fofl_xuanbei-Clear")
		room:addPlayerMark(source, "YingBianDirectlyEffective")
	end,
}
heg_fofl_xuanbeiVS = sgs.CreateZeroCardViewAsSkill{
	name = "heg_fofl_xuanbei",
	view_as = function()
		return heg_fofl_xuanbeiCard:clone()
	end,
	enabled_at_play = function(self, player)
		return player:getMark("@heg_fofl_xuanbei") > 0
	end,
}
heg_fofl_xuanbei = sgs.CreateTriggerSkill {
	name = "heg_fofl_xuanbei",
	events = {sgs.CardFinished, sgs.Death, sgs.EventPhaseChanging},
	view_as_skill = heg_fofl_xuanbeiVS,
	frequency = sgs.Skill_Limited,
	limit_mark = "@heg_fofl_xuanbei",
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardFinished then
			local use = data:toCardUse()
			if not use.card then return false end
			if player:getMark("&heg_fofl_xuanbei-Clear") == 0 then return false end
			if player:getMark("heg_fofl_xuanbei_target-Clear") == 0 then return false end
			if sgs.Sanguosha:getEngineCard(use.card:getEffectiveId()):property("YingBianEffects"):toString() ~= "" or (use.card:isVirtualCard() or use.card:getSkillName() ~= "" and player:getTag("YingBian_Effects_" .. use:card():toString()):toString() and player:getTag("YingBian_Effects_" .. use:card():toString()):toString() ~= "") then 
				...
				if not target then return false end
				player:peiyin(self)
				player:addMark("heg_fofl_xuanbei_target-Clear")
		...
		elseif event == sgs.Death then
			...
			if death.who:objectName() == player:objectName() then
				...
				if targets:isEmpty() then return false end
				local target = room:askForPlayerChosen(player,targets,self:objectName(),"@heg_fofl_xuanbei-change",true,true)
				if target then
					player:peiyin(self)
					ChangeGeneral(room, target, target:getGeneral2Name())
				end
			end
		end
	end,
...
heg_fofl_yangyan:addSkill(heg_fofl_xuanbei)
heg_fofl_yangyan:addSkill("jinxianwan")

heg_fofl_simazhou = sgs.General(extension_fakeoffline,  "heg_fofl_simazhou", "jin", 4)

-- 才望: 可以助战自己，助战后根据弃牌与强化牌颜色触发效果
heg_fofl_caiwang_buff = sgs.CreateTriggerSkill{
	name = "#heg_fofl_caiwang_buff",
	events = {sgs.EventAcquireSkill, sgs.GameStart, sgs.EventLoseSkill},
	on_trigger = function(self, event, player, data, room)
		if ((event == sgs.EventAcquireSkill and data:toString() == "heg_fofl_caiwang") or event == sgs.GameStart) then
			-- 添加可以助战自己的标记
			room:setPlayerMark(player, "YingBian_Ext_zhuzhan_self", 1)
		elseif event == sgs.EventLoseSkill and data:toString() == "heg_fofl_caiwang" then
			-- 移除可以助战自己的标记
			room:setPlayerMark(player, "YingBian_Ext_zhuzhan_self", 0)
		end
		return false
	end,
}

heg_fofl_caiwang_record = sgs.CreateTriggerSkill{
	name = "#heg_fofl_caiwang_record",
	events = {sgs.ChoiceMade},
	on_trigger = function(self, event, player, data, room)
		local str = data:toString():split(":")
		if #str < 3 then return false end
		
		-- 检查是否是应变助战弃牌: cardDiscard:yb_zhuzhan1:cards
		if str[1] == "cardDiscard" and (str[2] == "yb_zhuzhan1" or str[2] == "yb_zhuzhan2") then
			-- 解析弃置的牌
			local card_str = str[3]
			if card_str and card_str ~= "" then
				local card = sgs.Card_Parse(card_str)
				if card then
					local use = player:getTag("yb_zhuzhan2_data"):toCardUse()
					if use then
						player:setTag("heg_fofl_caiwang_zhuzhan_discard"..use.card:toString(), sgs.QVariant(card_str))
					end
				end
			end
		end
...
heg_fofl_caiwang = sgs.CreateTriggerSkill{
	name = "heg_fofl_caiwang",
	...
	on_trigger = function(self, event, player, data, room)
		...
		if event == sgs.CardFinished then
			...
			-- 检查是否有保存的助战弃牌信息
			local discard_str = player:getTag("heg_fofl_caiwang_zhuzhan_discard"..use.card:toString()):toString()
			if discard_str == "" then return false end
			
			-- 清除标记
			player:removeTag("heg_fofl_caiwang_zhuzhan_discard"..use.card:toString())
			
			-- 解析弃置的牌
			local discard_card = sgs.Card_Parse(discard_str)
			if not discard_card then return false end
			
			-- 获取强化牌的颜色
			local enhanced_color = use.card:isRed()
			
			-- 获取弃置牌的颜色（取第一张子卡）
			local discarded_color = nil
			if discard_card:subcardsLength() > 0 then
				local first_id = discard_card:getSubcards():first()
				local first_card = sgs.Sanguosha:getCard(first_id)
				if first_card then
					discarded_color = first_card:isRed()
				end
			end
			
			if discarded_color == nil then return false end
			
			-- 根据颜色是否相同触发不同效果
			if discarded_color ~= enhanced_color then
				-- 颜色不同：摸一张牌（每回合限一次）
				if player:getMark(self:objectName() .. "_draw-Clear") == 0 then
					if room:askForSkillInvoke(player, self:objectName(), data) then
						room:broadcastSkillInvoke(self:objectName())
						room:addPlayerMark(player, self:objectName() .. "_draw-Clear")
						player:drawCards(1, self:objectName())
					end
				end
			else
				-- 颜色相同：弃置其他角色一张牌（每回合限一次）
				if player:getMark(self:objectName() .. "_discard-Clear") == 0 then
					if room:askForSkillInvoke(player, self:objectName(), data) then
						room:broadcastSkillInvoke(self:objectName())
						room:addPlayerMark(player, self:objectName() .. "_discard-Clear")
						local targets = sgs.SPlayerList()
						for _, p in sgs.qlist(room:getOtherPlayers(player)) do
							if player:canDiscard(p, "he") then
								targets:append(p)
						...
						if not targets:isEmpty() then
							local target = room:askForPlayerChosen(player, targets, self:objectName(), "@heg_fofl_caiwang-discard", false, true)
							if target then
		...
		elseif event == sgs.PreCardUsed then
			...
			if string.startsWith(yingbian, "yb_zhuzhan") then
				use.card:setSkillName(self:objectName())
				setYingBianEffectsForCard(player, room, use.card, use.card:getSubcards())
				data:setValue(use)
			end
		end
		return false
...
heg_fofl_naxiang_buff = sgs.CreateTriggerSkill{
	name = "#heg_fofl_naxiang_buff",
	events = {sgs.PreCardUsed},
	on_trigger = function(self, event, player, data, room)
		if event == sgs.PreCardUsed then
			local yingbian = sgs.Sanguosha:getEngineCard(use.card:getEffectiveId()):property("YingBianEffects"):toString()
...
-- 纳降: 造成/受到伤害后发起军令，若不执行则获得助战增强
heg_fofl_naxiang = sgs.CreateTriggerSkill{
	name = "heg_fofl_naxiang",
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseStart then
			-- 清除标记
			if player:getPhase() == sgs.Player_RoundStart then
				if player:getMark("YingBian_Ext_all_as_zhuzhan") > 0 then
					room:setPlayerMark(player, "YingBian_Ext_all_as_zhuzhan", 0)
				end
				if player:getMark("YingBian_Ext_zhuzhan_no_type") > 0 then
					room:setPlayerMark(player, "YingBian_Ext_zhuzhan_no_type", 0)
				end
			end
		elseif event == sgs.Damage or event == sgs.Damaged then
			...
			if not other or other:objectName() == player:objectName() then return false end
			if not other:isAlive() then return false end
			
			room:sendCompulsoryTriggerLog(player, self:objectName())
			room:broadcastSkillInvoke(self:objectName())
			
			local invoked = askCommandTo(player, other, self:objectName(), false)
			
			if not invoked then
				-- 军令未执行，获得增强效果直到下回合开始
				-- 1. 助战无类别限制
				room:setPlayerMark(player, "YingBian_Ext_zhuzhan_no_type", 1)
				-- 2. 带强化效果的牌条件均视为助战（all_as_zhuzhan）
				room:setPlayerMark(player, "YingBian_Ext_all_as_zhuzhan", 1)
				room:setPlayerMark(player, "&heg_fofl_naxiang".."-Self"..sgs.Player_RoundStart.."Clear", 1)
...
heg_fofl_simazhou:addSkill(heg_fofl_caiwang)
heg_fofl_simazhou:addSkill(heg_fofl_caiwang_buff)
heg_fofl_simazhou:addSkill(heg_fofl_caiwang_record)
extension_fakeoffline:insertRelatedSkills("heg_fofl_caiwang", "#heg_fofl_caiwang_buff")
extension_fakeoffline:insertRelatedSkills("heg_fofl_caiwang", "#heg_fofl_caiwang_record")
heg_fofl_simazhou:addSkill(heg_fofl_naxiang)
heg_fofl_simazhou:addSkill(heg_fofl_naxiang_buff)
extension_fakeoffline:insertRelatedSkills("heg_fofl_naxiang", "#heg_fofl_naxiang_buff")

heg_fofl_jinsimayi = sgs.General(extension_fakeoffline,  "heg_fofl_jinsimayi", "jin", 4)

heg_fofl_quanbian_Clear = sgs.CreateTriggerSkill{
	name = "#heg_fofl_quanbian_Clear",
	events = {sgs.EventPhaseChanging},
	on_trigger = function(self, event, player, data, room)
		local change = data:toPhaseChange()
		if change.to == sgs.Player_NotActive then
			room:setPlayerProperty(player, "heg_fofl_quanbianRecords", sgs.QVariant())
		end
	end
}

heg_fofl_quanbian = sgs.CreateTriggerSkill{
	name = "heg_fofl_quanbian",
	events = {sgs.CardUsed, sgs.CardResponded},
	on_trigger = function(self, event, player, data, room)
		if player:getMark("&heg_fofl_quanbian-Clear") > 0 then
			return false
		end
		local card
		if event == sgs.CardUsed then
			if data:toCardUse().m_isHandcard then
				card = data:toCardUse().card
			end
		else
			if data:toCardResponse().m_isHandcard then
				card = data:toCardResponse().m_card
			end
		end
		if card and not card:isKindOf("SkillCard") and player:getPhase() == sgs.Player_Play then
			local record = player:property("heg_fofl_quanbianRecords"):toString()
			...
			if records and table.contains(records, suit) then
				...
				if room:askForSkillInvoke(player, self:objectName(), data) then
					...
					room:fillAG(ids)
					local to_obtain = room:askForAG(player, ids, false, self:objectName())
					if to_obtain then
						room:obtainCard(player, id, false)
						room:showCard(player, to_obtain)
						if card:getSuit() == sgs.Sanguosha:getCard(to_obtain):getSuit() then
							room:addPlayerMark(player, "&heg_fofl_quanbian-Clear")
						end
					end
					room:clearAG()
				end
			end
			room:setPlayerProperty(player, "heg_fofl_quanbianRecords", sgs.QVariant(table.concat(records, ",")))
			for _, mark in sgs.list(player:getMarkNames()) do
				if (string.startsWith(mark, "&heg_fofl_quanbian+#record") and player:getMark(mark) > 0) then
					room:setPlayerMark(player, mark, 0)
				end
			end
			local mark = "&heg_fofl_quanbian+#record"
...
heg_fofl_xiongzhi_buff = sgs.CreateTargetModSkill{
	name = "#heg_fofl_xiongzhi_buff",
	distance_limit_func = function(self, from, card)
		if card and table.contains(card:getSkillNames(), "heg_fofl_xiongzhi") then
			return 1000
		else
			return 0
		end
...
heg_fofl_xiongzhiCard = sgs.CreateSkillCard {
	name = "heg_fofl_xiongzhi",
	target_fixed = true,
	will_throw = false,
	on_use = function(self, room, source, targets)
		room:removePlayerMark(source, "@heg_fofl_xiongzhi")
		room:doSuperLightbox(source, "heg_fofl_xiongzhi")
		local list = room:getNCards(source:getMaxHp(), false)
		local log = sgs.LogMessage()
		...
		while source:isAlive() and not list:isEmpty() do
			...
			if not list:isEmpty() then
				room:notifyMoveToPile(source, list, "heg_fofl_xiongzhi", sgs.Player_DrawPile, true)
				local use_card = room:askForUseCard(source, "@@heg_fofl_xiongzhi!", "@heg_fofl_xiongzhi:"..card:objectName())
				room:notifyMoveToPile(source, list, "heg_fofl_xiongzhi", sgs.Player_DrawPile, false)
...
heg_fofl_xiongzhi = sgs.CreateViewAsSkill {
	name = "heg_fofl_xiongzhi",
	n = 1,
	view_filter = function(self, selected, to_select)
		if sgs.Sanguosha:getCurrentCardUsePattern() == "@@heg_fofl_xiongzhi!" then
			return sgs.Self:getPile("#heg_fofl_xiongzhi"):contains(to_select)
		end
		return false
	end,
	view_as = function(self, cards)
		if sgs.Sanguosha:getCurrentCardUsePattern() == "@@heg_fofl_xiongzhi!" then
			if #cards == 1 then
				local dc = sgs.Sanguosha:getCard(cards[1]:getEffectiveId())
				dc:setSkillName("heg_fofl_xiongzhi")
				return dc
			end
		else
			local skillcard = heg_fofl_xiongzhiCard:clone()
			return skillcard
		end
		return nil
	end,
	enabled_at_play = function(self, player)
		return player:getMark("@heg_fofl_xiongzhi") > 0
	end,
	enabled_at_response = function(self, player, pattern)
		return pattern == "@@heg_fofl_xiongzhi!"
	end
}


heg_fofl_jinsimayi:addSkill("jinyingshi")
heg_fofl_jinsimayi:addSkill(heg_fofl_quanbian)
heg_fofl_jinsimayi:addSkill(heg_fofl_quanbian_Clear)
extension_fakeoffline:insertRelatedSkills("heg_fofl_quanbian", "#heg_fofl_quanbian_Clear")
heg_fofl_jinsimayi:addSkill(heg_fofl_xiongzhi)
heg_fofl_jinsimayi:addSkill(heg_fofl_xiongzhi_buff)
extension_fakeoffline:insertRelatedSkills("heg_fofl_xiongzhi", "#heg_fofl_xiongzhi_buff")

heg_fofl_yanghuiyu = sgs.General(extension_fakeoffline,  "heg_fofl_yanghuiyu", "jin", 3, false)

heg_fofl_caiyuan = sgs.CreateTriggerSkill {
	name = "heg_fofl_caiyuan",
	events = {sgs.EventPhaseStart, sgs.EventPhaseChanging},
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseChanging then
			local change = data:toPhaseChange()
			if change.to == sgs.Player_NotActive then
				local handnum_start = room:getPlayerMark(player, "heg_fofl_caiyuan-Clear")
				if player:getHandcardNum() >= handnum_start then
		...
		elseif event == sgs.EventPhaseStart then
			if player:getPhase() == sgs.Player_RoundStart then
				room:setPlayerMark(player, "heg_fofl_caiyuan-Clear", player:getHandcardNum())
				room:setPlayerMark(player, "&heg_fofl_caiyuan-Clear", player:getHandcardNum())
			end
		end
	end
...
heg_fofl_yanghuiyu:addSkill("jinciwei")
heg_fofl_yanghuiyu:addSkill(heg_fofl_caiyuan)

heg_fofl_zuofen = sgs.General(extension_fakeoffline,  "heg_fofl_zuofen", "jin", 3, false)

heg_fofl_zhaosongCard = sgs.CreateSkillCard {
	name = "heg_fofl_zhaosong",
	target_fixed = false,
	will_throw = false,
	filter = function(self, targets, to_select)
	...
	on_use = function(self, room, source, targets)
		...
		room:showAllCards(target, source)
		-- 获得区域内一张牌
		local card_id = room:askForCardChosen(source, target, "he", "heg_fofl_zhaosong", false, sgs.Card_MethodNone)
		if card_id >= 0 then
			local card = sgs.Sanguosha:getCard(card_id)
			room:obtainCard(source, card, false)
			source:setSkillDescriptionSwap("heg_fofl_zhaosong","%arg2", "grey")
			room:changeTranslation(source, "heg_fofl_zhaosong", 2)
			room:addPlayerMark(source, "heg_fofl_zhaosong_obtain")
		end
	end,
}
heg_fofl_zhaosongVS = sgs.CreateZeroCardViewAsSkill{
	name = "heg_fofl_zhaosong",
	view_as = function()
		return heg_fofl_zhaosongCard:clone()
	end,
	enabled_at_play = function(self, player)
		return player:getMark("heg_fofl_zhaosong_obtain") == 0
	end,
}
heg_fofl_zhaosong = sgs.CreateTriggerSkill {
	name = "heg_fofl_zhaosong",
	events = {sgs.EnterDying, sgs.CardUsed},
	view_as_skill = heg_fofl_zhaosongVS,
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EnterDying then
			local dying = data:toDying()
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if p:getMark("heg_fofl_zhaosong_dying") == 0 then
					if room:askForSkillInvoke(p, self:objectName(), data) then
						room:broadcastSkillInvoke(self:objectName())
						local recover = sgs.RecoverStruct()
						recover.who = p
						...
						room:recover(player, recover, true)
						player:drawCards(1, self:objectName())
						p:setSkillDescriptionSwap("heg_fofl_zhaosong","%arg1", "grey")
						room:changeTranslation(p, "heg_fofl_zhaosong", 2)
						room:addPlayerMark(p, "heg_fofl_zhaosong_dying", 1)
					end
				end
			end
		elseif event == sgs.CardUsed then
			...
			if use.to:length() ~= 1 then return false end
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if p:getMark("heg_fofl_zhaosong_slash") == 0 then
					if room:askForSkillInvoke(p, self:objectName(), data) then
						local extra_targets = room:getCardTargets(player, use.card, use.to)
						if not extra_targets:isEmpty() then
							room:setTag("heg_fofl_zhaosong", data)
							local others = room:askForPlayersChosen(to, extra_targets, self:objectName(), 0, 2, "@heg_fofl_zhaosong", true, true)
							room:removeTag("heg_fofl_zhaosong")
							if others and others:length() > 0 then
								room:addPlayerMark(p, "heg_fofl_zhaosong_slash", 1)
...
heg_fofl_lisi = sgs.CreateTriggerSkill {
	name = "heg_fofl_lisi",
	events = {sgs.Death},
	on_trigger = function(self, event, player, data, room)
		local death = data:toDeath()
		if death.who:objectName() == player:objectName() then return false end
		local choices = {"dying", "slash", "obtain"}
		for _, choice in ipairs(choices) do
			if player:getMark("heg_fofl_zhaosong_" .. choice) == 0 then
				table.removeOne(choices, choice)
			end
		end
		if #choices > 0 and room:askForSkillInvoke(death.who, self:objectName(), ToData(player)) then
			local choice = room:askForChoice(death.who, self:objectName(), table.concat(choices, "+"), ToData(player))
			room:setPlayerMark(player, "heg_fofl_zhaosong_" .. choice, 0)
			player:setSkillDescriptionSwap("heg_fofl_zhaosong","%arg"..choices[choice], "blue")
			room:changeTranslation(player, "heg_fofl_zhaosong", 2)
		end
		return false
	end,
}
heg_fofl_zuofen:addSkill(heg_fofl_zhaosong)
heg_fofl_zuofen:addSkill(heg_fofl_lisi)

heg_fofl_zongyu = sgs.General(extension_fakeoffline,  "heg_fofl_zongyu", "shu", 3)

heg_fofl_chengshang = sgs.CreateTriggerSkill {
	name = "heg_fofl_chengshang",
	events = {sgs.CardFinished},
	on_trigger = function(self, event, player, data, room)
		local use = data:toCardUse()
		if not use.card then return false end
		...
		if use.card:hasFlag("DamageDone") then return false end
		if player:getMark("&heg_fofl_chengshang-PlayClear") > 0 then return false end
		if room:askForSkillInvoke(player, self:objectName(), data) then
			room:broadcastSkillInvoke(self:objectName())
			...
			for _, c in sgs.qlist(room:getDiscardPile()) do
				if c:getSuit() == suit and c:getNumber() == number then
					to_obtain:append(c:getEffectiveId())
				end
			end
			if to_obtain:isEmpty() then
				return false
			end
			room:addPlayerMark(player, "&heg_fofl_chengshang-PlayClear")
			room:obtainCards(player, to_obtain, false)
		end
		return false
	end,
}

heg_fofl_zongyu:addSkill("heg_qiao")
heg_fofl_zongyu:addSkill(heg_fofl_chengshang)

heg_fofl_chengong = sgs.General(extension_fakeoffline,  "heg_fofl_chengong", "wei+qun", 3)

heg_fofl_yinpanCard = sgs.CreateSkillCard {
	name = "heg_fofl_yinpan",
	target_fixed = false,
	will_throw = false,
	filter = function(self, targets, to_select)
		return #targets == 0 and to_select:objectName() ~= sgs.Self:objectName()
	...
	on_use = function(self, room, source, targets)
		...
		for _, p in sgs.qlist(room:getOtherPlayers(target)) do
			if p:canSlash(target, nil, false) and p:objectName() ~= source:objectName() then
				room:askForUseSlashTo(p, target, "@heg_fofl_yinpan-slash:" .. target:objectName(), false, false, false, source, nil, "heg_fofl_yinpan")
			end
		end
		if target:isAlive() and target:getMark(self:objectName().."-Clear") == 0 then
			local recover = sgs.RecoverStruct()
			recover.who = source
			recover.reason = self:objectName()
			room:recover(target, recover, true)
			room:setPlayerMark(target, self:objectName().."-Clear", 0)
		end
	end,
}
heg_fofl_yinpanVS = sgs.CreateZeroCardViewAsSkill{
	name = "heg_fofl_yinpan",
	view_as = function()
		return heg_fofl_yinpanCard:clone()
	end,
	enabled_at_play = function(self, player)
		return not player:hasUsed("#heg_fofl_yinpan")
	end,
}
heg_fofl_yinpan = sgs.CreateTriggerSkill {
	name = "heg_fofl_yinpan",
	events = {sgs.EventPhaseStart, sgs.Damaged},
	view_as_skill = heg_fofl_yinpanVS,
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseStart then
			if player:getPhase() == sgs.Player_RoundStart and player:getMark(self:objectName()) > 0 then
				room:addSlashCishu(player, player:getMark(self:objectName()), true)
				room:setPlayerMark(player, self:objectName(), 0)
			end
		elseif event == sgs.Damaged then
			local damage = data:toDamage()
			if damage.to:objectName() == player:objectName() and damage.card and damage.card:hasFlag("heg_fofl_yinpan") then
				room:setPlayerMark(player, self:objectName().."-Clear", 1)
...
heg_fofl_yumou = sgs.CreateTriggerSkill {
	name = "heg_fofl_yumou",
	events = {sgs.DrawNCards, sgs.BuryVictim},
	frequency = sgs.Skill_Compulsory,
	on_trigger = function(self, event, player, data, room)
		if event == sgs.BuryVictim then
			local death = data:toDeath()
			local killer = death.damage.from
			if (killer and killer:isAlive() and (killer:hasSkill("heg_fofl_yumou"))) or death.who:hasSkill("heg_fofl_yumou") then
				room:setTag("SkipNormalDeathProcess", sgs.QVariant(true))
			end
		elseif event == sgs.DrawNCards then
			local draw = data:toDraw()
			if draw.reason == "gamerule" then
				if player:hasSkill("heg_fofl_yumou") then
					room:sendCompulsoryTriggerLog(player, self:objectName())
					draw.num = 0
					data:setValue(draw)
				end
 ...
 heg_fofl_yumouClear = sgs.CreateTriggerSkill{
	name = "#heg_fofl_yumouClear",
	frequency = sgs.Skill_Compulsory,
	...
	on_trigger = function(self, event, player, data)
		...
		if reason then
			...
			if killer and killer:hasSkill("heg_fofl_yumou") then
				local room = player:getRoom()
...
heg_fofl_chengong:addSkill(heg_fofl_yinpan)
heg_fofl_chengong:addSkill(heg_fofl_yumou)
heg_fofl_chengong:addSkill(heg_fofl_yumouClear)
extension_fakeoffline:insertRelatedSkills("heg_fofl_yumou", "#heg_fofl_yumouClear")


heg_fofl_xugong = sgs.General(extension_fakeoffline,  "heg_fofl_xugong", "wei+qun", 3)


heg_fofl_biaozhaoCard = sgs.CreateSkillCard {
	name = "heg_fofl_biaozhao",
	target_fixed = false,
	will_throw = false,
	filter = function(self, targets, to_select)
		if #targets == 0 then
			...
			local knownboth = sgs.Sanguosha:cloneCard("heg_known_both", sgs.Card_NoSuit, 0)
			knownboth:setSkillName("heg_fofl_biaozhao")
			knownboth:deleteLater()
			return to_select:objectName() ~= sgs.Self:objectName() and to_select:objectName() ~= targets[1]:objectName() and (CanToCard(knownboth, sgs.Self, to_select) or CanToCard(knownboth, sgs.Self, targets[1]))
		end
	end,
	feasible = function(self, targets)
		return #targets == 2
	...
	on_use = function(self, room, source, targets)
		local victims = sgs.PlayerList()
		local knownboth = sgs.Sanguosha:cloneCard("heg_known_both", sgs.Card_NoSuit, 0)
		knownboth:setSkillName("heg_fofl_biaozhao")
		knownboth:deleteLater()
		for _, p in ipairs(targets) do
			if CanToCard(knownboth, source, p) then
				victims:append(p)
			end
		end
		local target = room:askForPlayerChosen(source, victims, "heg_fofl_biaozhao")
		if target then
			victims:removeOne(target)
			local use = sgs.CardUseStruct()
			use.from = source
			use.to:append(target)
			use.card = knownboth
		...
		local other = victims:first()
		if other and not source:isNude() then
			source:assignmentCards(source:getCards("he"), "@heg_fofl_biaozhao", victims, 1, 1, false)
			source:drawCards(1, self:objectName())
		end
	end
}

heg_fofl_biaozhao = sgs.CreateZeroCardViewAsSkill{
	name = "heg_fofl_biaozhao",
	view_as = function()
		return heg_fofl_biaozhaoCard:clone()
	end,
	enabled_at_play = function(self, player)
		return not player:hasUsed("#heg_fofl_biaozhao")
	end,
}

heg_fofl_yechou_limit = sgs.CreateCardLimitSkill{
	name = "#heg_fofl_yechou_limit",
	limit_list = function(self, player)
		return "use"
    end,
	limit_pattern = function(self,player)
		...
		for _,p in sgs.qlist(player:getSiblings())do
			if p:hasFlag("heg_fofl_yechou") and p:hasSkill("heg_fofl_yechou")
...
heg_fofl_yechou_buff = sgs.CreateTriggerSkill {
	name = "#heg_fofl_yechou_buff",
	events = {sgs.CardUsed, sgs.DamageCaused},
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardUsed then
			local use = data:toCardUse()
			if use.card and table.contains(use.card:getSkillNames(), "heg_fofl_yechou") then
				if use.card:hasFlag("heg_fofl_yechou_no_respond") then
					local no_respond_list = use.no_respond_list
					table.insert(no_respond_list, "_ALL_TARGETS")
					use.no_respond_list = no_respond_list
					data:setValue(use)
		...
		elseif event == sgs.DamageCaused then
			local damage = data:toDamage()
			if damage.card and table.contains(damage.card:getSkillNames(), "heg_fofl_yechou") then
				if damage.card:hasFlag("heg_fofl_yechou_damage") then
					damage.damage = damage.damage + 1
					local log = sgs.LogMessage()
					log.type = "#skill_add_damage"
					log.from = damage.from
				...
				if damage.to:isAlive() and damage.to:getHp() + damage.to:getHujia() <= damage.damage and damage.from then
					room:setPlayerFlag(damage.from, "heg_fofl_yechou")
				end
			end
		end
	end,
}
heg_fofl_yechou = sgs.CreateTriggerSkill {
	name = "heg_fofl_yechou",
	events = {sgs.Death},
	on_trigger = function(self, event, player, data, room)
		local death = data:toDeath()
		if death.who:objectName() ~= player:objectName() then return false end
		...
		local slash = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)
		slash:setSkillName("heg_fofl_yechou")
		slash:deleteLater()
		local target = death.damage.from
		if player:canSlash(target, slash, false) then
			room:setCardFlag(slash, "heg_fofl_yechou_no_respond")
			local use = sgs.CardUseStruct()
			use.from = player
			use.to:append(target)
			use.card = slash
		...
		room:setPlayerFlag(player, "-heg_fofl_yechou")
		local slash2 = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)
		slash2:setSkillName("heg_fofl_yechou")
		slash2:deleteLater()
		if target:isAlive() and player:canSlash(target, slash2, false) then
			room:setCardFlag(slash2, "SlashIgnoreArmor")
			local use = sgs.CardUseStruct()
			use.from = player
			use.to:append(target)
			use.card = slash2
		...
		room:setPlayerFlag(player, "-heg_fofl_yechou")
		local slash3 = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)
		slash3:setSkillName("heg_fofl_yechou")
		slash3:deleteLater()
		if target:isAlive() and player:canSlash(target, slash3, false) then
			room:setCardFlag(slash3, "heg_fofl_yechou_damage")
			local use = sgs.CardUseStruct()
			use.from = player
			use.to:append(target)
			use.card = slash3
...
heg_fofl_xugong:addSkill(heg_fofl_biaozhao)
heg_fofl_xugong:addSkill(heg_fofl_yechou_limit)
heg_fofl_xugong:addSkill(heg_fofl_yechou_buff)
heg_fofl_xugong:addSkill(heg_fofl_yechou)
extension_fakeoffline:insertRelatedSkills("heg_fofl_yechou", "#heg_fofl_yechou_limit")
extension_fakeoffline:insertRelatedSkills("heg_fofl_yechou", "#heg_fofl_yechou_buff")

heg_fofl_wuqiujian = sgs.General(extension_fakeoffline,  "heg_fofl_wuqiujian", "wei", 4)

heg_fofl_zhengrong = sgs.CreateTriggerSkill {
	name = "heg_fofl_zhengrong",
	...
	on_trigger = function(self, event, player, data, room)
		...
		if not target then return false end
		local same_kindom_exists = false
		for _, p in sgs.qlist(room:getOtherPlayers(target)) do
			if p:objectName() ~= player:objectName() and p:getKingdom() == target:getKingdom() then
				same_kindom_exists = true
				break
			end
		end
		if not same_kindom_exists then
			damage.damage = damage.damage + 1
			local log = sgs.LogMessage()
			log.type = "#skill_add_damage"
			log.from = damage.from
...
heg_fofl_hongjuCard = sgs.CreateSkillCard {
	name = "heg_fofl_hongju",
	filter = function(self, targets, to_select)
		return #targets == 0 and to_select:objectName() ~= sgs.Self:objectName()
	end,
	on_use = function(self, room, source, targets)
		local target = targets[1]
		for _, p in sgs.qlist(room:getOtherPlayers(target)) do
			local invoked = askCommandTo(source, p, "heg_fofl_hongju", p:objectName() == source:objectName())
			if not invoked then
				--TODO
			end
		end
...
heg_fofl_hongju = sgs.CreateZeroCardViewAsSkill{
	name = "heg_fofl_hongju",
	limit_mark = "@heg_fofl_hongju",
	view_as = function()
		return heg_fofl_hongjuCard:clone()
	end,
	enabled_at_play = function(self, player)
		return not player:hasUsed("#heg_fofl_hongju") and player:getMark("@heg_fofl_hongju") > 0
	end
}
heg_fofl_wuqiujian:addSkill(heg_fofl_zhengrong)
heg_fofl_wuqiujian:addSkill(heg_fofl_hongju)


heg_fofl_yuejin = sgs.General(extension_fakeoffline,  "heg_fofl_yuejin", "wei", 4)

heg_fofl_xiaoguo = sgs.CreateTriggerSkill {
	name = "heg_fofl_xiaoguo",
	events = {sgs.EventPhaseProceeding},
	on_trigger = function(self, event, player, data)
		if player:getPhase() == sgs.Player_Start then
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if p:objectName() ~= player:objectName() then
					local card = room:askForDiscard(p, self:objectName(), 999, 1, false, "@heg_fofl_xiaoguo", true, "BasicCard")
					if card then
						local equips = player:getEquips():length()
						local max = math.min(card:subcardsLength(), equips)
						if p:canDiscard(player, "e") then
							local remove = sgs.IntList()
							local to_obtain = dummyCard()
							for i = 1, max do--进行多次执行
								local id = room:askForCardChosen(p, player, "e", self:objectName(),
									false,--选择卡牌时手牌不可见
									sgs.Card_MethodDiscard,--设置为弃置类型
									remove,--将子卡表设置为不可选卡牌id表（保证每张卡只能被选择一次）
									i>1)--只有执行过一次选择才可取消
							...
							if to_obtain:subcardsLength() > 0 then
								room:throwCard(to_obtain, player, p)
							end
						end

						if card:subcardsLength() > equips then
							p:drawCards(1, self:objectName())
...
heg_fofl_yuejin:addSkill(heg_fofl_xiaoguo)

heg_fofl_dingfeng = sgs.General(extension_fakeoffline,  "heg_fofl_dingfeng", "wu", 4)

heg_fofl_dingfeng:addSkill("olduanbing")
heg_fofl_dingfeng:addSkill("fenxun")

heg_fofl_dianwei = sgs.General(extension_fakeoffline,  "heg_fofl_dianwei", "wei", 5)

heg_fofl_dianwei:addSkill("mobileqiangxi")

heg_fofl_luxun = sgs.General(extension_fakeoffline,  "heg_fofl_luxun", "wu", 3)

heg_fofl_duoshiCard = sgs.CreateSkillCard {
	name = "heg_fofl_duoshi",
	target_fixed = false,
	will_throw = true,
	filter = function(self, targets, to_select)
		local heg_await_exhausted = dummyCard("heg_await_exhausted")
		heg_await_exhausted:addSubcards(self:getSubcards())
		heg_await_exhausted:setSkillName("heg_fofl_duoshi")
		local qtargets = sgs.PlayerList()
		for _, p in ipairs(targets) do
			qtargets:append(p)
		end
		return heg_await_exhausted and heg_await_exhausted:targetFilter(qtargets, to_select, sgs.Self) and not sgs.Self:isProhibited(to_select, heg_await_exhausted)
	end,
	on_validate = function(self, card_use)
		local player = card_use.from
        local room = player:getRoom()
		...
		heg_await_exhausted:setSkillName("heg_fofl_duoshi")
		local invoke = false
		for _, p in sgs.qlist(room:findPlayersBySkillName("heg_fofl_duoshi")) do
			if p:getMark("heg_fofl_duoshi_lun") == 0 and p:objectName() ~= player:objectName() and room:askForSkillInvoke(p, self:objectName(), ToData(player)) then
				room:addPlayerMark(p, "heg_fofl_duoshi_lun")
				invoke = true
				break
			end
		end
		if not invoke then room:addPlayerMark(player, "heg_fofl_duoshi-PlayClear") return nil end
		return card
	end,
...
heg_fofl_duoshi_Attach = sgs.CreateOneCardViewAsSkill{
	name = "heg_fofl_duoshi_Attach&",
	filter_pattern = "red",
	view_as = function(self, card)
		local duoshi_card = heg_fofl_duoshiCard:clone()
		duoshi_card:addSubcard(card)
		return duoshi_card
	end,
	enabled_at_play = function(self, player)
		local invoke = false
		for _, p in sgs.qlist(player:getAliveSiblings()) do
			if p:hasSkill("heg_fofl_duoshi") and p:getMark("heg_fofl_duoshi_lun") == 0 then
				invoke = true
				break
			end
		end
		return invoke and player:getMark("heg_fofl_duoshi-PlayClear") == 0
	end,
}

heg_fofl_duoshi_buff = sgs.CreateTriggerSkill {
	name = "#heg_fofl_duoshi_buff",
	events = {sgs.CardUsed},
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		local use = data:toCardUse()
		if use.card and table.contains(use.card:getSkillNames(), "heg_fofl_duoshi") and use.card:isKindOf("heg_burning_camps") then
			local no_respond_list = use.no_respond_list
			table.insert(no_respond_list, "_ALL_TARGETS")
			use.no_respond_list = no_respond_list
			data:setValue(use)
...
heg_fofl_duoshi = sgs.CreateTriggerSkill {
	name = "heg_fofl_duoshi",
	events = {sgs.GameStart, sgs.EventAcquireSkill, sgs.CardFinished},
	on_trigger = function(self, event, player, data)
		if event == sgs.GameStart or (event == sgs.EventAcquireSkill and data:toString() == self:objectName()) then
			for _, p in sgs.qlist(player:getRoom():getAllPlayers()) do
				player:getRoom():attachSkillToPlayer(p, "heg_fofl_duoshi_Attach")
			end
		elseif event == sgs.CardFinished then
			local room = player:getRoom()
			local use = data:toCardUse()
			if not IsBigKingdomPlayer(player) and HasBigKingdomPlayer(room) then
				if use.card and table.contains(use.card:getSkillNames(), self:objectName()) and use.card:isKindOf("heg_await_exhausted") then
					local heg_burning_camps = dummyCard("heg_burning_camps")
					heg_burning_camps:setSkillName(self:objectName())
					local targets = sgs.SPlayerList()
					for _, p in sgs.qlist(room:getAllPlayers()) do
						local extra_target = room:getCardTargets(use.card, p)
						if heg_burning_camps:isAvailable(p) and not extra_target:isEmpty() then
							local x = 1

							-- 获取下家所在队列的其他成员
							local queue = GetQueueMembers(p:getNextAlive(1))
							if #queue >= 2 then
								for _, member in ipairs(queue) do
									-- 添加队列中除了下家之外的所有角色
									if member:objectName() ~= p:getNextAlive(1):objectName() and not p:isProhibited(member, heg_burning_camps) and member:objectName() ~= p:objectName() then
										x = x + 1
									end
								end
							end
					...
					if not targets:isEmpty() then
						local target = room:askForPlayerChosen(player, targets,self:objectName(), "@heg_fofl_duoshi", true)
						if target then
							local x = 1

							-- 获取下家所在队列的其他成员
							local queue = GetQueueMembers(target:getNextAlive(1))
							if #queue >= 2 then
								for _, member in ipairs(queue) do
									-- 添加队列中除了下家之外的所有角色
									if member:objectName() ~= target:getNextAlive(1):objectName() and not target:isProhibited(member, heg_burning_camps) and member:objectName() ~= target:objectName() then
										x = x + 1
									end
								end
							end
							local subcards = room:askForExchange(target, self:objectName(), x, x, false, "@heg_fofl_duoshi-discard:"..x, false)
							heg_burning_camps:addSubcards(subcards)
							local newuse = sgs.CardUseStruct()
							newuse.from = target
							newuse.card = heg_burning_camps
							newuse.to:append(target:getNextAlive(1))
							room:useCard(newuse)
						end
					end
				end
...
heg_fofl_luxun:addSkill("nosqianxun")
heg_fofl_luxun:addSkill(heg_fofl_duoshi)
heg_fofl_luxun:addSkill(heg_fofl_duoshi_buff)
extension_fakeoffline:insertRelatedSkills("heg_fofl_duoshi", "#heg_fofl_duoshi_buff")
if not sgs.Sanguosha:getSkill("heg_fofl_duoshi_Attach") then skills:append(heg_fofl_duoshi_Attach) end

heg_fofl_taishici = sgs.General(extension_fakeoffline,  "heg_fofl_taishici", "wu", 4)

heg_fofl_hanzhan = sgs.CreateTriggerSkill {
    name = "heg_fofl_hanzhan",
    frequency = sgs.Skill_Frequent,
    events = { sgs.Pindian },
    on_trigger = function(self, event, player, data, room)
        ...
        if pindian.from:hasSkill(self:objectName()) or pindian.to:hasSkill(self:objectName()) then
			...
			if loser and winner then
				local equip_count = winner:getEquips():length()
				if equip_count > 0 then
					if loser:askForSkillInvoke(self:objectName(), data) then
						local id = room:askForCardChosen(loser, winner, "e", self:objectName())
						local card = sgs.Sanguosha:getCard(id)
						room:obtainCard(loser, card, false)
					end
				end
			end
        end
...
heg_fofl_taishici:addSkill("tianyi")
heg_fofl_taishici:addSkill(heg_fofl_hanzhan)

heg_fofl_yanliangwenchou = sgs.General(extension_fakeoffline,  "heg_fofl_yanliangwenchou", "qun", 4)

heg_fofl_shuangxiongVS = sgs.CreateViewAsSkill {
    name = "heg_fofl_shuangxiong",
    n = 1,
    response_or_use = true,
    view_filter = function(self,to_select)
		if to_select:isEquipped() then return false end
		if sgs.Self:getMark(self:objectName()..to_select:getSuitString().."-Clear") > 0 then
			return to_select
		end
		return false
	end,
    ...
    enabled_at_play = function(self, player)
    	for _, card in sgs.qlist(player:getHandcards()) do
			if player:getMark(self:objectName()..card:getSuitString().."-Clear") >0  then
				return true
			end
		end
    end,
...
heg_fofl_shuangxiong = sgs.CreateTriggerSkill { -- 双雄 触发技
    name = "heg_fofl_shuangxiong",
    view_as_skill = heg_fofl_shuangxiongVS,
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.EventPhaseStart then
            if player:getPhase() == sgs.Player_Draw then -- 摸牌阶段
				...
				judge.reason = self:objectName()
				room:judge(judge)
				room:addPlayerMark(player, self:objectName()..judge.card:getSuitString().."-Clear", 1)
				room:addPlayerMark(player, "&"..self:objectName()..":"..judge.card:getSuitString().."-Clear", 1)
				return true
            end
		elseif event == sgs.FinishJudge then
			local card = judge.card
			if player:getPhase() ~= sgs.Player_NotActive and room:getCardPlace(card:getEffectiveId()) == sgs.Player_PlaceJudge then
				player:obtainCard(card)
				room:sendCompulsoryTriggerLog(player, self:objectName(), true)
			end
        end
...
heg_fofl_yanliangwenchou:addSkill(heg_fofl_shuangxiong)


heg_fofl_caohong = sgs.General(extension_fakeoffline,  "heg_fofl_caohong", "wei", 4)

heg_fofl_huyuanCard = sgs.CreateSkillCard{
	name = "heg_fofl_huyuan",
	will_throw = false,
	handling_method = sgs.Card_MethodNone,
	...
	on_effect = function(self, effect)
		...
		if card:isKindOf("EquipCard") then
			...
			local index = equip:location()
			if effect.to:getEquip(index) == nil then
				room:moveCardTo(self, caohong, effect.to, sgs.Player_PlaceEquip,sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_PUT, caohong:objectName(), "heg_fofl_huyuan", ""))
			end
		end
	end
}
heg_fofl_huyuanVS = sgs.CreateOneCardViewAsSkill{
	name = "heg_fofl_huyuan",
	filter_pattern = "EquipCard",
	response_pattern = "@@heg_fofl_huyuan",
	view_as = function(self, card)
		local first = heg_fofl_huyuanCard:clone()
		first:addSubcard(card:getId())
		first:setSkillName(self:objectName())
		return first
	end,
...
heg_fofl_huyuan = sgs.CreateTriggerSkill{
	name = "heg_fofl_huyuan",
	events = {sgs.EventPhaseProceeding},
	view_as_skill = heg_fofl_huyuanVS,
	on_trigger = function(self, event, player, data)
		local room = target:getRoom()
		if event == sgs.EventPhaseProceeding then
			if target:getPhase() == sgs.Player_Finish and not target:isNude() then
				room:askForUseCard(target, "@@heg_fofl_huyuan", "@huyuan-equip", -1, sgs.Card_MethodNone)
			end
		elseif event == sgs.CardsMoveOneTime then
			local move = data:toMoveOneTime()
			if move.to_place ~= sgs.Player_PlaceEquip then return false end
			...
			if to then
				for _, id in sgs.qlist(move.card_ids) do
					...
					if card:isKindOf("EquipCard") then
						local targets = sgs.SPlayerList()
						for _,p in sgs.qlist(room:getAllPlayers()) do
							if player:canDiscard(p, "hej") and to:distanceTo(p) <= 1 then
								targets:append(p)
							end
						end
						if not targets:isEmpty() then
						local to_dismantle = room:askForPlayerChosen(player, targets, "heg_fofl_huyuan", "@huyuan-discard:" .. effect.to:objectName())
						local card_id = room:askForCardChosen(player, to_dismantle, "he", "heg_fofl_huyuan", false,sgs.Card_MethodDiscard)
							room:throwCard(sgs.Sanguosha:getCard(card_id), to_dismantle, player)
						end
					end
				end
			end
...
heg_fofl_caohong:addSkill(heg_fofl_huyuan)
heg_fofl_caohong:addSkill("heyi")

heg_fofl_xusheng = sgs.General(extension_fakeoffline,  "heg_fofl_xusheng", "wu", 4)

heg_fofl_yicheng_Clear = sgs.CreateTriggerSkill{
	name = "#heg_fofl_yicheng_Clear",
	events = {sgs.EventPhaseChanging},
	on_trigger = function(self,event,player,data)
		local change = data:toPhaseChange()
		if change.to == sgs.Player_NotActive then
			local room = player:getRoom()
			for _, p in sgs.qlist(room:getAllPlayers()) do
				if p:getMark("&heg_fofl_yicheng-Clear") > 0 and p:canDiscard(p, "he") then
					local x = p:getMark("&heg_fofl_yicheng-Clear")
					room:askForDiscard(p, "heg_fofl_yicheng", x, x,		false, true, "@heg_fofl_yicheng-discard:"..x, ".", "heg_fofl_yicheng")
				end
			end
		end
		return false
...
heg_fofl_yicheng = sgs.CreateTriggerSkill{
	name = "heg_fofl_yicheng",
	events = {sgs.TargetConfirmed},
	on_trigger = function(self,event,player,data)
		local room = player:getRoom()
		local use = data:toCardUse()
		...
		for _,p in sgs.qlist(use.to) do
			...
			if room:askForSkillInvoke(player,self:objectName(),d) then
				p:drawCards(1, self:objectName())
				if p:isAlive() then
					room:askForUseCard(p, "EquipCard", "@heg_fofl_yicheng")
					room:addPlayerMark(p, "&heg_fofl_yicheng-Clear")
				end
				if not player:isAlive() then
					break
				end
...
heg_fofl_xusheng:addSkill(heg_fofl_yicheng)
heg_fofl_xusheng:addSkill(heg_fofl_yicheng_Clear)
extension_fakeoffline:insertRelatedSkills("heg_fofl_yicheng", "#heg_fofl_yicheng_Clear")

heg_fofl_dongzhuo = sgs.General(extension_fakeoffline,  "heg_fofl_dongzhuo", "qun", 4)

heg_fofl_baoling = sgs.CreateTriggerSkill{
	name = "heg_fofl_baoling",
	frequency = sgs.Skill_Compulsory,
	events = {sgs.EventPhaseEnd},
	waked_skills = "heg_fofl_benghuai",
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		room:notifySkillInvoked(player, self:objectName())
		if player:getGeneral2() and player:getGeneral2Name() ~= "" then
			local log = sgs.LogMessage()
			log.type = "#YHXiandao2"
			log.arg = player:getGeneral2Name()
			log.from = player
		...
		room:setPlayerMark(player, "heg_fofl_baoling", 1)
		if room:changeMaxHpForAwakenSkill(player, 3) then
			room:recover(player, sgs.RecoverStruct(player, nil, 3, self:objectName()))
			room:acquireSkill(player, "heg_fofl_benghuai")
		end
	end,
	can_trigger = function(self, target)
		return target:getPhase() == sgs.Player_Play and target:getMark("heg_fofl_baoling") == 0 and target:getMark("HengzhengUsed") >= 1
	end
}

heg_fofl_dongzhuo:addSkill("hengzheng")
heg_fofl_dongzhuo:addSkill(heg_fofl_baoling)

heg_fofl_benghuai = sgs.CreateTriggerSkill{
	name = "heg_fofl_benghuai",
	frequency = sgs.Skill_Compulsory,
	events = {sgs.EventPhaseProceeding},
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		...
		-- Check if player's HP is not the lowest
		local can_invoke = false
		for _, p in sgs.qlist(room:getOtherPlayers(player)) do
		...
		elseif choice == "beishui" then
			room:loseHp(player, 1, true, player, self:objectName())
			if player:isAlive() and player:getMaxHp() > 1 then
				room:loseMaxHp(player, 1, self:objectName())
			end
			if player:isAlive() then
				-- Grant extra draw phase
				local thread = room:getThread()
...
if not sgs.Sanguosha:getSkill("heg_fofl_benghuai") then skills:append(heg_fofl_benghuai) end

heg_fofl_lufan = sgs.General(extension_fakeoffline,  "heg_fofl_lufan", "wu", 3)

heg_fofl_tiaodu = sgs.CreateTriggerSkill {
	name = "heg_fofl_tiaodu",
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart then
			if player:getPhase() == sgs.Player_Play then
				...
				if not targets:isEmpty() then
					local target = room:askForPlayerChosen(player, targets, self:objectName(), "heg_fofl_tiaodu-invoke", true)
					if target and room:askForSkillInvoke(target, self:objectName(), ToData(player)) then
						local card_id = room:askForCardChosen(player, target, "e", self:objectName(), false, sgs.Card_MethodNone)
						local card = sgs.Sanguosha:getCard(card_id)
						local another = room:askForPlayerChosen(player, room:getOtherPlayers(target), self:objectName(), "@heg_fofl_tiaodu-give:"..card:objectName(), true)
						if another then
							room:obtainCard(another, card, false)
						end
					end
				end
			end
		elseif event == sgs.TargetConfirming then
			local use = data:toCardUse()
			if use.card and not use.card:isKindOf("SkillCard") and use.to:contains(use.from) and player:getMark(self:objectName()..use.card:getTypeId() .."-Clear") == 0 then
				if room:askForSkillInvoke(player, self:objectName(), ToData(use.from)) and room:askForSkillInvoke(use.from, self:objectName(), ToData(player)) then
					room:addPlayerMark(player, self:objectName()..use.card:getTypeId() .."-Clear", 1)
					room:addPlayerMark(player, "&"..self:objectName()..":"..use.card:getType() .."-Clear", 1)
					use.from:drawCards(1, self:objectName())
				end
			end
		end
...
heg_fofl_lufan:addSkill("bf_diancai")
heg_fofl_lufan:addSkill(heg_fofl_tiaodu)

heg_fofl_zuoci = sgs.General(extension_fakeoffline,  "heg_fofl_zuoci", "qun", 3)
heg_fofl_yiguiCard = sgs.CreateSkillCard{
	name = "heg_fofl_yigui",
	will_throw = false,
	handling_method = sgs.Card_MethodNone,
	filter = function(self, targets, to_select)
		local card = sgs.Self:getTag("heg_fofl_yigui"):toCard()
		if sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_RESPONSE_USE then
			local user_str = self:getUserString()
			card = sgs.Sanguosha:cloneCard(user_str)
		end
		...
		local generals = sgs.Sanguosha:getLimitedGeneralNames()
		for _,name in pairs (generals) do
			if sgs.Self:getMark("heg_fofl_yigui"..name) > 0 and not table.contains(kingdoms, sgs.Sanguosha:getGeneral(name):getKingdom()) then
				table.insert(kingdoms, sgs.Sanguosha:getGeneral(name):getKingdom())
			end
		end
		if card and card:targetFixed() then
	...
	feasible = function(self,targets,user)
		...
		local card = user:getTag("heg_fofl_yigui"):toCard()
		return card and card:targetsFeasible(plist,user) 
	end,
	on_validate = function(self,card_use)
		local player = card_use.from
		...
		local huashenss = {}
		for _,name in pairs (generals) do
			if player:getMark("heg_fofl_yigui"..name) > 0 then
				table.removeOne(generals, name)
				table.insert(huashenss, name)
			end
		end
		...
		local general = room:askForGeneral(player, table.concat(huashenss, "+"))
		if general == "cancel" then return false end
		room:setPlayerMark(player, "heg_fofl_yigui"..general, 0)
		table.removeOne(huashenss, general)
		local kingdom = sgs.Sanguosha:getGeneral(general):getKingdom()
		if self:getUserString() == "slash" and sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_RESPONSE_USE then
			local heg_fofl_yigui_list = {}
			table.insert(heg_fofl_yigui_list,"slash")
			table.insert(heg_fofl_yigui_list,"fire_slash")
			table.insert(heg_fofl_yigui_list,"thunder_slash")
			table.insert(heg_fofl_yigui_list,"ice_slash")
			aocaistring = room:askForChoice(player,"heg_fofl_yigui_slash",table.concat(heg_fofl_yigui_list,"+"))
		end
		local user_str = aocaistring
		local use_card = sgs.Sanguosha:cloneCard(user_str)
		use_card:setSkillName("heg_fofl_yigui")
		use_card:deleteLater()
		room:setCardFlag(use_card, "heg_fofl_yiguiusing")
		room:setCardFlag(use_card, kingdom)
		local available = true
		for _,p in sgs.qlist(card_use.to) do
			if card_use.from:isProhibited(p,use_card) then
		...
		available = available and use_card:isAvailable(card_use.from)
		if not available then return nil end
		room:addPlayerMark(player,"heg_fofl_yigui_guhuo_remove_"..use_card:objectName().."-Clear")
		for _,p in sgs.list(patterns())do
			local dc = dummyCard(p)
			if dc and player:getMark("heg_fofl_yigui_guhuo_remove_"..p.."-Clear")<1 then
                if ((use_card:isKindOf("BasicCard") and dc:isKindOf("BasicCard")) or (use_card:isKindOf("TrickCard") and dc:isKindOf("TrickCard"))) then
                    room:addPlayerMark(player, "heg_fofl_yigui_guhuo_remove_"..p.."-Clear")
                end
			end
		end
		return use_card
	...
	on_validate_in_response = function(self,user)
		...
		local huashenss = {}
		for _,name in pairs (generals) do
			if user:getMark("heg_fofl_yigui"..name) > 0 then
				table.removeOne(generals, name)
				table.insert(huashenss, name)
			end
		end
		...
		local general = room:askForGeneral(user, table.concat(huashenss, "+"))
		if general == "cancel" then return false end
		room:setPlayerMark(user, "heg_fofl_yigui"..general, 0)
		table.removeOne(huashenss, general)
		local kingdom = sgs.Sanguosha:getGeneral(general):getKingdom()
		local aocaistring
		if user_str == "peach+analeptic" then
			local heg_fofl_yigui_list = {}
			table.insert(heg_fofl_yigui_list,"peach")
			table.insert(heg_fofl_yigui_list,"analeptic")
			aocaistring = room:askForChoice(user,"heg_fofl_yigui_saveself",table.concat(heg_fofl_yigui_list,"+"))
		elseif user_str == "slash" then
			local heg_fofl_yigui_list = {}
			table.insert(heg_fofl_yigui_list,"slash")
			table.insert(heg_fofl_yigui_list,"fire_slash")
			table.insert(heg_fofl_yigui_list,"thunder_slash")
			table.insert(heg_fofl_yigui_list,"ice_slash")
			aocaistring = room:askForChoice(user,"heg_fofl_yigui_slash",table.concat(heg_fofl_yigui_list,"+"))
		else
			aocaistring = user_str
		end
		local user_str = aocaistring
		...
		room:setCardFlag(use_card, "heg_fofl_yiguiusing")
		room:setCardFlag(use_card, kingdom)
		room:addPlayerMark(user,"heg_fofl_yigui_guhuo_remove_"..use_card:objectName().."-Clear")
		for _,p in sgs.list(patterns())do
			local dc = dummyCard(p)
			if dc and user:getMark("heg_fofl_yigui_guhuo_remove_"..p.."-Clear")<1 then
                if ((use_card:isKindOf("BasicCard") and dc:isKindOf("BasicCard")) or (use_card:isKindOf("TrickCard") and dc:isKindOf("TrickCard"))) then
                    room:addPlayerMark(user, "heg_fofl_yigui_guhuo_remove_"..p.."-Clear")
                end
			end
		end
		return use_card
...
heg_fofl_yiguiVS = sgs.CreateZeroCardViewAsSkill{
	name = "heg_fofl_yigui",
	view_as = function(self)
		if sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_RESPONSE_USE then
				local pattern = sgs.Sanguosha:getCurrentCardUsePattern()
				local sc = sgs.Sanguosha:cloneCard(pattern:split("+")[1])
				local card = heg_fofl_yiguiCard:clone()
				card:setUserString(sc:objectName())
				return card
			end
		local c = sgs.Self:getTag("heg_fofl_yigui"):toCard()
		if c then
			local card = heg_fofl_yiguiCard:clone()
			card:setUserString(c:objectName())
			return card
	...
	enabled_at_play = function(self,player)
		...
		for _,patt in ipairs(patterns())do
			local dc = dummyCard(patt)
			if dc and player:getMark("heg_fofl_yigui_guhuo_remove_"..patt.."-Clear")<1 then
				dc:setSkillName(self:objectName())
				if dc:isAvailable(player)
				then return true end
			end
	...
	enabled_at_response = function(self,player,pattern)
		...
		for _,pt in sgs.list(pattern:split("+"))do
			local dc = dummyCard(pt)
			if dc:isKindOf("Nullification") then return false end
			if dc and player:getMark("heg_fofl_yigui_guhuo_remove_"..pt.."-Clear")<1 then
				return true
			end
		end
	end,
}
heg_fofl_yigui = sgs.CreateTriggerSkill{
	name = "heg_fofl_yigui",
	events = {sgs.GameStart},
	view_as_skill = heg_fofl_yiguiVS,
	on_trigger = function(self, event, player, data, room)
		local room = player:getRoom()
		local generals = sgs.Sanguosha:getLimitedGeneralNames()
		for _,name in pairs (generals) do
		...
		if #generals > 0 then
			room:broadcastSkillInvoke(self:objectName())
			local general = generals[math.random(1, #generals)]
			ChoiceLog(player, general, player, "heg_fofl_yigui")
			room:addPlayerMark(player, "heg_fofl_yigui"..general)
			room:doAnimate(4, player:objectName(), general)
			table.removeOne(generals, general)
			local general2 = generals[math.random(1, #generals)]
			ChoiceLog(player, general2, player, "heg_fofl_yigui")
			room:addPlayerMark(player, "heg_fofl_yigui"..general2)
			room:doAnimate(4, player:objectName(), general2)
		end
		return false
	end
}
heg_fofl_yigui_prohibit = sgs.CreateProhibitSkill{
    name = "#heg_fofl_yigui_prohibit",
    is_prohibited = function(self, from, to, card)
		if from:hasSkill("heg_fofl_yigui") and card and table.contains(card:getSkillNames(), "heg_fofl_yigui") and to and card:hasFlag("heg_fofl_yiguiusing") then
        	return not card:hasFlag(to:getKingdom())
		end
    end,
}
...
heg_fofl_jihun = sgs.CreateTriggerSkill{
	name = "heg_fofl_jihun",
	events = {sgs.Damaged, sgs.QuitDying, sgs.EventPhaseProceeding},
	frequency = sgs.Skill_Frequent,
	on_trigger = function(self, event, player, data, room)
		local room = player:getRoom()
		if event == sgs.Damaged then
			...
			if dying.who:isAlive() then
				for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					if p:objectName() ~= dying.who:objectName() and p:getKingdom() ~= dying.who:getKingdom() then
						local generals = sgs.Sanguosha:getLimitedGeneralNames()
						for _,name in pairs (generals) do
							if p:getMark("heg_fofl_yigui"..name) > 0 then
								table.removeOne(generals, name)
							end
						end
						for _,p in sgs.qlist(room:getAlivePlayers()) do
						...
						if #generals > 0  and room:askForSkillInvoke(p, self:objectName()) then
							...
							local general = generals[math.random(1, #generals)]
							ChoiceLog(p, general, p, self:objectName())
							room:addPlayerMark(p, "heg_fofl_yigui"..general)
							room:doAnimate(4, p:objectName(), general)
						end
					end
				end
            end
		elseif event == sgs.EventPhaseProceeding and player:getPhase() == sgs.Player_Start and player:hasSkill(self:objectName()) then
			
			local generals_list = {}
			local generals = sgs.Sanguosha:getLimitedGeneralNames()
			for _,name in pairs (generals) do
				if player:getMark("heg_fofl_yigui"..name) > 0 then
					table.insert(generals_list, name)
					table.removeOne(generals, name)
				end
			end
			if #generals_list > 0 and room:askForSkillInvoke(player, self:objectName()) then
				for _,p in sgs.qlist(room:getAlivePlayers()) do
					if table.contains(generals, p:getGeneralName()) then
						table.removeOne(generals, p:getGeneralName())
					end
				...
				local general = room:askForGeneral(player, table.concat(generals_list, "+"))
				table.removeOne(generals, general)
				room:setPlayerMark(player, "heg_fofl_yigui"..general, 0)				
				if #generals > 0 then
					if room:askForSkillInvoke(player, self:objectName()) then
						local general2 = room:askForGeneral(player, table.concat(generals_list, "+"))
						room:setPlayerMark(player, "heg_fofl_yigui"..general2, 0)	
						local general = generals[math.random(1, #generals)]
						ChoiceLog(player, general, player, self:objectName())
						room:addPlayerMark(player, "heg_fofl_yigui"..general)
						room:doAnimate(4, player:objectName(), general)
...
heg_fofl_zuoci:addSkill(heg_fofl_yigui)
heg_fofl_zuoci:addSkill(heg_fofl_yigui_prohibit)
heg_fofl_zuoci:addSkill(heg_fofl_jihun)
extension_fakeoffline:insertRelatedSkills("heg_fofl_yigui", "#heg_fofl_yigui_prohibit")

heg_fofl_lukang = sgs.General(extension_fakeoffline,  "heg_fofl_lukang", "wu", 3)

heg_fofl_jueyanClear = sgs.CreateTriggerSkill{
	name = "#heg_fofl_jueyan",
	events = {sgs.EventPhaseProceeding},
	on_trigger = function(self, event, player, data, room)
		if player:getPhase() == sgs.Player_Finish then
			local throw = {"hand", "equip", "judge"}
			for _, choice in ipairs(throw) do
				if player:getMark("heg_fofl_jueyan-"..choice.."-Clear") > 0 then
					if choice == "judge" then
						local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_THROW, player:objectName(), self:objectName(), nil)
						room:throwCard(player:getJudgingAreaID(), reason, nil)
...
heg_fofl_jueyan = sgs.CreateTriggerSkill{
	name = "heg_fofl_jueyan",
	events = {sgs.EventPhaseProceeding},
	on_trigger = function(self, event, player, data, room)
		if player:getPhase() == sgs.Player_Start then
			local choicelist = {}
			if player:canDiscard(player, "h") then
				table.insert(choicelist, "hand")
			end
			if player:canDiscard(player, "e") then
				table.insert(choicelist, "equip")
			end
			if player:canDiscard(player, "j") then
				table.insert(choicelist, "judge")
			end
			if #choicelist == 0 then return false end
			table.insert(choicelist, "cancel")
			local choice = room:askForChoice(player, self:objectName(), table.concat(choicelist, "+"))
			if choice ~= "cancel" then
				room:addPlayerMark(player, "&heg_fofl_jueyan+:+"..choice.."-Clear")
				room:addPlayerMark(player, "heg_fofl_jueyan-"..choice.."-Clear")
			end
			if choice == "hand" then
				room:addSlashCishu(player, 3, true)
			elseif choice == "equip" then
				player:drawCards(3, self:objectName())
				room:addMaxCards(player, 3, true)
			elseif choice == "judge" then
				player:skip(sgs.Player_Judge)
				room:acquireOneTurnSkills(player, "heg_fofl_jueyan", "jizhi")
			end
		end
	end
}
...
heg_fofl_keshouVS = sgs.CreateViewAsSkill{
	name = "heg_fofl_keshou",
	n = 2,
	view_filter = function(self, selected, to_select)
		if #selected < 2 then
			if #selected > 0 then
	...
	enabled_at_response = function(self, player, pattern)
		return pattern == "@@heg_fofl_keshou"
	end
}
heg_fofl_keshou = sgs.CreateTriggerSkill{
	name = "heg_fofl_keshou",
	events = {sgs.DamageInflicted, sgs.CardsMoveOneTime},
	view_as_skill = heg_fofl_keshouVS,
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		if event == sgs.DamageInflicted then
			local damage = data:toDamage()
			if damage.to and damage.to:objectName() == player:objectName() then
				if player:getHandcardNum() + player:getEquips():length() < 2 then return false end
				room:setTag("heg_fofl_keshou", data)
				local cards = room:askForUseCard(player, "@@heg_fofl_keshou", "@heg_fofl_keshou-discard", -1, sgs.Card_MethodDiscard)
				room:removeTag("heg_fofl_keshou")
				if cards and cards:subcardsLength() == 2 then
					room:notifySkillInvoked(player, self:objectName())
					player:damageRevises(data, -1)
		...
		elseif event == sgs.CardsMoveOneTime then
			local move = data:toMoveOneTime()
			local move = data:toMoveOneTime()
			if move.from and move.from:objectName() == player:objectName() and (move.from_places:contains(sgs.Player_PlaceHand) or move.from_places:contains(sgs.Player_PlaceEquip)) and move.to_place == sgs.Player_DiscardPile and (bit32.band(move.reason.m_reason, sgs.CardMoveReason_S_MASK_BASIC_REASON) == sgs.CardMoveReason_S_REASON_DISCARD or move.reason.m_reason == sgs.CardMoveReason_S_REASON_RULEDISCARD) then
				local count = 0
				for i = 0, move.card_ids:length() - 1, 1 do
					local from_place = move.from_places:at(i)
					if from_place == sgs.Player_PlaceHand or from_place == sgs.Player_PlaceEquip then
				...
				if count >= 2 and room:askForSkillInvoke(player, self:objectName()) then
					local judge = sgs.JudgeStruct()
					judge.who = player
					judge.pattern = ".|red"
					judge.good = true
...
heg_fofl_lukang:addSkill(heg_fofl_jueyan)
heg_fofl_lukang:addSkill(heg_fofl_jueyanClear)
heg_fofl_lukang:addSkill(heg_fofl_keshou)
extension_fakeoffline:insertRelatedSkills("heg_fofl_jueyan", "#heg_fofl_jueyanClear")


heg_fofl_zhonghui = sgs.General(extension_fakeoffline,  "heg_fofl_zhonghui", "wei", 4)

heg_fofl_paiyiCard = sgs.CreateSkillCard{
	name = "heg_fofl_paiyi",
	target_fixed = false,
	will_throw = false,
	filter = function(self, targets, to_select)
	...
	on_effect = function(self, effect)
		local room = effect.from:getRoom()
		local invoked = askCommandTo(effect.from, effect.to, "heg_fofl_paiyi", false)
		local x = math.min(math.max(effect.from:getPile("heg_quanji_power"):length(), 1), effect.from:getMaxHp())
		local throw = 1
		if invoked then
			effect.from:drawCards(x, self:objectName())
		else
			local targets = room:askForPlayersChosen(effect.from, room:getOtherPlayers(effect.from), self:objectName(), 0, x, "@heg_fofl_paiyi", true, true)
			if others and others:length() > 0 then
				local damage = sgs.DamageStruct()
				damage.reason = self:objectName()
				damage.from = effect.from
		...
		if throw > 0 then
			local card_ids = effect.from:getPile("heg_quanji_power")
			for i = 1, throw do
				room:fillAG(card_ids, effect.from)
				local id = room:askForAG(effect.from, card_ids, false, self:objectName())
				room:clearAG(effect.from)
				local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_REMOVE_FROM_PILE, "","heg_fofl_paiyi", "")
				local card = sgs.Sanguosha:getCard(id)
				room:throwCard(card, reason, nil)
			end
		end
...
heg_fofl_sunchen = sgs.General(extension_fakeoffline,  "heg_fofl_sunchen", "wu", 4)
		...
		if str:startsWith("notifyInvoked:") then
			...
			if player:hasSkill(strs[2],true) and not player:hasEquipSkill(strs[2]) then
				...
				and not sk:isAttachedLordSkill() then
					room:sendCompulsoryTriggerLog(player,"heg_fofl_shilu")
				end
			end
		end
	end,
...
heg_fofl_shilu = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self,event,player,data)
		...
		if event == sgs.ChoiceMade then
			...
			if str:startsWith("notifyInvoked:") then
				local strs = str:split(":")
				if player:hasSkill(strs[2],true) and not player:hasEquipSkill(strs[2]) and player:getGeneral2() ~= nil then
					
					for _, skill in sgs.qlist(player:getGeneral2():getVisibleSkillList()) do
						if skill:objectName() == strs[2] then
							local sk = sgs.Sanguosha:getSkill(strs[2])
							if sk and sk:getFrequency(player)~=sgs.Skill_Compulsory
							and not sk:isAttachedLordSkill() then
								room:sendCompulsoryTriggerLog(player,"heg_fofl_shilu")
								room:addPlayerMark(player, "heg_fofl_shilu-Clear")
								break
							end
						end
					end
		...
		elseif event == sgs.EventPhaseEnd then
			if player:getPhase() == sgs.Player_Play then
				local can_invoke = false
				if player:getMark("heg_fofl_shilu-Clear") == 0 then
					local kingdoms = sgs.Sanguosha:getKingdoms()
					table.removeOne(kingdoms, "god")
					local general_list = player:property("heg_fofl_shilu_generals"):toString():split("+")
					for _, name in ipairs(general_list) do
						local general = sgs.Sanguosha:getGeneral(name)
						if general and table.contains(kingdoms, general:getKingdom()) then
							table.removeOne(kingdoms, general:getKingdom())
						end
					end
					if #kingdoms == 0 then return false end
					local kingdom = kingdoms[math.random(1, #kingdoms)]
					if player:getGeneral2() ~= nil then
						table.insert(general_list, player:getGeneral2Name())
						room:setPlayerProperty(player, "heg_fofl_shilu_generals", sgs.QVariant(table.concat(general_list, "+")))
						player:setSkillDescriptionSwap("heg_fofl_shilu", "%arg1", table.concat(general_list, "+|+"))
						room:changeTranslation(player, "heg_fofl_shilu", 1)
					end
				end
			end
		elseif event == sgs.EventPhaseProceeding then
			if player:getPhase() == sgs.Player_Start then
				...
				if #general_list > 0 then
					for _, name in ipairs(general_list) do
						local general = sgs.Sanguosha:getGeneral(name)
						if general and not table.contains(kingdoms, general:getKingdom()) then
							table.insert(kingdoms, general:getKingdom())
						end
					end
					room:sendCompulsoryTriggerLog(player, self:objectName())
					if player:canDiscard(player, "h") then
						if #kingdoms >= player:getHandcardNum() then
							player:throwAllHandCards(self:objectName())
						else
							room:askForDiscard(player, self:objectName(), #kingdoms, #kingdoms, false, false)
						end
					end
					player:drawCards(#kingdoms, self:objectName())
				end
...
heg_fofl_xiongnue = sgs.CreateTriggerSkill{
	name = "heg_fofl_xiongnue",
	events = { sgs.DamageInflicted, sgs.DamageCaused },
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		local damage = data:toDamage()
		if damage.card and damage.card:isKindOf("Slash") then
			...
			if target then
				...
				if #general_list > 0 then
					for _, name in ipairs(general_list) do
						local general = sgs.Sanguosha:getGeneral(name)
						if general and target:getKingdom() == general:getKingdom() then
							room:sendCompulsoryTriggerLog(player, self:objectName())
							player:damageRevises(data, 1)
							break
						end
			...
			if event == sgs.DamageInflicted and damage.from then
				local general_list = player:property("heg_fofl_shilu_generals"):toString():split("+")
				local kingdoms = {}
				if #general_list > 0 then
					for _, name in ipairs(general_list) do
...
heg_fofl_sunchen:addSkill(heg_fofl_shilu)
heg_fofl_sunchen:addSkill(heg_fofl_shiluClear)
heg_fofl_sunchen:addSkill(heg_fofl_xiongnue)
extension_fakeoffline:insertRelatedSkills("heg_fofl_shilu", "#heg_fofl_shiluClear")

-- heg_fofl_gongsunyuan = sgs.General(extension_fakeoffline,  "heg_fofl_gongsunyuan", "qun", 4)
heg_fofl_2_zongyu = sgs.General(extension_fakeoffline,  "heg_fofl_2_zongyu", "shu", 3)

heg_fofl_2_chengshang_select = sgs.CreateSkillCard{
	name = "heg_fofl_2_chengshang",
	target_fixed = true,
	on_use = function(self, room, source, targets)
		local patterns = source:property("heg_fofl_2_chengshang"):toString():split("+")
		if #patterns == 0 then return end
		local choices = {}
		for _, patt in ipairs(patterns) do
			local dc = dummyCard(patt)
			if dc then
				dc:setSkillName("heg_fofl_2_chengshang")
				if dc:isAvailable(source) then
		...
		table.insert(choices, "cancel")
		local choice = room:askForChoice(source, "heg_fofl_2_chengshang", table.concat(choices, "+"))
		if choice ~= "cancel" then
			local card = sgs.Sanguosha:cloneCard(choice)
			card:setSkillName("heg_fofl_2_chengshang")
			card:addSubcards(self:getSubcards())
			if card:targetFixed() then
				if room:askForSkillInvoke(source, self:objectName(), ToData(card)) then
					local use = sgs.CardUseStruct()
					use.card = card
					use.from = source
					room:useCard(use)
				...
				room:setPlayerMark(source, "heg_fofl_2_chengshangId", self:getSubcards():first())
				room:setPlayerProperty(source, "heg_fofl_2_chengshang_card", sgs.QVariant(choice))
				room:askForUseCard(source, "@@heg_fofl_2_chengshang!", "@heg_fofl_2_chengshang")
				room:setPlayerProperty(source, "heg_fofl_2_chengshang_card", sgs.QVariant())
				room:setPlayerMark(source, "heg_fofl_2_chengshangId", 0)
			end
		end
	end
}
heg_fofl_2_chengshangCard = sgs.CreateSkillCard {
	name = "heg_fofl_2_chengshangCard",
	will_throw = false,
	filter = function(self, targets, to_select)
		local name = ""
		local card
	...
	on_validate_in_response = function(self, user)
		...
		if string.find(aocaistring, "+") then
			...
			local name = room:askForChoice(user, "heg_fofl_2_chengshang", table.concat(uses, "+"))
			use_card = sgs.Sanguosha:cloneCard(name, sgs.Card_NoSuit, -1)
		end
		use_card:addSubcard(user:getMark("heg_fofl_2_chengshangId"))
		use_card:setSkillName("heg_fofl_2_chengshang")
		room:addPlayerMark(user, "heg_fofl_2_chengshang_used-PlayClear")
		room:changeTranslation(user, "heg_fofl_2_chengshang", 1)
		return use_card
	end,
	on_validate = function(self, card_use)
		...
		if string.find(aocaistring, "+") then
			...
			local name = room:askForChoice(card_use.from, "heg_fofl_2_chengshang", table.concat(uses, "+"))
			use_card = sgs.Sanguosha:cloneCard(name, sgs.Card_NoSuit, -1)
		end
		...
		if not available then return nil end
		use_card:addSubcard(card_use.from:getMark("heg_fofl_2_chengshangId"))
		room:addPlayerMark(card_use.from, "heg_fofl_2_chengshang_used-PlayClear")
		room:changeTranslation(card_use.from, "heg_fofl_2_chengshang", 1)
		return use_card
	end
}
heg_fofl_2_chengshangVS = sgs.CreateViewAsSkill{
	name = "heg_fofl_2_chengshang",
	n = 1,
	response_or_use = true,
	view_filter = function(self, selected, to_select)
		local pattern = sgs.Sanguosha:getCurrentCardUsePattern()
		if pattern and pattern == "@@heg_fofl_2_chengshang" then
			return false
		else
			return not to_select:isEquipped()
		end
	...
	view_as = function(self, cards)
		if sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_PLAY then
			if #cards == 1 then
				local acard = heg_fofl_2_chengshang_select:clone()
				acard:addSubcard(cards[1]:getId())
				return acard
			end
		else
			local pattern = sgs.Sanguosha:getCurrentCardUsePattern()
			local acard = heg_fofl_2_chengshangCard:clone()
			if pattern and pattern == "@@heg_fofl_2_chengshang" then
				pattern = sgs.Self:property("heg_fofl_2_chengshang_card"):toString()
	...
	enabled_at_play = function(self, player)
		return player:getMark("heg_fofl_2_chengshang-PlayClear") > 0 and player:getMark("heg_fofl_2_chengshang_used-PlayClear") == 0
	end,
	enabled_at_response = function(self, player, pattern)
		return pattern == "@@heg_fofl_2_chengshang"
	end
}

heg_fofl_2_chengshang = sgs.CreateTriggerSkill{
	name = "heg_fofl_2_chengshang",
	events = {sgs.CardFinished},
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		local use = data:toCardUse()
		if use.from and use.from:objectName() == player:objectName() and player:hasSkill(self:objectName()) then
			local card = use.card
			if card and not card:isKindOf("SkillCard") and use.to:length() > 0 then
				local can_invoke = false
				for _, p in sgs.qlist(use.to) do
					if p:objectName() ~= player:objectName() then
						can_invoke = true
				...
				if can_invoke and not use.card:hasFlag("DamageDone") and room:askForSkillInvoke(player, self:objectName(), data) then
					room:broadcastSkillInvoke(self:objectName())
					player:drawCards(1, self:objectName())
					room:addPlayerMark(player, "heg_fofl_2_chengshang-PlayClear")
					local patterns = {}
					for _,id in sgs.list(room:getDrawPile())do
						local c = sgs.Sanguosha:getEngineCard(id)
						if c:getSuit() == card:getSuit() and c:getNumber() == card:getNumber() and (c:isKindOf("BasicCard") or c:isNDTrick()) and not table.contains(patterns, c:objectName()) then
							table.insert(patterns, c:objectName())
						end
					end
					room:setPlayerProperty(player, "heg_fofl_2_chengshang", sgs.QVariant(table.concat(patterns, "+")))
					player:setSkillDescriptionSwap("heg_fofl_2_chengshang", "%arg1", table.concat(patterns, "+|+"))
					room:changeTranslation(player, "heg_fofl_2_chengshang", 2)
				end
			end
		end
		return false
...
heg_fofl_2_zongyu:addSkill("heg_qiao")
heg_fofl_2_zongyu:addSkill(heg_fofl_2_chengshang)

heg_fofl_lifeng = sgs.General(extension_fakeoffline,  "heg_fofl_lifeng", "shu", 3)

heg_fofl_tunchu = sgs.CreateTriggerSkill{
	name = "heg_fofl_tunchu",
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.DrawNCards then
			...
			if room:askForSkillInvoke(player, self:objectName(), data) then
				...
				draw.num = draw.num + 2
				data:setValue(draw)
				room:addPlayerMark(player, "&heg_fofl_tunchu-Clear")
				room:setPlayerCardLimitation(player, "use", "Slash", true)
			end
		elseif event == sgs.EventPhaseEnd then
			if player:getPhase() == sgs.Player_Draw and player:getMark("&heg_fofl_tunchu-Clear") > 0 then
				local card = room:askForExchange(player, self:objectName(), 2, 1, false, "@heg_fofl_tunchu", false, "")
				player:addToPile(card, "heg_fofl_tunchu_liang")
			end
		end
	end
}
...
heg_fofl_lifengCard = sgs.CreateSkillCard{
	...
	filter = function(self,targets,to_select,source)
		if #targets>0 or to_select==source then return false end
		return to_select:hasFlag("CurrentPlayer")
	end,
	on_effect = function(self,effect)
		local room = effect.from:getRoom()
...
heg_fofl_wangling = sgs.General(extension_fakeoffline,  "heg_fofl_wangling", "wei", 4)

heg_fofl_mibei_select = sgs.CreateSkillCard{
	name = "heg_fofl_mibei",
	target_fixed = true,
	will_throw = false,
	on_use = function(self, room, source, targets)
		local ids = sgs.IntList()
		local patterns = {}
		for _, id in sgs.qlist(source:getHandcards()) do
			local card = sgs.Sanguosha:getCard(id)
			if card:hasFlag("cardTip:heg_fofl_mibei-Clear") and self:getSubcards():first() ~= id then
				ids:append(id)
				if card and card:isAvailable(source) and not table.contains(patterns, card:objectName()) then
		...
		if #patterns == 0 then return end
		table.insert(patterns, "cancel")
		local choice = room:askForChoice(source, "heg_fofl_mibei", table.concat(patterns, "+"))
		if choice ~= "cancel" then
			local use_card
			for _, id in sgs.qlist(ids) do
				if sgs.Sanguosha:getCard(id):objectName() == choice then
					use_card = sgs.Sanguosha:getCard(id)
					break
				end
			end
			local card = sgs.Sanguosha:cloneCard(choice, use_card:getSuit(), use_card:getNumber())
			card:setSkillName("heg_fofl_mibei")
			card:addSubcards(self:getSubcards())
			if card:targetFixed() then
				if room:askForSkillInvoke(source, self:objectName(), ToData(card)) then
					local use = sgs.CardUseStruct()
				...
				local use_card = sgs.Sanguosha:getCard(self:getSubcards():first())
				room:setPlayerMark(source, "heg_fofl_mibeiId", self:getSubcards():first())
				room:setPlayerProperty(source, "heg_fofl_mibei_card", sgs.QVariant(choice))
				room:askForUseCard(source, "@@heg_fofl_mibei!", "@heg_fofl_mibei")
				room:setPlayerProperty(source, "heg_fofl_mibei_card", sgs.QVariant())
				room:setPlayerMark(source, "heg_fofl_mibeiId", 0)
			end
		end
	end
}

heg_fofl_mibeiVS = sgs.CreateOneCardViewAsSkill{
	name = "heg_fofl_mibei",
	view_filter = function(self, to_select)
		local pattern = sgs.Sanguosha:getCurrentCardUsePattern()
		if pattern and pattern == "@@heg_fofl_mibei!" then
			return to_select:hasFlag("cardTip:heg_fofl_mibei-Clear") and to_select:getId() == sgs.Self:getMark("heg_fofl_mibeiId")
		else
			return to_select:hasFlag("cardTip:heg_fofl_mibei-Clear")
		end
	end,
	view_as = function(self, cards)
		if sgs.Self:getMark("heg_fofl_mibeiId") > 0 then
			if #cards == 1 then
				local card_id = sgs.Self:getMark("heg_fofl_mibeiId")
				local card = sgs.Sanguosha:getCard(card_id)
				local first = cards[1]
				local name = sgs.Self:property("heg_fofl_mibei_card"):toString()
			...
			return heg_fofl_mibei_select:clone()
		end
	end,
	enabled_at_play = function(self, player)
		return player:getMark("heg_fofl_mibei-Clear") > 0 and player:getMark("heg_fofl_mibei_used-Clear") == 0
	end,
	enabled_at_response = function(self, player, pattern)
		return pattern == "@@heg_fofl_mibei!"
	end
}

heg_fofl_mibei = sgs.CreateTriggerSkill{
	name = "heg_fofl_mibei",
	events = {sgs.EventPhaseProceeding, sgs.EventForDiy},
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseProceeding then
			local targets = sgs.SPlayerList()
			for _, p in sgs.qlist(room:getOtherPlayers(player)) do
			...
			if not targets:isEmpty() then
				local target = room:askForPlayerChosen(player, targets, self:objectName(), "@heg_fofl_mibei", true, true)
				if target then
					local invoked = askCommandTo(target, player, self:objectName())
				end
			end
		elseif event == sgs.EventForDiy then
			local str = data:toString()
			local strs = str:split(":")
			if strs[1] ~= "AfterCommandUse" then return false end
			if strs[3] ~= player:objectName() then return false end
			if strs[6] == "cancel" then
				if not player:isKongcheng() and player:getHandcardNum() > 0 then
					local cards = room:askForExchange(player, self:objectName(), 1, 3, false, "@heg_fofl_mibei-discard", false, "")
					room:showCard(player, cards)
					if cards:subcardsLength() > 1 then
						for _, id in sgs.qlist(cards) do
							room:setCardTip(sgs.Sanguosha:getCard(id), "heg_fofl_mibei-Clear")
						end
					end
				end

			elseif strs[6] ~= "cancel" then
				local from = room:findPlayerByObjectName(strs[2])
				if from and from:getHandcardNum() > player:getHandcardNum() then
					local draw_num = math.min(from:getHandcardNum() - player:getHandcardNum(), 5) 
					if draw_num > 0 then
...
heg_fofl_wangling:addSkill(heg_fofl_mibei)


heg_fofl_yanyan = sgs.General(extension_fakeoffline,  "heg_fofl_yanyan", "shu", 4)

heg_fofl_juzhan = sgs.CreateTriggerSkill{
	name = "heg_fofl_juzhan",
	frequency = sgs.Skill_NotFrequent,
	change_skill = true,
	...
	on_trigger = function(self, event, player, data, room)
		...
		if event == sgs.TargetConfirmed then
			...
			if use.card:isKindOf("Slash") and player:getChangeSkillState(self:objectName())==1 and use.from and use.to:contains(player) and use.from:objectName() ~= player:objectName() and room:askForSkillInvoke(player, self:objectName(), _data) then
				...
				local x = math.max(1, use.from:getLostHp())
				room:drawCards(players, x, self:objectName())
				room:addPlayerMark(use.from, "juzhan_from-Clear")
				room:addPlayerMark(player, "juzhan_to-Clear")
				room:addPlayerMark(target, "&heg_fofl_juzhan+to+#"..player:objectName().."-Clear")
				local skills = {}
				for _,sk in sgs.list(use.from:getSkillList())do
					if sk:isAttachedLordSkill() then continue end
					if use.from:getMark("Qingcheng"..sk:objectName()) == 0 then
				...
				room:addPlayerMark(use.from, "Qingcheng"..skill, 1)
        		room:addPlayerMark(use.from, "heg_fofl_juzhan_"..skill, 1)
			end
		else
			for _, p in sgs.qlist(use.to) do
				_data:setValue(p)
				if use.card:isKindOf("Slash") and player:getChangeSkillState(self:objectName())~=1 and not p:isNude() and p:getCardCount() >= math.max(1, use.from:getLostHp()) and room:askForSkillInvoke(player, self:objectName(), _data) then
					...
					room:addPlayerMark(player, "Qingcheng".."heg_fofl_juzhan", 1)
					room:addPlayerMark(player, "heg_fofl_juzhan_".."heg_fofl_juzhan", 1)
				end
			end
		end
		return false
...
heg_fofl_juzhanClear = sgs.CreateTriggerSkill{
    name = "#heg_fofl_juzhanClear",
    events = {sgs.EventPhaseStart},
    frequency = sgs.Skill_NotFrequent,
    on_trigger = function(self, event, player, data)
        local room = player:getRoom()
        ...
        for _,p in sgs.qlist(room:getAlivePlayers()) do
            for _,sk in sgs.list(p:getSkillList())do
                if sk:isAttachedLordSkill() then continue end
                if p:getMark("heg_fofl_juzhan_"..sk:objectName()) > 0 then
                    room:removePlayerMark(p, "heg_fofl_juzhan_"..sk:objectName(), 1)
                    room:removePlayerMark(p, "Qingcheng"..sk:objectName(), 1)
                end
            end
        end
    end,
...
heg_fofl_yanyan:addSkill(heg_fofl_juzhan)
heg_fofl_yanyan:addSkill(heg_fofl_juzhanClear)
extension_fakeoffline:insertRelatedSkills("heg_fofl_juzhan", "#heg_fofl_juzhanClear")

heg_fofl_zhuran = sgs.General(extension_fakeoffline,  "heg_fofl_zhuran", "wu", 4)

heg_fofl_danshou = sgs.CreateTriggerSkill{
	name = "heg_fofl_danshou",
	frequency = sgs.Skill_NotFrequent,
	events = {sgs.EventPhaseProceeding, sgs.EventPhaseStart, sgs.DrawNCards, sgs.CardsMoveOneTime},
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseProceeding then
			if player:getPhase() == sgs.Player_Start then
				for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					if p:canDiscard(p, "hej") and p:getMark("heg_fofl_danshou_lun") == 0 then
						if room:askForSkillInvoke(p, self:objectName(), data) then
							room:addPlayerMark(p, "heg_fofl_danshou_throw-Clear", p:getCardCount(true, true))
							room:addPlayerMark(p, "heg_fofl_danshou_lun", 1)
							room:addPlayerMark(p, "heg_fofl_danshou-Clear", 1)
							room:addPlayerMark(p, "&heg_fofl_danshou-Clear", 1)
						end
					end
				end
		...
		elseif event == sgs.EventPhaseStart then
			if player:getMark("heg_fofl_danshou-Clear") > 0 and player:getPhase() ~= sgs.Player_Finish and player:getPhase() ~= sgs.Player_Start then
				if player:getHandcardNum() <= player:getMark("heg_fofl_danshou_throw-Clear") then
					local choice = room:askForChoice(player, "heg_fofl_danshou", "draw+extra")
					if choice == "draw" then
						player:drawCards(1, self:objectName())
					else
						room:addPlayerMark(player, "heg_fofl_danshou_draw-Clear", 1)
					end
				end
			end
		elseif event == sgs.DrawNCards then
			if player:getMark("&heg_fofl_danshou-Clear") > 0 then
				local draw = data:toDraw()
				if draw.reason == "heg_fofl_danshou" then
					draw.num = draw.num + player:getMark("heg_fofl_danshou_draw-Clear")
					data:setValue(draw)
				end
			end
		elseif event == sgs.CardsMoveOneTime then
			 local move = data:toMoveOneTime()
			if move.to and move.to:objectName() == player:objectName() and (bit32.band(move.reason.m_reason, sgs.CardMoveReason_S_MASK_BASIC_REASON) == sgs.CardMoveReason_S_REASON_DRAW 
			    or move.reason.m_reason == sgs.CardMoveReason_S_REASON_DRAW) and move.reason.m_skillName ~= "heg_fofl_danshou" then
				if move.card_ids:length() == 4 then
					local current = room:getCurrent()
					if current and current:isAlive() then
						if room:askForSkillInvoke(player, self:objectName(), data) then
...
heg_fofl_zhuran:addSkill(heg_fofl_danshou)

heg_fofl_gaoshun = sgs.General(extension_fakeoffline,  "heg_fofl_gaoshun", "qun", 4)

heg_fofl_xunxi = sgs.CreateTriggerSkill{
	name = "heg_fofl_xunxi",
	frequency = sgs.Skill_NotFrequent,
	...
	on_trigger = function(self, event, player, data, room)
		...
		if damage.to:getPhase() ~= sgs.Player_NotActive then return false end
		local slash = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)
		slash:setSkillName(self:objectName())
		slash:deleteLater()
		for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
			if damage.to ~= p and p:getMark("heg_fofl_xunxi"..damage.to:objectName()) == 0 and p:canSlash(damage.to, slash, false) then 
				if room:askForSkillInvoke(p, self:objectName(), data) then
					room:addPlayerMark(p, "heg_fofl_xunxi"..damage.to:objectName(), 1)
					room:broadcastSkillInvoke(self:objectName())
					local use = sgs.CardUseStruct()
					use.card = slash
					use.from = p
...
heg_fofl_huanjia_weapon = sgs.CreateViewAsEquipSkill{
	name = "#heg_fofl_huanjia_weapon",
	view_as_equip = function(self, player)
		if player:getWeapon() == nil then
			return player:property("heg_fofl_huanjia_weapon"):toString()
		else
			return ""
		end
	end,
}
heg_fofl_huanjia_armor = sgs.CreateViewAsEquipSkill{
	name = "#heg_fofl_huanjia_armor",
	view_as_equip = function(self, player)
		if player:getArmor() == nil then
			return player:property("heg_fofl_huanjia_armor"):toString()
		else
			return ""
		end
	end,
}
heg_fofl_huanjia = sgs.CreateTriggerSkill{
	name = "heg_fofl_huanjia",
	frequency = sgs.Skill_Compulsory,
	events = {sgs.TargetConfirmed},
	on_trigger = function(self, event, player, data, room)
		local use = data:toCardUse()
		if use.card:isKindOf("Slash") then
			if use.to:contains(player) and player:getMark("heg_fofl_huanjia_armor_used-Clear") == 0 and use.from and use.from:getArmor() then
				room:addPlayerMark(player, "heg_fofl_huanjia_armor_used-Clear", 1)
				if player:getArmor() == nil then
					room:setPlayerProperty(player, "heg_fofl_huanjia_armor", sgs.QVariant(use.from:getArmor():objectName()))
				end
			end
			if use.to:length() == 1 and use.from == player and player:getMark("heg_fofl_huanjia_weapon_used-Clear") == 0 and use.to:first():getWeapon() then
				room:addPlayerMark(player, "heg_fofl_huanjia_weapon_used-Clear", 1)
				if player:getWeapon() == nil then
					room:setPlayerProperty(player, "heg_fofl_huanjia_weapon", sgs.QVariant(use.to:first():getWeapon():objectName()))
				end
			end
		end
	end
}
heg_fofl_huanjia_Clear = sgs.CreateTriggerSkill{
	name = "#heg_fofl_huanjia_Clear",
	events = {sgs.CardsMoveOneTime, sgs.EventPhaseChanging},
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardsMoveOneTime then
			local move = data:toMoveOneTime()
			if move.to and move.to:objectName() == player:objectName() and move.to_place == sgs.Player_PlaceEquip then
				for _, id in sgs.qlist(move.card_ids) do
					...
					if card:isKindOf("Weapon") then
						room:setPlayerProperty(player, "heg_fofl_huanjia_weapon", sgs.QVariant())
					elseif card:isKindOf("Armor") then
						room:setPlayerProperty(player, "heg_fofl_huanjia_armor", sgs.QVariant())
					end
				end
			end
		elseif event == sgs.EventPhaseChanging then
...
heg_fofl_gaoshun:addSkill(heg_fofl_xunxi)
heg_fofl_gaoshun:addSkill(heg_fofl_huanjia)
heg_fofl_gaoshun:addSkill(heg_fofl_huanjia_weapon)
heg_fofl_gaoshun:addSkill(heg_fofl_huanjia_armor)
heg_fofl_gaoshun:addSkill(heg_fofl_huanjia_Clear)
extension_fakeoffline:insertRelatedSkills("heg_fofl_huanjia", "#heg_fofl_huanjia_Clear")
extension_fakeoffline:insertRelatedSkills("heg_fofl_huanjia", "#heg_fofl_huanjia_armor")
extension_fakeoffline:insertRelatedSkills("heg_fofl_huanjia", "#heg_fofl_huanjia_weapon")
...
heg_fofl_xiongshu_buff =  = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		local damage = data:toDamage()
		if damage.card and (damage.card:isKindOf("Slash") or (damage.card:isDamageCard() and damage.card:isKindOf("TrickCard"))) then
			for _, p in sgs.qlist(room:getAlivePlayers()) do
				if p:getMark("heg_fofl_xiongshu"..damage.card:getEffectiveId().."-Clear") > 0 then
					damage.from = p
					data:setValue(damage)
				end
			end
...
heg_fofl_xiongshu = sgs.CreateTriggerSkill{
	name = "heg_fofl_xiongshu",
	frequency = sgs.Skill_NotFrequent,
	events = {sgs.TargetConfirmed},
	on_trigger = function(self, event, player, data, room)
		local use = data:toCardUse()
		if not player:hasTurn() then return end
		if (use.card:isKindOf("Slash") or (use.card:isDamageCard() and use.card:isKindOf("TrickCard"))) then
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if p:getMark("heg_fofl_xiongshu-Clear") == 0 then
					if use.from and use.from:objectName() == p:objectName() then
						local target = room:askForPlayerChosen(p, room:getOtherPlayers(p), self:objectName(), "@heg_fofl_xiongshu", true, true)
						if target then
							room:addPlayerMark(p, "heg_fofl_xiongshu-Clear")
							room:addPlayerMark(target, "heg_fofl_xiongshu"..use.card:getEffectiveId().."-Clear")
						end
					elseif  use.from and use.from:objectName() ~= p:objectName() and p:canDisCard(p, "he") then
						if room:askForDiscard(p, "heg_fofl_xiongshu", 1, 1, true, true) then
							room:addPlayerMark(p, "heg_fofl_xiongshu-Clear")
							room:addPlayerMark(p, "heg_fofl_xiongshu"..use.card:getEffectiveId().."-Clear")
						end
					end
				end
			end
...
heg_fofl_jianhui = sgs.CreateTriggerSkill{
	name = "heg_fofl_jianhui",
	frequency = sgs.Skill_NotFrequent,
	events = {sgs.Damage},
	on_trigger = function(self, event, player, data, room)
		local damage = data:toDamage()
		if damage.from and damage.to and damage.from:objectName() ~= damage.to:objectName() and damage.from:isAlive() and damage.to:isAlive() then
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				local targets = sgs.SPlayerList()
				if p:canDiscard(damage.from, "he") then
					targets:append(damage.to)
				end
				if p:canDiscard(damage.to, "he") then
					targets:append(damage.from)
				end
				if not targets:isEmpty() then
					local target = room:askForPlayerChosen(p, targets, self:objectName(), "@heg_fofl_jianhui", true, true)
					if target then
						
						target:drawCards(1, self:objectName())
						local to_discard = damage.from
						if target:objectName() == damage.from:objectName() then
							to_discard = damage.to
						end
						local card = room:askForCardChosen(p, to_discard, "he", self:objectName(), false, sgs.Card_MethodDiscard)
						if card then
							room:throwCard(card, to_discard, p)
						end
					end
				end
			end
...
heg_fofl_jiachong:addSkill(heg_fofl_xiongshu)
heg_fofl_jiachong:addSkill(heg_fofl_jianhui)
heg_fofl_jiachong:addSkill(heg_fofl_xiongshu_buff)
extension_fakeoffline:insertRelatedSkills("heg_fofl_xiongshu", "#heg_fofl_xiongshu_buff")

heg_fofl_yanghu = sgs.General(extension_fakeoffline,  "heg_fofl_yanghu", "jin", 4)

heg_fofl_suixinCard = sgs.CreateSkillCard{
	name = "heg_fofl_suixin",
	target_fixed = false,
	will_throw = false,
	filter = function(self, targets, to_select, player)
		if #targets > 0 then return false end
		local card = sgs.Sanguosha:getCard("heg_await_exhausted", sgs.Card_NoSuit, 0)
		card:deleteLater()
		return to_select:objectName() ~= sgs.Self:objectName() and CanToCard(self,player,to_select)
	end,
	on_use = function(self, room, source, targets)
		local card = sgs.Sanguosha:cloneCard("heg_await_exhausted", sgs.Card_NoSuit, 0)
		card:setSkillName("heg_fofl_suixin")
		if targets[1]:isAlive() then
			local use = sgs.CardUseStruct()
			use.card = card
			use.from = source
...
heg_fofl_suixin = sgs.CreateZeroCardViewAsSkill{
	name = "heg_fofl_suixin",
	view_as = function(self, cards)
		return heg_fofl_suixinCard:clone()
	end,
	enabled_at_play = function(self, player)
		return not player:hasUsed("#heg_fofl_suixin")
	end
}

heg_fofl_weirongCard = sgs.CreateSkillCard{
	name = "heg_fofl_weirong",
	target_fixed = true,
	will_throw = true,
	on_use = function(self, room, source, targets)
		local x = math.max(1, math.min(source:getMark("&heg_fofl_weirong"), source:getMaxHp()))
		if source:getChangeSkillState("heg_fofl_weirong") == 1 then
			room:setChangeSkillState(player,self:objectName(),2)
			room:setPlayerMark(source, "heg_fofl_weirong_effect_lun", 2)
			source:drawCards(x, self:objectName())
			room:addPlayerMark(source, "heg_fofl_weirong_draw_lun", x)
			room:setPlayerMark(source, "&heg_fofl_weirong+heg_fofl_weirong_effect_2_lun", 1)
		else
			room:setChangeSkillState(player,self:objectName(),1)
			room:setPlayerMark(source, "heg_fofl_weirong_effect_lun", 1)
			room:addPlayerMark(source, "heg_fofl_weirong_discard_lun", self:subcardsLength())
			room:setPlayerMark(source, "&heg_fofl_weirong+heg_fofl_weirong_effect_1_lun", 1)
		end
	end
}
heg_fofl_weirongVS = sgs.CreateViewAsSkill{
	name = "heg_fofl_weirong",
	n = 999,
	view_filter = function(self, to_select)
		return true,
	...
	view_as = function(self, cards)
		...
		local x = math.max(1, math.min(sgs.Self:getMark("&heg_fofl_weirong"), sgs.Self:getMaxHp()))
		if #cards > 0 and sgs.Self:getChangeSkillState("heg_fofl_weirong") ~= 1 and #cards == x then
			card:addSubcards(cards)
		end
		if (sgs.Self:getChangeSkillState("heg_fofl_weirong") ~= 1 and #cards == x) or (sgs.Self:getChangeSkillState("heg_fofl_weirong") == 1 and #cards == 0) then
			return card
		end
	end,
	enabled_at_play = function(self, player)
		return player:getMark("heg_fofl_weirong_effect_lun") == 0
	end
}

heg_fofl_weirong = sgs.CreateTriggerSkill{
	name = "heg_fofl_weirong",
	events = {sgs.CardsMoveOneTime},
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardsMoveOneTime then
			local move = data:toMoveOneTime()
			if event == sgs.CardsMoveOneTime and not room:getTag("FirstRound"):toBool() and move.to and move.to:objectName() == player:objectName() and player:hasSkill(self:objectName()) and move.reason.m_skillName ~= "heg_fofl_weirong" then
				if player:getMark("heg_fofl_weirong_effect_lun") == 1 then
					player:drawCards(1, self:objectName())
					room:addPlayerMark(player, "heg_fofl_weirong_draw_lun", 1)
					local x = math.abs(player:getMark("heg_fofl_weirong_draw_lun") - player:getMark("heg_fofl_weirong_discard_lun"))
					room:setPlayerMark(player, "&heg_fofl_weirong", x)
				end
			end
			if event == sgs.CardsMoveOneTime and move.from and move.from:objectName() == player:objectName() and player:hasSkill(self:objectName()) and move.reason.m_skillName ~= "heg_fofl_weirong" then
				if player:canDiscard(player, "he") then
					room:askForDiscard(player, self:objectName(), 1, 1, true, true)
					room:addPlayerMark(player, "heg_fofl_weirong_discard_lun", 1)
					local x = math.abs(player:getMark("heg_fofl_weirong_draw_lun") - player:getMark("heg_fofl_weirong_discard_lun"))
					room:setPlayerMark(player, "&heg_fofl_weirong", x)
				end
			end
...
heg_fofl_yanghu:addSkill(heg_fofl_suixin)
heg_fofl_yanghu:addSkill(heg_fofl_weirong)

heg_fofl_2_wuqiujian = sgs.General(extension_fakeoffline,  "heg_fofl_2_wuqiujian", "wei", 4)

heg_fofl_2_zhengrong_buff = sgs.CreateDistanceSkill{
	name = "#heg_fofl_2_zhengrong_buff",
	correct_func = function(self, from, to)
		if from:hasSkill("heg_fofl_2_zhengrong") and IsBigKingdomPlayerClient(to) then
			return -1
		end
		return 0
	end
...
heg_fofl_2_zhengrong = sgs.CreateTriggerSkill {
	name = "heg_fofl_2_zhengrong",
	events = {sgs.DamageInflicted, sgs.DamageCaused},
	on_trigger = function(self, event, player, data, room)
		local damage = data:toDamage()
		local target = nil
...
heg_fofl_2_wuqiujian:addSkill(heg_fofl_2_zhengrong)

heg_fofl_2_chengong = sgs.General(extension_fakeoffline,  "heg_fofl_2_chengong", "qun", 3)

heg_fofl_2_yinpanCard = sgs.CreateSkillCard {
	name = "heg_fofl_2_yinpan",
	target_fixed = false,
	will_throw = false,
	filter = function(self, targets, to_select)
		return #targets == 0 and to_select:objectName() ~= sgs.Self:objectName()
	...
	on_use = function(self, room, source, targets)
		...
		for _, p in sgs.qlist(room:getOtherPlayers(target)) do
			if p:canSlash(target, nil, false) and p:objectName() ~= source:objectName() then
				room:askForUseSlashTo(p, target, "@heg_fofl_2_yinpan-slash:" .. target:objectName(), false, false, false, source, nil, "heg_fofl_2_yinpan")
			end
		end
		if target:isAlive() and target:getMark(self:objectName().."-Clear") == 0 then
			local recover = sgs.RecoverStruct()
...
heg_fofl_2_yinpanVS = sgs.CreateZeroCardViewAsSkill{
	name = "heg_fofl_2_yinpan",
	view_as = function()
		return heg_fofl_2_yinpanCard:clone()
	end,
	enabled_at_play = function(self, player)
		return not player:hasUsed("#heg_fofl_2_yinpan")
	end,
}
heg_fofl_2_yinpan = sgs.CreateTriggerSkill {
	name = "heg_fofl_2_yinpan",
	events = {sgs.EventPhaseStart, sgs.Damaged, sgs.EnterDying},
	view_as_skill = heg_fofl_2_yinpanVS,
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseStart then
			if player:getPhase() == sgs.Player_RoundStart and player:getMark(self:objectName()) > 0 then
				room:addSlashCishu(player, player:getMark(self:objectName()), true)
		...
		elseif event == sgs.Damaged then
			local damage = data:toDamage()
			if damage.to:objectName() == player:objectName() and damage.card and damage.card:hasFlag("heg_fofl_2_yinpan") then
				room:addPlayerMark(player, self:objectName(), damage.damage)
			end
		elseif event == sgs.EnterDying then
			local dying = data:toDying()
			if dying.who:objectName() == player:objectName() and dying.damage and dying.damage.card and dying.damage.card:hasFlag("heg_fofl_2_yinpan") then
				room:setPlayerMark(player, self:objectName().."-Clear", 1)
			end
		end
		return false
...
heg_fofl_2_chengong:addSkill(heg_fofl_2_yinpan)
heg_fofl_2_chengong:addSkill("heg_fofl_yumou")
heg_fofl_2_chengong:addSkill("heg_fofl_yumouClear")

heg_fofl_2_jinsimayi = sgs.General(extension_fakeoffline,  "heg_fofl_2_jinsimayi", "jin", 3)

heg_fofl_2_quanbianCard = sgs.CreateSkillCard{
	name = "heg_fofl_2_quanbian",
	target_fixed = true,

	will_throw = false,
	about_to_use = function(self,room,use)
		local moves = sgs.CardsMoveList()
		local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_RECYCLE,use.from:objectName(),"heg_fofl_2_quanbian","")
...
heg_fofl_2_quanbianVS = sgs.CreateViewAsSkill{
	name = "heg_fofl_2_quanbian",
	n = 2,
	expand_pile = "#heg_fofl_2_quanbian",
	view_filter = function(self, selected, to_select)
		if #selected >= 2 then return false end
		if #selected > 0 then
			local first_id = selected[1]
			if sgs.Self:getPileName(first_id) ~= "#heg_fofl_2_quanbian" then
				return to_select:objectName() == sgs.Self:objectName() and sgs.Self:handCards():contains(first_id)
			else
				return sgs.Self:getPileName(to_select:getEffectiveId()) == "#heg_fofl_2_quanbian"
			end
		end
        return true
	end,
	view_as = function(self, cards)
		if #cards > 1 then
			...
			for _,c in sgs.list(cards)do
				if sgs.Self:getPileName(c:getEffectiveId())=="#heg_fofl_2_quanbian"
				then x = x+1 else n = n+1 end
			end
			if x~=n then return end
			local sc = heg_fofl_2_quanbianCard:clone()
			for _,c in sgs.list(cards)do
				sc:addSubcard(c)
			end
			return sc
	...
	enabled_at_response = function(self,player,pattern)
		return string.find(pattern,"heg_fofl_2_quanbian")
	end,
	enabled_at_play = function(self, player)
		return false
	end, 
...
heg_fofl_2_quanbian = sgs.CreateTriggerSkill {
	name = "heg_fofl_2_quanbian",
	view_as_skill = heg_fofl_2_quanbianVS,
	events = {sgs.CardUsed, sgs.CardResponded},
	on_trigger = function(self, event, player, data, room)
		local card = nil
		if event == sgs.CardUsed then
			...
			card = use.card
		elseif event == sgs.CardResponded then
			local response = data:toResponse()
			card = response.m_card
		end
		if card and player:hasSkill(self:objectName()) and not card:isKindOf("SkillCard") and player:getPhase() ~= sgs.Player_NotActive then
			local ids = room:getNCards(player:getMaxHp(), false)
			room:notifyMoveToPile(player,ids,"heg_fofl_2_quanbian")
			room:askForUseCard(player, "@@heg_fofl_2_quanbian", "@heg_fofl_2_quanbian", -1, sgs.Card_MethodNone)
			room:notifyMoveToPile(player,ids,"heg_fofl_2_quanbian",sgs.Player_DrawPile,false)
		end
	end
}


heg_fofl_zhouting_buff = sgs.CreateTargetModSkill{
	name = "#heg_fofl_zhouting_buff",
	distance_limit_func = function(self, from, card)
		if card and table.contains(card:getSkillNames(), "heg_fofl_zhouting") then
			return 1000
		else
			return 0
		end
...
heg_fofl_zhoutingCard = sgs.CreateSkillCard {
	name = "heg_fofl_zhouting",
	target_fixed = true,
	will_throw = false,
	on_use = function(self, room, source, targets)
		room:removePlayerMark(source, "@heg_fofl_zhouting")
		room:doSuperLightbox(source, "heg_fofl_zhouting")
		local list = room:getNCards(source:getMaxHp(), false)
		local log = sgs.LogMessage()
		log.type = "$ViewDrawPile"
		log.from = source
		...
		while source:isAlive() and not list:isEmpty() do
			...
			if not list:isEmpty() then
				room:notifyMoveToPile(source, list, "heg_fofl_zhouting", sgs.Player_DrawPile, true)
				local use_card = room:askForUseCard(source, "@@heg_fofl_zhouting!", "@heg_fofl_zhouting:"..card:objectName())
				room:notifyMoveToPile(source, list, "heg_fofl_zhouting", sgs.Player_DrawPile, false)
				if not use_card then
					break
				end
			end
...
heg_fofl_zhoutingVS = sgs.CreateViewAsSkill {
	name = "heg_fofl_zhouting",
	n = 1,
	limit_mark = "@heg_fofl_zhouting",
	view_filter = function(self, selected, to_select)
		if sgs.Sanguosha:getCurrentCardUsePattern() == "@@heg_fofl_zhouting!" then
			return sgs.Self:getPile("#heg_fofl_zhouting"):contains(to_select)
		end
		return false
	end,
	view_as = function(self, cards)
		if sgs.Sanguosha:getCurrentCardUsePattern() == "@@heg_fofl_zhouting!" then
			if #cards == 1 then
				local dc = sgs.Sanguosha:getCard(cards[1]:getEffectiveId())
				dc:setSkillName("heg_fofl_zhouting")
				return dc
			end
		else
			local skillcard = heg_fofl_zhoutingCard:clone()
			return skillcard
		end
		return nil
	end,
	enabled_at_play = function(self, player)
		return player:getMark("@heg_fofl_zhouting") > 0
	end,
	enabled_at_response = function(self, player, pattern)
		return pattern == "@@heg_fofl_zhouting!"
	end
}

heg_fofl_zhouting = sgs.CreateTriggerSkill{
	name = "heg_fofl_zhouting",
	view_as_skill = heg_fofl_zhoutingVS,
	events = {sgs.Death},
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		if (event == sgs.Death) then
			...
			and table.contains(death.damage.card:getSkillNames(),self:objectName()) then
				if player:getMark("@heg_fofl_zhouting") == 0 then
					room:setPlayerMark(player, "@heg_fofl_zhouting", 1)
				end
			end
		end
	end,
...
heg_fofl_2_jinsimayi:addSkill("jinyingshi")
heg_fofl_2_jinsimayi:addSkill(heg_fofl_2_quanbian)
heg_fofl_2_jinsimayi:addSkill(heg_fofl_zhouting)
heg_fofl_2_jinsimayi:addSkill(heg_fofl_zhouting_buff)
extension_fakeoffline:insertRelatedSkills("heg_fofl_zhouting", "#heg_fofl_zhouting_buff")

heg_fofl_zhanghuyuechen = sgs.General(extension_fakeoffline,  "heg_fofl_zhanghuyuechen", "jin", 4)
heg_fofl_xijue = sgs.CreateTriggerSkill{
	name = "heg_fofl_xijue",
	waked_skills = "tenyeartuxi,heg_fofl_xiaoguo",
	events = { sgs.DrawNCards, sgs.AfterDrawNCards, sgs.EventPhaseStart, sgs.GameStart, sgs.EventPhaseChanging, sgs.DamageCaused },
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		if event == sgs.DrawNCards and player:hasSkill(self:objectName()) then
			local draw = data:toDraw()
			if draw.reason ~= "draw_phase" then return false end
			local x = math.max(math.min(player:getMark("&heg_fofl_xijue"), player:getMaxHp()), 0)
			draw.num = x
			data:setValue(draw)
			if player:getMark("&heg_fofl_xijue") > 0 then
				local tuxi = sgs.Sanguosha:getTriggerSkill("tenyeartuxi")
				local can_invoke = false
				local other_players = room:getOtherPlayers(player)
				for _, q in sgs.qlist(other_players) do
				...
				if tuxi and can_invoke and room:askForSkillInvoke(player, self:objectName(), data) then
					room:setPlayerMark(player, "tenyeartuxi", draw.num)
					room:notifySkillInvoked(player,self:objectName())
					tuxi:trigger(event,room,player,data)
					room:setPlayerMark(player, "tuxi", 0)
					room:removePlayerMark(player, "&heg_fofl_xijue", 1)
				end
			end
		elseif event == sgs.AfterDrawNCards and player:hasSkill(self:objectName()) then
			local draw = data:toDraw()
		...
		elseif event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Start then
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if p:objectName() ~= player:objectName() and p:getMark("&heg_fofl_xijue") > 0 then
					local x = p:getMark("&heg_fofl_xijue")
					local card = room:askForDiscard(p, self:objectName(), x, 1, false, "@heg_fofl_xiaoguo", true, "BasicCard")
					if card then
						local equips = player:getEquips():length()
						local max = math.min(card:subcardsLength(), equips)
						if p:canDiscard(player, "e") then
		...
		elseif event == sgs.GameStart then
			if player:hasSkill(self:objectName()) then
				room:setPlayerMark(player, "&heg_fofl_xijue", 2)
			end
		elseif event == sgs.DamageCaused then
			local damage = data:toDamage()
			if damage.from and damage.from:objectName() == player:objectName() and player:hasSkill(self:objectName()) then
				room:addPlayerMark(player, "heg_fofl_xijue-Clear", 1)
			end
		elseif event == sgs.EventPhaseChanging then
			local change = data:toPhaseChange()
			if change.to == sgs.Player_NotActive and player:hasSkill(self:objectName()) and player:getMark("heg_fofl_xijue-Clear") > 0 then
				room:addPlayerMark(player, "&heg_fofl_xijue")
			end
		end
		return false
	end,
...
heg_fofl_zhanghuyuechen:addSkill(heg_fofl_xijue)

heg_fofl_2_yanghuiyu = sgs.General(extension_fakeoffline,  "heg_fofl_2_yanghuiyu", "jin", 3, false)

heg_fofl_ciwei = sgs.CreateTriggerSkill{
	name = "heg_fofl_ciwei",
	events = {sgs.CardUsed, sgs.CardFinished},
	on_trigger = function(self, event, player, data, room)
		local use = data:toCardUse()
		if event == sgs.CardUsed then
			if use.card and not use.card:isKindOf("SkillCard") and use.from:getMark("used-Clear") % 2 == 0 then
				for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					if p:objectName() ~= player:objectName() and p:canDiscard(p, "he") then
						if room:askForDiscard(p, self:objectName(), 1, 1, true, true) then
							use.to = sgs.SPlayerList()
							data:setValue(use)
							room:setCardFlag(use.card, "heg_fofl_ciwei")
							room:setCardFlag(use.card, "heg_fofl_ciwei"..p:objectName())
						end
					end
				end
			end
		elseif event == sgs.CardFinished then
			if use.card and not use.card:isKindOf("SkillCard") and use.card:hasFlag("heg_fofl_ciwei") then
				for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					if use.card:hasFlag("heg_fofl_ciwei"..p:objectName()) then
						if use.card:isKindOf("EquipCard") then
							local equip = sgs.Sanguosha:getCard(use.card:getEffectiveId())
							room:obtainCard(p, equip, false)
							local use_struct = sgs.CardUseStruct()
							use_struct.card = equip
							use_struct.from = p
							room:useCard(use_struct)
							break
						end
					end
				end
...
heg_fofl_huirong = sgs.CreateTriggerSkill{
	name = "heg_fofl_huirong",
	events = {sgs.Damaged},
	frequency = sgs.Skill_Limited,
	limit_mark = "@heg_fofl_huirong",
	on_trigger = function(self, event, player, data, room)
		local damage = data:toDamage()
		if damage.to and damage.to:objectName() == player:objectName() and player:getPhase() ~= sgs.Player_NotActive and player:getMark("@heg_fofl_huirong") > 0 then
			local targets = sgs.SPlayerList()
			for _, p in sgs.qlist(room:getAlivePlayers()) do
			...
			if targets:isEmpty() then return false end
			local target = room:askForPlayerChosen(player, targets, self:objectName(), "heg_fofl_huirong-invoke", true, true)
			if target then
				room:removePlayerMark(player, "@heg_fofl_huirong", 0)
				local x = math.abs(target:getHp() - target:getHandcardNum())
				if target:getHp() > target:getHandcardNum() then
					target:drawCards(x, self:objectName())
				elseif target:getHp() < target:getHandcardNum() then
					room:askForDiscard(target, self:objectName(), x, x, false, true)
				end
			end
		end
...
heg_fofl_2_yanghuiyu:addSkill(heg_fofl_ciwei)
heg_fofl_2_yanghuiyu:addSkill(heg_fofl_huirong)

heg_fofl_wangyuanji = sgs.General(extension_fakeoffline,  "heg_fofl_wangyuanji", "jin", 3, false)

heg_fofl_yanxiCard = sgs.CreateSkillCard{
	name = "heg_fofl_yanxi",
	target_fixed = false,
	will_throw = false,
	filter = function(self, targets, to_select)
	...
	on_use = function(self, room, source, targets)
		local target = targets[1]
		local card_ids = room:showDrawPile(source, 2, self:objectName(), false, true)
		...
		room:clearAG()
		if card_id >= 0 then
			local color_or_type = nil
			local chosen_card = sgs.Sanguosha:getCard(card_id)
			local to_obtain = sgs.IntList()
			local Istype = sgs.IntList()
			local Iscolor = sgs.IntList()
			for _, id in sgs.qlist(card_ids) do
				local c = sgs.Sanguosha:getCard(id)
				if (c:getColor() == chosen_card:getColor()) then
					Iscolor:append(id)
				end
				if (c:getTypeId() == chosen_card:getTypeId()) then
					Istype:append(id)
				end
			end
			if Istype:length() > 0 and Iscolor:length() > 0 then
				color_or_type = room:askForChoice(source, self:objectName(), "color+type", data)
			elseif Istype:length() > 0 then
				color_or_type = "type"
			elseif Iscolor:length() > 0 then
				color_or_type = "color"
			end
			for _, id in sgs.qlist(card_ids) do
				local c = sgs.Sanguosha:getCard(id)
				if (c:getColor() == chosen_card:getColor()) and color_or_type == "color" then
					to_obtain:append(id)
				end
				if (c:getTypeId() == chosen_card:getTypeId()) and color_or_type == "type" then
					to_obtain:append(id)
				end
			end

			if to_obtain:isEmpty() then return end
			room:obtainCard(source, to_obtain, false)
			room:addMaxCards(source, to_obtain:length(), true)
			local to_obtain = sgs.IntList()
			for _, id in sgs.qlist(card_ids) do
				if not to_obtain:contains(id) then
					to_obtain:append(id)
				end
			end
			if to_obtain:isEmpty() then return end
			room:obtainCard(target, to_obtain, false)
		end
	end,
}

heg_fofl_yanxi = sgs.CreateZeroCardViewAsSkill{
	name = "heg_fofl_yanxi",
	view_as = function()
		return heg_fofl_yanxiCard:clone()
	end,
	enabled_at_play = function(self, player)
		return not player:hasUsed("#heg_fofl_yanxi")
	end,
	enabled_at_response = function(self, player, pattern)
		return pattern == "@@heg_fofl_yanxi"
	end,
}

heg_fofl_shiren = sgs.CreateTriggerSkill{
	name = "heg_fofl_shiren",
	events = {sgs.Damaged},
	frequency = sgs.Skill_Limited,
	limit_mark = "@heg_fofl_shiren",
	on_trigger = function(self, event, player, data, room)
		local damage = data:toDamage()
		if damage.to and damage.to:objectName() == player:objectName() and player:getPhase() ~= sgs.Player_NotActive and player:getMark("@heg_fofl_shiren") > 0 then
			if room:askForUseCard(player, "@@heg_fofl_yanxi", "@heg_fofl_shiren", -1, sgs.Card_MethodNone) then
				room:removePlayerMark(player, "@heg_fofl_shiren", 0)
			end
		end
		return false
	end,
...
heg_fofl_wangyuanji:addSkill(heg_fofl_yanxi)
heg_fofl_wangyuanji:addSkill(heg_fofl_shiren)


heg_fofl_shenguanyu = sgs.General(extension_fakeoffline,  "heg_fofl_shenguanyu", "god", 5)

heg_fofl_wushen = sgs.CreateFilterSkill{
	name = "heg_fofl_wushen", 
	view_filter = function(self,to_select)
		local room = sgs.Sanguosha:currentRoom()
		local place = room:getCardPlace(to_select:getEffectiveId())
		return (to_select:getSuit() == sgs.Card_Heart) and (place == sgs.Player_PlaceHand) and not to_select:isKindOf("BasicCard")
	end,
	view_as = function(self, originalCard)
		local slash = sgs.Sanguosha:cloneCard("slash", originalCard:getSuit(), originalCard:getNumber())
		slash:setSkillName(self:objectName())
...
heg_fofl_wushen_buff = sgs.CreateTargetModSkill{
	name = "#heg_fofl_wushen_buff",
	distance_limit_func = function(self, from, card)
		if from:hasSkill("heg_fofl_wushen") and (card:getSuit() == sgs.Card_Heart) then
			return 1000
		else
			return 0
		end
	...
	residue_func = function(self, player, card )
		if player:hasSkill("heg_fofl_wushen") and (card:getSuit() == sgs.Card_Heart) then
			return 999
		end
	end,
}
...
heg_fofl_shenguanyu:addSkill(heg_fofl_wushen)
heg_fofl_shenguanyu:addSkill(heg_fofl_wushen_buff)
extension_fakeoffline:insertRelatedSkills("heg_fofl_wushen", "#heg_fofl_wushen_buff")
heg_fofl_shenguanyu:addSkill("kechengguanjue")


heg_fofl_shenzhouyu = sgs.General(extension_fakeoffline,  "heg_fofl_shenzhouyu", "god", 4)

heg_fofl_yeyanCard = sgs.CreateSkillCard{
	name = "heg_fofl_yeyan",
	target_fixed = false,
	will_throw = true,
	filter = function(self, targets, to_select)
	...
	on_use = function(self, room, source, targets)
		room:removePlayerMark(source, "@heg_fofl_yeyan", 0)
		for _, target in ipairs(targets) do
			local damage = sgs.DamageStruct()
			damage.from = source
			damage.to = target
...
heg_fofl_yeyan = sgs.CreateViewAsSkill{
	name = "heg_fofl_yeyan",
	n = 0,
	limit_mark = "@heg_fofl_yeyan",
	view_as = function()
		return heg_fofl_yeyanCard:clone()
	end,
	enabled_at_play = function(self, player)
		return player:getMark("@heg_fofl_yeyan") > 0
	end,
}


heg_fofl_shenzhouyu:addSkill("qinyin")
heg_fofl_shenzhouyu:addSkill(heg_fofl_yeyan)


heg_fofl_shencaocao = sgs.General(extension_fakeoffline,  "heg_fofl_shencaocao", "god", 3)

heg_fofl_guixin = sgs.CreateTriggerSkill{
	name = "heg_fofl_guixin",
	events = {sgs.Damaged},
	frequency = sgs.Skill_NotFrequent,
	on_trigger = function(self, event, player, data, room)
		local players = room:getOtherPlayers(player)
		local damage = data:toDamage()
		if player:getMark("heg_fofl_guixin".."-Self"..sgs.Player_RoundStart.."Clear") < 1 and player:askForSkillInvoke(self:objectName(), data) then
			room:addPlayerMark("heg_fofl_guixin".."-Self"..sgs.Player_RoundStart.."Clear")
...
heg_fofl_shencaocao:addSkill("feiying")


heg_fofl_shenzhaoyun = sgs.General(extension_fakeoffline,  "heg_fofl_shenzhaoyun", "god", 1)

heg_fofl_juejing_buff = sgs.CreateMaxCardsSkill{
	name = "#heg_fofl_juejing_buff",
	extra_func = function(self, target)
		local n = 0
		if target:hasSkill("heg_fofl_juejing") then n = n + 2 end
		return n
	end,
}
heg_fofl_juejing = sgs.CreateTriggerSkill{
    name = "heg_fofl_juejing",
	frequency = sgs.Skill_Compulsory,
	events = {sgs.EnterDying},
	on_trigger = function(self, event, player, data, room)
		room:sendCompulsoryTriggerLog(player, self:objectName(), true, true)
...
heg_fofl_longhun = sgs.CreateViewAsSkill{
	name = "heg_fofl_longhun" ,
	n = 1 ,
	view_filter = function(self, selected, to_select)
		if (#selected >= 1) or to_select:hasFlag("using") then return false end
		if sgs.Self:getMark("heg_fofl_longhun"..to_select:getSuitString().."-Clear") > 0 then return nil end
		if sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_PLAY then
	...
	view_as = function(self, cards)
		...
		local card = cards[1]
		local new_card = nil
		if sgs.Self:getMark("heg_fofl_longhun"..card:getSuitString().."-Clear") > 0 then return nil end
	...
	enabled_at_response = function(self, player, pattern)
		local suits = {Slash = "diamond", Jink = "club", Peach = "heart", Nullification = "spade"}
		for _, pt in sgs.list(pattern:split("+")) do
			for kind, suit in pairs(suits) do
				if dummyCard(pt):isKindOf(kind) and player:getMark("heg_fofl_longhun" .. suit .. "-Clear") == 0 then
					if kind ~= "Peach" or not player:hasFlag("Global_PreventPeach") then
						return true
					end
				end
			end
...
heg_fofl_shenzhaoyun:addSkill(heg_fofl_juejing)
heg_fofl_shenzhaoyun:addSkill(heg_fofl_juejing_buff)
extension_fakeoffline:insertRelatedSkills("heg_fofl_juejing", "#heg_fofl_juejing_buff")
heg_fofl_shenzhaoyun:addSkill(heg_fofl_longhun)

heg_fofl_shensimayi = sgs.General(extension_fakeoffline,  "heg_fofl_shensimayi", "god", 4)
heg_fofl_jilueClear = sgs.CreateTriggerSkill{
	name = "#heg_fofl_jilueClear",
	events = {sgs.KingdomChange},
	on_trigger = function(self, event, player, data, room)
		local kingdom = data:toString()
		if player:getKingdom() ~= kingdom then
			room:detachSkillFromPlayer(player, "fangzu")
			room:detachSkillFromPlayer(player, "jizhi")
			room:detachSkillFromPlayer(player, "zhiheng")
			room:detachSkillFromPlayer(player, "wansha")
		end
	end
}

heg_fofl_jilue = sgs.CreateTriggerSkill{
	name = "heg_fofl_jilue",
	events = {sgs.GameStart, sgs.KingdomChanged},
	waked_skills = "guicai",
	on_trigger = function(self, event, player, data, room)
		if event == sgs.GameStart then
		...
		if player:getKingdom() == "wei" then
			room:handleAcquireDetachSkills(player, "fangzu")
		elseif player:getKingdom() == "shu" then
			room:handleAcquireDetachSkills(player, "jizhi")
		elseif player:getKingdom() == "wu" then
			room:handleAcquireDetachSkills(player, "zhiheng")
		elseif player:getKingdom() == "qun" then
			room:handleAcquireDetachSkills(player, "wansha")
		else
			room:handleAcquireDetachSkills(player, "fangzu+jizhi+zhiheng+wansha")
			if player:getMark("heg_fofl_jilue") == 0 then
				room:setPlayerMark(player, "heg_fofl_jilue", 1)
				room:loseMaxHp(player, self:objectName())
			end
		end
	end
}

heg_fofl_shensimayi:addSkill(heg_fofl_jilueClear)
heg_fofl_shensimayi:addSkill(heg_fofl_jilue)
extension_fakeoffline:insertRelatedSkills("heg_fofl_jilue", "#heg_fofl_jilueClear")
heg_fofl_shensimayi:addSkill("lianpo")


heg_fofl_shenzhangliao = sgs.General(extension_fakeoffline,  "heg_fofl_shenzhangliao", "god", 4)

heg_fofl_duorui_record = sgs.CreateTriggerSkill{
	name = "heg_fofl_duorui",
	frequency = sgs.Skill_Compulsory,
...
heg_fofl_duorui = sgs.CreateTriggerSkill{
	name = "heg_fofl_duorui",
	frequency = sgs.Skill_Compulsory,
	events = {sgs.Damage},
	on_trigger = function(self, event, player, data, room)
		local damage = data:toDamage()
		if not damage.to:isAllNude() and damage.to:objectName() ~= player:objectName() and player:getPhase() == sgs.Player_Play  then
			 local dummy = sgs.Sanguosha:cloneCard("slash")
			dummy:deleteLater()
			local flags = {}
			...
			room:addPlayerMark(player, "&heg_fofl_duorui-Clear")
			if player:getMark("&heg_fofl_duorui-Clear") > 2 and player:getMark("heg_fofl_duorui_used-Clear") == 0 then
				room:addPlayerMark(player, "heg_fofl_duorui_used-Clear")
				for _, p in sgs.qlist(room:getAllPlayers()) do
					if p:getMark("heg_fofl_duorui_damage-Clear") > 0 then
						--TODO
					end
				end
			end
...
heg_fofl_zhiti_buff = sgs.CreateTargetModSkill{
	name = "#heg_fofl_zhiti_buff",
	residue_func = function(self, player, card )
		if player:hasSkill("heg_fofl_zhiti") then
			local x = 0
			for _, p in sgs.qlist(player:getAliveSiblings()) do
				if p:isWounded() then
					x = x + 1
...
heg_fofl_zhiti = sgs.CreateTriggerSkill{
	name = "heg_fofl_zhiti",
	frequency = sgs.Skill_Compulsory,
	events = {sgs.DrawNCards},
	on_trigger = function(self, event, player, data, room)
...
heg_fofl_shenzhangliao:addSkill(heg_fofl_duorui_record)
heg_fofl_shenzhangliao:addSkill(heg_fofl_duorui)
extension_fakeoffline:insertRelatedSkills("heg_fofl_duorui", "#heg_fofl_duorui_record")
heg_fofl_shenzhangliao:addSkill(heg_fofl_zhiti)
heg_fofl_shenzhangliao:addSkill(heg_fofl_zhiti_buff)
extension_fakeoffline:insertRelatedSkills("heg_fofl_zhiti", "#heg_fofl_zhiti_buff")


heg_fofl_liuyan = sgs.General(extension_fakeoffline,  "heg_fofl_liuyan", "qun", 3)


heg_fofl_tushe = sgs.CreateTriggerSkill{
	name = "heg_fofl_tushe",
	frequency = sgs.Skill_Compulsory,
	events = {sgs.TargetSpecifing},
	on_trigger = function(self, event, player, data, room)
		local use = data:toCardUse()
		if use.card and not use.card:isKindOf("SkillCard") and use.from:objectName() == player:objectName() and use.to:length() > 0 then
			local can_invoke = false
			for _, p in sgs.qlist(use.to) do
				if p:objectName() ~= player:objectName() then can_invoke = true break end
			end
			if not can_invoke then return false end
			local x = 0
			for _, p in sgs.qlist(room:getAlivePlayers()) do
...
heg_fofl_limuCard = sgs.CreateSkillCard{
	name = "heg_fofl_limu",
	target_fixed = true,
	will_throw = false,
	about_to_use = function(self, room, use)
		...
		recover.reason = self:objectName()
		room:recover(use.from, recover, true)
		room:addPlayerMark(use.from, "&heg_fofl_limu-Clear", c:getNumber())
		room:addSlashCishu(use.from, c:getNumber(), true)
		
	end
}
heg_fofl_limu = sgs.CreateOneCardViewAsSkill{
	name = "heg_fofl_limu",
	filter_pattern = ".|diamond|.|.",
	response_or_use = true,
	view_as = function(self, card)
		local lm = heg_fofl_limuCard:clone()
...
heg_fofl_liuyan:addSkill(heg_fofl_tushe)
heg_fofl_liuyan:addSkill(heg_fofl_limu)

heg_fofl_weiyan = sgs.General(extension_fakeoffline,  "heg_fofl_weiyan", "shu", 4)

heg_fofl_kuanggu = sgs.CreateTriggerSkill{
    name = "heg_fofl_kuanggu",
    ...
    on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed then
			local use = data:toCardUse()
			if use.card and use.to:length() == 1 and use.card:isDamageCard() then
				local x = player:getLostHp() + 1
				local extra_targets = room:getCardTargets(player, use.card, use.to)
				if not extra_targets:isEmpty() then
					room:setTag("heg_fofl_kuanggu", data)
					local others = room:askForPlayersChosen(to, extra_targets, self:objectName(), 0, 2, "@heg_fofl_kuanggu", true, true)
					room:removeTag("heg_fofl_kuanggu")
					if others and others:length() > 0 then
						for i,p in sgs.list(log.to)do
							use.to:append(p)
						end
...
heg_fofl_weiyan:addSkill(heg_fofl_kuanggu)
...
heg_fofl_shichangshi:setGender(sgs.General_Sexless)

local function heg_jiedang(player, room, cs)
	room:setPlayerMark(player,"&chang_shi",cs:length())
	local cs2s,cssk,taunts = {}, {}, {}
	for _, s in ipairs(player:tag("heg_jiedangSkills").toStringList()) do
		if s:startsWith("-") then
			table.insert(cssk, "-" .. s)
		end
	end
	local cs1 = cs[math.random(1, #cs)]
	player:setAvatarIcon(cs1)
	table.insert(cssk, ChangshiSkills[cs1])
	table.remove(cs, cs1)
	
	player:setTag("heg_jiedangSkills", sgs.QVariant(table.concat(cssk, ",")))
	room:handleAcquireDetachSkills(player,cssk,true)
	player:setTag("heg_jiedangCards", sgs.QVariant(table.concat(cs, ",")))
	room:setPlayerMark(player,"&chang_shi",cs:length())
end

heg_fofl_danggu = sgs.CreateTriggerSkill{
	name = "heg_fofl_danggu",
	events = {sgs.GameStart, sgs.Revived},
	frequency = sgs.Skill_Compulsory,
	on_trigger = function(self, event, player, data, room)
		if event == sgs.GameStart then
			room:sendCompulsoryTriggerLog(player, self:objectName())
			local cs = {"cs_zhangrang", "cs_zhaozhong", "cs_sunzhang", "cs_bilan", "cs_xiayun", "cs_hanli", "cs_lisong", "cs_duangui", "cs_guosheng", "cs_gaowang"}
			for _, g in ipairs(cs) do
				room:doAnimate(1, player:objectName(), g)
				room:addPlayerMark(player, "&chang_shi")
				room:getThread():delay(233)
			end
			player:setTag("heg_jiedangCards", sgs.QVariant(table.concat(cs, ",")))
			heg_jiedang(player, room, cs)
		elseif event == sgs.Revived then
			local cs = player:tag("heg_jiedangCards"):toStringList()
			if cs:isEmpty() then return false end
			room:sendCompulsoryTriggerLog(player, self:objectName())
			heg_jiedang(player, room, cs)
			player:drawCards(1, self:objectName())
		end
...
heg_fofl_mowang = sgs.CreateTriggerSkill{
	name = "heg_fofl_mowang",
	events = {sgs.BeforeGameOverJudge, sgs.EventPhaseChanging, sgs.EventPhaseStart},
	frequency = sgs.Skill_Compulsory,
	on_trigger = function(self, event, player, data, room)
		if event == sgs.BeforeGameOverJudge then
			local death = data:toDeath()
			if death.who:objectName() == player:objectName() and player:getMaxHp() > 0 and player:hasSkill(self:objectName()) and not player:getTag("ChangshiCards"):toStringList():isEmpty() then
				room:sendCompulsoryTriggerLog(player, self)
				room:restPlayer(player, self, true)
				local csai = player:property("avatarIcon").toString()
				if not csai:isEmpty() then player:setAvatarIcon(csai .. "2") end
		...
		elseif event == sgs.EventPhaseStart then
			if player:getPhase() ~= sgs.Player_RoundStart then return false end
			for _, p in sgs.qlist(room:getAllPlayers(true)) do
				if not p:property("RestPlayer"):toBool() then continue end
				if not p:getTag("MowangRestActive"):toBool() then continue end
				if p:getTag("RestTurn"):toInt() < room:getTag("TurnLengthCount"):toInt() then
					room:sendCompulsoryTriggerLog(p, self)
					room:unrestPlayer(p, true)
					p:setTag("MowangRestActive", nil)
					p:setTag("RestTurn", nil)
					break
				end
			end
		elseif event == sgs.EventPhaseChanging then
			local change = data:toPhaseChange()
			if change.to ~= sgs.Player_NotActive or player:isDead() or not player:hasSkill(self) then return false end
			room:sendCompulsoryTriggerLog(player, self)
			room:killPlayer(player)
		end
		return false
...
heg_fofl_shichangshi:addSkill(heg_fofl_danggu)
heg_fofl_shichangshi:addSkill(heg_fofl_mowang)

heg_fofl_hankui = sgs.General(extension_fakeoffline,  "heg_fofl_hankui", "qun", 0, false, true)
heg_fofl_hankui:setGender(sgs.General_Sexless)

heg_fofl_xiaoluCard = sgs.CreateSkillCard{
	name = "heg_fofl_xiaolu",
	target_fixed = true,
	will_throw = false,
	on_use = function(self, room, source, targets)
		source:drawCards(2, self:objectName())
		local choicelist = {}
		if source:canDiscard(source, "he") then table.insert(choicelist, "discard") end
		if not source:isNude() and room:getOtherPlayers(source):length() > 0 then table.insert(choicelist, "give") end
		if #choicelist == 0 then return end
		local choice = room:askForChoice(source, self:objectName(), table.concat(choicelist, "+"))
		if choice == "discard" then
			room:askForDiscard(source, self:objectName(), 1, 2, false, true)
		elseif choice == "give" then
			local to_give = room:askForPlayerChosen(source, room:getOtherPlayers(source), self:objectName(), "@heg_fofl_xiaolu-give", true, true)
			local cards = room:askForExchange(source, self:objectName(), 2, 0, "@heg_fofl_xiaolu-give", true, true)
			if cards and cards:length() > 0 then
				room:obtainCard(to_give, cards)
			end
		end
	end,
}
heg_fofl_xiaolu = sgs.CreateZeroCardViewAsSkill{
	name = "heg_fofl_xiaolu",
	view_as = function()
		return heg_fofl_xiaoluCard:clone()
	end,
	enabled_at_play = function(self, player)
		return not player:hasUsed(self:objectName())
	end,
}
heg_fofl_hankui:addSkill(heg_fofl_xiaolu)

heg_fofl_gaowang = sgs.General(extension_fakeoffline,  "heg_fofl_gaowang", "qun", 0, false, true)
heg_fofl_gaowang:setGender(sgs.General_Sexless)

heg_fofl_miaoyu = sgs.CreateViewAsSkill{
	name = "heg_fofl_miaoyu",
	n = 1,
	view_filter = function(self, selected, to_select)
...
heg_fofl_miaoyu_buff = sgs.CreateTargetModSkill{
	name = "#heg_fofl_miaoyu_buff",
	pattern = "Slash",
	residue_func = function(self, player, card)
		if player:hasSkill("heg_fofl_miaoyu") and card:isKindOf("Slash") then
			return 999
		end
		return 0
	end,
...
heg_fofl_gaowang:addSkill(heg_fofl_miaoyu)
heg_fofl_gaowang:addSkill(heg_fofl_miaoyu_buff)
extension_fakeoffline:insertRelatedSkills("heg_fofl_miaoyu", "#heg_fofl_miaoyu_buff")

heg_fofl_caoying = sgs.General(extension_fakeoffline,  "heg_fofl_caoying", "wei", 4, false)

heg_fofl_lingren = sgs.CreateTriggerSkill{
	name = "heg_fofl_lingren",
	events = {sgs.TargetSpecifing},
	frequency = sgs.Skill_NotFrequent,
	on_trigger = function(self, event, player, data, room)
		local use = data:toCardUse()
		if not use.card or use.from:objectName() ~= player:objectName() or use.to:length() == 0 then return false end
		if not use.card:isDamageCard() then return false end
		local targets = sgs.SPlayerList()
		for _, p in sgs.qlist(use.to) do
			if p:getHandcardNum() <= player:getHandcardNum() then targets:append(p) end
		end
		if targets:isEmpty() then return false end
		local target = room:askForPlayerChosen(player, targets, self:objectName(), "heg_fofl_lingren-invoke", true, true)
		if target then
			local choice = room:askForChoice(player, self:objectName(), "draw+damage")
			if choice == "draw" then
				player:drawCards(2, self:objectName())
			elseif choice == "damage" then
				room:setCardFlag(use.card, "heg_fofl_lingren"..target:objectName())
			end
		end
		return false
	end,
...
heg_fofl_lingren_buff = sgs.CreateTriggerSkill{
	name = "#heg_fofl_lingren_buff",
	events = {sgs.DamageCaused},
	frequency = sgs.Skill_Compulsory,
	on_trigger = function(self, event, player, data, room)
		local damage = data:toDamage()
		if damage.card and damage.card:hasFlag("heg_fofl_lingren"..damage.to:objectName()) then
			damage.damage = damage.damage + 1
			data:setValue(damage)
		end
		return false
...
heg_fofl_fujian = sgs.CreateTriggerSkill{
	name = "heg_fofl_fujian",
	events = {sgs.EventPhaseProceeding},
	frequency = sgs.Skill_Frequent,
	on_trigger = function(self, event, player, data, room)
		if player:getPhase() == sgs.Player_Start or player:getPhase() == sgs.Player_Finish then
			local knownboth = sgs.Sanguosha:cloneCard("heg_known_both", sgs.Card_NoSuit, 0)
			knownboth:setSkillName("heg_fofl_fujian")
			knownboth:deleteLater()
			local targets = room:getCardTargets(player, knownboth)
			if targets:isEmpty() then return false end
			local others = room:askForPlayersChosen(player, targets, "heg_fofl_fujian", 1, 1+sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_ExtraTarget, player, knownboth), "heg_fofl_fujian-invoke", true, true)
			if others:isEmpty() then return false end
			if knownboth then
				local use = sgs.CardUseStruct()
				use.card = knownboth
...
heg_fofl_caoying:addSkill(heg_fofl_lingren)
heg_fofl_caoying:addSkill(heg_fofl_lingren_buff)
heg_fofl_caoying:addSkill(heg_fofl_fujian)
extension_fakeoffline:insertRelatedSkills("heg_fofl_lingren", "#heg_fofl_lingren_buff")

heg_fofl_caochun = sgs.General(extension_fakeoffline,  "heg_fofl_caochun", "wei", 4)

heg_fofl_shanjia = sgs.CreateTriggerSkill{
	name = "heg_fofl_shanjia",
	events = {sgs.EventPhaseStart},
	frequency = sgs.Skill_Frequent,
	on_trigger = function(self, event, player, data, room)
		...
		if x > 0 and room:askForSkillInvoke(player, self:objectName()) then
			player:drawCards(x, self:objectName())
			local discard = room:askForCard(player, "..", "@heg_fofl_shanjia-discard", sgs.QVariant(), sgs.Card_MethodDiscard)
			if discard and discard:length() > 0 then
				room:throwCard(discard, player)
				if discard:first():isKindOf("EquipCard") then
					local slash = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)
					slash:setSkillName(self:objectName())
					slash:deleteLater()
					room:setPlayerFlag(player, "InfinityAttackRange")
					local targets = room:getCardTargets(player, slash)
					room:setPlayerFlag(player, "-InfinityAttackRange")
					if targets:isEmpty() then return false end
					local others = room:askForPlayersChosen(player, targets, "heg_fofl_shanjia", 1, 1+sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_ExtraTarget, player, slash), "heg_fofl_shanjia-invoke", true, true)
					if others:isEmpty() then return false end
					if slash then
						local use = sgs.CardUseStruct()
						use.card = slash
...
heg_fofl_caochun:addSkill(heg_fofl_shanjia)

heg_fofl_zhangyao = sgs.General(extension_fakeoffline,  "heg_fofl_zhangyao", "wu", 4, false)

heg_fofl_yuanyuCard = sgs.CreateSkillCard{
	name = "heg_fofl_yuanyu",
	target_fixed = true,
	will_throw = false,
	on_use = function(self, room, source, targets)
		source:drawCards(1, self:objectName())
		if not source:isKongcheng() then
			local cards = room:askForCard(source, ".|.|.|hand", "@heg_fofl_yuanyu-put", sgs.QVariant(), sgs.Card_MethodNone)
			local card = sgs.Sanguosha:getCard(cards:first())
			source:addToPile("heg_fofl_yuanyu", card, false)
		end
	end,
}
heg_fofl_yuanyu = sgs.CreateZeroCardViewAsSkill{
	name = "heg_fofl_yuanyu",
	view_as = function()
		return heg_fofl_yuanyuCard:clone()
	end,
	enabled_at_play = function(self, player)
		return not player:hasUsed(self:objectName())
	end,
...
heg_fofl_xiyan = sgs.CreateTriggerSkill{
	name = "heg_fofl_xiyan",
	events = {sgs.Damaged},
	frequency = sgs.Skill_NotFrequent,
	on_trigger = function(self, event, player, data, room)
		local damage = data:toDamage()
		if damage.from and damage.from:isAlive() and damage.card then
			local color = damage.card:getColor()
			local same_color = false
			for _, id in sgs.qlist(player:getPile("heg_fofl_yuanyu")) do
				local c = sgs.Sanguosha:getCard(id)
				if c:getColor() == color then
					same_color = true
					break
				end
			end
			if same_color then
				local choice = room:askForChoice(damage.from, self:objectName(), "maxhandcard+forbid_basic")
				if choice == "maxhandcard" then
					room:addMaxCards(damage.from, -4, true)
				elseif choice == "forbid_basic" then
					room:setPlayerCardLimitation(damage.from, "use", "BasicCard", true)
				end
			end
		end
		return false
...
heg_fofl_zhangyao:addSkill(heg_fofl_yuanyu)
heg_fofl_zhangyao:addSkill(heg_fofl_xiyan)

heg_fofl_zhangxuan = sgs.General(extension_fakeoffline,  "heg_fofl_zhangxuan", "wu", 4, false)
	
heg_fofl_tongli = sgs.CreateTriggerSkill{
	name = "heg_fofl_tongli",
	events = {sgs.TargetSpecifing},
	frequency = sgs.Skill_Frequent,
	on_trigger = function(self, event, player, data, room)
		local use = data:toCardUse()
		...
		if player:getMark("heg_fofl_tongli-Clear") > 0 then return false end
		
		if room:askForSkillInvoke(player, self:objectName()) then
			room:addPlayerMark(player, "heg_fofl_tongli-Clear")
			room:showAllCards(player)
			local first_color = sgs.Sanguosha:getCard(player:getHandcards():first()):getColor()
			for _, id in sgs.qlist(player:getHandcards()) do
				if sgs.Sanguosha:getCard(id):getColor() ~= first_color then return false end
			end
			room:setCardFlag(use.card, "heg_fofl_tongli")
		end
		return false
	end,
}
heg_fofl_tongli_buff = sgs.CreateTriggerSkill{
	name = "#heg_fofl_tongli_buff",
	events = {sgs.CardEffected},
	frequency = sgs.Skill_Compulsory,
	on_trigger = function(self, event, player, data, room)
		local effect = data:toCardEffect()
		if effect.card and effect.card:hasFlag("heg_fofl_tongli") then
			effect.extra_effect = effect.extra_effect + 1
			data:setValue(effect)
		end
		return false
	end,
...
heg_fofl_shezang = sgs.CreateTriggerSkill{
	name = "heg_fofl_shezang",
	events = {sgs.EnterDying},
	frequency = sgs.Skill_Frequent,
	on_trigger = function(self, event, player, data, room)
		if player:objectName() ~= data:toDying().who:objectName() then return false end
		if player:getMark("&heg_fofl_shezang") == 0 and room:askForSkillInvoke(player, self:objectName()) then
			player:drawCards(4, self:objectName())
		end
		room:setPlayerMark(player, "&heg_fofl_shezang", 1)
		return false
	end,
}

heg_fofl_zhangxuan:addSkill(heg_fofl_tongli)
heg_fofl_zhangxuan:addSkill(heg_fofl_tongli_buff)
heg_fofl_zhangxuan:addSkill(heg_fofl_shezang)
extension_fakeoffline:insertRelatedSkills("heg_fofl_tongli", "#heg_fofl_tongli_buff")
...
sgs.LoadTranslationTable{
	...
	["heg_fofl_maihuo"] = "埋祸",
	[":heg_fofl_maihuo"] = "每回合每名角色限一次，一名角色成为非虚拟【杀】的目标后，你可以获得1枚祸并令此【杀】对其无效；攻击范围内包含你的其他角色出牌阶段开始时，其可以视为对你使用等量张不计入次数的【杀】，其他角色以此法对你造成伤害时，你移除副将并防止此伤害。",

	["heg_fofl_yangyan"] = "杨艳-国[官盗]",
    ["&heg_fofl_yangyan"] = "杨艳",
	["#heg_fofl_yangyan"] = "武元皇后",
	["illustrator:heg_fofl_yangyan"] = "",
	["~heg_fofl_yangyan"] = "",
	["information:heg_fofl_yangyan"] = "应变晋国战",
	["heg_fofl_xuanbei"] = "选备",
	...
	["@heg_fofl_caiwang-discard"] = "才望：选择一名其他角色弃置其一张牌",
	["heg_fofl_naxiang"] = "纳降",
	...
	[":heg_fofl_zhaosong2"] = "每局每项限一次：<font color='%arg1'>1.一名角色进入濒死状态时，你可令其回复体力至2点并摸一张牌；</font><font color='%arg2'>2.出牌阶段，你可观看一名角色手牌，然后可获得其区域内一张牌；</font><font color='%arg3'>3.一名角色使用杀指定唯一目标时，你可为此【杀】增加至多两个目标。</font>",
	["heg_fofl_lisi"] = "离思",
	...
	["heg_fofl_wuqiujian"] = "毌丘俭-国[官盗]",
    ["&heg_fofl_wuqiujian"] = "毌丘俭",
	["#heg_fofl_wuqiujian"] = "镌功铭征荣",
	["illustrator:heg_fofl_wuqiujian"] = "",
	["~heg_fofl_wuqiujian"] = "",
	["information:heg_fofl_wuqiujian"] = "群雄割据",
	["heg_fofl_zhengrong"] = "征荣",
	[":heg_fofl_zhengrong"] = "锁定技，当你造成或受到伤害时，若场上没有与受伤角色或伤害来源势力相同的其他角色，此伤害+1。",
	["heg_fofl_hongju"] = "鸿举",
	[":heg_fofl_hongju"] = "限定技，出牌阶段，你可以选择一名其他角色，然后令其以外的角色依次执行一次“军令”（你必须执行），不执行的角色本回合移出游戏。",
	--TODO
    ...
    ["~heg_fofl_yuejin"] = "",
    ["illustrator:heg_fofl_yuejin"] = "巴萨小马",
    ["information:heg_fofl_yuejin"] = "十年经典",
	["heg_fofl_xiaoguo"] = "骁果",
  	[":heg_fofl_xiaoguo"] = "其他角色的准备阶段，你可以弃置任意张基本牌，然后弃置其装备区等量的牌（不足全弃，无牌不弃），若你以此法弃置的基本牌多于装备牌，你摸一张牌并对其造成1点伤害。",
	["@heg_fofl_xiaoguo"] = "骁果：你可以弃置任意张基本牌",

	["heg_fofl_dingfeng"] = "丁奉-国[官盗]",
    ["&heg_fofl_dingfeng"] = "丁奉",
    ["#heg_fofl_dingfeng"] = "清侧重臣",
    ["~heg_fofl_dingfeng"] = "",
    ["illustrator:heg_fofl_dingfeng"] = "",
    ["information:heg_fofl_dingfeng"] = "十年经典",

	["heg_fofl_dianwei"] = "典韦-国[官盗]",
    ["&heg_fofl_dianwei"] = "典韦",
    ["#heg_fofl_dianwei"] = "古之恶来",
    ["~heg_fofl_dianwei"] = "",
    ["illustrator:heg_fofl_dianwei"] = "",
    ["information:heg_fofl_dianwei"] = "十年经典",

	["heg_fofl_luxun"] = "陆逊-国[官盗]",
    ["&heg_fofl_luxun"] = "陆逊",
    ["#heg_fofl_luxun"] = "擎天之柱",
    ["~heg_fofl_luxun"] = "",
    ["illustrator:heg_fofl_luxun"] = "",
    ["information:heg_fofl_luxun"] = "十年经典",
	["heg_fofl_duoshi"] = "度势",
	[":heg_fofl_duoshi"] = "每轮限一次，一名角色的出牌阶段，其可以询问你，若你同意，其将一张红色手牌当【以逸待劳】使用，然后若你为小势力角色，你可以令一名角色将X张手牌当【火烧连营】使用（X为此牌的目标数），此牌不能被响应。",
	["@heg_fofl_duoshi"] = "度势：你可以令一名角色将X张手牌当【火烧连营】使用",
	["@heg_fofl_duoshi-discard"] = "度势：将%src张手牌当【火烧连营】使用",

	["heg_fofl_taishici"] = "太史慈-国[官盗]",
    ["&heg_fofl_taishici"] = "太史慈",
    ["#heg_fofl_taishici"] = "笃烈之士",
    ["~heg_fofl_taishici"] = "",
    ["illustrator:heg_fofl_taishici"] = "",
    ["information:heg_fofl_taishici"] = "十年经典",
	["heg_fofl_hanzhan"] = "酣战",
	[":heg_fofl_hanzhan"] = "当你拼点后，没赢的角色可以获得与其拼点的角色装备区里的一张牌。",

	["heg_fofl_yanliangwenchou"] = "颜良＆文丑-国[官盗]",
    ["&heg_fofl_yanliangwenchou"] = "颜良文丑",
    ["#heg_fofl_yanliangwenchou"] = "虎狼兄弟",
    ["~heg_fofl_yanliangwenchou"] = "",
    ...
    ["&heg_fofl_caohong"] = "曹洪",
    ["#heg_fofl_caohong"] = "魏之福将",
    ["~heg_fofl_caohong"] = "",
    ["illustrator:heg_fofl_caohong"] = "",
    ["information:heg_fofl_caohong"] = "十年经典",
	["heg_fofl_huyuan"] = "护援",
	[":heg_fofl_huyuan"] = "你的回合内，当一张装备牌置入一名角色的装备区后，你可以弃置其距离1的另一名角色区域内的一张牌；结束阶段，你可以将一张装备牌置于一名角色的装备区。",

	["heg_fofl_xusheng"] = "徐盛-国[官盗]",
    ["&heg_fofl_xusheng"] = "徐盛",
	["#heg_fofl_xusheng"] = "江东的铁壁",
	["illustrator:heg_fofl_xusheng"] = "",
	["~heg_fofl_xusheng"] = "",
	["information:heg_fofl_xusheng"] = "十年经典",
	["@heg_fofl_yicheng-discard"] = "疑城：弃置%src张牌",
	["@heg_fofl_yicheng"] = "疑城：你可以使用一张装备牌",
	["heg_fofl_yicheng"] = "疑城",
	[":heg_fofl_yicheng"] = "一名角色成为【杀】的目标后，你可以令其摸一张牌，然后其可以使用一张装备牌。若如此做，其于当前回合结束时弃置X张牌（X为本回合你对其发动此技能的次数）。",

	["heg_fofl_dongzhuo"] = "董卓-国[官盗]",
    ["&heg_fofl_dongzhuo"] = "董卓",
	["#heg_fofl_dongzhuo"] = "魔王",
	["illustrator:heg_fofl_dongzhuo"] = "",
	["~heg_fofl_dongzhuo"] = "",
	["information:heg_fofl_dongzhuo"] = "十年经典",
	["heg_fofl_baoling"] = "暴凌",
	[":heg_fofl_baoling"] = "觉醒技，出牌阶段结束时，若你本局游戏发动过“横征”，你移除副将，然后加3点体力上限并回复3点体力，获得“崩坏”。",
	["heg_fofl_benghuai"] = "崩坏",
	[":heg_fofl_benghuai"] = "结束阶段开始时，若你的体力值不是全场最少（或之一），你必须选择一项：失去1点体力；失去1点体力上限；失去1点体力，然后失去1点体力上限并额外进行一个摸牌阶段。",

	["heg_fofl_lufan"] = "吕范-国[官盗]",
    ["&heg_fofl_lufan"] = "吕范",
	["#heg_fofl_lufan"] = "忠笃亮直",
	["illustrator:heg_fofl_lufan"] = "",
	["~heg_fofl_lufan"] = "",
	["information:heg_fofl_lufan"] = "十年经典",
	["@heg_fofl_tiaodu-give"] = "调度：你可以将 %src 交给一名其他角色",
	["heg_fofl_tiaodu-invoke"] = "你可以发动“调度” <br/> <b>操作提示</b>: 选择一名装备区有牌的其他角色→点击确定<br/>",
	["heg_fofl_tiaodu"] = "调度",
	[":heg_fofl_tiaodu"] = "出牌阶段开始时，你可以选择一名装备区有牌的其他角色选择令你是否获得其装备区里的一张牌，并可以将之交给另一名角色。当每回合每种类别限一次，有角色使用指定自己为目标的牌时，你可以令其选择是否摸一张牌。",

	["heg_fofl_zuoci"] = "左慈-国[官盗]",
    ["&heg_fofl_zuoci"] = "左慈",
	["#heg_fofl_zuoci"] = "鬼影神道",
	["illustrator:heg_fofl_zuoci"] = "",
	["~heg_fofl_zuoci"] = "",
	["information:heg_fofl_zuoci"] = "十年经典",
	["heg_fofl_yigui"] = "役鬼",
	[":heg_fofl_yigui"] = "游戏开始前，你将剩余武将牌堆中的两张牌扣置于武将牌上，称为“魂”牌；每回合每种类别限一次，你可以展示并移去一张“魂”牌，视为使用任意一张你本回合未以此法使用过的基本牌或普通锦囊牌，且目标必须是与“魂”牌势力相同的角色。",
	["heg_fofl_jihun"] = "汲魂",
	[":heg_fofl_jihun"] = "当你受到伤害后，或与你势力不同的角色的濒死结算结束且存活后，你可以将剩余武将牌堆中的一张牌扣置于你的武将牌上，称为“魂”牌。准备阶段，你可以移去至多两张“魂”，获得等量的“魂”。",

	["heg_fofl_lukang"] = "陆抗-国[官盗]",
    ["&heg_fofl_lukang"] = "陆抗",
	["#heg_fofl_lukang"] = "社稷之瑰宝",
	["illustrator:heg_fofl_lukang"] = "",
	["~heg_fofl_lukang"] = "",
	["information:heg_fofl_lukang"] = "十年经典",
	["heg_fofl_jueyan"] = "决堰",
	[":heg_fofl_jueyan"] = "准备阶段，你可以选择一个区域的牌于结束阶段弃置，并于本回合获得相应效果：判定区，视为拥有“集智”并跳过判定阶段；装备区，摸三张牌且手牌上限+3；手牌区，使用【杀】的次数+3。",
	["heg_fofl_keshou"] = "恪守",
	[":heg_fofl_keshou"] = "当你受到伤害时，你可弃置两张颜色相同的牌，令此伤害值-1。当你一次性弃置至少两张牌后，你可以判定，若为红色，你摸一张牌。",

	["heg_fofl_zhonghui"] = "钟会-国[官盗]",
    ["&heg_fofl_zhonghui"] = "钟会",
	["#heg_fofl_zhonghui"] = "桀骜的野心家",
	["illustrator:heg_fofl_zhonghui"] = "",
	["~heg_fofl_zhonghui"] = "",
	["information:heg_fofl_zhonghui"] = "十年经典",
	--权计：每回合各限一次
	["heg_fofl_paiyi"] = "排异",
	[":heg_fofl_paiyi"] = "出牌阶段限一次，你可以对一名角色发起一次“军令”。若其执行，你摸X张牌并移去一张“权”；若其不执行，你可以对至多X名其他角色各造成1点伤害并移去等量的“权”（X为“权”数，至少为1，至多为你的体力上限）。",
	...
	["heg_fofl_shilu"] = "嗜戮",
	[":heg_fofl_shilu"] = "锁定技，出牌阶段结束时，若你的副将本回合未发动非锁定技，你变更副将为“戮”没有的势力；以此法被变更的副将改为置于你的武将牌旁，称为“戮”；准备阶段，你弃置X张手牌（不足全弃，无牌不弃），摸X张牌（X为“戮”的势力数）。",
	["heg_fofl_xiongnue"] = "凶虐",
	[":heg_fofl_xiongnue"] = "锁定技，当你使用【杀】造成伤害或受到【杀】造成的伤害时，若受伤角色或伤害来源与“戮”的势力相同，此伤害+1；与“戮”势力相同的角色不因【杀】对你造成伤害时，此伤害-1。",

	["heg_fofl_gongsunyuan"] = "公孙渊-国[官盗]",
    ["&heg_fofl_gongsunyuan"] = "公孙渊",
	["#heg_fofl_gongsunyuan"] = "狡黠的投机者",
	["illustrator:heg_fofl_gongsunyuan"] = "",
	["~heg_fofl_gongsunyuan"] = "",
	["information:heg_fofl_gongsunyuan"] = "十年经典",
	["heg_fofl_huaiyi"] = "怀异",
	[":heg_fofl_huaiyi"] = "出牌阶段限一次，你可以弃置一个区域的所有牌或移除副将，令所有大势力角色依次交给你至少一张牌，然后你选择一名交给你牌数较少的其他角色，移除其一张副将。",
	["heg_fofl_zisui"] = "恣睢",
	[":heg_fofl_zisui"] = "锁定技，被你移除的副将改为置于你武将牌旁，称为“异”；摸牌阶段，你多摸X张牌；结束阶段，若X大于体力上限，你死亡（X为“异”数）。",
	--TODO

	["heg_fofl_2_zongyu"] = "宗预-国[官盗]",
    ["&heg_fofl_2_zongyu"] = "宗预",
	["#heg_fofl_2_zongyu"] = "九酝鸿胪",
	["illustrator:heg_fofl_2_zongyu"] = "",
	["~heg_fofl_2_zongyu"] = "",
	["information:heg_fofl_2_zongyu"] = "十年经典",
	--气傲
	["heg_fofl_2_chengshang"] = "承赏",
	[":heg_fofl_2_chengshang"] = "出牌阶段限一次，当你使用牌结算后，若此牌指定过其他角色且未造成过伤害，你可以摸一张牌。若如此做，此阶段你可以将一张手牌当游戏牌堆中与你此次使用的牌花色点数均相同的另一张基本牌或普通锦囊牌使用。",	

	["heg_fofl_lifeng"] = "李丰-国[官盗]",
	["&heg_fofl_lifeng"] = "李丰",
	["#heg_fofl_lifeng"] = "继父尽事",
	["illustrator:heg_fofl_lifeng"] = "NOVART",
	["information:heg_fofl_lifeng"] = "十年经典",
	["heg_fofl_tunchu"] = "屯储",
	[":heg_fofl_tunchu"] = "摸牌阶段，可以额外摸两张牌，然后你将一至两张手牌置于武将牌上，称为“粮”，若如此做，你本回合不能使用【杀】。",
	["heg_fofl_shuliang"] = "输粮",
	[":heg_fofl_shuliang"] = "一名角色的结束阶段，若你计算与其的距离小于等于X（X为“粮”数），你可以移去一张“粮”，令其摸两张牌。",

	["heg_fofl_2_duyu"] = "杜预-国[官盗]",
    ["&heg_fofl_2_duyu"] = "杜预",
	["#heg_fofl_2_duyu"] = "文成武德",
	["illustrator:heg_fofl_2_duyu"] = "",
	["~heg_fofl_2_duyu"] = "",
	["information:heg_fofl_2_duyu"] = "十年经典",
	["heg_fofl_zhuchuan"] = "注传",
	[":heg_fofl_zhuchuan"] = "出牌阶段每种花色各限一次，你可以弃置一张牌，根据此牌花色令你此阶段使用下一张基本牌或普通锦囊牌增加对应的强化属性与效果：\
		黑桃，残躯 摸一张牌；红桃，助战 目标+1；\
		梅花，空巢 伤害+1；富甲 不可响应。",
	--TODO

	["heg_fofl_wangling"] = "王凌-国[官盗]",
    ["&heg_fofl_wangling"] = "王凌",
	["#heg_fofl_wangling"] = "风节格尚",
	["illustrator:heg_fofl_wangling"] = "",
	["~heg_fofl_wangling"] = "",
	["information:heg_fofl_wangling"] = "十年经典",
	["heg_fofl_mibei"] = "秘备",
	[":heg_fofl_mibei"] = "准备阶段，若你的手牌不为最多，你令手牌最多的另一名角色对你发起“军令”；当你执行“军令”后，你摸牌至与发起者相同（至多摸五张）；当你拒绝执行“军令”后，本回合你展示一至三张手牌，且你可以将其中一张牌当其中另一张基本牌或普通锦囊牌使用一次。",

	["heg_fofl_wangji"] = "王基-国[官盗]",
    ["&heg_fofl_wangji"] = "王基",
	["#heg_fofl_wangji"] = "经行合一",
	["illustrator:heg_fofl_wangji"] = "",
	["~heg_fofl_wangji"] = "",
	["information:heg_fofl_wangji"] = "十年经典",
	["heg_fofl_qizhi"] = "奇制",
	[":heg_fofl_qizhi"] = "当你于回合内使用非装备牌时，你可以弃置不为此牌目标的一名角色的一张牌，令其摸一张牌，若你使用的牌带有强化且与弃置的牌花色相同，此牌无视条件触发强化效果。",
	--进趋
	--TODO
	
	["heg_fofl_yanyan"] = "严颜-国[官盗]",
    ["&heg_fofl_yanyan"] = "严颜",
	["#heg_fofl_yanyan"] = "断头将军",
	["illustrator:heg_fofl_yanyan"] = "",
	["~heg_fofl_yanyan"] = "",
	["information:heg_fofl_yanyan"] = "十年经典",
	["heg_fofl_juzhan"] = "拒战",
	[":heg_fofl_juzhan"] = "转换技，①当你成为【杀】的目标后，你可以与使用者各摸X张牌，然后你可以令其一个技能本回合失效。②当你使用【杀】指定目标后，你可以获得其X张牌（X为此【杀】使用者已损失的体力值且至少为1），然后本回合此技能失效。",

	["heg_fofl_zhuran"] = "朱然-国[官盗]",
    ["&heg_fofl_zhuran"] = "朱然",
	["#heg_fofl_zhuran"] = "不动之督",
	["illustrator:heg_fofl_zhuran"] = "",
	["~heg_fofl_zhuran"] = "",
	["information:heg_fofl_zhuran"] = "十年经典",
	["heg_fofl_danshou"] = "胆守",
	[":heg_fofl_danshou"] = "每轮限一次，一名角色的准备阶段，你可以弃置区域内所有牌，本回合每阶段开始时（准备阶段和结束阶段除外），若你的手牌数小于等于以此法弃置的牌数，你摸一张牌或令你本回合以此法摸牌时多摸一张牌，当你一次性以此法摸四张牌后，你可以对其造成1点伤害。",

	["heg_fofl_gaoshun"] = "高顺-国[官盗]",
    ["&heg_fofl_gaoshun"] = "高顺",
	["#heg_fofl_gaoshun"] = "攻无不克",
	["illustrator:heg_fofl_gaoshun"] = "",
	["~heg_fofl_gaoshun"] = "",
	["information:heg_fofl_gaoshun"] = "十年经典",
	["heg_fofl_xunxi"] = "迅析",
	[":heg_fofl_xunxi"] = "其他角色于其回合外受到伤害结算完结时，你可以视为对其使用一张【杀】，每名角色限一次。",
	["heg_fofl_huanjia"] = "擐甲",
	[":heg_fofl_huanjia"] = "锁定技，每回合各限一次，当你成为【杀】的目标后，本回合你视为装备着使用者的防具，直到你的装备区有防具牌；当你使用【杀】指定唯一目标后，本回合你视为装备着其武器，直到你的装备区有武器牌。",

	["heg_fofl_jiachong"] = "贾充-国[官盗]",
    ["&heg_fofl_jiachong"] = "贾充",
	["#heg_fofl_jiachong"] = "鲁郡公",
	["illustrator:heg_fofl_jiachong"] = "",
	["~heg_fofl_jiachong"] = "",
	["information:heg_fofl_jiachong"] = "十年经典",
	["heg_fofl_xiongshu"] = "凶竖",
	[":heg_fofl_xiongshu"] = "每回合限一次，有角色使用【杀】或伤害类锦囊牌指定目标后，若你为使用者，你可以摸一张牌并令一名其他角色成为此牌的伤害来源；若你不为使用者，你可以弃置一张牌并成为此牌的伤害来源。",
	["heg_fofl_jianhui"] = "奸回",
	[":heg_fofl_jianhui"] = "一名角色对其他角色造成伤害后，你可以令其中一名角色摸一张牌，然后弃置其中另一名角色的一张牌。",

	["heg_fofl_yanghu"] = "羊祜-国[官盗]",
    ["&heg_fofl_yanghu"] = "羊祜",
	["#heg_fofl_yanghu"] = "执德清劭",
	["illustrator:heg_fofl_yanghu"] = "",
	["~heg_fofl_yanghu"] = "",
	["information:heg_fofl_yanghu"] = "十年经典",
	["heg_fofl_suixin"] = "祟信",
	[":heg_fofl_suixin"] = "出牌阶段限一次，你可以选择一名其他角色，视为使用一张仅以你与其为目标的【以逸待劳】。",
	["heg_fofl_weirong"] = "卫戎",
	[":heg_fofl_weirong"] = "转换技，每轮限一次，出牌阶段，你可以①弃X张牌，本轮你不以此法获得牌后摸一张牌。②摸X张牌，本轮你不以此法失去牌后弃一张牌（X为你上轮以此法摸和弃的牌数之差，至少为1，至多为你的体力上限）。",

	["heg_fofl_2_wuqiujian"] = "毌丘俭-国[官盗]",
    ["&heg_fofl_2_wuqiujian"] = "毌丘俭",
	["#heg_fofl_2_wuqiujian"] = "镌功铭征荣",
	["illustrator:heg_fofl_2_wuqiujian"] = "",
	["~heg_fofl_2_wuqiujian"] = "",
	["information:heg_fofl_2_wuqiujian"] = "十年经典",
	["heg_fofl_2_zhengrong"] = "征荣",
	[":heg_fofl_2_zhengrong"] = "你计算与大势力角色的距离-1；你发起“军令”时额外抽取一张；若没有与你势力相同的其他角色，你造成或受到的伤害+1。",
	--鸿举 TODO

	["heg_fofl_2_chengong"] = "陈宫-国[官盗]",
    ["&heg_fofl_2_chengong"] = "陈宫",
	["#heg_fofl_2_chengong"] = "鸷鸟夺凤翎",
	["illustrator:heg_fofl_2_chengong"] = "",
	["~heg_fofl_2_chengong"] = "",
	["information:heg_fofl_2_chengong"] = "十年经典",
	["heg_fofl_2_yinpan"] = "引叛",
	[":heg_fofl_2_yinpan"] = "出牌阶段限一次，你可以选择一名其他角色，令场上其他角色依次选择是否对其使用一张无距离次数限制的【杀】，然后其下回合使用【杀】的次数+X（X为其以此法受到的伤害数），若以此法进入过濒死，其回复1点体力。",
	--与谋

	["heg_fofl_2_jinsimayi"] = "司马懿-国[官盗]",
    ["&heg_fofl_2_jinsimayi"] = "司马懿",
	["#heg_fofl_2_jinsimayi"] = "通达权变",
	["illustrator:heg_fofl_2_jinsimayi"] = "",
	["~heg_fofl_2_jinsimayi"] = "",
	["information:heg_fofl_2_jinsimayi"] = "十年经典",
	["heg_fofl_2_quanbian"] = "权变",
	[":heg_fofl_2_quanbian"] = "当你于回合内使用或打出牌时，你可以用一张手牌替换牌堆顶X张牌中的一张（X为你的体力上限）。",
	["heg_fofl_zhouting"] = "骤霆",
	[":heg_fofl_zhouting"] = "限定技，出牌阶段，你可以依次使用牌堆顶X张牌中能使用的牌（X为你的体力上限），以此法使用牌无距离和次数限制，然后获得其中不能使用的牌，若你以此法杀死了角色，此技能视为未发动过。",
	--jinyingshi 

	["heg_fofl_zhanghuyuechen"] = "张虎乐綝-国[官盗]",
    ["&heg_fofl_zhanghuyuechen"] = "张虎乐綝",
    ["#heg_fofl_zhanghuyuechen"] = "文成武德",
    ["~heg_fofl_zhanghuyuechen"] = "",
    ["cv:heg_fofl_zhanghuyuechen"] = "",
    ["illustrator:heg_fofl_zhanghuyuechen"] = "凝聚永恒",
	["information:heg_fofl_zhanghuyuechen"] = "十年经典",
	["heg_fofl_xijue"] = "袭爵",
	[":heg_fofl_xijue"] = "游戏开始时，你获得2枚“爵”。回合结束时，若你本回合造成过伤害，你获得1杖“爵”。你可以发动技能“突袭”或“骁果”（至多以此法弃置X张基本牌），并弃1枚“爵”。你的摸牌阶段摸牌数为X（X为“爵”数，至多为你的体力上限）。",

	["heg_fofl_2_yanghuiyu"] = "羊徽瑜-国[官盗]",
    ["&heg_fofl_2_yanghuiyu"] = "羊徽瑜",
    ["#heg_fofl_2_yanghuiyu"] = "克明礼教",
    ["~heg_fofl_2_yanghuiyu"] = "",
    ["cv:heg_fofl_2_yanghuiyu"] = "",
    ["illustrator:heg_fofl_2_yanghuiyu"] = "Jzeo",
	["information:heg_fofl_2_yanghuiyu"] = "十年经典",
	["heg_fofl_ciwei"] = "慈威",
  	[":heg_fofl_ciwei"] = "其他角色于一回合内使用第偶数张牌时，你可以弃置一张牌并取消之，然后若此牌为装备牌，你获得并使用之。",
	["heg_fofl_huirong"] = "慧容",
  	[":heg_fofl_huirong"] = "限定技，当你于回合外受到伤害时，你可以令一名角色将手牌摸或弃至体力值。",

	["heg_fofl_wangyuanji"] = "王元姬-国[官盗]",
    ["&heg_fofl_wangyuanji"] = "王元姬",
    ["#heg_fofl_wangyuanji"] = "垂心万物",
    ["~heg_fofl_wangyuanji"] = "",
    ["cv:heg_fofl_wangyuanji"] = "",
    ["illustrator:heg_fofl_wangyuanji"] = "六道目",
	["information:heg_fofl_wangyuanji"] = "十年经典",
	["heg_fofl_yanxi"] = "宴戏",
  	[":heg_fofl_yanxi"] = "出牌阶段限一次，你可以展示一名其他角色的一张手牌和牌堆顶的两张牌。若如此做，你获得其中一种颜色或类别的所有牌并于本回合增加等量的手牌上限，然后令其获得其余的牌。",
	["heg_fofl_shiren"] = "识人",
  	[":heg_fofl_shiren"] = "限定技，当你于回合外受到伤害时，你可以发动一次“宴戏”",


	["heg_fofl_shenguanyu"] = "神关羽-国[官盗]",
    ["&heg_fofl_shenguanyu"] = "神关羽",
    ["#heg_fofl_shenguanyu"] = "鬼神再临",
    ["~heg_fofl_shenguanyu"] = "",
	["information:heg_fofl_shenguanyu"] = "全武将无双",
	["heg_fofl_wushen"] = "武神",
  	[":heg_fofl_wushen"] = "锁定技，你的红桃非基本手牌均视为【杀】；你使用红桃【杀】无距离和次数限制。",
	--冠绝

	["heg_fofl_shenzhouyu"] = "神周瑜-国[官盗]",
    ["&heg_fofl_shenzhouyu"] = "神周瑜",
    ["#heg_fofl_shenzhouyu"] = "赤壁的火神",
    ["~heg_fofl_shenzhouyu"] = "",
	["information:heg_fofl_shenzhouyu"] = "全武将无双",
	--琴音
	["heg_fofl_yeyan"] = "业炎",
  	[":heg_fofl_yeyan"] = "限定技，出牌阶段你可以选择至多三名角色并对这些角色各造成1点火炎伤害。",

	["heg_fofl_shencaocao"] = "神曹操-国[官盗]",
    ["&heg_fofl_shencaocao"] = "神曹操",
    ["#heg_fofl_shencaocao"] = "超世之英杰",
    ["~heg_fofl_shencaocao"] = "",
	["information:heg_fofl_shencaocao"] = "全武将无双",
	["heg_fofl_guixin"] = "归心",
  	[":heg_fofl_guixin"] = "当你受到伤害后，你可以获得每名其他角色区域里各一张牌，然后此技能失效直到你下回合开始。",
	--飞影

	["heg_fofl_shenzhaoyun"] = "神赵云-国[官盗]",
    ["&heg_fofl_shenzhaoyun"] = "神赵云",
    ["#heg_fofl_shenzhaoyun"] = "神威如龙",
    ["~heg_fofl_shenzhaoyun"] = "",
	["information:heg_fofl_shenzhaoyun"] = "全武将无双",
	["heg_fofl_juejing"] = "绝境",
  	[":heg_fofl_juejing"] = "锁定技，你的手牌上限+2；当你进入濒死状态时，你摸一张牌。",
	["heg_fofl_longhun"] = "龙魂",
  	[":heg_fofl_longhun"] = "每回合每花色限一次，你可以将一张：红桃牌当桃使用；方块牌当火杀使用或打出；梅花牌当闪使用或打出；黑桃牌当无懈可击使用​​。",

	["heg_fofl_shensimayi"] = "神司马懿-国[官盗]",
    ["&heg_fofl_shensimayi"] = "神司马懿",
    ["#heg_fofl_shensimayi"] = "晋国之祖",
    ["~heg_fofl_shensimayi"] = "",
	["information:heg_fofl_shensimayi"] = "全武将无双",
	["heg_fofl_jilue"] = "极略",
  	[":heg_fofl_jilue"] = "游戏开始时，你获得鬼才；根据你的势力，获得以下技能：魏~ 放遂 蜀~集智 吴~ 制衡；群~完杀；其他~你的体力上限减少1枚单独的阴阳鱼，获得所有技能。",
	--连破

	["heg_fofl_shenzhangliao"] = "神张辽-国[官盗]",
    ["&heg_fofl_shenzhangliao"] = "神张辽",
    ["#heg_fofl_shenzhangliao"] = "雁门之刑天",
    ["~heg_fofl_shenzhangliao"] = "",
	["information:heg_fofl_shenzhangliao"] = "全武将无双",
	["heg_fofl_duorui"] = "夺锐",
  	[":heg_fofl_duorui"] = "当你于出牌阶段对其他角色造成伤害后，你可以获得其每区域各一张牌，当你本回合以此法获得牌数超过两张后，你可以令本回合受到过伤害的角色各一个技能失效直到其回合结束。",
	["heg_fofl_zhiti"] = "止啼",
  	[":heg_fofl_zhiti"] = "锁定技，若已受伤角色数：大于1，你摸牌阶段摸牌数+1；大于2，你使用【杀】次数+1。",

	["heg_fofl_liuyan"] = "刘焉-国[官盗]",
    ["&heg_fofl_liuyan"] = "刘焉",
    ["#heg_fofl_liuyan"] = "裂土之宗",
    ["~heg_fofl_liuyan"] = "",
	["information:heg_fofl_liuyan"] = "全武将无双",
	["heg_fofl_tushe"] = "图射",
  	[":heg_fofl_tushe"] = "当你使用牌指定其他角色为目标时，你可以摸等同大势力角色数的牌。",
	["heg_fofl_limu"] = "立牧",
  	[":heg_fofl_limu"] = "出牌阶段限一次，你可以将一张方片牌当【乐不思蜀】对自己使用，然后回复1点体力，本回合你使用【杀】的次数+X（X为此牌的点数）。",

	["heg_fofl_weiyan"] = "魏延-国[官盗]",
    ["&heg_fofl_weiyan"] = "魏延",
    ["#heg_fofl_weiyan"] = "嗜血的独狼",
    ["~heg_fofl_weiyan"] = "",
	["information:heg_fofl_weiyan"] = "全武将无双",
	["heg_fofl_kuanggu"] = "狂骨",
  	[":heg_fofl_kuanggu"] = "当你造成或受到1点伤害时，你可摸一张牌；你使用目标为一的伤害牌可以指定至多X个目标（X为你已损失的体力值+1）。",

	["heg_fofl_yuanshu"] = "袁术-国[官盗]",
    ["&heg_fofl_yuanshu"] = "袁术",
    ["#heg_fofl_yuanshu"] = "仲家帝",
    ["~heg_fofl_yuanshu"] = "",
	["information:heg_fofl_yuanshu"] = "全武将无双",
	["heg_fofl_yongsi"] = "庸肆",
	...
	["heg_fofl_zhugedan"] = "诸葛诞-国[官盗]",
    ["&heg_fofl_zhugedan"] = "诸葛诞",
    ["#heg_fofl_zhugedan"] = "薤露蒿里",
    ["~heg_fofl_zhugedan"] = "",
	["information:heg_fofl_zhugedan"] = "全武将无双",
	["heg_fofl_gongao"] = "功獒",
  	[":heg_fofl_gongao"] = "锁定技，当一名其他角色死亡时，你增加1点体力上限，回复1点体力。准备阶段，你摸牌至体力上限。",

	["heg_fofl_lvbu"] = "吕布-国[官盗]",
    ["&heg_fofl_lvbu"] = "吕布",
    ["#heg_fofl_lvbu"] = "武的化身",
    ["~heg_fofl_lvbu"] = "",
	["information:heg_fofl_lvbu"] = "全武将无双",
	["heg_fofl_wuchang"] = "无常",
  	[":heg_fofl_wuchang"] = "锁定技，当你得到其他角色的牌后，你本回合对其造成伤害+1。当一名角色死亡后，你摸两张牌或将一张牌当【出其不意】使用。",
	["heg_fofl_liyu"] = "利驭",
  	[":heg_fofl_liyu"] = "出牌阶段开始时，你可以获得埸上一张牌，因此失去牌的角色摸一张牌。你使用【杀】的次数+X且需至少X张【闪】才能响应（X为你装备区的牌数）。",

	["heg_fofl_shichangshi"] = "十常侍-国[官盗]",
    ["&heg_fofl_shichangshi"] = "十常侍",
    ["#heg_fofl_shichangshi"] = "祸乱纲常",
    ["~heg_fofl_shichangshi"] = "",
	["information:heg_fofl_shichangshi"] = "全武将无双",
	["heg_fofl_danggu"] = "党锢",
  	[":heg_fofl_danggu"] = "锁定技，游戏开始时，你获得十张常侍，然后亮出其中一张。当你休整结束后，你亮出一张常侍，然后摸一张牌。你拥有亮出的常侍的技能。",
	["heg_fofl_mowang"] = "殁亡",
  	[":heg_fofl_mowang"] = "锁定技，你死亡时，若有未亮出的常侍，此次死亡改为休整一轮。回合结束时，你死亡。",

	["heg_fofl_hankui"] = "韩悝-国[官盗]",
    ["&heg_fofl_hankui"] = "韩悝",
    ["#heg_fofl_hankui"] = "贪财好贿",
    ["~heg_fofl_hankui"] = "",
	["information:heg_fofl_hankui"] = "全武将无双",
	["heg_fofl_xiaolu"] = "宵赂",
  	[":heg_fofl_xiaolu"] = "出牌阶段限一次，你可以摸两张牌，然后弃置至多两张牌或将两张牌交给一名其他角色。",

	["heg_fofl_gaowang"] = "高望-国[官盗]",
    ["&heg_fofl_gaowang"] = "高望",
    ["#heg_fofl_gaowang"] = "蛇蝎为药",
    ["~heg_fofl_gaowang"] = "",
	["information:heg_fofl_gaowang"] = "全武将无双",
	["heg_fofl_miaoyu"] = "妙语",
  	[":heg_fofl_miaoyu"] = "你可以将一张方牌牌当火【杀】使用。你使用【杀】无次数限制。",

	["heg_fofl_caoying"] = "曹婴-国[官盗]",
    ["&heg_fofl_caoying"] = "曹婴",
    ["#heg_fofl_caoying"] = "洞若观火",
    ["~heg_fofl_caoying"] = "",
	["information:heg_fofl_caoying"] = "全武将无双",
	["heg_fofl_lingren"] = "凌人",
  	[":heg_fofl_lingren"] = "每回合限一次，当你使用伤害牌指定一个目标后，若其手牌数小于等于你，你可以摸两张牌或令此牌对其伤害+1。",
	["heg_fofl_fujian"] = "伏间",
  	[":heg_fofl_fujian"] = "准备阶段和结束阶段，你可以视为对一名手牌数小于等于你的角色使用一张【知己知彼】。",

	["heg_fofl_caochun"] = "曹纯-国[官盗]",
    ["&heg_fofl_caochun"] = "曹纯",
    ["#heg_fofl_caochun"] = "虎豹骑首",
    ["~heg_fofl_caochun"] = "",
	["information:heg_fofl_caochun"] = "全武将无双",
	["heg_fofl_shanjia"] = "缮甲",
  	[":heg_fofl_shanjia"] = "出牌阶段开始时，你可以摸X张牌（X为与你势力相同的角色数），然后弃置一张牌，若此牌为装备牌，你视为使用一张【杀】。",

	["heg_fofl_zhangyao"] = "张媱-国[官盗]",
    ["&heg_fofl_zhangyao"] = "张媱",
    ["#heg_fofl_zhangyao"] = "琼楼孤蒂",
    ["~heg_fofl_zhangyao"] = "",
	["information:heg_fofl_zhangyao"] = "全武将无双",
	["heg_fofl_yuanyu"] = "怨语",
  	[":heg_fofl_yuanyu"] = "出牌阶段限一次，你可以摸一张牌并将一张手牌置于武将牌上，称为“怨”。",
	["heg_fofl_xiyan"] = "夕颜",
  	[":heg_fofl_xiyan"] = "当你受到伤害后，若伤害牌与其中一张怨颜色相同，你令来源选择一项：1.本回合手牌上限-4；2.本回合不能使用基本牌。",

	["heg_fofl_zhangxuan"] = "张嫙-国[官盗]",
    ["&heg_fofl_zhangxuan"] = "张嫙",
    ["#heg_fofl_zhangxuan"] = "玉宇嫁蔷",
    ["~heg_fofl_zhangxuan"] = "",
	["information:heg_fofl_zhangxuan"] = "全武将无双",
	["heg_fofl_tongli"] = "同礼",
  	[":heg_fofl_tongli"] = "每回合限一次，当你使用牌指定目标后，你可以展示所有手牌，若颜色均相同，此牌多结算一次。",
	["heg_fofl_shezang"] = "奢葬",
  	[":heg_fofl_shezang"] = "当你首次进入濒死状态时，你可以摸四张牌。",
...
return {extension_heg, extension_hegbian, extension_hegquan, extension_heglordex, extension_hegpurplecloud, extension_goldenseal, extension_hegol, extension_hegmobile, extension_hegtenyear, extension_fakeoffline, extension_hegcard, extension_hegadvantagecard}
