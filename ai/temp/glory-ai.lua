...
EnableExtraSkill = false --启用额外技能
PreciousExperience = false --珍贵的经验
--[[****************************************************************
	数据信息
]]--****************************************************************
...
function amWinner()
	...
	local n_lord = sgs.playerRoles["lord"]
	local n_loyalist = sgs.playerRoles["loyalist"]
	local n_renegade = sgs.playerRoles["renegade"]
	local n_rebel = sgs.playerRoles["rebel"]
...
function isGameOver()
	...
	local player = sgs.glory_data["player"]
	updateAlivePlayerRoles()
	local n_lord = sgs.playerRoles["lord"] or 0
	local n_loyalist = sgs.playerRoles["loyalist"] or 0
	local n_renegade = sgs.playerRoles["renegade"] or 0
	local n_rebel = sgs.playerRoles["rebel"] or 0
