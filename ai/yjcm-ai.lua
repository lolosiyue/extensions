...
function sgs.ai_armor_value.yizhong(p,s,card)
...
sgs.ai_skill_use_func.GanluCard = function(card,use,self)
	...
	for _,friend in ipairs(self.friends)do
		for _,enemy in ipairs(self.enemies)do
			if not self:hasSkills(sgs.lose_equip_skill,enemy) and not hasTuntianEffect(enemy,true) then
				...
				if math.abs(ee-fe)<=self.player:getLostHp() and ee>0 and (ee>fe or ee==fe and value>0) then
	...
	for _,friend in ipairs(self.friends)do
		...
		or (friend:hasEquip() and (self:hasSkills(sgs.lose_equip_skill,friend) or hasTuntianEffect(friend,true)))
		then target = friend break end
	end
	if not target then return end
	for _,friend in ipairs(self.friends)do
		if friend:objectName()~=target:objectName()
		and math.abs(friend:getEquips():length()-target:getEquips():length())<=self.player:getLostHp() then
			use.card = card
			use.to:append(friend)
...
sgs.ai_skill_invoke.nosquanji = function(self,data)
	...
	return self:doDisCard(to,"j",true)
	or self:isEnemy(to) and to:getHandcardNum()<self.player:getHandcardNum()
end
...
local nosyexin = {}
nosyexin.name = "nosyexin"
table.insert(sgs.ai_skills,nosyexin)
nosyexin.getTurnUseCard = function(self)
	local hs = self.player:getHandcards()
	hs = sgs.QList2Table(hs)
	self:sortByKeepValue(hs)
	local cards = {}
	for _,id in sgs.qlist(self.player:getPile("nospower"))do
		table.insert(cards,sgs.Sanguosha:getCard(id))
	end
	local ids = {}
	for _,h in sgs.list(hs)do
		for _,c in sgs.list(cards)do
			if table.contains(ids,c:getId()) then continue end
			if self:getKeepValue(h)<self:getKeepValue(c) and self:aiUseCard(c).card then
	...
	if #ids>1 then
		return sgs.Card_Parse("@NosYexinCard="..table.concat(ids,"+"))
	end
end
...
sgs.ai_use_priority.NosYexinCard = 9.2

sgs.ai_skill_use["@@nospaiyi"] = function(self,prompt)
	local valid,to = {},nil
	local cards = {}
	for _,id in sgs.qlist(self.player:getPile("nospower"))do
		table.insert(cards,sgs.Sanguosha:getCard(id))
	...
	if c and p then
		sgs.ai_skill_choice.nospaiyi = "hand_area"
		return string.format("@NosPaiyiCard=%s:->%s",c:getId(),p:objectName())
	end
      	...
      	if self:isEnemy(p) then
			for _,c in sgs.list(cards)do
				if c:isKindOf("DelayedTrick") then
					if seif.player:isProhibited(p,c) then continue end
					sgs.ai_skill_choice.nospaiyi = "judge_area"
					return string.format("@NosPaiyiCard=%s:->%s",c:getId(),p:objectName())
				elseif c:isKindOf("EquipCard") then
					if #self:poisonCards({c},p)<1 then continue end
					if seif.player:isProhibited(p,c) then continue end
					local n = c:getRealCard():toEquipCard():location()
					if p:hasEquip(n) then continue end
					sgs.ai_skill_choice.nospaiyi = "equip_area"
					return string.format("@NosPaiyiCard=%s:->%s",c:getId(),p:objectName())
				end
			end
		elseif self:isFriend(p) then
			for _,c in sgs.list(cards)do
				if c:isKindOf("EquipCard") then
					if #self:poisonCards({c},p)>0 then continue end
					if seif.player:isProhibited(p,c) then continue end
					local n = c:getRealCard():toEquipCard():location()
					if p:hasEquip(n) then continue end
					sgs.ai_skill_choice.nospaiyi = "equip_area"
