-- V2 custom actions use the common proxy and retain their skill identity.
...
local heg_qingnang_skill = {}
heg_qingnang_skill.name = "heg_qingnang"
table.insert(sgs.ai_skills, heg_qingnang_skill)
heg_qingnang_skill.getTurnUseCard = function(self)
	if not self:willShowForDefence() then return nil end
	if self.player:getHandcardNum() < 1 then return nil end
	if self.player:usedTimes("HQingnangCard") > 0 then return nil end
	...
	local card_str = ("@HQingnangCard=%d&heg_qingnang"):format(cards[1]:getId())
	return sgs.Card_Parse(card_str)
end

sgs.ai_skill_use_func.HQingnangCard = function(card, use, self)
...
sgs.ai_use_priority.HQingnangCard = 4.2
sgs.ai_card_intention.HQingnangCard = -100

sgs.dynamic_value.benefit.HQingnangCard = true

sgs.ai_view_as.heg_jijiu = function(card, player, card_place)
	...
	if (card_place ~= sgs.Player_PlaceSpecial or player:getPile("wooden_ox"):contains(card_id)) and card:isRed() and player:getPhase() == sgs.Player_NotActive
		and not player:hasFlag("Global_PreventPeach") and (player:getMark("@heg_qianxi_red") <= 0 or card:isEquipped()) then
		return ("peach:heg_jijiu[%s:%s]=%d&heg_jijiu"):format(suit, number, card_id)
	end
end

sgs.heg_jijiu_suit_value = {
	heart = 6,
	diamond = 6
}

sgs.ai_cardneed.heg_jijiu = function(to, card)
	return card:isRed()
end

sgs.ai_suit_priority.heg_jijiu= "club|spade|diamond|heart"

sgs.ai_skill_cardask["@heg_wushuang-slash-1"] = function(self, data, pattern, target)
...
sgs.ai_skill_invoke.heg_wushuang = function(self, data)
...
function SmartAI:findLijianTarget(card_name, use)
	...
	local findFriend_maxSlash = function(self, first)
		...
		if friend_maxSlash then
			local safe = false
			if sgs.originalHegemonyHasShownSkills(first, "heg_fankui|heg_ganglie") then
	...
	if not self.player:hasUsed(card_name) then
		...
		local males, others = {}, {}
		local first, second
		local heg_zhugeliang_kongcheng, heg_xunyu

		for _, enemy in ipairs(self.enemies) do
			if enemy:isMale() then
				if enemy:hasShownSkill("heg_kongcheng") and enemy:isKongcheng() then heg_zhugeliang_kongcheng = enemy
		...
		if #males == 1 and #self.friends_noself > 0 then
			self:log("Only 1")
			first = males[1]
			if heg_zhugeliang_kongcheng and self:hasTrickEffective(duel, first, heg_zhugeliang_kongcheng) then
				table.insert(males, heg_zhugeliang_kongcheng)
			else
...
local heg_lijian_skill = {}
heg_lijian_skill.name = "heg_lijian"
table.insert(sgs.ai_skills, heg_lijian_skill)
heg_lijian_skill.getTurnUseCard = function(self)
	if self.player:hasUsed("HLijianCard") or self.player:isNude() then
		return
	end
	local card_id = self:getLijianCard()
	if card_id then return sgs.Card_Parse("@HLijianCard=" .. card_id .. "&heg_lijian") end
end

sgs.ai_skill_use_func.HLijianCard = function(card, use, self)
	local first, second = self:findLijianTarget("HLijianCard", use)
...
sgs.ai_use_value.HLijianCard = 8.5
sgs.ai_use_priority.HLijianCard = 4



sgs.dynamic_value.damage_card.HLijianCard = true

sgs.ai_skill_invoke.heg_biyue = function(self, data)
...
local heg_luanji_skill = {}
heg_luanji_skill.name = "heg_luanji"
table.insert(sgs.ai_skills, heg_luanji_skill)
heg_luanji_skill.getTurnUseCard = function(self)
...
sgs.ai_skill_invoke.heg_shuangxiong = function(self, data)
	...
	if self.player:hasSkill("heg_luanji") and self.player:getHandcardNum() >= 5 then return false end
...
sgs.ai_cardneed.heg_shuangxiong = function(to, card, self)
	return not self:willSkipDrawPhase(to)
end

local heg_shuangxiong_skill = {}
heg_shuangxiong_skill.name = "heg_shuangxiong"
table.insert(sgs.ai_skills, heg_shuangxiong_skill)
heg_shuangxiong_skill.getTurnUseCard = function(self)
	if self.player:getMark("heg_shuangxiong") == 0 then return nil end
	local mark = self.player:getMark("heg_shuangxiong")
...
heg_luanwu_skill = {}
heg_luanwu_skill.name = "heg_luanwu"
table.insert(sgs.ai_skills, heg_luanwu_skill)
heg_luanwu_skill.getTurnUseCard = function(self)
	...
	if good > bad then return sgs.Card_Parse("@HLuanwuCard=.&heg_luanwu") end
end

sgs.ai_skill_use_func.HLuanwuCard = function(card, use, self)
	use.card = card
end

sgs.dynamic_value.damage_card.HLuanwuCard = true

sgs.ai_skill_cardask["@heg_luanwu-slash"] = function(self)
	local targets = {}
	for _, player in sgs.qlist(self.room:getOtherPlayers(self.player)) do
...
sgs.ai_skill_invoke.heg_wansha = function(self, data)
	return not self:isFriend(data:toDying().who)
end

sgs.ai_skill_invoke.heg_mengjin = function(self, data)
	if not self:willShowForAttack() then return false end
...
sgs.ai_skill_cardask["@heg_guidao-card"]=function(self, data)
	...
	if needTokeep then
		if self.player:hasSkill("heg_leiji") then keptspade = 2 end
	end
	local cards = {}
	...
	if keptspade == 1 and not self.player:hasSkill("heg_leiji") then return "." end

	local card_id = self:getRetrialCardId(cards, judge)
	if card_id == -1 then
		if self:needRetrial(judge) and judge.reason ~= "heg_beige" then
...
function sgs.ai_cardneed.heg_guidao(to, card, self)
	for _, player in sgs.qlist(self.room:getAllPlayers()) do
		if player:containsTrick("lightning") and self:getFinalRetrial(to, "lightning") == 1  then
			return card:getSuit() == sgs.Card_Spade and card:getNumber() >= 2 and card:getNumber() <= 9 and not self.player:hasSkill("heg_hongyan")
		end
	...
	if to:hasShownSkill("heg_leiji") and self:getFinalRetrial(to, "heg_leiji")then
		return card:isBlack()
	end
end

function SmartAI:findLeijiTarget(player, heg_leiji_value, slasher)
	if not player:hasShownSkill("heg_leiji") then return end
	if slasher then
		if not self:slashIsEffective(sgs.cloneCard("slash"), player, slasher, slasher:hasWeapon("QinggangSword")) then return nil end
		if slasher:hasShownSkill("heg_liegong") and slasher:getPhase() == sgs.Player_Play and self:isEnemy(player, slasher)
	...
	local getCmpValue = function(enemy)
		...
		if enemy:hasShownSkill("heg_hongyan") then return 99 end
	...
	for _, enemy in ipairs(enemies) do
		if getCmpValue(enemy) < heg_leiji_value then return enemy end
	end
	return nil
end

sgs.ai_skill_playerchosen.heg_leiji = function(self, targets)
	self:updatePlayers()
...
sgs.ai_playerchosen_intention.heg_leiji = 80

function sgs.ai_slash_prohibit.heg_leiji(self, from, to, card)
	...
	if from:hasShownSkill("heg_liegong") and (hcard >= from:getHp() or hcard <= from:getAttackRange()) then return false end
	if (from:getHp() >= 4 and (getCardsNum("Peach", from, to) > 0 or from:hasShownSkill("heg_ganglie"))) or from:hasShownSkill("heg_hongyan") and #self.friends == 1 then
		return false end
...
sgs.heg_guidao_suit_value = {
	spade = 3.9,
	club = 2.7
}

sgs.ai_suit_priority.heg_guidao= "diamond|heart|club|spade"

sgs.ai_skill_discard.heg_beige = function(self)
...
function sgs.ai_cardneed.heg_beige(to, card)
...
sgs.ai_skill_choice.heg_duanchang = function(self, choices, data)
	...
	if self:isFriend(who) then
		if sgs.originalHegemonySkillCount(who, true) >= sgs.originalHegemonySkillCount(who, false) then
...
sgs.ai_fill_skill.heg_xiongyi = function(self)
	...
	if sgs.originalHegemonyGameProcess() == "qun>>>" then
		return qunV2Card("heg_xiongyi")
	end
end

sgs.ai_skill_use_func.heg_xiongyi = function(card, use, self)
	use.card = card
end

sgs.ai_card_intention.heg_xiongyi = -80
sgs.ai_use_priority.heg_xiongyi = 9.31

sgs.ai_skill_invoke.heg_mingshi = true

sgs.ai_skill_invoke.heg_lirang = function(self, data)
...
sgs.ai_skill_askforyiji.heg_lirang = function(self, card_ids)
	self:updatePlayers()
	local cards = {}
...
sgs.ai_skill_playerchosen.heg_shuangren = function(self, targets)
...
sgs.ai_skill_playerchosen["heg_shuangren_slash"] = sgs.ai_skill_playerchosen.zero_card_as_slash
sgs.ai_playerchosen_intention.heg_shuangren = 20
...
sgs.ai_skill_playerchosen.heg_sijian = function(self, targets)
	return self:findPlayerToDiscard()
end

sgs.ai_playerchosen_intention.heg_sijian = function(self, from, to)
	local intention = 80
	if (to:hasShownSkill("heg_kongcheng") and to:getHandcardNum() == 1) or self:needToThrowArmor(to) then
		intention = 0
	end
...
sgs.ai_skill_choice.heg_kuangfu_equip = function(self, choices, data)
	local who = data:toPlayer()
	if self:isFriend(who) then
		...
		if sgs.originalHegemonyHasShownSkills(who, "heg_jijiu|heg_beige|heg_weimu|heg_qingcheng") and not self:doNotDiscard(who, "e", false, 1, reason) then
			...
			if choices:match("3") and (not who:hasShownSkill("heg_jijiu") or who:getOffensiveHorse():isRed()) then return "3" end
			if choices:match("0") and (not who:hasShownSkill("heg_jijiu") or who:getWeapon():isRed()) then return "0" end
		end
...
sgs.ai_skill_choice.heg_kuangfu = function(self, choices)
	return "move"
end
