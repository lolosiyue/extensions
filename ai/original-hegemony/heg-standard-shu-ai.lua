-- Source: TODO/QSanguosha-For-Hegemony-xxyheaven/lua/ai/standard-shu-ai.lua (cf61c15).
--[[********************************************************************
...
function SmartAI:shouldUseRende()
	if (self:hasCrossbowEffect() or self:getCardsNum("Crossbow") > 0 or self.player:hasSkill("heg_paoxiao") ) and self:getCardsNum("Slash") > 0     then
		...
		for _, enemy in ipairs(self.enemies) do
			...
			local inPaoxiaoAttackRange =  self.player:distanceTo(enemy) <= self.player:getAttackRange() and self.player:hasSkill("heg_paoxiao")
	...
	if self.player:hasSkill("heg_kongcheng") and keepNum <= 2 then--有空城时手牌少时
		return true
...
sgs.ai_skill_use_func.rende = function(card, use, self)
	...
	for i = 1, #cards do
		...
		if self.player:getHandcardNum() < 3 and self.player:hasSkill("heg_kongcheng") then
		...
		use.card = rendeV2Card(card:getId())
...
sgs.ai_skill_use["@@rende_basic"] = function(self, prompt, method)
	...
	-- Submit the declaration through the native V2 request, not a prebuilt reward.
	local function basicDeclaration(card)
		local activation = sgs.ActiveSkillCard()
		activation:setSkillName("rende_basic")
		activation:setUserString(card:objectName())
		...
		for _, slash in ipairs(clone_slashes) do
			for _, enemy in ipairs(self.enemies) do
				if self:isWeak(enemy) and self.player:canSlash(enemy, slash, true) and not self:slashProhibit(slash, enemy)
					...
					return basicDeclaration(slash) .. "->" .. enemy:objectName()
				end
			end
		end
	end
				...
				for _, enemy in ipairs(self.enemies) do
					if self:isWeak(enemy) and not self.player:canSlash(enemy, slash, true) and not self:slashProhibit(slash, enemy)
						...
						if self:shouldUseAnaleptic(enemy, use) then
							return basicDeclaration(analeptic)
						end
					end
				end
			end
...
sgs.ai_view_as.tenyearwusheng = function(card, player, card_place)
	...
	if (card_place ~= sgs.Player_PlaceSpecial or player:getHandPile():contains(card_id)) and (player:getLord() and player:getLord():hasShownSkill("heg_shouyue") or card:isRed()) and not card:isKindOf("Peach") and not card:hasFlag("using") then
		return ("slash:tenyearwusheng[%s:%s]=%d&tenyearwusheng"):format(suit, number, card_id)
	end
end

local wusheng_skill = {}

wusheng_skill.name = "tenyearwusheng"
...
wusheng_skill.getTurnUseCard = function(self, inclusive)
	...
	for _, card in sgs.qlist(hecards) do
		if (self.player:getLord() and self.player:getLord():hasShownSkill("heg_shouyue") or card:isRed())
			...
			local number = card:getNumberString()
			local card_id = card:getEffectiveId()
			local card_str = ("slash:tenyearwusheng[%s:%s]=%d&tenyearwusheng"):format(suit, number, card_id)
...
function sgs.ai_cardneed.tenyearwusheng(to, card)
	return (to:getHandcardNum() < 3 and card:isRed()) or card:isKindOf("Crossbow")
end

sgs.ai_suit_priority.tenyearwusheng = "club|spade|heart|diamond"

sgs.ai_skill_invoke.heg_paoxiao = function(self, data)
	--[[if not self:willShowForAttack() and not self.player:hasSkills("tenyearwusheng|tenyearkuanggu") then return false end]]--
	return true
end

function sgs.ai_cardneed.heg_paoxiao(to, card, self)
	...
	if need_weapon then
		...
		return to:hasWeapon("Spear") or card:isKindOf("Slash") or (getCardsNum("Slash", to, self.player) > 1 and card:isKindOf("Analeptic")) or card:isKindOf("HHalberd")
	end
end
...
sgs.ai_skill_invoke.heg_kongcheng = true
...
longdan_skill.name = "heg_longdan"
...
longdan_skill.getTurnUseCard = function(self)
	...
	local card_str = ("slash:heg_longdan[%s:%s]=%d&heg_longdan"):format(suit, number, card_id)
	local slash = sgs.Card_Parse(card_str)
	assert(slash)
...
sgs.ai_view_as.heg_longdan = function(card, player, card_place)
	...
	if card_place == sgs.Player_PlaceHand or player:getHandPile():contains(card_id) then
		if card:isKindOf("Jink") then
			return ("slash:heg_longdan[%s:%s]=%d&heg_longdan"):format(suit, number, card_id)
		elseif card:isKindOf("Slash") then
			return ("jink:heg_longdan[%s:%s]=%d&heg_longdan"):format(suit, number, card_id)
		end
	end
end

sgs.ai_skill_playerchosen["heg_longdan_damage"] = function(self, targets)
...
sgs.ai_skill_invoke.heg_tieqi = function(self, data)
	if not self:willShowForAttack() and not self.player:hasSkills("tenyearkuanggu|heg_paoxiao") then return false end
...
sgs.ai_skill_choice.heg_tieqi = function(self, choices, data)
	local target = data:toPlayer()
...
sgs.ai_skill_invoke.heg_liegong = function(self, data)
	if not self:willShowForAttack() and not self.player:hasSkills("tenyearkuanggu|heg_paoxiao") then return false end
	local target = data:toPlayer()
...
sgs.ai_skill_choice.heg_liegong = function(self, choices, data)
	...
	if self.liegong_tg then--data无效，暂时用self记录
		...
		if self.liegong_tg:getMark("##heg_qianxi+no_suit_red") > 0 and not self.liegong_tg:hasShownSkill("qingguo") then
			low_defense = true
		end
		if self.player:hasWeapon("DragonPhoenix") or self.player:hasSkills("heg_tieqi|heg_tieqi_xh") then
	...
	if self.player:hasSkill("heg_wanglie") and self.player:getMark("##heg_wanglie") > 0 then--已发动wanglie
		low_defense = true
	end
	if low_defense then
		return "adddamage"
...
function SmartAI:canLiegong(to, from)
	...
	if sgs.originalHegemonyHasShownSkills(from, "heg_liegong|heg_liegong_xh") and to:getHp() >= from:getHp() then return true end
	return false
end

function sgs.ai_cardneed.heg_liegong(to, card, self)
	--[[旧技能
...
sgs.ai_skill_invoke.tenyearkuanggu = function(self, data)
	if not self:willShowForAttack() and not self.player:hasSkills("heg_tieqi|heg_paoxiao|heg_liegong|heg_qianxi")  then
		return false
	end
	return true
end

function sgs.ai_cardneed.tenyearkuanggu(to, card, self)
	if (card:isKindOf("OffensiveHorse") or card:isKindOf("HSixDragons"))
...
lianhuan_skill.name = "heg_lianhuan"
...
lianhuan_skill.getTurnUseCard = function(self)
	...
	for _, acard in ipairs(cards) do
		if acard:getSuit() == sgs.Card_Club and not (self.player:hasSkill("heg_jizhi") and acard:isKindOf("IronChain"))then
	...
	local number = card:getNumberString()
	local card_id = card:getEffectiveId()
	local card_str = ("iron_chain:heg_lianhuan[club:%s]=%d%s"):format(number, card_id, "&heg_lianhuan")
	local skillcard = sgs.Card_Parse(card_str)
	assert(skillcard)
	return skillcard
end

sgs.ai_cardneed.heg_lianhuan = function(to, card)
	return card:getSuit() == sgs.Card_Club and to:getHandcardNum() <= 2
end

sgs.ai_skill_invoke.heg_niepan = function(self, data)
...
sgs.ai_suit_priority.heg_lianhuan= "club|diamond|heart|spade"

local huoji_skill = {}

huoji_skill.name = "heg_huoji"
...
huoji_skill.getTurnUseCard = function(self)
	...
	for _,acard in ipairs(cards) do
		...
		if self.player:hasSkill("heg_jizhi") and acard:isKindOf("TrickCard") then
	...
	local number = card:getNumberString()
	local card_id = card:getEffectiveId()
	local card_str = ("fire_attack:heg_huoji[%s:%s]=%d&heg_huoji"):format(suit, number, card_id)
	local skillcard = sgs.Card_Parse(card_str)
...
sgs.ai_cardneed.heg_huoji = function(to, card, self)
	return to:getHandcardNum() > 2 and card:isRed()
end

sgs.ai_suit_priority.heg_huoji= "club|spade|diamond|heart"

sgs.ai_view_as.heg_kanpo = function(card, player, card_place)
	local suit = card:getSuitString()
	local number = card:getNumberString()
	local card_id = card:getEffectiveId()
	if card_place == sgs.Player_PlaceHand or player:getHandPile():contains(card_id) then
		if card:isBlack() and not card:isKindOf("HegNullification") and not (player:hasSkill("heg_jizhi") and card:isKindOf("Nullification")) then
			return ("nullification:heg_kanpo[%s:%s]=%d%s"):format(suit, number, card_id, "&heg_kanpo")
		end
	end
end

sgs.ai_cardneed.heg_kanpo = function(to, card, self)
	return card:isBlack()
end

sgs.ai_suit_priority.heg_kanpo= "diamond|heart|club|spade"
...
sgs.ai_skill_invoke.heg_fangquan = function(self, data)
	...
	for _, card in ipairs(cards) do
		if card:isKindOf("TrickCard") and self:getUseValue(card) > 3.69 then
			...
			if dummy_use.card then
				...
				if card:isKindOf("ExNihilo") or card:isKindOf("HBefriendAttacking") or card:isKindOf("HAllianceFeast") then
					shouldUse = shouldUse +  1
				end
			end
		end
	...
	if self.player:hasSkill("heg_shengxi") then
		shouldUse = shouldUse -  1
	end
	...
	if self.player:hasSkill("heg_dangxian") then--当先
		shouldUse = shouldUse -  1
	end
	...
	if AssistTarget and not self:willSkipPlayPhase(AssistTarget) then
		...
		self.fangquan_card_str = "@HFangquanCard=" .. to_discard .. "&heg_fangquan->" .. AssistTarget:objectName()
		return true
	end

	self:sort(self.friends_noself, "handcard", true)
	for _, target in ipairs(self.friends_noself) do--怎样优化一下目标？
		if sgs.originalHegemonyHasShownSkills(target, "heg_zhiheng|" .. sgs.priority_skill .. "|heg_shensu") and (not self:willSkipPlayPhase(target) or target:hasShownSkill("heg_shensu")) then
			self.fangquan_target = target
			self.fangquan_card_str = "@HFangquanCard=" .. to_discard .. "&heg_fangquan->" .. target:objectName()
			return true
		end
	end
	return false
...
sgs.ai_skill_use["@@fangquan_ask"] = function(self, prompt)
	...
	if self.fangquan_target then
		for i = #cards, 1, -1 do
			local card = cards[i]
			if not isCard("Peach", card, self.player) and not self.player:isJilei(card) then
				return "@HFangquanCard=" .. card:getEffectiveId() .. "&heg_fangquan->" .. self.fangquan_target:objectName()
			end
		end
	end
end

sgs.ai_card_intention.HFangquanCard = -120

sgs.ai_skill_invoke.heg_zaiqi = function(self, data)
...
sgs.ai_skill_playerchosen.heg_zaiqi = function(self, targets, max_num, min_num)
	local result = {}
...
sgs.ai_skill_choice.heg_zaiqi = function(self, choices)
	local heg_menghuo = sgs.findPlayerByShownSkillName("heg_zaiqi")
	if string.find(choices, "recover") and heg_menghuo:getHp() < 3 and heg_menghuo:canRecover() then
		return "recover"
	end
	return "drawcard"
end

sgs.ai_skill_invoke.heg_juxiang = function(self, data)
...
sgs.ai_cardneed.heg_lieren = function(to, card, self)
...
sgs.ai_skill_invoke.heg_lieren = function(self, data)
...
function sgs.ai_skill_pindian.heg_lieren(minusecard, self, requestor)
	local cards = sgs.QList2Table(self.player:getHandcards())
	self:sortByKeepValue(cards)
...
sgs.ai_skill_invoke.shenzhi = function(self, data)
	...
	if self:getCardsNum("HAllianceFeast") > 0 then return false end
...
sgs.ai_skill_invoke.heg_huoshou = function(self, data)
	if not self:willShowForDefence() and not self:willShowForAttack() then
		return false
	end
	return true
...
sgs.ai_skill_invoke.heg_yizhi = sgs.ai_skill_invoke.guanxing
