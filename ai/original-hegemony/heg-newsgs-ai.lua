-- Source: TODO/QSanguosha-For-Hegemony-xxyheaven/lua/ai/newsgs-ai.lua (cf61c15).
...
--蒋干
sgs.ai_skill_invoke.heg_weicheng = true

local daoshu_skill= {}
daoshu_skill.name = "heg_daoshu"
table.insert(sgs.ai_skills, daoshu_skill)
daoshu_skill.getTurnUseCard = function(self, inclusive)
	if self.player:hasUsed("HDaoshuCard") then return end
	if #self.enemies == 0 then return end
	return sgs.Card_Parse("@HDaoshuCard=.&heg_daoshu")
end

sgs.ai_skill_use_func.HDaoshuCard = function(card, use, self)
	sgs.ai_use_priority.HDaoshuCard = 2.9--合纵连横之后
		...
		if not enemy:isKongcheng() and not self:doNotDiscard(enemy, "h") and self:damageIsEffective(enemy, nil, self.player) then
			...
			for _, suit in ipairs(known_suit) do
				if suit == enemy:getHandcardNum() then--如果已知花色等于手牌数
					sgs.ai_use_priority.HDaoshuCard = 5.3
...
sgs.ai_skill_suit.heg_daoshu= function(self)--有空可以增加配合合纵连横，估计需要改合纵连横的ai
	--global_room:writeToConsole("选择花色:"..self.daoshu_suit)
	return self.daoshu_suit
end

sgs.ai_skill_cardask["@heg_daoshu-give"] = function(self, data, pattern, target, target2)
...
--周夷
sgs.ai_skill_invoke.heg_zhukou = true
...
sgs.ai_skill_playerchosen.heg_lianyou = function(self, targets)
	...
	for _, target in ipairs(targetlist) do--考虑方便火烧，考虑下家队列长度？
		if self:isFriendWith(target) and target:getHp() > 1 and (self:isEnemy(target:getNextAlive()) or target:hasSkill("heg_huoji")) then
			return target
		end
	end
	...
	for _, target in ipairs(targetlist) do
		if self:isFriend(target) and target:getHp() > 1 and (self:isEnemy(target:getNextAlive()) or target:hasSkill("heg_huoji")) then
			return target
		end
	end
	for _, target in ipairs(targetlist) do
...
sgs.ai_skill_invoke.heg_xinghuo =  function(self, data)
...
function sgs.ai_cardneed.heg_xinghuo(to, card, self)
	return card:isKindOf("HBurningCamps") or isCard("FireAttack", card, to) or isCard("FireSlash", card, to)
end
...
local jinghe_skill = {}
jinghe_skill.name = "heg_jinghe"
table.insert(sgs.ai_skills, jinghe_skill)
jinghe_skill.getTurnUseCard = function(self, inclusive)
	if self.player:isKongcheng() or self.player:hasUsed("HJingheCard") then return end
	local jinghe_show = {}
	...
	return sgs.Card_Parse("@HJingheCard=" .. table.concat(jinghe_show, "+") .."&heg_jinghe")
end

sgs.ai_skill_use_func.HJingheCard = function(card, use, self)
	use.card = card
	if use.to then
...
sgs.ai_card_intention.HJingheCard = -90
sgs.ai_use_priority.HJingheCard = 9.23--远交近攻和无中生有之后，更详细的判断？如配合敕令
...
sgs.ai_skill_choice.jinghe_skill = function(self, choices, data)
	...
	if table.contains(choices, "heg_yanzheng") then
		if self.player:objectName() == current:objectName() and #choices > 1 then
			table.removeOne(choices, "heg_yanzheng")
		end
	end
	if table.contains(choices, "heg_leiji_tianshu") then
		if self.player:hasSkills(sgs.wizard_skill) then
			return "heg_leiji_tianshu"
		end
		for _, p in ipairs(targets) do
			if p:hasSkills(sgs.wizard_skill) and #choices > 1 then
				table.removeOne(choices, "heg_leiji_tianshu")
			end
		end
	end
	if table.contains(choices, "heg_yinbing")then
		...
		for _, p in ipairs(targets) do
			if (self:hasCrossbowEffect(p) or getKnownCard(p, self.player, "Crossbow", false) > 0) and #choices > 1 then
				table.removeOne(choices, "heg_yinbing")
			end
		end
	end
	if table.contains(choices, "heg_yinbing")then
	...
	if table.contains(choices, "heg_guizhu") then
		if self.player:hasSkills("jijiu|heg_zhendu|heg_xishe") then
			return "heg_guizhu"
		end
		for _, p in ipairs(targets) do
			if p:hasSkills("jijiu|heg_zhendu|heg_xishe") and #choices > 1 then
				table.removeOne(choices, "heg_guizhu")
			end
		end
	end
	if table.contains(choices, "heg_yanzheng") then
		local yanzheng_card = sgs.ai_skill_exchange.heg_yanzheng(self)
		if #yanzheng_card > 0 and not self:isWeak() then
			return "heg_yanzheng"
		end
	end
	if table.contains(choices, "heg_huoqi") then
		local min_hp = 99
	...
	if table.contains(choices, "heg_xianshou") and not self.player:isWounded() then
		return "heg_xianshou"
	end
	if table.contains(choices, "heg_lundao") and self.player:getHandcardNum() < 3 then
		return "heg_lundao"
	end
	if table.contains(choices, "heg_guanyue") then
		return "heg_guanyue"
	end
	return choices[math.random(1, #choices)]
end
...
--雷击
sgs.ai_skill_playerchosen.heg_leiji_tianshu = function(self, targets)
	self:updatePlayers()
...
function sgs.ai_slash_prohibit.heg_leiji_tianshu(self, from, to, card)
	if self:isFriend(to, from) then return false end
	if self:canLiegong(to, from) then return false end
	if not sgs.originalHegemonyHasShownSkills(to, sgs.wizard_skill) then return false end
	if sgs.originalHegemonyHasShownSkills(from, "heg_tieqi|heg_tieqi_xh") then return false end
	if from:hasShownSkill("tenyearjianchu") and (to:hasEquip() or to:getCardCount(true) == 1) then
		return false
	end
	if (to:getMark("##heg_qianxi+no_suit_red") + to:getMark("##heg_qianxi+no_suit_black") > 0) and (not self:hasEightDiagramEffect(to) or IgnoreArmor(from, to)) then
		return false
	end
	local hcard = to:getHandcardNum()
	if (from:getHp() >= 4 and (getCardsNum("Peach", from, to) > 0 or from:hasShownSkill("heg_ganglie"))) or from:hasShownSkill("hongyan") and #self.friends == 1 then
		return false
	end
...
--阴兵
sgs.ai_skill_invoke.heg_yinbing = true

function sgs.ai_cardneed.heg_yinbing(to, card, self)
	return card:isKindOf("Axe") or (self:hasCrossbowEffect(to) and isCard("Slash", card, to))
end

--活气
local huoqi_skill = {}
huoqi_skill.name = "heg_huoqi"
table.insert(sgs.ai_skills, huoqi_skill)
huoqi_skill.getTurnUseCard = function(self)
	if self.player:isKongcheng() or self.player:hasUsed("HHuoqiCard") then return end
	local cards = self.player:getCards("he")
	cards = sgs.QList2Table(cards)
	self:sortByUseValue(cards, true)
	local card_str = ("@HHuoqiCard=%d&heg_huoqi"):format(cards[1]:getId())
	return sgs.Card_Parse(card_str)
end

sgs.ai_skill_use_func.HHuoqiCard = function(card, use, self)
	local target = nil
	local min_hp = 99
	for _, p in sgs.qlist(self.room:getAlivePlayers()) do
		if p:getHp() < min_hp then
...
sgs.ai_use_priority.HHuoqiCard = 4.2
sgs.ai_card_intention.HHuoqiCard = -100

--鬼助
sgs.ai_skill_invoke.heg_guizhu = true

--仙授
local xianshou_skill = {}
xianshou_skill.name = "heg_xianshou"
table.insert(sgs.ai_skills, xianshou_skill)
xianshou_skill.getTurnUseCard = function(self)
	if self.player:hasUsed("HXianshouCard") then return end
	return sgs.Card_Parse("@HXianshouCard=.&heg_xianshou")
end

sgs.ai_skill_use_func.HXianshouCard = function(card, use, self)
	local target
...
sgs.ai_card_intention.HXianshouCard = -20
sgs.ai_use_priority.HXianshouCard = 5

--论道
sgs.ai_skill_invoke.heg_lundao =  function(self, data)
...
--观月
sgs.ai_skill_invoke.heg_guanyue = true
...
--言政
sgs.ai_skill_exchange.heg_yanzheng = function(self,pattern,max_num,min_num,expand_pile)
	...
	for _, card in ipairs(cards) do
		if (isCard("HBefriendAttacking", card, self.player) or isCard("ExNihilo", card, self.player) or isCard("HThreatenEmperor", card, self.player))
	...
	for _, card in ipairs(cards) do
		if (isCard("HAllianceFeast", card, self.player) or isCard("Peach", card, self.player)) and card:isAvailable(self.player) then
			valuable_num = valuable_num + 1
...
function sgs.ai_cardneed.heg_yanzheng(to, card, self)
	return to:getHandcardNum() < 2
end

--吕玲绮
sgs.ai_skill_invoke.heg_guowu =  function(self, data)
	return true--暂时简化
end
...
sgs.ai_skill_playerchosen["#guowu_effect"] = function(self, targets, max_num, min_num)
	...
	elseif card:isKindOf("HBefriendAttacking") then
		self:sort(targetlist, "handcard")
		for _, target in ipairs(targetlist) do
			if self:isFriend(target) and #result < max_num and not table.contains(result, target) then
		  		table.insert(result, target)
	...
	elseif card:isKindOf("Duel") or card:isKindOf("HDrowning") then--决斗详细？
		self:sort(targetlist, "hp")
		for _, target in ipairs(targetlist) do
...
zhuangrong_skill.name = "heg_zhuangrong"
table.insert(sgs.ai_skills, zhuangrong_skill)
zhuangrong_skill.getTurnUseCard = function(self)
	if self.player:hasUsed("HZhuangrongCard") or self.player:hasSkill("wushuang") then return end
	...
	for _, card in ipairs(cards) do
		if card:getTypeId() == sgs.Card_TypeTrick and self:getUseValue(card) < 5 then--值多少合适？
			local card_str = ("@HZhuangrongCard=%d&heg_zhuangrong"):format(card:getId())
			return sgs.Card_Parse(card_str)
		end
	end
end

sgs.ai_skill_use_func.HZhuangrongCard = function(card, use, self)
	use.card = card
end

sgs.ai_use_priority.HZhuangrongCard = 4.2

sgs.ai_skill_invoke.heg_wushuang_lvlingqi = sgs.ai_skill_invoke.wushuang

sgs.ai_cardneed.heg_wushuang_lvlingqi = sgs.ai_cardneed.wushuang

sgs.ai_skill_invoke.heg_shenwei = true
