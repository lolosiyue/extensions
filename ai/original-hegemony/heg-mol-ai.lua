-- Source: TODO/QSanguosha-For-Hegemony-xxyheaven/lua/ai/mol-ai.lua (cf61c15).
if not sgs.original_hegemony_ai_loading then return end

sgs.ai_skill_use["@@heg_qianzheng"] = function(self)
	local cards = sgs.QList2Table(self.player:getCards("he"))
	if #cards < 2 then return "." end
	self:sortByKeepValue(cards)
	return "@DummyCard=" .. cards[1]:getEffectiveId() .. "+" .. cards[2]:getEffectiveId()
end
...
--杜预
sgs.ai_skill_invoke.heg_wuku = true

local miewu_skill = {}
miewu_skill.name = "heg_miewu"
table.insert(sgs.ai_skills, miewu_skill)
miewu_skill.getTurnUseCard = function(self)
	if self.player:getMark("#heg_wuku") == 0 or self.player:hasFlag("MiewuUsed") or self.player:isNude() then
        return
    end
    local cards = sgs.QList2Table(self.player:getCards("he"))
...
--[[{联军盛宴,火烧连营,桃园结义,南蛮入侵,万箭齐发,
    ...
    for _, trick in ipairs(trickcards) do
		...
		local fixed_cards = {"ex_nihilo","heg_threaten_emperor","await_exhausted"}
		if self.player:hasShownSkill("heg_weimu") and cards[1]:isBlack() and table.contains(fixed_cards, trick) then continue end--贾诩不开黑无中
    ...
    if #can_use > 0 then
		...
		sgs.ai_use_priority.heg_miewu = math.max(4, self:getDynamicUsePriority(sgs.cloneCard(card_name)))--4是否合适
        return hegemonyConvertedCard("heg_miewu", card_name, cards[1])
    end
end


function sgs.ai_cardsview.heg_miewu(self, class_name, player, cards)
	if not player:hasShownSkill("heg_miewu") then return end
	if player:getMark("#heg_wuku") == 0 or player:hasFlag("MiewuUsed") or player:isNude() or player:isRemoved() then
        return
    end
    ...
    or class_name == "Slash" or class_name == "Jink" or class_name == "Nullification" then
        if not cards then
			...
			for _, c in sgs.qlist(player:getCards("he")) do
				if sgs.cardIsVisible(c, player, self.player) and not c:isKindOf(class_name) then
		...
		--只有一个灭吴并且持牌充裕时,考虑保留
		if player:getMark("#heg_wuku") == 1 and player:getCardCount(true) > 1 and (class_name == "Slash" or class_name == "Jink") then
			for _, c in ipairs(cards) do
        ...
        --[[if player:getPhase() <= sgs.Player_Play then
                return self:getUseValue(acard)
            else

            end]]
            return self:getKeepValue(acard)
        end
		...
		for _, c in ipairs(cards) do--未考虑留桃留无懈等
			...
			if card_value < value or nullification_indulgence or(card_value <= 4.1 and player:getCardCount(true) == 1) then
                ...
                return hegemonyConvertedCard("heg_miewu", object_name, c):toString()
            end
        end
	end
end

sgs.ai_view_as.heg_miewu = function(card, player, card_place)--只为无懈自己的乐,因为ai_cardsview触发频率不够,经常2灭吴被乐弃一把高质量牌
	if not player:hasShownSkill("heg_miewu") then return end
	if player:getMark("#heg_wuku") == 0 or player:hasFlag("MiewuUsed") or player:isNude() or player:isRemoved() then
        return
    end
	...
	if (card_place ~= sgs.Player_PlaceSpecial or player:getHandPile():contains(card_id)) then
		if not card:isKindOf("Nullification") then
			return ("nullification:heg_miewu[%s:%s]=%d%s"):format(suit, number, card_id, "&heg_miewu")
		end
	end
end

function sgs.ai_cardneed.heg_miewu(to, card, self)
	return to:isNude() and to:getMark("#heg_wuku") > 0
end

--李丰
sgs.ai_skill_invoke.heg_tunchu = function(self, data)
...
sgs.ai_skill_exchange.heg_shuliang = function(self, pattern, max_num, min_num, expand_pile)
...
--凌操
sgs.ai_skill_invoke.heg_dujin = function(self, data)
...
sgs.ai_fill_skill.heg_miewu = miewu_skill.getTurnUseCard
