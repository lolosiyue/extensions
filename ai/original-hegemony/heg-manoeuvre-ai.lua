-- Source: TODO/QSanguosha-For-Hegemony-xxyheaven/lua/ai/manoeuvre-ai.lua (cf61c15).
...
--华歆
sgs.ai_skill_invoke.heg_wanggui = true

sgs.ai_skill_playerchosen.heg_wanggui = sgs.ai_skill_playerchosen.damage

sgs.ai_need_damaged.heg_wanggui = function(self, attacker, player)
  if not player:hasShownSkill("heg_wanggui") or player:hasFlag("WangguiUsed") then
    return false
  end
...
sgs.ai_skill_invoke.heg_xibing =  function(self, data)
  ...
  if target:hasShownAllGenerals() then--发动技能后双亮
    if (self.player:hasShownGeneral1() and self.player:inDeputySkills("heg_xibing") and self.player:canShowGeneral("d"))
    or (self.player:hasShownGeneral2() and self.player:inHeadSkills("heg_xibing") and self.player:canShowGeneral("h")) then
      eachother_shown = true
    end
  end
  local xibing_firstskills = --注意有优先顺序
              "heg_paiyi|heg_suzhi|heg_shilu|heg_huaiyi|heg_luanji|heg_yigui|heg_paoxiao|heg_kuangcai|heg_diaodu|heg_xuanhuo|"..
              "heg_jixi|heg_qice|heg_zaoyun|heg_jinfa|"..
							"heg_jizhi|heg_tieqi|tenyearkuanggu|heg_jili|heg_tongdu|"..
							"heg_xiaoji|heg_guose|heg_xuanlue|"..
"wansha|tenyearjianchu|heg_qianhuan|"..
							"heg_zhukou|heg_boyan|heg_guishu|heg_miewu"
  ...
  if target:hasShownSkill("heg_buqu") and target:getPile("scars"):length() > 4 and self:isFriend(target) and eachother_shown then
    self.xibing_skill = "heg_buqu"
    return true
  end
  local not_firstskill = true
  for _, p in ipairs(self.enemies) do
    if sgs.originalHegemonyHasShownSkills(p, xibing_firstskills) and p:hasShownAllGenerals() then
      not_firstskill = false
    end
  end
  if not_firstskill then
    local xibing_secondskills = "|heg_duanliang|heg_qiangxi|heg_juejue|heg_daoshu|tenyearwusheng|heg_shengxi|heg_sanyao|"..
              "heg_zhiheng|qixi|heg_kurou|heg_fanjian|heg_keji|heg_duoshi|tianyi|dimeng|heg_ganlu|"..
              "shuangxiong|lijian|lirang|heg_chuanxin|heg_xiongsuan|heg_weidi|heg_midao|heg_baolie"
    if self:isEnemy(target) and eachother_shown then
...
sgs.ai_skill_choice.heg_xibing = function(self, choices, data)
  ...
  if table.contains(choices,"cancel") then
    ...
    if self.player:hasSkill("heg_tuntian") and not self.player:getPile("field"):isEmpty() then
      if self.player:inDeputySkills("heg_tuntian") then
        return "head"
      else
        return "deputy"
      end
    end
    if self.player:hasSkill("heg_paiyi") and not self.player:getPile("power_pile"):isEmpty() then
      return "deputy"
    end
    if self.player:hasSkill("heg_xiongnve") and self.player:getMark("#massacre") > 0 then
      return "deputy"
    end
    if self.player:hasSkill("heg_zisui") and not self.player:getPile("&disloyalty"):isEmpty() then
      if self.player:getPile("&disloyalty"):length() >= self.player:getMaxHp() then
        return "head"
      end
      return "deputy"
    end
    if self.player:hasSkill("heg_sidi") and not self.player:getPile("drive"):isEmpty() then
      if self.player:inDeputySkills("heg_sidi") then
        return "head"
      else
        return "deputy"
      end
    ...
    local xibing_defenseskills = "heg_yiji|nosfankui|heg_ganglie|mobilefangzhu|qingguo|heg_shicai|heg_zhaoxin"
    if self.player:hasSkill("heg_jieming") and self:getJiemingDrawNum(self.player) >= 2 then
      xibing_defenseskills = "heg_jieming|" .. xibing_defenseskills
    end
    if current:canSlash(self.player, nil, true) and not self.player:hasSkills(xibing_defenseskills) then
      if self.player:inDeputySkills("heg_xibing") and table.contains(choices,"head") then--暗无防御将
        return "head"
      else
        return "deputy"
      end
    end
    if self.player:inDeputySkills("heg_xibing") and table.contains(choices,"deputy") then--暗着可以发动息兵
      return "deputy"
    else
      return "head"
    end
...
--陆郁生
sgs.ai_skill_invoke.heg_zhente = function(self, data)
  if self.player:hasSkill("guzheng") then
    local lord_sunquan = self.room:getLord(self:originalHegemonyOwnKingdom())
    if lord_sunquan and lord_sunquan:getPile("flame_map"):length() > 1 then
      return true
    end
    for _, p in ipairs(self.friends_noself) do
      if sgs.originalHegemonyHasShownSkills(p, sgs.drawcard_skill) then
        return true
      end
    end
  end
  ...
  if target and self:isFriend(target) then
    if (card:isKindOf("IronChain") or card:isKindOf("HFightTogether") or card:isKindOf("FireAttack") or card:isKindOf("NatureSlash"))
      and not self.player:isChained() then
        return true
    elseif card:isKindOf("Slash") or card:isKindOf("Duel") or card:isKindOf("HDrowning")
      or card:isKindOf("HBurningCamps") or card:isKindOf("SavageAssault") or card:isKindOf("ArcheryAttack") then
        return true
    else
      return false
    end
...
sgs.ai_skill_choice.heg_zhente = function(self, choices, data)
  local use = data:toCardUse()
  local heg_luyusheng = sgs.findPlayerByShownSkillName("heg_zhente")
  if heg_luyusheng and use.to:contains(heg_luyusheng) and self:isEnemy(heg_luyusheng) then
...
sgs.ai_skill_playerchosen.heg_zhiwei = function(self, targets)
  local current = self.room:getCurrent()
  if current:objectName() ~= self.player:objectName()
  and (current:hasShownSkill("heg_yigui") and #(current:property("Huashens"):toString():split("+")) > 3
      or (current:getHandcardNum() > 3 and (current:hasShownSkill("heg_luanji") or self:hasCrossbowEffect(current)
  ...
  for _, p in ipairs(targets) do
    if self.player:isFriendWith(p) and sgs.originalHegemonyHasShownSkills(p, sgs.priority_skill) then
      return p
    end
  end
  for _, p in ipairs(targets) do
    if self:isFriend(p) and sgs.originalHegemonyHasShownSkills(p, sgs.priority_skill) then
      return p
    end
  end
  for _, p in ipairs(targets) do
...
sgs.ai_skill_invoke.heg_chengshang = true

--祢衡
sgs.ai_skill_invoke.heg_kuangcai = function(self, data)
  if sgs.getHegemonyCardUsedTimes(self.player, ".") == 0 then
    return true
  end
  return false
end

function sgs.ai_cardneed.heg_kuangcai(to, card, self)
	return card:isKindOf("Slash") or card:isKindOf("Analeptic") or card:isKindOf("HHalberd") or to:hasWeapon("Spear")
end
...
sgs.kuangcai_keep_value = {
	...
	ThunderSlash = 5.5,
	ExNihilo = 4.7,
	HBefriendAttacking = 5
}--复制的咆哮，是否合理？

sgs.ai_skill_invoke.heg_shejian =  function(self, data)
  if not self:willShowForDefence() then
    return false
  end
  local target = data:toPlayer()
  ...
  and target:getHp() <= (self.player:hasSkill("heg_congjian") and 2 or 1) and self:isWeak(target) then
    return true
  end
	return false
end

function sgs.ai_cardneed.heg_shejian(to, card, self)
	return to:isKongcheng() and not self:needKongcheng(to)
end

--冯熙
sgs.ai_skill_invoke.heg_yusui =  function(self, data)
  if not self:willShowForDefence() then
    return false
  end
  ...
  local can_losehp = not self.yusui_target:hasSkill("heg_hongfa") or self.yusui_target:getPile("heavenly_army"):isEmpty()
...
sgs.ai_skill_choice.heg_yusui = function(self, choices, data)--没有来源的data，暂时用self
  choices = choices:split("+")
  local can_losehp = not self.yusui_target:hasSkill("heg_hongfa") or self.yusui_target:getPile("heavenly_army"):isEmpty()
...
sgs.ai_trick_prohibit.heg_yusui = function(self, card, to, from)
  
end

local boyan_skill = {}
boyan_skill.name = "heg_boyan"
table.insert(sgs.ai_skills, boyan_skill)
boyan_skill.getTurnUseCard = function(self)
	if self.player:hasUsed("HBoyanCard") then return end
	return sgs.Card_Parse("@HBoyanCard=.&heg_boyan")
end

sgs.ai_skill_use_func.HBoyanCard = function(card, use, self)
  local target
...
sgs.ai_use_priority.HBoyanCard = 5--优先度多少合适？

sgs.ai_skill_choice.heg_boyan = function(self, choices, data)
  local target = data:toPlayer()
...
boyanzongheng_skill.name = "heg_boyanzongheng"
table.insert(sgs.ai_skills, boyanzongheng_skill)
boyanzongheng_skill.getTurnUseCard = function(self)
	if self.player:hasUsed("HBoyanZonghengCard") then return end
	return sgs.Card_Parse("@HBoyanZonghengCard=.&heg_boyanzongheng")
end

sgs.ai_skill_use_func.HBoyanZonghengCard = function(card, use, self)
  local target
  self:sort(self.enemies, "hp")
  for _, p in ipairs(self.enemies) do
...
sgs.ai_use_priority.HBoyanZonghengCard = 5

--邓芝
sgs.ai_skill_invoke.heg_jianliang = true

local weimeng_skill = {}
weimeng_skill.name = "heg_weimeng"
table.insert(sgs.ai_skills, weimeng_skill)
weimeng_skill.getTurnUseCard = function(self)
	if self.player:hasUsed("HWeimengCard") then return end
	return sgs.Card_Parse("@HWeimengCard=.&heg_weimeng")
end

sgs.ai_skill_use_func.HWeimengCard = function(card, use, self)
  local target
...
sgs.ai_use_priority.HWeimengCard = 5
...
sgs.ai_skill_choice.heg_weimeng = function(self, choices, data)
  local target = data:toPlayer()
  if self:isFriend(target) then
    return "yes"
  end
...
weimengzongheng_skill.name = "heg_weimengzongheng"
table.insert(sgs.ai_skills, weimengzongheng_skill)
weimengzongheng_skill.getTurnUseCard = function(self)
	if self.player:hasUsed("HWeimengZonghengCard") then return end
	return sgs.Card_Parse("@HWeimengZonghengCard=.&heg_weimengzongheng")
end

sgs.ai_skill_use_func.HWeimengZonghengCard = function(card, use, self)
  local target
  local _, friend = self:getCardNeedPlayer(sgs.QList2Table(self.player:getCards("he")))
...
sgs.ai_use_priority.HWeimengZonghengCard = 5

--荀谌
local fenglve_skill = {}
fenglve_skill.name = "heg_fenglve"
table.insert(sgs.ai_skills, fenglve_skill)
fenglve_skill.getTurnUseCard = function(self)
	if self:willShowForAttack() and not self.player:hasUsed("HFenglveCard") and not self.player:isKongcheng() then
    return sgs.Card_Parse("@HFenglveCard=.&heg_fenglve")
  end
end

sgs.ai_skill_use_func.HFenglveCard = function(FLCard, use, self)
	if #self.enemies == 0 and #self.friends_noself == 0 then return end
  self.fenglve_card = nil
  sgs.ai_use_priority.HFenglveCard = 0.5--是否合适？
  ...
  for _, friend in ipairs(self.friends_noself) do--拆判定区多于1的队友
    if not friend:isKongcheng() and friend:getJudgingArea():length() > (self:needToThrowArmor(friend) and 0 or 1) then
      ...
      if friend_min_card and friend:hasShownSkill("heg_yingyang") then friend_number = math.max(friend_number - 3, 1) end
      if max_point> friend_number then
        local hcards = sgs.QList2Table(self.player:getHandcards())
        self:sortByUseValue(hcards,true)
        for _, c in ipairs(hcards) do
          if c:getNumber() + (self.player:hasShownSkill("heg_yingyang") and 3 or 0) > friend_number then
            sgs.ai_use_priority.HFenglveCard = 4.2
      ...
      if not friend_min_card and max_point > 8 then
        sgs.ai_use_priority.HFenglveCard = 4.2--顺之后
	...
	for _, enemy in ipairs(self.enemies) do
		if not enemy:isKongcheng() and enemy:getCardCount(true) > 2 then
          ...
          if notlose or (max_point > enemy_number and allknown) or max_point > 11 then
            sgs.ai_use_priority.HFenglveCard = 5
          end
					self.fenglve_card = max_card:getEffectiveId()
					use.card = FLCard
					if use.to then
...
function sgs.ai_skill_pindian.heg_fenglve(minusecard, self, requestor)
  local max_card = self:getMaxNumberCard()
...
sgs.ai_cardneed.heg_fenglve = sgs.ai_cardneed.bignumber
...
sgs.ai_skill_choice.heg_fenglve = function(self, choices, data)
  local target = data:toPlayer()
...
fenglvezongheng_skill.name = "heg_fenglvezongheng"
table.insert(sgs.ai_skills, fenglvezongheng_skill)
fenglvezongheng_skill.getTurnUseCard = function(self)
	if self:willShowForAttack() and not self.player:hasUsed("HFenglveZonghengCard") and not self.player:isKongcheng() then
    return sgs.Card_Parse("@HFenglveZonghengCard=.&heg_fenglvezongheng")
  end
end

sgs.ai_skill_use_func.HFenglveZonghengCard = function(FLCard, use, self)
	if #self.enemies == 0 and #self.friends_noself == 0 then return end
  sgs.ai_use_priority.HFenglveZonghengCard = 0.5
	local max_card = self:getMaxNumberCard()
	local max_point = max_card:getNumber()
	if self.player:hasShownSkill("heg_yingyang") then max_point = math.min(max_point + 3, 13) end
	...
	for _, enemy in ipairs(self.enemies) do
		if not enemy:isKongcheng() and enemy:getCardCount(true) > 1 then
          ...
          if notlose or (max_point > enemy_number and allknown) then
            sgs.ai_use_priority.HFenglveZonghengCard = 5
          end
...
function sgs.ai_skill_pindian.heg_fenglvezongheng(minusecard, self, requestor)
  local max_card = self:getMaxNumberCard()
...
sgs.ai_cardneed.heg_fenglvezongheng = sgs.ai_cardneed.bignumber

sgs.ai_skill_invoke.heg_anyong =  function(self, data)
  ...
  local function damageCount(tp,num,chained)
    local n = num
    if tp:hasShownSkill("heg_mingshi") and not from:hasShownAllGenerals() then
      n = n - 1
    end
    ...
    if tp:hasShownSkill("heg_gongqing") then
    ...
    for _, p in sgs.qlist(self.room:getOtherPlayers(target)) do
      if p:isChained() then
        ...
        if self:damageIsEffective_(damageStruct) then
          if from:hasSkill("heg_xinghuo") and damageStruct.nature == sgs.DamageStruct_Fire then--xinghuo是预置加伤可连续传导
...
--羊祜
sgs.ai_skill_invoke.heg_deshao = function(self, data)
  if not self:willShowForDefence() then
    return false
  end
  local target = data:toPlayer()
...
local mingfa_skill = {}
mingfa_skill.name = "heg_mingfa"
table.insert(sgs.ai_skills, mingfa_skill)
mingfa_skill.getTurnUseCard = function(self)
	if self.player:hasUsed("HMingfaCard") then return end
	return sgs.Card_Parse("@HMingfaCard=.&heg_mingfa")
end

sgs.ai_skill_use_func.HMingfaCard = function(card, use, self)
  local target
  ...
  if #self.enemies == 0 then
    ...
    for _, p in sgs.qlist(self.room:getAlivePlayers()) do
      if not self.player:willBeFriendWith(p) and p:getMark("##heg_mingfa") < 1 then
    ...
    for _, p in ipairs(self.enemies) do
      if p:getMark("##heg_mingfa") < 1 then
        target = p
        break
      end
    end
...
sgs.ai_use_priority.HMingfaCard = 0.5

sgs.ai_skill_choice.heg_mingfa = function(self, choices, data)
  local target = data:toPlayer()
  if self:isFriend(target) then
    return "yes"
  end
...
mingfazongheng_skill.name = "heg_mingfazongheng"
table.insert(sgs.ai_skills, mingfazongheng_skill)
mingfazongheng_skill.getTurnUseCard = function(self)
	if self.player:hasUsed("HMingfaZonghengCard") or self.player:isNude() then return end
  local target
  ...
  if #self.enemies == 0 then
    local targets = {}
    for _, p in sgs.qlist(self.room:getAlivePlayers()) do
      if not self.player:isFriendWith(p) and p:getMark("##heg_mingfa") < 1 then
        table.insert(targets, p)
      end
    end
    table.sort(targets, compare_func)
  ...
  (math.min(self.player:getHandcardNum(), self:getOverflow(self.player,true))
    ...
    cards = sgs.QList2Table(cards)
    self:sortByUseValue(cards, true)
    return sgs.Card_Parse("@HMingfaZonghengCard=" .. cards[1]:getEffectiveId() .."&heg_mingfazongheng")
  end
end

sgs.ai_skill_use_func.HMingfaZonghengCard = function(card, use, self)
...
sgs.ai_use_priority.HMingfaZonghengCard = 2
