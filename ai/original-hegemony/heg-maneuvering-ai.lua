-- Source: TODO/QSanguosha-For-Hegemony-xxyheaven/lua/ai/maneuvering-ai.lua (cf61c15).
--[[********************************************************************
...
function sgs.ai_weapon_value.Fan(self, enemy,player)
	...
	--if sgs.originalHegemonyHasShownSkills(player, "liegong|liegong_xh") then return 3.1 end
end

function sgs.ai_armor_value.Vine(player, self)
	if self:needKongcheng(player) and player:getHandcardNum() == 1 then
		return player:hasShownSkill("heg_kongcheng") and 5 or 3.8
	end
	...
	for _, enemy in ipairs(self:getEnemies(player)) do
		...
		if (enemy:canSlash(player) and enemy:hasWeapon("Fan")) or sgs.originalHegemonyHasShownSkills(enemy, "heg_huoji|heg_midao") then return -2 end
	...
	for _, p in sgs.qlist(self.room:getOtherPlayers(player)) do
		if p:hasShownSkill("heg_luanji") and p:getHandcardNum() > 3 then return 4.2 end
	end
...
function SmartAI:shouldUseAnaleptic(target, card_use)
	...
	for _, p in sgs.qlist(self.room:getAlivePlayers()) do
		if p:hasShownSkill("heg_qianhuan") and not p:getPile("sorcery"):isEmpty() and p:getKingdom() == target:getKingdom() and card_use.to:length() <= 1 then
			return false
		end
	end
	...
	if target:hasShownSkill("heg_gongqing") and self.player:getAttackRange() < 3 then
		return false
	end
	...
	if self.player:hasSkills("wushuang|heg_wushuang_lvlingqi") then
		if getKnownCard(target, target, "Jink", true, "he") >= 2 then return false end
		return getCardsNum("Jink", target, self.player) < 2
	end
	if self.player:hasSkills("heg_tieqi|heg_tieqi_xh") then
		return true
	end
if self.player:hasShownSkill("tenyearjianchu") and (target:hasEquip() or target:getCardCount(true) == 1) then
		return true
	end
	if target:getMark("##heg_qianxi+no_suit_red") > 0 and not target:hasShownSkill("qingguo") then
		return true
	end
...
function SmartAI:useCardSupplyShortage(card, use)
	...
	local getvalue = function(enemy)
		if card:isBlack() and enemy:hasShownSkill("heg_weimu") then return -100 end
		...
		if sgs.originalHegemonyHasShownSkills(enemy, sgs.priority_skill) then
		  value = value + 3
		end
		if sgs.originalHegemonyHasShownSkills(enemy, sgs.cardneed_skill) then
			value = value + 5
		end
		--[[if sgs.originalHegemonyHasShownSkills(enemy, sgs.drawcard_skill) or (enemy:hasShownSkill("zaiqi") and enemy:getLostHp() > 2) then
			value = value + 5
		end]]
		...
		if sgs.originalHegemonyHasShownSkills(enemy, "guanxing|"..sgs.wizard_skill) then value = value - 5 end
...
function SmartAI:isGoodChainTarget_(damageStruct)
	...
	if nature == sgs.DamageStruct_Fire and from:hasSkill("heg_xinghuo") then
		damage = damage + 1
	end
	...
	local function getChainedPlayerValue(target, dmg)
		...
		if damage + (dmg or 0) >= target:getHp() then
			...
			if self:isEnemy(target) and self:isFriend(from) and from:getHandcardNum() < 2 and sgs.originalHegemonyHasShownSkills(target, "heg_ganglie") and from:getHp() == 1
...
function SmartAI:useCardIronChain(card, use)
	...
	for _, friend in ipairs(self.friends) do
		if friend:isChained() and not self:isGoodChainPartner(friend) and self:trickIsEffective(card, friend, self.player) then
			...
			elseif not friend:hasShownSkill("heg_fenming") then
	...
	local chainSelf = self:trickIsEffective(card, self.player, self.player) and not self.player:isChained()
						and (not self.player:hasSkill("heg_duanxie") or self.player:hasUsed("HDuanxieCard"))
...
function SmartAI:useCardFireAttack(fire_attack, use)
	...
	local can_fire_attack = function(enemy)
		...
		return self:objectiveLevel(enemy) > 3 and not enemy:isKongcheng()
				...
				and (not (enemy:hasShownSkill("nosjianxiong") and not self:isWeak(enemy)) and not self:needDamagedEffects(enemy, self.player)
	...
	if can_FireAttack_self and self.player:isChained() and self:isGoodChainTarget(self.player, nil, nil, nil, fire_attack)
		...
		elseif HasBuquEffect(self.player)then
			...
			if self.player:hasSkill("heg_enyuan") then leastHP = leastHP + 1 end
			if self.player:hasSkill("heg_congjian") then leastHP = leastHP + 1 end
			if self.player:hasShownSkill("heg_gongqing") and self.player:getAttackRange() > 3 then leastHP = leastHP + 1 end
			if self.player:hasSkill("heg_xinghuo") then leastHP = leastHP + 1 end
	...
	if ((suitnum == 2 and lack.diamond == false) or suitnum <= 1)
		and self:getOverflow() <= ((self.player:hasSkill("heg_jizhi") and not fire_attack:isVirtualCard()) and -2 or 0)
		and #targets == 0 then
			return
	end
	...
	for _, enemy in ipairs(enemies) do
		local damage = 1
		if self.player:hasSkill("heg_xinghuo") then
			damage = damage + 1
		end
		if enemy:hasShownSkill("heg_mingshi") and not self.player:hasShownAllGenerals() then
			damage = damage - 1
		end
		...
		if enemy:hasShownSkill("heg_gongqing") then
