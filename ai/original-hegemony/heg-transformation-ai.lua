-- Source: TODO/QSanguosha-For-Hegemony-xxyheaven/lua/ai/transformation-ai.lua (cf61c15).
...
---荀攸
local qice_skill = {}
qice_skill.name = "heg_qice"
table.insert(sgs.ai_skills, qice_skill)
qice_skill.getTurnUseCard = function(self)
	if self.player:hasUsed("HQiceCard") or self.player:isKongcheng() then return end
	...
	local cardsavailable = function(use_card, hcardnum)--增加手牌数方便判断吃桃
		local target_num = 0
		if use_card:isKindOf("HAllianceFeast") then
			...
			for _, p in sgs.qlist(self.room:getOtherPlayers(self.player)) do--计算人数最少的势力
				...
				and (self.player:getRole() == "careerist" or self:originalHegemonyOwnKingdom() ~= p:getKingdom()) then
			...
			return false
		elseif use_card:isKindOf("HAwaitExhausted") then
			for _, p in sgs.qlist(self.room:getAlivePlayers()) do
				...
				and p:getRole() ~= "careerist" and p:hasShownOneGeneral() and p:getKingdom() == self:originalHegemonyOwnKingdom() then
					target_num = target_num + 1
				end
			end
		elseif use_card:getSubtype() == "global_effect" and not use_card:isKindOf("HFightTogether") then
			for _, p in sgs.qlist(self.room:getAlivePlayers()) do
				if not p:isRemoved() and not self.player:isProhibited(p, use_card) then
					target_num = target_num + 1
				end
			end
		elseif use_card:getSubtype() == "aoe" and not use_card:isKindOf("HBurningCamps") then
			for _, p in sgs.qlist(self.room:getOtherPlayers(self.player)) do
				if not p:isRemoved() and not self.player:isProhibited(p, use_card) then
					target_num = target_num + 1
				end
			end
		elseif use_card:isKindOf("HBurningCamps") then
...
--[[{联军盛宴,火烧连营,桃园结义,南蛮入侵,万箭齐发,
	...
	"collateral","known_both","fire_attack","heg_threaten_emperor"}--几乎不会用到的单体锦囊
	...
	for _,card in ipairs(cards) do
		...
		if card:isKindOf("HThreatenEmperor") and self:originalHegemonyIsBigKingdomPlayer(self.player) then--手牌多可以aoe的时候？
			return
		end
	...
	local str = "@HQiceCard=" .. id .. ":"
	sgs.ai_use_priority.HQiceCard = 0.05--一般在最后，考虑有挟天子的情况？
	if handcardnum == 1 then
		sgs.ai_use_priority.HQiceCard = 3
	end
...
--还需要设置优先度
		...
		if table.contains(available_tricks,"alliance_feast") and not known_null then
			...
			if dummyuse.card then
				if self.qicenum["alliance_feast"] and self.qicenum["alliance_feast"] == handcardnum then
					sgs.ai_use_priority.HQiceCard = 8
				end
	...
	if not has_peach and table.contains(available_tricks,"god_salvation") then--桃园
		...
		if self:willUseGodSalvation(cloneg) then
			if self.qicenum["god_salvation"] and self.qicenum["god_salvation"] == handcardnum then
				sgs.ai_use_priority.HQiceCard = 3--调虎离山是4.9
			end
	...
	if not has_peach and table.contains(available_tricks,"alliance_feast") and self.player:getLostHp() > 1 then-- 联军
		...
		if dummyuse.card then
			if self.qicenum["alliance_feast"] and self.qicenum["alliance_feast"] == handcardnum and self:getAllPeachNum() < 1 then
				sgs.ai_use_priority.HQiceCard = 8--优先度是否合适？
			end
	...
	if not has_peach and table.contains(available_tricks,"burning_camps") then
		...
		if dummyuse.card and can_burn then
			if self.qicenum["burning_camps"] and self.qicenum["burning_camps"] == handcardnum then
				sgs.ai_use_priority.HQiceCard = 1.5
			end
	...
	if (table.contains(available_tricks,"archery_attack") or table.contains(available_tricks,"savage_assault")) then--万剑、南蛮
		...
		local heg_zhurong = sgs.findPlayerByShownSkillName("heg_juxiang")--小心祝融拿牌
		local heg_caocao = sgs.findPlayerByShownSkillName("nosjianxiong")
		local heg_mengda = sgs.findPlayerByShownSkillName("heg_qiuan")
		--[[
		...
		and (not heg_caocao or self:isFriend(heg_caocao))
		and (not heg_mengda or self:isFriend(heg_mengda)) then
		...
		and (not heg_caocao or self:isFriend(heg_caocao))
		and (not heg_mengda or self:isFriend(heg_mengda))
		and (not heg_zhurong or self:isFriend(heg_zhurong)) then
...
--参考的身份
sgs.ai_skill_use_func.HQiceCard = function(card, use, self)
...
sgs.ai_skill_choice["transform_qice"] = function(self, choices)
	...
	local importantsklii = {"heg_yiji", "guicai", "mobilefangzhu", "heg_luoshen", "heg_jieming", "heg_jieyue", "heg_shicai", "heg_wanggui", "heg_sidi"}--还有哪些？
	...
	for _, skill in ipairs(skills) do
		...
		if skill:objectName() == "heg_qice" and not (sgs.originalHegemonyGeneral(self.player, true):getKingdom() == "careerist" and self:isWeak()) then--换自己
			return "no"
		end
...
sgs.ai_skill_invoke.heg_zhiyu = function(self, data)
...
sgs.ai_skill_invoke.heg_wanwei = true
...
sgs.ai_skill_invoke.heg_yuejian = function(self, data)
...
xiongsuan_skill.name = "heg_xiongsuan"
...
xiongsuan_skill.getTurnUseCard = function(self)
	...
	for _, acard in ipairs(cards) do
		if not self:isValuableCard(acard) then
			return sgs.Card_Parse("@HXiongsuanCard=".. acard:getEffectiveId() .."&heg_xiongsuan")
		end
	end
end

sgs.ai_skill_use_func.HXiongsuanCard = function(card, use, self)
	...
	if not target then--大部分参考苦肉，暂未考虑给无限定技的队友
		...
		if self.player:getHp() > 1 and
			((self.player:hasSkill("heg_luanji") and self:getAoeValue(sgs.cloneCard("archery_attack")) > 0)
			or (self.player:hasSkill("shuangxiong") and self.player:hasFlag("shuangxiong"))) then--攻击技能
			can_xiongsuan = true
		end
		if self.player:hasSkills("heg_qianhuan|heg_jihun|heg_bushi|heg_chenglve") and self:getCardsNum("Peach") >= 1 then--卖血技能
			can_xiongsuan = true
		end
		if self.player:hasSkill("heg_congjian") then
			can_xiongsuan = false
		end
		if self.player:hasSkill("heg_yuanyu") then
			can_xiongsuan = true
		end
		if can_xiongsuan then
			self.xiongsuan_skill = "heg_xiongsuan"
			target = self.player
		end
	end
	if target then
...
sgs.ai_skill_choice.heg_xiongsuan = function(self, choices, data)
	return self.xiongsuan_skill
end

sgs.ai_card_intention.HXiongsuanCard = -40
sgs.ai_use_priority.HXiongsuanCard = 6.8--复制的苦肉优先度
		...
		if sgs.lose_equip_skill:match(skill) then
			...
			for _, p in sgs.qlist(self.room:getOtherPlayers(player)) do
				if self:isFriend(p) then
					if self:hasKnownSkill("heg_duoshi", p) then
						current_value = current_value + 0.4
					end
...
local yigui_skill = {}
yigui_skill.name = "heg_yigui"
table.insert(sgs.ai_skills, yigui_skill)
yigui_skill.getTurnUseCard = function(self)
	if not self.player:hasShownSkill("heg_yigui") then return end
	local huashens = self:getGeneralPile(self.player, "soul")
...
--[[{联军盛宴,火烧连营,桃园结义,南蛮入侵,万箭齐发,
	...
	if #yigui_kingdom[self:originalHegemonyOwnKingdom()] > 0 and not self.player:hasFlag("Yigui_GodSalvation") then
		if (#yigui_kingdom[self:originalHegemonyOwnKingdom()] < 3 and (weak_count > 1 or wounded_count > 2))
		or (#yigui_kingdom[self:originalHegemonyOwnKingdom()] >=3 and wounded_count > 1) then
			soul_name = yigui_kingdom[self:originalHegemonyOwnKingdom()][1]
	...
	for key, value in pairs(kingdoms) do
		...
		if key and self:originalHegemonyOwnKingdom() ~= key and value < f_num then
	...
	local function getYiguiAoeValue(AOE_name,kingdom,kingdom2)--甚至可以加上判断暗将桃园、五谷
		...
		if clone_trick:isKindOf("SavageAssault") then
			...
			if heg_menghuo and not self:isFriend(heg_menghuo) and heg_menghuo:hasSkill("heg_zhiman") then
				return 0
			end
		end
		local value = 0
			...
			for _, p in sgs.qlist(self.room:getOtherPlayers(self.player)) do
				if not p:isRemoved() and not p:hasShownOneGeneral() and self:aoeIsEffective(clone_trick, p, self.player) then
					if self:evaluateKingdom(p) == "unknown"then
						value = value + 0.5
					elseif string.find(self:evaluateKingdom(p), self:originalHegemonyOwnKingdom()) then
						value = value - 1
					else
						value = value + 1
					end
	...
	if #yigui_kingdom["double"] > 0 then
		for _, name in ipairs(yigui_kingdom["double"]) do
			...
			if table.contains(double_kingdoms,self:originalHegemonyOwnKingdom()) then
				table.removeOne(double_kingdoms,self:originalHegemonyOwnKingdom())
...
sgs.ai_skill_use_func.HYiguiCard = function(card, use, self)
	local userstring = card:toString()
	userstring = (userstring:split(":"))[3]
...
sgs.ai_use_priority.HYiguiCard = 2.8

function sgs.ai_cardsview.heg_yigui(self, class_name, player)
	if not player:hasShownSkill("heg_yigui") then return end
	local huashens = self:getGeneralPile(player, "soul")
	if  #huashens == 0 then return end
	if class_name == "Peach" or class_name == "Analeptic" then
		...
		if dying then
			...
			if soul_name and class_string then
				return "@HYiguiCard=.&" .. ":".. class_string .. "+" .. soul_name
			end
		end
	end
end

sgs.ai_skill_invoke.heg_jihun = true

--沙摩柯
sgs.ai_skill_invoke.heg_jili = function(self, data)
...
function SmartAI:shamokeUseWeaponPriority(card)
	...
	if self.player:hasSkill("heg_jili") and not self.player:isKongcheng()
	and card:isKindOf("Weapon") and not card:isKindOf("Crossbow") then
		...
		if sgs.getHegemonyCardUsedTimes(self.player, ".") + sgs.getHegemonyCardRespondedTimes(self.player, ".") + 2 == sgs.weapon_range[class_name] + sgs.Sanguosha:correctAttackRange(self.player,true,false)
...
sgs.ai_cardneed.heg_jili = sgs.ai_cardneed.weapon

--马谡
sgs.ai_skill_invoke.heg_zhiman = function(self, data)
	...
	if self:hasKnownSkill(sgs.masochism_skill, target) and self.player:canGet(target, "e")
...
sgs.ai_choicemade_filter.skillInvoke.heg_zhiman = function(self, player, promptlist)
...
sgs.ai_skill_choice.heg_zhiman = function(self, choices)
...
sgs.ai_skill_choice["transform_zhiman"] = function(self, choices)
	...
	if sgs.ai_AOE_data then--变更只能一次，判断aoe保留变更。是否命令变更没有信息判断，只能放这在
		...
		for _, p in sgs.qlist(use.to) do
			if self.player:isFriendWith(p) and self:playerGetRound(p) > self:playerGetRound(self.player) then
				local p_skills = sgs.originalHegemonySlotSkills(p, false)
				...
				if sgs.originalHegemonyGeneral(p, false):objectName():match("sujiang") then
					save_transform = true
				end
			end
	...
	local importantsklii = {"heg_xuanhuo", "heg_paoxiao", "tenyearkuanggu", "heg_tieqi", "heg_jizhi", "heg_shengxi",  "heg_jili", "heg_tongdu"}
	local skills = sgs.originalHegemonySlotSkills(self.player, false)
	for _, skill in ipairs(skills) do
		if table.contains(importantsklii, skill:objectName()) then--重要技能
			return "no"
	...
	if self.player:hasSkills("heg_paoxiao+tenyearwusheng") then
		return "no"
	end
...
sgs.ai_choicemade_filter.cardChosen.heg_zhiman = function(self, player, promptlist)
	local intention = 10
...
sanyao_skill.getTurnUseCard = function(self)
	...
	if self.player:hasUsed("HSanyaoCard") then return end
	if self.player:isNude() then return end
	return sgs.Card_Parse("@HSanyaoCard=.&heg_sanyao")
end

sgs.ai_skill_use_func.HSanyaoCard = function(card, use, self)
	...
	if self.player:getMark("zhimantransformUsed") == 0 then--增加优先变将
		for _, p in sgs.qlist(targets) do
			if self.player:isFriendWith(p) then
				...
				local skills = sgs.originalHegemonySlotSkills(p, false)
				for _, skill in ipairs(skills) do
					if skill:getFrequency() == sgs.Skill_Limited and (skill:getLimitMark() ~= "" and p:getMark(skill:getLimitMark()) == 0) then--限定技已发动
						target = p
						break
	...
	if not target then
		for _, p in sgs.qlist(targets) do
			if not self:isFriend(p) and self:getDangerousCard(p) and self.player:canGet(p, self:getDangerousCard(p)) then
				target = p
				break
			end
		end
	...
	if self:needToThrowArmor() then
		use.card = sgs.Card_Parse("@HSanyaoCard=" .. self.player:getArmor():getId() .. "&heg_sanyao")
		...
		if card_id then
			use.card = sgs.Card_Parse("@HSanyaoCard=" .. card_id .. "&heg_sanyao")
			if use.to then
...
--凌统
sgs.ai_skill_playerchosen.heg_xuanlue = function(self, targets)
	if not (self:willShowForAttack() or self:willShowForDefence()) then return nil end
	return self:findPlayerToDiscard()
end

sgs.ai_choicemade_filter.cardChosen.heg_xuanlue = function(self, player, promptlist)
	local intention = 10
	local id = promptlist[3]
	local card = sgs.Sanguosha:getCard(id)
	local target = self.room:findPlayerByObjectName(promptlist[5])
...
sgs.ai_cardneed.heg_xuanlue = sgs.ai_cardneed.equip

local yongjin_skill = {}
yongjin_skill.name = "heg_yongjin"
...
yongjin_skill.getTurnUseCard = function(self)
	...
	return sgs.Card_Parse("@HYongjinCard=.&")
end

sgs.ai_skill_use_func.HYongjinCard = function(card, use, self)
	...
	if equip_count > 2 or (hasSilverLion and weak_count > 1) or (self.player:getHp() == 1 and equip_count > 1) then
		use.card = sgs.Card_Parse("@HYongjinCard=.&heg_yongjin")
	end
end

sgs.ai_use_priority.HYongjinCard = 10--优先度多少合适？

sgs.ai_skill_use["@@yongjin_move"] = function(self, prompt, method)
	if prompt ~= "@heg_yongjin-next" then
		return "."
	end
	local YJMoveCard = "@YongjinMoveCard=.&->"

		for _, friend in ipairs(self.friends_noself) do
			if friend:hasEquip() and sgs.originalHegemonyHasShownSkills(friend, sgs.lose_equip_skill) and self:getMoveCardorTarget(friend, "." ,"e") then
...
sgs.ai_skill_invoke.heg_diaodu = function(self, data)
	--if not self:willShowForAttack() then return false end
	return true
end

sgs.ai_skill_playerchosen.heg_diaodu = function(self, targets)--还可以细化条件，装备转移？
...
sgs.ai_skill_cardchosen.heg_diaodu = function(self, who, flags, method, disable_list)
	self.diaodu_id = nil
...
sgs.ai_skill_choice["heg_diaodu"] = function(self, choices, data)
	return "yes"
end

sgs.ai_skill_invoke.heg_diancai = function(self, data)
	if not self:willShowForDefence() then return false end
	return true
end
...
sgs.ai_skill_choice["transform_diancai"] = function(self, choices)
	...
	local importantsklii = {"heg_xiaoji", "heg_xuanlue", "heg_tianxiang", "heg_guose", "heg_yingzi_zhouyu", "heg_zhukou"}--还有哪些？
	local skills = sgs.originalHegemonySlotSkills(self.player, false)
	for _, skill in ipairs(skills) do
		if table.contains(importantsklii, skill:objectName()) then--重要技能
		...
		if skill:objectName() == "heg_diancai" and not self:isWeak() then--换自己
	...
	if self.player:hasSkill("heg_keshou") then
	...
	if self.player:hasSkills("heg_keshou|heg_hunshang") and self:isWeak() and sgs.findPlayerByShownSkillName("heg_buqu") then
		return "no"
	end
	if self.player:inDeputySkills("heg_buqu") then
	...
	if (sgs.general_value[g2name] and sgs.general_value[g2name] < 7) then--or self.player:inDeputySkills("heg_yinghun_sunjian")
...
--君孙权
local lianzi_skill = {}
lianzi_skill.name = "heg_lianzi"
table.insert(sgs.ai_skills, lianzi_skill)
lianzi_skill.getTurnUseCard = function(self)
	if self.player:hasUsed("HLianziCard") or self.player:isKongcheng() then return end
	...
	if num >= 3 then
		...
		return sgs.Card_Parse("@HLianziCard=" .. hcards[1]:getEffectiveId() .. "&heg_lianzi")
	end
end

sgs.ai_skill_use_func.HLianziCard = function(card, use, self)
	use.card = card
end

sgs.ai_skill_invoke.heg_jubao = true

sgs.ai_skill_cardchosen.heg_jubao = function(self, who, flags, method, disable_list)
...
flamemap_skill.name = "heg_flamemap"
...
flamemap_skill.getTurnUseCard = function(self)
	...
	if not self.player:hasUsed("HFlameMapCard") then
		return sgs.Card_Parse("@HFlameMapCard=.&showforviewhas")
	end
end

sgs.ai_skill_use_func.HFlameMapCard = function(card,use,self)
	local heg_sunquan = self.room:getLord(self:originalHegemonyOwnKingdom())
	if not heg_sunquan or not heg_sunquan:hasLordSkill("heg_jiahe") then return end
	local full = (heg_sunquan:getPile("flame_map"):length() >= 5)
	sgs.ai_use_priority.HFlameMapCard = 0
	if self.player:hasSkills(sgs.lose_equip_skill) then
		...
		for _, hcard in sgs.qlist(self.player:getCards("h")) do
			if hcard:isKindOf("EquipCard") and self:getSameEquip(hcard) then
				...
				if dummy_use.card and dummy_use.card:isKindOf("EquipCard") then
					sgs.ai_use_priority.HFlameMapCard = 20
					--global_room:writeToConsole("弃置装备技能有相同武器2：" .. self:getSameEquip(hcard):getLogName())
					use.card = sgs.Card_Parse("@HFlameMapCard=" .. self:getSameEquip(hcard):getEffectiveId() .. "&showforviewhas")
					return
				end
			end
		end
		if self:needToThrowArmor() then
			...
			use.card = sgs.Card_Parse("@HFlameMapCard=" .. self.player:getArmor():getEffectiveId() .. "&showforviewhas")
			return
		end
		if self.player:getOffensiveHorse() then
			--self.player:speak("有返回：" .. self.player:getOffensiveHorse():getLogName())
			use.card = sgs.Card_Parse("@HFlameMapCard=" .. self.player:getOffensiveHorse():getEffectiveId() .. "&showforviewhas")
			return
		end
		if self.player:getWeapon() then
			--self.player:speak("有返回：" .. self.player:getWeapon():getLogName())
			use.card = sgs.Card_Parse("@HFlameMapCard=" .. self.player:getWeapon():getEffectiveId() .. "&showforviewhas")
			return
		end
		if self.player:getArmor() and not(self.player:getArmor():isKindOf("HPeaceSpell") and self:isWeak()) then
			--self.player:speak("有返回：" .. self.player:getArmor():getLogName())
			use.card = sgs.Card_Parse("@HFlameMapCard=" .. self.player:getArmor():getEffectiveId() .. "&showforviewhas")
			return
		end
		if self.player:getDefensiveHorse() then
			...
			use.card = sgs.Card_Parse("@HFlameMapCard=" .. self.player:getDefensiveHorse():getEffectiveId() .. "&showforviewhas")
			return
		end
	else
			...
			if hcard:isKindOf("EquipCard") then
				...
				if not dummy_use.card then
					if self.room:getCardPlace(hcard:getEffectiveId()) == sgs.Player_PlaceHand then
						...
						use.card = sgs.Card_Parse("@HFlameMapCard=" .. hcard:getEffectiveId() .. "&showforviewhas")
						return
					end
				else
...
sgs.ai_use_value.HFlameMapCard = 10

--缘江烽火图【宏图】
sgs.ai_skill_choice.heg_flamemap = function(self, choices)
	--英姿、好施、涉猎、度势
	...
	choices = choices:split("+")
	local heg_sunquan = self.room:getLord(self:originalHegemonyOwnKingdom())
	local n = heg_sunquan:getPile("flame_map"):length()

	local congcha_draw = false
	if self.player:hasSkill("heg_congcha") then--不臣新加入的摸牌技能
		congcha_draw = true
		for _, p in sgs.qlist(self.room:getAlivePlayers()) do
			if not p:hasShownOneGeneral() then
				congcha_draw = false
	...
	if self.player:hasSkill("haoshi") and table.contains(choices, "heg_haoshi_flamemap") then
		table.removeOne(choices, "heg_haoshi_flamemap")--太复杂情况不考虑
	...
	if n > 4 and table.contains(choices, "heg_haoshi_flamemap") and
	self.player:getHandcardNum() > 3 and sgs.ai_skill_invoke.heg_haoshi_flamemap(self) then--手牌大于3时触发好施必定有队友
		return "heg_haoshi_flamemap"--手牌充裕时好施给队友
	end

	if self.player:isKongcheng() and not (self.player:hasSkills("heg_yingzi_zhouyu|heg_yingzi_sunce|haoshi") or self.player:hasTreasure("JadeSeal") or congcha_draw) then
		table.removeOne(choices, "heg_duoshi_flamemap")--0张手牌，没额外摸牌技移除度势
	end

	if self.player:hasSkill("heg_duoshi") and self.player:getHandcardNum() < 4
	and not (self.player:hasSkills("heg_yingzi_zhouyu|heg_yingzi_sunce|heg_yingzi_flamemap|haoshi|heg_haoshi_flamemap|heg_shelie") or self.player:hasTreasure("JadeSeal") or congcha_draw) then
		table.removeOne(choices, "heg_duoshi_flamemap")--已有度势，没额外摸牌技和手牌少时移除度势
	end

	if n > 4 and table.contains(choices, "heg_duoshi_flamemap") then
		return "heg_duoshi_flamemap"--能选择两项则必选度势
	end

	if table.contains(choices, "heg_yingzi_flamemap") and table.contains(choices, "heg_haoshi_flamemap")  then
		if self.player:hasSkills("heg_yingzi_zhouyu|heg_yingzi_sunce") --已有英姿且能好施，0手牌或有目标
		and sgs.ai_skill_invoke.heg_haoshi_flamemap(self) then
			return "heg_haoshi_flamemap"
		end
		...
		if (self.player:getLostHp() > (self:willSkipPlayPhase() and 0 or 1)
			and not (self.player:hasSkill("heg_keji") or self.player:getMaxCards() > 3))--需要手牌上限
		or self.player:hasSkills("heg_yingzi_zhouyu|heg_yingzi_sunce")--已有英姿，不适合好施
		or self.player:hasTreasure("JadeSeal")--已有玉玺，不适合好施
		or congcha_draw then--聪察摸牌，不适合好施
			return "heg_yingzi_flamemap"
		end
	end

	if table.contains(choices, "heg_haoshi_flamemap") and sgs.ai_skill_invoke.heg_haoshi_flamemap(self) then
		return "heg_haoshi_flamemap"
	end

	if table.contains(choices, "heg_shelie") and sgs.ai_skill_invoke.heg_shelie(self) then
		return "heg_shelie"
	end

	if table.contains(choices, "heg_yingzi_flamemap") then
		return "heg_yingzi_flamemap"
	end

	if table.contains(choices, "heg_duoshi_flamemap") then
		return "heg_duoshi_flamemap"
	end
end

--缘江烽火图【烽火】：藏专属装备坑下其他君主，把定澜夜明珠放到烽火里面的话赶紧放出来，技能修改
sgs.ai_skill_askforag.heg_flamemap = function(self, card_ids)
	for _, id in ipairs(card_ids) do
		local card = sgs.Sanguosha:getCard(id)
		if card:isKindOf("HLuminousPearl") then
			return id
		end
	end
	for _, id in ipairs(card_ids) do
		local card = sgs.Sanguosha:getCard(id)
		if card:isKindOf("HDragonPhoenix") then
			local lord = self.room:getLord("shu")
			if not lord or self:isFriend(lord) then
				return id
			end
		elseif card:isKindOf("HPeaceSpell") then
...
sgs.ai_skill_invoke.heg_yingzi_flamemap = function(self, data)
	return true
end


--好施，复杂情况不考虑双好施
sgs.ai_skill_invoke.heg_haoshi_flamemap = sgs.ai_skill_invoke.haoshi
--[[
...
--涉猎
sgs.ai_skill_invoke.heg_shelie = function(self, data)
	if self.player:hasSkills("haoshi|heg_haoshi_flamemap") and self.player:getHandcardNum() < 2 then return false end
	local extra = 0
	if self.player:hasTreasure("JadeSeal") then
		extra = extra+1
	end
	if self.player:hasSkill("heg_yingzi_zhouyu") then
		extra = extra+1
	end
	if self.player:hasSkill("heg_yingzi_sunce") then
		extra = extra+1
	end
	if self.player:hasSkill("heg_yingzi_flamemap") then
		extra = extra+1
	end
	if sgs.ai_skill_invoke.haoshi(self) then
		...
		if self.player:hasSkill("heg_haoshi_flamemap") then
			extra = extra+2
		end
	end
	if self.player:hasSkill("heg_congcha") then
...
sgs.ai_skill_movecards.heg_shelie = function(self, upcards, downcards, min_num, max_num)
	local upcards_copy, enableds, down = table.copyFrom(upcards), table.copyFrom(upcards), {}
	while #enableds ~= 0 do
		local card = self:askForAG(enableds, false, "heg_shelie")
...
duoshi_flamemap_skill.name = "heg_duoshi_flamemap"
table.insert(sgs.ai_skills, duoshi_flamemap_skill)
duoshi_flamemap_skill.getTurnUseCard = function(self, inclusive)
	local DuoTime = 1
	if self.player:hasSkills("hongyan|heg_yingzi_zhouyu|heg_yingzi_sunce|heg_yingzi_flamemap|haoshi|heg_haoshi_flamemap") then
		DuoTime = 2
	end
	for _, player in ipairs(self.friends) do
		if sgs.originalHegemonyHasShownSkills(player, "heg_xiaoji|heg_xuanlue|heg_diaodu") then
			DuoTime = 2
			break
		end
	end
	if self.player:hasSkills("heg_xiaoji|heg_xuanlue|heg_diaodu") then
		DuoTime = 2
	...
	if red_card then
		...
		local card_str = string.format("await_exhausted:heg_duoshi_flamemap[%s:%d]=%d&heg_duoshi_flamemap",red_card:getSuitString(), red_card:getNumber(), red_card:getEffectiveId())
...
LuminousPearl_skill.name = "heg_LuminousPearl"
table.insert(sgs.ai_skills, LuminousPearl_skill)
LuminousPearl_skill.getTurnUseCard = function(self)
	if not self.player:hasUsed("HZhihengLPCard") and not self.player:hasShownSkill("heg_zhiheng") and self.player:hasTreasure("LuminousPearl") then
		return sgs.Card_Parse("@HZhihengLPCard=.")
	end
end

sgs.ai_use_priority.HLuminousPearl = 5.7
sgs.ai_keep_value.HLuminousPearl = 4.2


--变更武将相关
function sgs.readGeneralValuefromtxt()
    local values = {}
    for key, value in pairs(sgs.general_value) do values[key] = value end
    return values
end

function sgs.readGeneralPairValuefromtxt()
    local values = {}
    for key, value in pairs(sgs.general_pair_value) do values[key] = value end
    return values
end
...
function SmartAI:getGeneralValue(player, position)
	...
	if general:objectName() == "anjiang" then
		...
		if position then
			general = sgs.originalHegemonyGeneral(player, true)
		else
			general = sgs.originalHegemonyGeneral(player, false)
		end
	end
	local ajust = 0
...
function SmartAI:needToTransform()
	local g1 = sgs.originalHegemonyGeneral(self.player, true)
	local g2 = sgs.originalHegemonyGeneral(self.player, false)
	local current_value = 0
	for name, value in pairs(sgs.general_pair_value) do
		if g1:objectName() .. "+" .. g2:objectName() == name or g2:objectName() .. "+" .. g1:objectName() == name then
			current_value = value
...
--[[
	...
	for _, g2name in ipairs(generals) do
		...
		for pairname, value in pairs(sgs.general_pair_value) do
			if (g1name .. "+" .. g2name == pairname or g2name .. "+" .. g1name == pairname) then
				...
				elseif pairname == "heg_xunyou+heg_xiahouyuan" or pairname == "heg_lvfan+heg_sunjian"
				or pairname == "heg_masu+heg_menghuo" or pairname == "heg_masu+heg_zhurong" then--单独去掉一些选将配对值
					continue
	...
	local singlechoice
	for _, g2name in ipairs(generals) do
		if self:isWeak() and (g2name == "heg_pangtong" or g2name == "heg_xushu" or g2name == "heg_zhoutai" or g2name == "heg_sunce" or g2name == "heg_lukang") then
...
-- The pearl has its own registered card/history while sharing the discard heuristic.
sgs.ai_skill_use_func.HZhihengLPCard = function(card, use, self)
    local decide = sgs.ai_skill_use_func.heg_zhiheng
    if not decide then return end
    decide(card, use, self)
    if not use.card then return end
    local ids = {}
    local treasure = self.player:getTreasure()
    for _, id in sgs.qlist(card:getSubcards()) do
        if (not treasure or id ~= treasure:getEffectiveId()) and #ids < self.player:getMaxHp() then
            ids[#ids + 1] = id
        end
    end
    card:clearSubcards()
    for _, id in ipairs(ids) do card:addSubcard(id) end
    if #ids == 0 then use.card = nil end
end
sgs.ai_use_priority.HZhihengLPCard = sgs.ai_use_priority.heg_zhiheng or 2.61
