-- Source: TODO/QSanguosha-For-Hegemony-xxyheaven/lua/ai/overseas-ai.lua (cf61c15).
if not sgs.original_hegemony_ai_loading then return end

-- Native V2 conversions carry the declared object name and material in the card wire.
local function hegemonyConvertedCard(skill, name, material)
    local card = sgs.cloneCard(name, material:getSuit(), material:getNumber())
...
guishu_skill.getTurnUseCard = function(self)
    ...
    if trickcards[index] == "known_both" and self:getUseValue(spadecards[1]) > sgs.ai_use_value.HKnownBoth then
        return--#spadecards < 2 每回合不重置
    end
    --global_room:writeToConsole("鬼术卡类型:"..trickcards[index])
    return hegemonyConvertedCard("heg_guishu", trickcards[index], spadecards[1])
end


sgs.ai_use_priority.heg_guishu = sgs.ai_use_priority.HBefriendAttacking + 0.1
sgs.ai_use_value.heg_guishu = sgs.ai_use_value.HBefriendAttacking

sgs.guishu_suit_value = {
	spade = 3.9
}

sgs.ai_skill_invoke.heg_yuanyu = true

--曹真
sgs.ai_skill_invoke.heg_sidi = function(self, data)
...
sgs.ai_skill_exchange.heg_sidi = function(self,pattern,max_num,min_num,expand_pile)
...
--禁卡
        if getCardsNum("TrickCard", current, self.player) > (current:hasShownSkill("heg_jizhi") and 1 or 2)
        or (sgs.originalHegemonyHasShownSkills(current, "heg_guose|heg_luanji|heg_guishu") and current:getHandcardNum() > 1)
        or (current:hasShownSkill("heg_jixi") and current:getPile("field"):length() > 1)
        or (sgs.originalHegemonyHasShownSkills(current, "heg_qice|heg_yigui")) then
            if table.contains(drivetypes, "TrickCard") then
                self.sidi_cardtype = "TrickCard"
            end
        end
        if sgs.originalHegemonyHasShownSkills(current, "heg_diaodu+heg_xiaoji|heg_diaodu+heg_xuanlue") then
        ...
        or ((self:hasCrossbowEffect(current) or sgs.originalHegemonyHasShownSkills(current, sgs.force_slash_skill))
...
"heg_suzhi|heg_jinghe|heg_miewu|heg_jieyue|heg_jili|heg_tongdu|chuli|wansha|heg_zaoyun|heg_jinfa|heg_yingzi_zhouyu|heg_zhukou|heg_boyan"
        self.sidi_skill = findskill(sidi_firstskills)
        if sgs.originalHegemonyHasShownSkills(current, sgs.lose_equip_skill.."|heg_diaodu") and (not self.sidi_cardtype or self.sidi_cardtype ~= "EquipCard") then
            
        end
        if sgs.originalHegemonyHasShownSkills(current, "heg_luanji|heg_guose|heg_jixi|heg_qice|heg_yigui|heg_guishu") and (not self.sidi_cardtype or self.sidi_cardtype ~= "TrickCard") then
            self.sidi_skill = findskill("heg_luanji|heg_guose|heg_jixi|heg_qice|heg_yigui|heg_guishu")
        end
        if sgs.originalHegemonyHasShownSkills(current, "heg_paoxiao|tenyearkuanggu|heg_kuangcai") and (not self.sidi_cardtype or self.sidi_cardtype ~= "BasicCard") then
            self.sidi_skill = findskill("heg_paoxiao|tenyearkuanggu|heg_kuangcai")
        end
        if current:hasShownSkill("heg_hongfa") and current:getPile("heavenly_army"):isEmpty()
        and self.player:getPlayerNumWithSameKingdom("AI", "qun") > 1 then
            self.sidi_skill = "heg_hongfa"
        end
        if current:hasShownSkill("heg_jiahe") and not current:getPile("flame_map"):isEmpty() then
            self.sidi_skill = "heg_jiahe"
        end
        if current:hasShownSkill("heg_zisui") and current:getPile("&disloyalty"):length() > 2 then
            self.sidi_skill = "heg_zisui"
        end
        if current:hasShownSkill("heg_xiongnve") and current:getMark("#massacre") > (self:isWeak(current) and 1 or 3) then
            self.sidi_skill = "heg_xiongnve"
        end
        if current:hasShownSkill("heg_paiyi") and current:getPile("power_pile"):length() > 3 then
            self.sidi_skill = "heg_paiyi"
        end
    end
--回复
    local weis = {}
...
sgs.ai_skill_playerchosen["sidi_recover"] = function(self, targets)
    ...
    if self:isFriend(targets[1]) then
        ...
        for _, p in ipairs(targets) do
            if not sgs.originalHegemonyHasShownSkills(p, sgs.priority_skill) and p:getHp() > 1 then
                return p
            end
        end
        return targets[1]
...
function sgs.ai_cardneed.heg_sidi(to, card, self)
	return to:getHandcardNum() < 2
end

sgs.ai_need_damaged.heg_sidi = function (self, attacker, player)

end

--廖化
sgs.ai_skill_invoke.heg_dangxian = true

--诸葛瑾
sgs.ai_skill_cardask["@heg_huanshi-card"] = function(self, data)
...
function sgs.ai_cardneed.heg_huanshi(to, card, self)
...
hongyuan_skill.name = "heg_hongyuan"
table.insert(sgs.ai_skills, hongyuan_skill)
hongyuan_skill.getTurnUseCard = function(self)
	if self.player:isKongcheng() or self.player:hasUsed("HHongyuanCard") then return end
	return sgs.Card_Parse("@HHongyuanCard=.&heg_hongyuan")
--[[使用ViewAsSkill的写法
...
sgs.ai_skill_use_func.HHongyuanCard = function(hycard, use, self)
	...
	if #friends > 0 then
	    ...
	    self:sortByUseValue(cards, true)
		local card, friend = self:getCardNeedPlayer(cards, friends, "heg_hongyuan")
		if card and friend then
            use.card = sgs.Card_Parse("@HHongyuanCard=" .. card:getId() .. "&heg_hongyuan")
...
sgs.ai_use_priority.HHongyuanCard = 3.1
sgs.ai_card_intention.HHongyuanCard = -40

sgs.ai_skill_playerchosen.heg_hongyuan = function(self, targets)
	targets = sgs.QList2Table(targets)
...
sgs.ai_skill_invoke.heg_mingzhe = true

--全琮
sgs.ai_skill_playerchosen.qinzhong = function(self, targets)
    targets = sgs.QList2Table(targets)
    for _, p in ipairs(targets) do
        if p:hasShownSkill("heg_hunshang") and self.player:getHp() == 1 then
            return p
        end
    end

    local function g2value(player)
        local g2name = sgs.originalHegemonyGeneral(player, false):objectName()
...
--杨修
sgs.ai_skill_invoke.heg_danlao = function(self, data)
	...
	elseif ucard:isKindOf("IronChain") and self.player:isChained() then
        return false
    elseif ucard:isKindOf("HAwaitExhausted") and self.player:getHandcardNum() > 2
      and self:getCardsNum({"Peach", "Jink"}) == 0 then
        return false
    elseif ucard:isKindOf("HAllianceFeast") and use.from == self.player then
        return false
    elseif ucard:isKindOf("HConquering") then
        return false
    elseif ucard:isKindOf("ExNihilo") or ucard:isKindOf("HBefriendAttacking") then
        return false
...
sgs.ai_skill_invoke.heg_jilei = function(self, data)
	local damage = data:toDamage()
	return not self:isFriend(damage.from)
end

sgs.ai_skill_choice.heg_jilei = function(self, choices, data)
	local dfrom = data:toDamage().from
    local b_limited = dfrom:getMark("##heg_jilei+BasicCard") > 0
    local t_limited = dfrom:getMark("##heg_jilei+TrickCard") > 0
    local e_limited = dfrom:getMark("##heg_jilei+EquipCard") > 0
...
--祖茂
sgs.ai_skill_exchange["heg_yinbingx"] = function(self,pattern,max_num,min_num,expand_pile)
    local cards = self.player:getCards("he")
	cards = sgs.QList2Table(cards)
    self:sortByKeepValue(cards)
...
sgs.ai_skill_invoke.heg_yinbingx = false

sgs.ai_skill_invoke.heg_juedi = true

sgs.ai_skill_choice.heg_juedi = function(self, choices, data)
...
sgs.ai_skill_playerchosen.heg_juedi = function(self, targets)
    targets = sgs.QList2Table(targets)
    self:sort(targets, "hp")
    for _, p in ipairs(targets) do
...
--伏完
sgs.ai_skill_invoke.heg_moukui = function(self, data)
...
sgs.ai_skill_choice.heg_moukui = function(self, choices, data)
	local target = data:toPlayer()
...
--陈到
sgs.ai_skill_invoke.heg_wanglie = function(self, data)
...
--田豫
sgs.ai_skill_invoke.heg_zhenxi = function(self, data)
	local target = data:toPlayer()
    return not self:isFriend(target)
end

sgs.ai_skill_choice.heg_zhenxi = function(self, choices, data)
	local target = data:toPlayer()
...
sgs.ai_skill_use["@@heg_zhenxi_trick"] = function(self, prompt, method)
    local target
    ...
    if card and target then
        ...
        if suit == "diamond" then
            card_str = ("indulgence:_heg_zhenxi[diamond:%s]=%d&"):format(number, card_id)
        elseif suit == "club" then
            card_str = ("supply_shortage:_heg_zhenxi[club:%s]=%d&"):format(number, card_id)
        end
...
sgs.ai_skill_invoke.heg_jiansu = true

sgs.ai_skill_use["@@heg_jiansu"] = function(self, prompt, method)
    local money_cards = {}
    ...
    for _, p in ipairs(self.friends) do
        if p:getHp() <= num and self.player:isFriendWith(p) and p:canRecover() then
            ...
            return "@HJiansuCard=" .. table.concat(discards, "+") .. "->" .. p:objectName()
        end
    end
    for _, p in ipairs(self.friends) do
...
mumeng_skill.getTurnUseCard = function(self, inclusive)
    ...
    for _,c in ipairs(cards) do
        ...
        and not c:isKindOf("HBefriendAttacking") and self:getUseValue(c) < sgs.ai_use_value.HBefriendAttacking then
            b_card = c
            break
        end
    end
    for _,c in ipairs(cards) do
        ...
        and not c:isKindOf("HFightTogether") and self:getUseValue(c) < sgs.ai_use_value.HFightTogether then
            f_card = c
            break
        end
    end
...
sgs.ai_skill_playerchosen["naman_target"] = function(self, targets)
    ...
    elseif card:isKindOf("HAwaitExhausted") then
        if is_friend then
            return {}
        else
            return targetlist[1]
    ...
    elseif card:isKindOf("HFightTogether") then
        self:sort(tos, "defenseSlash")
        for _, p in ipairs(tos) do
...
--华雄
sgs.ai_skill_invoke.heg_yaowu = true

sgs.ai_skill_invoke.heg_shiyong = function(self, data)
    if self.player:inHeadSkills("heg_baoling") and not self.player:hasShownGeneral2() then
        return false
    end
...
--刘夫人
sgs.ai_skill_invoke.heg_zhuidu = function(self, data)
	local target = data:toPlayer()
    return not self:isFriend(target)
end
...
sgs.ai_skill_choice.zhuidu_choice = function(self, choices, data)
    ...
    if target:hasShownSkill("heg_gongqing") and from:getAttackRange() < 3 then
        return "damage"
    end
    --暂时不详细考虑-1伤害
    return "throw"
...
sgs.ai_skill_invoke.heg_shigong = function(self, data)
...
sgs.ai_skill_playerchosen.dingke = function(self, targets)
    ...
    for _, p in ipairs(targets) do
        if p:getPhase() == sgs.Player_NotActive and self.player:isFriendWith(p) and not self:needKongcheng(p) then
            if self.player:hasSkill("heg_shengxi") or self.player:getHandcardNum() > 2
...
sgs.ai_skill_invoke.kangrui = function(self, data)
    ...
    if self:isEnemy(target) and self:isWeak(target) then
        if target:hasArmorEffect("SilverLion") or (target:hasShownSkill("heg_gongqing") and friend:getAttackRange() < 3) then
            return false
        end
		local damage = {}
		damage.to = target
        ...
        if target:hasSkills("wushuang|heg_wushuang_lvlingqi") then
...
sgs.ai_skill_choice.kangrui = function(self, choices, data)
    if string.find(choices, "useduel") then
        local target = data:toPlayer()
        if target:hasArmorEffect("SilverLion") or (target:hasShownSkill("heg_gongqing") and self.player:getAttackRange() < 3) then
...
sgs.ai_skill_playerchosen.huxun = function(self, _targets, max_num, min_num)
	...
	self:sort(self.enemies, "defense")
		...
		if self.player:hasEquip() and sgs.originalHegemonyHasShownSkills(self.player, sgs.lose_equip_skill) and self:getMoveCardorTarget(self.player, ".", "e") then
...
--夏侯尚
sgs.ai_skill_playerchosen.heg_tanfeng = function(self, targets)
	local need_skip_judge = false
	local need_skip_discard = (self:getOverflow() > 1 and not self.player:hasSkills("heg_shensu|qiaobian") and not self.player:isSkipped(sgs.Player_Discard))
	...
	local prevent_skip = ((self.player:hasSkill("heg_jieyue") and self.player:getHandcardNum() > 1 and not self.player:isSkipped(sgs.Player_Draw))
						or (self.player:hasSkills("heg_wangxi|heg_qice|heg_mingfa|heg_daoshu|heg_zaoyun") and not self.player:isSkipped(sgs.Player_Play)))
	local dis_targets = {}
	for _, p in ipairs(targets) do
		if self:isFriend(p) then
...
sgs.ai_skill_cardchosen.heg_tanfeng = function(self, who, flags, method, disable_list)
	--dismantlement
...
sgs.ai_skill_choice.heg_tanfeng = function(self, choices)
	...
	if self:isFriend(current) then
		...
		if self:needDamagedEffects(self.player, current) or self:needToLoseHp() then
			...
			if not self:hasKnownSkill("heg_shensu", current) and string.find(choices, "judge") then return "judge" end
		end
	elseif self:isEnemy(current) then
		local need_skip_draw = (current:getMark("JieyueExtraDraw") > 0 or current:hasSkill("heg_zisui"))and string.find(choices, "draw")and not self.player:isSkipped(sgs.Player_Draw)
		local need_skip_play = self:hasKnownSkill("heg_wangxi|heg_qice|heg_mingfa|heg_daoshu|heg_zaoyun", current)and string.find(choices, "play")and not self.player:isSkipped(sgs.Player_Play)
...
sgs.ai_fill_skill.heg_guishu = guishu_skill.getTurnUseCard
