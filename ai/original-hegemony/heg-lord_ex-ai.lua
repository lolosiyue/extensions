-- Source: TODO/QSanguosha-For-Hegemony-xxyheaven/lua/ai/lord_ex-ai.lua (cf61c15).
...
sgs.ai_skill_invoke.heg_qiuan = function(self, data)
  ...
  if damage.card:isKindOf("AOE") then--优先于奸雄
    if self.get_AOE_subcard then
      ...
      return not self.player:hasSkill("nosjianxiong")
    end
    return true
  end
  ...
  if self.player:hasSkill("heg_wangxi") and damage.from and self:isFriend(damage.from) and damage.damage < 2 then
    return false
  end
	return true
end

sgs.ai_skill_invoke.heg_liangfan = true

sgs.ai_skill_choice.heg_liangfan = function(self, choices, data)
  local damage = data:toDamage()
...
--唐咨
sgs.ai_skill_invoke.heg_xingzhao = function(self, data)
...
sgs.ai_skill_invoke.heg_xunxun_tangzi = true

sgs.ai_skill_movecards.heg_xunxun_tangzi = function(self, upcards, downcards, min_num, max_num)
	local upcards_copy = table.copyFrom(upcards)
	local down = {}
	local id1 = self:askForAG(upcards_copy,false,"heg_xunxun_tangzi")
	down[1] = id1
	table.removeOne(upcards_copy,id1)
	local id2 = self:askForAG(upcards_copy,false,"heg_xunxun_tangzi")
	down[2] = id2
	table.removeOne(upcards_copy,id2)
	return upcards_copy,down
end
--张鲁
sgs.ai_skill_invoke.heg_bushi = function(self, data)
...
sgs.ai_skill_exchange.heg_bushi = function(self,pattern,max_num,min_num,expand_pile)
...
sgs.ai_skill_invoke.heg_midao = function(self, data)
	if not self:willShowForAttack() and not self:willShowForDefence() then
		return false
	end
...
sgs.ai_skill_cardask["@heg_midao-card"] = function(self, data)
...
sgs.ai_skill_use["@@heg_midao"] = function(self, prompt)
	local rice = {}
	for _, id in sgs.qlist(self.player:getPile("rice")) do
		table.insert(rice, sgs.Sanguosha:getCard(id))
	end
	if #rice == 0 then return "." end
	-- The V2 response selects one physical rice-pile card; the judge data is
	-- carried by the native request, while the prompt retains the donor reason.
	self:sortByKeepValue(rice)
	return "$" .. rice[1]:getEffectiveId()
end

sgs.ai_skill_use["@@heg_shilu"] = function(self, prompt)
	local count = self.player:getGeneralPile("massacre"):length()
	if count < 1 then return "." end
	local response = sgs.ai_skill_cardask["@heg_shilu"](self, nil, nil, nil, nil, tostring(count))
	if type(response) ~= "string" or response:sub(1, 1) ~= "$" then return "." end
	return "@DummyCard=" .. response:sub(2)
end

sgs.ai_skill_use["@@heg_chaosselect!"] = function(self)
	local cards = sgs.QList2Table(self.player:getHandcards())
	local usable = {}
	for _, card in ipairs(cards) do
		if not self.player:isJilei(card) then table.insert(usable, card) end
	end
	if #usable == 0 then return "." end
	self:sortByKeepValue(usable)
	local selected = {usable[1]}
	for index = 2, #usable do
		if usable[index]:getTypeId() ~= selected[1]:getTypeId() then
			table.insert(selected, usable[index])
			break
		end
	end
	if #selected == 1 then
		for _, card in ipairs(usable) do
			if card:getTypeId() ~= selected[1]:getTypeId() then return "." end
		end
	end
	local ids = {}
	for _, card in ipairs(selected) do table.insert(ids, card:getEffectiveId()) end
	return "@DummyCard=" .. table.concat(ids, "+")
end

sgs.ai_skill_use["@@heg_tongling_usecard"] = function(self)
	local target_name = self.player:property("tongling_usetarget"):toString()
	local target = target_name ~= "" and self.room:findPlayerByObjectName(target_name) or nil
	for _, card in sgs.qlist(self.player:getHandcards()) do
		if card:isAvailable(self.player) then
			if card:targetFixed() then return "@HTonglingCard=" .. card:getEffectiveId() end
			if target and not self.room:isProhibited(self.player, target, card) then
				local selected = sgs.SPlayerList()
				if card:targetFilter(selected, target, self.player) then
					selected:append(target)
				end
				if not selected:isEmpty() and card:targetsFeasible(selected, self.player) then
					return "@HTonglingCard=" .. card:getEffectiveId() .. "->" .. target_name
				end
			end
		end
	end
...
--糜芳＆傅士仁
sgs.ai_skill_invoke.heg_fengshix = function(self, data)
	if not self:willShowForAttack() then
    return false
  end
  ...
  if card:isKindOf("HDrowning") and target:getEquips():length() == 1 then
...
sgs.ai_skill_choice.heg_fengshix = function(self, choices, data)
  self.fengshix_discard = nil
...
sgs.ai_skill_cardchosen.heg_fengshix = function(self, who, flags, method, disable_list)
...
sgs.ai_cardneed.heg_fengshix = function(to, card, self)
...
sgs.ai_skill_exchange["wenji_give"] = function(self,pattern,max_num,min_num,expand_pile)
  local heg_liuqi = sgs.findPlayerByShownSkillName("heg_wenji")
  if self:isFriendWith(heg_liuqi) then--队友：杀、duel、AOE
...
function SmartAI:hasWenjiBuff(card)
	if self.player:hasSkill("heg_wenji") then
...
sgs.ai_skill_invoke.heg_tunjiang = true

--士燮
sgs.ai_skill_invoke.heg_lixia = true

sgs.ai_skill_choice.heg_lixia = function(self, choices, data)
  local heg_shixie = sgs.findPlayerByShownSkillName("heg_lixia")
  --[[if not shixie then
    return "no"
  end
  ...
  if self:isEnemy(heg_shixie) then
    local canslash_shixie = false
    for _, p in ipairs(self.friends) do
      if p:canSlash(heg_shixie, nil, true) then
        canslash_shixie = true
        break
      end
    end
    if self.player:getHp() > 2 and (not canslash_shixie
      or (heg_shixie:hasTreasure("WoodenOx") and heg_shixie:getPile("wooden_ox"):length() > 1)) then
      return "discard"
    end
  end
	return "draw"
...
quanjin_skill.getTurnUseCard = function(self, inclusive)
  ...
  if not self.player:hasUsed("HQuanjinCard") then
    ...
    if can_quanjin then
      return sgs.Card_Parse("@HQuanjinCard=.&heg_quanjin")
    end
  end
end

sgs.ai_skill_use_func.HQuanjinCard= function(qjcard, use, self)
  sgs.ai_use_priority.HQuanjinCard = 2.4
  if self.player:hasSkill("heg_daoshu") then
    sgs.ai_use_priority.HQuanjinCard = 2.95--盗书之前
  end
  local target
  ...
  local card_str = "@HQuanjinCard=" .. card:getEffectiveId() .."&heg_quanjin"
...
local zaoyun_skill = {}
zaoyun_skill.name = "heg_zaoyun"
table.insert(sgs.ai_skills, zaoyun_skill)
zaoyun_skill.getTurnUseCard = function(self, inclusive)
  if self.player:isKongcheng() then return end
  if not self.player:hasUsed("HZaoyunCard") and self.player:hasShownOneGeneral() then
    --self.player:speak("zaoyun技能卡:"..self.player:objectName())
    return sgs.Card_Parse("@HZaoyunCard=.&heg_zaoyun")
  end
end

sgs.ai_skill_use_func.HZaoyunCard= function(card, use, self)
  sgs.ai_use_priority.HZaoyunCard = 2.65--杀之前，怎么配合一技能先后？
  if self.player:hasSkill("heg_daoshu") then
    sgs.ai_use_priority.HQuanjinCard = 2.97--盗书之前
  end
  ...
  for _, p in ipairs(self.enemies) do
    ...
    and self.player:distanceTo(p) > 1 and self.player:getHandcardNum() + 1 >= self.player:distanceTo(p) then
      ...
      if p:getHp() == 1 and self:isWeak(p) and self.player:getHandcardNum() > 3 then
        sgs.ai_use_priority.HZaoyunCard = 3.4--AOE后，手牌充裕
        target = p
        break
      end
  ...
  if target then
    ...
    if #card_list == need_num then
      local card_str = ("@HZaoyunCard=" .. table.concat(card_list, "+") .."&heg_zaoyun")
      use.card =  sgs.Card_Parse(card_str)
      assert(use.card)
		  if use.to then
			  use.to:append(target)
...
sgs.ai_skill_invoke.pozhen = function(self, data)
  local target = data:toPlayer()
  local has_attack_skill = target:hasSkills("heg_luanji|shuangxiong")
...
sgs.ai_skill_choice["heg_qiance"] = function(self, choices, data)
...
sgs.ai_skill_use["@@heg_jujian"] = function(self)
	local cards, targets = {}, {}
	for _, card in sgs.qlist(self.player:getCards("he")) do
		if not card:isKindOf("BasicCard") and not self.player:isJilei(card) then table.insert(cards, card) end
	end
	for _, target in sgs.qlist(self.room:getAlivePlayers()) do
		if self.player:isFriendWith(target) and target:getMark("jujiantransformUsed") == 0 then table.insert(targets, target) end
	end
	if #cards == 0 or #targets == 0 then return "." end
	self:sortByKeepValue(cards)
	self:sort(targets, "defense")
	local response = sgs.ActiveSkillCard()
	response:setSkillName("heg_jujian")
	response:addSubcard(cards[1]:getEffectiveId())
	return response:toString() .. "->" .. targets[1]:objectName()
end

--吴景
local diaogui_skill = {}
diaogui_skill.name = "heg_diaogui"
table.insert(sgs.ai_skills, diaogui_skill)
diaogui_skill.getTurnUseCard = function(self)
  if self.player:hasUsed("HDiaoguiCard") then return end
  ...
  if equipcard then
    return sgs.Card_Parse("@HDiaoguiCard=" .. equipcard:getEffectiveId())
  end
end

sgs.ai_skill_use_func.HDiaoguiCard = function(card, use, self)
--[[
...
sgs.ai_use_priority.HDiaoguiCard = sgs.ai_use_priority.HLureTiger - 0.05--先用普通的掉虎

sgs.ai_skill_invoke.heg_fengyang = true

--严白虎
sgs.ai_skill_invoke.heg_zhidao = function(self, data)
	if not self:willShowForDefence() or not self:willShowForAttack() then
    return false
  end
  if (self:getCardsNum("AmazingGrace") + self:getCardsNum("GodSalvation") +  self:getCardsNum("HAwaitExhausted") +
...
sgs.ai_skill_playerchosen.heg_zhidao = function(self, targets)
  if self:getCardsNum("AmazingGrace") > 0 or self:getCardsNum("GodSalvation") > 0  or self:getCardsNum("HAwaitExhausted") > 0 then
...
sgs.ai_skill_invoke.heg_jilix = function(self, data)
...
--钟会
sgs.ai_skill_invoke.heg_quanji = function(self, data)
...
sgs.ai_skill_exchange._quanji = function(self,pattern,max_num,min_num,expand_pile)
  ...
  for _, c in ipairs(cards) do--藏君主装备
    if c:isKindOf("HLuminousPearl") then
      local lord = self.room:getLord("wu")
      if lord and self:isEnemy(lord) then
        return c:getEffectiveId()
      end
    elseif c:isKindOf("HDragonPhoenix") then
      local lord = self.room:getLord("shu")
      if lord and self:isEnemy(lord) then
        return c:getEffectiveId()
      end
    elseif c:isKindOf("HPeaceSpell") then
      local lord = self.room:getLord("qun")
      if lord and self:isEnemy(lord) then
        return c:getEffectiveId()
      end
    elseif c:isKindOf("HSixDragons") then
...
local paiyi_skill = {}
paiyi_skill.name = "heg_paiyi"
table.insert(sgs.ai_skills, paiyi_skill)
paiyi_skill.getTurnUseCard = function(self)
  if self.player:getPile("power_pile"):length() > 0 and not self.player:hasUsed("HPaiyiCard") then
		return sgs.Card_Parse("@HPaiyiCard=" .. self.player:getPile("power_pile"):first())
	end
...
sgs.ai_skill_use_func.HPaiyiCard = function(card, use, self)
  sgs.ai_use_priority.HPaiyiCard = 2.4
	local target
	  ...
	  if not target then
		  for _, enemy in ipairs(self.enemies) do
			  if enemy:getHp() == 1 and self:isWeak(enemy)
				and not sgs.originalHegemonyHasShownSkills(enemy, sgs.masochism_skill)
  ...
  if self.player:getPile("power_pile"):length() > 7 then
    sgs.ai_use_priority.HPaiyiCard = 10
  end
	if target then
		use.card = card
		if use.to then
...
sgs.ai_card_intention.HPaiyiCard = function(self, card, from, tos)
	local to = tos[1]
...
function sgs.ai_cardneed.heg_paiyi(to, card, self)
  return card:isKindOf("Crossbow")
end

--司马昭
sgs.ai_skill_invoke.heg_suzhi = function(self, data)
	return self:willShowForAttack()
end

function sgs.ai_cardneed.heg_suzhi(to, card, self)
	return card:isKindOf("Axe") or isCard("Duel",card, to)
end

sgs.ai_skill_invoke.heg_fankui_simazhao = function(self, data)
...
sgs.ai_skill_invoke.heg_zhaoxin = function(self, data)
...
--孙綝
sgs.ai_skill_invoke.heg_shilu = true

sgs.ai_skill_cardask["@heg_shilu"] = function(self, data, pattern, target, target2, arg, arg2)
  ...
  local name = sgs.ai_skill_choice.xiongnve_attack(self, table.concat(self:getGeneralPile(self.player, "massacre"), "+"))
  ...
  local has_Crossbow = self:getCardsNum("Crossbow") > 0
	if xiongnve_nolimit or (#self.enemies > 0 and (has_Crossbow or self:hasCrossbowEffect()) and self.player:getCardCount(true) >= 4) then
		...
		for _, acard in ipairs(allcards) do
			...
			and not isCard("HBefriendAttacking", acard, self.player) and not isCard("HAllianceFeast", acard, self.player)
  ...
  if self.player:getHp() < 3 then--复制的制衡，未详细修过
		...
		for _, acard in ipairs(allcards) do
			...
			and not isCard("HBefriendAttacking", acard, self.player) and not isCard("HAllianceFeast", acard, self.player) then
...
sgs.ai_skill_invoke.heg_xiongnve = function(self, data)
  ...
  if data:toString() == "defence"  then
    ...
    local useless_num = 0
    local generals = self:getGeneralPile(self.player, "massacre")
...
sgs.ai_skill_choice.xiongnve_attack = function(self, generals)
  ...
  local burningcamps = self:getCard("HBurningCamps")
...
sgs.ai_skill_choice.heg_xiongnve = function(self, choices, data)
  choices = choices:split("+")
...
function sgs.ai_cardneed.heg_xiongnve(to, card, self)
	return card:isKindOf("HHalberd")
end
...
local huaiyi_skill = {
  ...
  getTurnUseCard = function(self, inclusive)
      if self.player:hasUsed("HHuaiyiCard") or self.player:isKongcheng() then
        return nil
      end
      ...
      if red and black then
        return sgs.Card_Parse("@HHuaiyiCard=.&heg_huaiyi")
      end
  end,
}
table.insert(sgs.ai_skills, huaiyi_skill)

sgs.ai_skill_use_func["HHuaiyiCard"] = function(card, use, self)
...
sgs.ai_use_priority.HHuaiyiCard = 4.2
...
sgs.ai_skill_playerchosen["huaiyi_snatch"] = function(self, targets, max_num, min_num)
  ...
  for _, target in ipairs(targetlist) do
		if self:isEnemy(target) and sgs.originalHegemonyHasShownSkills(target, sgs.cardneed_skill)
...
sgs.ai_skill_cardchosen.heg_huaiyi = function(self, who, flags, method, disable_list)
  local flag_str
  ...
  if flag_str == "he" then--藏君主装备
    for _, equip in sgs.qlist(who:getEquips()) do
      if equip:isKindOf("HLuminousPearl") then
        local lord = self.room:getLord("wu")
        if lord and self:isEnemy(lord) then
          return equip:getEffectiveId()
        end
      elseif equip:isKindOf("HDragonPhoenix") then
        local lord = self.room:getLord("shu")
        if lord and self:isEnemy(lord) then
          return equip:getEffectiveId()
        end
      elseif equip:isKindOf("HPeaceSpell") then
        local lord = self.room:getLord("qun")
        if lord and self:isEnemy(lord) then
          return equip:getEffectiveId()
        end
      elseif equip:isKindOf("HSixDragons") then
        local lord = self.room:getLord("wei")
        if lord and self:isEnemy(lord) then
          return equip:getEffectiveId()
        end
...
sgs.ai_skill_invoke.heg_zisui = true

--许攸
sgs.ai_skill_invoke.heg_shicai = function(self, data)
  local damage = data:toDamage()
	return damage.damage < 2
end

sgs.ai_skill_invoke.heg_chenglve = true
...
--夏侯霸
sgs.ai_skill_invoke.heg_baolie = function(self, data)
	if not self:willShowForAttack() then
    return false
  end
  --非常粗糙的条件？
...
function sgs.ai_cardneed.heg_baolie(to, card, self)
  if to:getHp() <= 2 then
    return card:isKindOf("Slash") or card:isKindOf("Analeptic") or to:hasWeapon("Spear")
  end
	return card:isKindOf("HHalberd")--方天画戟
end

--潘濬
sgs.ai_skill_invoke.heg_congcha = true

sgs.ai_skill_playerchosen.heg_congcha = sgs.ai_skill_playerchosen.damage

sgs.ai_skill_invoke.heg_gongqing = function(self, data)
  local damage = data:toDamage()
...
jinfa_skill.getTurnUseCard = function(self)
  ...
  if equipcard then
    return sgs.Card_Parse("@HJinfaCard=" .. equipcard:getEffectiveId() .."&heg_jinfa")
  end

  local cards = self.player:getCards("he")
  cards = sgs.QList2Table(cards)
  self:sortByUseValue(cards, true)
  return sgs.Card_Parse("@HJinfaCard=" .. cards[1]:getEffectiveId() .."&heg_jinfa")
end

sgs.ai_skill_use_func.HJinfaCard = function(card, use, self)
  local target
  for _, friend in ipairs(self.friends_noself) do
...
sgs.ai_use_priority.HJinfaCard = 4.2--顺之后

sgs.ai_card_intention.HJinfaCard = function(self, card, from, tos)
	local to = tos[1]
	if to:objectName() == from:objectName() then return end
...
sgs.ai_skill_exchange["_heg_jinfa"] = function(self,pattern,max_num,min_num,expand_pile)
  if self:getCardsNum("EquipCard") == 0 then
    return {}
  end
  local heg_wenqin = sgs.findPlayerByShownSkillName("heg_jinfa")
  if self:isFriend(heg_wenqin) then
    return {}
  end
  local equipcard
  if self:needToThrowArmor() then
  ...
  if not equipcard then
    ...
    self:sortByKeepValue(cards)
    if not self:isFriend(heg_wenqin) then
      for _, c in ipairs(cards) do
...
sgs.ai_skill_choice.heg_lianpian = function(self, choices, data)
  local heg_sufei = sgs.findPlayerByShownSkillName("heg_lianpian")
  if self:isFriend(heg_sufei) then
    return "recover"
  else
    return "discard"
  end
...
--诸葛恪
function sgs.ai_cardsview.heg_aocai(self, class_name, player)
  if self.player:objectName() ~= player:objectName() or not player:hasSkill("heg_aocai") then return end
  ...
  or sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_RESPONSE then
    if class_name == "Slash" or class_name == "Jink" then
      ...
      if pattern and pattern == "slash" or pattern == "jink" then
        local card = "@HAocaiCard=.&heg_aocai:" .. pattern
	...
	if class_name == "Peach" or class_name == "Analeptic" then
        ...
        if dying and dying:objectName() == player:objectName() then
            return "@HAocaiCard=.&heg_aocai:peach+analeptic"
        else
            local user_string
            if class_name == "Analeptic" then user_string = "analeptic" else user_string = "peach" end
            return "@HAocaiCard=.&heg_aocai:" .. user_string
        end
	end
end

sgs.ai_use_priority.HAocaiCard = 20


local duwu_skill = {}
duwu_skill.name = "heg_duwu"
table.insert(sgs.ai_skills, duwu_skill)
duwu_skill.getTurnUseCard = function(self, inclusive)
	if self.player:getMark("@heg_duwu") < 1 then return end
  return sgs.Card_Parse("@HDuwuCard=.&heg_duwu")
end

sgs.ai_skill_use_func.HDuwuCard= function(card, use, self)
  ...
  and (self.player:getMark("Global_TurnCount") > 1 or string.find(sgs.originalHegemonyGameProcess(), ">>>")) then--防止开局就使用
    use.card = card
  end
...
sgs.ai_card_intention.HDuwuCard = 80
sgs.ai_use_priority.HDuwuCard= 3.6
...
function sgs.ai_cardneed.heg_duwu(to, card, self)
	return card:isKindOf("Weapon") and sgs.weapon_range[card:getClassName()] >=3
end

--黄祖
sgs.ai_skill_cardask["@heg_xishe-slash"] = function(self, data, pattern, target, target2)
...
sgs.ai_skill_choice["transform_xishe"] = function(self, choices)
	...
	local importantsklii = {"heg_congjian", "jijiu", "heg_qianhuan", "heg_yigui", "heg_shicai", "heg_jinghe"}--还有哪些？
	local skills = sgs.originalHegemonySlotSkills(self.player, false)
...
sgs.ai_cardneed.heg_xishe = sgs.ai_cardneed.equip

--刘巴
sgs.ai_skill_invoke.heg_tongdu = true

sgs.ai_skill_choice.heg_tongdu = "yes"

local qingyin_skill = {}
qingyin_skill.name = "heg_qingyin"
table.insert(sgs.ai_skills, qingyin_skill)
qingyin_skill.getTurnUseCard = function(self)
	if self.player:getMark("@heg_qingyin") < 1 then return end
  ...
  if count > 1 or (self.player:getHp() == 1 and self:isWeak() and self:getAllPeachNum() < 1) then
  	return sgs.Card_Parse("@HQingyinCard=.&heg_qingyin")
  end
end

sgs.ai_skill_use_func.HQingyinCard = function(card, use, self)
  --global_room:writeToConsole("使用刘巴技能")
	use.card = card
end

sgs.ai_card_intention.HQingyinCard = -80
sgs.ai_use_priority.HQingyinCard = 1--桃之前

--朱灵
sgs.ai_skill_invoke.heg_juejue = function(self, data)--暂不考虑紫砂
	if not self:willShowForAttack() then
    return false
  end
  local friend_weak = 0
  ...
  if self:getOverflow() > (self.player:getMark("@halfmaxhp") < 1 and 1 or 3) and self.player:getHp() > 1 then
    ...
    if (self.player:getHp() > 2 or self.player:hasSkill("heg_wangxi")) and self:getCardsNum("Peach") > 0 and (#self.enemies > 2 or #self.enemies > #self.friends) then
      return true
    end
  end
	return false
...
sgs.ai_skill_cardask["@heg_juejue-discard"] = function(self, data, pattern, target, target2)
...
sgs.ai_skill_invoke.heg_fangyuan = true

sgs.ai_skill_playerchosen["_heg_fangyuan"] = function(self, targets)
...
--诏书
sgs.ai_use_priority.HImperialEdict = 9
sgs.ai_keep_value.HImperialEdict = 4

local imperialedicttrick_skill = {}
imperialedicttrick_skill.name = "heg_imperialedicttrick"
table.insert(sgs.ai_skills, imperialedicttrick_skill)
imperialedicttrick_skill.getTurnUseCard = function(self)
	if self.player:hasUsed("HImperialEdictTrickCard") then
		return
	end
  local suits = {}
  for _, id in sgs.qlist(self.player:getPile("HImperialEdict")) do
    local card = sgs.Sanguosha:getCard(id)
    if not card:isKindOf("HImperialEdict") then
  ...
  if #suits == 4 then
    return sgs.Card_Parse("@HImperialEdictTrickCard=.&")
  end
end

sgs.ai_skill_use_func.HImperialEdictTrickCard = function(card, use, self)
  use.card = card
end

sgs.ai_use_priority.HImperialEdictTrickCard = 10

local imperialedictattach_skill = {}
imperialedictattach_skill.name = "heg_imperialedictattach"
table.insert(sgs.ai_skills, imperialedictattach_skill)
imperialedictattach_skill.getTurnUseCard = function(self)
	if not self.player:hasUsed("HImperialEdictAttachCard") then
		return sgs.Card_Parse("@HImperialEdictAttachCard=.&")
	end
end

sgs.ai_skill_use_func.HImperialEdictAttachCard = function(card, use, self)
  sgs.ai_use_priority.HImperialEdictAttachCard = 0.2
  local attach_cards = {}
  local attach_num = 1
  if not self:originalHegemonyIsBigKingdomPlayer(self.player) then
    for _, p in sgs.qlist(self.room:getAlivePlayers()) do
      if self:originalHegemonyIsBigKingdomPlayer(p) then
        attach_num = 2
        break
      end
    end
  ...
  for _, p in sgs.qlist(self.room:getAlivePlayers()) do
    if not p:getPile("HImperialEdict"):isEmpty() and self.player:isFriendWith(p) then
      for _,id in sgs.qlist(p:getPile("HImperialEdict")) do
        local c = sgs.Sanguosha:getCard(id)
        if not c:isKindOf("HImperialEdict") then
...
--[[self:sortByKeepValue(hcards)
  ...
  if #attach_cards > 0 then
    if #suits == #attach_cards and not self.player:getPile("HImperialEdict"):isEmpty() then--自己回合可以获取锦囊
      sgs.ai_use_priority.HImperialEdictAttachCard = 5
    end
    use.card = sgs.Card_Parse("@HImperialEdictAttachCard=" .. table.concat(attach_cards, "+"))
  end
end
...
function SmartAI:useCardRuleTheWorld(card, use)
  ...
  self:sort(self.enemies, "hp")
  for _, enemy in ipairs(self.enemies) do
    if enemy:getHp() > min_hp and self:trickIsEffective(card, enemy) and sgs.originalHegemonyHasShownSkills(enemy, sgs.priority_skill)
...
sgs.ai_use_priority.HRuleTheWorld = 4.5
sgs.ai_keep_value.HRuleTheWorld = 3.5
sgs.ai_use_value.HRuleTheWorld = 9
sgs.ai_card_intention.HChaos = 150

sgs.ai_skill_choice["Hrule_the_world"] = function(self, choices, data)
...
sgs.ai_skill_discard["Hrule_the_world"] = function(self, discard_num, min_num, optional, include_equip)
  return self:askForDiscard("dummy_reason", 1, 1, false, false)
end

sgs.ai_nullification.HRuleTheWorld = function(self, card, from, to, positive, keep)
...
sgs.ai_use_priority.HConquering = 9.25
sgs.ai_keep_value.HConquering = 3.88
sgs.ai_use_value.HConquering = 9
sgs.ai_card_intention.HConquering = -80
...
sgs.ai_nullification.HConquering = function(self, card, from, to, positive, keep)
...
sgs.ai_use_priority.HConsolidateCountry = 9.25
sgs.ai_keep_value.HConsolidateCountry = 4.3
sgs.ai_use_value.HConsolidateCountry = 10

sgs.ai_skill_exchange["Hconsolidate_country"] = function(self,pattern,max_num,min_num,expand_pile)
  local discards = {}
  local cards = self.player:getCards("he")
	cards = sgs.QList2Table(cards)
	self:sortByUseValue(cards,true)
...
sgs.ai_skill_use["@@heg_consolidatecountrygive"] = function(self, prompt, method)
  local targets = {}
  for _, p in sgs.qlist(self.room:getOtherPlayers(self.player)) do
  ...
  for _, p in ipairs(targets) do
    ...
    if max_num > 0 then
      ...
      if #card_ids > 0 then
        return "@HConsolidateCountryGiveCard=" .. table.concat(card_ids, "+") .. "->" .. p:objectName()
      end
    end
  end
  return "."
...
sgs.ai_nullification.HConsolidateCountry = function(self, card, from, to, positive, keep)
  if card:hasFlag("CompleteEffect") then
	...
	if keep then return false end
	if positive then
		if self:isEnemy(from) and (self:isWeak(from) or sgs.originalHegemonyHasShownSkills(from, sgs.cardneed_skill)) then
			return true, true
		end
	else
...
sgs.ai_use_priority.HChaos = 7
sgs.ai_keep_value.HChaos = 3.47
sgs.ai_use_value.HChaos = 9
sgs.ai_card_intention.HChaos = 80
...
sgs.ai_nullification.HChaos = function(self, card, from, to, positive, keep)
  local targets = sgs.SPlayerList()
	local players = self.room:getTag("targets" .. card:toString()):toList()
	for _, q in sgs.qlist(players) do
		targets:append(q:toPlayer())
...
-- Both views are private, server-filtered AG lists; never read the draw pile.
local function hegemonyChooseDrawCard(self, ids)
    local cards = {}
    for _, id in ipairs(ids) do cards[#cards + 1] = sgs.Sanguosha:getCard(id) end
    self:sortByUseValue(cards, true)
    return cards[#cards] and cards[#cards]:getEffectiveId() or -1
end
sgs.ai_skill_askforag.heg_xunxun_tangzi = hegemonyChooseDrawCard
sgs.ai_skill_askforag.heg_aocai = function(self, ids)
    if type(sgs.ai_skill_askforag.aocai) == "function" then
        return sgs.ai_skill_askforag.aocai(self, ids)
    end
    return hegemonyChooseDrawCard(self, ids)
end
