-- Source: TODO/QSanguosha-For-Hegemony-xxyheaven/lua/ai/strategic_advantage-ai.lua (cf61c15).
--[[********************************************************************
...
sgs.ai_skill_use_func.TransferCard = function(transferCard, use, self)
	...
	for _, c in sgs.qlist(self.player:getCards("h")) do
		if c:isTransferable() and (not isCard("Peach", c, self.player) or #friends_shown > 0) then
			...
			elseif self:hasCrossbowEffect() and c:isKindOf("Slash") then
				continue
			elseif self:originalHegemonyIsBigKingdomPlayer(self.player) and c:isKindOf("HThreatenEmperor") then
				continue
			elseif self.player:getMark("GlobalBattleRoyalMode") > 0
				and (isCard("Analeptic", c, self.player) or isCard("HBurningCamps", c, self.player) or isCard("HBreastplate", c, self.player)) then
				continue
			elseif c:getNumber() > 10 and self.player:hasSkills("tianyi|quhu|shuangren|heg_lieren") then
				continue
			end
	...
	for _, card in ipairs(cards) do
		if card:isKindOf("HThreatenEmperor") then
			has_TE = true
		end
		if card:isKindOf("HBurningCamps") then
			has_BC = true
		end
	end
	...
	if has_TE or has_BC then
			...
			if self:originalHegemonyIsBigKingdomPlayer(self.player) then
				for _, card in ipairs(cards) do
					if card:isKindOf("HThreatenEmperor") then--大势力留下挟天子
		...
		elseif has_BC then
			...
			if string.find(gameProcess, self:originalHegemonyOwnKingdom() .. ">>>") then--必须要>个数多的在前，因为只会find第一个>
				for _, card in ipairs(cards) do
					if card:isKindOf("HBurningCamps") then--大国？火烧联营不能给
						table.removeOne(cards, card)
						global_room:writeToConsole(">>>大优势，火烧联营不能给")
					end
				end
			elseif string.find(gameProcess, self:originalHegemonyOwnKingdom() .. ">>")  then
				for _, p in sgs.qlist(self.room:getOtherPlayers(self.player)) do
...
sgs.ai_use_priority.TransferCard = 3
...
function SmartAI:useCardDrowning(card, use)
	...
	for _, friend in ipairs(self.friends_noself) do
		...
		and (self:needToThrowArmor(friend) or (sgs.originalHegemonyHasShownSkills(friend, sgs.lose_equip_skill) and not friend:getArmor() and not friend:getTreasure())) then
...
sgs.ai_card_intention.HDrowning = function(self, card, from, tos)
...
sgs.ai_skill_choice.drowning = function(self, choices, data)
	...
	if self.player:hasSkill("heg_tianxiang") and getKnownCard(self.player, self.player, "heart", true, "h") > 0 then
		return "damage"
	end
	...
	for _, equip in sgs.qlist(self.player:getEquips()) do--值是否合适？
		...
		elseif equip:isKindOf("Armor") then
			...
			elseif equip:isKindOf("HBreastplate") and self.player:getHp() <= 1 then value = value + 99
			elseif equip:isKindOf("HPeaceSpell") then value = value + 99
			end
		elseif equip:isKindOf("OffensiveHorse") then value = value + 2
		elseif equip:isKindOf("DefensiveHorse") then value = value + 3
		elseif equip:isKindOf("HSixDragons") then value = value + 4.5
	...
	if self.player:getHp() == 1 and not self.player:hasArmorEffect("Breastplate") then
		if value > 12 and (self:getAllPeachNum() > 0
			or (self.player:hasSkill("heg_buqu") and self.player:getPile("scars"):length() <= 4)
			or (self.player:hasSkill("heg_jizhao") and self.player:getMark("@heg_jizhao") > 0)) then
				return "damage"
		else
			return "throw"
		end
...
sgs.ai_nullification.HDrowning = function(self, card, from, to, positive, keep)
	if positive then
...
sgs.ai_use_value.HDrowning = 5.3

sgs.ai_use_priority.HDrowning = sgs.ai_use_priority.Dismantlement + 0.05

sgs.ai_keep_value.HDrowning = 3.4

function sgs.ai_armor_value.HIronArmor(player, self)
	if self:isWeak(player) then
		for _, p in sgs.qlist(self.room:getOtherPlayers(player)) do
			if p:hasShownSkill("heg_huoji") and self:isEnemy(player, p) then
				return 3.5
			end
		end
	end
	...
	if not self:isFriend(player,lp) and (sgs.originalHegemonyHasShownSkills(lp, "heg_qice|heg_yigui") or getKnownCard(lp, player, "HBurningCamps") > 0) then--上家有奇策、役鬼,火烧
		return 4.5
	end
	for _, enemy in ipairs(self:getEnemies(player)) do
		if not self:isFriend(player,lp) and getKnownCard(enemy, player, "HBurningCamps") > 0 then--敌方可能合纵连横火烧
			return 3.5
		end
	end
	return 2.5
...
sgs.ai_use_priority.HIronArmor = 0.82
...
function SmartAI:useCardBurningCamps(card, use)
	...
	if shouldUse then
		local chaincard = self:getCard("HFightTogether")
...
sgs.ai_nullification.HBurningCamps = function(self, card, from, to, positive, keep)
...
sgs.ai_use_value.HBurningCamps = 7.1

sgs.ai_use_priority.HBurningCamps = 4.7

sgs.ai_keep_value.HBurningCamps = 3.38

sgs.ai_card_intention.HBurningCamps = 120

sgs.ai_skill_invoke.heg_Breastplate = true

function sgs.ai_armor_value.HBreastplate(player, self)
...
sgs.ai_use_priority.HBreastplate = 0.9

function SmartAI:useCardLureTiger(HLureTiger, use)
	sgs.ai_use_priority.HLureTiger = 4.9
	if not HLureTiger:isAvailable(self.player) then return end

	local players = sgs.PlayerList()

	local card = self:getCard("HBurningCamps")
	if card and card:isAvailable(self.player) then
		local nextp = self.room:nextPlayer(self.player)
		local first
		while true do
			if HLureTiger:targetFilter(players, nextp, self.player) and self:trickIsEffective(HLureTiger, nextp, self.player) then
				if not first then
	...
	if card and card:isAvailable(self.player) and self:getAoeValue(card) > 0 and #self.friends_noself > 0 then
		...
		for _, friend in ipairs(self.friends_noself) do
			if self:isFriendWith(friend) and HLureTiger:targetFilter(players, friend, self.player)
			and self:trickIsEffective(HLureTiger, friend, self.player) and self:aoeIsEffective(card, friend, self.player) then
				players:append(friend)
			end
		end
		for _, friend in ipairs(self.friends_noself) do
			if HLureTiger:targetFilter(players, friend, self.player) and not players:contains(friend)
			and self:trickIsEffective(HLureTiger, friend, self.player) and self:aoeIsEffective(card, friend, self.player) then
				players:append(friend)
			end
		end
		if players:length() > 0 then
			sgs.ai_use_priority.HLureTiger = sgs.ai_use_priority.ArcheryAttack + 0.2
			use.card = HLureTiger
			if use.to then use.to = sgs.PlayerList2SPlayerList(players) end
			return
		end
	...
	if card and card:isAvailable(self.player) and self:getAoeValue(card) > 0 and #self.friends_noself > 0 then
		...
		if players:length() > 0 then
			sgs.ai_use_priority.HLureTiger = sgs.ai_use_priority.SavageAssault + 0.2
			use.card = HLureTiger
			if use.to then use.to = sgs.PlayerList2SPlayerList(players) end
			return
		end
	...
	if card and self:slashIsAvailable(self.player, card) then
		...
		if dummyuse.card then
			local total_num = 2 + sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_ExtraTarget, self.player, HLureTiger)
			local function getPlayersFromTo(one)
				local targets1 = sgs.PlayerList()
				local targets2 = sgs.PlayerList()
				local nextp = self.room:nextPlayer(self.player)
				while true do
					if HLureTiger:targetFilter(targets1, nextp, self.player) and self:trickIsEffective(HLureTiger, nextp, self.player) then
				...
				while true do
					if HLureTiger:targetFilter(targets2, nextp, self.player) and self:trickIsEffective(HLureTiger, nextp, self.player) then
			...
			for _, to in sgs.qlist(dummyuse.to) do
				if self.player:distanceTo(to) > self.player:getAttackRange() and self.player:distanceTo(to, -total_num) <= self.player:getAttackRange() then
					...
					if sps then
						sgs.ai_use_priority.HLureTiger = 3
						use.card = HLureTiger
	...
	if card and card:isAvailable(self.player) and #self.enemies > 0 then
		...
		for _, enemy in ipairs(self.enemies) do
			if HLureTiger:targetFilter(players, enemy, self.player) and self:trickIsEffective(HLureTiger, enemy, self.player) then
				players:append(enemy)
			end
		end
		if players:length() > 0 then
			sgs.ai_use_priority.HLureTiger = sgs.ai_use_priority.GodSalvation + 0.1
			use.card = HLureTiger
			if use.to then use.to = sgs.PlayerList2SPlayerList(players) end
			return
		end
	...
	if (can_slash or can_duel or can_fireattack) and #self.enemies > 1 then--调离敌方队友防救人
		...
		if to then
			...
			if #enemys_copy > 0 then
				for _, enemy in ipairs(enemys_copy) do
					if HLureTiger:targetFilter(players, enemy, self.player) and self:trickIsEffective(HLureTiger, enemy, self.player)
			...
			if players:length() > 0 then
				if can_slash or can_duel then
					sgs.ai_use_priority.HLureTiger = sgs.ai_use_priority.Duel + 0.1
				end
				if can_fireattack then
					sgs.ai_use_priority.HLureTiger = sgs.ai_use_priority.FireAttack + 0.1
				end
				use.card = HLureTiger
				if use.to then use.to = sgs.PlayerList2SPlayerList(players) end
				return
	...
	if card and card:isAvailable(self.player) and #self.enemies > 0 then
		...
		if players:length() > 0 then
			sgs.ai_use_priority.HLureTiger = sgs.ai_use_priority.AmazingGrace + 0.1
			use.card = HLureTiger
			if use.to then use.to = sgs.PlayerList2SPlayerList(players) end
			return
		end
	...
	if self.player:hasShownSkill("heg_jizhi") or aoedraw then
		for _, player in sgs.qlist(self.room:getOtherPlayers(self.player)) do
			if HLureTiger:targetFilter(players, player, self.player) and self:trickIsEffective(HLureTiger, player, self.player) then
				players:append(player)
			end
		end
		if (self.player:hasShownSkill("heg_jizhi") and players:length() > 0) or (aoedraw and players:length() > 1) then
			sgs.ai_use_priority.HLureTiger = 0.4
			use.card = HLureTiger
			if use.to then use.to = sgs.PlayerList2SPlayerList(players) end
			return
		end
...
sgs.ai_nullification.HLureTiger = function(self, card, from, to, positive)
--[[
	if positive then
...
sgs.ai_use_value.HLureTiger = 4.8

sgs.ai_use_priority.HLureTiger = 4.9

sgs.ai_keep_value.HLureTiger = 2.5
			...
			for _, p in ipairs(bigs) do
				...
				elseif self:isEnemy(p) then
					if p:isChained() then
						...
						or (IamSmall and string.find(gameProcess, self:originalHegemonyOwnKingdom() .. ">>"))
						or self.player:hasSkill("heg_fenming") then
							v_big = v_big + 2
						else
							v_big = v_big + 1
						end
			...
			for _, p in ipairs(smalls) do
				...
				elseif self:isEnemy(p) then
					if p:isChained() then
						...
						if (p:hasShownOneGeneral() and string.find(gameProcess, p:getKingdom() .. ">>"))
						or (IamBig and string.find(gameProcess, self:originalHegemonyOwnKingdom() .. ">>"))
						or self.player:hasSkill("heg_fenming") then
							v_small = v_small + 2
						else
							v_small = v_small + 1
...
sgs.ai_nullification.HFightTogether = function(self, card, from, to, positive, keep)
	local targets = sgs.SPlayerList()
	local players = self.room:getTag("targets" .. card:toString()):toList()
	for _, q in sgs.qlist(players) do
	...
	if positive then
		if to:isChained() and not keep then
			if self:isEnemy(to) and sgs.originalHegemonyHasShownSkills(to, sgs.cardneed_skill) then
				for _, p in sgs.qlist(targets) do
...
sgs.ai_use_value.HFightTogether = 5.5

sgs.ai_use_priority.HFightTogether = 8.7

sgs.ai_keep_value.HFightTogether = 3.33
	...
	for _, kingdom in ipairs(effect_kingdoms) do
		...
		if kingdom:startsWith("sgs") then
			if self.player:isWounded() then
				...
				value = value + 0.5
				if sgs.originalHegemonyHasShownSkills(self.player, sgs.cardneed_skill) then value = value + 0.5 end
			end
			local target = self.room:findPlayerByObjectName(kingdom)
			if self:isFriend(target) then
				value = value + 0.5
				if sgs.originalHegemonyHasShownSkills(target, sgs.cardneed_skill) then value = value + 0.5 end
			...
			for _, p in sgs.qlist(self.room:getOtherPlayers(self.player)) do
				if p:hasShownOneGeneral() and p:getRole() ~= "careerist" and p:getKingdom() == kingdom then
					...
					if self:isFriend(p) and self:trickIsEffective(card, p, self.player) then
						...
						if sgs.originalHegemonyHasShownSkills(p, sgs.cardneed_skill) then self_value = self_value + 0.5 end
					...
					elseif self:isEnemy(p) and self:trickIsEffective(card, p, self.player) then
						...
						if sgs.originalHegemonyHasShownSkills(p, sgs.cardneed_skill) then enemy_value = enemy_value + 0.5 end
			...
			if their_num > self.player:getLostHp() then
				...
				self_value = self_value + (their_num - self.player:getLostHp())*(sgs.originalHegemonyHasShownSkills(self.player, sgs.cardneed_skill) and 1 or 0.5)
			else
...
sgs.ai_use_value.HAllianceFeast = 9.5

sgs.ai_use_priority.HAllianceFeast = 8.8

sgs.ai_keep_value.HAllianceFeast = 4.4

sgs.ai_nullification.HAllianceFeast = function(self, card, from, to, positive, keep)
	...
	if to:objectName() == from:objectName() then
		if targets:length() -1 > from:getLostHp() then
			...
			from_value = from_value + (targets:length() -1 - from:getLostHp())*(sgs.originalHegemonyHasShownSkills(from, sgs.cardneed_skill) and 1 or 0.5)
		else
	...
	if hegnull then
		for _, p in ipairs(targets_t) do
			if self:trickIsEffective(card, p, from) then
				value = value + 0.5
				if sgs.originalHegemonyHasShownSkills(p, sgs.cardneed_skill) then value = value + 0.5 end
...
function SmartAI:useCardThreatenEmperor(card, use)
	...
	if self.player:hasSkill("heg_shensu") and sgs.ai_skill_use["@@shensu3"](self) ~= "." then return end
	if self.player:hasSkills("qiaobian|heg_qiaobian_egf") and (self.player:getHandcardNum() - self.player:getMaxCards() > 1) then return end
	use.card = card
end

sgs.ai_use_value.HThreatenEmperor = 6

sgs.ai_use_priority.HThreatenEmperor = 0

sgs.ai_keep_value.HThreatenEmperor = 3.2

sgs.ai_nullification.HThreatenEmperor = function(self, card, from, to, positive, keep)
	if positive then
...
sgs.ai_skill_cardask["@heg_threaten_emperor"] = function(self)
	...
	if self.player:getHandcardNum() > 1 then
		for _, card in ipairs(cards) do
			if not card:isKindOf("heg_threaten_emperor") then--如果可以连着挟天子
...
sgs.ai_use_value.HImperialOrder = 0

sgs.ai_use_priority.HImperialOrder = 9.2

sgs.ai_keep_value.HImperialOrder = 0

sgs.ai_nullification.HImperialOrder = function(self, card, from, to, positive)
	return false
end
...
sgs.ai_skill_cardask["@imperial_order-equip"] = function(self)
	...
	if sgs.GetConfig("EnableLordConvertion", true) and self.player:getMark("Global_RoundCount") <= 1 then--君主
		if self.player:inHeadSkills("rende") or self.player:inHeadSkills("guidao")
			or self.player:inHeadSkills("heg_zhiheng") or self.player:inHeadSkills("nosjianxiong") then
				return "."
		end
	end
	local discard
...
sgs.ai_skill_choice.imperial_order = function(self, choices, data)
	...
	if sgs.originalHegemonyGeneral(self.player, true):getKingdom() == "careerist" and table.contains(choices,"show_deputy") then--野心家角色
		--global_room:writeToConsole("敕令野心家")
		return "show_deputy"
	end

	if self:originalHegemonyOwnKingdom() == "shu" and table.contains(choices,"show_head") and table.contains(choices,"show_deputy") then
...
sgs.ai_skill_use["@@heg_JadeSeal!"] = function(self, prompt, method)
	...
	if dummyuse.card and not dummyuse.to:isEmpty() then
		...
		return "known_both:heg_JadeSeal[no_suit:0]=.&->" .. table.concat(tos, "+")
	end
...
sgs.ai_use_priority.HJadeSeal = 5.6

sgs.ai_keep_value.HJadeSeal = 4.2

sgs.ai_skill_use["@@HHalberd"] = function(self, prompt)
	--先选中所有能选择的角色
	local targets = {}
	local list = sgs.PlayerList()
	local halberdCard = sgs.Card_Parse("@HHalberdCard=.")
	...
	if #targets > 0 then
		return "@HHalberdCard=.&->" .. table.concat(targets, "+")
	end
end

function sgs.ai_slash_weaponfilter.HHalberd(self, to, player)
	return player:distanceTo(to) <= math.max(sgs.weapon_range.HHalberd, player:getAttackRange())
		and (sgs.card_lack[to:objectName()]["Jink"] == 1 or getCardsNum("Jink", to, self.player) < 1)
end

function sgs.ai_weapon_value.HHalberd(self, enemy, player)
	if sgs.originalHegemonyHasShownSkills(player, sgs.force_slash_skill .. "|" .."heg_paoxiao|heg_paoxiao_xh|heg_baolie|heg_xiongnve|heg_kuangcai") then return 4 end
	return 1.1
end
...
wooden_ox_skill.getTurnUseCard = function(self)
	...
	if self.player:hasUsed("HWoodenOxCard") or self.player:isKongcheng() or not self.player:hasTreasure("WoodenOx") then return end
	...
	and (self:getOverflow() > 0 or self:isWeak(friend) or (self.player:hasSkills(sgs.lose_equip_skill) and self:isFriendWith(friend))) then
		...
		return sgs.Card_Parse("@HWoodenOxCard=" .. card:getEffectiveId())
	end
	if self:getOverflow() > 0 or (self:needKongcheng() and #cards == 1) then
		self.wooden_ox_assist = nil
		return sgs.Card_Parse("@HWoodenOxCard=" .. cards[1]:getEffectiveId())
	end
end

sgs.ai_skill_use_func.HWoodenOxCard = function(card, use, self)
	sgs.ai_use_priority.HWoodenOxCard = 0
	if self.player:hasSkills(sgs.lose_equip_skill) then
		sgs.ai_use_priority.HWoodenOxCard = 10
	end
	use.card = card
end
...
function sgs.ai_slash_weaponfilter.HBlade(self, to, player)
	return player:distanceTo(to) <= math.max(sgs.weapon_range.HBlade, player:getAttackRange()) and not to:hasShownAllGenerals()
		and (sgs.card_lack[to:objectName()]["Jink"] == 1 or getCardsNum("Jink", to, self.player) < 1)
end

function sgs.ai_weapon_value.HBlade(self, enemy, player)
	if not enemy then
		return 0
	else
		local v = 0
		if not enemy:hasShownGeneral() then v = v + 1 end
