-- Source: TODO/QSanguosha-For-Hegemony-xxyheaven/lua/ai/guanxing-ai.lua (cf61c15).
--[[********************************************************************
...
local function GuanXing(self, cards)
	...
	while (#bottom > drawCards) do
		...
		for index, for_judge in ipairs(bottom) do
			...
			if self:isFriend(next_player) then
				if next_player:hasShownSkill("heg_luoshen") then
				...
				if next_player:hasShownSkill("heg_luoshen") and for_judge:isRed() and not luoshen_flag then
					table.insert(next_judge, for_judge)
					table.remove(bottom, index)
					nextplayer_has_judged = true
					judged_list[pos] = 1
	...
	if drawCards > 0 and #bottom > 0 then
		...
		for index = 1, #bottom do
			...
			if not insert and not willSkipPlayPhase and self.player:hasSkills("heg_jizhi") and isCard("TrickCard", gcard, self.player) then
				insert = true
			end
			if not insert and not willSkipPlayPhase and self.player:hasSkill("heg_yizhi") and isCard("Lightning", gcard, self.player) then
				insert = true
			end
			if not insert then
				...
				elseif not willSkipPlayPhase and isCard("ExNihilo", gcard, self.player) then
					insert = true
				elseif not willSkipPlayPhase and isCard("HBefriendAttacking", gcard, self.player) then
					insert = true
			...
			if insert then
				...
				elseif isCard("HBefriendAttacking", gcard, self.player) then
	...
	if #bottom > drawCards and #bottom > 0 and not nextplayer_judge_failed then
		...
		if self:isFriend(next_player) then
			...
			local i = 0
			for index = 1, #bottom do
				if bottom[index - i]:isKindOf("Lightning") and not next_player:hasShownSkill("nosleiji") or bottom[index - i]:isKindOf("GlobalEffect") then
					table.insert(next_judge, table.remove(bottom, index - i))
					i = i + 1
					if maxCount == i then break end
				end
	...
	if not self_has_judged and not nextplayer_has_judged and #next_judge == 0 and #up >= drawCards_copy and drawCards_copy > 1 then
		for i = 1, drawCards_copy - 1 do
			if isCard("ExNihilo", up[i], self.player) or isCard("HBefriendAttacking", up[i], self.player) then
...
function SmartAI:getValuableCardForGuanxing(cards, up_cards)
	...
	for _, card in ipairs(cards) do
		...
		elseif card:isKindOf("HIronArmor") then ironarmor = card
		...
		elseif card:isKindOf("HHalberd") then halberd = card
	...
	if eightdiagram then
		if not self.player:hasSkills("jgyizhong|bazhen") and self.player:hasSkills("tiandu|nosleiji|hongyan") then
			return eightdiagram
		end
	...
	if silverlion then
		...
		for _, aplayer in sgs.qlist(self.room:getOtherPlayers(self.player)) do
			if aplayer:hasShownSkill("nosleiji") and self:isEnemy(aplayer) then
				return silverlion
			end
			if aplayer:containsTrick("lightning") then
				lightning = true
			end
			if sgs.originalHegemonyHasShownSkills(aplayer, "guicai|guidao") and self:isEnemy(aplayer) then
				canRetrial = true
			end
		end
			...
			for _, slash in ipairs(slashes) do
				if self:slashIsEffective(slash, enemy) and self.player:canSlash(enemy, slash) and self:slashIsAvailable() then
					...
					if getCardsNum("Jink", enemy, self.player) < 1
						...
						or sgs.originalHegemonyHasShownSkills(self.player, sgs.force_slash_skill)
	...
	if weapon and self:getCardsNum("Slash") > 0 and self:slashIsAvailable() then
		...
		if crossbow then
			if self:getCardsNum("Slash") > 1 or self.player:hasSkills("heg_kurou|heg_keji") then
				return crossbow
			end
		...
		if gudingdao then
			...
			for _, enemy in ipairs(self.enemies) do
				if self.player:canSlash(enemy, slash, true, range_fix) and not enemy:hasShownSkill("heg_kongcheng")
