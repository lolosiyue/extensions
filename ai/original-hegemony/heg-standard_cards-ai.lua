-- Source: TODO/QSanguosha-For-Hegemony-xxyheaven/lua/ai/standard_cards-ai.lua (cf61c15).
--[[********************************************************************
...
function sgs.isGoodHp(player, observer)
	-- Resolve the caller's existing Room-owned mind, so rescue relationships and
	-- visible cards use the same viewer instead of the donor's shared recorder.
	local ai = current_self
	if observer then ai = sgs.ais[observer:objectName()] end
	assert(ai and ai.player and ai.room, "Original Hegemony isGoodHp requires an observer AI")
	observer = observer or ai.player
	...
	if goodHp then
		...
		local n = 0
		for _, friend in ipairs(ai:getFriends(player)) do
			local current = ai.room:getCurrent()
if current and current:hasShownSkill("wansha") and player:objectName() ~= friend:objectName() then continue end
...
function sgs.isGoodTarget(player, targets, self, isSlash)
	...
	local arr = { "heg_jieming", "heg_yiji", "mobilefangzhu" }--其他卖血技能？
	local m_skill = false
	...
	for _, masochism in ipairs(arr) do
		if player:hasShownSkill(masochism) then
			if masochism == "heg_jieming" and self and self:getJiemingDrawNum(player) < 2 then m_skill = false
			elseif masochism == "heg_yiji" and self and not self:findFriendsByType(sgs.Friend_Draw, player) then m_skill = false
			else
				m_skill = true
				break
			end
	...
	if isSlash and self and self.player:hasWeapon("Crossbow") and sgs.originalHegemonyHasShownSkills(player, sgs.throw_crossbow_skill) then
		return false
	end

	if m_skill and sgs.isGoodHp(player, self and self.player) and not self.player:hasSkills("heg_tieqi|heg_tieqi_xh|heg_yinbing") then
		return false
	else
		return true
	end
...
function sgs.getDefenseSlash(player, self)
	...
	local heg_jiangqin = sgs.findPlayerByShownSkillName("heg_niaoxiang")
	if heg_jiangqin then
		if heg_jiangqin:inSiegeRelation(heg_jiangqin, player) then
			niaoxiang_BA = true
		end
	end
	local need_double_jink = sgs.originalHegemonyHasShownSkills(attacker, "wushuang|heg_wushuang_lvlingqi") or niaoxiang_BA
	if need_double_jink and knownJink < 2 and unknownJink < 1.5 then
		defense = 0
	end

if attacker:hasShownSkill("tenyearjianchu") and (player:hasEquip() or player:getCardCount(true) == 1) then
		defense = 0
	end
	...
	if player:getMark("##heg_qianxi+no_suit_red") > 0 then
		if player:hasShownSkill("qingguo") then
			defense = defense - 0.5
		else
			defense = 0
		end
	elseif player:getMark("##heg_qianxi+no_suit_black") > 0 then
		if player:hasShownSkill("qingguo") then
			defense = defense - 1
		end
	end

	if player:getMark("##heg_boyan") > 0 or player:getMark("command4_effect") > 0 then
		defense = 0
	end

	if attacker:hasWeapon("DragonPhoenix") or attacker:hasSkills("heg_tieqi|heg_tieqi_xh") then
	...
	if hasEightDiagram then
		...
		if sgs.originalHegemonyHasShownSkills(player, "qingguo+tiandu") then defense = defense + 10
		elseif sgs.originalHegemonyHasShownSkills(player, "tiandu|heg_zhuwei") then defense = defense + 1 end
		if player:hasShownSkill("nosleiji") then defense = defense + 0.6 end
		if sgs.originalHegemonyHasShownSkills(player, "hongyan|guicai|heg_huanshi") then defense = defense + 0.5 end
	end
	if player:hasArmorEffect("RenwangShield") and not IgnoreArmor(attacker, player) and player:hasShownSkill("heg_jiang") then
		defense = defense + 1.6
	end

	if player:hasShownSkill("heg_tuntian") and player:hasShownSkill("heg_jixi") and unknownJink >= 1 and not attacker:hasSkills("heg_tieqi|heg_tieqi_xh") then
		defense = defense + 1.5
	end

	if not attacker:hasSkills("heg_tieqi|heg_tieqi_xh|heg_yinbing") then
		...
		if player:hasShownSkill("heg_fudi") and player:getHandcardNum() > 1 and attacker:getHp() + 1 >= player:getHp() then
			defense = defense + 1
		end
	end
	...
	if player:hasShownSkill("tenyearkuanggu") and player:getHp() > 1 then defense = defense + 0.2 end
	if player:hasShownSkill("heg_zaiqi") and player:getHp() > 1 then defense = defense + 0.35 end
	...
	if player:hasShownSkill("heg_tianxiang") then defense = defense + player:getHandcardNum() * 0.8 end
	...
	if player:isKongcheng() and player:getHandPile():isEmpty() and not sgs.originalHegemonyHasShownSkills(player, "heg_kongcheng") then
	...
	if player:getRole() == "careerist" and sgs.originalHegemonyGeneral(player, true):getKingdom() == "careerist" then
		defense = defense - 0.5
	end
...
function SmartAI:canLiuli(heg_daqiao, another)
if not heg_daqiao:hasShownSkill("liuli") then return false end
	if type(another) == "table" then
		if #another == 0 then return false end
		for _, target in ipairs(another) do
			if target:getHp() < 3 and self:canLiuli(heg_daqiao, target) then return true end
		end
		return false
	end
	...
	local n = heg_daqiao:getHandcardNum()
	if n > 0 and (heg_daqiao:distanceTo(another) <= heg_daqiao:getAttackRange()) then return true
	elseif heg_daqiao:getWeapon() and heg_daqiao:getOffensiveHorse() and (heg_daqiao:distanceTo(another) <= heg_daqiao:getAttackRange()) then return true
	elseif heg_daqiao:getWeapon() or heg_daqiao:getOffensiveHorse() then return heg_daqiao:distanceTo(another) <= 1
...
function SmartAI:slashIsEffective(slash, to, from, ignore_armor)
	...
	from = from or self.player
	if to:hasShownSkill("heg_kongcheng") and to:isKongcheng() then return false end
	...
	damage.damage = 1
	if not from:hasShownAllGenerals() and to:hasShownSkill("heg_mingshi") then
	...
	if not (ignore_armor or IgnoreArmor(from, to)) then
		if to:hasArmorEffect("RenwangShield") and slash:isBlack() then
			if (from:hasWeapon("DragonPhoenix") or from:hasWeapon("DoubleSword") and (from:isMale() and to:isFemale() or from:isFemale() or to:isMale()))
				and (to:getCardCount(true) == 1 or #self:getEnemies(from) == 1) then
elseif from:hasShownSkill("tenyearjianchu") or (from:hasShownSkill("kuangfu") and not from:hasFlag("kuangfuUsed")) then--需要配合单独判断弃防具
				return true
			else
				return false
			end
		...
		if to:hasArmorEffect("Vine") and not slash:isKindOf("NatureSlash") then
			if (from:hasWeapon("DragonPhoenix") or from:hasWeapon("DoubleSword") and (from:isMale() and to:isFemale() or from:isFemale() or to:isMale()))
				and (to:getCardCount(true) == 1 or #self:getEnemies(from) == 1) then
elseif from:hasShownSkill("tenyearjianchu") or (from:hasShownSkill("kuangfu") and not from:hasFlag("kuangfuUsed")) then
				return true
			else
...
function SmartAI:isPriorFriendOfSlash(friend, card, source)
	...
	local heg_huatuo = sgs.findPlayerByShownSkillName("jijiu")
	if source:hasSkill("heg_zhiman") then
	...
	if not self:hasHeavySlashDamage(source, card, friend)
		...
		or (friend:hasShownSkill("heg_jieming") and source:hasShownSkill("rende") and heg_huatuo and self:isFriend(heg_huatuo, source))) then
		return true
	end
...
function SmartAI:useCardSlash(card, use)
	if card:getClassName() == "HAocaiCard" then
		card = sgs.cloneCard("slash")
	end
	if card:getClassName() == "HMiewuCard" then
	...
	if self.player:hasSkill("heg_xiaoji") and sgs.Sanguosha:getCard(card:getEffectiveId()) and sgs.Sanguosha:getCard(card:getEffectiveId()):isKindOf("EquipCard")
	...
	local function canAppendTarget(target)
		if self.player:hasSkill("heg_paoxiao") then--咆哮屯杀
			local enough_pxslash = false
			if self:getCardsNum("Slash") > 0 then
			  local yongjue_slash = 0
			  if self.player:getMark("GlobalPlayCardUsedTimes") == 0 then
				for _, p in ipairs(self.friends) do
					if p:hasShownSkill("heg_yongjue") and self.player:isFriendWith(p) then--self.player:getSlashCount() == 0
					yongjue_slash = 1
					break
					end
				end
			...
			and not (self.player:hasSkill("heg_jili") and sgs.getHegemonyCardUsedTimes(self.player, ".") + sgs.getHegemonyCardRespondedTimes(self.player, ".") + 1 == self.player:getAttackRange()) then
	...
	for _, target in ipairs(targets) do
		...
		--[[旧克己
			and not (not self:isWeak(target) and #self.enemies > 1 and #self.friends > 1 and self.player:hasSkill("heg_keji")
			and self:getOverflow() > 0 and not self:hasCrossbowEffect()) ]]

			if target:getHp() > 1 and target:hasShownSkill("nosjianxiong") and self.player:hasWeapon("Spear") and card:getSkillName() == "Spear" then
			...
			if self.player:hasSkill("heg_duanbing") or (self.player:hasSkills("tenyearkuanggu|heg_kuanggu_xh") and self:hasCrossbowEffect()) then--需要额外筛选目标的技能，但是酒怎么处理？
	...
	if self.player:hasSkill("heg_duanbing") and #canSlashTargets > 0 then
	...
	if self.player:hasSkills("tenyearkuanggu|heg_kuanggu_xh") and self:hasCrossbowEffect() and #canSlashTargets > 0 then
		for _, target in ipairs(canSlashTargets) do
	...
	for _, slash in ipairs(slashes) do
		...
		for _, friend in ipairs(self.friends_noself) do
			if not self:hasHeavySlashDamage(self.player, slash, friend)
				...
				and (self:findLeijiTarget(friend, 50, self.player) or (friend:hasShownSkill("heg_jieming") and self.player:hasSkill("rende")))
...
sgs.ai_skill_cardask["slash-jink"] = function(self, data, pattern, target)
	...
	if self:isFriend(self.player, target) and target:hasSkill("heg_zhiman") and not self.player:hasSkill("nosleiji") then return "." end
	local slash
	...
	if self:isFriend(target) then
		...
		if target:hasShownSkill("jieyin") and not self.player:isWounded() and self.player:isMale() and not self.player:hasSkill("nosleiji") then return "." end
		if target:hasShownSkill("rende") and self.player:hasSkill("heg_jieming") then return "." end
	else
		...
		if target:hasWeapon("Axe") then
			if sgs.originalHegemonyHasShownSkills(target, sgs.lose_equip_skill) and target:getEquips():length() > 1 and target:getCardCount(true) > 2 then return not isdummy and "." end
...
if from:hasShownSkill("tenyearjianchu") and to:hasEquip() then return true end
		--[[
	...
	if to:getMark("##heg_qianxi+no_suit_red") > 0 and not hasBlack then return true end
	if to:getMark("##heg_qianxi+no_suit_black") > 0 and not hasRed then return true end
...
function SmartAI:useCardPeach(card, use)
	...
	if self.player:getHandcardNum() < 3 then
		for _, enemy in ipairs(self.enemies) do
			if sgs.originalHegemonyHasShownSkills(enemy, sgs.drawpeach_skill) or getCardsNum("Dismantlement", enemy, self.player) >= 1
				or enemy:hasShownSkill("heg_jixi") and enemy:getPile("field"):length() >0 and enemy:distanceTo(self.player) == 1
				...
				or (sgs.originalHegemonyHasShownSkills(enemy, "heg_tiaoxin|heg_baolie") and (self.player:inMyAttackRange(enemy) and self:getCardsNum("Slash") < 1 or not self.player:canSlash(enemy)))
or enemy:hasShownSkill("chuli")--旋略怎么考虑呢？
	...
	if self.player:hasSkill("heg_buqu") and self.player:getHp() < 1 and maxCards == 0 then
		use.card = card
		return
	end
	if not mustusepeach and not overflow and (self.player:hasSkill("heg_hunshang")
			or (self.player:hasSkill("heg_yinghun_sunjian") and self.player:getHp() + 1 < self.player:getMaxHp())) then
		return
	end
...
sgs.weapon_range.HBlade = 3

sgs.weapon_range.Spear = 3

sgs.weapon_range.HHalberd = 4

sgs.weapon_range.KylinBow = 5

sgs.weapon_range.HSixSwords = 2

sgs.weapon_range.HDragonPhoenix = 2

sgs.weapon_range.HTriblade = 3
...
sgs.ai_skill_cardask["double-sword-card"] = function(self, data, pattern, target)
	...
	for _, card in sgs.qlist(cards) do
		if (card:isKindOf("Slash") and self:getCardsNum("Slash") > 1)
			...
			or (not self.player:hasSkill("heg_jizhi") and (card:isKindOf("Collateral") or card:isKindOf("GodSalvation")
...
sgs.ai_skill_invoke.ice_sword = function(self, data)
	...
	if self:isFriend(target) then
		...
		if target:hasShownSkill("heg_tianxiang") and self.player:getMark("GlobalBattleRoyalMode") > 0 and self:cantbeHurt(target) then return true end
		...
		if self.player:hasSkill("heg_tieqi") or self:canLiegong(target, self.player) then return false end
		if target:hasShownSkill("heg_tuntian") and target:getPhase() == sgs.Player_NotActive then return false end
		if sgs.originalHegemonyHasShownSkills(target, sgs.need_kongcheng) then return false end
...
function sgs.ai_slash_weaponfilter.IceSword(self, to, player)
	return player:distanceTo(to) <= math.max(sgs.weapon_range.IceSword, player:getAttackRange())
		and to:hasShownSkill("heg_tianxiang") and player:getMark("GlobalBattleRoyalMode") > 0
end

function sgs.ai_weapon_value.IceSword(self, enemy, player)
	if enemy and player:getMark("GlobalBattleRoyalMode") > 0 and enemy:hasShownSkill("heg_tianxiang") then return 10 end
end
...
function SmartAI:needToThrowAll(player)
	...
	if heg_erzhang and self:isFriend(heg_erzhang, player) then return false end
...
sgs.ai_skill_cardask["@Axe"] = function(self, data, pattern, target)
	...
	if self:hasHeavySlashDamage(self.player, effect.slash, target)
	  ...
	  or (self.player:hasSkills("tenyearkuanggu|heg_kuanggu_xh") and self.player:isWounded() and self.player:distanceTo(effect.to) == 1)
	  or (effect.to:getHp() == 1 and not effect.to:hasShownSkill("heg_buqu"))
...
function sgs.ai_weapon_value.Axe(self, enemy, player)
	if sgs.originalHegemonyHasShownSkills(player, "heg_luoyi|heg_suzhi") and not sgs.originalHegemonyHasShownSkills(player, sgs.force_slash_skill) then return 6 end
	...
	if player:getPhase() == sgs.Player_Play then
		if player:hasShownSkill("heg_fengshix") and player:getHandcardNum() > 3
		and player:getHandcardNum() > enemy:getHandcardNum() then
			v = v + 1
		end
	end
	if enemy and not sgs.originalHegemonyHasShownSkills(player, sgs.force_slash_skill) then
	...
	if class_name == "Slash" then
		if not cards then
			...
			for _, c in sgs.qlist(player:getHandcards()) do
				if sgs.cardIsVisible(c, player, self.player) and not c:isKindOf("Slash") then
...
local function turnUse_spear(self, inclusive, skill_name)
	if self.player:hasSkills("tenyearwusheng|heg_wusheng_xh") then
		local cards = self.player:getCards("he")
		cards = sgs.QList2Table(cards)
	...
	local newcards = {}
	for _, card in ipairs(cards) do
		if not isCard("Slash", card, self.player) and not isCard("Peach", card, self.player) and not isCard("HAllianceFeast", card, self.player)
		and not ((isCard("ExNihilo", card, self.player) or isCard("HBefriendAttacking", card, self.player)) and self.player:getPhase() == sgs.Player_Play)
		and not ((isCard("HThreatenEmperor", card, self.player)) and card:isAvailable(self.player)) then
			table.insert(newcards, card)
		end
	end
	if #cards <= self.player:getHp() - 1 and self.player:getHp() <= 4 and not self:hasHeavySlashDamage(self.player)
		and not self.player:hasSkills("heg_kongcheng|heg_paoxiao") then return end
	if #newcards < 2 then return end
...
function sgs.ai_weapon_value.Spear(self, enemy, player)
	if sgs.originalHegemonyHasShownSkills(player, "heg_paoxiao|heg_paoxiao_xh|heg_xiongnve|heg_kuangcai") or (player:hasShownSkill("heg_baolie") and player:getHp() < 3) then
...
if damage.from:hasShownSkill("kuangfu") and damage.to:getCards("e"):length() == 1 then return false end
	if sgs.originalHegemonyHasShownSkills(damage.to, sgs.lose_equip_skill) then
...
function sgs.ai_weapon_value.KylinBow(self, enemy,player)
	--if sgs.originalHegemonyHasShownSkills(player, "liegong|liegong_xh") then return 3.5 end
...
sgs.ai_skill_invoke.eight_diagram = function(self, data)
	...
	if heg_zhangjiao and self:isEnemy(heg_zhangjiao) then
		if getKnownCard(heg_zhangjiao, self.player, "black", false, "he") > 1 then return false end
		if self:getCardsNum("Jink") > 1 and getKnownCard(heg_zhangjiao, self.player, "black", false, "he") > 0 then return false end
		if heg_zhangjiao:getHandcardNum() - getKnownNum(heg_zhangjiao, self.player) >= 3 then return false end
	end
	return true
end
...
function sgs.ai_armor_value.EightDiagram(player, self)
	...
	if sgs.originalHegemonyHasShownSkills(player, "tiandu|nosleiji|heg_zhuwei") then
		return 6
	end
	for _, p in sgs.qlist(self.room:getOtherPlayers(player)) do
		if p:hasShownSkill("heg_luanji") then
			return 4.1
		end
	end
	return 4
...
function sgs.ai_armor_value.RenwangShield(player, self)
	if player:hasShownSkill("bazhen") then return 0 end
	if player:hasShownSkill("heg_jiang") then return 6 end
	if player:hasShownSkill("nosleiji") and getKnownCard(player, self.player, "Jink", true) > 1 and player:hasShownSkill("guidao")
...
function sgs.ai_armor_value.SilverLion(player, self)
	...
	if player:hasShownSkill("heg_shicai") then return 5 end
	if player:hasShownSkill("heg_gongqing") then
...
sgs.ai_use_priority.HBlade = 2.688
...
sgs.ai_use_priority.HHalberd = 2.65
...
sgs.ai_nullification.SavageAssault = function(self, card, from, to, positive, keep)
	...
	if positive then
		if self:isFriend(to) then
			local heg_zhurong = sgs.findPlayerByShownSkillName("heg_juxiang")
			if heg_menghuo then targets:removeOne(heg_menghuo) end
			if heg_zhurong then targets:removeOne(heg_zhurong) end

			if keep then
				for _, p in sgs.qlist(targets) do
					if self:isFriend(p) and self:aoeIsEffective(card, p, heg_menghuo or from)
						and self:needDamagedEffects(p, heg_menghuo or from) and self:isWeak(p)
			...
			local heg_null_card = self:getCard("HegNullification") or (self.room:getTag("NullifyingTimes"):toInt() > 0 and self.room:getTag("NullificatonType"):toBool())
			if heg_null_card and self:aoeIsEffective(card, to, heg_menghuo or from) then
				targets:removeOne(to)
				for _, p in sgs.qlist(targets) do
					if to:isFriendWith(p) and self:aoeIsEffective(card, p, heg_menghuo or from) then return true, false end
				end
			end

			if not self:isFriendWith(to) and not self:isWeak(to) then
				return
			elseif not self:aoeIsEffective(card, to, heg_menghuo or from) then
				return
			elseif self:needDamagedEffects(to, heg_menghuo or from) then
				return
			elseif to:objectName() == self.player:objectName() and self:canAvoidAOE(card) then
				return
		...
		if keep then
			for _, p in sgs.qlist(targets) do
				if self:isEnemy(p) and self:aoeIsEffective(card, p, heg_menghuo or from)
					and self:needDamagedEffects(p, heg_menghuo or from) and self:isWeak(p)
					and getKnownCard(p, self.player, "Slash", true, "he") == 0 then
					keep = false
				end
		...
		if nulltype then
			...
			local weak
			for _, p in sgs.qlist(targets) do
				if to:isFriendWith(p) and self:aoeIsEffective(card, p, heg_menghuo or from) then
					num = num + 1
				end
				if self:isWeak(to) or self:isWeak(p) then
					weak = true
...
sgs.ai_skill_cardask.aoe = function(self, data, pattern, target, name)
	...
	if aoe:isKindOf("ArcheryAttack") then
		if not self.player:hasSkill("nosleiji") and self:isFriend(self.player, target) and target:hasSkill("heg_zhiman") then return "." end
	elseif aoe:isKindOf("SavageAssault") then
		if not heg_menghuo and self:isFriend(self.player, target) and target:hasSkill("heg_zhiman") then return "." end
		if heg_menghuo and heg_menghuo:hasShownSkill("heg_zhiman") and self:isFriend(self.player, heg_menghuo) then return "." end
	end
	if heg_menghuo and aoe:isKindOf("SavageAssault") then attacker = heg_menghuo end
	...
	if (self.player:hasSkill("nosjianxiong") or (self.player:hasSkill("heg_qiuan") and self.player:getPile("letter"):isEmpty()))--新增孟达
...
function SmartAI:useCardAmazingGrace(card, use)
	...
	local aoedraw = heg_xuyou and self.player:isFriendWith(heg_xuyou)
	if (self:needKongcheng() and self.player:getHandcardNum() == 1) or self.player:hasSkill("heg_jizhi") or aoedraw then
		suf = 0.6
		coeff = 0.6
	end
	for _, player in sgs.qlist(self.room:getOtherPlayers(self.player)) do
...
function SmartAI:willUseGodSalvation(card)
	...
	local heg_xuyou = sgs.findPlayerByShownSkillName("heg_chenglve")
	local aoedraw = heg_xuyou and self.player:isFriendWith(heg_xuyou)
	if self.player:hasSkill("heg_jizhi") or aoedraw then good = good + 6 end
	if (self.player:hasSkill("heg_kongcheng") and self.player:getHandcardNum() == 1) or not self:hasLoseHandcardEffective() then good = good + 5 end
	...
	for _, friend in ipairs(self.friends) do
		...
		if self:trickIsEffective(card, friend, self.player) then
			if friend:canRecover() then
				...
				if sgs.originalHegemonyHasShownSkills(friend, sgs.masochism_skill) then
					good = good + 5
				end
	...
	for _, enemy in ipairs(self.enemies) do
		...
		if self:trickIsEffective(card, enemy, self.player) then
			if enemy:canRecover() then
				...
				if sgs.originalHegemonyHasShownSkills(enemy, sgs.masochism_skill) then
					bad = bad + 5
				end
...
function SmartAI:useCardDuel(duel, use)
	...
	duel:setFlags("-AI_Using")
	if self.player:hasSkills("wushuang|heg_wushuang_lvlingqi") then n1 = n1 * 2 end
	local heg_huatuo = sgs.findPlayerByShownSkillName("jijiu")
	local targets = {}
	...
	for _, friend in ipairs(friends) do
		...
		if friend:hasSkill("heg_jieming") and self.player:hasSkill("rende") and (heg_huatuo and self:isFriend(heg_huatuo)) then
			table.insert(targets, friend)
		end
		if self.player:hasSkill("heg_zhiman") and (self.player:canGet(friend, "j") or ((sgs.originalHegemonyHasShownSkills(friend, sgs.lose_equip_skill) or self:needToThrowArmor(friend)) and self.player:canGet(friend, "e"))) then
			table.insert(targets, friend)
		end
	end
	...
	local cmp = function(a, b)
		...
		if not self:isWeak(a) and a:hasSkill("nosjianxiong") then v1 = v1 + 10 end
		if not self:isWeak(b) and b:hasSkill("nosjianxiong") then v2 = v2 + 10 end

		if self:needToLoseHp(a) then v1 = v1 + 5 end
		if self:needToLoseHp(b) then v2 = v2 + 5 end

		if sgs.originalHegemonyHasShownSkills(a, sgs.masochism_skill) then v1 = v1 + 5 end
		if sgs.originalHegemonyHasShownSkills(b, sgs.masochism_skill) then v2 = v2 + 5 end

		if not self:isWeak(a) and a:hasSkill("heg_jiang") then v1 = v1 + 5 end
		if not self:isWeak(b) and b:hasSkill("heg_jiang") then v2 = v2 + 5 end
	...
	for _, enemy in ipairs(enemies) do
		...
		if enemy:hasSkills("wushuang|heg_wushuang_lvlingqi") then n2 = n2 * 2 end
		...
		useduel = n1 >= n2 or self:needToLoseHp(self.player, nil, nil, true) or noresponse
					...
					or ((self.player:hasSkill("nosjianxiong") or self.player:hasFlag("shuangxiong")) and sgs.isGoodHp(self.player, self.player)
...
sgs.ai_skill_cardask["duel-slash"] = function(self, data, pattern, target)
	if self:isFriend(self.player, target) and target:hasSkill("heg_zhiman") then return "." end
	...
	if self:isFriend(target) and target:hasSkill("rende") and self.player:hasSkill("heg_jieming") then return "." end
	...
	if heg_caopi and self:isFriend(heg_caopi) then
		saveforcaopi = true
	end
...
--有优先顺序
	if weapon and (weapon:isKindOf("Crossbow") or weapon:isKindOf("GudingBlade")) and self:isEnemy(who) then
		for _, friend in ipairs(self.friends) do
			...
			if weapon:isKindOf("GudingBlade") and who:inMyAttackRange(friend) and friend:isKongcheng() and not friend:hasShownSkill("heg_kongcheng") and getCardsNum("Slash", who) > 0 then
				return weapon:getEffectiveId()
			end
		end
	end
	if weapon and weapon:isKindOf("Axe") and sgs.originalHegemonyHasShownSkills(who, "heg_luoyi|heg_suzhi") then
		return weapon:getEffectiveId()
	end

	if treasure and treasure:isKindOf("HJadeSeal") then
		return treasure:getEffectiveId()
	end
	if treasure and treasure:isKindOf("HLuminousPearl") and sgs.originalHegemonyHasShownSkills(who, sgs.lose_equip_skill .. "|heg_zhiheng|lirang") then
		return treasure:getEffectiveId()
	end
	...
	if weapon and weapon:isKindOf("HHalberd") and sgs.originalHegemonyHasShownSkills(who, sgs.force_slash_skill .. "|heg_paoxiao|heg_paoxiao_xh|heg_baolie|heg_xiongnve|heg_kuangcai") then
		return weapon:getEffectiveId()
	end
	if weapon and weapon:isKindOf("Spear") and sgs.originalHegemonyHasShownSkills(who, "heg_paoxiao|heg_paoxiao_xh|heg_baolie|heg_xiongnve|heg_kuangcai") and who:getHandcardNum() >= 1 then
		return weapon:getEffectiveId()
	end
	if weapon and weapon:isKindOf("HDragonPhoenix") and sgs.originalHegemonyHasShownSkills(who, "heg_paoxiao|heg_paoxiao_xh|heg_baolie|heg_suzhi|heg_xiongnve|heg_kuangcai") then
		return weapon:getEffectiveId()
	end

	if armor and armor:isKindOf("EightDiagram") and sgs.originalHegemonyHasShownSkills(who, sgs.wizard_skill) then
		return armor:getEffectiveId()
	end
	if armor and armor:isKindOf("RenwangShield") and who:hasShownSkill("heg_jiang") then
		return armor:getEffectiveId()
	end
	if armor and armor:isKindOf("HPeaceSpell") and self.player:getPlayerNumWithSameKingdom("AI", who:getKingdom()) > 2
	and not sgs.findPlayerByShownSkillName("heg_wendao") then
		return armor:getEffectiveId()
	end
...
function SmartAI:getValuableCard(who)
	...
	and (not armor:isKindOf("HPeaceSpell") or not sgs.findPlayerByShownSkillName("heg_wendao")) then
		return armor:getEffectiveId()
	end

	if offhorse and sgs.originalHegemonyHasShownSkills(who, "tenyearkuanggu|heg_duanbing|heg_qianxi") then
		return offhorse:getEffectiveId()
	end

	local equips = sgs.QList2Table(who:getEquips())
	for _,equip in ipairs(equips) do
		if who:hasShownSkill("heg_guose") and equip:getSuit() == sgs.Card_Diamond then  return equip:getEffectiveId() end
		if sgs.originalHegemonyHasShownSkills(who, "qixi|heg_duanliang|guidao") and equip:isBlack() then  return equip:getEffectiveId() end
		if sgs.originalHegemonyHasShownSkills(who, "tenyearwusheng|jijiu") and equip:isRed() then  return equip:getEffectiveId() end
		if sgs.originalHegemonyHasShownSkills(who, sgs.need_equip_skill) and not sgs.originalHegemonyHasShownSkills(who, sgs.lose_equip_skill) then return equip:getEffectiveId() end
	end
...
function SmartAI:useCardSnatchOrDismantlement(card, use)
	local isJixi = card:getSkillName() == "heg_jixi"
	...
	local canOperate = function(target, card_id)--注意card_id为字符串的重载
		local heg_wujing = sgs.findPlayerByShownSkillName("heg_fengyang")
		if heg_wujing and heg_wujing:inFormationRalation(target) and not self.player:isFriendWith(target)
		...
		if card:isKindOf("Snatch") then
			if target:hasSkill("heg_jubao") and type(card_id) == "number"
			and target:getTreasure() and target:getTreasure():getEffectiveId() == card_id then
				return false
			end
			return self.player:canGet(target, card_id)
		else
	...
	if self:slashIsAvailable() then
		...
		if not dummyuse.to:isEmpty() then
			...
			for _, to in ipairs(tos) do
				if to:getHandcardNum() == 1 and to:getHp() <= 2 and self:hasLoseHandcardEffective(to) and not to:hasSkill("heg_kongcheng")
	...
	for _, enemy in ipairs(enemies) do
		if not enemy:isNude() then
			if sgs.originalHegemonyHasShownSkills(enemy, "jijiu|jieyin") then
				local cardchosen
			...
			if not enemy:isNude() and not (self:needKongcheng(enemy) and i <= 2) and not self:doNotDiscard(enemy) then
				if (enemy:getHandcardNum() == i and sgs.getDefenseSlash(enemy, self) < 6 + (isJixi and 6 or 0) and enemy:getHp() <= 3 + (isJixi and 2 or 0)) then
					...
					elseif enemy:getArmor() and not self:needToThrowArmor(enemy) and not self:doNotDiscard(enemy, "e")
						and (not enemy:getArmor():isKindOf("HPeaceSpell") or not sgs.findPlayerByShownSkillName("heg_wendao"))
						and canOperate(enemy, enemy:getArmor():getEffectiveId())then
						cardchosen = enemy:getArmor():getEffectiveId()
	...
	for _, enemy in ipairs(enemies) do
		if not enemy:isKongcheng() and not self:doNotDiscard(enemy, "h")
			and sgs.originalHegemonyHasShownSkills(enemy, sgs.cardneed_skill) and canOperate(enemy, "h") then
			if addTarget(enemy, self:getCardRandomly(enemy, "h")) then return end
		end
	end
		...
		elseif place == sgs.Player_PlaceEquip then
			...
			if card:isKindOf("SilverLion") then
				if to:getLostHp() > 1 then
					if sgs.originalHegemonyHasShownSkills(to, sgs.use_lion_skill) then
			...
			elseif sgs.originalHegemonyHasShownSkills(to, sgs.lose_equip_skill) then
...
function SmartAI:useCardCollateral(card, use)
	...
	needCrossbow = needCrossbow and self:getCardsNum("Slash") > 2 and not self.player:hasSkills("heg_paoxiao|heg_kuangcai")
	...
	for _, enemy in ipairs(fromList) do
		if self:trickIsEffective(card, enemy)
			and not sgs.originalHegemonyHasShownSkills(enemy, sgs.lose_equip_skill)
			and not (enemy:hasSkill("heg_weimu") and card:isBlack())
			and not enemy:hasSkill("heg_tuntian")
	...
	for _, friend in ipairs(fromList) do
		if friend:getWeapon() and sgs.originalHegemonyHasShownSkills(friend, sgs.lose_equip_skill)
			and self:trickIsEffective(card, friend)
...
function SmartAI:enemiesContainsTrick(EnemyCount)
	...
	local heg_zhanghe = sgs.findPlayerByShownSkillName("qiaobian")
	if heg_zhanghe and (not self:isEnemy(heg_zhanghe) or heg_zhanghe:isKongcheng() or not heg_zhanghe:faceUp()) then heg_zhanghe = nil end

	if self.player:hasSkill("heg_guose") then
		for _, acard in sgs.qlist(self.player:getCards("he")) do
			if acard:getSuit() == sgs.Card_Diamond then indul_num = indul_num + 1 end
		end
	end

	if self.player:hasSkill("heg_duanliang") then
		for _, acard in sgs.qlist(self.player:getCards("he")) do
	...
	for _, enemy in ipairs(self.enemies) do
		if enemy:containsTrick("indulgence") then
			if not enemy:hasSkill("heg_keji") and   (not heg_zhanghe or self:playerGetRound(enemy) >= self:playerGetRound(heg_zhanghe)) then
		...
		if self.player:distanceTo(enemy) == 1 or self.player:hasSkill("heg_duanliang") and self.player:distanceTo(enemy) <= 2 then
			if enemy:containsTrick("supply_shortage") then
				if not enemy:hasSkill("heg_shensu") and (not heg_zhanghe or self:playerGetRound(enemy) >= self:playerGetRound(heg_zhanghe)) then
					trick_all = trick_all + 1
					if not temp_enemy or temp_enemy:objectName() ~= enemy:objectName() then
						enemy_num = enemy_num + 1
						temp_enemy = enemy
...
function SmartAI:useCardIndulgence(card, use)
	...
	local heg_zhanghe = sgs.findPlayerByShownSkillName("qiaobian")
	local zhanghe_seat = heg_zhanghe and heg_zhanghe:faceUp() and not heg_zhanghe:isKongcheng() and not self:isFriend(heg_zhanghe) and heg_zhanghe:getSeat() or 0
	...
	local getvalue = function(enemy)
		...
		if enemy:hasShownSkill("heg_qianxun") then return -101 end
		if enemy:hasShownSkill("heg_weimu") and card:isBlack() then return -101 end
		...
		if zhanghe_seat > 0 and (self:playerGetRound(heg_zhanghe) <= self:playerGetRound(enemy) and self:enemiesContainsTrick() <= 1 or not enemy:faceUp()) then
			return -101
		end
		...
		if sgs.originalHegemonyHasShownSkills(enemy, sgs.priority_skill) then value = value + 5 end
		--if sgs.originalHegemonyHasShownSkills(enemy, "qixi|heg_guose|heg_duanliang|luoshen|jizhi|wansha") then value = value + 5 end
		--if sgs.originalHegemonyHasShownSkills(enemy, "guzheng|heg_duoshi") then value = value + 3 end
		...
		if enemy:getRole() == "careerist" and sgs.originalHegemonyGeneral(enemy, true):getKingdom() == "careerist" then
			value = value + 3
		end

		if self:objectiveLevel(enemy) < 3 then value = value - 10 end
		if not enemy:faceUp() then value = value - 10 end
		if sgs.originalHegemonyHasShownSkills(enemy, "heg_shensu") then value = value - enemy:getHandcardNum() end
		if sgs.originalHegemonyHasShownSkills(enemy, "heg_keji|lirang|heg_shengxi|heg_xingzhao|heg_tongdu") then value = value - 4 end
		if sgs.originalHegemonyHasShownSkills(enemy, "guanxing|tenyeartuxi|heg_tianxiang|"..sgs.wizard_skill) then value = value - 3 end
...
function SmartAI:willUseLightning(card)
	...
	if self.player:hasSkill("heg_weimu") and card:isBlack() then
		local shouldUse = true
		for _, p in sgs.qlist(self.room:getOtherPlayers(self.player)) do
			if self:evaluateKingdom(p) == "unknown" then shouldUse = false break end
			if self:evaluateKingdom(p) == self:originalHegemonyOwnKingdom() then shouldUse = false break end
		end
	...
	local function hasDangerousFriend()
		for _, aplayer in ipairs(self.enemies) do
			if sgs.originalHegemonyHasShownSkills(aplayer, sgs.wizard_skill.."|hongyan|heg_wendao") and #self.enemies < 3 then return true end
			if sgs.originalHegemonyHasShownSkills(aplayer, "guanxing|heg_yizhi") and self:isFriend(aplayer:getNextAlive()) then return true end
		end
		return false
	end
	...
	elseif not hasDangerousFriend() then
		if self.player:hasSkills("guanxing+heg_kongcheng") and self.player:isLastHandCard(card) then return true end
		...
		for _, player in ipairs(players) do
			if self:objectiveLevel(player) >= 4 and not player:hasSkill("hongyan") and not (player:hasSkill("heg_weimu") and card:isBlack()) then
				enemies = enemies + 1
			elseif self:isFriend(player) and not player:hasSkill("hongyan") and not (player:hasSkill("heg_weimu") and card:isBlack()) then
				friends = friends + 1
			end
		end
...
---------------
	...
	for _, card in ipairs(cards) do
		if isCard("ExNihilo", card, self.player) then
			if not NextPlayerCanUse or (not self:willSkipPlayPhase() and (self.player:hasSkills("heg_jizhi|heg_zhiheng|rende") or not sgs.originalHegemonyHasShownSkills(NextPlayer, "heg_jizhi|heg_zhiheng|rende"))) then
		...
		elseif isCard("Dismantlement", card, self.player) then
			dismantlement = card
		elseif isCard("HBefriendAttacking", card, self.player) then
	...
	for _, card in ipairs(cards) do
		...
		elseif card:isKindOf("HIronArmor") then ironarmor = card:getEffectiveId()
		...
		elseif card:isKindOf("HHalberd") then halberd = card:getEffectiveId()

		elseif card:isKindOf("HJadeSeal") then jadeseal = card:getEffectiveId()  end

		if not weapon and card:isKindOf("Weapon") then weapon = card:getEffectiveId() end
	end

	if eightdiagram then
		if not self.player:hasSkill("bazhen") and self.player:hasSkills("tiandu|nosleiji|hongyan") and not self.player:getArmor() then
			return eightdiagram
		end
		if NextPlayerisEnemy and sgs.originalHegemonyHasShownSkills(NextPlayer, "tiandu|nosleiji|hongyan") and not NextPlayer:getArmor() then
			return eightdiagram
		end
	end
	...
	if silverlion then
		...
		for _, p in sgs.qlist(self.room:getOtherPlayers(self.player)) do
			if p:hasSkill("nosleiji") and self:isEnemy(p) then
				return silverlion
			end
			if p:containsTrick("lightning") then
				lightning = true
			end
			if sgs.originalHegemonyHasShownSkills(p, "guicai|guidao") and self:isEnemy(p) then
				canRetrial = true
			end
		end
	...
	if ironarmor then
		for _, enemy in ipairs(self.enemies) do
			if enemy:hasShownSkill("heg_huoji") then return ironarmor end
			...
			if enemy:getFormation():contains(self.player) and getCardsNum("HBurningCamps", enemy, self.player) > 0 then return ironarmor end
		end
	end

	if DefHorse and (not self.player:hasSkill("nosleiji") or self:getCardsNum("Jink") == 0) then
	...
	if weapon and (self:getCardsNum("Slash") > 0 and self:slashIsAvailable() or not SelfisCurrent) then
		...
		if crossbow then
			if #self:getCards("Slash") > 1 or self.player:hasSkills("heg_kurou|heg_keji")
				or (self.player:hasSkills("heg_luoshen|guzheng") and not SelfisCurrent and self.room:alivePlayerCount() >= 4) then
			...
			if self:isEnemy(NextPlayer) then
				local CanSave, heg_huanggai, heg_zhenji
				for _, enemy in ipairs(self.enemies) do
					if enemy:hasShownSkill("jijiu") and getKnownCard(enemy, self.player, "red", nil, "he") > 1 then CanSave = true end
					if enemy:hasShownSkill("heg_kurou") then heg_huanggai = enemy end
					if enemy:hasShownSkill("heg_keji") then return crossbow:getEffectiveId() end
					if sgs.originalHegemonyHasShownSkills(enemy, "heg_luoshen|guzheng") then return crossbow:getEffectiveId() end
				end
				if heg_huanggai then
					if heg_huanggai:getHp() > 2 then return crossbow:getEffectiveId() end
		...
		if halberd then
			if self.player:hasSkills(sgs.force_slash_skill .. "|" .."heg_paoxiao|heg_paoxiao_xh|heg_baolie|heg_xiongnve|heg_kuangcai") then
				return halberd
			end
		end
	...
	local classNames = { "Snatch", "Dismantlement", "Indulgence", "SupplyShortage", "Collateral", "Duel", "HDrowning", "ArcheryAttack", "SavageAssault", "FireAttack",
	...
	local className2objectName = { Snatch = "snatch", Dismantlement = "dismantlement", Indulgence = "indulgence", SupplyShortage = "supply_shortage", Collateral = "collateral",
									Duel = "duel", HDrowning = "drowning", ArcheryAttack = "archery_attack", SavageAssault = "savage_assault", FireAttack = "fire_attack",
									GodSalvation = "god_salvation", Lightning = "lightning" }
	local new_enemies = {}
	if #self.enemies > 0 then new_enemies = self.enemies
	else
		for _, aplayer in sgs.qlist(self.room:getOtherPlayers(self.player)) do
			if not string.find(self:evaluateKingdom(aplayer), self:originalHegemonyOwnKingdom()) then
...
function SmartAI:useCardAwaitExhausted(HAwaitExhausted, use)
	if not HAwaitExhausted:isAvailable(self.player) then return end
	use.card = HAwaitExhausted
end

sgs.ai_use_priority.HAwaitExhausted = 2.8

sgs.ai_use_value.HAwaitExhausted = 4.9

sgs.ai_keep_value.HAwaitExhausted = 3.22

sgs.ai_card_intention.HAwaitExhausted = function(self, card, from, tos)
	for _, to in ipairs(tos) do
		sgs.updateIntention(from, to, -50)
	end
end

sgs.ai_nullification.HAwaitExhausted = function(self, card, from, to, positive, keep)
	local targets = sgs.SPlayerList()
	local players = self.room:getTag("targets" .. card:toString()):toList()
	for _, q in sgs.qlist(players) do
		targets:append(q:toPlayer())
	...
	if positive then
		...
		if self:isEnemy(to) and targets:length() > 1 and hegnull then
			for _, p in sgs.qlist(targets) do
				if (sgs.originalHegemonyHasShownSkills(p, sgs.lose_equip_skill) and p:getEquips():length() > 0)
			...
			if self:isEnemy(to) and self:evaluateKingdom(to) ~= "unknown" then
				...
				if sgs.originalHegemonyHasShownSkills(to, sgs.lose_equip_skill) and to:getEquips():length() > 0 then return true, true end
...
function SmartAI:useCardBefriendAttacking(HBefriendAttacking, use)
	if not HBefriendAttacking:isAvailable(self.player) then return end
	local targets = sgs.PlayerList()
	local players = sgs.QList2Table(self.room:getOtherPlayers(self.player))
	self:sort(players)
	for _, to_select in ipairs(players) do
		if to_select:hasShownSkill("heg_luanji") and to_select:getHandcardNum() > 3 then continue end
		if self:isFriend(to_select) and HBefriendAttacking:targetFilter(targets, to_select, self.player) and not targets:contains(to_select)
			and self:trickIsEffective(HBefriendAttacking, to_select, self.player) then
	...
	if targets:isEmpty() then															--对野心家使用
		for _, to_select in ipairs(players) do
			if to_select:hasShownSkill("heg_luanji") and to_select:getHandcardNum() > 3 then continue end
			if HBefriendAttacking:targetFilter(targets, to_select, self.player) and not targets:contains(to_select)
				and self:trickIsEffective(HBefriendAttacking, to_select, self.player) and to_select:getRole() == "careerist" then
				targets:append(to_select)
				if use.to then use.to:append(to_select) end
			end
		end
	...
	if self.player:getRole() ~= "careerist" then
		table.removeOne(kingdoms, self:originalHegemonyOwnKingdom())
	end
	local num = 10
	local targetKingdom
	...
	if targets:isEmpty() then															--对该势力使用
		for _, to_select in ipairs(players) do
			if to_select:hasShownSkill("heg_luanji") and to_select:getHandcardNum() > 3 then continue end
			if HBefriendAttacking:targetFilter(targets, to_select, self.player) and not targets:contains(to_select)
				and self:trickIsEffective(HBefriendAttacking, to_select, self.player) and to_select:getKingdom() == targetKingdom then
				targets:append(to_select)
				if use.to then use.to:append(to_select) end
			end
		end
	...
	if not targets:isEmpty() then
		use.card = HBefriendAttacking
		return
	end
end

sgs.ai_use_priority.HBefriendAttacking = 9.28

sgs.ai_use_value.HBefriendAttacking = 10

sgs.ai_keep_value.HBefriendAttacking = 3.9

sgs.ai_nullification.HBefriendAttacking = function(self, card, from, to, positive, keep)
	if keep then return false end
	if positive then
		if not self:isFriend(to) and self:isEnemy(from) and (self:isWeak(from) or sgs.originalHegemonyHasShownSkills(from, sgs.cardneed_skill)) then
			return true, true
		end
	else
...
function SmartAI:useCardKnownBoth(HKnownBoth, use)
	self.knownboth_choice = {}
	if not HKnownBoth:isAvailable(self.player) then return false end
	local targets = sgs.PlayerList()
	local total_num = 1 + sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_ExtraTarget, self.player, HKnownBoth)
	for _, player in sgs.qlist(self.room:getOtherPlayers(self.player)) do
		if HKnownBoth:targetFilter(targets, player, self.player) and sgs.isAnjiang(player) and not targets:contains(player)
			and player:getMark(("KnownBoth_%s_%s"):format(self.player:objectName(), player:objectName())) == 0 and self:trickIsEffective(HKnownBoth, player, self.player) then
			use.card = HKnownBoth
	...
	if total_num > targets:length() then
		...
		for _, enemy in ipairs(self.enemies) do
			if HKnownBoth:targetFilter(targets, enemy, self.player) and enemy:getHandcardNum() - self:getKnownNum(enemy, self.player) > 3 and not targets:contains(enemy)
				and self:trickIsEffective(HKnownBoth, enemy, self.player) then
				use.card = HKnownBoth
	...
	if total_num > targets:length() and not targets:isEmpty() then
		...
		for _, friend in ipairs(self.friends_noself) do
			if self:getKnownNum(friend, self.player) ~= friend:getHandcardNum() and HKnownBoth:targetFilter(targets, friend, self.player) and not targets:contains(friend)
				and self:trickIsEffective(HKnownBoth, friend, self.player) then
	...
	if not use.card then
		...
		if not self.player:isCardLimited(HKnownBoth, sgs.Card_MethodRecast) and HKnownBoth:canRecast() then
			use.card = HKnownBoth
			if use.to then use.to = sgs.SPlayerList() end
		end
	end
...
sgs.ai_use_priority.HKnownBoth = 9.1

sgs.ai_use_value.HKnownBoth = 5

sgs.ai_keep_value.HKnownBoth = 3.24

sgs.ai_nullification.HKnownBoth = function(self, card, from, to, positive)
	return false
end
...
sgs.ai_skill_use["@@heg_Triblade"] = function(self, prompt)
	...
	for _, target in sgs.qlist(targets) do
		if self:isEnemy(target) and self:damageIsEffective(target, nil, self.player) and not self:getDamagedEffects(target, self.player)
			...
			-- Let the V2 request reconstruct the authoritative proxy card.
			return { cards = { id }, targets = { target:objectName() } }
		end
	end
	for _, target in sgs.qlist(targets) do
...
function sgs.ai_slash_weaponfilter.HTriblade(self, to, player)
	if player:distanceTo(to) > math.max(sgs.weapon_range.HTriblade, player:getAttackRange()) then return end
	return sgs.card_lack[to:objectName()]["Jink"] == 1 or getCardsNum("Jink", to, self.player) < 1
end

function sgs.ai_weapon_value.HTriblade(self, enemy, player)
...
sgs.ai_use_priority.HTriblade = 2.673

function sgs.ai_slash_weaponfilter.HSixSwords(self, to, player)
	return player:distanceTo(to) <= math.max(sgs.weapon_range.HSixSwords, player:getAttackRange())
		and (sgs.card_lack[to:objectName()]["Jink"] == 1 or getCardsNum("Jink", to, self.player) < 1)
end

function sgs.ai_weapon_value.HSixSwords(self, enemy, player)
	local v = 0
