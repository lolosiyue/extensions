-- Source: TODO/QSanguosha-For-Hegemony-xxyheaven/lua/ai/power-ai.lua (cf61c15).
...
sgs.ai_skill_use["@@heg_zhengbi"] = function(self, prompt, method)
  ...
  for _, p in sgs.qlist(self.room:getOtherPlayers(self.player)) do
		if not self:isFriend(p) and not p:hasShownOneGeneral() then
			if self:getCardsNum("Slash") > 1 and (p:getHp() < 3 or self:getCardsNum("HHalberd") > 0) then
        return "@HZhengbiCard=".. "->" .. p:objectName()
      end
		end
	end
  ...
  if card:isKindOf("Peach") then
    ...
    for _, friend in ipairs(self.friends_noself) do
      if friend:getCardCount(true) > 1 and (friend:isWounded() or self:getOverflow() > 0) and friend:hasShownOneGeneral() then
        ...
        return "@HZhengbiCard=" .. card:getEffectiveId() .. "->" .. friend:objectName()
      end
    end
    return "."
  end
  ...
  for _, target in ipairs(self.enemies) do
    if not target:isKongcheng() and (target:getHandcardNum() < 3 or self:isWeak(target)) and target:hasShownOneGeneral() then
      if not (card:isKindOf("Analeptic") and target:hasEquip() and self:isWeak(target)) then
        ...
        if not card:hasFlag("visible") then card:setFlags(visibleflag) end
        return "@HZhengbiCard=" .. card:getEffectiveId() .. "->" .. target:objectName()
      end
    end
  end
  return "."
...
sgs.ai_skill_cardask["@heg_zhengbi-give"] = function(self, data, pattern, target, target2)
...
fengying_skill.name = "heg_fengying"
...
fengying_skill.getTurnUseCard = function(self, inclusive)
  ...
  return sgs.Card_Parse("@HFengyingCard=.&")
end

sgs.ai_skill_use_func.HFengyingCard = function(card, use, self)
  local draw_count = 0
  ...
  if draw_count > 3 or self.player:getHp() == 1 then
    ...
    if self.player:getHandcardNum() == 1 then
      sgs.ai_use_priority.HFengyingCard = 2
    end
    use.card = card--不弃牌使用挟天子更优的情况估计得在挟天子弃牌的ai里写，需要data判定card:getSkillName()才行
  end
end

sgs.ai_card_intention.HFengyingCard = -80
sgs.ai_use_priority.HFengyingCard = 0

--于禁
sgs.ai_skill_use["@@heg_jieyue"] = function(self, prompt, method)
  if self.player:isKongcheng() then
    return "."
  end
  if self:willSkipDrawPhase()--手中最后一张牌是无懈？
  and not (self.player:hasSkill("qiaobian") and self.player:getHandcardNum() < 2)
  and not (self.player:hasSkill("heg_elitegeneralflag") and self.player:getHandcardNum() < 3) then
    return "."
  end
	local handcards = self.player:getCards("h")
	handcards = sgs.QList2Table(handcards)
  ...
  for _, p in ipairs(targets) do
    if self:isFriend(p) then
      ...
      if not card:hasFlag("visible") then card:setFlags(visibleflag) end
        return "@HJieyueCard=" .. card:getEffectiveId() .. "->" .. p:objectName()
    end
  end
...
sgs.ai_skill_choice["docommand_jieyue"] = function(self, choices, data)
  ...
  if is_enemy then
    ...
    if index == 3 and self.player:hasSkill("heg_hongfa") and not self.player:getPile("heavenly_army"):isEmpty() then
      return "yes"
    end
    if index == 4 then
...
function sgs.ai_cardneed.heg_jieyue(to, card)
	return to:isKongcheng()
end

--王平
local jianglve_skill = {}
jianglve_skill.name = "heg_jianglve"
...
jianglve_skill.getTurnUseCard = function(self, inclusive)
  ...
  return sgs.Card_Parse("@HJianglveCard=.&heg_jianglve")
end

sgs.ai_skill_use_func.HJianglveCard= function(card, use, self)
	use.card = card
end

sgs.ai_card_intention.HJianglveCard = -120
sgs.ai_use_priority.HJianglveCard = 9.15
...
sgs.ai_skill_choice["docommand_jianglve"] = function(self, choices, data)
  local source = data:toPlayer()
  local index = self.player:getMark("command_index")
  if sgs.originalHegemonyGeneral(self.player, true):getKingdom() == "careerist" then
    return "yes"
  end
  if index == 4 then
    if self.player:getMark("command4_effect") > 0 then
    ...
    if self.player:hasSkill("heg_xuanhuo") and not source:hasUsed("HXuanhuoAttachCard") and source:getHandcardNum() > 5 then
      return "no"
    end
  end
  if index == 5 then
  ...
  if index == 6 then
    if (self.player:getEquips():length() < 4
      and self.player:getHandcardNum() < (self.player:hasSkills("heg_xuanhuoattach|heg_paoxiao") and 5 or 4))
...
sgs.ai_skill_choice["heg_jianglve"] = function(self, choices, data)--ai势力召唤
  choices = choices:split("+")
  ...
  if table.contains(choices,"show_both_generals") then
    ...
    local xuanhuo_priority = {"heg_paoxiao", "heg_tieqi", "tenyearkuanggu", "heg_liegong", "tenyearwusheng", "heg_longdan"}
...
--法正
sgs.ai_skill_invoke.heg_enyuan = function(self, data)
...
sgs.ai_skill_exchange["_enyuan"] = function(self,pattern,max_num,min_num,expand_pile)
  ...
  local heg_fazheng = sgs.findPlayerByShownSkillName("heg_enyuan")
  local visibleflag = string.format("%s_%s_%s", "visible", self.player:objectName(), heg_fazheng:objectName())
  ...
  if cards[1]:isKindOf("Peach") then
    if self:isFriend(heg_fazheng) then
...
local function getSlashtarget(self)
  ...
  local heg_liubei = self.room:getLord(self:originalHegemonyOwnKingdom())
  if heg_liubei and heg_liubei:hasLordSkill("heg_shouyue") then
    local can_chooseliegong  = true
    for _, p in sgs.qlist(self.room:getAlivePlayers()) do
      if p:hasSkill("heg_liegong") then
...
local function shouldUseXuanhuo(self)
  local xuanhuoskill = {"tenyearwusheng", "heg_paoxiao", "heg_longdan", "heg_tieqi", "heg_liegong", "tenyearkuanggu"}
  for _, p in sgs.qlist(self.room:getAlivePlayers()) do
  ...
  local choice = sgs.ai_skill_choice.heg_xuanhuo(self, xuanhuochoices)
  self.room:writeToConsole("---眩惑预选技能:"..sgs.Sanguosha:translate(choice).."---")

  --如何去除没有连弩或咆哮却选武圣，牌少又断杀等情况
  if choice ~= "heg_paoxiao" and not self:slashIsAvailable() and self:getOverflow() < 1 then
    return false
  end

  if self:getCardsNum("Slash") == 0 then
    if (choice == "tenyearwusheng" or choice == "heg_longdan") and self:getOverflow() > 1 then
  ...
  if self:getCardsNum("Slash") == 1 and (choice == "tenyearwusheng" or choice == "heg_longdan") then
  ...
  --[[
  if not self.player:hasSkill("tenyearkuanggu") and table.contains(xuanhuoskill,"tenyearkuanggu") and self:getCardsNum("Slash") < 2 and
  (self:getCardsNum("SavageAssault") + self:getCardsNum("ArcheryAttack") > 0) and
  (self.player:getOffensiveHorse() or sgs.originalHegemonyHasShownSkills(self.player, "mashu_machao|mashu_madai")
  ...
  if not self.player:hasSkill("heg_liegong") and table.contains(xuanhuoskill,"heg_liegong") and self.need_liegong_distance then
    local heg_liubei = self.room:getLord(self:originalHegemonyOwnKingdom())
    if heg_liubei and heg_liubei:hasLordSkill("heg_shouyue") then
  ...
  if self.player:hasSkills("heg_tieqi|heg_liegong|heg_qianxi")
  and (choice == "heg_liegong" or (choice == "heg_tieqi" and not target:hasShownSkill("heg_tianxiang"))) then
    return false
  end
...
xuanhuoattach_skill.name = "heg_xuanhuoattach"
...
xuanhuoattach_skill.getTurnUseCard = function(self, inclusive)
  ...
  if not self.player:hasUsed("HXuanhuoAttachCard") and shouldUseXuanhuo(self) then
    local cards = self.player:getHandcards()
	  cards = sgs.QList2Table(cards)
	  self:sortByUseValue(cards, true) -- 按使用价值从小到大排序
		return sgs.Card_Parse("@HXuanhuoAttachCard=" .. cards[2]:getEffectiveId())--给牌弃牌可能把武器或杀给了，导致第二次丢失目标
	end
end

sgs.ai_skill_use_func.HXuanhuoAttachCard= function(card, use, self)
  sgs.ai_use_priority.HXuanhuoAttachCard = 5
  --self.room:writeToConsole("发动眩惑:"..self.player:objectName())
  --sgs.debugFunc(self.player, 2)
  self.player:speak("发动眩惑")
  if self.player:hasSkill("heg_jizhi") then--使用锦囊后
    sgs.ai_use_priority.HXuanhuoAttachCard = 2.8
  end
  if self.need_kuanggu_AOE then--使用AOE前
    sgs.ai_use_priority.HXuanhuoAttachCard = 3.6
  end
  if self.player:hasSkill("heg_jili") then--使用完武器后
    sgs.ai_use_priority.HXuanhuoAttachCard = 6
  end
  for _, p in ipairs(self.friends) do
    if p:hasShownSkill("heg_yongjue") and self.player:isFriendWith(p) then
      sgs.ai_use_priority.HXuanhuoAttachCard = 9.6--勇决杀的优先调整到9.5
    end
  end
  if self.player:getMark("@strategy") >= 1 then--在王平限定技发动前
    sgs.ai_use_priority.HXuanhuoAttachCard = sgs.ai_use_priority.HJianglveCard + 0.1
  end
  if sgs.originalHegemonyGeneral(self.player, true):getKingdom() == "careerist" then
    sgs.ai_use_priority.HXuanhuoAttachCard = 20--野心家
  end
  --考虑配合仁德？
	use.card = card
end

sgs.ai_card_intention.HXuanhuoAttachCard = -90
...
sgs.ai_skill_choice.heg_xuanhuo = function(self, choices)
  choices = choices:split("+")
  local xuanhuoskill = {"tenyearwusheng", "heg_paoxiao", "heg_longdan", "heg_tieqi", "heg_liegong", "tenyearkuanggu"}
  local has_wusheng = self.player:hasSkills("tenyearwusheng|heg_wusheng_xh")
  local has_paoxiao = self.player:hasSkills("heg_paoxiao|heg_paoxiao_xh")
  local has_longdan = self.player:hasSkills("heg_longdan|heg_longdan_xh")
  local has_tieqi = self.player:hasSkills("heg_tieqi|heg_tieqi_xh")
  local has_liegong = self.player:hasSkills("heg_liegong|heg_liegong_xh")
  local has_kuanggu = self.player:hasSkills("tenyearkuanggu|heg_kuanggu_xh")
  local has_qianxi = self.player:hasSkill("heg_qianxi")
  local has_Crossbow = self:getCardsNum("Crossbow") > 0
  local has_baolie = self.player:hasSkill("heg_baolie") and self.player:getHp() < 3--夏侯霸新技能豹烈
...
--集中判断保证自己没有相应的技能和选项里有技能，避免每次都重复判断
  ...
  if self.need_kuanggu_AOE then--需要眩惑狂骨AOE
    ...
    return "tenyearkuanggu"
  end

  if not has_longdan and table.contains(choices,"heg_longdan") and self:getCardsNum("Jink") >= 1 then--龙胆可以杀队友进行回复或伤害，不需要target，虽然ai目前不会
  ...
  if not has_Crossbow and not has_paoxiao and not has_baolie and table.contains(choices,"heg_paoxiao") and enough_pxslash then
    self.room:writeToConsole(sgs.Sanguosha:translate(string.format("SEAT(%s)",self.player:getSeat()))..":眩惑可咆哮")
    can_paoxiao = true
  end
  if not has_wusheng and table.contains(choices,"tenyearwusheng") then
    self.room:writeToConsole(sgs.Sanguosha:translate(string.format("SEAT(%s)",self.player:getSeat()))..":眩惑可武圣")
    can_wusheng = true
  end
  if not has_tieqi and table.contains(choices,"heg_tieqi") then
  ...
  if not has_liegong and table.contains(choices,"heg_liegong") and target:getHp() >= self.player:getHp() then
    self.room:writeToConsole(sgs.Sanguosha:translate(string.format("SEAT(%s)",self.player:getSeat()))..":眩惑可烈弓")
    can_liegong = true--符合烈弓发动条件
  end
  if not has_kuanggu and table.contains(choices,"tenyearkuanggu") and (sgs.originalHegemonyHasShownSkills(self.player, "mashu_machao|mashu_madai") or self.player:distanceTo(target) < 2
  ...
  if not has_longdan and table.contains(choices,"heg_longdan") and self:getCardsNum("Jink") >= 1 then
    local heg_liubei = self.room:getLord(self:originalHegemonyOwnKingdom())
    if heg_liubei and heg_liubei:hasLordSkill("heg_shouyue") then
  ...
  if (has_tieqi or has_liegong or has_qianxi) then--马超兄弟/黄忠
    ...
    elseif can_paoxiao then--咆哮
        return "heg_paoxiao"
    elseif target:hasShownSkill("heg_tianxiang") and need_tieqi then
        return "heg_tieqi"
    elseif can_kuanggu then
        return "tenyearkuanggu"
    end
...
--武圣
sgs.ai_view_as.heg_wusheng_xh = function(card, player, card_place)
	local suit = card:getSuitString()
	local number = card:getNumberString()
	local card_id = card:getEffectiveId()
	if (card_place ~= sgs.Player_PlaceSpecial or player:getHandPile():contains(card_id)) and (player:getLord() and player:getLord():hasShownSkill("heg_shouyue") or card:isRed()) and not card:isKindOf("Peach") and not card:hasFlag("using") then
		return ("slash:heg_wusheng_xh[%s:%s]=%d&heg_wusheng_xh"):format(suit, number, card_id)
	end
end

local wusheng_xh_skill = {}
wusheng_xh_skill.name = "heg_wusheng_xh"
...
wusheng_xh_skill.getTurnUseCard = function(self, inclusive)
	...
	if self:getCardsNum("Slash") < 2 or self.player:hasSkill("heg_paoxiao|heg_paoxiao_xh") or (self.player:hasSkill("heg_baolie") and self.player:getHp() < 3) then disCrossbow = true end
	...
	for _, card in sgs.qlist(hecards) do
		if (self.player:getLord() and self.player:getLord():hasShownSkill("heg_shouyue") or card:isRed())
      ...
      and not isCard("HBefriendAttacking", card, self.player) and not isCard("HAllianceFeast", card, self.player)
			and (not isCard("Crossbow", card, self.player) or disCrossbow ) then
			local suit = card:getSuitString()
			local number = card:getNumberString()
			local card_id = card:getEffectiveId()
			local card_str = ("slash:heg_wusheng_xh[%s:%s]=%d&heg_wusheng_xh"):format(suit, number, card_id)
...
sgs.ai_suit_priority.heg_wusheng_xh = "club|spade|heart|diamond"

--咆哮
sgs.ai_skill_invoke.heg_paoxiao_xh = true

--龙胆
local longdan_xh_skill = {}
longdan_xh_skill.name = "heg_longdan_xh"
...
longdan_xh_skill.getTurnUseCard = function(self)
	...
	local card_str = ("slash:heg_longdan_xh[%s:%s]=%d&heg_longdan_xh"):format(suit, number, card_id)
	local slash = sgs.Card_Parse(card_str)
	assert(slash)
...
sgs.ai_view_as.heg_longdan_xh = function(card, player, card_place)
	local suit = card:getSuitString()
	local number = card:getNumberString()
	local card_id = card:getEffectiveId()
	if card_place == sgs.Player_PlaceHand or player:getHandPile():contains(card_id) then
		if card:isKindOf("Jink") then
			return ("slash:heg_longdan_xh[%s:%s]=%d&heg_longdan_xh"):format(suit, number, card_id)
		elseif card:isKindOf("Slash") then
			return ("jink:heg_longdan_xh[%s:%s]=%d&heg_longdan_xh"):format(suit, number, card_id)
		end
	end
end

--铁骑
sgs.ai_skill_invoke.heg_tieqi_xh = function(self, data)
	local target = data:toPlayer()
	if self:isFriend(target) then return false end
	return true
end

sgs.ai_skill_choice.heg_tieqi_xh = function(self, choices, data)
	local target = data:toPlayer()
...
--烈弓
sgs.ai_skill_invoke.heg_liegong_xh = function(self, data)
	local target = data:toPlayer()
...
sgs.ai_skill_choice.heg_liegong_xh = sgs.ai_skill_choice.heg_liegong

--狂骨
sgs.ai_skill_invoke.heg_kuanggu_xh = function(self, data)
	return true
end

sgs.ai_skill_choice.heg_kuanggu_xh = function(self, choices)
...
--吴国太
local ganlu_skill = {}
ganlu_skill.name = "heg_ganlu"
table.insert(sgs.ai_skills, ganlu_skill)
ganlu_skill.getTurnUseCard = function(self)
	if not self.player:hasUsed("HGanluCard") then
		return sgs.Card_Parse("@HGanluCard=.&heg_ganlu")
	end
end

sgs.ai_skill_use_func.HGanluCard = function(card, use, self)
	...
	for _, friend in ipairs(self.friends) do
		for _, enemy in ipairs(self.enemies) do
			if not sgs.originalHegemonyHasShownSkills(enemy, sgs.lose_equip_skill) then
				...
				if math.abs(ee - fe) <= lost_hp and ee > 0 and (ee > fe or ee == fe and value>0) then
					if sgs.originalHegemonyHasShownSkills(friend, sgs.lose_equip_skill) then
						use.card = card
						if use.to then
	...
	target = nil
	for _, friend in ipairs(self.friends) do
		if self:needToThrowArmor(friend) or (sgs.originalHegemonyHasShownSkills(friend, sgs.lose_equip_skill)	and not friend:getEquips():isEmpty()) then
				target = friend
				break
		end
	end
...
sgs.ai_use_priority.HGanluCard = sgs.ai_use_priority.Dismantlement + 0.1
sgs.dynamic_value.control_card.HGanluCard = true

sgs.ai_card_intention.HGanluCard = function(self,card, from, to)
	local compare_func = function(a, b)
		return a:getEquips():length() < b:getEquips():length()
	end
...
sgs.ai_skill_invoke.heg_buyi = true
...
sgs.ai_skill_choice["docommand_buyi"] = function(self, choices, data)
  ...
  if index == 3 and is_enemy and (self.player:getHp() > (has_peach and 1 or 2)
      or self.player:isRemoved() or (self.player:hasSkill("heg_hongfa") and not self.player:getPile("heavenly_army"):isEmpty())) then
    return "yes"
  end
  if index == 4 then
    if self.player:getMark("command4_effect") > 0 then
...
--陆抗
sgs.ai_skill_invoke.heg_keshou = function(self, data)
...
sgs.ai_skill_cardask["@heg_keshou"] = function(self, data, pattern, target, target2)
	if self.player:getHandcardNum() < 2 then--缺手牌
    return "."
  end

  if self.player:hasSkill("heg_tianxiang")
  and not (self.player:getMark("heg_tianxiang_damage-turn") > 0 and self.player:getMark("heg_tianxiang_losehp-turn") > 0) then--配合小乔
...
sgs.ai_skill_choice.heg_zhuwei = function(self, choices, data)
...
sgs.ai_slash_prohibit.heg_zhuwei = sgs.ai_slash_prohibit.tiandu--考虑天香配合？

sgs.ai_skill_use["@@heg_zhengbigive!"] = function(self, prompt)
	local fields = prompt:split(":")
	local target = fields[2] and self.room:findPlayerByObjectName(fields[2])
	if not target or target:isDead() then return "." end
	local response = sgs.ai_skill_cardask["@heg_zhengbi-give"](self, nil, nil, target)
...
sgs.ai_skill_use["@@heg_keshou"] = function(self)
	local cards = sgs.QList2Table(self.player:getCards("he"))
	local eligible = {}
	for _, card in ipairs(cards) do
		if not self.player:isJilei(card) then table.insert(eligible, card) end
	end
	self:sortByKeepValue(eligible)
	for first = 1, #eligible do
		for second = first + 1, #eligible do
			if eligible[first]:sameColorWith(eligible[second]) then
				local response = sgs.ActiveSkillCard()
				response:setSkillName("heg_keshou")
				response:addSubcard(eligible[first]:getEffectiveId())
				response:addSubcard(eligible[second]:getEffectiveId())
...
sgs.ai_skill_exchange.heg_fudi= function(self,pattern,max_num,min_num,expand_pile)
...
function sgs.ai_slash_prohibit.heg_fudi(self, from, to)--杀禁止
  ...
  if from:hasSkills("heg_tieqi|heg_tieqi_xh|heg_yinbing") then return false end
...
sgs.ai_need_damaged.heg_fudi = function(self, attacker, player)--主动卖血
  ...
  if self.player:getPhase() ~= sgs.Player_NotActive then
	...
	if self:isEnemy(attacker) and attacker:getHp() >= (self.player:getHp() - 1) and self:isWeak(attacker) and self:damageIsEffective(attacker, nil, self.player)
		and not (attacker:hasShownSkill("heg_buqu")) and sgs.isGoodTarget(attacker, self:getEnemies(attacker), self) then
		return true
	end
	return false
end

function sgs.ai_cardneed.heg_fudi(to, card, self)
	return to:isKongcheng() and not self:needKongcheng(to)
end

sgs.ai_skill_invoke.heg_congjian = function(self, data)
  if self.player:getPhase() ~= sgs.Player_NotActive then
    return false
  end
...
--袁术
local weidi_skill = {}
weidi_skill.name = "heg_weidi"
table.insert(sgs.ai_skills, weidi_skill)
weidi_skill.getTurnUseCard = function(self)
	if not self.player:hasUsed("HWeidiCard") then
		return sgs.Card_Parse("@HWeidiCard=.&heg_weidi")
	end
	return nil
end

sgs.ai_skill_use_func["HWeidiCard"] = function(card, use, self)
  local target
	local targets = {}
	for _, p in sgs.qlist(self.room:getAlivePlayers()) do
...
sgs.ai_use_priority.HWeidiCard = 5
...
sgs.ai_skill_choice["docommand_weidi"] = function(self, choices, data)
  ...
  if index == 3 and not is_friend and (has_peach or self.player:isRemoved()
      or (self.player:hasSkill("heg_hongfa") and not self.player:getPile("heavenly_army"):isEmpty())) then
    return "yes"
  end
...
sgs.ai_skill_invoke.heg_yongsi = false--明牌负面效果

--君曹操
local huibian_skill = {}
huibian_skill.name = "heg_huibian"
table.insert(sgs.ai_skills, huibian_skill)
huibian_skill.getTurnUseCard = function(self)
	if self.player:hasUsed("HHuibianCard") then return end
	return sgs.Card_Parse("@HHuibianCard=.&heg_huibian")
end

sgs.ai_skill_use_func.HHuibianCard = function(card, use, self)
	--global_room:writeToConsole("使用挥鞭")
  local can_huibian = false
  local maixueskills = {"mobilefangzhu","heg_yiji","heg_wangxi","heg_bushi","heg_shicai","heg_zhiyu"}--不同卖血技能有优先顺序，是否可以用need_damage判断
  ...
  if heg_xunyu and heg_xunyu:getSeemingKingdom() == "wei" then
    local jieming_dnum= self:getJiemingDrawNum(self.player)
    if jieming_dnum >= 3 then
      table.insert(maixueskills, 1, "heg_jieming")--3牌以上放到首位
    elseif jieming_dnum == 2 then
      table.insert(maixueskills, 3, "heg_jieming")--2牌放到3位
    elseif jieming_dnum == 1 then
      table.insert(maixueskills, 5, "heg_jieming")--1牌放到5位
    end
  end
  local heg_huaxin = sgs.findPlayerByShownSkillName("heg_wanggui")
  if heg_huaxin and heg_huaxin:getSeemingKingdom() == "wei" and not heg_huaxin:hasFlag("WangguiUsed") then
    local wanggui_dnum = 0
    if heg_huaxin:hasShownAllGenerals() then
      ...
      if #self.enemies > 0 then
        for _, p in ipairs(self.enemies) do
          if self:isWeak(p) then
            table.insert(maixueskills, 1, "heg_wanggui")--虚弱打伤害放到首位
          end
        end
        table.insert(maixueskills, 5, "heg_wanggui")
      end
    elseif wanggui_dnum >= 3 then
      table.insert(maixueskills, 1, "heg_wanggui")
    elseif wanggui_dnum == 2 then
      table.insert(maixueskills, 3, "heg_wanggui")
  ...
  for _, p in ipairs(targets) do
    if self:isWeak(p) and p:canRecover() and not recover_target and sgs.originalHegemonyHasShownSkills(p, sgs.priority_skill) then--先回复重要队友
      recover_target = p
...
sgs.ai_use_priority.HHuibianCard = 5--优先度多少合适？

sgs.ai_skill_invoke.heg_zongyu = true
...
sgs.ai_skill_cardask["@heg_elitegeneralflag"] = function(self, data, pattern, target, target2)
  local jianan_skills = {"tenyeartuxi", "qiaobian", "xiaoguo", "heg_jieyue", "heg_duanliang"}
  for _, p in sgs.qlist(self.room:getAlivePlayers()) do
  ...
  if self.player:hasSkill("heg_jieyue") then--和五子良将纛同一时机触发的技能，设置优先发动jieyue
    table.insert(jianan_skills ,"heg_jieyue")
  end
  ...
  if (self.player:hasSkill(choice) and choice ~= "heg_jieyue") or choice == "xiaoguo" then--骁果
    return "."
  end
  if self.player:isLord() and self.player:inHeadSkills("heg_sidi") and not self.player:getPile("drive"):isEmpty() then
    return "."
  end
  if choice == "qiaobian" then
    return discard:toString()
  end
  if self.player:hasSkills("qiaobian|heg_qiaobian_egf") and choice == "tenyeartuxi" then
    return "."
  end
  if not isCard("Peach", discard, self.player) then
    local g1name = sgs.originalHegemonyGeneral(self.player, true):objectName()
    local g2name = sgs.originalHegemonyGeneral(self.player, false):objectName()
    if shouldUseJiananByValue(self, g1name) or shouldUseJiananByValue(self, g2name)
    or sgs.originalHegemonyDuanchang(self.player, true) or sgs.originalHegemonyDuanchang(self.player, false) then
...
sgs.ai_skill_choice.jianan_hide = function(self, choices)
  ...
  if self.player:hasSkill("heg_jieyue") then
    if self.player:inHeadSkills("heg_jieyue") then
      return "head"
    else
      return "deputy"
    end
  ...
  if sgs.originalHegemonyDuanchang(self.player, true) then
    v1 = 0
  end
  if sgs.originalHegemonyDuanchang(self.player, false) then
    v2 = 0
  end
  if self.player:hasShownSkill("heg_luoshen") then--甄姬值9-4
    if self.player:inHeadSkills("heg_luoshen") and v2 > 5 then
      v1 = v1 - 4
    elseif self.player:inDeputySkills("heg_luoshen") and v1 > 5 then
      v2 = v2 - 4
    end
	end
  if self.player:hasShownSkill("heg_tuntian") then
    if self.player:inHeadSkills("heg_tuntian") then
      v1 = v1 + self.player:getPile("field"):length()
    else
      v2 = v2 + self.player:getPile("field"):length() * 0.5
    end
	end
  if self.player:hasSkill("heg_sidi") then
    if self.player:inHeadSkills("heg_sidi") then
...
sgs.ai_skill_choice.jianan_skill = function(self, skills)
    ...
    table.removeOne(skills, "tenyeartuxi")
  end
  if table.contains(skills, "tenyeartuxi") and self:findTuxiTarget() then--没牌时
    if self.player:isKongcheng() then
      return "tenyeartuxi"
    end
  ...
  if table.contains(skills, "heg_jieyue") then--能发动节钺时
    if sgs.ai_skill_use["@@heg_jieyue"](self) ~= "." and not self:willSkipDrawPhase() then
      return "heg_jieyue"
    elseif #skills > 1 then
      table.removeOne(skills, "heg_jieyue")
    end
	end
  if table.contains(skills, "heg_duanliang") then
    ...
    if duanliang_count > ((kongcheng_enemy > 0 or needcards_enemy > 1) and 0 or 1) then--数量多少合适？
      return "heg_duanliang"
    end
    if duanliang_count == 0 and #skills > 1 then--预选时和选择时手牌不同怎么处理
      table.removeOne(skills, "heg_duanliang")
    end
	end
  if table.contains(skills, "tenyeartuxi") and self:findTuxiTarget() then
    return "tenyeartuxi"
  end
...
function sgs.ai_cardneed.heg_elitegeneralflag(to, card)
	return to:isNude()
end

--突袭
sgs.ai_skill_playerchosen.heg_tuxi_egf = sgs.ai_skill_playerchosen.tenyeartuxi

--巧变
sgs.ai_skill_discard.heg_qiaobian_egf = sgs.ai_skill_discard.qiaobian

--骁果相同

--节钺
sgs.ai_skill_use["@@heg_jieyue_egf"] = sgs.ai_skill_use["@@heg_jieyue"]

--断粮
local duanliang_egf_skill = {}
duanliang_egf_skill.name = "heg_duanliang_egf"
...
duanliang_egf_skill.getTurnUseCard = function(self)
  ...
  if self.player:hasFlag("DuanliangEGFCannot") then
	...
	local number = card:getNumberString()
	local card_id = card:getEffectiveId()
	local card_str = ("supply_shortage:heg_duanliang_egf[%s:%s]=%d%s"):format(suit, number, card_id, "&heg_duanliang_egf")
...
sgs.ai_suit_priority.heg_duanliang_egf= "club|spade|diamond|heart"

--六龙骖驾
sgs.ai_use_priority.HSixDragons = 2.70
...
sgs.ai_skill_choice.startcommand_to = function(self, choices, data)--含目标的通用选择军令
  ...
  if table.contains(choices, "command3") and (target:isRemoved() or (target:hasSkill("heg_hongfa") and not target:getPile("heavenly_army"):isEmpty())) then
