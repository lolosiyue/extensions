...
sgs.ai_skill_use["@@shensheng"]=function(self,prompt)
	...
	elseif card[2]:endsWith("shoot") then
		...
		local dummy_use = self:aiUseCard(sgs.Sanguosha:getCard(card[5]), dummy())
...
local gaoda_zhongjie_skill = {}
gaoda_zhongjie_skill.name = "gaoda_zhongjie"
table.insert(sgs.ai_skills, gaoda_zhongjie_skill)
gaoda_zhongjie_skill.getTurnUseCard = function(self, inclusive)
	...
	if self.player:getMark("@gaoda_zhongjie") > 0 and ((self.player:getHp() <= 1 and self.room:alivePlayerCount() > 2 and not self.player:isLord()) or
		(self.player:getHp() <= 2 and self:isWeak() and #self.enemies == 1 and self.enemies[1]:getHp() == 1)) then
		return sgs.Card_Parse("#gaoda_zhongjie:.:")
	end
end

sgs.ai_skill_use_func["#gaoda_zhongjie"] = function(card, use, self)
	self:sort(self.enemies, "hp")
	use.card = card
	if use.to then
...
sgs.ai_use_value["gaoda_zhongjie"] = sgs.ai_use_value.Slash
sgs.ai_use_priority["gaoda_zhongjie"] = 0.01
...
sgs.ai_skill_invoke.gaoda_chihun = function(self, data)
	return true
end

sgs.ai_can_damagehp.gaoda_chihun = function(self,from,card,to)
	return to:getHp()+self:getAllPeachNum()-self:ajustDamage(from,to,1,card)>0
	and self:canLoseHp(from,card,to) and to:getMark("@gaoda_chihun") == 0
end
...
jvjian_skill.getTurnUseCard = function(self, inclusive)
	if self.player:getMark("@gaoda_chihun") > 0 and self.player:getMark("@jvjian") > 0 and #self.enemies > 0 then
...
tuci_skill.getTurnUseCard = function(self, inclusive)
	if self.player:getMark("@gaoda_chihun") > 0 and self.player:getMark("@tuci") > 0 and #self.enemies > 0 then
		local x = 1
...
sgs.ai_skill_cardask["@huimie"] = function(self,data,pattern)
	local use = data:toCardUse()
	if sgs.ai_skill_use["@@huimie"](self, "#huimie:"..use.card:objectName()) == "." then return "." end
...
sgs.ai_skill_choice.zhonggong = function(self, choices, data)
	...
	for _,installed in sgs.qlist(self.player:getEquips()) do
		for _,equip in ipairs(equips) do
			...
			-- 	return equip
			-- end
			if equip ~= "@Equip"..installed:getRealCard():toEquipCard():location().."lose" then
				return equip
			end
		end
	end
...
sgs.ai_target_revises.qingzhuang = function(to,card)
	if card:isKindOf("Slash") and card:isRed() and not to:hasEquipArea()
...
sgs.ai_skill_invoke.duilie = function(self, data)
	...
	local enemy = self.room:findPlayerByObjectName(promptlist[2])
...
sgs.ai_target_revises.duilie = function(to,card)
	if to:getMark("@duilieB") > 0 and math.mod(card:getNumber(),2) == 0 and card:getNumber() > 0 
	then return true end
end
...
sgs.ai_target_revises["@renwang_shield"] = function(to,card,self)
	if card:isBlack() and card:isKindOf("Slash") and not card:hasFlag("qinggang")
	then return true end
end
...
sgs.ai_skill_invoke.gaoda_shenshi = function(self, data)
	local use = data:toCardUse()
	if use.from:objectName() == self.player:objectName() then
		...
		for _,p in sgs.qlist(use.to) do
			...
			if self:hasHeavyDamage(self.player, use.card, p) then 
				return false
			end
		...
		if self:hasTrickEffective(card, use.from, self.player) and (self:getCardsNum("Slash") > 0 or self:getCardsNum("Jink") == 0
			or self:getCardsNum("Nullification") > 0 or self:hasHeavyDamage(use.from, use.card, self.player)) then
			return true
		end
	end
end
...
sgs.ai_ajustdamage_to["@xuanguang"]   = function(self, from, to, card, nature)
	if nature ~= "N" and to:getEquips():isEmpty() then
		return -99
	end
end
...
sgs.ai_skill_invoke.gaoda_zuzhou = function(self, data)
	local use = data:toCardUse()
	if self:getSuitNum("black", false, self.player) > 0 and self:isEnemy(use.from) then
		self:sort(self.enemies, "defenseSlash")
		for _,enemy in ipairs(self.enemies) do
...
sgs.ai_skill_use["@@gaoda_zuzhou"] = function(self, prompt)
	self:updatePlayers()
	self:sort(self.enemies, "defenseSlash")
	local targets = {}
	local slash = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuitBlack, 0)
	slash:setSkillName("gaoda_zuzhou")
	for _, enemy in ipairs(self.enemies) do
	...
	if #targets > 0 then
		...
		for _,card in ipairs(cards) do
			if card:isBlack() and not (self.player:getHp() == 1 and card:isKindOf("Analeptic")) then
				return ("slash:gaoda_zuzhou[%s:%s]=%d->%s"):format(card:getSuitString(), card:getNumberString(), card:getEffectiveId(), table.concat(targets, "+"))
			end
		end
	else
		return "."
...
qiji_skill.getTurnUseCard=function(self)
	if self.player:getChangeSkillState("qiji") ~= 2 then return false end
...
sgs.ai_view_as.qiji = function(card, player, card_place)
	...
	if player:getHandcardNum() == 1 and player:getChangeSkillState("qiji") == 2 then
...
sgs.need_kongcheng = sgs.need_kongcheng .. "|fangcheng"
...
--海盗X1
sgs.ai_use_revises.haidao = function(self,card,use)
	if card:isKindOf("EquipCard") and self:evaluateArmor(card)>-5
	then use.card = card return true end
end
sgs.ai_cardneed.haidao = sgs.ai_cardneed.equip
...
sgs.ai_can_damagehp.chaojimoshi = function(self,from,card,to)
	return to:getHp()+self:getAllPeachNum()-self:ajustDamage(from,to,1,card)>0
	and self:canLoseHp(from,card,to) and to:getMark("@supermode") == 0 and to:getMark("supermode_damage") < 3
end
...
sgs.ai_can_damagehp.anzhang = function(self,from,card,to)
	if from and to:getHp()+self:getAllPeachNum()-self:ajustDamage(from,to,1,card)>0
	and self:canLoseHp(from,card,to)
	then
		return self:isEnemy(from) and sgs.ai_skill_cardask["@anzhang"](self) ~= "."
	end
end
...
sgs.ai_can_damagehp.mosu = function(self,from,card,to)
	if from and to:getHp()+self:getAllPeachNum()-self:ajustDamage(from,to,1,card)>0
	and self:canLoseHp(from,card,to)
	then
		return self:isEnemy(from) and from:getPhase() ~= sgs.Player_NotActive
	end
end
...
shuangpao_skill.getTurnUseCard=function(self,inclusive)
	...
	local willUse = false
	for _,c in sgs.list(cards)do
		if c:isKindOf("Slash") and self:willUse(self.player,c) then
			willUse = true
			break
		end
	end
...
sgs.ai_ajustdamage_from.shuangpao = function(self, from, to, card, nature)
	if card and card:hasFlag("shuangpao") then
		return 1
	end
end
...
sgs.ai_skill_invoke.gaoda_ansha = function(self, data)
...
sgs.ai_skill_cardask["@@shuanglianjink"] = function(self, data, pattern)
	local cards = self.player:getCards("he")
	cards = sgs.QList2Table(cards)
	self:sortByKeepValue(cards)
...
sgs.ai_skill_cardask["@@shuanglianjinkres"] = function(self, data, pattern)
	local target = data:toCardUse().from
	if target and self:needToLoseHp(self.player, target) then return "." end
	local cards = self.player:getCards("he")
	cards = sgs.QList2Table(cards)
	self:sortByKeepValue(cards)
	for _,h in sgs.list(cards)do
...
sgs.ai_playerchosen_intention.shuanglian = function(self, from, to)
	if not self:needToLoseHp(to, from) then
...
shuanglong_skill.getTurnUseCard=function(self,inclusive)
	-- if self.player:getHandcardNum() < 2 or self.player:hasFlag("shuanglong_success") or self.player:hasFlag("shuanglong_failed") then return false end
	 if self.player:getHandcardNum() < 2 or self.player:hasUsed("#shuanglong") then return false end
...
sgs.ai_cardneed.shuanglong = sgs.ai_cardneed.bignumber
...
sgs.ai_ajustdamage_from.weibo = function(self,from,to,card,nature)
	if card and card:isKindOf("Slash") and from:getMark("weibo") > 0
	then return from:getMark("weibo") end
end
...
sgs.ai_skill_invoke.gaoda_difa = function(self, data)
	return true
end

sgs.ai_skill_use["@@gaoda_difa"] = function(self, prompt, method)
	local judge = self.player:getTag("gaoda_difa"):toJudge()
	local ids = self.player:getPile("gaoda_difa")
	if self.room:getMode():find("_mini_46") and not judge:isGood() then return "#gaoda_difa:"..ids:first()..":" end
...
sgs.ai_ajustdamage_from.huanzhuang = function(self,from,to,card,nature)
	if card and card:isKindOf("Slash") and from:getMark("huanzhuangb") > 0
	then return 1 end
end
...
local gaoda_qishe_skill = {}
gaoda_qishe_skill.name = "gaoda_qishe"
table.insert(sgs.ai_skills, gaoda_qishe_skill)
gaoda_qishe_skill.getTurnUseCard = function(self, inclusive)
	if self.player:isKongcheng() then return false end
	if (#self.enemies > 1 or (#self.enemies == 1 and (self:getCardsNum("Slash") == 0 or self.player:distanceTo(self.enemies[1]) > self.player:getAttackRange()))) and
		...
		local card_str = ("fire_slash:gaoda_qishe[%s:%s]=%s"):format("to_be_decided", 0, table.concat(ids, "+"))
		local card = sgs.Card_Parse(card_str)
		return card
	end
end

sgs.ai_use_value["gaoda_qishe"] = sgs.ai_use_value.FireSlash
sgs.ai_use_priority["gaoda_qishe"] = sgs.ai_use_priority.Slash - 0.5
...
wenshen_skill.getTurnUseCard = function(self)
	...
	if (self:getCardsNum("EquipCard") < 2 and self:getCardsNum("Slash") == 0) or not sgs.Analeptic_IsAvailable(self.player) then
		...
		if sgs.Analeptic_IsAvailable(self.player, analeptic) and willUse then
			assert(analeptic)
			return analeptic
		end
	end
...
sgs.ai_card_priority.liesha = function(self,card)
	if card:isKindOf("Slash") and card:isBlack()
	then if self.useValue
		then return 1 end end
end
function sgs.ai_slash_prohibit.jinduan(self,from,to,card)
	if self:isFriend(to,from) then return false end
	if from:hasFlag("NosJiefanUsed") then return false end
	if card and not card:isRed() then return false end
	for _,friend in ipairs(self:getFriends(from,true))do
		if self:slashIsEffective(card,friend,from) then return true end
	end
end
...
sgs.ai_ajustdamage_from.quanyu = function(self, from, to, card, nature)
	if card and card:isKindOf("Slash") and card:hasFlag("quanyu_S1: " .. to:objectName()) then
		return 1
	end
end
...
sgs.ai_can_damagehp.shenyuan = function(self,from,card,to)
	return to:getHp()+self:getAllPeachNum()-self:ajustDamage(from,to,1,card)>0
	and self:canLoseHp(from,card,to) and card and card:isRed() and not to:isKongcheng() and to:hasSkill("hundun") and sgs.ai_skill_use["@@hundun"] ~= "."
end
...
sgs.ai_skill_choice.daohe = function(self, choices, data)
	...
	if table.contains(choices, "jiying") then
		...
		local willUse = false
		for _,c in sgs.list(cards)do
			if c:isKindOf("Analeptic") and self:willUse(self.player,c) then
				n = 1
			end
		end
		for _,c in sgs.list(cards)do
			if c:isKindOf("Slash") and self:willUse(self.player,c) then
				if self.player:canSlash(enemy, slash) and self:slashIsEffective(slash, enemy) and
	...
	if table.contains(choices, "meiying") then
		...
		local willUse = false
		for _,c in sgs.list(cards)do
			if c:isKindOf("Slash") and self:willUse(self.player,c) and self:getCardsNum("Slash") > 1 then
				return "meiying"
			end
		end
...
sgs.ai_can_damagehp.emeng = function(self,from,card,to)
	return to:getHp()+self:getAllPeachNum()-self:ajustDamage(from,to,1,card)>0
	and self:canLoseHp(from,card,to) and to:hasSkill("daohe") and from and (not to:inMyAttackRange(from)) and card and card:isKindOf("Slash")
end

--救世主
sgs.ai_skill_invoke.gaoda_shanzhuan = function(self, data)
	return true
end

sgs.ai_skill_use["@@gaoda_shanzhuan"] = function(self, prompt)
	self:updatePlayers()
	self:sort(self.enemies, "defenseSlash")
...
sgs.ai_target_revises.bachi = function(to, card, self)
	if (card:isRed() and (card:isKindOf("Slash")) or card:objectName():endsWith("shoot"))
	then
		return to:canDiscard(to, "he")
	end
end
...
sgs.ai_skill_invoke.hanwei = function(self, data)
	...
	local enemy = self.room:findPlayerByObjectName(name)
	return enemy and self:isEnemy(enemy)
end
sgs.ai_cardneed.hanwei = sgs.ai_cardneed.equip
...
sgs.ai_cardneed.shijiu = sgs.ai_cardneed.equip

--命运
sgs.ai_ajustdamage_from.feiniao = function(self,from,to,card,nature)
	if card and card:objectName():endsWith("shoot") and from:getAttackRange() >= 4
	then return 1 end
end
sgs.ai_use_revises.feiniao = function(self,card,use)
	if card:isKindOf("Slash") and self.player:getAttackRange() == 2 then
...
sgs.ai_ajustdamage_from.nuhuo = function(self,from,to,card,nature)
	if from:getMark("nuhuo_buff") > 0 
	then return 1 end
end
...
--红异端
local gaoda_jianhun_skill = {}
gaoda_jianhun_skill.name = "gaoda_jianhun"
table.insert(sgs.ai_skills, gaoda_jianhun_skill)
gaoda_jianhun_skill.getTurnUseCard = function(self, inclusive)
	if self.player:hasUsed("gaoda_jianhun") or self.player:getPile("hun"):length() >= 3 then return false end
	local cards = self.player:getCards("he")
	cards = sgs.QList2Table(cards)
	self:sortByUseValue(cards, true)
	for _,card in ipairs(cards) do
		if card:isRed() and not card:isKindOf("ExNihilo") and not (self.player:isWounded() and card:isKindOf("Peach"))
			...
			local number = card:getNumberString()
			local card_id = card:getEffectiveId()
			local card_str = ("ex_nihilo:gaoda_jianhun[%s:%s]=%d"):format(suit, number, card_id) --AI不肯热血地喝酒，要用无中生有来骗他！
			local analeptic = sgs.Card_Parse(card_str)
...
sgs.ai_use_value["gaoda_jianhun"] = sgs.ai_use_value.Analeptic + 0.1
sgs.ai_use_priority["gaoda_jianhun"] = sgs.ai_use_priority.Analeptic + 0.1

sgs.ai_skill_invoke.gaoda_jianhun = function(self, data)
	local target = data:toPlayer()
	return self:isEnemy(target)
end

sgs.ai_use_revises.gaoda_jianhun = function(self,card,use)
	if card:isKindOf("Slash") and card:isBlack() and self.player:hasFlag("gaoda_jianhun-diamond") then
		card:setFlags("Qinggang")
	end
end
...
sgs.gaoda_jianhun_suit_value = {
	heart = 3.9,
	diamond = 3.9
}

function sgs.ai_cardneed.gaoda_jianhun(to, card)
	return card:isRed()
end
		...
		if card:objectName() == "slash" and not card:hasFlag("using") then
			...
			for _,enemy in ipairs(self.enemies) do
				if self.player:canSlash(enemy, jianmieslash) and self:slashIsEffective(jianmieslash, enemy) then
					if #targets == 1 then
						if enemy:isChained() or self.room:findPlayerByObjectName(targets[1]):isChained() then
							table.insert(targets, enemy:objectName())
						end
					else
...
sgs.ai_skill_use_func["#jianmie"] = function(card, use, self)
	...
	if use.to then
		local targets = {}
		self:sort(self.enemies, "defenseSlash")
		local jianmieslash = dummyCard("fire_slash")
		for _,enemy in ipairs(self.enemies) do
			if self.player:canSlash(enemy, jianmieslash) and self:slashIsEffective(jianmieslash, enemy) then
				if #targets == 1 then
					if enemy:isChained() or self.room:findPlayerByObjectName(targets[1]):isChained() then
...
sgs.ai_use_revises.yuanjian = function(self,card,use)
	if card:isKindOf("Slash") and (card:getSuit() == sgs.Card_Spade or (self.player:getMark("exia_transammark") > 0 and card:isBlack())) then
		card:setFlags("Qinggang")
	end
end
...
exia_transam_skill.getTurnUseCard = function(self, inclusive)
	...
	if self.player:getMark("@exia_transam") > 0 and willUse then
...
sgs.ai_card_priority.jvji = function(self,card,v)
	if card:isKindOf("Shoot")
	then return 6 end
end
...
sgs.ai_skill_use["@@chaobing"] = function(self, prompt)
	...
	local to = self.room:findPlayerByObjectName(toObjName)
...
sgs.ai_card_priority.hongjue = function(self,card,v)
	if card:isKindOf("PierceShoot")
	then return 6 end
end
sgs.ai_ajustdamage_from.hongjue = function(self,from,to,card,nature)
	if card and card:objectName():endsWith("shoot") and card:getTag("drank"):toInt() > 0
...
virtue_transam_skill.getTurnUseCard = function(self, inclusive)
	if self.player:getMark("@virtue_transam") > 0 then
		...
		if use.card then
			local shoots = {"Shoot", "PierceShoot", "SpreadShoot"}
			local n = self:getCardsNum(shoots)
			if self.player:hasSkill("hongjue") then
				n = n + self:getCardsNum("Slash")
			end
			self:sort(self.enemies, "hp")
			if (n >= 2 and #self.enemies > 0 and not self.enemies[1]:isProhibited(self.enemies[1], card) and not self.enemies[1]:hasSkill("mosu")) or self.player:getHp() == 1 then
...
--纳德雷
sgs.ai_skill_invoke.gaoda_shenpan = function(self, data)
	return true
end

sgs.ai_skill_use["@@gaoda_shenpan"] = function(self, prompt)
	local cards = self.player:getCards("h")
	cards = sgs.QList2Table(cards)
	self:sortByKeepValue(cards)
	for _, card in ipairs(cards) do
		if card:isKindOf("BasicCard") and (not isCard("Peach", card, self.player)) then
			...
			acard:setSkillName("gaoda_shenpan")
...
function sgs.ai_cardneed.gaoda_shenpan(to, card)
...
local gaoda_liuyan_skill = {}
gaoda_liuyan_skill.name = "gaoda_liuyan"
table.insert(sgs.ai_skills, gaoda_liuyan_skill)
gaoda_liuyan_skill.getTurnUseCard = function(self, inclusive)
	...
	for _,id in sgs.qlist(self.player:getPile("jian")) do
		...
		if (suit_no == spade and suit == "spade") or (suit_no == heart and suit == "heart") or (suit_no == club and suit == "club")
			...
			if peach:isAvailable(self.player) then
				local peach_str = ("peach:gaoda_liuyan[%s:%s]=%d"):format(suit, number, card_id)
...
sgs.ai_view_as.gaoda_liuyan = function(card, player, card_place)
	...
	if (suit_no == spade and suit == "spade") or (suit_no == heart and suit == "heart") or (suit_no == club and suit == "club")
		...
		if card_place == sgs.Player_PlaceSpecial and player:getPileName(card_id) == "jian" and player:getMark("Global_PreventPeach") == 0 then
			return ("peach:gaoda_liuyan[%s:%s]=%d"):format(suit, number, card_id)
		end
	end
end

sgs.ai_use_value["gaoda_liuyan"] = sgs.ai_use_value.Peach
sgs.ai_use_priority["gaoda_liuyan"] = sgs.ai_use_priority.Peach
...
harute_transam_skill.getTurnUseCard = function(self, inclusive)
	...
	if self.player:getMark("@harute_transam") > 0 and willUse then
...
-- sgs.ai_card_priority.ronghe = function(self,card)
-- 	if self.player:getPile("&ronghe"):contains(card:getEffectiveId())
-- 	then
-- 		return 5 
-- 	end
-- end
...
sgs.ai_skill_invoke.quanren = function(self, data)
	...
	if self.player:hasSkill("fs_transam") and self.player:getMark("@fs_transam") > 0 then
		return (willUse and self:getCardsNum("Slash") >= 2) or (self:getCardsNum("Slash") == 0 and want_attack)
	end
...
fs_transam_skill.getTurnUseCard = function(self, inclusive)
	...
	if self.player:getMark("@fs_transam") > 0 and willUse and slash_count >= 2 then
...
shineng_skill.getTurnUseCard = function(self, inclusive)
	...
	if self.player:getMark("@shineng") > 0 and self.player:getPile("neng"):length() > 1 and (willUse or self.player:hasUsed("Slash") or self.player:getMaxCards() <= 1) then
...
sgs.ai_ajustdamage_from.tiequan = function(self, from, to, card, nature)
...
dm_transam_skill.getTurnUseCard = function(self, inclusive)
	...
	if self.player:getMark("@dm_transam") > 0 and equips:length() > 0 and installed
		and ((willUse and (not unlimited) and self:getCardsNum("Slash") > (2 - math.floor(equips:length() / 2)))
...
sgs.ai_ajustdamage_to.gaoda_mingren = function(self, from, to, card, nature)
	if card and card:isKindOf("Duel")
	then
		return -99
	end
end
sgs.ai_ajustdamage_from.gaoda_binghuo = function(self, from, to, card, nature)
	if card and card:isKindOf("FireSlash") and to:getMark("@ice") > 0 then
		return 1
	end
end
...
sgs.ai_skill_use["@@ciyuanbawangliu"] = function(self, prompt)
	...
	for _,p in sgs.qlist(use.to) do
		for _,id in sgs.qlist(quanfa) do
			...
			if self.player:getMark("@gaoda_tonghua") > 0 and qcard:isRed() then
				name = "fire_slash"
			end
			...
			if self.player:getMark("@gaoda_tonghua") == 0 then
...
sgs.ai_ajustdamage_from.huanse = function(self, from, to, card, nature)
	if from:getMark("G_SELF_ASS") > 0 and card and card:isRed() and not card:isKindOf("SkillCard") then
		return 1
	end
end
...
sgs.ai_ajustdamage_from.huancai = function(self, from, to, card, nature)
	if from:getMark("G_SELF_ASS") > 0 and card and card:isRed() and not card:isKindOf("SkillCard") then
		return 1
	end
end
...
sgs.ai_ajustdamage_from.tiexue = function(self, from, to, card, nature)
	if card and (card:isKindOf("Slash") or card:isKindOf("Duel")) and from:getMark("tiexue") > 0 then
...
sgs.ai_skill_cardask["@@tianlang"] = function(self, data, pattern)
    local use = data:toCardUse()
	for _, p in sgs.qlist(use.to) do
...
sgs.ai_skill_invoke.diwang = function(self, data)
	...
	if x and x:startsWith("draw") then
		...
		if x < 2 then
			...
			return willUse and not self:isWeak(self.player)
		else
			return true
		end
	else
...
sgs.ai_ajustdamage_from.diwang = function(self, from, to, card, nature)
	if from:getHandcardNum() > to:getHandcardNum() and card and (card:isKindOf("Slash") or card:isKindOf("Duel")) then
		return 1
	end
end
...
local gaoda_zhufu_skill={}
gaoda_zhufu_skill.name = "gaoda_zhufu"
table.insert(sgs.ai_skills,gaoda_zhufu_skill)
gaoda_zhufu_skill.getTurnUseCard=function(self)
	if self.player:getHandcardNum() < 2 then return nil end
	if self.player:hasUsed("#gaoda_zhufu") then return nil end
	...
	local card_str = ("#gaoda_zhufu:%d+%d:"):format(first, second)
	assert(card_str)
	return sgs.Card_Parse(card_str)
end

sgs.ai_skill_use_func["#gaoda_zhufu"] = function(card, use, self)
...
sgs.ai_use_priority["gaoda_zhufu"] = sgs.ai_use_priority.JieyinCard

sgs.ai_card_intention["gaoda_zhufu"] = function(self, card, from, tos)
	if not from:hasFlag("jieyin_isenemy_"..tos[1]:objectName()) then
		sgs.updateIntention(from, tos[1], -80)
	end
end

sgs.dynamic_value.benefit["gaoda_zhufu"] = true
...
--焰龙号
local gaoda_huiyun_skill={}
gaoda_huiyun_skill.name="gaoda_huiyun"
table.insert(sgs.ai_skills,gaoda_huiyun_skill)
gaoda_huiyun_skill.getTurnUseCard=function(self)
	if self.player:getMark("gaoda_huiyun") == 1 then return false end
	...
	local card_str = ("slash:gaoda_huiyun[%s:%s]=%d"):format(suit, number, card_id)
	local slash = sgs.Card_Parse(card_str)
	assert(slash)
...
sgs.ai_view_as.gaoda_huiyun = function(card, player, card_place)
	local suit = card:getSuitString()
	local number = card:getNumberString()
	local card_id = card:getEffectiveId()
	if card_place == sgs.Player_PlaceHand and player:getMark("gaoda_huiyun") ~= 1 then
		if card:isKindOf("Jink") then
			return ("slash:gaoda_huiyun[%s:%s]=%d"):format(suit, number, card_id)
		elseif card:isKindOf("Slash") then
			return ("jink:gaoda_huiyun[%s:%s]=%d"):format(suit, number, card_id)
		end
	end
end

sgs.ai_skill_cardask["@gaoda_huiyun"] = function(self, data)
	if self.player:getMark("gaoda_huiyun") == 1 then
