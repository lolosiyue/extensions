...
sgs.ai_choicemade_filter.cardUsed.NosLijianCard = function(self,player,carduse)
...
sgs.ai_skill_cardask["@multi-jink-start"] = function(self,data,pattern,target,target2,arg)
	...
	if self.player:hasSkill("kongcheng") then
		...
		--add
		if target:hasSkill("sfofl_wushuang") and not data:toCardEffect().card:hasFlag("sfofl_wushuang"..self.player:objectName()) then
			return sgs.ai_skill_cardask["slash-jink"](self,data,pattern,target)
		end
		--add
		if target:hasSkill("ahmouwushuang") and not data:toCardEffect().card:hasFlag("ahmouwushuang"..self.player:objectName()) then
			return sgs.ai_skill_cardask["slash-jink"](self,data,pattern,target)
		end
...
sgs.ai_choicemade_filter.cardUsed.LijianCard = function(self,player,carduse)
	sgs.ai_lijian_effect = true
	end
