...
sgs.ai_need_damaged.vsganglie = function(self,attacker,player)
	for _,enemy in sgs.list(self.enemies)do
		...
		and not (enemy:hasSkills(sgs.need_kongcheng) and not hasBuquEffect(enemy) and enemy:getHandcardNum()>1) then
			return true
		end
	end
	return false
...
sgs.ai_card_intention.ZhenweiCard = function(self,card,from,tos)
	if #self:getFriends(from,true)>0 then
