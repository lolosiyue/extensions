...
sgs.ai_skill_use_func.s4_jiyang = function(card, use, self, request)
	local max_card, max_point = self:getPindianMaxCard(self.player,
        { ai = self, reason = "s4_jiyang", is_from = true })
    if not max_card then return end
    ...
    for _, enemy in sgs.list(dummy_use.to) do
        if self.player:canPindian(enemy) then
            ...
            local enemy_point = self:getPindianMaxPoint(enemy,
                { opponent = self.player, ai = self, reason = "s4_jiyang",
                  is_from = false })
...
sgs.ai_use_priority.s4_jiyang = sgs.ai_use_priority.Dual
...
sgs.ai_skill_use_func.s4_zhiba = function(card, use, self, request)
	local max_card, max_point = self:getPindianMaxCard(self.player,
        { ai = self, reason = "s4_zhiba", is_from = true })
    ...
    for _, enemy in ipairs(self.enemies) do
        if enemy:hasLordSkillKingdom("wu", self.player)
			...
			local enemy_max = self:getMaxCard(enemy)
			local enemy_point = self:getPindianMaxPoint(enemy,
                { opponent = self.player, ai = self, reason = "s4_zhiba",
                  is_from = false })
