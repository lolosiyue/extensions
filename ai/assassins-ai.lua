...
sgs.ai_skill_use_func.MizhaoCard = function(card,use,self)
	...
	if not target then
		...
		for _,friend in sgs.list(self.friends_noself)do
			if hasTuntianEffect(friend, true) and not friend:hasSkill("manjuan") and not self:isWeak(friend)
	...
	if from then
		...
		for _,to in sgs.list(self.enemies)do
			if targets:contains(to) and self:slashIsEffective(slash,to,from) and not self:needToLoseHp(to,from,slash)
				and not self:needToLoseHp(to,from,slash) and not self:findLeijiTarget(to,50,from) then
				return to
			end
		end
	end
