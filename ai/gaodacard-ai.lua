...
function SmartAI:useCardShoot(card, use)
	...
	if use.to then
		...
		for _,enemy in ipairs(self.enemies) do
			...
			if self:ajustDamage(self.player, enemy, 1, card) < 1 then continue end
			use.to:append(enemy)
...
sgs.ai_skill_cardask["@Guard"] = function(self, data, pattern)
	...
	--一般情况
	return self:askForCard(pattern, "@Guard-AI", data, sgs.Card_MethodUse, data:toDamage().from)
end

sgs.ai_skill_cardask["@Guard-AI"] = function(self, data, pattern)
...
function SmartAI:useCardTacticalCombo(card, use)
	...
	if use.to then
		for _,friend in ipairs(self.friends) do
			...
			if use.to:length() >= 2 + sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_ExtraTarget, self.player, card) then return end
		end
		--如果没有有效目标，则不空发射击
		if use.to:isEmpty() then
			use.card = nil
