...
sgs.ai_choicemade_filter.skillInvoke.chongzhen = function(self,player,promptlist)
	if self.chongzhenTarget then
		local intention = 60
		if promptlist[3]=="yes" then
			if not self:hasLoseHandcardEffective(self.chongzhenTarget)
			or (self:needKongcheng(self.chongzhenTarget) and self.chongzhenTarget:getHandcardNum()==1)
			then intention = 0 end
			if self:getOverflow(self.chongzhenTarget)>2 then intention = 0 end
			sgs.updateIntention(player,self.chongzhenTarget,intention)
		else
			if self:needKongcheng(self.chongzhenTarget) and self.chongzhenTarget:getHandcardNum()==1 then intention = 0 end
			sgs.updateIntention(player,self.chongzhenTarget,-intention)
		end
	end
end
...
local function will_discard_zhaolie(self,nobasic)
	...
	if nobasic>0 and not spliubei:hasSkill("jueqing") then
		...
		if self.player:hasSkill("heg_mingshi") and not spliubei:hasShownAllGenerals() and damage_num>0 then
...
sgs.ai_skill_discard.zhaolie = function(self,discard_num,min_num,optional,include_equip)
	...
	for i = #cards,1,-1 do
		if not self.player:isJilei(cards[i]) then
...
yinling_skill.getTurnUseCard = function(self,inclusive)
	...
	if black_card then
		return sgs.Card_Parse("@YinlingCard="..black_card:getEffectiveId())
	end
end
...
sgs.ai_need_damaged.fenyong = function (self,attacker,player)
	...
	for _,enemy in sgs.list(self.enemies)do
		if self.player:canSlash(enemy,nil,false) and not self:slashProhibit(nil,enemy)
		and self:slashIsEffective(dummyCard(),enemy)
		then return true end
	end
	return false
end
...
sgs.ai_skill_choice.xuehen = function(self,choices)
	...
	if self:isEnemy(current) then
		...
		if self:hasSkills("jijiu|beige",current) or hasTuntianEffect(current, true) and n>=2 and current:getCardCount(true)>=2 then return "discard" end
	end
	self:sort(self.enemies,"defenseSlash")
	for _,enemy in sgs.list(self.enemies)do
		local def = self:getDefenseSlash(enemy)
